#!/usr/bin/env node
/* 世界-122 初始视野可见能源节点核实（2026-08-16）：
 * - 进 scene8 → 读玩家位置/相机 worldView/zoom
 * - 统计：总节点数 / 初始视野内节点数 / 最近节点距离
 * - 截图供人工核对
 * 用法：node tools/cdp-scene8-view.mjs（需 vite dev server 在 5173）
 */
...
console.log('初始视野:', await ev(`(async () => {
  ...
  const inView = nodes.filter(n => n.x >= wv.x && n.x <= wv.x + wv.width && n.y >= wv.y && n.y <= wv.y + wv.height);
  const nearest = nodes.reduce((best, n) => {
    const d = Math.hypot(n.x - p.x, n.y - p.y);
    return d < best.d ? { d, x: n.x, y: n.y } : best;
  }, { d: Infinity });
  return {
    player: { x: Math.round(p.x), y: Math.round(p.y) },
    zoom: cam.zoom,
    worldView: { x: Math.round(wv.x), y: Math.round(wv.y), w: Math.round(wv.width), h: Math.round(wv.height) },
    totalNodes: nodes.length,
    nodesInInitialView: inView.length,
    inViewPositions: inView.slice(0, 8).map(n => [Math.round(n.x), Math.round(n.y)]),
    nearestNode: { dist: Math.round(nearest.d), pos: [Math.round(nearest.x), Math.round(nearest.y)] },
  };
})()`));

const shot = await send('Page.captureScreenshot', { format: 'png' });
fs.mkdirSync(path.join(ROOT, 'tools/verify-shots'), { recursive: true });
fs.writeFileSync(path.join(ROOT, 'tools/verify-shots/scene8_initial_view.png'), Buffer.from(shot.result.data, 'base64'));
console.log('saved tools/verify-shots/scene8_initial_view.png');
