...
(async () => {
    ...
    function select() {
        sequence=data.sequences.find(s=>s.branch===el('branch').value&&s.entryRunFrame===Number(el('origin').value));
    ...
    for(const id of ['branch','origin'])el(id).addEventListener('change',select);
