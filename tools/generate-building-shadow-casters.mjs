...
if (WRITE_IF_SOURCE) {
    ...
    const isRegisteredTarget = targets.some((target) => (
...
entries.sort((left, right) => left.textureKey.localeCompare(right.textureKey)
...
if (CHECK) {
    ...
    const auditChanged = writeIfChanged(AUDIT_PATH, auditText);
    console.log(`building shadow casters: ${entries.length}/${targets.length} generated; `
        + `${manifestChanged ? 'manifest updated' : 'manifest unchanged'}; `
        + `${auditChanged ? 'audit updated' : 'audit unchanged'}`);
} else {
