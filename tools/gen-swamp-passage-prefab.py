#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""生成沼泽版通道预制（左右通道·沼泽 / 上下通道·沼泽）：按僵尸通道预制同轴同偏移，
直墙/门换成沼泽墙几何（face 中点对齐、尺度换沼泽档）。2026-08-08 多房迷宫：
同时支持 v1（左右通道，轴 (0.866,0.5)）与 v2（上下通道，轴 (0.866,-0.5)）两个源。"""
import json, io, math
...
def main():
    ...
    # 找左右/上下通道源：6 件 = 2 门 + 4 直墙，门轴对齐 v1=(0.866,0.5) 或 v2=(0.866,-0.5)
    passage_keys = []
    for k, p in d.items():
        ...
        if L > 100 and max(
            abs(((gB[0] - gA[0]) * 0.866 + (gB[1] - gA[1]) * 0.5) / L),
            abs(((gB[0] - gA[0]) * 0.866 - (gB[1] - gA[1]) * 0.5) / L),
        ) > 0.8:
            passage_keys.append(k)
    if not passage_keys:
        raise SystemExit('左右/上下通道 prefab not found')
    print('source passages:', passage_keys)
    ...
    print('saved', len(passage_keys), 'swamp passage prefabs')
