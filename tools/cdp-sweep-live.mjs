#!/usr/bin/env node
/* 游戏内防叠图自愈实机验证（2026-08-16）：
 * - 进 scene8 → 注入 (1324,2110)×3 叠点 → 不手动调用，等 2.5s
 * - 验证 GameScene.update 里的自愈自动清掉 2 个
 * 用法：node tools/cdp-sweep-live.mjs（需 vite dev server 在 5173）
 */
...
console.log('自愈验证:', JSON.stringify(await ev(`(async () => {
  const sleep = (ms) => new Promise(r => setTimeout(r, ms));
  // 插桩：记录 sweepStacked 调用次数与返回值
  ...
  window.__sweepCalls = [];
  const orig = EnergyNodeSystem.sweepStacked;
  EnergyNodeSystem.sweepStacked = function () {
    const r = orig.apply(this, arguments);
    window.__sweepCalls.push(r);
    return r;
  };
  // 注入用户实锤的全部残留：门边叠点 ×3 + 北边散点 ×3
  const strays = [[1324,2110],[1324,2110],[1324,2110],[2183,107],[2137,199],[3031,354]];
  strays.forEach(([x, y], i) => {
    window.Game.entities.set('stray_' + i, {
      id: 'stray_' + i, active: true, _isEnergyNode: true,
      x, y, hp: 3000 + i * 100, maxHp: 3000 + i * 100,
      groundRadius: 45, update() {},
    });
  });
  const strayCount = () => Array.from(window.Game.entities.values())
    .filter(e => e && e._isEnergyNode && strays.some(([sx, sy]) => Math.abs(e.x - sx) < 5 && Math.abs(e.y - sy) < 5)).length;
  const before = strayCount();
  await sleep(1500); // 等自愈（每 60 帧 ≈ 1s）
  const after = strayCount();
  const totalAfter = Array.from(window.Game.entities.values()).filter(e => e && e._isEnergyNode && e.active).length;
  return { injected: 6, before, after, sweepCalls: window.__sweepCalls, totalNodesAfter: totalAfter, cleaned: after === 0 };
})()`)));
