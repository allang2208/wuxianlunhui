...
def fit_on_canvas(subject: Image.Image, width: int, height: int, content_ratio: float) -> Image.Image:
    ...
    print(
        f"[phalanx-reference] subject={cropped.size} -> {resized.size} "
        f"at ({x},{y}) on {width}x{height}"
    )
    return canvas
...
def main() -> None:
    ...
    parser.add_argument("--content-ratio", type=float, default=0.70)
    args = parser.parse_args()

    args.out_dir.mkdir(parents=True, exist_ok=True)
    subject = remove_white_background(args.mother)
    alpha_out = args.out_dir / "hamster-phalanx-double-axe-v02-rgba.png"
    subject.save(alpha_out)
    reference_out = args.out_dir / "hamster-phalanx-double-axe-white-1024x576.png"
    fit_on_canvas(subject, args.width, args.height, args.content_ratio).save(reference_out)
    print(f"[phalanx-reference] {args.mother} -> {alpha_out} -> {reference_out}")
