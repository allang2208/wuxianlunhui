...
def build_alpha(rgb: np.ndarray, backgrounds: np.ndarray,
                clear_distance: float, solid_distance: float,
                fill_subject_holes: bool = False) -> tuple[np.ndarray, np.ndarray]:
    ...
    if count:
        ...
        if fill_subject_holes:
            # Light armor, cloth, teeth, and eye highlights may match a baked
            # checker color even though they sit inside the closed subject
            # silhouette. Restore only enclosed holes; genuine exterior gaps
            # remain connected to the canvas border and stay transparent.
            filled = ndimage.binary_fill_holes(keep)
            alpha = np.where(filled & ~keep, 1.0, alpha)
...
def process(src: Path, dst: Path, size: int, margin: int,
            clear_distance: float, solid_distance: float,
            fill_subject_holes: bool = False) -> None:
    rgb = np.asarray(Image.open(src).convert("RGB"))
    backgrounds = checker_colors(rgb)
    alpha, nearest_index = build_alpha(
        rgb, backgrounds, clear_distance, solid_distance, fill_subject_holes)
...
def main() -> None:
    ...
    parser.add_argument(
        "--fill-subject-holes", action="store_true",
        help="restore only checker-colored holes enclosed by the subject silhouette")
    ...
    if src.is_dir():
        for path in sorted(src.glob("*.png")):
            process(path, dst / path.name, args.size, args.margin,
                    args.clear_distance, args.solid_distance, args.fill_subject_holes)
    else:
