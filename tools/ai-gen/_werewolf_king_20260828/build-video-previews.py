#!/usr/bin/env python3
"""Build direct-review GIFs and contact sheets for Werewolf King source videos."""
...
SOURCES = {
    ...
    "dying": "dying-doubao-v02-fixed-scale.mp4",
    "pounce": "pounce-doubao-v02-side-plane-lock.mp4",
    "howl": "howl-doubao-v01.mp4",
}
...
def build(name: str, source_name: str) -> dict[str, object]:
    ...
    preview_stride = max(1, round(fps / 12.0))
    preview_frames = [
        frame.resize((512, 288), Image.Resampling.LANCZOS)
        for frame in frames[::preview_stride]
    ]
    preview_frames[0].save(
        PREVIEW_DIR / f"{name}-preview.gif",
        save_all=True,
        append_images=preview_frames[1:],
        duration=round(1000 / (fps / preview_stride)),
        loop=0,
        disposal=2,
    )

    sample_step = 6 if name in {
        "attacking", "dying", "pounce", "howl",
    } else 10
