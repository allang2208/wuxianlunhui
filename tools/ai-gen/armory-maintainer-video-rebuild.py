#!/usr/bin/env python3
"""Build the armory maintainer's three transparent animation sheets from 1.mp4.

Source contract (24 fps, 720x720, 145 frames):
  idle:        f2..f16 sampled every 2 frames, then ping-ponged to f4
  walking:     f62..f78, with same-phase endpoint f79 excluded
  maintenance: f114..f129, with same-phase endpoint f130 excluded

The three actions share the approved hamster-civilian visible height, a stable
torso anchor, and one fixed ground line. Transitions at f44/f94 are excluded.
"""
...
MODULE_SPEC = importlib.util.spec_from_file_location("maintainer_sprite_base", BASE_SCRIPT)
...
def main() -> None:
    ...
    if len(frames) != 145 or abs(source_fps - 24.0) > 0.01:
        raise RuntimeError(
            f"Unexpected maintainer source: {len(frames)} frames at {source_fps:.4f} fps; "
            "expected 145 frames at 24 fps"
        )

    specs = (
        ActionSpec("idle", (2, 4, 6, 8, 10, 12, 14, 16, 14, 12, 10, 8, 6, 4)),
        ActionSpec("walking", tuple(range(62, 79)), playback_fps=17.0),
        ActionSpec("maintenance", tuple(range(114, 130)), playback_fps=16.0),
    )
    ...
    def get_cutout(index: int) -> np.ndarray:
        if index not in cache:
            ...
            print(f"[armory-maintainer] cutout f{index}", flush=True)
        return cache[index]
    ...
    for spec in specs:
        ...
        print(
            f"[armory-maintainer] {spec.key}: {len(cells)} frames, "
