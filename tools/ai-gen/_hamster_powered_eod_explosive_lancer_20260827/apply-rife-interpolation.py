#!/usr/bin/env python3
"""Apply mandatory RIFE v4.6 2x interpolation to the five source sheets."""
...
def main() -> None:
    ...
    combined: dict[str, object] = {
        ...
        "policy": "RIFE v4.6 RGB/alpha 2x; loops include last-to-first midpoint; one-shot attacks and death never wrap",
        "actions": {},
    }
    for action, spec in source_report["actions"].items():
        ...
        command = [
            ...
            "--name", f"hamster-powered-eod-explosive-lancer-{action.replace('_', '-')}",
        ...
        combined["actions"][action] = json.loads(report_path.read_text(encoding="utf-8"))
    (ROOT / "interpolation-report.json").write_text(
        json.dumps(combined, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(f"[powered-lancer-rife] wrote {ROOT / 'interpolation-report.json'}")
