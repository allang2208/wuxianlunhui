...
import {
    POPULATION_GROWTH, restorePopulationGrowth, getPopulationNextEventMs,
    advancePopulationGrowth, settlePopulationEvents, distributeResidents,
} from './population-growth.js';
import {
    POPULATION_HAPPINESS, advancePopulationHappiness, getPopulationSafety,
    recordEntertainmentService, recordCommerceService,
} from './population-happiness.js';
import { getFarmProfile, getFarmPlaneEffect } from './farm-production-profile.js';
...
import { BUILDING_FRONT_ROAD_KIND, buildingRoadLayout } from './building-road-system.js';
...
import {
    accumulateWorkerOutputProgress,
    getConfiguredWorkerOutputFactor,
    resetWorkerOutputProgress,
    resolveWorkerOutputProgress,
} from './workforce-output-factor.js';
...
function _snapshotBuildingRoadCells(structure, config = producerBuildingsJson[structure?.cfgKey] || {}) {
    const savedBaseRoads = config.playerBase && Array.isArray(structure.playerBaseRoadCells)
        ? structure.playerBaseRoadCells : null;
    const layout = buildingRoadLayout(structure.x, structure.y, Number(config.footprintCells) || 2, {
        // 未物化过的新版本旧档仍按原正门道路结算，不能在后台凭空补出被占用的三侧。
        perimeterTile: config.playerBase && !savedBaseRoads ? BUILDING_FRONT_ROAD_KIND : (config.perimeterTile ?? 'road'),
        frontRoadSide: config.frontRoadSide,
        mirror: !!structure.mirror,
    });
    if (!savedBaseRoads) return layout.roadCells;
    const acceptedKeys = new Set(savedBaseRoads.map((cell) => `${cell.i},${cell.j}`));
    return layout.roadCells.filter((cell) => acceptedKeys.has(cell.key));
}
...
function _snapshotRoadContext(snapshot) {
    ...
    for (const structure of snapshot.structures || []) {
        ...
        const perimeterTile = config.perimeterTile ?? 'road';
        if (perimeterTile !== 'road' && perimeterTile !== BUILDING_FRONT_ROAD_KIND) continue;
        for (const cell of _snapshotBuildingRoadCells(structure, config)) addCell(cell);
    }
...
function _snapshotBuildingAccessKeys(context, structure) {
    return _snapshotBuildingRoadCells(structure)
        .map((cell) => cell.key)
        .filter((key) => context.network.byKey.has(key));
}

function _snapshotReachableWarehouses(context, structure, warehouses, orderedStarts = null) {
    const starts = orderedStarts || _snapshotBuildingAccessKeys(context, structure);
    const reachable = [];
    for (const warehouse of warehouses || []) {
        const targets = _snapshotBuildingAccessKeys(context, warehouse);
        let route = null;
        if (orderedStarts) {
            for (const start of starts) {
                route = shortestRoadRoute(context.network, [start], targets);
                if (route) break;
            }
        } else {
            route = shortestRoadRoute(context.network, starts, targets);
        }
...
function _snapshotFarmPoint(structure, config, point) {
    const source = config.sourceCanvas || config.cowVisual?.sourceCanvas || {};
    if (!point || !Number(source.width) || !Number(source.height)) {
        return { x: Number(structure.x) || 0, y: Number(structure.y) || 0 };
    }
    const building = producerBuildingsJson[structure.cfgKey] || {};
    const mirror = building.economyType === 'mushroom_farm' && structure.mirror ? -1 : 1;
    return {
        x: (Number(structure.x) || 0) + (Number(point[0]) / Number(source.width) - 0.5)
            * Math.max(1, Number(building.displayW) || 512) * mirror,
        y: (Number(structure.y) || 0) - (Number(building.footOffsetY) || 0)
            + (Number(point[1]) / Number(source.height) - 0.5)
                * Math.max(1, Number(building.displayH) || 293),
    };
}

function _remainingFarmRoute(job, points) {
    for (let index = 0; index + 1 < points.length; index++) {
        const from = points[index], to = points[index + 1];
        const dx = to.x - from.x, dy = to.y - from.y;
        const lengthSq = dx * dx + dy * dy;
        if (lengthSq <= 0) continue;
        const t = ((job.x - from.x) * dx + (job.y - from.y) * dy) / lengthSq;
        if (t >= -0.000001 && t <= 1.000001
                && Math.hypot(job.x - from.x - t * dx, job.y - from.y - t * dy) < 0.01) {
            return points.slice(index + 1);
        }
    }
    const last = points[points.length - 1];
    if (last && Math.hypot(job.x - last.x, job.y - last.y) < 0.01) return [];
    return null;
}

// 只从当前脚点沿道路折线续行；不保存派生路线，也不因每次账本读取重置运输。
function _snapshotFarmRoute(job, entry, home, context) {
    const returning = job.phase === 'to_farm';
    const points = returning ? [...entry.route].reverse().concat(home) : [home, ...entry.route];
    const remaining = _remainingFarmRoute(job, points);
    if (remaining) return remaining;
    const [i, j] = blockCellOf(job.x, job.y);
    const currentKey = `${i},${j}`;
    if (context.network.byKey.has(currentKey)) {
        const targets = returning ? [entry.route[0].key]
            : _snapshotBuildingAccessKeys(context, entry.warehouse);
        const reroute = shortestRoadRoute(context.network, [currentKey], targets);
        if (!reroute) return null;
        const continuation = returning ? [...reroute, home] : reroute;
        return _remainingFarmRoute(job, continuation) || continuation;
    }
    // 门内到入口的一段也要收费；仓库改道后仍从真实位置起步。
    return points;
}

function _advanceSnapshotFarmTransport(job, route, availableMs, speed) {
    if (!route) return { remainingMs: availableMs, arrived: false };
    let remainingMs = availableMs;
    let index = 0;
    for (; index < route.length; index++) {
        const point = route[index];
        const distance = Math.hypot(point.x - job.x, point.y - job.y);
        const duration = distance / speed * 1000;
        if (duration > remainingMs) {
            const ratio = remainingMs / duration;
            job.x += (point.x - job.x) * ratio;
            job.y += (point.y - job.y) * ratio;
            remainingMs = 0;
            break;
        }
        job.x = point.x;
        job.y = point.y;
        remainingMs = Math.max(0, remainingMs - duration);
    }
    let remainingDistance = 0;
    let previous = job;
    for (const point of route.slice(index)) {
        remainingDistance += Math.hypot(point.x - previous.x, point.y - previous.y);
        previous = point;
    }
    job.phaseRemainMs = remainingDistance / speed * 1000;
    job.phaseTotalMs = Math.max(Number(job.phaseTotalMs) || 0, job.phaseRemainMs);
    return { remainingMs, arrived: index >= route.length };
}
...
function _economyModuleValue(structure, economyType, moduleId) {
    ...
    const savedModules = {
        ...
        corn_farm: structure?.cornFarmModules,
        mushroom_farm: structure?.mushroomFarmModules,
...
function _workerOutputFactor(structure, economyType) {
    return getConfiguredWorkerOutputFactor(
        structure?.assignedWorkers,
        populationEconomyConfig[economyType]
    );
}
...
const TAVERN_OUTPUT_TARGETS = new Set([
    'windmill', 'bakery', 'chain_restaurant', 'cheese_farm', 'mushroom_farm',
    'miner_camp', 'deep_drill', 'steam_power_plant', 'wind_power_plant', 'solar_power_plant', 'geothermal_power_plant', 'planar_resonator',
...
function _snapshotTavernMultiplier(economyType, economyStructures) {
    ...
    for (const tavern of economyStructures || []) {
        ...
        strongest = Math.max(strongest,
            _tavernConfiguredBonus(tavern) * _workerOutputFactor(tavern, 'tavern'));
    }
    return 1 + strongest;
}
...
function _simulateTavernService(structure, elapsedMs, laborEfficiency,
    ...
    if (Math.floor(Number(structure.assignedWorkers) || 0) <= 0) {
        return { activeMs: 0, foodSpent: 0, serviceWindows: [] };
    }
    ...
    while (remaining > 0 && guard-- > 0) {
        if (job.phase === 'serving') {
            ...
            serviceWindows.push({ startMs: elapsedMs - remaining, endMs: elapsedMs - remaining + spent });
            activeMs += spent;
            remaining -= spent;
    ...
    return { activeMs, foodSpent, serviceWindows };
}
...
export function getWorld122ResearchSummary(snapshot) {
    ...
    const laborEfficiency = assignedPopulation > 0
        ? Math.max(0, Math.min(1, (Number(snapshot?.populationEconomy?.populationVersion) >= 1
            ? Math.max(0, Number(snapshot.populationEconomy.total) || 0) : populationCapacity) / assignedPopulation))
        : 1;
...
function _unitDps(kind, levelOverrides = null) {
    ...
    if (kind === 'warrior' || kind === 'special_forces') {
        ...
        const expectedExtraTargets = Math.max(0,
            Number(ability?.expectedExtraTargets) || WORLD122_SIM.sweepExpectedExtraTargets);
        dps += dps * baseAoeMul * (1 + upgradeAoeBonus)
            * expectedExtraTargets;
    }
...
const LOCAL_MODULE_UPGRADES = [
    ...
    ['cornFarm', 'corn_farm_economy'],
    ['mushroomFarm', 'mushroom_farm_economy'],
...
function _createSettlementReport(elapsedMs) {
    return {
        ...
        windEnergyProduced: 0, solarEnergyProduced: 0, geothermalEnergyProduced: 0,
        ...
        foodSpentOnUnits: 0,
        populationBorn: 0, populationLost: 0, foodSpentOnPopulation: 0,
...
export function settleWorld122(snap, elapsedMs, opts = {}) {
    ...
    _prepareSnapshotPopulation(target);
    syncSnapshotBuildingTiers(target, producerBuildingsJson, opts.isBuildingTierUnlocked);
    ...
    let cursor = 0;
    while (cursor < elapsedMs - 1e-8) {
        const housing = _snapshotPopulationHousing(target);
        const warehouses = _snapshotWarehouses(target);
        const state = target.populationEconomy;
        const populationEvent = getPopulationNextEventMs(state, housing.capacity, _foodInWarehouses(warehouses));
        const upgradeBoundary = boundaries.find((atMs) => atMs > cursor + 1e-8) ?? elapsedMs;
        const serviceBoundary = _snapshotTavernNextBoundaryMs(target);
        const boundary = Math.min(upgradeBoundary, cursor + populationEvent, cursor + serviceBoundary);
        const step = Math.max(0, boundary - cursor);
        const segmentOpts = Number.isFinite(opts.gameTimeMs)
            ? { ...opts, gameTimeMs: opts.gameTimeMs - elapsedMs + boundary } : opts;
        if (step > 0) {
            advancePopulationHappiness(state, step, getPopulationSafety(opts.sceneId || target.sceneId, target.worldEpoch));
            advancePopulationGrowth(state, step, housing.capacity, _foodInWarehouses(warehouses));
            const segment = _settleWorld122Interval(target, step, segmentOpts, simulation);
        ...
        const survivingWarehouses = _snapshotWarehouses(target);
        const changes = settlePopulationEvents(state, housing.capacity,
            () => _foodInWarehouses(survivingWarehouses),
            (amount) => _deductFoodFromWarehouses(survivingWarehouses, amount),
            () => {
                const residents = distributeResidents(housing.houses, state.total);
                return housing.houses.map((house) => ({ ...house, level: house.key.economyLevel,
                    residents: residents.get(house.key) || 0 }));
            });
        report.populationBorn += changes.born;
        report.populationLost += changes.lost;
        report.foodSpentOnPopulation += changes.foodConsumed;
        state.foodStored = _foodInWarehouses(survivingWarehouses);
        if (changes.lost) _reconcileSnapshotWorkers(target);
    ...
    for (const structure of target.structures || []) {
        if (structure.continuous || structure.economyContinuous) {
            Object.assign(structure, getSnapshotContinuousUpgradeTargets(structure));
        }
    }
    return report;
}

function _snapshotPopulationHousing(target) {
    const levels = populationEconomyConfig.house?.levels || [];
    const houses = (target.structures || []).filter((structure) => structure.kind === 'producer'
        && Number(structure.hp ?? 1) > 0 && producerBuildingsJson[structure.cfgKey]?.economyType === 'housing')
        .map((house) => ({ key: house, id: house.id || `${house.cfgKey}:${house.x}:${house.y}`,
            capacity: (levels.find((level) => level.level === (Number(house.economyLevel) || 1)) || levels[0])?.populationCapacity || 0 }));
    return { houses, capacity: houses.reduce((sum, house) => sum + house.capacity, 0) };
}

// 人口口粮、生产入库和经营扣费必须共用同一组存活仓库。
function _snapshotWarehouses(target) {
    return (target.structures || []).filter((structure) => structure.kind === 'producer'
        && Number(structure.hp ?? 1) > 0 && getProducerStorageCap(structure) > 0);
}

function _snapshotTavernNextBoundaryMs(target) {
    const retryMs = Math.max(1, Number(populationEconomyConfig.tickMs) || 1000);
    let next = Infinity;
    for (const structure of target.structures || []) {
        if (structure.kind !== 'producer' || !(Number(structure.hp ?? 1) > 0)
            || producerBuildingsJson[structure.cfgKey]?.economyType !== 'tavern'
            || Math.floor(Number(structure.assignedWorkers) || 0) <= 0) continue;
        const job = structure.tavernJob;
        const serving = job?.phase === 'serving';
        const remaining = Number(serving ? job.serviceRemainMs : job?.phaseRemainMs);
        // 宴饮按真正结束时间切段；等粮/运输最多推进一个经济 tick 后重新读取库存。
        // 本段稍后产出的粮只能供下一段使用，不能预支未来粮食或整期忽略中途补粮。
        if (remaining > 1e-8) next = Math.min(next, remaining);
        if (!serving || !(remaining > 1e-8)) next = Math.min(next, retryMs);
    }
    return next;
}

function _snapshotWorkforce(target) {
    return (target.structures || []).filter((structure) => {
        if (!(Number(structure.hp ?? 1) > 0)) return false;
        if (structure.kind === 'hut') return true;
        const type = producerBuildingsJson[structure.cfgKey]?.economyType;
        return structure.kind === 'producer' && !!type && _economyWorkerSlots(structure, type) > 0;
    });
}

function _prepareSnapshotPopulation(target) {
    const saved = target.populationEconomy || {};
    const warehouses = _snapshotWarehouses(target);
    let legacyFoodPending = Math.max(0, Number(saved.legacyFoodPending) || 0);
    if ((Number(saved.storageVersion) || 1) < 2) legacyFoodPending += Math.max(0, Number(saved.foodStored) || 0);
    legacyFoodPending -= _depositFoodToWarehouses(warehouses, legacyFoodPending);
    const assigned = _snapshotWorkforce(target).reduce((sum, structure) => sum + Math.max(0,
        Math.floor(Number(structure.assignedWorkers ?? (structure.kind === 'hut' ? structure.miners : 0)) || 0)), 0);
    const state = restorePopulationGrowth(saved, Math.max(_snapshotPopulationHousing(target).capacity, assigned));
    if (state.foodSatisfied == null && state.total > 0) {
        state.foodSatisfied = _foodInWarehouses(warehouses) >= state.total * POPULATION_GROWTH.foodPerPopulation;
    }
    target.populationEconomy = { ...saved, ...state, storageVersion: 2, legacyFoodPending,
        foodStored: _foodInWarehouses(warehouses) };
    _reconcileSnapshotWorkers(target);
}

function _reconcileSnapshotWorkers(target) {
    const workers = _snapshotWorkforce(target);
    let used = 0;
    for (const structure of workers) {
        const type = producerBuildingsJson[structure.cfgKey]?.economyType;
        const slots = structure.kind === 'hut' ? getMinerEconomyStats(structure.modules || {}).count
            : _economyWorkerSlots(structure, type);
        structure.assignedWorkers = Math.max(0, Math.min(slots,
            Math.floor(Number(structure.assignedWorkers ?? (structure.kind === 'hut' ? structure.miners : 0)) || 0)));
        used += structure.assignedWorkers;
    }
    let excess = Math.max(0, used - target.populationEconomy.total);
    const foodTypes = new Set(['windmill', 'cheese_farm', 'corn_farm', 'mushroom_farm', 'bakery', 'chain_restaurant']);
    workers.sort((a, b) => Number(foodTypes.has(producerBuildingsJson[a.cfgKey]?.economyType))
        - Number(foodTypes.has(producerBuildingsJson[b.cfgKey]?.economyType)) || String(b.id).localeCompare(String(a.id)));
    for (const structure of workers) {
        const removed = Math.min(excess, structure.assignedWorkers);
        structure.assignedWorkers -= removed;
        excess -= removed;
        if (structure.kind === 'hut') structure.miners = Math.min(Number(structure.miners) || 0, structure.assignedWorkers);
        if (!excess) break;
    }
}
...
function _settleWorld122Interval(target, elapsedMs, opts, simulation) {
    ...
    Object.assign(target.populationEconomy, {
        storageVersion: 2,
        foodStored: _foodInWarehouses(warehouses),
        legacyFoodPending,
    });
    ...
    const laborEfficiency = assignedPopulation > 0
        ? Math.max(0, Math.min(1, target.populationEconomy.total / assignedPopulation))
        : 1;
    const residents = distributeResidents(_snapshotPopulationHousing(target).houses, target.populationEconomy.total);
    let tavernWeightedMultiplier = 1;
    const entertainmentWindows = [];
    for (const tavern of economyStructures) {
        if (producerBuildingsJson[tavern.cfgKey]?.economyType !== 'tavern') continue;
        ...
        const seats = POPULATION_HAPPINESS.tavernSeats * _workerOutputFactor(tavern, 'tavern') * laborEfficiency;
        for (const window of result.serviceWindows) entertainmentWindows.push({ ...window, seats });
        const weightedBonus = _tavernConfiguredBonus(tavern)
            * _workerOutputFactor(tavern, 'tavern')
    ...
    recordEntertainmentService(target.populationEconomy, elapsedMs, entertainmentWindows);
    target.tavernLastWeightedMultiplier = tavernWeightedMultiplier;
    const foodWeatherMultiplier = opts.productionWeather?.food ?? getFoodProductionWeatherEffect(
    ...
    for (const structure of economyStructures) {
        ...
        if (economyType === 'royal_mint') {
            ...
            const servicePopulation = economyStructures.reduce((sum, house) => {
                ...
                return sum + (residents.get(house) || 0);
            }, 0);
            ...
            const servicePaid = energyCost <= 0 || _deductFromWarehouses(warehouses, energyCost);
            if (servicePaid) report.energySpentOnGrandMall += energyCost;
            if (settlements > 0 && servicePaid) {
                const coveredHouses = economyStructures.filter((house) =>
                    producerBuildingsJson[house.cfgKey]?.economyType === 'housing'
                    && Math.hypot((house.x || 0) - (structure.x || 0),
                        (house.y || 0) - (structure.y || 0)) <= serviceRange);
                recordCommerceService(target.populationEconomy, structure.id || structure.cfgKey,
                    coveredHouses.map((house) => ({ id: house.id || house.cfgKey + ':' + house.x + ':' + house.y,
                        residents: residents.get(house) || 0 })),
                    staffEfficiency * laborEfficiency, settlements * settlementIntervalMs);
            ...
            const baseAndPopulation = (configuredBase
                + target.populationEconomy.total * populationRate) * operatingFactor;
            ...
            const effectiveServicePopulation = economyStructures.reduce((sum, house) => {
                ...
                const population = residents.get(house) || 0;
            ...
            const weatherMultiplier = opts.productionWeather?.[isSolar ? 'solar' : 'wind'] ?? (isSolar
                ? getSolarPowerWeatherEffect : getWindPowerWeatherEffect)(
                opts.sceneId || 'scene8', opts.gameTimeMs
            ).multiplier;
            const energyPerEffectiveCycle = energyPerCycle * conversionRate * staffFactor
                * laborEfficiency * _workshopEfficiencyMultiplier(structure, economyStructures)
                * tavernWeightedMultiplier * weatherMultiplier;
            ...
            const workerOutputFactor = _workerOutputFactor(structure, economyType);
            const cycleMs = Math.max(100,
            ...
            if (assigned > 0 && job.pendingFood <= 0 && (Number(job.cargoFood) || 0) > 0) {
                ...
                if (availableMs >= effectiveRemainingProcessMs) {
                    ...
                    accumulateWorkerOutputProgress(job, workerOutputFactor,
                        remainingProcessMs);
                    const batchWorkerOutputFactor = resolveWorkerOutputProgress(
                        job, workerOutputFactor);
                    ...
                    const exactOutput = Math.max(0, Number(structure[remainderField]) || 0)
                        + consumedFood * outputMultiplier * batchWorkerOutputFactor
                            * tavernWeightedMultiplier
                    ...
                    const workMs = Math.min(remainingProcessMs,
                        availableMs * workshopMultiplier);
                    accumulateWorkerOutputProgress(job, workerOutputFactor, workMs);
                    job.phase = 'processing';
                    job.processTotalMs = processMs;
                    job.processRemainMs = Math.max(0,
                        remainingProcessMs - workMs);
                    availableMs = 0;
                }
            }
            let guard = 10000;
            while (assigned > 0 && job.pendingFood <= 0 && availableMs >= cycleMs && guard-- > 0) {
                ...
                availableMs -= cycleMs;
                const exactOutput = Math.max(0, Number(structure[remainderField]) || 0)
                    + inputFood * outputMultiplier * workerOutputFactor * tavernWeightedMultiplier
                        * foodWeatherMultiplier
                        * getProductionResourceMul();
                const produced = Math.max(1, Math.floor(exactOutput));
                structure[remainderField] = exactOutput - produced;
            ...
            if (job.cargoFood <= 0 && job.pendingFood <= 0 && job.offlineProgressMs > 0) {
                resetWorkerOutputProgress(job);
                accumulateWorkerOutputProgress(job, workerOutputFactor,
                    Math.min(processMs, job.offlineProgressMs * workshopMultiplier));
            }
            if (job.cargoFood <= 0) job.phase = job.pendingFood > 0 ? 'waiting_deposit' : 'idle';
            job.targetWarehouseId = null;
        } else if (getFarmProfile(economyType)) {
            const profile = getFarmProfile(economyType);
            const farmCfg = populationEconomyConfig[economyType] || {};
            const prefix = profile.savePrefix;
            const jobField = `${prefix}Job`;
            const remainderField = `${prefix}OutputRemainder`;
            const { processModule, outputModule, scaleModule, moveModule } = profile;
            const planeMultiplier = getFarmPlaneEffect(economyType, opts.sceneId || 'scene8').multiplier;
            const baseScale = Math.max(1, Number(
                farmCfg[profile.baseScaleKey]
            ) || 2);
            const job = structure[jobField] && typeof structure[jobField] === 'object'
                ? structure[jobField]
                : {};
            structure[jobField] = job;
            const home = _snapshotFarmPoint(structure, farmCfg,
                farmCfg.workerDoorPoint || farmCfg.workerServicePoint);
            const roadEntry = _snapshotFarmPoint(structure, farmCfg,
                farmCfg.workerRoadEntryPoint || farmCfg.workerDoorPoint || farmCfg.workerServicePoint);
            const farmStarts = _snapshotBuildingAccessKeys(roadContext, structure).sort((a, b) => {
                const from = roadContext.network.byKey.get(a), to = roadContext.network.byKey.get(b);
                return Math.hypot(from.x - roadEntry.x, from.y - roadEntry.y)
                    - Math.hypot(to.x - roadEntry.x, to.y - roadEntry.y);
            });
            const reachableWarehouseRoutes = _snapshotReachableWarehouses(
                roadContext,
                structure,
            ...
            // 与前台一致：断路或无人时冻结加工、运输与待入库成品。
            if (reachableWarehouseRoutes.length <= 0 || assigned <= 0 || laborEfficiency <= 0) continue;
            const processMs = Math.max(1000,
                _economyModuleValue(structure, economyType, processModule)
                    || Number(farmCfg.baseProcessTimeMs) || 20000);
            const foodPerBatch = Math.max(1,
                _economyModuleValue(structure, economyType, outputModule)
                    || Number(farmCfg.baseFoodPerBatch) || 40);
            const scaleCount = Math.max(1, Math.floor(
                _economyModuleValue(structure, economyType, scaleModule) || baseScale));
            const moveSpeed = Math.max(1, (Number(farmCfg.baseMoveSpeed) || 80)
                * (_economyModuleValue(structure, economyType, moveModule) || 1));
            const workshopMultiplier = _workshopEfficiencyMultiplier(structure, economyStructures);
            const workerOutputFactor = _workerOutputFactor(structure, economyType);
            const warehouseId = (entry) => entry.warehouse.id ?? `${entry.warehouse.x},${entry.warehouse.y}`;
            const canDeposit = (entry) => _warehouseFree(entry.warehouse)
                >= _warehouseFactor(entry.warehouse, 'warehouse_food_density');
            job.pendingFood = Math.max(0, Math.floor(Number(job.pendingFood) || 0));
            job.completedBatches = Math.max(0, Math.floor(Number(job.completedBatches) || 0));
            if (!Number.isFinite(job.x) || !Number.isFinite(job.y)) Object.assign(job, home);
            if (!['processing', 'waiting_deposit', 'to_deposit', 'to_farm'].includes(job.phase)) {
                job.phase = job.pendingFood > 0 ? 'waiting_deposit' : 'processing';
            }
            if (job.pendingFood > 0 && job.phase === 'processing') job.phase = 'waiting_deposit';
            const beginProcessing = () => {
                job.phase = 'processing';
                Object.assign(job, home);
                job.targetWarehouseId = null;
                job.processTotalMs = processMs;
                job.processRemainMs = processMs;
                job.phaseRemainMs = 0;
                job.phaseTotalMs = 0;
                resetWorkerOutputProgress(job);
            };
            if (job.pendingFood <= 0 && ['waiting_deposit', 'to_deposit'].includes(job.phase)) {
                beginProcessing();
            }
            if (job.phase === 'processing' && !(Number(job.processTotalMs) > 0)) beginProcessing();
            if (job.phase === 'processing') Object.assign(job, home);
            let availableMs = Math.max(0, Number(job.offlineProgressMs) || 0)
                + elapsedMs * laborEfficiency;
            job.offlineProgressMs = 0;
            // 每轮消耗加工/路程时间或有限的仓库容量；阻塞时退出，不截断可用时间。
            while (availableMs > 0) {
                if (job.phase === 'processing') {
                    const remainingWork = Math.max(0, Number(job.processRemainMs) || 0);
                    const workMs = Math.min(remainingWork, availableMs * workshopMultiplier);
                    availableMs = Math.max(0, availableMs - workMs / Math.max(0.01, workshopMultiplier));
                    job.processRemainMs = Math.max(0, remainingWork - workMs);
                    accumulateWorkerOutputProgress(job, workerOutputFactor, workMs);
                    if (job.processRemainMs > 0) break;
                    const exactOutput = Math.max(0, Number(structure[remainderField]) || 0)
                        + foodPerBatch * scaleCount / baseScale
                            * resolveWorkerOutputProgress(job, workerOutputFactor) * tavernWeightedMultiplier
                            * foodWeatherMultiplier * planeMultiplier * getProductionResourceMul();
                    job.pendingFood = Math.max(1, Math.floor(exactOutput));
                    structure[remainderField] = exactOutput - job.pendingFood;
                    resetWorkerOutputProgress(job);
                    job.phase = 'waiting_deposit';
                    job.targetWarehouseId = null;
                    continue;
                }
                let entry = reachableWarehouseRoutes.find(route => warehouseId(route) === job.targetWarehouseId);
                if (job.phase === 'waiting_deposit' || (job.phase === 'to_deposit' && !entry)) {
                    entry = reachableWarehouseRoutes.find(canDeposit);
                    if (!entry) break;
                    job.phase = 'to_deposit';
                    job.targetWarehouseId = warehouseId(entry);
                    job.phaseTotalMs = 0;
                }
                if (job.phase === 'to_farm' && !entry) entry = reachableWarehouseRoutes[0];
                if (!entry) break;
                const route = _snapshotFarmRoute(job, entry, home, roadContext);
                // 保留前台 _move 的现有口径：任务时间、运输速度各应用人口效率。
                const travel = _advanceSnapshotFarmTransport(job, route, availableMs,
                    moveSpeed * laborEfficiency);
                availableMs = travel.remainingMs;
                if (!travel.arrived) break;
                if (job.phase === 'to_farm') {
                    beginProcessing();
                    continue;
                }
                // 必须抵达指定仓库才能入库；满仓保留货物并重新选择目的地。
                const stored = _depositFoodToWarehouses([entry.warehouse], job.pendingFood);
                report.foodProduced += stored;
                job.pendingFood -= stored;
            ...
            const slotCount = Math.max(1,
                Math.floor(Number(steamCfg.operationalWorkerCap) || 2));
            ...
            for (let index = 0; index < activeCount; index++) {
                ...
                if (job.pendingEnergy <= 0 && job.cargoFood > 0) {
                    ...
                    if (availableMs >= effectiveRemaining) {
                        ...
                        const exactOutput = Math.max(0,
                            ...
                            + energyPerBatch * batchWorkerOutputFactor * tavernWeightedMultiplier
                                * getProductionResourceMul();
                        const produced = Math.max(1, Math.floor(exactOutput));
                ...
                while (job.pendingEnergy <= 0 && availableMs >= cycleMs && guard-- > 0) {
                    ...
                    const exactOutput = Math.max(0,
                        Number(structure.steamOutputRemainder) || 0)
                        + energyPerBatch * workerOutputFactor * tavernWeightedMultiplier
                            * getProductionResourceMul();
                    const produced = Math.max(1, Math.floor(exactOutput));
                    structure.steamOutputRemainder = exactOutput - produced;
                    const stored = _depositToWarehouses(steamWarehouses, produced);
                ...
                job.offlineProgressMs = Math.min(cycleMs, availableMs);
                if (job.cargoFood <= 0 && job.pendingEnergy <= 0 && job.offlineProgressMs > 0) {
                    resetWorkerOutputProgress(job);
                    accumulateWorkerOutputProgress(job, workerOutputFactor,
                        Math.min(processMs, job.offlineProgressMs * workshopMultiplier));
                }
