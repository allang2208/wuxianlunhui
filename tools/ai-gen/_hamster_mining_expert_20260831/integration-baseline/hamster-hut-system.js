...
import { renderBuildingUpgradeCard, renderBuildingUpgradeIcon, buildingUpgradeControlsChanged } from '../ui/panels/building-upgrade-card.js';
...
export class HamsterHut extends DamageableEntity {
    ...
    unloadMiner(miner) {
        ...
        const multiplier = TavernEconomySystem.getPlaneOutputMultiplier('miner_camp')
            * getProductionResourceMul();
        let submitted = 0;
        if (total > 0 && EnergyManager) {
            // 先用只读容量查询找出本次可完整提交的原始矿量；酒馆只放大最终入库量，
    ...
    _syncMinerWorkforce(immediateAssignmentCount = 0) {
        ...
        if (immediateCount > 0 && activeWorkers.length < desired) {
            ...
            this._respawnTimer = assignedFilled ? HAMSTER_CONFIG.hut.respawnMs : 0;
            this._spawnRetryTimer = 0;
            if (assignedFilled) {
                this._spawnBlocked = false;
                this._spawnEnergyBlocked = false;
    ...
    getEconomyUpgradeProject(projectId) {
        const module = HAMSTER_CONFIG.modules?.[projectId];
        if (!module) return null;
        return {
            name: module.name || projectId,
            maxed: !this.canUpgradeModule(projectId),
            busy: !!this._upgrade,
            inProgress: this._upgrade?.moduleId === projectId,
            start: () => this.startModuleUpgrade(projectId),
        };
    }

    getContinuousUpgradeLockReason() {
        return getEconomyContinuousUpgradeLockReason(this);
    }
                ...
                if (this._spawnRetryTimer <= 0) {
                    ...
                    if (miner) {
                        ...
                        this._respawnTimer = this._restoreTopUp > 0
                            ? 800
                            : (this._assignedTopUpPending > 0 ? 0 : HAMSTER_CONFIG.hut.respawnMs);
    ...
    _destroyHutCleanup({ silent = false } = {}) {
...
class HamsterHutPanel extends BasePanel {
    constructor() {
        ...
        this.hut = null;
        this.player = null;
        this._refreshTimer = null; // 500ms 原位刷新数值，保留岗位/升级控件。
    ...
    buildContent(el) {
        ...
        el.querySelector('#hhModules').addEventListener('click', (event) => {
            const button = event.target.closest('[data-economy-continuous]');
            if (!button || button.disabled || !this.hut) return;
            const projectId = button.dataset.economyContinuous;
            const name = HAMSTER_CONFIG.modules?.[projectId]?.name || projectId;
            const result = toggleEconomyContinuousUpgrade(this.hut, projectId);
    ...
    _tickProgress() {
        ...
        const h = this.hut;
        if (!el || !h) return;
        if (this._economyContinuousRevision !== h._economyContinuousRevision) {
            this.refresh();
            return;
        }
        if (buildingUpgradeControlsChanged(this, h)) {
            this.refresh();
            return;
        }
        const upgrade = h._upgrade;
        if (this._renderedUpgrade !== upgrade) {
            this.refresh();
            return;
        }
        if (upgrade) {
    ...
    _refreshLiveStatus() {
        ...
        const workforcePct = workforce?.slots > 0
            ? Math.round((workforce.assigned || 0) / workforce.slots * 100) : 0;
        const staffing = `在岗 ${h.aliveMinerCount()}/${h.minerCount()}`;
        refreshBuildingDetailHeader(el, {
            hp: h.hp, maxHp: h.maxHp,
            status: `经济建筑 · ${staffing} · 岗位 ${h.minerCapacity()}`,
        });
        const values = {
            hhStaffing: h._spawnBlocked ? '出口阻塞' : staffing,
            hhMiningDamage: Math.round(mults.attackDamage * mults.miningMult),
            hhMiningEfficiency: `${mults.attackDamage} / +${Math.round((mults.miningMult - 1) * 100)}%`,
            hhAttackInterval: `${mults.attackInterval}ms`,
            hhWalkSpeed: mults.walkSpeed,
            hhBackpackCapacity: mults.backpackCapacity,
            hhCarried: carried,
            hhStoredEnergy: `${energy}/${storageCapacity}`,
            hhFreePopulation: population.free,
            hhWorkforce: `${workforce?.assigned || 0}/${workforce?.slots || 0} · 人口效率 ${Math.round((workforce?.laborEfficiency || 0) * 100)}%`,
            hhWorkforcePct: `${workforcePct}%`,
            hhGold: `${gold} 金`,
            hhEnergy: `${energy} 能`,
        };
        ...
        el.querySelector('#hhStaffing')?.classList.toggle('is-blocked', !!h._spawnBlocked);
        const workforceBar = el.querySelector('#hhWorkforceBar');
        if (workforceBar) workforceBar.style.width = `${workforcePct}%`;
        const blockedNote = el.querySelector('#hhWorkforceBlocked');
        if (blockedNote) blockedNote.hidden = !h._spawnBlocked;
        el.querySelectorAll('[data-hh-worker], [data-hh-worker-max]').forEach((button) => {
            button.disabled = !workforce || (button.dataset.hhWorker === '-1'
                ? workforce.assigned <= 0 : workforce.freeSlots <= 0 || population.free <= 0);
        });
    ...
    refresh() {
        ...
        const h = this.hut;
        this._economyContinuousRevision = h._economyContinuousRevision;
        this._renderedUpgrade = h._upgrade;
        ...
        st.innerHTML = `
            <div class="economy-panel-heading">
                <span>⛏️ 采矿物流档案</span>
                <span id="hhStaffing" class="economy-panel-badge"></span>
            </div>
            <div class="economy-stat-grid">
                <div><span>采矿攻击力</span><b id="hhMiningDamage"></b></div>
                <div><span>基础伤害 / 效率</span><b id="hhMiningEfficiency"></b></div>
                <div><span>攻击间隔</span><b id="hhAttackInterval"></b></div>
                <div><span>移动速度</span><b id="hhWalkSpeed"></b></div>
                <div><span>单人背包</span><b id="hhBackpackCapacity"></b></div>
                <div><span>携带中</span><b id="hhCarried"></b></div>
                <div><span>仓库能源</span><b id="hhStoredEnergy" class="economy-unit-energy"></b></div>
                <div><span>空闲人口</span><b id="hhFreePopulation"></b></div>
            </div>
            <div class="economy-workforce">
                <div class="economy-workforce-copy">
                    <div class="economy-workforce-label"><span>矿工岗位</span><b id="hhWorkforce"></b></div>
                    <div class="economy-progress-label"><span>岗位安排</span><b id="hhWorkforcePct"></b></div>
                    <div class="economy-progress"><div id="hhWorkforceBar"></div></div>
                    <div class="economy-workforce-note">新增岗位会立即生成矿工；能源只在矿工返营提交后进入仓库。<span id="hhWorkforceBlocked" class="is-warning" hidden>出口受阻，腾空后自动生成缺额矿工。</span></div>
                </div>
                <div class="economy-workforce-actions">
                    <button class="troop-panel-unit-button" data-hh-worker="-1" disabled>−1</button>
                    <button class="troop-panel-unit-button" data-hh-worker="1" disabled>+1</button>
                    <button class="troop-panel-unit-button" data-hh-worker-max disabled>最大</button>
                </div>
            </div>
            `;
        ...
        modBox.innerHTML = `
            <div class="economy-panel-heading">
                <span>矿工营地升级项目</span>
                <span class="economy-panel-meta">持有 <span id="hhGold" class="economy-unit-gold"></span> / <span id="hhEnergy" class="economy-unit-energy"></span></span>
            </div>
        ...
        // 与当前按钮一起记录科技/互斥状态，避免首次轮询前解锁后仍保留旧的 disabled 基线。
        buildingUpgradeControlsChanged(this, h, { force: true });
...
export const HamsterHutSystem = {
    ...
    tryInteract(mx, my, player, options = {}) {
        ...
        const h = options.entity === undefined
            ? pickStructureAtScreen(Game, mx, my) : options.entity;
        if (!h?.active || !h._isHamsterHut) return false;
        if (!Game?._buildMode && !options.remote
            && Math.hypot(h.x - player.x, h.y - player.y) > 260) return false;
