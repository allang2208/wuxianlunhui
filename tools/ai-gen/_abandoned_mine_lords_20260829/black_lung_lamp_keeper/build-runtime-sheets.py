#!/usr/bin/env python3
"""Build lossless runtime sheets for the approved Black-Lung Lamp Keeper.

Each action receives one uniform crop.  Visible pixels are never resampled;
horizontal crops remain symmetric around the approved 640px cell centre and
vertical crops update footY.  All actions retain the same authored 460px body
scale, including death's accepted horizontal collapse trajectory.
"""
...
REPO = ROOT.parents[3]
RUNTIME = REPO / "assets" / "enemies" / "black_lung_lamp_keeper"
RUNTIME_BODY_HEIGHT = 260

SOURCES = {
    "idle": ROOT / "previews" / "idle-rebuild-v01" / "black-lung-lamp-keeper-idle-base-v01.png",
    "walking": ROOT / "sheets" / "interpolated" / "walking.png",
    "pickaxeSlam": ROOT / "sheets" / "interpolated" / "pickaxeSlam.png",
    "blackLungCough": ROOT / "sheets" / "interpolated" / "blackLungCough.png",
    "lanternOverload": ROOT / "sheets" / "interpolated" / "lanternOverload.png",
    "dying": ROOT / "sheets" / "interpolated" / "dying.png",
}
OUTPUTS = {
    "idle": "idle.png",
    "walking": "walking.png",
    "pickaxeSlam": "pickaxe_slam.png",
    "blackLungCough": "black_lung_cough.png",
    "lanternOverload": "lantern_overload.png",
    "dying": "dying.png",
}
SOURCE_LAYOUTS = {
    "idle": {"columns": 6, "frameCount": 60, "frameRate": 12, "repeat": -1},
    "walking": {"columns": 8, "frameCount": 62, "frameRate": 12, "repeat": -1},
    "pickaxeSlam": {"columns": 8, "frameCount": 61, "duration": 5083, "contactFrame": 32, "repeat": 0},
    "blackLungCough": {"columns": 8, "frameCount": 61, "duration": 5083, "releaseFrame": 20, "repeat": 0},
    "lanternOverload": {"columns": 8, "frameCount": 61, "duration": 5083, "releaseFrame": 30, "repeat": 0},
    "dying": {"columns": 8, "frameCount": 41, "duration": 3417, "corpseSettledFrame": 24, "repeat": 0},
}
...
def analyze(source: Image.Image, layout: dict[str, int]) -> dict[str, int | float]:
    ...
    return {
        ...
        "runtimePixelScale": RUNTIME_BODY_HEIGHT / 460,
    }
...
def repack(source: Image.Image, output_path: Path, info: dict[str, int | float]) -> None:
    ...
    for index in range(frame_count):
        ...
        cell = source.crop((
            old_col * 640 + crop_left,
            old_row * 640 + crop_top,
            old_col * 640 + crop_left + new_w,
            old_row * 640 + crop_top + new_h,
        ))
...
def main() -> None:
    ...
    for action, source_path in SOURCES.items():
        ...
        info.update({
            ...
            if key not in info and key not in {"columns"}
        })
    ...
    manifest = {
        "characterKey": "blackLungLampKeeper",
        "assetOnly": False,
