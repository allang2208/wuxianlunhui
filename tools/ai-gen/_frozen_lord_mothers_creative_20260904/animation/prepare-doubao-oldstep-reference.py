#!/usr/bin/env python3
"""Fit and compare the approved Aurora Fate Weaver oldstep Doubao reference."""
...
KEYFRAME = ROOT / "action-keyframes" / "02-aurora-fate-weaver-oldstep-prepare-v01.png"
IDENTITY = ROOT / "reference-source" / "02-aurora-fate-weaver-action-ready-v02.png"
OUTPUT = ROOT / "action-references" / "02-aurora-fate-weaver-oldstep-prepare-v01-1024x576.png"
CONTACT = ROOT / "action-references" / "02-aurora-fate-weaver-oldstep-direction-gate-v01.png"
...
def fit_keyframe() -> Image.Image:
    ...
    print(f"[doubao-oldstep-reference] {OUTPUT.name}: crop={crop.size} fitted={fitted.size} at {(x, y)}")
    return canvas
...
def make_contact(reference: Image.Image) -> None:
    ...
    panels = [
        ...
        (keyframe, "oldstep action keyframe"),
...
def main() -> None:
    if OUTPUT.exists() or CONTACT.exists():
        raise RuntimeError("refusing to overwrite an existing oldstep reference or gate contact")
