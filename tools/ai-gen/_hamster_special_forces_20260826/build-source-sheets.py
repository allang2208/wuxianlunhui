#!/usr/bin/env python3
"""Build accepted hamster-special-forces videos into transparent source sheets."""
...
SPEC = importlib.util.spec_from_file_location("special_forces_sprite_base", BASE_PATH)
...
VIDEO_NAMES = {
    "idle": "hamster_special_forces_idle_doubao.mp4",
    "running": "hamster_special_forces_running_doubao_v02.mp4",
    "attacking": "hamster_special_forces_attacking_doubao_v02.mp4",
    "dying": "hamster_special_forces_dying_doubao.mp4",
}
...
SPECS = (
    ...
    BASE.ActionSpec(
        ...
        (0, 8, 16, 24, 32, 38, 40, 42, 48, 56, 64, 72, 76, 80, 84, 88, 92, 100, 108, 116, 120),
...
def remove_attack_artifacts(
    ...
    """Keep actor/shotgun while deleting detached shell, watermark and gray smoke."""
    ...
    if count > 1:
        ...
        keep = labels == largest
        removed_components = (output[..., 3] > 0) & ~keep
        remove |= removed_components
        output[removed_components] = 0
...
def save_compare(cell: np.ndarray, output: Path) -> None:
    reference_sheet = Image.open(
        REPO / "assets" / "companions" / "hamster_sniper" / "idle.png"
    ).convert("RGBA")
    ...
    draw.text((12, 520), "hamster sniper / effective body reference", fill="white")
    draw.text((524, 520), "hamster special forces / matched effective body height", fill="white")
    compare.save(output)
...
def main() -> None:
    ...
    removed_pixels: dict[str, dict[int, int]] = {name: {} for name in VIDEO_NAMES}
    ...
    for spec in SPECS:
        ...
        for source_index in spec.indices:
            ...
            rgba, removed = BASE.strip_small_cutout_components(rgba, min_source_area=1000)
            ...
            print(f"[special-forces-sheet] {spec.name} BiRefNet f{source_index}", flush=True)
    ...
    report: dict[str, object] = {
        ...
        "loopScaleMode": "per-frame effective body height normalization",
        "cleanup": {
            "model": "BiRefNet-general",
            "smallComponentMinimumArea": 1000,
            "attack": "largest actor component plus bounded warm muzzle region; detached smoke, shell and watermark removed",
    ...
    for spec in SPECS:
        ...
        reference_anchor = (
            None if spec.horizontal_mode == "center-body" else BASE.body_anchor_x(rgba_frames[0])
        )
        cells: list[np.ndarray] = []
        scales: list[float] = []
        for rgba in rgba_frames:
            if spec.name in {"idle", "running"}:
                _, y0, _, y1 = BASE.opened_body_bbox(rgba)
                scale = TARGET_BODY_HEIGHT / max(1, y1 - y0 + 1)
            else:
                scale = fixed_scale
            scales.append(scale)
            cells.append(place(rgba, scale, spec.horizontal_mode, spec.vertical_mode, reference_anchor))
