# 障碍物素材统一提示词策略（2026-08-03）

目标：新障碍物（沙袋掩体、木制拒马等）与项目现有障碍物家族风格统一、彼此统一。
依据：GLM-4V 对现有道具（木桶/陶罐/骨头/铁链/火把/木材堆/石柱）的风格审计。

## 风格基准（所有障碍物提示词固定共用，不可随意改动）
...
要点：
- 画风：写实 3D 渲染（非卡通风/像素风），材质要“暗色真实质感”（木/石/帆布/金属）。
- 光源：一律左上，阴影柔和，主体底部带轻微地面阴影。
- 构图：主体居中、占画面约 60~75%，四周留白便于抠图。
- 背景：纯白（角点像素均值判定，浅灰渐变也算不合格）。
...
khaki and olive sand colors, frontal view, straight-on, slight three-quarter perspective,
facing the camera, <风格基准>
```

### 木制拒马（正面）
```text
wooden barricade made of crossed logs with sharpened pointed tips, medieval defense barrier,
weathered dark brown wood with bark and grain, frontal view, straight-on, slight three-quarter
perspective, facing the camera, <风格基准>
```

## 验收标准

1. GLM-4V 定性：主体正确 / 视角符合指定 / 无文字水印 / 无断件畸形 / 背景纯净。
2. 像素统计定量：角点背景均值接近纯白；半透明边缘灰调残留 <5%。
3. 两张新道具互相并排验收时分开单张提问（GLM 多图会串扰）。
