"""
Flux2 Fun ControlNet - Runtime Patch for ComfyUI (compat-fixed)
...
compat-fixed (2026-08-04):
- patched_forward_orig now accepts timestep_zero_index (FLUX.2 core passes it
  as a keyword argument since ComfyUI 0.30.0), fixing
  "patched_forward_orig() got an unexpected keyword argument 'timestep_zero_index'".
- When no Flux2 Fun controlnets are active, the patch delegates to the original
  core forward_orig, keeping plain FLUX.2 dev / klein generation identical to core.
- When controlnets ARE active, the core's timestep_zero_index / reference-latent
  modulation handling is replicated (vec split + modulation_dims kwargs).
"""
...
def patched_forward_orig(
    ...
    # Compatibility fix: without active Flux2 Fun controlnets, delegate to the
    # original core forward_orig so plain FLUX.2 dev / klein generation is
    # byte-for-byte identical to core (handles timestep_zero_index / references).
    if not transformer_options.get('flux2_fun_controlnets'):
        return _original_forward_orig(
    ...
    extra_kwargs = {}
    if timestep_zero_index is not None:
        modulation_dims_combined = list(map(
            lambda x: (0 if x[0] == 0 else x[0] + txt.shape[1],
                       x[1] + txt.shape[1], x[2]),
            modulation_dims))
    ...
    for i, block in enumerate(self.single_blocks):
        ...
        if ("single_block", i) in blocks_replace:
            ...
            img = block(img, vec=vec, pe=pe, attn_mask=attn_mask,
                        transformer_options=transformer_options, **extra_kwargs)
