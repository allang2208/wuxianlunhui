...
class MeshRemoteStub(nn.Module):
    ...
    def __init__(self, param_sig: dict | None = None):
        ...
        # ComfyUI 0.30's lowvram loader walks model attribute paths for every
        # state-dict key (model_patcher.partially_load / restore_loaded_backups
        # -> comfy.utils.set_attr_param). The stub is an empty module, so paths
        # like `img_attn.0.to_q.weight` fail with `'MeshRemoteStub' object has
        # no attribute 'img_attn'`. Materialize dummy submodules so the stub is
        # traversable exactly like a real block. They hold no parameters, so
        # the zero-VRAM property is preserved.
