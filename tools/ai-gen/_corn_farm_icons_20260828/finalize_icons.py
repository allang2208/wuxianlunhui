#!/usr/bin/env python3
"""Finalize the two corn-farm technology badges for runtime use."""
...
def main() -> None:
    out = PROJECT / "assets" / "ui" / "technology-icons"
    out.mkdir(parents=True, exist_ok=True)
    for name in ("corn_cultivation", "corn_standardization"):
        source = Image.open(RAW / f"{name}-raw.png")
        normalize(cut_hex_badge(source)).save(out / f"{name}.png", optimize=True)
