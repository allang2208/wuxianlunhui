...
当前exploration v16已接入，用户反馈基本达标并授权推广到其他地牢。此目录是制作来源，不是运行时UI或实机截图。
...
布局和路由以docs/dungeon-exploration-console-v9.md的当前v16节与skill/10-ui-party.md为准：固定row连续房图、40/60等比cover背景、原HUD、对称撤离与右边缘悬停栏目。概念图里的样例数值、模拟路线和旧按钮比例不是配置真源。未运行测试或运行时验证，按约定由用户测试。
