...
def build(action: str, video: Path) -> dict:
    ...
    previews = ROOT / "previews/polar-night-high-priest"
    ...
    gif = previews / f"{stem}-preview.gif"
...
def main() -> None:
    ...
    report_path = ROOT / "previews/polar-night-high-priest/preview-report.json"
    report = json.loads(report_path.read_text(encoding="utf-8")) if report_path.exists() else {}
    manifest = json.loads((ROOT / "manifest.json").read_text(encoding="utf-8"))
    video = ROOT / manifest["units"][UNIT]["actions"][args.action]["video"]
