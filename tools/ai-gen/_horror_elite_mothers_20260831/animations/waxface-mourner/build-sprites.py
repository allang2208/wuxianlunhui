"""Approved mourner sources -> BiRefNet + RIFE, with fixed camera transforms.
...
VIDEOS = {a: ROOT/'videos'/f'{a}-doubao-v01.mp4' for a in ACTIONS}
spec = importlib.util.spec_from_file_location('mourner_pack', ROOT/'producer/sprite_packing.py')
...
def prepare():
    ...
    for action in ACTIONS:
        ...
        if action in ('idle', 'walking'):
            ...
            # Keep every native palm-release frame; sparse keys during preparation/hold.
            indices = sorted(set(range(0,41,4)) | set(range(41,53)) | set(range(56,121,4)))
            # 550ms anticipation, 300ms palm extension, 650ms hold/recovery.
            # Retiming only: all original poses and their order remain unchanged.
            times = [float(np.interp(i,[0,40,52,121],[0,550,850,1500])) for i in indices]
            start,end,duration = 0,121,1500
        else:
            # Preserve kneel/fall and settled corpse; omit only redundant final still tail.
            indices = sorted(set(range(0,85,3)) | set(range(60,81)))
            times = [i/24*1000 for i in indices]
            start,end,duration = 0,85,85/24*1000
        selected[action] = dict(sourceVideo=VIDEOS[action].relative_to(ROOT).as_posix(),
            ...
            timingPolicy='Attack: source f0/40/52/121 map to 0/550/850/1500ms; other states retain source time. No pose changes.')
...
def clean(rgb, alpha):
    ...
    keep = main_mask.copy()
    # Keep nearby candle/bowl fragments; remove isolated background logo components.
...
def compose():
    ...
    for action in ACTIONS:
        ...
        if action=='attacking':
            rec['contactSourceFrame']=47
            rec['contactFrame']=2*s['sourceFrameIndices'].index(47)
            rec['contactMs']=sum(durations[:rec['contactFrame']])
        state={'walking':'walk','attacking':'attack','dying':'death'}.get(action,action)
        rec['sheet']=f'../../../../../assets/enemies/waxface_mourner/{state}.png'
        rec['textureKey']=f'enemy_waxface_mourner_{state}'
    ...
    write(BUILD/'manifest.json',dict(**{k:v for k,v in previous.items() if k not in ['actor','sourceApproval','transformPolicy','calibration','rgbaMiB','targetMiB','testsRun','runtimeVerified','actions']},actor='waxfaceMourner',sourceApproval='User: 检查。没问题以后下一个; approved mother and existing Doubao sources after source review',
...
def interpolate():
    ...
    for r in manifest['actions']:
        ...
        cmd=[sys.executable,'-X','utf8',str(ROOT/'producer/rife-spritesheet-interpolate.py'),'--sheet',str(ROOT/r['sourceSheet']),
             ...
             '--out',str(target),'--name',f'waxface-mourner-{action}','--frame-width',str(r['frameWidth']),
...
def finish():
    ...
    labels=dict(idle='待机',walking='移动',attacking='封蜡诅咒',dying='死亡')
    for n in range(96):
