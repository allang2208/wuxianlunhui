...
> Git发布范围：本页素材与游戏接线随恐怖地牢升级功能分支一同交付；运行时验收仍由用户进行。
...
- 可再生抠图/RIFE预览缓存不纳入本次Git交付；历史路径、字节与原因保留在[归档清单](sprite-build-v01/review-20260831/archive-manifest.json)。
...
- 魔攻×1.4魔法伤害，零额外击退。实际受伤且仍存活的目标减速20%持续2000ms；同名只刷新不叠层，与原通用50%致残分开。玩家/友军读取同一倍率，可被圣辉领域净化，状态免疫拒绝入库。
...
新角色类`src/entities/enemy-types/waxface-mourner.js`、独立效果`src/effects/wax-seal-effect.js`、减速入口`src/combat/wax-seal-status.js`；角色导出、Boot加载、地牢工厂和`data/public`配置已接线。玩家及AI移速各增加一次封蜡倍率；状态栏、圣辉领域净化名单与状态移除同步该新状态，不改变旧减速数值。
