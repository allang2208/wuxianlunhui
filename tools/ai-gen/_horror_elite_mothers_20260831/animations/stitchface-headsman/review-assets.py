...
configs=[json.loads((REPO/p).read_text(encoding='utf-8'))['stitchfaceHeadsman'] for p in ['data/enemy-config.json','public/data/enemy-config.json']]
errors=[];records=[]
...
result=dict(scope='User requested offline sprite/workflow review, no game modules or runtime tests',
    passed=not errors,errors=errors,actions=records,contactFrame=attack['contactFrame'],contactMs=event,
