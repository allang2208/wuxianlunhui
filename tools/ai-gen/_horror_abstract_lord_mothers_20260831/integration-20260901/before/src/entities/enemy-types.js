...
import { canStartGroundSkill } from './enemy-types/_shared/attack-shapes.js';
import { ensureRotbogBroodVisuals, findRotbogBroodPoint } from './enemy-types/_shared/rotbog-brood.js';
...
import {
    ...
    destroyVenomZones,
    updateVenomZones,
} from './enemy-types/_shared/venom-bottle.js';
import {
    createGroundWarning,
    destroyWarning,
    fireGroundShockwave,
...
class PurpleBlightAncientEnemy extends ZombieDogEnemy {
    ...
    _onCombatActionInterruptedByControl() {
        if (this._bossAction) this._cancelBossAction();
        this._queuedComboThrow = null;
    ...
    _getAttackVisualClock() {
        if (this._bossAction) {
            const duration = this._bossActionDuration;
            return { elapsed: duration - this._bossActionTimer, duration };
        }
        return super._getAttackVisualClock();
    }
    ...
    update(dt, entities) {
        ...
        if (controlled) this._queuedComboThrow = null;
    ...
    _chooseBossAction(target) {
        ...
        if (this._meleeCooldown <= 0 && canStartGroundSkill(this, target, { kind: 'sector',
            range: Math.min(Number(meleeCfg.triggerRange) || 300, Number(meleeCfg.range) || 280),
            arcDegrees: Number(meleeCfg.arcDegrees) || 150 })) {
    ...
    _startAction(kind, target, duration) {
        ...
        this._animTimer = 0;
        if (target) {
            this.rotation = Math.atan2(this._bossActionPoint.y - (this.collider?.y ?? this.y),
                this._bossActionPoint.x - (this.collider?.x ?? this.x));
    ...
    _updateBossAction(dt, entities) {
        ...
        if (this.isCombatActionBlocked() || this._bossAction !== kind) return;
        this._frozenForCast = true;
        this._animState = this._getPurpleAncientVisualState();
        this._lockToRoots();
    ...
    _resolveMeleeSweep(entities) {
        ...
        const shape = new GroundSector(
            originX,
            originY,
            Math.atan2(Math.sin(this.rotation || 0) / PERSPECTIVE_SCALE_Y, Math.cos(this.rotation || 0)),
            range,
            arc,
        ...
        for (const target of this._hostiles(entities)) {
            if (this.isCombatActionBlocked() || this._bossAction !== 'melee') break;
            if (!shape.intersectsEntity(target)) continue;
            const tx = target.collider?.x ?? target.x;
            const ty = target.collider?.y ?? target.y;
            if (!hasAttackLineOfSight(this, target, originX, originY)) continue;
...
/**
 * 腐沼小独角仙：沼泽普通池的最低阶地面昆虫。只使用配置化单体短角冲顶，
 * 不附带控制、吸血、毒素或范围技能；walking 同时承担逻辑 run。
 */
...
class ReedShadowSickleMantisEnemy extends ZombieDogEnemy {
    ...
    _onCombatActionInterruptedByControl() {
        if (this._fanSweepActive) { this._cancelFanSweep(); }
        super._onCombatActionInterruptedByControl();
    }
    ...
    _getAttackVisualClock() {
        if (this._fanSweepActive) {
            ...
            return { elapsed: duration - this._fanSweepTimer, duration };
        }
        return super._getAttackVisualClock();
    }
    ...
    _canStartFanSweep(target) {
        ...
        return canStartGroundSkill(this, target, { kind: 'sector',
            range: Math.min(Number(cfg.triggerRange) || 300, Number(cfg.range) || 280),
            arcDegrees: Number(cfg.arcDegrees) || 120 });
    }
    ...
    _startFanSweep(target) {
        ...
        this._fanSweepAngle = Math.atan2(dy, dx);
        this._fanSweepGroundAngle = Math.atan2(dy / PERSPECTIVE_SCALE_Y, dx);
    ...
    _updateFanSweep(dt, entities) {
        ...
        if (this.isCombatActionBlocked() || !this._fanSweepActive) return;
        this._frozenForCast = true;
        this._animState = 'fan_sweep';
        this.rotation = this._fanSweepAngle;
        this.vx = 0;
    ...
    _releaseFanSweep(entities) {
        ...
        for (const target of candidates) {
            if (this.isCombatActionBlocked() || !this._fanSweepActive) break;
            if (!target || target === this || !target.active || target._isDead
...
class BlackKingCobraEnemy extends BrownSnakeEnemy {
    ...
    _onCombatActionInterruptedByControl() {
        if (this._venomSprayActive) { this._cancelVenomSpray(); }
        super._onCombatActionInterruptedByControl();
    }
    ...
    _getAttackVisualClock() {
        if (this._venomSprayActive) {
            ...
            return { elapsed: duration - this._venomSprayTimer, duration };
        }
        return super._getAttackVisualClock();
    }
    ...
    _canStartVenomSpray(target) {
        ...
        const range = Math.min(
            Math.max(0, Number(this._venomSprayCfg.triggerRange) || 450),
            Math.max(1, Number(this._venomSprayCfg.range) || 420)
        );
    ...
    _updateVenomSpray(dt, entities) {
        ...
        if (this.isCombatActionBlocked() || !this._venomSprayActive) return;
        this._frozenForCast = true;
        this._animState = 'attack';
        this.rotation = this._venomSprayAngle;
        this.vx = 0;
    ...
    _releaseVenomSpray(entities) {
        ...
        for (const target of values) {
            if (this.isCombatActionBlocked() || !this._venomSprayActive) break;
            if (!target || target === this || !target.active || target._isDead
...
class MedusaEnemy extends ZombieDogEnemy {
    ...
    _onCombatActionInterruptedByControl() {
        if (this._petrifyingGazeActive) { this._cancelPetrifyingGaze(); }
        super._onCombatActionInterruptedByControl();
    }
    ...
    _getAttackVisualClock() {
        if (this._petrifyingGazeActive) {
            ...
            return { elapsed: duration - this._petrifyingGazeTimer, duration };
        }
        return super._getAttackVisualClock();
    }
    ...
    _resolveTailSweepAoe() {
        ...
        for (const target of candidates) {
            if (this.isCombatActionBlocked() || this._pendingThrust !== pending || !pending.active) break;
            if (!target || target === this || !target.active || target._isDead
    ...
    _canStartPetrifyingGaze(target) {
        ...
        if (this._attackTimer > 0 || this._attackAnimTimer > 0
            || this._pendingThrust?.active || this._attackTelegraphTimer > 0) return false;
        const range = Math.min(
            Math.max(0, Number(this._petrifyingGazeCfg.triggerRange) || 560),
            Math.max(1, Number(this._petrifyingGazeCfg.range) || 500)
        );
        if (distanceToEntityShape(target, this.x, this.y) > range) return false;
        const ignore = target._coverSeg ? { segs: new Set([target._coverSeg]) } : null;
        return hasRangedLineOfSight(this, target, 8, ignore);
    ...
    _updatePetrifyingGaze(dt, entities) {
        ...
        if (this.isCombatActionBlocked() || !this._petrifyingGazeActive) return;
        this._frozenForCast = true;
        this._animState = 'petrify';
        this.rotation = this._petrifyingGazeAngle;
        this.vx = 0;
    ...
    _releasePetrifyingGaze(entities) {
        ...
        for (const target of values) {
            if (this.isCombatActionBlocked() || !this._petrifyingGazeActive) break;
            if (!target || target === this || !target.active || target._isDead
...
class WerewolfKingEnemy extends ZombieDogEnemy {
    ...
    _getAttackVisualClock() {
        if (this._stealthPhase === 'howling' || this._stealthPhase === 'fadingOut') {
            const duration = Math.max(100, Number(this._stealthCfg.howlDurationMs) || 1800);
            return { elapsed: this._stealthPhase === 'howling' ? duration - this._stealthTimer : duration, duration };
        }
        return super._getAttackVisualClock();
    }
    ...
    _getPhaserOptions() {
        ...
        if (this._pounceState !== 'idle') {
            // 飞扑只由技能时钟推进；父类普攻状态与 Phaser 自播都不能重置动作帧。
            const layout = this._getFrameLayout('pounce');
            const totalFrames = Math.max(2, Number(layout.frameCount) || 49);
            const prepareFrames = Math.max(1, Math.min(totalFrames - 1,
                Number(this._pounceCfg.prepareFrames) || 20));
            const prepareMs = Math.max(1, Number(this._pounceCfg.prepareMs) || 1000);
            const chargeMs = Math.max(1, Number(this._pounceCfg.chargeMs) || 1000);
            const elapsed = this._pounceElapsed || 0;
            options.frame = elapsed < prepareMs
                ? Math.min(prepareFrames - 1, Math.floor(elapsed / prepareMs * prepareFrames))
                : prepareFrames + Math.min(totalFrames - prepareFrames - 1,
                    Math.floor((elapsed - prepareMs) / chargeMs * (totalFrames - prepareFrames)));
    ...
    _onCombatActionInterruptedByControl() {
        this._pendingStealthStrike = null;
        if (this._pounceState !== 'idle') this._endPounce();
        if (this._stealthPhase !== 'idle') this._cancelStealthCast();
        super._onCombatActionInterruptedByControl();
    }
    ...
    _canStartPounce(target) {
        if (!this._isReadyForSkill() || !target?.active || target._isDead
            || target.hittable === false || !(target.hp > 0) || this.hasStatusEffect('bind')) return false;
        if (!canMeleeShareSurface(this, target) || !hasAttackLineOfSight(this, target)) return false;
    ...
    _startPounce(target) {
        ...
        // 蓄力一开始就锁定落点；结束时不再追着闪避目标重新转向。
        this._pounceTargetPos = { x: target.collider?.x ?? target.x, y: target.collider?.y ?? target.y };
        this._pounceDamaged = false;
    ...
    _updatePounce(dt) {
        ...
        const prepareMs = Math.max(1, Number(this._pounceCfg.prepareMs) || 1000);
        const chargeMs = Math.max(1, Number(this._pounceCfg.chargeMs) || 1000);
        const totalMs = prepareMs + chargeMs;
        const previousElapsed = this._pounceElapsed;
        this._pounceElapsed = Math.min(totalMs, previousElapsed + dt);
        this._frozenForCast = true;
        this._animState = 'pounce';
        this._attackTimer = this._attackAnimTimer = Math.max(0, totalMs - this._pounceElapsed);
        this.vx = 0;
        this.vy = 0;
        this.isMoving = false;
        if (this._pounceState === 'prepare') {
            this._pounceTimer = Math.max(0, prepareMs - this._pounceElapsed);
            if (this._pounceTargetPos) {
                this.rotation = Math.atan2(
                    this._pounceTargetPos.y - this.y,
                    this._pounceTargetPos.x - this.x
                );
                this._lastHorizontalFacing = Math.cos(this.rotation) < 0 ? 'left' : 'right';
            }
            if (this._pounceTimer > 0) return;
            this._startPounceCharge();
        }

        // 跨越蓄力边界的这一帧只消费剩余飞行时间，不丢帧也不重复位移。
        const motionDt = this._pounceElapsed - Math.max(prepareMs, previousElapsed);
        const target = this._pounceTarget;
        if (!target?.active || target._isDead || !(target.hp > 0)) this._pounceSpeed = 0;
        const fromX = this.x;
        const fromY = this.y;
        const distToEnd = this._pounceTargetPos
            ? Math.hypot(this._pounceTargetPos.x - this.x, this._pounceTargetPos.y - this.y)
        ...
        let intendedY = fromY;
        if (distToEnd > 10 && this._pounceSpeed > 0) {
            const step = Math.min(this._pounceSpeed * motionDt / 1000, distToEnd);
        ...
        if (!this._pounceDamaged && target?.active && target.hittable !== false
            ...
            if (this._pounceState === 'idle') return;
            if (result.hit) {
                // 命中仍立刻停位移，但剩余落地/收势帧继续播完，不跳回待机。
                this._pounceSpeed = 0;
            }
        }

        if (motionBlocked || distToEnd <= 10) this._pounceSpeed = 0;
        this._pounceTimer = Math.max(0, totalMs - this._pounceElapsed);
    ...
    _startPounceCharge() {
        ...
        this._pounceState = 'charge';
        this._pounceSpeed = 0;
        if (this._pounceMotionCancelled || this.hasStatusEffect('bind')) return;
        if (!target?.active || target._isDead || !(target.hp > 0)) {
            return;
        }
        const aim = this._pounceTargetPos;
        if (!aim || !canMeleeShareSurface(this, target)) return;
...
class RotbogRhinocerosBeetleKingEnemy extends ZombieDogEnemy {
    ...
    _updateSummon(dt) {
        ...
        if (!this._summonReleased && elapsed >= duration * releaseFrame / frameCount) {
            ...
            // 释放时资源失效则消耗本次事件并正常收势，不停在释放帧无限等待。
    ...
    _releaseSummons() {
        ...
        const radius = Math.max(
            4,
            Number(enemyConfigData.smallRotbogRhinocerosBeetle?.collisionRadius) || 20
        );
...
/**
 * 腐潮蟾祖·格罗玛：沼泽地牢最终首领。基础攻击使用长距离单体舌刺；
 * 舌扫、坐压、毒雾与召唤震鸣各自绑定专属动画和独立接触帧。66%/33%
 * 生命阈值会强制施放一次召唤震鸣并推进阶段，召唤物继续使用既有零收益契约。
 */
class RotTideToadAncestorEnemy extends ZombieDogEnemy {
    ...
    _onCombatActionInterruptedByControl() {
        if (this._toadAction) { this._cancelToadAction(); }
        super._onCombatActionInterruptedByControl();
    }

    _getAttackVisualClock() {
        if (this._toadAction) {
            const duration = this._toadActionDuration;
            return { elapsed: duration - this._toadActionTimer, duration };
        }
        return super._getAttackVisualClock();
    }
    ...
    updateWhilePetrified(dt) {
        super.updateWhilePetrified(dt);
        if (this.active && !this._isDead) updateVenomZones(this, dt, this._lastToadEntities);
    }
    ...
    _canStartToadAction(skill, target) {
        ...
        if (skill === 'summonCroak') return hasAttackLineOfSight(this, target);
        if (skill === 'bodySlam') {
            const radius = Number(cfg.radius) || 310;
            return canStartGroundSkill(this, target, { kind: 'ellipse', radiusX: radius, radiusY: radius * PERSPECTIVE_SCALE_Y });
        }
        return canStartGroundSkill(this, target, { kind: 'sector', range: Number(cfg.range) || 430,
            arcDegrees: Number(cfg.arcDegrees) || 160 });
    }

    _startToadAction(skill, target, phaseUp = false) {
        const cfg = this._toadSkillCfg[skill] || {};
        // 阶段转换可照常播放，幼虫资源失败不能把Boss卡在转换门槛。
    ...
    _updateToadAction(dt) {
        ...
        if (this.isCombatActionBlocked() || !this._toadAction) return;
        this._frozenForCast = true;
    ...
    _resolveToadPoisonBelch() {
        ...
        if (this.isCombatActionBlocked() || this._toadAction !== 'poisonBelch') return;
    ...
    _resolveToadCroak() {
        ...
        if (!this.isCombatActionBlocked() && this._toadAction === 'summonCroak') this._releaseToadSummons();
    }
    ...
    _damageToadShape(shape, cfg, skillId, isMelee) {
        ...
        for (const target of this._collectToadTargets()) {
            if (this.isCombatActionBlocked() || !this._toadAction) break;
            if (!shape.intersectsEntity(target)) continue;
            const targetX = target.collider?.x ?? target.x;
            const targetY = target.collider?.y ?? target.y;
            if (!hasAttackLineOfSight(this, target, originX, originY)) continue;
    ...
    _findToadSummonPoint(registry, summonRadius, index, count) {
        ...
        return findRotbogBroodPoint(this, registry, summonRadius, baseAngle, 150 + (index % 2) * 42);
    }
...
class BlackBearEnemy extends ZombieDogEnemy {
    ...
    _onCombatActionInterruptedByControl() {
        const frozenState = this._animState;
        super._onCombatActionInterruptedByControl();
        this._cancelDruidCast();
        // 作废尚未释放的法术，但石化画面仍保留已显示的姿势。
        if (this.hasStatusEffect?.('petrified')) this._animState = frozenState;
    }

    _getAttackVisualClock() {
        if (this._castActive) {
            const duration = this._spellCfg.castDuration ?? 1000;
            return { elapsed: duration - this._castTimer, duration };
        }
        return super._getAttackVisualClock();
    }

    _syncAnimationFootOffset(layout = this._getFrameLayout()) {
        // 父构造器也会调用：从 config 读取，不能依赖尚未初始化的 _transformCfg。
        const textures = this.config?.textures || {};
        const druid = !this._isTransformed && !this._isTransforming;
        const referenceCell = (druid ? textures.druid : this._isTransforming ? textures.transformations : textures)?.referenceCell ?? 512;
        const spriteSize = druid ? (this.config?.transform?.druidRender?.spriteSize ?? 167.3)
            : (this.config?.render?.spriteSize ?? 180);
        const height = layout.frameHeight ?? referenceCell;
        this.footOffsetY = ((layout.footY ?? height) - height / 2) * spriteSize / referenceCell;
    }

    _syncPetrifiedBodyAnchor(sprite) {
        const key = sprite.texture?.key || '';
        const textures = this.config?.textures || {};
        const layout = key.startsWith('enemy_black_bear_transform_')
            ? textures.transformations?.frameLayouts?.[key.slice('enemy_black_bear_transform_'.length)]
            : key.startsWith('enemy_black_druid_')
                ? textures.druid?.frameLayouts?.[key.slice('enemy_black_druid_'.length)]
                : textures.frameLayouts?.[key.slice('enemy_black_bear_'.length)];
        if (!layout) return;
        if (Number.isFinite(layout.anchorX)) {
            sprite.x += (layout.frameWidth / 2 - layout.anchorX)
                * Math.abs(sprite.scaleX) * (sprite.flipX ? -1 : 1);
        }
        sprite.y += this.footOffsetY
    ...
    update(dt, entities) {
        ...
        const controlled = this.isCombatActionBlocked();
        if (controlled && this._castActive) this._onCombatActionInterruptedByControl();
        ...
        if (!controlled && this._castActive && !this._isTransforming && !this._isTransformed) {
    ...
    _getPhaserOptions() {
        ...
        if (this._isTransforming) {
            ...
            const duration = this._transformCfg.duration ?? 2000;
            options.manualFrame = true;
            options.frame = attackFrameAt(this._getFrameLayout(), duration - this._transformTimer, duration);
            options.dynamicSpriteSize = true;
            return options;
        }
        if (this._isTransformed) {
