...
ACTIONS = ("idle", "crawling", "attack", "dying")

def build(action: str, stem: str | None = None) -> dict:
    stem = stem or f"{action}-v01"
    ...
    return {
        ...
        "gifPlayback": "repeats entire source for viewing; seamless cycle not yet selected" if action in ("idle", "crawling") else "once, holds final frame",
...
def main() -> None:
    ...
    parser.add_argument("--stem", help="Explicit candidate filename stem for one action")
    args = parser.parse_args()
    if args.stem and len(args.actions) != 1:
        parser.error("--stem applies to exactly one action")
    report_path = ROOT / "previews" / "preview-report.json"
    report = json.loads(report_path.read_text(encoding="utf-8")) if report_path.exists() else {}
    manifest_path = ROOT / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    for action in args.actions:
        report[action] = build(action, args.stem)
