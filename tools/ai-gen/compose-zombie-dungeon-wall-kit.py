...
def build_diamond_preview(wall: Image.Image, closed_gate: Image.Image, geometry: dict, output: Path):
    ...
    gate_scale = 0.8  # six 128x64 cells / Blender base span
    gate = closed_gate.resize((round(closed_gate.width * gate_scale), round(closed_gate.height * gate_scale)), Image.Resampling.LANCZOS)
    gate = gate.transpose(Image.Transpose.FLIP_LEFT_RIGHT)
    ...
    p0 = gate_geo["base"][0]
    gate_x = opening_a[0] - (gate_geo["canvas"][0] - p0[0]) * gate_scale
    gate_y = opening_a[1] - p0[1] * gate_scale
    ...
    draw.text((46, 38), "Zombie dungeon standard: 12-cell diamond / shared corners / 6-cell portcullis", fill=(226, 221, 211, 255))
