...
def apply_far_hook_fade(cell: Image.Image, index: int, layout: dict) -> Image.Image:
    """Fade only the distant hook/cable; preserve body pixels and frame geometry."""
    fade = layout.get("farHookFade")
    if not fade:
        return cell
    anchor_x = layout["anchorXByFrame"][index]
    start = anchor_x + fade["startOffsetX"]
    end = anchor_x + fade["endOffsetX"]
    if end <= start:
        raise ValueError("far hook fade requires endOffsetX > startOffsetX")
    progress = np.clip((np.arange(cell.width, dtype=np.float32) - start) / (end - start), 0, 1)
    opacity = 1 - progress * progress * (3 - 2 * progress)
    pixels = np.asarray(cell).copy()
    old_alpha = pixels[..., 3].copy()
    pixels[..., 3] = np.rint(old_alpha * opacity[None, :]).astype(np.uint8)
    pixels[(old_alpha > 0) & (pixels[..., 3] == 0), :3] = 0
...
def repack(source: Image.Image, output_path: Path,
    ...
    overrides = {record["frame"]: record for record in result.get("frameOverrides", [])}
    ...
    for index in range(frame_count):
        ...
        if index in overrides:
            # 原视频原生中点替换已确认破损的RIFE帧；不重生成姿态、不改动作时钟。
            cell = Image.open(REPO / overrides[index]["path"]).convert("RGBA")
            if cell.size != (new_w, new_h):
                raise ValueError(f"native body repair f{index}: layout mismatch")
        cell = apply_far_hook_fade(cell, index, result)
...
def main() -> None:
    ...
    anchor_path = ROOT / "runtime-anchors.json"
    anchors = json.loads(anchor_path.read_text(encoding="utf-8"))["actions"] if anchor_path.exists() else {}
    effect_path = ROOT / "runtime-alpha-effects.json"
    effects = json.loads(effect_path.read_text(encoding="utf-8"))["actions"] if effect_path.exists() else {}
    for action, filename in SOURCES.items():
        ...
        if action in anchors:
            # 校准值对应最终裁格；重建若更换布局，须重新标定，不能带着旧锚点接入。
            calibration = anchors[action]
            for key in ("frameWidth", "frameHeight", "frameCount"):
                if result[key] != calibration[key]:
                    raise ValueError(f"{action}: ground anchor calibration no longer matches {key}")
            for key in ("anchorXByFrame", "footYByFrame"):
                result[key] = calibration[key]
            if "frameOverrides" in calibration:
                result["frameOverrides"] = calibration["frameOverrides"]
        result.update(effects.get(action, {}))
    ...
    manifest = {
        ...
        "anchorCalibration": "runtime-anchors.json in the source production folder",
        "alphaEffects": "runtime-alpha-effects.json in the source production folder",
        "actions": report,
    }
