#!/usr/bin/env python3
"""Build direct-review GIFs and contact sheets for crossbowman source videos."""
...
SOURCES = {
    ...
    "moving": "moving-doubao-v01-loop.mp4",
...
def decode(path: Path) -> tuple[list[Image.Image], float]:
    with av.open(str(path)) as container:
        ...
        frames = [Image.fromarray(frame.to_ndarray(format="rgb24"), "RGB") for frame in container.decode(stream)]
    return frames, fps
...
def build(name: str, source_name: str) -> dict[str, object]:
    ...
    sample_step = 6 if name in {"attacking", "dying"} else 12
