...
## 普通攻击一段跟手 + 方向性运动模糊（2026-08-03 落地，用户验收通过；08-03 二轮复核修正）
...
- 当前轨迹（握把，即减去剑柄补偿后的手位）：f0 `(20.8,-24.8)` → f8 `(-41.2,-78.5)`
  （蓄力顶）→ f16 `(18.0,-78.5)`（前举）→ f22+ `(87.0,2.7)`（下劈到前伸手收势）。
...
- **HAND_PX 最终值（2026-08-03 二轮复核）**：
  f0(196,118) f1(185,118) f2(150,115) f3(128,116) f4(282,115) f5(310,116) f6(330,120) f7(425,278)。
  依据：GLM-4V 逐帧读数 + 剪影复核（f3 的 x128 与独立剪影读取一致，非头顶；
  头顶在 x235-297）；与旧配置偏差表吻合（旧 f2 握把(171,113) 恰在拳(150,115) 右侧 21px——
  即"脱手"根因；f6 旧锚点右/下大幅偏离已修）。
- **实机复核（冻结逐帧）**：`tools/cdp-sword-hold-frames.mjs` 手动定格 8 帧
  （play+timeScale=0 冻结帧 + 占位 tween 保 attacking 状态机 + 40s 进度时钟反推），
  逐帧握把落点与锚点误差 <0.25px；对无武器截图做剪影采样，
  8 帧握把落点全部落在角色手/臂像素上（on-sprite 71%~100%），无一帧悬空。
