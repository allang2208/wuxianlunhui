class SpecialAttackSystem {
    ...
    trigger(targetX, targetY, entities) {
        ...
        this.player._specialAttackCooldown = 5000; // 5秒冷却
        ...
        const beam = new NightFlameBeamEffect(centerX, centerY, this.player._specialAttackAngle, 56, clampedLength, 3000);
        this.player._specialAttackBeam = beam;
        EffectManager.add(beam);
        // 显示范围提示（从武器贴图中心开始，使用截断后的长度）
        if (Game.showAttackRange) {
            EffectManager.add(new AttackRangeEffect(centerX, centerY, this.player._specialAttackAngle, clampedLength, 56, 'triangle', 3000, 0.4, true));
        }
    }
    ...
    update(dt, entities) {
        ...
        // 3000ms后结束，触发复位动画
        if (this.player._specialAttackTimer >= 3000) {
