...
// - 参考露娜远程攻击模式：AimHelper.lead 提前量瞄准「目标贴图中心」，
//   攻击动画第 10 帧（launchFrame）发射投射物，每 2s 一支，60 物理伤害；
...
export class HamsterShooterAI {
    constructor(shooter) {
        this.m = shooter;
        this.cfg = shooter.aiConfig || {};
        this._decisionTimer = stableAiPhase(shooter, this.cfg.decisionMs ?? 120);
        this._attackTimer = 0;
        this._attackInterval = this.cfg.attackInterval ?? 2000;
        this._attackDamage = this.cfg.attackDamage ?? 60;
        this._attackRange = this.cfg.attackRange ?? 600;
        this._engageRange = getRtsAcquireRange(shooter);
    ...
    /**
     * 每帧入口（由 HamsterShooter.update 调用）。
     * @param {number} dt 毫秒
    ...
    /** 发射投射物：AimHelper.lead 提前量瞄准目标贴图中心（参考露娜） */
    _fireProjectile() {
        const m = this.m;
