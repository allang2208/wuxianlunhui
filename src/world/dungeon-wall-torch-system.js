...
export const DungeonWallTorchSystem = {
    ...
    spawn({ rooms = [], corridors = [], gates = [], avoidPoints = [] } = {}) {
        ...
        if (style?.wallDecorations !== 'horrorRelics') return 0;
        const cfg = style?.wallTorches;
        const scene = typeof window !== 'undefined' ? window.__phaserScene : null;
        const torch = ISO_WALL_GEO.torch;
        if (!cfg?.enabled || !scene || !scene.textures.exists(torch.tex)) return 0;
        ...
        if (style.wallDecorations === 'horrorRelics') {
            this._placeDecorations(candidates, horrorWallDecorConfig);
        }
