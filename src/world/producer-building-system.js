...
class ProducerBuildingPanel extends BasePanel {
    ...
    _renderIndustrialEconomy(st, modBox, building, cfg, population) {
        ...
        if (!snapshot) return;
        const presentation = {
            oil_power_plant: { icon: '🛢️', title: '燃油发电运行档案' },
            cannery: { icon: '🥫', title: '罐装生产运行档案' },
            trading_company: { icon: '🏢', title: '贸易公司运行档案' },
        }[building._economyType];
        const upgradeTechnologyName = Object.keys(cfg.modules || {})
            .map((moduleId) => TechnologySystem.getUnlockRequirementLabel('upgrade', moduleId))
            .find(Boolean) || '对应建筑研究';
