...
class ProducerBuildingPanel extends BasePanel {
        ...
        if (isEconomy) {
            ...
            if (cfg.economyType === 'weather_forecast') {
                ...
                modBox.innerHTML = `${this._renderWorkforceControls(b)}${moduleEntries.length
                    ? '<div class="economy-panel-heading"><span>高能实验室升级项目</span><span class="economy-panel-meta">需要科技“高能实验室标准化”</span></div>'
    ...
    _showAdvancedResearchTip(moduleId, event) {
        ...
        showBuildingUpgradeTooltip(`
            ...
            <div style="margin-top:2px;">${unlocked ? (maxed ? '已达到最高等级' : `升级费用：${cost.gold} 金币 + ${cost.energy} 能源`) : `需要科技：${technologyName || '高能实验室标准化'}`}</div>
