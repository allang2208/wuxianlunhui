...
export const PopulationEconomySystem = {
    ...
    startAdvancedResearchUpgrade(building, moduleId) {
        if (building?._economyType !== 'advanced_research'
            || !building?._cfg?.upgradeProject
            || Object.keys(building?._cfg?.modules || {}).length <= 0) {
            return { ok: false, reason: '该科研设施没有本栋升级项目' };
        }
        const module = building._cfg.modules?.[moduleId];
        ...
        if (building._advancedResearchUpgrade) return { ok: false, reason: '已有科研设施项目正在升级' };
