...
export function getMainHubArchitectureTextureEntries() {
    ...
    const entries = [];
    // Continuous hub paving is preprojected and loaded from its own source.
    for (const source of mainHub?.floor?.textureSources || []) {
        if (source?.key && source.path) {
            entries.push({ key: source.key, path: source.path });
        }
    }
    ...
    if (backdrop?.enabled === true && backdrop.textureKey && backdrop.assetPath) {
        ...
        for (const variant of Object.values(backdrop.variants || {})) {
            if (variant?.textureKey && variant.assetPath) {
                entries.push({ key: variant.textureKey, path: variant.assetPath });
            }
        }
    }
