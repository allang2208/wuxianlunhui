...
export const SceneManager = {
    ...
    async switchScene(sceneId, player) {
        if (this.isLoading || this.currentScene === sceneId) return;
    ...
    _saveMainSceneState() {
        ...
        // 只保存普通场景合集的树木
        this._mainTrees = WallSystem.trees ? WallSystem.trees.filter(t => t.sceneGroup === 'normal').map(t => ({ ...t })) : [];
    },
        ...
        for (let t = 0; t < monsterTypes.length; t++) {
            for (let i = 0; i < 5; i++) {
                ...
                do {
                    ...
                    const radius = 3000 + Math.random() * 1000; // 3000~4000px随机半径
                    ...
                    attempts++;
                } while (distToPlayer < 2500 && attempts < 10); // 如果太近则重试，最多10次
