...
export const PartyUI = {
    ...
    _slotHtml(member, isPlayer) {
        if (isPlayer) {
            return `<div class="party-slot party-slot--player" data-player="1" title="玩家（主角）">
        ...
        return `<div class="party-slot party-slot--member" data-companion="${member.id}" title="${member.name} · ${member.title}">
