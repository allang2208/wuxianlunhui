...
class WeaponTransform {
    ...
    static getAttackAnimOffset(weaponType, anim, s) {
        ...
        if (weaponType === 'pistol' || weaponType === 'deagle') {
            const isDeagle = weaponType === 'deagle';
            if (anim.state === 'windup') {
                recoilX = -s * (isDeagle ? 0.02 : 0.04) * easeOutQuad(anim.timer / wa.windupMs);
            } else if (anim.state === 'swing') {
                const st = anim.timer / wa.swingMs;
                recoilX = s * (isDeagle ? 0.05 : 0.1) * (1 - st);
                recoilY = (Math.random() - 0.5) * (isDeagle ? 1 : 3) * (1 - st);
            } else if (anim.state === 'recover') {
                const rt = anim.timer / wa.recoverMs;
                recoilX = -s * (isDeagle ? 0.02 : 0.04) * (1 - rt);
            }
        } else if (weaponType === 'pkm' || weaponType === 'akm' || weaponType === 'qbz191' || weaponType === 'qjb201') {
            if (anim.state === 'windup') {
                recoilX = -s * 0.03 * easeOutQuad(anim.timer / wa.windupMs);
            } else if (anim.state === 'swing') {
                const st = anim.timer / wa.swingMs;
                recoilX = s * 0.08 * (1 - st);
                recoilY = (Math.random() - 0.5) * 4 * (1 - st);
            } else if (anim.state === 'recover') {
                const rt = anim.timer / wa.recoverMs;
                recoilX = -s * 0.03 * (1 - rt);
            }
            ...
            if (anim.state === 'windup') {
                recoilX = -s * 0.04 * easeOutQuad(anim.timer / wa.windupMs);
            } else if (anim.state === 'swing') {
                const st = anim.timer / wa.swingMs;
                recoilX = s * 0.12 * (1 - st);
                recoilY = (Math.random() - 0.5) * 5 * (1 - st);
            } else if (anim.state === 'recover') {
                const rt = anim.timer / wa.recoverMs;
                recoilX = -s * 0.04 * (1 - rt);
            }
    ...
    static getWeaponSize(weaponType, scaleOverride = null) {
        ...
        } else if (weaponType === 'pistol' || weaponType === 'deagle') {
