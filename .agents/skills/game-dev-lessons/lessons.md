> 历史教训按需参考；其中实机、CDP、测试和构建步骤只在用户明确授权后执行，不覆盖根AGENTS。公共规则以根SKILL指向的现行合同为准。
...
### 42.2 关门只做有限门段的局部排障
- 只在`close()`瞬间处理真正重叠单位，开门及关闭稳态均不推人；持续改坐标会与普通移动解析冲突。
- 点到门段的距离必须包含端点以外的切向分量，不能把无限延长线的法向距离当作有限门段距离；端帽附近应径向向外推，不把单位吸向门端。
- 碰撞中心偏移、解析后位移上限/同侧/离段检查、Collider同步、墙顶/楼梯/桥接隔离及侍从去重，统一见第08卷“城门关门排障：有限门段与单位坐标契约”。
- 无合法局部落点保持原位；本轮代码修复未运行测试，历史贴线/双门探针不能充当当前版本验收，交付状态见`docs/gate-unstick-publication-2026-08-31.md`。
...
- **用户确认源视频步态后优先保留完整轨迹**（2026-08-26 仓鼠方阵）：精灵表腿部异常若来自过度抽帧，
  不要重新搜索短“标准周期”或用几何对齐重写轨迹；在完整源时长上固定步长采样，按原生fps同比降关键帧率，
  再做含回绕的2×插帧。这样既保留自然离散步态，也保持原视频墙钟节奏；最终仍需验腿部相位与接缝步幅。
...
- **高阶子类的兵种识别顺序**（2026-08-26 仓鼠方阵）：高阶单位继承低阶实体会同时保留父子标记，
  `getUnitKind()`必须先判子类再判父类；否则生产工厂虽然能生成，升级、兵线和快照却会记成低阶兵种。
  父类AI/RTS/承伤可直接复用，死亡动画时长等素材相关生命周期参数由子类在调用父流程后覆盖。
...
- **阵型职责**：框选复数单位下令时，以选中组中心到目的地的方向作为前进轴，在等距地面 `u/v`
  坐标生成以点击点为几何中心的方阵；不要用黄金角/随机散点冒充编队。
- **槽位分配**：方阵间距取最大单位半径的两倍再加余量；中心槽优先，每个槽领取当前位置最近的
  未分配单位，减少交叉换位。玩家本体仍走专用控制器，不能塞进友军 `_command`。
- **性能边界**：最近槽贪心 O(n²) 只在下令瞬间执行；不要增加逐帧队长跟随、全组重排或整队共用
  A*。单位继续独立做落点投影、路径与避障，允许窄口打散并在终点重组，也不强制统一移速。
- **表面边界**：楼梯/墙顶不套地面方阵；继续逐单位解析高架路线并服从入口 FIFO，避免方阵槽把
  多单位重新压回同一楼梯入口或墙中心。
- **入口队列与分离不能双重管制**：`_surfaceNavWaiting` 已由 FIFO 阻止穿越 Portal，等待者继续参与
  友军分离会把已获权者推出入口，造成队列反复回靠和超时。队列态应只豁免相关友军 unit-vs-unit
  推挤；墙、建筑、楼梯侧边、防坠线与方向许可不能一并绕过。
- **队列超时是无进展看门狗，不是总排队时长**：长队持续有人获权时，后续单位的顺位已在前进，应从这次
  真实进展重新计时；否则最后 1～2 人会固定在轮到前超时重排。但重复 `request`/`touch` 不得续期，
  否则完全卡死的队列会永不触发恢复。
- **强制超时哨兵不能被真值回退吞掉**：将进度时间设为 `0` 后若以 `value || now` 读取，下一帧会把 `0` 当缺省值，
  从而每帧重置超时起点。显式高架路线会因此在退出 FIFO 后永久卡在旧入口。应对时间值做有限数检查，并以
  `now - timeout` 显式表示“下一帧立即恢复”。

## 61. 长柄武器不计主体体量：先去细杆再对齐躯干（2026-08-23）

- **整框 Alpha 不是长武器角色的体量**：草叉、长枪、法杖和旗帜会抬高 alpha bbox 顶边；直接用整框高度求缩放会把
  装备长度错当成人物身高，反向“补偿”时还会把主体放大近一倍。
- **主体轮廓定量法**：对有效帧的二值 alpha mask 做形态学开运算，阈值必须大到能真正切断杆件，不能固定照抄。
  旧 running 用 5px 得到的 156.5px 仍混入较粗草叉；2026-08-24 结合新 idle 复核，11px 开运算得到站立主体约
  129px。按仓鼠战士约 79.029 世界像素的站立可见高度反算 `displaySize:313.664397`，同时避开全 Alpha 误差与
  原 `460` 的过度放大。
- **脚线必须同批缩放**：用去杆后仍稳定的中位底线反算 `footOffsetY`（本次为 `58.199449`），并让
  `spriteOffsetY = -footOffsetY`；碰撞半径、
  `bodyHeight`、攻击范围与武器长度都不随视觉缩放改动。

## 62. 视频动作安全框：先缩镜头留方向性余量，再生成横向扑击（2026-08-24）

- **问题**：棕熊攻击参考母图主体宽约80%，静止时看似有余量；扑击第40~64帧鼻嘴触到
  豆包原视频右边界，源像素已经丢失。后续 BiRefNet 抠图并放进640宽帧格后虽然
  `edgeHitFrames=[]`，也只是把已经被切掉的轮廓缩进了格子，不能恢复鼻、嘴和爪。
- **动作安全框**：横向扑击/冲锋/前刺/长武器动作派生专用参考图，不改正式 idle 母版。
  主体宽度控制在画面50%~55%；面向右时中心位于35%~40%，前方空白至少20%、后方至少12%、
  上下至少10%（向左动作镜像）。提示词明确保持 wide framing，禁止自动 zoom/crop/reframe/track。
- **高幅扑击要继续缩镜头**：黑熊在50%宽参考图下仍于扑击峰值把鼻口和前掌推到右边界；
  改用35%主体宽、中心约28%的攻击专用参考图后，峰值右侧仍保留203px。50%~55%只是普通横向动作
  的起点，不是固定答案；应按动作峰值位移继续缩小并后置主体，直到源视频关键轮廓不触边。
- **不同安全构图必须逐动作归一尺寸**：若 attack 为35%宽而 idle/walk 为约78%宽，不能共用一份
  源像素缩放，否则攻击状态会突然缩小。应以每个动作首个中立帧的有效身体高度分别反算缩放，
  再统一到同一运行时目标高度；动作内部仍固定该缩放，禁止逐帧追高导致呼吸式缩放。
- **两层边界验收**：先检查原视频关键帧主体 bbox，关键轮廓触碰原视频任一边即重生；再检查
  透明精灵表的 blank/edge/semi/transRGB。前者证明视频没裁主体，后者只证明重建没有二次裁切。
- **调整边界**：若源视频仍完整，只是精灵表碰边，可增大非方形 cell 或调整锚点；若原视频已
  触边，则裁剪信息不可逆，禁止靠缩图、平移、补白或AI修补伪装，必须用安全框参考图重新生成。

## 63. 怪物普通近战：接敌距离与接触帧伤害盒必须分离（2026-08-25）
...
- **声明与消费分开核对**：派生怪物仅有 `basicMelee.timeline` 不会自动启用通用有向近战，必须设置 `basicMeleeResolver: true` 或由类显式启用 `_usesDirectedBasicMelee`，并确认移动、战斗和攻击三处实际走入同一合同。`impactReach` 只量攻击者根点到可见接触前沿，目标 footprint 在形状相交时只计一次，禁止把目标半径再次叠进 reach 造成超距命中。
...
- 方形母图进入 H3 首帧前只能等比缩放并补成 16:9；直接改宽高会把角色横向拉伸，后处理统一比例无法修复。
- 在原视频中用中立躯干 bbox 对比宽高比、面积和中心轨迹；异常放大、镜头推近或根节点漂移应判废重生，不能靠透明表逐帧缩放掩盖。
- 冲锋若由状态机推进 Collider，视频只负责原地蓄力、旋转和冲刺姿态，重建时固定中心/脚线；只有明确采用表内位移时才保留源根运动，二者禁止叠加。
