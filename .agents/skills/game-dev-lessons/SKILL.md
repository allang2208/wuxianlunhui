...
- ⚠ **只挡人、不推"开门人"（2026-08-16 二修）**：`opening` 态与 `open()` 不要推人——
  开门时门洞已放行，推人会把靠近/正通过门口的玩家反复弹开 = "卡在门上"
  （两门衔接处被两扇门同时弹更明显）。推人只在 `closing`（碰撞段刚注册）与 `closed`
  （双门 face 叠 51px 的重叠区残留单位，怪物 GATE-WAIT 会永久等待）时执行。
- 双门接缝验证：两门共线，单位在接缝重叠区被任一门沿法线推出后即同时离开两条门线
  （50px > 42 阈值），无冲突。
