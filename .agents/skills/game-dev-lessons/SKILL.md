...
- 正解：`GATE_SNAP_OVERLAP=4px`——门面线端点只叠 4px，两根端柱近于叠合，
  `max 面线 y + 12` 深处盖浅处 = 视觉单柱无缝；与掩体"端帽叠合互盖"同思路。

### 42.2 开关门必须推开站线单位
- 关门瞬间门洞碰撞段（`_gateHole`）注册，站线上的玩家/怪物被卡：怪物走
  GATE-WAIT 原地等待 → 波次卡死；开门动画期间栅栏滑动也会蹭到单位。
- 正解：`unstickUnitsFromGate(A, B, halfThick)`——开/关瞬间 + 动画期间，把脚点距面线
  < halfThick+16 的单位沿面线法线推出到安全距离（实测贴线 1px → 50px）；
  排除防御结构（塔/掩体/基地），玩家与敌对单位一视同仁。
