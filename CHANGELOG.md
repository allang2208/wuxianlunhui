...
### 对话：本地豆包 Seedance 全自动视频转精灵管线打通（2026-08-24）
- `ai-asset.py` 新增 `--provider doubao`，统一接入本地豆包客户端的 Seedance 2.0 Mini；支持自动启动本地客户端、选择模型/画幅/时长、上传参考图、逐字回读提示词、单次提交、等待生成、下载 MP4、多候选命名和 provenance JSON。自动化仅开放 loopback CDP，不读取 Cookie/Token，不杀普通豆包进程；提交状态不明、额度不足或风控时停止，避免重复消耗额度。
- 伊莉丝 running、walking、idle 已完成端到端实战：视频经 BiRefNet 清除背景/阴影/角标，按自然周期截帧、脚线与下身根节点对齐，生成透明 spritesheet 并接入游戏。豆包无像素级尾帧锁、无影提示不可靠、严格侧向不能只靠文字等限制已沉淀到 `skill/02-ai-asset-pipeline.md`、`tools/ai-gen/WORKFLOW.md` 和通用人物动作提示词模板；肖像保护命中时移除人物专名仅重试一次。未在本轮重新运行测试或游戏验证。
