...
### 对话：袭击栏更名为时间进度栏并优化事件显示（2026-08-23）
...
- 时间进度栏从480px扩展至最高560px并接入冷钢主题变量；事件列表只在事件、位置或可见倒计时变化时重建，不再随10Hz HUD刷新反复创建和解码图标。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户检查1080p/窄屏布局、事件端点、密集预报、键盘焦点和入侵支援状态。
