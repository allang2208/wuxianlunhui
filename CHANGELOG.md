...
### 功能/美术：僵尸地牢统一单格黑砖房、实体宝箱房与高级 Boss 房（2026-08-27）
- 初级/中级/高级僵尸战斗房与整数格通道统一采用 `worldBlock1x1`：128×64 格心墙块、共享转角、六格门洞和当前墙样式 `block/gate` 真源；V4 黑砖墙与16帧锈铁升降闸作为正式运行时素材，长门按浅/中/深三段排序，避免浅端覆盖邻墙。
- 末房中央新增12格边长实体黑砖宝箱房，使用独立六格闸门，不占用战斗房出口的全局门实例；完整保留60秒倒计时、战斗后开门、超时封闭、刷怪排除区与原奖励数值。高级集合体 Boss 房同步迁移到单格墙环，玩家从 RB 门内侧入场、Boss 在 LT 对侧生成，离场归还全部格网门上下文。
- 素材生成固定 `world122-building-v4`、Klein Depth、12步结构3张、48步精修2张；保留结构/精修候选与早期目录中的 Blender 几何、控制图和分层 mask 作为 V4 最终合成真源，仅清理两个 `.blend1` 自动备份。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查墙角、通道、宝箱门、Boss 出口与碰撞对齐。
