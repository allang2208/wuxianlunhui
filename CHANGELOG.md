...
- 雪原初级地牢新增 `combatRoom.wallConstruction:"worldBlock1x1"` 专用构建模式：房间四边严格按世界位面 128×64 的 1×1 footprint 铺设独立墙块，普通房为20格/边并按房间档位等比例扩展；每块墙携带独立边界线段和 depth，房间改用2:1菱形，避免旧长墙整体拉伸、转角重叠和单深度遮挡。
