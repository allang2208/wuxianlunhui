...
### 功能/美术：补齐普通至传说轻机枪系列（2026-08-27）
- 新增 `weapon31—weapon38`：普通 RPD、优质 M249 SAW / Ultimax 100 Mk8、普通 MG42、史诗熔核轻机枪、神话奇点织机，以及原创传说“天穹测绘者 / 冥约颂炮”；同步接入权威武器数据、双份装备与改造配置、攻击/弹药/自动枪族、商店、世界生成、附魔、开发工具、ADS、独立音效和防御塔枪管资源。
- 史诗能量轻机枪与熔核轻机枪的专属改造重构为三个独立二选一组，跨组效果可完整组合；奇点织机围绕有限弹射与分支协议构建改造；两把传说枪分别使用星图锚点分支和血/骨/烬三印裁决，追加伤害通过事件来源与次数边界阻断无限循环。
- 新增并归档整套武器、图标、12枚史诗改造图标、12枚传说改造图标、三段式武器音效、塔载枪管裁片、现实枪械参考、原创生成提示词/原图/后处理脚本与武器规格。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点验证开火、过热、弹射/裁决组合、塔载一致性、ADS枪口方向及改造图标显示。

### 美术/规划：八类仓鼠募兵母图定稿归档与骑兵热兵器路线收口（2026-08-27）
- 用户确认仓鼠弩手、长弓、突击、重机枪、骑兵、翼骑兵、侦察游骑兵和动力爆矛重骑兵八张母图；统一为右向三分之四正交视角、纯白无影背景和同系列写实PBR游戏资产。弩手仅收窄脸部而保持身材；两级骑兵分别继承轻骑/骑士原线仓鼠身份并统一换猫，三级轻骑改为无蓝羽、带耳机的侦察步枪手，三级重骑改为俄式重型防爆衣、凯夫拉猫甲、防弹插板与双动力助推爆矛。
- `task-index.json` 固化八个活动键、直接编辑源图/提示词、占位替代关系、用户批准状态、最终总览和骑兵血统预览；批准后删除36个被否体型、旧马匹、侦察车/坦克、过期联系图与提示词，共约35.8MB，只保留39个定稿及最小可编辑链文件。
- 知识库补充多兵种母图活动索引/废案收口、骑手与猫双血统身份锁、局部修改硬锁及“热兵器不等于机械化”边界；`TODO.md` 登记未来按豆包生成待机、移动、攻击、死亡并完成正式精灵表与运行时接入。当前 `runtimeIntegrationActive=false`，未修改生产配置、科技、数值、AI或存档；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户后续验收动画与接入。

### 功能/美术：仓鼠方阵插帧动画与运行时注册（2026-08-26）
- 将用户最终确认的军团大盾、双刃斧仓鼠方阵四动作导入运行时：待机24帧@16fps、稳步行走62帧@12fps、攻击41帧@24fps、死亡31帧@20fps。行走表按完整5.17秒源视频每4帧取样，再由RIFE补齐循环中间帧，保留源视频的缓慢腿部轨迹与正确朝向的双刃斧；同步生成兵种图标，四套动画统一为512×512单元格透明精灵表。
- 新增`hamster_phalanx`实体与双份同步配置，继承仓鼠盾卫成熟的近战AI、RTS指挥、自动防御、碰撞与死亡解绑合同；粗设为480生命、45伤害、2.4秒攻击间隔、75移动速度，攻击动画第13帧结算。补齐预载/动画、通用单位工厂、升级登记、步兵分类、兵线统计、UI图标与世界-122后台模拟映射。
- 正常招募配置依赖尚未落库的整阶招募框架，本提交不单独加入`unitTypes`，避免旧生产器绕过二级门禁；共享工作树中的方阵槽位继续等待该框架所属批次一并提交。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户测试。

### UI：右上经济面板新增可展开仓容与产出详情（2026-08-26）
- 经济面板折叠态继续与时钟保持74px等高，首行金币/能源/食物不变；第二行改为对称的“兵力 已用/上限”和“工作 已用/上限”，军事人口与经济岗位人口各走独立占用线路，只共享房屋提供的人口容量。
- 按用户修正，展开交互保持经济面板当前宽度不变，只让详情从缩略面板底部像卷轴一样向下展开并轻微位移淡入，边框与阴影同步加强；收起时向上卷回原74px缩略版，右侧时钟保持锚定，不另开浮层，并继续尊重系统的减少动态效果偏好。
- 容量条颜色按用户指定进一步收口：只有共享仓库“总占用”条使用绿→青→黄→红危险度色带，并随占用宽度逐段显露；能源、食物、兵力和工作人口仍保留原资源语义色，不改变容量计算或其他进度条规则。
- 微调经济面板与顶部天气/时间进度栏的展开箭头：箭头容器现在占满各自的小按钮并做水平、垂直双向居中，展开旋转继续以格子正中心为轴；按钮尺寸、挂接位置和卷轴展开逻辑不变。
- 根据用户浏览器反馈，经济缩略面板和时钟的共享宽度由不明显的262px收紧到250px，现保持74px等高、250px等宽；展开详情、内容容器和缩略栏全部锁定100%内宽并移除详情的负横向边距，长容量文案不再把下半部分撑出上方外壳。
- 经济与天气/时间面板的箭头由受字体基线影响的`⌄`字符改为7px CSS折线，水平垂直居中并围绕自身中心旋转；减少动态效果模式同步关闭新的箭头旋转过渡。
- 修复经济缩略面板仍比时钟略高：此前外壳使用自动高度，折叠详情的内边距/分隔线可能在`0fr`下留下像素级高度；现在折叠态硬性锁定74px最大高度，展开时才以240ms缓动向下释放最大高度，宽度继续固定250px，外接箭头不参与外壳高度。
- 新增底部向下箭头，可将面板向下展开为详细经济面板；详情显示当前位面仓库总物理占用、能源/食物各自距离满仓的剩余量、兵力与工作人口容量条，以及金币/能源/食物近5秒实际净产出。仓容直接读取`EnergyManager`活动仓库，并按每座仓库的能源/食物压缩倍率折算；金币仍按现有背包单格无限堆叠规则明确显示“无上限”，不伪造容量条或另造第二套经济公式。
- 新增无循环依赖的`EconomyHudSystem`只读人口桥，展开按钮同步维护键盘焦点和`aria-expanded`，动态容量条只更新内联宽度，其余视觉继续归冷钢主题；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户测试。

### 文档：科研与科技树长期数值合同（2026-08-26）
- 将当前73节点、71240点折算成本固化为“约三分之一内容”的可重算基线，建立最终41万—57万科研成本预算；新增科技一律按折算后成本、可触达时的有效科研和单项预计分钟数定价，普通中期/高级/时代门槛/终局工程分别使用1500—5000、5000—15000、15000—25000、25000—40000点档位，并要求最终分支成本优先弥补当前仅4.3%的军事线。
- 确立科研吞吐三阶段：当前继续使用12点全效/20%超额/36有效上限；中期与后期只在对应新增成本已落地且关键科技解锁时，才允许分别提高到16/22%/45与20/25%/60，禁止只抬高吞吐或只堆高成本。新科研建筑同步约束单座产值、岗位、房屋占比、每位面/全局数量上限和集群上限；当已达当期速度上限90%时，后续设施应提供分支专长或吞吐升档，不再单纯叠加全局点数。
- 根`SKILL.md`已新增“科研长期扩展数值合同”索引，完整规范落在`skill/07-world122-defense.md`，包含成本预算、分支占比、时间档位、吞吐分期、新建筑填值与七项强制检查清单；并明确全部计划科技、科研建筑及升级开发完成后，必须再做一次完整科技树、全部产出、产业集群、岗位住房和五至六位面极限的收口数值审计。本次只建立后续开发标准，未修改现有运行数值或功能代码。

### 美术：全位面核心传送门统一放大至4×4（2026-08-26）
- 将共享`portal`配置从旧2×2规格统一调整为4×4：逻辑半径改为256、`footprintCells=4`，运行时显示改为`512×450 / footOffsetY 225`，显式`visualFootprint`严格映射512×256；沙漠、雪原、林地与地牢遗迹位面的核心传送门因此共用同一尺寸、占地和接地标定，主神空间同源传送门也保持一致。
- 移除仅沙漠位面存在的`coreVisual`重复覆盖，避免其在4×4逻辑碰撞上继续继承旧2×2视觉比例；所有核心传送门继续由`ProducerBuilding`统一执行`applyBuildingFootprint + setupStructureDepth`，未修改传送目的地、位面门禁、耐久、入侵结算、出售保护或其他玩法。
- 重新生成接地拟合清单中的传送门条目；该不可建造核心不生成建筑栏缩略图，原始`portal.png`及光照派生贴图内容未变，仅调整游戏内显示标定。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由玩家重点检查四个位面的传送门体量、贴地、前后/左右前角遮挡及相邻建筑交叠。
- 按建筑定稿瘦身规则清理18个确定废案（九个哈希一致的包根重复副本、三份`.blend1`自动备份、三栋玩家手工回灌后废弃的自动运行时图及其元数据），共释放约6.27 MiB；accepted raw/body/玩家处理稿、正式运行时候选、最终模型、Depth、预览与入库元数据均保留。其余未逐项证明可恢复的历史候选未做递归删除。

### 美术：森林神殿半木石哥特—印加/阿兹特克稿正式入库（2026-08-26）
- 按玩家最终确认，采用森林神殿12步v02与48步v01；从确认raw以边缘连通绿幕阈值100重新提取透明主体，保留内部绿松石彩窗、苔藓、火焰和深色木构，再执行4px绿幕去边、单像素内缘修复和小于8px孤立组件清理。终稿为801×849 RGBA、单一连通主体、透明区RGB清零，正式运行时显示标定为`512×543 / footOffsetY 269`。
- `jungle_temple`继续使用4×4逻辑footprint与半径256；显式`visualFootprint`更新为严格映射512×256的`centerYRatio 0.759669 / widthRatio 1 / depthRatio 0.471455`，未修改占格、碰撞、寻路、出兵、造价、耐久、位面门禁或玩法。正式贴图、128×64缩略图、接地拟合清单、silhouette/projection/height/normal光照派生图、资产哈希、候选清单和入库元数据已同步。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由玩家重点检查scene10中的接地、4×4体量、道路衔接、前后/两侧遮挡、火盆边缘以及昼夜结构阴影。

### 修正：三座4×4位面建筑采用玩家手工处理稿重新入库（2026-08-25）
- 以各候选目录内玩家处理完成的`explorer_camp_refine_v01.png`、`snow_castle_refine_v02.png`和`desert_mansion_refine_v02.png`替换上一轮自动body结果；三张1024² RGBA均保留玩家Alpha，不再执行自动分割，仅清除透明区残留RGB、修复白色matte软边并紧身裁剪。三栋`alpha>16`均为单一连通主体，探索者营地旧稿的碎石Alpha组件与雪原城堡离体装饰组件不再存在。
- 重新标定4×4运行时尺寸：探索者营地`512×449 / footOffsetY 222`、雪原城堡`512×544 / 269`、沙漠官邸`512×472 / 234`，三者继续严格映射512×256逻辑footprint；同步更新正式贴图、资产哈希、候选/安装元数据、接地拟合清单、营地与官邸缩略图及三栋光照派生图。森林神殿继续暂停，未改贴图；未修改碰撞、占格、寻路、生产或玩法。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查三栋在对应位面的接地、体量、道路衔接、前后遮挡和昼夜阴影。

### 美术：三座位面特色建筑完成4×4正式入库（2026-08-25）
- 按玩家最终确认，将探索者营地48步v01、雪原城堡48步v02、沙漠官邸48步v02提升为正式运行时贴图；三套被接受的raw、透明body、预览、generation元数据、紧裁运行时候选和安装记录均已归档到仓库。森林神殿继续暂停，未生成48步稿、未替换运行时资产。
- 三栋统一按4×4逻辑棱柱使用512px显示宽度，并分别从最终紧裁画布反算为营地`512×408 / footOffsetY 202`、城堡`512×543 / 269`、官邸`512×470 / 233`；显式`visualFootprint`严格映射512×256，未修改占格、碰撞、寻路、造价、生产、位面门禁或玩法。营地继续保留单塔与不对称档案翼/补给廊，城堡和官邸保持左右成对结构。
- 更新探索者营地与沙漠官邸128×64建筑卡缩略图，并为三栋增量生成silhouette/projection/height/normal光照派生图；接地清单生成器同步按`footprintCells`区分2×2的256×128与4×4的512×256，避免4×4资产清单回退。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查三栋在对应位面的体量、脚线、道路衔接、前后遮挡与昼夜阴影。

### 对话：仓鼠长戟手RIFE插帧与角色导入强制门禁（2026-08-25）
- 将仓鼠长戟手待机24帧、奔跑15帧、攻击20帧、死亡16帧透明母表通过离线RIFE v4.6执行RGB/Alpha分通道2×插帧；待机/奔跑包含末→首回绕并分别输出48/30帧，攻击/死亡禁止回绕并输出39/31帧。RGB透明区先做最近前景色填充，插后清零透明RGB，中间帧按相邻原帧底边均值整像素复校；原关键帧逐像素保留在偶数索引，正式表继续使用512×512格和民兵同款脚线/主体体量，长戟不参与缩放，攻击横向轨迹与死亡终帧保持原样。
- 新增通用`tools/ai-gen/rife-spritesheet-interpolate.py`，统一生成未插帧母表备份、插帧正式表、GIF/联系图和量化报告。SKILL总索引、AI资产管线以及怪物/友军添加工作流同步加入强制门禁：以后新导入或重做的怪物、NPC、侍从、士兵、工人角色动画都须先确认正确关键姿态，再执行循环N→2N或一次性N→2N−1插帧，并同步重映射接触/发射/声音等关键帧。当前仍只完成美术资产优化，没有创建单位实体、配置、AI、生产入口或任何战斗/经济数值；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查四态动画观感。
- 用户确认长戟兵继续作为预留美术资产保留：不得接入兵营、草屋、靶场、骑兵学校或其他任何出兵建筑，也不创建运行时单位注册；制作索引固定为`assetStatus=reserved_art_only`、`runtimeIntegrationActive=false`和`producerBuildingIntegration=false`，后续只有收到新的明确指示才可解锁。

### 优化：全量友军仓鼠动画RIFE补帧与黑闪门禁（2026-08-25）
- 对世界-122已登记的战士、盾卫、民兵、射手、斥候、火枪手、牧师、骑士、轻骑兵、探险家、赏金猎人、骆驼骑兵以及矿工执行统一2× RIFE 处理；静态单帧待机/投射物保持原样，循环动作N→2N，一次性动作N→2N−1，原关键帧固定在偶数索引，播放帧率同步×2。命中、出膛、施法释放与矿工定格帧按同一时间点映射，未修改生命、伤害、移速、攻击间隔、射程或生产等数值。
- 定位长戟兵奔跑黑闪来自RIFE在中间帧发明不透明近黑像素，而不是空帧或透明背景。通用工具升级为版本化时间异常检测：对远离两侧关键帧原有暗部的近黑块，以两侧最近前景色场重建并二次清除，避免黑块，也避免用复制前帧造成伪2×重复。正式报告新增`pipelineVersion`、修复像素、残留黑块和关键帧保真字段；怪物/友军添加分卷及统一资产工作流同步固化该门禁。
- 本轮保留每个单位的未插帧母表、逐动作JSON报告、透明GIF和联系图，正式资产继续保持512×512单格；长柄武器只参与安全格宽，不计入主体缩放。盾卫与民兵原图由另一沙箱会话占用，未结束对方进程或强制覆盖，改为整套写入同目录`*_rife.png`并让配置统一指向该版本，避免半套新旧混用；旧文件暂时保留为未引用来源。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点实机检查各兵种循环缝、命中/出膛时刻、死亡末帧与快速运动中的黑闪。

### 对话：推击废案清理与知识库收敛（2026-08-25）
- 删除未跟踪的 `_skill_upgrade_20260825` 实验目录（多轮推击调参候选、GIF/联系表、RIFE 中间帧、图标重复源图）以及5个硬编码该临时目录的一次性生成/分层/预览脚本，约清理150MB；保留正式运行时推击精灵图、最终技能图标、无人机资源和已隐藏的实现。
- `skill/03-player-weapon-anim.md` 沉淀可换装双手枪械近战挂载边界：先定枪口朝前→调转枪托→前顶→收回的关键姿态，用当前离散人物帧的主/副握点解算真实枪械 Sprite，按阶段显式切前/后景深度，禁止用补帧掩盖错误拓扑。`skill/05-skills-combat.md` 沉淀 `hidden + disabled` 的可逆搁置合同，并同步更新根索引和 `TODO.md`。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证；本次仅做废案清理、文档收敛与 Git 交付。

### 对话：推击技能暂时搁置与全入口隐藏（2026-08-25）
- 推击现以 `hidden + disabled` 数据标记暂时搁置：技能网格、详情页、通用技能列表与技能等级调试下拉框均不再显示；快捷栏拒绝新绑定，并会清除热更或旧会话遗留的绑定。
- 保留推击全部战斗逻辑、动画配置与正式运行时精灵图；组件触发层额外禁用，避免旧入口绕过 UI 释放。实验候选和一次性调参资料已在后续废案清理中移除，可复用结论已收敛到 SKILL/TODO。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户实机确认技能页和快捷栏不再出现推击。

### 对话：怪物普通近战攻击时间轴与接触帧判定升级（2026-08-25）
- 通用普通近战新增逐怪 `basicMelee` 合同：AI 接敌/刹停使用 `approachReach`，伤害只使用更贴合精灵接触姿态的 `impactReach + width`；攻击时间轴按动作 `durationMs/frameCount` 推进，在 0-based `activeFrames` 覆盖或跨越时复查锁定主目标、承载面、墙体和有向矩形，替代全怪共用的 188ms 前摇/250ms 挥击伤害时序。长帧跳过窗口时会补判一次，未迁移怪物仍保留旧时序。
- 首批迁移普通僵尸、胖子僵尸、僵尸犬、棕熊和熊德鲁伊变熊后的普通攻击，并按各自精灵表接触姿态配置不同命中帧；普通僵尸逻辑突进由100px收敛为35px，接触帧只按当前脚点重锚命中盒，仍锁定起手方向与主目标。普通僵尸与胖子僵尸攻击期间不再追踪转向；胖子攻击音改在配置接触帧播放。僵尸犬及其熊类派生补齐攻击期间移动锁定计时，避免贴图攻击时继续滑行。
- 碰撞编辑器的怪物近战调试会显示起手/命中两套判定框、失败原因及实际动画帧；`data/enemy-config.json` 与 `public/data/enemy-config.json` 已同步。专属扑击、多段连击、范围技能、伤害数值、冷却和动画资产均未改动。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点实机检查五类怪物贴脸/极限距离/横向擦边、接触帧伤害同步、目标退出后的空挥、普通僵尸短推进撞墙，以及熊德鲁伊变身前法术与变身后普攻边界。
- 已将攻击方式分类、标准 `basicMelee` 配置模板、接触帧/双距离/推进与锁向规则，以及贴脸、极限距离、横向擦边、后撤、隔墙和长帧六场景验收门禁写入怪物新增标准工作流；以后新增近战怪物不得只填写单一 `attackRange`，范围/位移技能和远程施法也必须选择各自独立合同。

### 对话：地牢宝箱房双状态正式贴图入库（2026-08-25）
- 复核发现两态虽然同为192px画布显示宽度且Alpha底线仅差1px，但开启态底座在源图中仅325px、闭合态408px，开箱后实体箱体会视觉缩小；运行时改为分态规格：闭合192px/originY 0.75，开启241px/originY 0.78015，使下半箱体等宽，并把两态可见底边都锁在逻辑点下方约26.625px。只调整 Sprite 视觉尺寸与锚点，不改宝箱坐标、靠近范围、奖励、碰撞或生命周期。
- 将确认的闭合 refine v02 与开启 refine v02 清理版通过 `build-dungeon-chest-runtime-assets.py` 预乘 Alpha 等比缩放为512×512，正式覆盖 `assets/terrain/chest_closed.png` 与 `chest_opened.png`；两态 Alpha 底边分别455/456，运行时再按各自底座跨度设置显示宽度与 origin，不改宝箱坐标、深度、靠近范围、倒计时、奖励或碰撞。
- 宝箱房移除旧 `chest_open_anim` 九帧表预载，改为复用同一 Sprite：140ms淡出闭合态、切换开启态后260ms淡入；过渡 tween 登记在 `_chest.openAnim`，清理时先停止再销毁，避免切场/离场后残留显示对象。旧 `assets/terrain/chest_open_anim.png` 已从运行时资产中删除，可由 Git 恢复。
- 入库元数据记录已接受源图、运行时尺寸、Alpha bbox、origin与过渡方式；`skill/06-dungeon-scene.md` 同步更新双状态生命周期合同。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查闭合体量、接地、开箱淡出/淡入、奖励落点、超时淡出和离场清理。
- 用户明确授权后，按 accepted v02 精确保留白名单清理两个生成目录：共删除144个未选候选、联系表、淘汰稿及可再生中间层（约111.09MiB），闭合态与开启态各保留19个文件，包括 `.blend` 模型、模型预览/Depth、提示词与控制图、入选 structure/refine v02 原图/主体/预览及生成元数据；已删除内容未受 Git 跟踪，不能由 Git 直接恢复，但可由保留的模型、控制图、提示词与种子重新生成，运行时贴图和入库元数据未受影响。

### 对话：地牢宝箱房闭合/开启 48 步精修候选（2026-08-25）
- 按用户确认的闭合 v02 + 开启 v02 进入标准低重绘阶段，各生成3个 1024×1024 精修候选；固定48步、Depth 0.75、denoise 0.30、CFG 3.5、Euler/simple，同一 Blender Depth 和选中的12步原图继续锁定箱体比例、锁板位置、四脚接地点及58°开盖状态。
- 开启态三张图均继承了 init image 后铰链处的细绿线；新增 `repair-local-green-spill.py`，只在显式局部矩形内替换绿色主导 RGB 为最近的不透明非绿主体色，并逐像素保持 Alpha 不变，避免全局去绿在灰色钢板上制造针孔。原始 body/preview 保留，另输出 `*_body_seam_cleaned.png`、`*_preview_seam_cleaned.png` 和清理后联系表。
- 视觉复核建议最终配对仍为闭合 refine v02 + 开启 refine v02（开启态使用 seam-cleaned body）；闭合 v02 另通过既有 Depth 遮罩移除3个独立单像素 Alpha 杂点，原始候选同样保留。当前只完成48步候选与局部清理，未覆盖运行时贴图、未接入宝箱房代码，也未运行测试、构建、浏览器探针或游戏运行时验证，待用户确认最终配对后再执行入库与运行时替换。

### 对话：地牢宝箱房闭合/开启 12 步结构候选（2026-08-25）
- 依照标准 Blender→同机位 Depth→ComfyUI 结构候选流程，为 `dungeon_chest_closed` 与 `dungeon_chest_open` 各生成5个 1024×1024 候选；统一使用12步、Depth 0.78、CFG 3.5、Euler/simple，并保留原图、抠底、锚定、预览、提示词和生成元数据。
- 首轮闭合候选暴露建筑风格词串入小型道具的问题，已把该批35个过程文件移入 `rejected_architecture_leak_20260825/` 留档，并为 `assetClass:"prop"` 增加独立渲染合同，避免半木结构、窗户和建筑基座污染宝箱。复跑后闭合 v01/v02/v03/v05 与开启 v01～v05 均保持暗钢/古金、四脚、锁板和侧提环语义；建议配对为闭合 v02 + 开启 v02。
- 开启候选后铰链处仍有细绿线；尝试全局 `removeAllGreen` 会在灰色钢板上产生针孔，已撤回该方案，待选中后在48步/最终清理阶段做局部处理。当前仅完成12步候选，未进入48步、未替换运行时贴图，也未运行测试、构建、浏览器探针或游戏运行时验证。

### 对话：地牢宝箱房闭合/开启双模型（2026-08-25）
- 复用主神空间仓库旧宝箱的四脚、箱体、半圆拱盖、铰链父级与可编辑分件，提炼为共享 `_build_treasure_chest()` 装配；新增 `dungeon_chest_closed` 和 `dungeon_chest_open` 两套 manifest，保持同一190×142×78箱体与202×152×62盖体，仅由58°开盖参数切换状态，避免两态结构漂移。
- 按地牢宝箱房现有 `chest_open_anim.png` 首尾帧重做深灰金属/古金主题：移除白色大理石、蓝宝石与蓝色内腔，补充盖面太阳花徽章、双端拱肋、前部大锁板/钥匙孔、连续卷草纹、侧面下垂提环和纯黑空腔；闭合/开启模型均已输出可编辑 `.blend`、30°正交预览与同相机 Depth。
- 当前只完成建模和模型预览，未进入12步/48步候选生成，也未替换 `assets/terrain/` 正式贴图或修改宝箱房运行时代码。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定待用户确认两个模型后再进入候选出图阶段。

### 对话：熊系动画废案清理与沼泽分阶怪物池规则沉淀（2026-08-25）
- 清理棕熊、黑熊和黑袍德鲁伊生成目录中已被正式版本淘汰的旧视频、旧母图、过程截图和重复中间表，共55个未跟踪文件、约19.65MiB；保留正式母图、正式视频及元数据、提示词、构建脚本、manifest、最终精灵表/GIF和全部运行时素材，未触碰任何 `.blend` 建模资产。
- `skill/06-dungeon-scene.md` 补充遭遇白名单阶级匹配合同：`poolKeys` 默认保留跨阶级语义，只有显式 `matchPoolRanks:true` 才按 `enemy-config.rank` 匹配 normal/elite/lord/boss；同时记录阶级缺项会回退 family/默认池的风险与双份配置、总表同步要求。未运行测试、构建或游戏运行时验证。

### 对话：沼泽地牢四类林地怪物按阶级入池（2026-08-25）
- 沼泽初级、中级、高级的普通、精英和独立领主遭遇统一加入 `blackWolf / redWolfKing / brownBear / blackBear` 白名单，并启用 `matchPoolRanks`：普通槽只在黑狼与棕熊中随机，精英槽只生成红狼王，领主槽只生成显示名为“熊德鲁伊”的 `blackBear`；现有逐波数量、房间数、精英概率、奖励、事件与解锁规则均未改动。
- 地牢工厂补充红狼王永久警戒构造，并让白名单阶级匹配成为显式可选协议；未开启该开关的既有遭遇继续保留原先跨阶级占位语义。地牢要素表及其生成器同步显示实际阶级分池。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查三档沼泽普通波的黑狼/棕熊随机、精英槽红狼王与领主/Boss尾波熊德鲁伊。

### 对话：黑熊领主正式更名为熊德鲁伊（2026-08-24）
- `blackBear` 怪物的正式显示名称由“黑熊”改为“熊德鲁伊”，双份怪物配置同步；内部 ID、工厂、动画资源键、属性和战斗机制均保持不变。未运行测试或游戏运行时验证，按约定由用户检查血条、图鉴及其他名称展示入口。

### 对话：黑熊领主改为德鲁伊起手、半血变熊（2026-08-24）
- 黑熊领主的初始形态调整为黑袍德鲁伊；生命首次降到50%时播放现有“人变熊”2秒动画，变身期间继续保留90%减伤，完成后恢复满生命并按原配置将最大生命、六维、攻防和常规移速提升50%。基础1400生命、等级12、六维、300移速以及变身强化倍率均未调整。
- 德鲁伊阶段退出旧暗林仪式和近战链，改为720施法距离内按冰锥→闪电→火球循环施法；三种法术统一播放1000ms `attacking` 动画并在56%进度释放，受眩晕、冻结或恐惧时中断未释放施法。冰锥/火球复用通用弹道与墙体、承载面、命中特效链，闪电复用锁定、传导、感电与眩晕链；所有伤害、范围、冷却和节拍集中在双份 `enemy-config.json#blackBear.attackSkills`。
- 变熊后恢复原有方向性单目标近战与50%概率2.5倍暴击，碰撞体、攻击距离和死亡动画随当前形态切换；图鉴说明、攻击类型和技能条目同步更新为“三系魔法→半血变熊→熊王暴击”。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查出生形态、三法术循环/攻击动作释放帧、半血人变熊衔接、回满强化和熊形近战暴击。

### 对话：黑熊晋升两阶段领主并强化基础属性（2026-08-24）
- 黑熊由等级10精英晋升为等级12领主，正式字段同步改为 `type:"领主" / rank:"lord"`；基础生命900→1400，六维由58/36/48/24/32/12提升为68/42/58/32/42/16。按怪物属性唯一公式，基础物攻55、物防107、魔攻37、魔防60、暴击18%、暴击抗性58，图鉴综合战力约37级；领主阶级同时接入既有领主经验、掉落、名称色和斩杀/眩晕豁免规则。
- 保留与棕熊一致的300配置移速、半血触发、2秒90%减伤、回满、1.5倍强化、50%概率2.5倍暴击及暗林仪式节奏；二阶段最大生命为2100，六维为102/63/87/48/63/24，常规移速450。未修改动画、碰撞、攻击范围、冷却或仪式参数。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查领主标识/掉落、半血前后生存强度、攻击压力与仪式叠加后的实际节奏。

### 对话：黑熊与黑袍德鲁伊双向变身动画（2026-08-24）
- 使用 MiniMax H3 双端锁定分别生成“黑熊→黑袍德鲁伊”和“黑袍德鲁伊→黑熊”两段独立124帧视频，不以倒放复用。两端直接取现有正式母图，并在1344×768共享画布中固定中心、脚线和游戏内可见体型比例；正向表现黑熊下蹲起立、前肢转手臂、黑毛转兜帽长袍，反向表现德鲁伊伏地、袍服羽肩转黑毛、四肢落成熊足，均无切镜、重复身体、额外肢体或触边。
- 标准 BiRefNet 管线将每段有效变形区间密采样为20帧、5×4、768×512透明精灵表，并强制真实尾帧收口；两段统一固定缩放0.768328与脚线y=500，不逐帧拉齐熊/人高度。两套成品均为空帧0、触边帧0、透明区RGB残留0，运行时资源写入 `assets/enemies/black_druid/transform_to_{druid,bear}.png`。
- 黑熊半血2秒阶段已改为播放正向变身动画，不再用熊 idle 等待后瞬切；反向动画同步完成预载、配置和动画注册，但现有怪物没有返祖条件，因此暂不触发。生成端点、双向提示词、视频、联系图、GIF、构建脚本及双份manifest保存在 `tools/ai-gen/_black_druid_20260824/`。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查正向2秒播放、末帧切德鲁伊idle的体型/脚线衔接、左右翻转和反向动画预览。

### 对话：性能采样体系与实时太阳阴影结构优化（2026-08-24）
- 交互开发面板性能页升级为可切换采样窗口的完整 Markdown 报告：同时输出帧耗时/原始帧间隔分位数、互斥 CPU 分项、工作量计数器与慢帧样本，能够区分持续提交成本、周期性重建尖峰和未埋点/GPU/调度间隔。
- 针对荒漠位面“太阳移动+结构阴影”爆卡完成结构优化：静态投射体改为非渲染句柄，统一结构阴影层先做保守预裁切，再按画质简化扫描轮廓与 capsule 段数；Earcut 只在脏帧执行并缓存 `fillTriangle` 命令，补充视口前后剔除、原始/简化顶点、三角形、命令数和重建耗时计数器。用户同场景第一轮报告由约49 FPS改善到约144 FPS，后续轮次反馈已基本达标。
- 新增独立 `skill/14-performance-optimization.md`，汇总采样判读、纯视觉裁切、实时阴影、AI/寻路预算、多位面后台数据化、聚合入侵、UI/粒子/Phaser 4 filters 和升级条件；原领域分卷继续保留具体业务契约。未运行测试、构建、浏览器探针或游戏运行时验证，本轮运行时依据来自用户导出的性能报告与实机反馈。

### 对话：已探索视野盲区允许建造（2026-08-24）
- 位面建造门禁由“完整占地必须当前可见”调整为“完整占地必须已经探索”：`VISIBLE` 与暂时失去视野的 `EXPLORED` 灰雾区域均允许建造，战争黑雾 `UNEXPLORED` 仍拒绝；敌人、掉落物、动态情报和RTS目标仍保持实时视野门禁，不受本次调整影响。
- `FogOfWarSystem` 新增严格多边形 `isPolygonFullyExplored()`；建造预览、连续建造和扣费前最终校验统一改用该接口，继续覆盖2×2建筑及外围道路/田地预约格、单格墙/道路、四格门、楼梯全部段、墙门线段与通用圆形占地。任何实际伸入黑雾的正面积都会拒绝，提示改为“战争黑雾覆盖区域无法建造”。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查灰雾建造、跨黑雾边界占地、四格门/楼梯/外围道路，以及黑雾拒绝时不扣除资源。

### 对话：多位面后台数据化与入侵聚合结算 M3（2026-08-24）
- 永久位面离开前台后继续只保留世界快照，不创建或更新建筑、单位、AI、碰撞、寻路、动画和渲染对象；新增 `backgroundLedger` 派生账本，缓存科研贡献、建筑/驻军军力摘要与下一离散事件时间。原后台驱动的逐位面 1Hz 全量结算改为“1Hz 仅推进全局科研并检查期限”，连续经济、矿工补员、建筑升级、探险、面包屋与招募队列只在事件到期、面板读取、存档或入场前按时间差一次性结算。
- 后台入侵改为默认10秒阶段窗：同一时间点先结算经济、升级、出兵和跨位面增援，再以聚合DPS/HP解析攻防；单次工作量只随建筑种类、兵种批次和波次数增长，不随帧数或士兵实体数增长。玩家正在地牢地图、战斗、商店或事件界面时，全局世界时钟仍照常累计，因此位面入侵、传送门告警、胜负和世界毁灭不会漏算，也不会为了参战临时物化大量实体。
- 后台新兵与跨位面兵线由逐兵记录改为可合并兵种批次，军力和承伤按批次总数计算；只有切入目标位面时才按安全出生位逐个物化，每次最多24名、其余沿用750ms重试继续分批落地，未能生成的余量保持为紧凑批次。离线长时段的矿工补员、招募和军械库概率产出也改为数学批量结算；旧版兵线存档和旧世界快照继续迁移，派生账本读档后强制按当前配置重建。
- 存档、世界面板打开和位面物化被定义为权威读取边界，顺序统一为“经济账本 → 剩余入侵窗口 → 序列化/物化”；新增 `window.WorldSimDriver.getDebugModel()` 可查看各后台位面的下一唤醒时间、结构/单位聚合数和结算次数。未运行测试、构建、lint、浏览器探针、游戏运行或性能采样，按约定由用户重点验证地牢中触发入侵、长时间离线生产、跨位面大批增援、存档读档、切入受袭位面和传送门毁灭/重建。

### 对话：建筑与环境资产废案清理及最小可复现归档（2026-08-24）
- 对已正式入库的研究院 LV1/LV2/LV3、位面谐振塔、研究院/教堂接地层和雪原松树执行定稿瘦身：删除未选 12/48 步候选、可再生 keyed/cleaned/anchored 中间层、联系表、审阅对比图及 Blender `.blend1` 自动备份，共 157 个文件、约 97.21 MiB；道路杂物废案仍保持完整删除。
- 保留每项正式资产所需的最小可复现源集：可编辑模型、模型预览、Depth/Body Depth、提示词/控制规格、正式版本实际引用的 accepted raw + body/cutout + preview、入库元数据和 manifest；研究院三级与位面谐振塔的正式 source 均在清理后复核存在，仙人掌与雪松每个已入库形态的 raw/cutout/模型均保留。
- 将障碍物共用的 BiRefNet 紧裁工具由道路专名改为通用命名；AI 资产分卷新增定稿后归档合同，世界-122 分卷补充研究院三级视觉真源与不改变逻辑 footprint 的升级边界。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户测试正式素材显示与研究院升级切图。

### 对话：研究院全局/本栋升级拆分与服务建筑择优（2026-08-24）
- 研究院详情将“塔楼扩建、扩编研究员、精密研究设备”集中列入“本栋研究院升级”，等级、费用、读条和效果均保存到当前建筑；原“防御结构强化、能源回收矩阵、快速募兵”三项集中列入“全局通用研究”，继续由全局能力表保存并作用于所有位面。
- “学术分院网络”退出可购买项目，研究院每个位面上限改由科技直接控制：基础1座、制度化科研2座、高塔学府3座。旧档原全局岗位/设备等级会一次性迁移到旧档已有的每座研究院，旧分院等级作为隐藏兼容上限保留，新建研究院不继承旧的本栋升级。
- 同一位面存在多座可工作天气塔时，天气预报与顶部时间进度栏选择四项模块总等级最高的一座，不再依赖数组第一座或拼接多塔效果。同一患者位于多家战地医院范围内时，只采用实际治疗率最高的一家；相同则比较实际接诊容量和稳定建筑ID，治疗效果不叠加。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查研究院双分区、跨位面全局研究、本栋升级/旧档迁移、科技数量上限、多塔预报与医院重叠治疗。

### 对话：怪物与可生产兵种常规移速统一下调（2026-08-24）
- 全体可移动怪物的统一移速倍率由0.75下调为0.60，继续只作用于 Enemy 构造阶段的常规 `speed/maxSpeed/_baseSpeed`；站桩怪以及冲锋、扑击、突进和击退等技能位移不受影响。
- 为15种可生产军事单位新增0.85统一常规移速倍率，集中接入兵种最终升级补丁；普通生产建筑与跨位面增援现在共用同一最终 `walkSpeed`，原有移动升级继续在统一倍率之后叠加。
- 时空突击特工近战形态速度由260下调为200，并同步修改缺省兜底值。玩家移动速度保持不变；铠甲骑士冲锋的700最大速度按要求保持不变。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查怪物追击节奏、各兵种跟随与列队手感、时空突击特工形态切换，以及铠甲骑士冲锋速度未发生变化。

### 对话：骑士长剑冲刺突击体量恢复为原方案微调（2026-08-24）
- 撤销上一版把低姿态突刺按59px头高强制缩小的方案：该处理把末段有效身体高度从原方案约418px压到约324px。改为按不含武器的有效身体高度标定到400px，只比原方案收约4%，脚线继续固定在y=492；15帧人物贴图与骑士长剑专属握把轨迹同步重建。
- 专属 `dash_recover_thrust` 首帧改为读取当前突刺末帧实际72px头高，再平滑过渡到 idle 的54px，避免突刺结束切 recover 时再次突然缩小。绿色残块清零规则、600ms连续突刺时序、骑士长剑专属范围以及其他武器原冲刺动画均保持不变。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查奔跑→突刺→recover→idle的体量连续性和左右朝向。

### 对话：麦田风车四项本栋升级与满级产粮闭环（2026-08-24）
- 为麦田风车新增“精选麦种、风帆传动、田垄扩建、轮作农法”四项独立升级：单人基础产粮由1.5提升到2.0，传动倍率由1.0提升到1.2，岗位由4提升到6，田地产量倍率由1.0提升到1.25；不含外部增效时，满员产量由6.00提升到18.00粮食/秒。
- 四项升级接入真实金币/能源支付与独立读条；`windmillModules / windmillUpgrade` 进入建筑快照，后台位面继续推进升级并按相同公式产粮。田垄扩建后的岗位控件、人口占用和纯视觉农夫上限同步提升到6，旧档无模块字段时保持原4人、6粮食/秒基础状态。
- 风车详情面板新增实际产量、满员配置产量、单人产量、传动/轮作倍率及统一升级卡，明确配置产量不包含人口超额、工坊和祭品外部倍率。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查四项升级读条、4→6岗位与农夫显示、前后台切场、满仓余量及满级18粮食/秒。

### 对话：空间查询热路径与特效对象池内存收口（2026-08-24）
- `SpatialPartitionSystem` 新增调用方缓冲区版 `queryRadiusInto()`；实体分离碰撞复用实体数组、索引 Map 和查询结果数组，投射物 broadphase 复用实例候选数组，保留原半径、2048结果上限、`i<j` 去重与无网格全表回退语义，消除密集场景逐实体/逐弹体的短命数组及每帧碰撞 Map 分配。
- 投射物池复用现有 `hitTargets` Set 和候选数组；回池前主动断开射手、实体集合、命中目标、墙体上下文、武器效果快照和图片引用。所有特效池新增逐类型容量上限，完整切场、失败清场和世界毁灭清场同时释放空闲池，避免峰值战斗对象永久保留旧场景对象图。
- 性能监控将独立 Phaser rAF 的同步耗时累加进相邻逻辑帧总 CPU 耗时，外部分项多次上报改为累加；HUD 文本缓存改为 `entity -> role -> Text` 两级索引，视口裁切、迷雾隐藏、名字查询和失效清理不再为每个实体扫描整张 HUD Map。
- 寻路出口回退只对 RegionIndex 外层工作计费一次，内部 `findPath()` 保持自身 A* 计费；路径缓存统计改读真实 `_pathCache`，帧预算读取双份性能配置已有的 `pathQueue.drainBudgetMs`。调度器每帧先为有效性检查保留最多一次执行机会，再处理原有优先级重算和剩余检查，持续重算不再永久饿死 validation，`PATH_DEFERRED` 仍保留原任务到后续帧。
- 道路边缘杂物及其四张超规格纹理已由同日独立移除任务撤销，本轮不重新引入或改写该并行范围。未运行测试、构建、lint、浏览器探针、游戏运行或性能采样，按约定由用户重点测试高密度单位碰撞、连续枪战、穿透弹命中去重、切场后再次开火及长时间多场景往返。

### 对话：战地医院科技调整至盾阵学后置链（2026-08-24）
- 军事指挥科技树中的医疗线调整为“盾阵学 → 战地医疗 → 医疗标准化”：战地医疗由原先直接依赖军事组织改为依赖盾阵学，医疗标准化继续依赖战地医疗。
- 战地医疗与医疗标准化分别移动到军事指挥第3、4列的第2轨道，与盾阵学保持同一水平链路；研究费用、建筑解锁、四项医院升级门禁和正式科技图标均未修改。未运行测试或游戏运行时验证，按约定由用户检查科技树连线与旧档已完成科技状态。

### 对话：战地医院升级与医疗科技专属图标（2026-08-24）
- 为巡诊范围、药剂储备、急救分诊、医护编制四项医院升级生成并接入专属256×256冷钢四铆钉图标，移除对工坊升级图标的复用；视觉主题分别为听诊巡诊雷达、药剂瓶与安瓿、患者分诊汇流、仓鼠医护编制。
- 为“战地医疗”和“医疗标准化”科技生成并接入专属1024×1024六边形冷钢徽章，配置以 `iconPath` 加载正式图片并保留 emoji 作为失败回退；原图、提示词和可复现透明化/尺寸收尾脚本保存在 `tools/ai-gen/_field_hospital_icons_20260824/`。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查医院升级卡和科技树节点在实际缩放下的清晰度、透明边缘与加载回退。

### 对话：普通怪物棕熊四动作豆包视频与运行时接入（2026-08-24）
- 以全新棕熊母版为身份锚，通过豆包 Seedance 视频管线分别生成 idle、walking、attacking、dying 四段5秒方形视频；发现首版攻击视频的鼻口已在源画面右侧被截断后，使用50%母图宽度、x=36%偏左锚点重新生成 V3，逐帧源视频审计确认720×720全121帧最小边距为左75/右87/上263/下211 px，通过10%安全框。V2虽未截断但右侧仅10 px，未进入正式资源。
- 新增通用 `video-safe-reference.py` 与 `video-source-margin-audit.py`，把“生成视频前缩小母图并按动作方向留白、抠图前先审源视频边距”固化为管线规则；V3 的58~61帧存在短暂红绿色散，正式21帧采样改用干净第62帧。
- 四动作统一按 idle 首帧比例经 BiRefNet 重建：idle 12帧/512格，walking 22帧/640×512格，attacking 与 dying 各21帧/640×512格；共同脚线 footY=410，攻击保留源位移、死亡逐帧落地，空白帧、贴边、半透明污染和透明区脏RGB均为0。
- 棕熊普通怪数值定稿为配置等级6、360生命、300基础移速与36/32/36/7/8/11六维，派生物攻34、物防64、魔攻7、魔防11、暴击13%、暴抗36%，各项均高于黑狼；170px攻击距离、950ms普攻冷却、180px动态判定与24px击退进一步保持近战压制。新增与胖子僵尸同协议的 `rangedDamageReduction:0.5`“厚实皮毛”，统一伤害链会将枪械、箭矢等远程物理伤害减半，近战与魔法不受影响；图鉴自动显示机制与50%数值。
- 新增普通动物怪物 `brownBear`：独立双份配置、四动作资源键、普通近战状态和死亡保尸链；逻辑 run 复用纯 walking 动画，并注册共享工厂和碰撞编辑器入口。未修改任何地牢 `poolKeys`，沼泽地牢现有黑狼占位池保持不变。
- 未运行构建、lint、开发服务器、浏览器探针或游戏运行时验证，按约定由用户重点检查棕熊体量、左右翻转、移动循环、攻击命中节拍、死亡末帧与尸体清理。

### 对话：道路边缘随机杂物系统移除（2026-08-24）
- 按用户反馈完整撤销道路边缘木箱、桶组、麻袋和长凳的确定性随机生成：`BuildingRoadSystem` 删除坐标哈希、外侧邻格选择、随机镜像、局部刷新和附属 Sprite 生命周期，恢复为只管理道路/田地铺装与建筑预约。
- BootScene 不再读取道路杂物配置或预载四张贴图；删除 `roadside-props.json`、四张运行时资源、道路小物专用 Blender 建模脚本/模型候选目录和单件收尾脚本。道路铺设、建筑外围道路、道路移动加成、退款、存档、碰撞与寻路均未修改；通用 BiRefNet 收尾工具因已被仙人掌和雪松流程复用而保留。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户确认新铺和既有道路均不再出现随机杂物，且普通道路功能保持正常。

### 对话：经济建筑无人岗位旋转禁止标识（2026-08-24）
- 为所有“当前存在有效人口岗位但上岗人数为0”的经济建筑增加统一状态标识，覆盖矿工营地、研究院、天气预测塔、风车、市场、银行、经济工坊、位面谐振塔、军械库、战地医院和面包屋；判定集中在 `PopulationEconomySystem`，房屋、仓库及没有岗位的建筑不会误显示，安排任意1名人员后当帧隐藏。
- 标识由 Phaser Graphics 运行时绘制，不增加图片或粒子资源：深色外描边承托高亮红环与斜杠，圆心区域保持透明镂空；整枚标识以4.8秒一周缓慢旋转并轻微呼吸，不同建筑按稳定ID错开相位，避免大批建筑机械同步。
- 标识位置锚定在建筑可见贴图顶端上方，深度使用建筑标签通道；已接入视口裁切、战争迷雾、地图模式、指挥压平模式、出售/摧毁沉陷、实体移除与场景清理生命周期。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查0→1→0岗位切换、各种高度建筑的顶部位置、密集城区遮挡以及切换地图/指挥模式后的显隐。

### 对话：战地医院 V03 正式入库与岗位治疗闭环（2026-08-24）
- 将用户确认的 V03 48步精修图提升为 `assets/terrain/field_hospital.png`，按建筑运行时管线完成真实 Alpha 紧身裁切、300×244显示标定、121px脚底偏移、面板缩略图、算法8接地拟合清单，以及轮廓/投影/高度/法线环境光照派生资源；逻辑占地仍保持普通2×2，不由画面外扩反推碰撞。
- 新增标准人口经济建筑“战地医院”：建造需先完成“战地医疗”科技并满足经济建筑既有仓库前置；默认2个医护岗位，每名医护发挥20%配置效率，优先治疗700px范围内生命比例最低的玩家与友军。基础满员速度为每秒恢复最大生命3%，同时接诊2人，只治疗存活单位，不复活阵亡单位；实际速度叠加人口劳动效率和范围内最强经济工坊增效。
- 新增“医疗标准化”科技，统一解锁巡诊范围、药剂储备、急救分诊、医护编制四项本栋升级；满级分别达到1200px、每秒6%最大生命、5名患者和5个岗位。详情面板复用经济建筑的岗位控制、状态、参数网格、二级进度和升级卡格式，并显示满员/实际治疗速度、接诊人数、病床和工坊增效。
- 医院模块、升级读条与治疗节拍进入世界快照，离开位面后继续推进升级并正确恢复岗位上限；后台位面不伪造逐单位治疗，因为现有驻军快照未保存逐单位生命值。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查建造科技门禁、接地/遮挡、岗位增减、低血量目标选择、治疗速度、四项升级与切场读档。

### 对话：世界-123 雪原五形态松树 V2 建模与贴图替换（2026-08-24）
- 为直立密冠、左倾承雪、右倾断层、强风偏冠、老龄疏枝五种既有雪原障碍分别建立可编辑 Blender 模型，明确锁定树干曲线、枝层数量、冠层缺口、风向与疏密差异；统一采用暗蓝绿色针叶、风化深褐树皮、冷白枝顶积雪和柔和左上顶侧光的游戏化 PBR 风格。
- 五张内置 ImageGen 精修图均经 BiRefNet 重建真实 Alpha、边缘色修复和紧身裁切后覆盖 `obstacle_snow_pine_01~05` 稳定资源键；`ISO_WALL_GEO` 使用新贴图真实尺寸，并按分辨率等比换算 foot，保持统一 390 世界高度、原有碰撞占地、38 棵散布、360 最小间距、五形态等概率、随机水平镜像和存档协议不变。
- 重新生成五类雪松的环境光照轮廓/高度/伪法线派生数据；建模脚本、五份 `.blend`、模型预览、生图母版、透明终稿、提示词及清单均保留在 `tools/ai-gen/`。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查雪原中的五种轮廓、树冠宽度、接地、镜像及太阳投影。

### 对话：沙漠位面四形态仙人掌 V2 建模与贴图替换（2026-08-24）
- 为双臂巨柱、单臂巨柱、桶状和多节鹿角柱四种既有沙漠障碍分别建立可编辑 Blender 模型；形态从旧版粗糙平面素材升级为肋棱、刺座、风化疤痕、分节关节与差异化枝组明确的独立轮廓，并用统一提示词锁定 44.8° 等距方向、30° 俯视、低饱和沙漠植物 PBR 和柔和左上顶侧光。
- 四张 ImageGen 材质精修图经 BiRefNet 重建真实 Alpha、边缘色修复和紧身裁切后，覆盖既有 `obstacle_cactus_*` 稳定资源键；`ISO_WALL_GEO` 同步使用新贴图真实尺寸，并按分辨率等比换算 foot，保持 obstacleH、世界碰撞占地、散布数量、排除规则、确定性随机镜像和旧存档协议不变。
- 重新生成四类仙人掌的环境光照轮廓/高度/伪法线派生数据；建模脚本、四份 `.blend`、模型预览、生图母版、透明终稿与提示词均保留在 `tools/ai-gen/` 供后续复现。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查沙漠位面中的四种轮廓、视觉尺寸、接地、镜像和太阳投影。

### 对话：玩家双手持剑冲刺突刺 MiniMax H3 视频候选（2026-08-24）
- 以现有 `assets/player/dash_attack.png` 第0帧的黑白骨架玩家和 `assets/weapons/1-rusty_sword_euip.png` 锈剑为首帧视觉锚，构建1344×768纯绿背景参考图；提示词锁定严格右侧视角、两步冲刺、双手同柄、末步水平前刺及末帧弓步定格，并禁止下劈、横挥、旋转、镜头运动和剑体变形。
- 两轮仅首帧约束的 H3 结果均因中段把剑抬至头顶而判定失败；第三轮改用“冲刺首帧 + 第一轮合格前刺末帧”双端像素约束，124帧/24FPS/5.17秒候选在关键第48~102帧保持剑尖朝右，第66~78帧由跑姿直接进入双手水平前刺，没有再出现竖剑或过顶动作。候选、首尾帧、联系图和参数记录保存在 `tools/ai-gen/_scratch/player_dash_thrust_20260824/`。
- 用户确认候选可用后，另生成无武器角色体视频。首版把奔跑与突刺稀疏抽成17帧再压进600ms，单帧动作自然但运行时节奏过快发僵；现改为只保留原视频 `f66~f80` 的15个连续突刺帧，按25FPS在600ms内播放，相对24FPS原片仅约4%加速。人物高、脚线和根节点统一，技能冲刺距离、位移窗口、伤害、冷却均不变。
- 该人物动画只由骑士长剑的 `dashAttackThrust` 触发；通用 `dash_attack` 保持原上劈下砍贴图。武器不烘焙进人物图；`sword.dashThrust` 同步重采样为15点连续握把轨迹，独立 `dashThrustHand` 入口只由骑士长剑读取并保持90°水平刃向。通用 `dashHand` 仍为原 `gripArc`，其他武器继续使用 `sword.dash` 30点挥砍轨迹。
- 为消除骑士长剑突刺末帧切回旧俯身回砍收势的跳姿，再用突刺末帧与现有站立末帧做H3双端约束；跳过模型内部静止段后抽取14帧，安装为独立 `dash_recover_thrust`。骑士长剑武器 recover 从 `dashThrust` 实际末帧握把/角度反推中心，并通过三次贝塞尔先后撤再回 idle；通用 `dash_recover` 与其他武器原退回流程均已恢复。
- 清理无武器人物精灵图残留的绿幕色块：对15帧所有可见像素执行 `G>R 且 G>B` 的全量透明化，共删除91,918个绿色主导像素；生成脚本在输出前强制检查剩余绿色像素必须为0。人物轮廓、帧序、600ms播放时长、武器轨迹及战斗参数均未修改。
- 修复实测中突刺人物错误放大：原构建按弓步全身 bbox 高度归一，低姿态反而把头骨放到约80px，明显大于 `idle` 的54px和 `running` 的64px中位值。现改用姿态无关的头骨高度标定，突刺固定为两者中间的59px并统一脚线到 y=492；专属 recover 从59px平滑收敛到 idle 54px，15帧骑士长剑握把轨迹随新人物尺度重算。攻击/收势帧数、600ms攻击时长、技能位移、伤害、冷却及其他武器均未修改。
- 生成母版、首尾参考、15帧联系图、600ms GIF、轨迹JSON与可复现脚本保存在 `tools/ai-gen/_scratch/player_dash_thrust_20260824/` 和 `tools/ai-gen/`。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查骑士长剑突刺速度、起手衔接、双手握柄与专属收势连续性，以及其他三把近战武器是否保持原冲刺挥砍和退回。

### 对话：V2道路场景元素正式入库与确定性镜像（2026-08-24）
- 将已确认的木板箱、橡木桶组、麻袋绳卷和木制长凳四张 `world122-building-v2` 精修透明图提升到 `assets/terrain/roadside-props/`，替换初始Blender材质预览；按每张新画布宽高比微调运行时显示宽度，保持原有高度和脚底尺度，避免非等比拉伸。
- 道路场景元素新增配置化 `mirrorChance=0.5`；仍以道路格坐标哈希决定镜像，因此约一半实例水平翻转，同一格在读档、切场或道路邻接刷新后保持一致。生成密度、外侧边缘判定、无碰撞、无占格、无寻路和无存档对象的合同均未改变。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查四件新贴图的实际大小、左右镜像分布和道路边缘落点。

### 对话：天气预测塔岗位化、气象科研与位面灾害预警（2026-08-24）
- 天气预测塔改为标准人口经济建筑，恒定1个气象员岗位；没有人口上岗时不提供天气预测、不向顶部时间进度栏注册事件，也不产生科研点。详情面板统一采用其他经济建筑的气象档案、参数网格、岗位进度和升级卡格式，原物防/魔防介绍替换为本位面未来天气列表、监测范围、时段解析、灾害预警和科研产出。
- 四项升级重构为：“预报展望”显示监测窗口内全部降雨并把范围由2天扩至5天，同时承载天气强度区分；原“多轮推演”改为三级“气象科研”，单岗位约产0.58/1.17/1.75科研点/秒，满级约为满配三级研究院3.5点/秒的一半；“时段解析”保留结束时间与持续时长；原“风暴预警”改为“灾害预警”，只预测本塔所在位面的特殊天气。高级气象学继续统一门禁四项升级。
- 新增 `WorldSpecialWeatherRegistry` 特殊天气注册协议，沙漠scene8沙尘暴和僵尸位面scene11死寂雾潮已作为provider接入；未来位面只需声明场景和返回统一天气事件。沙尘暴确认沿用既有3~6日随机排期；死寂雾潮由原手动临时开关升级为4~7日随机间隔、提前0.35日预警、持续0.75~1.5日的可存档状态机，离开位面继续推进，开发按钮复用正式触发/结束入口。
- 降雨预测不再按轮数截断，而是稳定生成并保存监测窗口内全部排期；气象科研与研究院科研在 `WorldSimDriver` 汇总后只推进一次科技树，前台/后台共同读取岗位、人口效率、工坊增效和模块等级。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查0/1岗位切换、未来天气列表、沙尘暴/雾潮时间栏、切场读档、三级科研速度与特殊天气位面隔离。

### 对话：地牢终局奖励链原子发放与异常退出收口（2026-08-24）
- 通关卡牌金币改为“背包→主神空间仓库”统一路由，选择前按本卡基础金币与额外金币合计检查真实容量；容量不足时不锁卡、不发部分奖励、不关闭面板，并给出明确缺口。普通发奖建立可回滚快照，任一数据项失败会恢复背包、仓库、新增地面掉落、等级和属性点；浮动文字及UI刷新异常被隔离，不再打断奖励数据。
- 奖励卡只在整包发放成功后进入选中态并启动2秒关闭，关闭会取消自身定时器。地牢死亡、异常切场或 shutdown 清理奖励节点时会同步关闭 `RewardSystem`、恢复卡牌和清轮询；奖励面板打开失败则返回路线图，保留节点供重新进入。
- Boss金币同样改走背包/仓库，仍有溢出时在Boss场内生成可拾取金币并显示真实入库/落地数量；开门与传送门提示的表现异常不再阻断出口逻辑。奖励物品先尝试堆叠入背包，失败后才落地，修复满格但已有未满材料堆时被错误丢地的问题。出征说明改为读取真实通关武器池口径“稀有~史诗”，不再展示未启用的旧Boss卡牌配置。
- 本轮仅完成代码与调用链静态核对，未运行测试、构建、lint或浏览器运行时验证，按项目约定由用户重点测试正常三张卡、满背包自动入仓、背包/仓库同时满、奖励页强制退出、Boss金币溢出及返回主世界通关登记。

### 对话：近战武器 walking 跟手与 running 背负（2026-08-24）
- 仅调整真实剑类近战武器：walking 的21帧轨迹改为与人物 `walk` / `walk_body` 当前帧一一对应的握点数据，武器 Sprite 原点移到剑柄；不再在离散人物帧之间自行插值，左右朝向继续以人物 `flipX` 为唯一权威，手层保持在武器上方，因此行走时剑柄会实时钉在持剑手上。
- running 保留现有稳定背负锚点，并把剑的渲染深度切到人物主体背层，退出奔跑后自动恢复普通前景层与中心原点；开发面板的 walking 预览、点击命中与拖拽也同步按剑柄原点绘制，避免后续调参把握点数据当中心点写歪。法杖、弓、枪械、攻击/冲刺/收势轨迹及战斗数值均未修改。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查四把剑的 walking 21帧、左右镜像、walk↔run↔idle 切换及 running 背负遮挡。

### 对话：World-122 建筑半木石哥特画风 V2 唯一化（2026-08-24）
- 将后续正式建筑生图的唯一画风合同提升为 `world122-building-v2`：所有新建筑主体统一采用中世纪欧洲经典半木石结构，并融合克制的哥特式尖拱、彩色玻璃窗、窗饰与立面雕花边饰；单栋建筑仍可保留功能构件和局部配色，但不能覆盖共同建筑语法。
- 材质统一锁定游戏向次世代PBR：石材保留中尺度自然风化与缺角，木构保留纹理、榫接和使用磨损，铁件保持哑光，黄铜只作少量功能装饰并呈自然氧化；光影统一为柔和左上顶侧光、宽缓高光和克制接触遮蔽，不允许电影轮廓光、强明暗、泛光或彩色调色。
- 正式候选生成器现在同时强校验 `world122-building-v2` 与唯一模板路径；默认清单和三份天气塔任务清单均已收敛到同一入口，天气塔私有 `world122-building-style-runtime-settlement-v2.md` 已删除，旧版本名只保留在既有候选元数据中用于历史追溯，无法再被误调用。道路小物仅继承PBR材质、磨损、金属氧化、低饱和与柔和光照，不错误添加尖拱窗等建筑构件。
- 以四件已确认Blender道路模型为严格结构参考生成了V2材质精修候选；ImageGen原始输出实际为无Alpha的RGB棋盘格，已使用项目统一 BiRefNet 在本机一次加载后逐张重建真实Alpha、以最近可靠主体色修复半透明边缘并紧身裁切。精修图仍保留为候选，不直接覆盖运行时资产，等待玩家确认。未运行测试、构建、lint、浏览器探针或游戏运行时验证。

### 对话：玩家枪械弹体命中障碍物火花（2026-08-24）
- 玩家手枪、自动步枪/机枪与霰弹枪创建的实体弹体新增显式 `playerGunWallSparks` 标记；只有该标记的弹体被真实墙体/障碍物碰撞截停时，才在 `projectileWallHit` 返回的精确三维交点弹出白黄橙火花。玩家弓箭、魔法投射物、友军士兵、敌军与怪物弹体默认均不触发，墙顶模型命中特例也不会误报障碍物火花。
- 火花使用单场景共享 Phaser 发射器和内部粒子复用，不为每次命中创建/销毁发射器；每次7粒、最长280ms、并发硬上限72粒，并加入视口外裁剪、战争迷雾裁剪、地图模式停用以及同点34ms/18px合并，兼顾全自动射击和霰弹多弹丸撞墙时的峰值开销。场景关闭时共享发射器随场景销毁。
- 弹道伤害、穿透、墙体受击、嵌墙“只出不进”、射程衰减和实体命中逻辑均未修改。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查各类玩家枪械撞墙位置、上下墙高差、霰弹密集撞墙、连续扫射，以及士兵/怪物射击确实无火花。

### 对话：伊莉丝风车技能单次世界音效（2026-08-24）
- 重做伊莉丝风车为独立的0.55秒短促重击音效 `assets/sounds/companions/elise/whirlwind.wav`：快速低频风压在约0.2秒直接打出主冲击，末段迅速收住；峰值约-0.72dBFS、RMS约-9.36dBFS，约86%能量位于500Hz以下，与普通攻击的尖锐剑刃声明确区分。同步更新确定性生成脚本与波形报告，后续可复现调音。
- 伊莉丝攻击、防御和风车三种动作音效统一放入 `companion-config.json#sounds`，`CompanionAI` 不再维护角色专属路径常量。
- 风车音效只在 `_tryWhirlwind()` 成功进入技能、建立本次命中集合后播放一次，走 `SoundManager.playWorld` 世界坐标距离衰减；不会随持续800ms内的逐帧范围命中重复播放，条件未满足或技能被拦截时也不会误响。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查近距离音量、远距离衰减、每次风车单响以及普通攻击/防御原音效是否保持正常。

### 对话：中世纪道路边缘场景元素建模与确定性随机生成（2026-08-24）
- 新建四套可编辑 Blender 模型与透明运行时贴图：双层木板箱、橡木桶与水桶、麻袋与绳卷、木制街边长凳；统一使用 `world122-building-v1` 的44.8°等距方向、30°俯视、低饱和旧木/黑铁/粗麻材质，并紧身裁切透明边缘。
- 新增 `roadside-props.json` 统一登记纹理、显示尺寸、生成密度与道路外侧偏移；BootScene 按配置预载四件资源。道路系统仅在道路格邻接空地的外侧边缘，以格网坐标哈希确定是否生成、物件种类、边缘方向和镜像；相邻道路或建筑预约变化时只刷新本格及四邻，因此道路继续铺设后装饰会离开新道路中央，读档/换场也不会随机跳变。
- 所有道路场景元素均为静态纯视觉 Sprite，不写入存档、不增加碰撞体、不占建筑格、不参与寻路或道路移速计算；默认密度为合适边缘格的16%。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查连续铺路时的生成/消失位置、密集城区遮挡、四件显示尺度和视觉密度。

### 对话：天气预测塔功能升级、科技门禁与时间栏闭环（2026-08-24）
- 核实并保留现有正式链路：`WorldWeatherSystem` 维护全位面降雨排期，只有完成“天气预测”且对应位面存在存活天气塔时，weather provider 才把稳定事件交给顶部时间进度栏；天气塔摧毁或出售后该位面预报立即撤下。
- 新增工程科技“高级气象学”（前置“天气预测”），统一解锁天气塔四项本栋升级：“预报展望”将提前量由2天提升至5天；“多轮推演”将稳定预报由1轮提升至3轮；“时段解析”显示预计结束时间与持续时长；“风暴预警”给大雨/暴风雨增加黄色/红色预警标识。基础科技仍只解锁建塔，UI门禁与升级业务入口均会校验新科技。
- 新增 `WeatherForecastTowerSystem`，升级真实扣除金币/能源并使用独立读条；等级与剩余时间进入世界快照，后台位面继续推进，读档恢复后续接。多轮天气写入可序列化 `forecastQueue`，当前轮结束后按已公布顺序兑现，HUD刷新不会重新随机；四项升级只改变预报范围与信息，不修改真实天气。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查科技解锁、四卡读条、切场/读档、多轮标记、结束时段与风暴预警。

### 对话：通关奖励结算页冷钢化与时钟机械卡框（2026-08-24）
- 通关/任务奖励共享结算页重构为冷钢档案式居中模态：新增英文档案眉题、中文主标题和选择说明，三张奖励卡使用 2:3 响应式网格；普通金色发光与持续漂浮动画移除，悬停、键盘焦点、选中和禁用统一改用冷钢银蓝边线与短时反馈。
- 奖励卡由可点击 `div` 改为原生 `button`，补齐对话框语义、卡片选择说明、`aria-pressed`、禁用状态和打开后的首卡焦点；卡牌奖励内容、发放顺序、金币、升级与两秒关闭逻辑均未改变。
- 使用内置生图工具生成 `assets/ui/reward_card_bg_cold_steel.png`：1024×1536 RGBA 竖版，外缘真实透明，枪灰/深钢为主、少量旧铜点缀，钟表、齿轮、铆钉和机械管线只位于边框，中部保留约65%冷灰留白供动态文字与图标。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查1080p/低高度窗口的三卡布局、文字安全区、焦点轮廓和选中后反馈。

### 对话：三把近战武器游戏内手持贴图升级（2026-08-24）
- 以“生锈的长剑”的正面垂直构图、写实材质密度、整体比例和握柄位置为基准，重新生成骑士长剑、符文长剑、夜与火之剑三张独立透明手持贴图；三者分别强化制式精钢、冷蓝符文刻槽、暗钢蓝金双能量的武器辨识度，并控制细节与光效在约126px游戏显示高度下仍保持清楚轮廓。
- 生成结果经真实 alpha 检查后去除烘焙棋盘格，并把三张图的护手线归一到锈剑画布高度66.3%的握持基准，避免切换武器时影响既有三段攻击与 recover 跟手。Phaser纹理预加载、装备模板和运行时物品数据仅更新 `equipImage`/手持纹理路径；`iconImage`、`slotImage` 与已有 `dropImage` 全部保留旧图，因此装备栏、背包及地面掉落贴图不变。
- 新增 `assets/weapons/knights_sword_ingame_v2.png`、`assets/weapons/rune_sword_ingame_v2.png`、`assets/weapons/night_flame_sword_ingame_v2.png`。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查待机、三段攻击、recover及左右镜像下的握柄贴合、轮廓清晰度和能量纹路亮度。

### 对话：全枪械背包装备栏图标统一升级（2026-08-24）
- 为 G18、沙漠之鹰、P4040、Beretta 93R、.357 左轮、PKM、AKM、M416、QBZ-191、QJB-201、Super90、能量轻机枪、SAIGA-12K 共13把枪械重新生成独立透明背包图标，统一为冷钢枪械材质、青色边缘光和略俯视的装备栏展示语言；长枪采用对角构图，P4040 与能量轻机枪保留各自的科幻配色辨识度。
- `iconImage` 与 `slotImage` 已切换到 `assets/icons/firearms/`，掉落物、手持武器贴图及战斗参数保持不变；远程武器字段补全会从规范配置刷新这两个展示字段，使旧存档中的枪械实例也能使用新图标。
- 13张成品均规范为1536×1536 RGBA、透明背景，主体最长边约占画布90%，并完成逐张透明边界与构图检查。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查背包、装备栏及旧存档枪械的显示效果。

### 对话：伊莉丝豆包自然待机循环入库与肖像保护降级规则（2026-08-24）
- 使用本地豆包 Seedance 2.0 Mini 生成伊莉丝自然 Idle；首次含角色专名的图生视频被肖像保护拦截后，按新增工作流规则移除全部人物专名和身份指认，改称“参考图中的原创游戏角色 / 不涉及真人肖像”，仅重试一次后成功出片。标准模板与 WORKFLOW 已固化该降级策略：再次拦截时停止图生视频，改走无专名文生视频或明显非真人风格参考图。
- 从成功视频第68帧到第144帧选取约3.17秒自然周期，每隔2帧保留一帧；BiRefNet 清除白底、浅灰阴影和豆包角标后生成38帧、512²格、5×8、12 FPS 的透明 Idle sheet。针对用户反馈的左右漂移，重建工具新增 `lower-body` 水平锚点并以双腿/靴子稳定区锁定 X，排除长剑、盾牌和披风轮廓干扰；缩放落位后再做整数像素二次纠偏，全部帧固定为461px角色高度、y=479 脚线与同一 X 根节点，下身水平偏移跨度为0px，0空帧、0贴边，接缝步幅比1.32。
- 伊莉丝运行时 Idle 由单帧替换为38帧循环，配置与精灵表守卫同步更新；双脚保持站定，动作仅包含轻微呼吸及马尾/披风的自然摆动，未修改其他五种状态、角色逻辑或技能。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查 Idle 循环接缝、双脚稳定、Idle→Walking/Running 的尺度与脚点切换。

### 对话：祭坛地牢分组、逐级解锁与钥匙代币图标（2026-08-24）
- 祭坛出征下拉列表改为按地牢系列显示父标题，系列内统一使用“地牢名-初级/中级/高级”格式并按难度升序排列；父标题与子项采用不同字号、字重和颜色，锁定层级会直接标注前置通关条件。
- 僵尸地牢与沼泽地牢启用逐级通关解锁：初级默认可进入，成功通关初级后开放中级，成功通关中级后开放高级。解锁读取既有 `WorldProgressionSystem` 通关存档；确认出征前再次校验，未解锁时不会消耗钥匙。
- 新生成透明背景冷钢钥匙代币图标 `assets/ui/icons/dungeon-key-token.png`，统一用于商店及装备数据中的 F~A 级时空锚点代币，原旋涡字符保留为图片加载失败时的回退。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查原生下拉分组样式、旧存档解锁状态与各商店/背包图标显示。

### 对话：天气预测塔风向标随降雨强度变速（2026-08-24）
- 天气预测塔的独立风向仪运动层新增配置化天气倍率：晴朗1倍、小雨1.25倍、中雨1.5倍、大雨2倍、暴风雨3倍；天气停止后同帧恢复基础速度，普通麦田风车和其他未声明倍率的建筑动画不受影响。
- GameScene 每帧只从 `WorldWeatherSystem` 读取一次当前位面的统一天气状态，并同时提供给降雨视觉与建筑运动层；自动排期、存档恢复和开发工具手动切换均走同一状态真源，不读取雨粒子表现，也不新增计时器。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查四档切换、晴天回速、暂停/切场和多建筑表现。

### 对话：天气预测塔风向标分层旋转动画（2026-08-24）
- 按麦田风车沉淀的“静态主体 + 独立运动层”合同，把天气预测塔上方风向箭头与三杯风速仪从主体分离：桅杆、避雷针和轴承帽留在静态主体，运动件由原 Blender 模型同相机渲染为32帧透明 spritesheet；风向箭头每4秒旋转一圈，风速杯同期旋转两圈。
- 天气塔配置新增独立面板纹理和 `animation` overlay；BootScene 的动画注册从风车特例收敛为遍历所有配置化生产建筑运动层。既有中立建筑渲染链继续统一同步 overlay 的位置、等比缩放、镜像、结构深度、战争迷雾、地图模式、指挥态金色轮廓与销毁生命周期。
- 建造逻辑、2×2占格、碰撞、寻路、天气预测数值和存档均未修改；静态详情图继续展示完整风向标。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查实际轴心、转速、雾中隐藏、地图模式、指挥态轮廓及建筑销毁清理。

### 对话：玩家三段普通攻击武器收势轨迹优化（2026-08-24）
- 逐段还原攻击末帧到 idle 的现有轨迹：一段约150°→105°且中心回移约37×20px，直线回位缺少收刀弧；二段约85°→105°但中心横移约87px，是“大位移、小转角”的僵硬主因；三段末两帧129°→164°后旧收势立即反向到105°，角速度在切换点反号造成角度错乱。
- `sword.attack/attack2/attack3` 分别新增 recover 三次贝塞尔控制参数。位置从各段实际末帧连续出发，第一段轻抬回收、第二段沿较大的上方回撤弧归位、第三段先顺着突刺末势向左下延续再回到 idle；角度同样用分段控制点，第三段先延续正向旋转再平滑回正。左右朝向会镜像 X 与角度控制量，端点、末帧拉伸和 idle 尺寸保持连续。
- 人物仍复用原13帧 `recover` sheet，三段收势时长仍为约330/300/400ms；命中形状、伤害、击退、眩晕、顿帧、第三段36px前迈和连段规则均未修改。冲刺攻击及缺少 recover 配置的旧武器继续走原线性兜底。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查三段左右朝向的末帧接续、第二段回撤弧、第三段转向连续性和最终握持位置。

### 对话：研究院与教堂接地覆盖层接入游戏（2026-08-24）
- 将已确认的研究院、教堂透明接地覆盖层提升到 `assets/terrain/building-contact/`，并在 `producer-buildings.json` 通过通用 `groundContact` 配置登记纹理、显示尺寸与独立脚点偏移；BootScene 按配置自动加载，不维护建筑ID硬编码名单。
- GameScene 新增独立 `groundContactSprite`：使用结构排序 `rearFx` 通道，固定高于中央道路填充、低于建筑主体和风车等前景运动层；研究院升级为LV2/LV3后仍按地面脚点保持原尺寸覆盖层，不随塔楼高度拉伸。覆盖层不进入主体alpha拟合或遮挡AABB，不改变2×2占格、碰撞、寻路、道路与 `visualFootprint`。
- 建造幽灵同步显示并染色接地层，层级位于道路预览与主体幽灵之间；正式实体同步镜像、视口裁剪、战争迷雾、地图模式、压平视图、拓扑深度、下沉/移动和销毁清理。沉降特效同时接管接地层与既有运动附加层，并立即清理工作态Graphics，补上中立实体记录移除后的残影缺口。已从 `TODO.md` 移除该项目。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户进入游戏重点查看研究院LV1/LV2/LV3及教堂的墙脚衔接、单位前后遮挡、迷雾/压平切换和拆除残留。

### 对话：天气预测塔正式贴图入库（2026-08-24）
- 采用用户处理完成的 `weather_forecast_tower_refine48_v01.png` 作为天气预测塔正式主体，保留原始 RGBA/alpha，仅紧身裁剪并修复半透明边缘 RGB；正式资产为 `assets/terrain/weather_forecast_tower.png`（606×718），运行时按 256×303 等比显示，脚点偏移为150。
- 天气预测塔退出研究院占位贴图，独立注册 `weather_forecast_tower` 纹理并同步正式名称、资产路径、面板缩略图来源和2×2视觉 footprint；天气系统、科技解锁、建造上限、数值、逻辑占格、碰撞及寻路均未改动。
- 光照派生管线登记天气预测塔，仅增量生成该建筑的剪影、投影、高度和法线资源。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户检查建造面板预览、落脚位置、风向标完整度、建筑遮挡和天气详情面板。

### 对话：研究院与教堂专属接地覆盖层第一版建模（2026-08-24）
- 新增 `build-building-ground-contact-overlays.py`，复用现有组件化建筑、30°正交相机和44.8°建筑旋转，为研究院与教堂建立独立可编辑的接地组件集合；保存的 `.blend` 同时保留只用于对位的主体参照集合，覆盖层可单独隐藏、修改和导出。
- 研究院候选由四翼短墙脚压边、四个菱形围柱脚块、双层入口门槛和6块贴墙冷灰碎石组成；教堂候选由双层入口门槛、6个扶壁脚块、5段断续暖石压边和4块贴墙碎石组成。两者都不形成完整2×2地台，不含草、箱桶或高物件。
- 新增 `compose-building-ground-contact-review.py`，使用正式贴图原始裁切元数据把1024透明覆盖层回贴到当前运行时画布，并按现有 `visualFootprint`/显示尺寸生成“当前正式主体+道路”与“道路+覆盖层+正式主体”对照图。所有产物仍位于建模包候选目录，未修改 `assets/terrain/church.png`、`assets/terrain/research_institute.png`，也未接入运行时、占格、碰撞、寻路或主体 `visualFootprint`。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定等待用户确认候选造型后再进入接线阶段。

### 对话：伊莉丝跨状态视觉体型统一并保留风车下蹲（2026-08-24）
- 以 Idle 站立主体高 461px 为基准，把六状态放到同脚线、同像素尺度联系图中复核；Walking、Attack 站立首尾与 Defend 已落在 458~477px 同一体型带，不做无意义重采样。新 Running 整体偏大约 5.7%，整套按 0.943 等比缩放；Windmill 站起段偏大约 4.5%，整套按 0.955 等比缩放。两者都围绕逐帧躯干中心和脚底锚点处理，未修改帧格、帧数、帧率或运行时配置。
- 风车中段 364~394px 的低高度来自真实下蹲姿势，不再按 Alpha 总高度逐帧拉齐；统一缩放后站起段约 463px、最低下蹲约 348px，原有下蹲/站起比例和剑弧完整保留。新增 `elise-normalize-state-scale.py` 与同像素尺度联系图工具，审计脚本逐帧补报内容高度，后续可明确区分体型缩放错误和姿态变化。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查 Idle→Walking→Running 的切换体型、风车下蹲与站起过程、剑弧完整性和脚底稳定性。

### 对话：密集城区建筑/单位统一图层拓扑重构（2026-08-24）
- 回滚“只扩大横向命中范围即可解决遮挡”的错误模型：建筑主体与运动叠加层现在缓存完整世界 alpha AABB，动态结构索引由 X 列升级为二维网格；只有单位与建筑在画面 X/Y 都真实相交时才进入仲裁，同一横列但上下相隔的仓库、经济工房、房屋、银行不再共同污染一名玩家的深度。
- 普通格网建筑与玩家、敌人、友军、侍从及纯视觉平民统一使用逻辑脚点/footprint 的 u/v 前后比较器和上下界约束；普通建筑退出墙/门面线算法，墙、门、掩体继续保留独立线段仲裁。静态建筑拓扑间隔由0.25扩大为1.25，为动态单位在相邻结构之间预留前后各0.5的完整深度槽；真实画面相交但地面关系歧义的建筑按稳定基础深度建边，不串联远处无关结构。
- 删除已无运行时调用的 `structureFrontYAtX` / `structureDepthRelationAtPoint` 旧前缘采样链，并补入仓库、银行、经济工房、房屋围绕单一玩家的密集约束、同X异Y剔除、二维旋转包围盒与动态插槽契约。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查密集城区单玩家沿建筑后方、侧角、夹道和正前方连续移动时的遮挡与稳定性。

### 对话：魔法体系第二阶段——冰墙单线阻挡与分段遮挡（2026-08-24）
- 冰墙逻辑层由“每个视觉段一条碰撞线”收敛为“每次施法一条连续 `_iceWall` 阻挡线”：玩家、怪物与投射物继续共用 `WallSystem` 判定，高等级段数增长只增加视觉冰晶，不再线性增加碰撞/寻路对象。冰墙线作为临时硬障碍进入 A* SpatialHash，创建、自然碎裂与场景清理均按包围盒局部失效路径缓存，单位不再沿旧路线持续撞墙。
- 冰晶段保持纯视觉对象、不进入 `Game.entities`；每段新增独立水平脚线与深度，并并入动态实体统一遮挡仲裁。墙前单位会抬到对应冰晶之前，墙后单位仍被当前段遮挡，横向/斜向长墙不再由整墙单一深度覆盖目标。
- 500ms 延迟破土队列同步携带施法属性快照和链式伤害倍率，重叠施法不再通过系统级临时字段互相覆盖，落点伤害也不会在延迟期间因切换武器改变。已从 `TODO.md` 移除冰墙条目并更新战斗、寻路和 lessons 对应口径；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查玩家/怪物/投射物阻挡、敌人绕路、碎裂与换场清理、重复施法，以及墙前/墙后/墙端图层。

### 对话：World-122 建筑画风管线唯一化与接地覆盖层方案（2026-08-24）
- 新增 `prompts/world122-building-style.md` 作为 `world122-building-v1` 唯一画风真源，固定低饱和半写实策略建筑、统一PBR材质尺度、48px可读性、柔和中性光照、克制边缘磨损和禁止卡通/微缩模型/电影级调色等共同语言；单栋建筑今后只允许在候选 manifest 中描述结构、功能细节和局部配色。
- 标准候选入口现在显式锁定 `flux2-dev-depth`、1024²、CFG 3.5、Euler/simple及12步/48步参数，并为每张候选写出包含画风版本和完整参数的 `_generation.json`。改变步数或denoise必须显式声明 `--allow-nonstandard`，正式建筑不再允许以手写 `comfyui-gen.py` 命令绕过标准入口；SKILL与WORKFLOW中遗留的Control 0.88旧口径已收敛为结构Depth 0.78、精修Depth 0.75。
- 针对研究院和教堂主体直接切到道路、缺乏墙脚过渡的问题，确定不重新生成主体贴图：后续先以原Blender模型制作门槛、低矮勒脚石、扶壁脚块和少量贴墙碎石组成的建筑专属透明接地覆盖层候选。运行时视觉层尚未实现，已登记到 `TODO.md`；不得提前入库、恢复通用独立地基或改变2×2占格、碰撞、寻路与主体visualFootprint。本轮仅修改工作流、参数真源和方案记录，未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户确认接地覆盖层方向后再建模。

### 对话：魔法体系第一阶段——施法媒介、副手、回蓝与成长收口（2026-08-24）
- 法杖限定为当前启用主手，副手/备用副手只允许盾牌及预留的 `spellbook/magic_book` 魔法书类型；枪械、法杖、剑等主手武器不再进入副手。拖拽、双击装备、商店/改造区回装与旧档迁移统一消费共享槽位规则，旧档非法副手优先迁回背包，背包满时安全掉落，无法安全迁出时保留并告警；装备副手挤下双手武器时也补上背包满保护。
- 法杖 `matkFormula` 只计当前主手，切换武器组立即重算；旧存档法杖同步当前强化曲线。练习法杖的智力/精神每强化成长由 `0.25` 收敛为 `0.20`，在常见同属性配点下，+15 总魔攻约为 +0 的三倍左右。施法动画字段 `castAnimKey` 纳入武器字段补全，避免商店/旧档实例丢失法杖施法动画。
- 玩家魔法回复改为战斗内外常态每秒恢复，公式统一为 `1.0 + 精神×0.08 + 智力×0.02`，角色面板与说明同步为每秒口径。法袍与法杖改造的冷却缩减改走公共倍率，补齐暴风雪、陨星、焰甲等快捷栏显示并修复 Bolt 法术重复乘算。
- 新增单次施法快照：伤害属性、主手法杖改造、魔法穿透、魔法暴击、暴击倍率与法袍魔伤在释放时冻结，飞行物/持续领域命中时不再读取后来切换的装备；魔法暴击现在实际放大伤害。光系伤害不再读取治疗词条，光系治疗不再读取通用魔伤或链式伤害词条；链式强化与施法加速只由本次施法的法杖词条产生，焰甲补齐链式消费与施法尾效。
- 冰墙“无实体、线段阻挡、分段深度”的改造已登记到 `TODO.md`，本阶段未改其碰撞/寻路实现。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查旧档副手迁移、法杖/盾牌组合、切枪前后魔攻、战斗中回蓝、持续法术切枪、魔法暴击/穿透、各系冷却及光系伤害/治疗。

### 对话：完整删除建筑窗口灯光闪烁系统（2026-08-24）
- 根据静态性能排查结论，完整移除通用建筑亮窗系统：11类建筑不再配置 `windowGlow`，BootScene 不再统一预载15张亮窗蒙版，中立建筑渲染链删除附加 ADD Sprite 的创建、逐帧位置/尺寸/镜像/旋转/alpha/深度同步，以及地图模式、战争迷雾、结构排序、压平视图和销毁清理分支。按 RGBA8 原始尺寸估算，启动后不再为该系统常驻约109 MiB纹理内存，同屏建筑也不再产生对应叠加绘制与批次切换。
- 删除 `assets/terrain/window-glow/` 全部15张蒙版、蒙版生成脚本及联系预览；知识库将通用常驻亮窗明确标为已取消。位面谐振塔晶尖蓝/青/金工作特效是独立的小范围按需 Graphics，继续保留，并改用通用稳定视觉相位字段，不依赖旧亮窗命名或资源。
- 已静态确认建筑配置可解析且运行时代码、配置和工具不存在亮窗引用；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查各类建筑不再闪窗、房屋/研究院升级换图正常、风车 overlay 与位面谐振塔晶尖工作特效仍正常。

### 对话：天气预报塔本地可编辑模型与 Depth 控制图（2026-08-24）
- 按 World-122 建筑资产标准流程完成天气预报塔的本地 Blender 组件化白模：采用完整2×2石基、低矮观测厅、相连单座八角观测塔与蓝灰屋顶，并把三杯风速仪、风向标、浅雷达碟和双雨量筒固定在塔顶或相连屋面平台；全部组件使用稳定英文对象名，可在 `.blend` 内独立编辑。
- 已生成 1024×1024 透明材质预览、完整 Depth、排除地基与石台的 Body Depth，以及 48×48 可读性缩略预览；专用生成提示词、尺寸/相机/配色清单和组件登记表同步补齐。经用户明确授权后，Body Depth 已发送到局域网 `192.168.3.142:8188`，按 FLUX.2 Dev Depth、12步、Depth强度0.78和固定种子 `824201～824205` 生成5张结构候选，并完成绿幕抠图、伪地基清理、主体锚定、轮廓遮罩和道路占位联系表；当前推荐 `structure_v01`，等待用户选定后再进入3张48步低重绘精修。
- 当前仍为候选建模资产，未替换 `assets/terrain` 运行时占位贴图，也未修改天气机制、建筑数值、逻辑占格、碰撞或寻路。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户审阅5张结构稿并选择精修起点。

### 对话：位面谐振塔晶尖工作闪耀特效（2026-08-24）
- 位面谐振塔工作时，晶体尖端新增持续循环的蓝色、青色、金色加法闪耀：包含平滑变色呼吸光晕、十字/斜向星芒和四颗确定性上升光屑；锚点按正式 877×702 主体贴图实测登记为归一坐标 `(0.502, 0.100)`，并随建筑最终显示尺寸、镜像和结构深度同步。
- 特效启停直接读取现有发电公式的真实工作态：只有 `actualEnergyPerSecond > 0` 才显示，无人上岗、建筑沉没、销毁、离开视口、进入地图模式或被战争迷雾隐藏时同步熄灭/清理。只新增渲染层和瞬时标记，不改位面谐振塔产能数值、岗位、升级、占格、碰撞、寻路、阴影或存档。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查 0 岗/有岗的即时启停、蓝→青→金循环、晶尖锚点、地图模式与战争迷雾下的可见性，以及建筑销毁后的残留清理。

### 对话：矿工营地/仓库遮挡单位的共享算法修复（2026-08-24）
- 排查确认矿工营地、可建造仓库和主神空间仓库均已接入标准 footprint、结构深度及状态切换刷新，并非两处漏接线；根因是动态单位横向命中只覆盖“逻辑占地 + 单位自身宽度”，遗漏建筑贴图相对 footprint 的屋檐/底座外伸，导致左右前角存在画面已相交但未触发抬层的窄带。
- 新增共享 `structureOcclusionXBounds()`：优先读取建筑主体及运动叠加层当前帧真实 alpha 横向范围，未创建 Sprite 时从现有 `visualFootprint` 与最终视觉缩放确定性回退；候选索引与局部前缘仲裁共同消费。前后关系仍钳在逻辑菱形最近前边，不改碰撞、占格、寻路或贴图尺寸，也不给矿工营地/仓库写特例。补充两种真实配置的前角回归用例。
- 后续全链路审计收口：动态结构 X 列索引改为只登记建筑自身边界、按单位真实 `sideRange` 跨列查询，几何不变时跨帧复用列 Map；真实 alpha 边界补齐 flip/rotation 变换。掩体 `_faceDepth` 固定为几何前缘，拓扑结果只写 `_structureRenderDepth`；动态单位阶段不再第三次覆写静态建筑深度，并删除已无创建入口的 `backSprite`/双层裁剪残留。回归新增超宽单位、旋转边界、索引复用和字段职责约束。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查矿工营地与仓库左右前角、正前/正后，以及其他带屋檐或底座外伸建筑的单位遮挡。

### 对话：建筑详情无缝切换与指挥态悬停轮廓（2026-08-24）
- 建筑详情分组在点击画布上的另一栋建筑时不再由面板外点击守卫提前关闭并吞掉本次输入；同类建筑直接在现有面板内刷新目标，跨类型建筑先接管新详情再收起旧面板，点击空地、再次点击同一建筑、关闭按钮与 Esc 的既有关闭语义保持不变。
- 指挥模式新增全建筑可见主体命中：鼠标悬停任意存活、可见的防御结构时，按实际 Phaser 贴图层选择最上层建筑，并只为当前目标的主体/运动叠加层或防御塔三层贴图添加金色轮廓；不改变产兵建筑单选、真实占地提示和右键集结规则。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查同类/跨类建筑连续切换、同建筑再次点击、空地关闭，以及指挥态下普通生产建筑、矿工营地、墙门与防御塔的悬停轮廓。

### 对话：研究院建筑预览同步与研究员效率重平衡（2026-08-24）
- 建筑面板的 `research_institute` 缩略图已由当前正式 LV1 蓝白哥特式研究院贴图重新生成，并在建筑配置中显式登记正式资产与缩略图路径，避免继续依赖旧版学院城堡预览。
- 研究院基础岗位从 3 个调整为 6 个，每名上岗研究员贡献从 20% 调整为 10%；因此初始研究院满编为 60% 科研效率，“扩编研究员”两级改为每级增加 2 个岗位，容量和满编效率依次达到 8人/80%、10人/100%。无岗位仍不产生科研点，LV1/LV2/LV3 基础科研点、精密设备加成、人口效率和经济工坊增效公式保持不变。
- 前台研究院详情、当前位面结算、后台位面结算和知识库口径已同步；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查建筑面板新缩略图、6/8/10 岗位上限及 60%/80%/100% 科研效率。

### 对话：图鉴战力命名收口、奖励循环依赖修复与待办规范（2026-08-24）
- 配置 `level` 继续专用于地牢成长、经验结算和存档语义；图鉴派生指标从公式键到返回值统一命名为 `combatLevel`，避免后续把横向战力误接入运行时等级。未来真实 DPS、射程、控制、召唤和阶段威胁评分，以及图鉴战力排序，登记到根目录 `TODO.md`，尚未实现的项目不再混入 SKILL 或已完成日志。
- 修复 `DamageableEntity → dungeon-rewards → DungeonConfig → tribute-effects → Game → Enemy → Combatant` 循环导入：奖励规则改为直接读取原始地牢 JSON 和纯战斗公式，保持底层奖励工具为叶子依赖，消除 `Cannot access 'DamageableEntity' before initialization`。
- 可复用规则分别沉淀到怪物、冷钢 UI 和循环依赖调试分卷：怪物配置/运行时/图鉴同源、双等级边界、机制参数与贴图/删除闭环，图鉴宽窄屏滚动职责和键盘可访问性，以及底层实体依赖复核方式。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户检查图鉴宽窄布局、战力显示、配置等级经验语义和启动加载。

### 对话：天气预测与位面谐振科技图标替换（2026-08-24）
- 为“天气预测”新增气象观测塔徽章，以蓝顶观测建筑、风向仪、雷达扫描、晴雨分区和预测曲线表达跨位面气象演算；为“位面谐振”新增双位面圆盘、紫色晶核、同步波纹与能源导流组成的谐振装置徽章；为“谐振校准”新增刻度盘、调节旋钮、频率波形及四类状态节点组成的精密调校徽章。
- 三枚图标均按现有科技树冷钢六边形系列生成，归一为 1024×1024 并套用统一透明轮廓，分别接入 `weather_forecasting`、`planar_resonance`、`resonance_calibration` 的 `iconPath`；科技费用、前置条件、描述和解锁内容保持不变。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户检查科技树实际缩放和明暗状态显示。

### 对话：枪械高收益改造二次校准与冲突修复（2026-08-24）
- G18 轻量化击发组件由错误的 `-300ms` 收敛为 `-15ms`，并建立枪械统一 40ms 攻击间隔下限；主手、副手、连发收尾、实际散布和装备提示统一读取同一射速解析结果，避免负冷却退化为逐帧开火，也修复双持武器改造效果串用主手配置的问题。
- `shotSpreadDelta` 由仅改变准星反馈改为实际弹道散布；普通霰弹同步实际弹丸锥，独头弹采用 Super90 ±15°、其余霰弹 ±20° 的后坐上限，并在换弹或切枪时正常恢复/清空，准星和提示显示同一实际角度。
- P4040“轻量化快速板机”保留 `-100ms` 射速收益，移除额外 `-500ms` 换弹收益；命中加速改为独立的 2 秒移速 +10% 状态，同一发穿透弹只触发一次，后续命中只刷新持续时间，不再与通用 haste 叠层或无限延长。P4040/Beretta 全自动特殊改造仍保持 4 张改造券，原伤害公式不动。
- 机枪过热状态改为按装备槽和武器实例隔离，切换 PKM/QJB-201/能量轻机枪不再互相继承或改写热量；`overheatTimeDelta`、恢复增量和百分比现对三类过热武器统一生效。Beretta 三档弹匣整理为 +3、+6/300ms、+9/500ms，消除同收益下严格劣化的重复项，价格不统一上调。双份 `craft-config.json` 保持同步；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查 G18 极限射速、双持异配、P4040 穿透加速、独头弹连续射击、PKM/QJB 切换过热及 Beretta 三档弹匣。

### 对话：D/C阶段金币消耗重平衡（2026-08-24）
- 常规军事与矿工项目从固定1000金币/级改为500～4800的十级递增曲线，单个10级全局模块由10000提高到20700；铁匠铺和研究院能力金币成长同步提高，整套分别约70596与82532，避免一次C级地牢直接清空全局永久体系。
- 银行、经济工坊、位面谐振塔、军械库和面包屋改为按项目价值分档的逐级金币曲线，满级总价分别约50950、48000、48000、45600与47350；仓库物流继续保持30000，能源和读条价格不变。防御塔芯片单项上限从99收敛到30，费用增长从1.45调整为1.28，单项10→30约29652金币，移除数学上不可达的后段。
- 保留当前装备强化总价和已校准枪械公式；知识库明确沙漠之鹰零强化成长属于既有合法设计，本轮不借经济调整改变重手枪战斗定位。B/A级地牢尚未完成，不提前预设后期金币项目或具体价格；待路线、房间、怪物、Boss和宝箱收益全部落地后，再用真实通关样本复审主动/被动收益与后期消耗。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查建筑升级卡真实下一级金币、防御塔30级封顶及旧存档现有等级显示。

### 对话：研究院升级与科技树图标替换（2026-08-24）
- 为“扩编研究员”“精密研究设备”“学术分院网络”新增 209×209 透明建筑升级图标，分别以研究员团队、精密光学研究仪器、三座蓝顶哥特分院互联表达岗位、基础科研点和同位面研究院容量，并接入 `building-upgrades.json` 的 `iconImage`。
- 为对应科技“制度化科研”“高塔学府”新增 1024×1024 透明六边形冷钢徽章：前者以学术典籍、研究院和三类制度节点组合，后者以逐层升高的蓝顶高塔、学术典籍及上升能量束表达三级研究院解锁；两项均接入 `technology-tree.json` 的 `iconPath`。
- 五枚图标按现有冷钢、深蓝、青色发光与少量金色点缀的系列规范生成并清理透明背景；本次仅替换视觉资源及其配置引用，未改动科研数值、升级等级、科技前置或解锁项目。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户检查升级卡片与科技树中的实际缩放辨识度。

### 对话：研究院 LV1/LV2/LV3 正式贴图与等级光照接入（2026-08-24）
- 按用户反馈补齐此前遗漏的 LV1 正式替换：以已确认哥特式母体生成默认5张12步结构稿，选择结构最克制的 `structure_v05` 进入3张48步低重绘精修，并正式采用无门板伪仪器的 `refine_v02` 覆盖 `research_institute.png`（577×616）。LV2 `refine_v01` 与 LV3 `refine_v02` 继续分别使用 `research_institute_lv2.png`（579×688）和 `research_institute_lv3.png`（580×784），三档均保持蓝、白、灰四面裙楼、围柱与逐级加高塔楼。
- 研究院等级配置现完整切换三档贴图：LV1/LV2/LV3 显示尺寸分别为 256×273、256×304、256×346，并按各自脚底偏移严格映射回共同的 256×128 视觉占地；逻辑放置、碰撞、占格与寻路仍保持 2×2，不随塔楼高度扩大。
- 三档研究院均配置独立冷蓝窗光遮罩，以及剪影、投影、高度、法线环境光照派生资源；启动预载、窗光等级变体和光照资源清单已接入。仅完成静态资源与配置核对，未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查新建/旧档 LV1 外观、升级瞬间贴图切换、脚底锚点、夜间窗光和动态阴影。

### 对话：地牢普通/精英房数与主动收益成长（2026-08-24）
- 地牢战斗房数改为等级与战斗类型双维配置：F级普通/精英均1房，E级普通1房、精英3房，D/C/B/A级统一普通3房、精英5房；Boss战保持现有独立遭遇规则。场地显式接收本场房间数，3房走线性串联、5房走蛇形迷宫；建成后仍按真实房数强制同数波次，末房宝箱、门控、陷阱和出口继续读取 `arena.rooms.length`。
- 新增地牢金币收益真源 `combat-formulas.json#dungeonRewards`：F→A分别成长怪物金币倍率、战斗节点清剿奖、Boss金币和通关基础金币；两类Boss路径、路线预期奖励与通关卡牌统一读取同一档位，非地牢奖励仍保留旧兜底值，避免固定Boss奖造成高等级主动探索收益塌缩。
- 竞技场宝箱房改为每箱必得当前等级的强化石和改造券；额外分支为75%获得等级金币（其中25%是尚未实现宝箱怪的金币兜底）、25%获得等级粉尘，精英战50%额外装备保持不变。宝箱金币F/E/D/C/B/A调整为500/1000/1800/3000/4800/7500，石券维持1/1/1/2/3/4的稳定成长；按当前路线结构，目标总金币约为F 0.5万、E 1.0～1.2万、D 2.3～2.7万、C 4.0～4.6万，使15分钟主动收益高于同阶段优化银行。双份 `dungeon-config.json` 保持同步；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查E级精英3房、D级普通3房/精英5房、两类Boss金币、通关金币及宝箱必掉石券。

### 对话：研究院 LV2/LV3 48 步低重绘精修（2026-08-24）
- 经用户确认，以 LV2 `structure_v02_raw` 和 LV3 `structure_v02_raw` 作为纯绿底初始图，继续使用各自 `Body Depth`，在局域网 `192.168.3.142:8188` 通过 `flux2-dev-depth`、1024×1024、48步、CFG 3.5、Depth strength 0.75、denoise 0.30 各生成3张低重绘精修候选；未启用未经确认的第二路边缘控制。
- 6张精修图均完成 raw、全画布 HSV 去绿、伪地台清理、底边锚定、主体遮罩和审批预览派生链，并生成 LV2/LV3 各自的精修联系表。逐像素检查12张 `body/preview` 派生图，符合项目绿色判定区间的可见绿色像素均为0。
- 当前视觉首选为 LV2 `refine_v01` 与 LV3 `refine_v02`：单主塔、四面裙楼、四根围柱和等级高度关系均保持，尖券窗、石砌层次与蓝色屋面肋条清晰。LV2 `refine_v02` 因柱帽/山墙出现伪文字不推荐，LV3 `refine_v03` 因局部暖色窗格偏离蓝白灰配色不推荐。正式 `assets/terrain/research_institute.png` 未替换，尚未进入运行时缩放、锚点与光照图入库；未运行测试、构建、lint、浏览器探针或游戏运行时验证，等待用户确认最终精修编号。

### 对话：枪械收益校准、远距衰减与冷钢准星重构（2026-08-24）
- 保留全部枪械原有双属性加成结构，只下调原生高射速全自动武器的基础值、每级强化固定值和双属性强化成长；半自动重手枪与霰弹枪的定位公式保留。G18 及各自动步枪/机枪纳入基础与 +15 强化曲线校准；按后续修正，P4040 与 Beretta 93R 恢复改动前的基础/强化伤害公式，不以伤害削减平衡全自动改造。旧存档和商店模板会迁移到当前枪械基础配置，强化等级、附魔和已付费改造仍按实例继续叠加；弓与近战不参与此次迁移。
- P4040 与 Beretta 93R 的全自动扳机保留原伤害及 -100ms/-75ms 射击间隔收益，并改为按实际发射数累计散布；两项配置标记为枪械“特殊改造”，首次安装即消耗4张改造券。改造系统新增数据驱动的 `ticketCost/specialModification`：普通首次安装仍为1张、普通同槽替换仍为4张，特殊改造取自身成本与替换成本的较高值，选择列表以青色标签和成本文案区分，避免先装普通配件再低成本替换绕过。其余枪械也由“按住鼠标多久”改为“成功出弹多少发”的主副手独立热量，点射/停火按各枪恢复时间回正，现有瞄具、枪管、移动散布和后坐恢复改造继续折算生效；物品提示同步显示稳定发数、最大散布发数、角度和恢复时间。
- 手枪、步枪、机枪和霰弹枪最大射程整体延长，同时新增平滑距离衰减：每把枪在配置的满伤距离后逐步降至远端最低倍率，射程改造会同时延伸最大射程并按 60% 推进满伤区。投射物对象池显式重置衰减参数，避免非枪械投射物复用上一发的远距衰减。
- 准星改为 96px 青色冷钢机械刻度：直接读取主/副手当前实际散布角，结合玩家到鼠标的瞄准距离和相机缩放投影开合，并保留单发 kick 与三级命中标记；中心改为青白菱形，DOM 置顶方案继续保证商店/改造等面板上可见。换弹仅微调 G18、AKM、M416、QBZ-191 的基础时间，原有减少换弹时间改造继续在新基础值上生效。未调整敌人、魔法伤害或魔法恢复；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查 +0/+15 枪械伤害、两把手枪全自动改造、远距离伤害、双持准星、霰弹/独头弹、射程/散布/换弹改造。

### 对话：怪物综合战斗等级区分度重构（2026-08-24）
- 原六维派生公式单点权重仅 `0.015～0.06`，当前24个怪物集中在1～8级，且图鉴派生等级会覆盖运行时配置等级，间接干扰地牢属性成长与经验语义。现拆分为两条明确口径：`enemy-config.json.level` 继续作为成长/经验配置等级；新 `combatLevel` 只负责图鉴战力横向比较，不参与经验、地牢锚定成长或存档等级。
- 综合战斗等级以六维为主体，力量/敏捷/智力/精神各×0.08、体质×0.10、幸运×0.04；最大生命按平方根曲线贡献并封顶8级，配置移速从80px/s以上开始贡献并封顶4级，普通/次要/精英/领主/首领分别追加0/1/3/5/7级机制威胁补偿，最终四舍五入且最低1级。公式唯一真源明确命名为 `data/combat-formulas.json#enemy.calculateCombatStats.combatLevel`，派生结果也只通过 `combatLevel` 返回，代码兜底镜像相同默认值，避免与配置 `level` 再次混淆。
- 当前24个怪物的静态分布由旧1～8级、7个唯一档位扩展为5～32级、17个唯一档位；胖子僵尸8、普通僵尸10、黑狼15、矿石蜘蛛22、红狼王23、集合体27、僵尸工头29、时空特工突击32。怪物卡片直接显示战力等级，详情同时展示“配置等级（成长/经验）”“综合战斗等级”及基础/六维/生命/移速/阶级五项构成，后续调整配置可直接看到贡献变化。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查怪物图鉴等级排序、旧地牢成长/经验是否保持原配置等级口径。

### 对话：怪物图鉴机制、贴图映射与废弃怪物清理（2026-08-24）
- 胖子僵尸新增“腐蚀毒域/厚重腐肉”机制说明；活体毒域、死亡动画、尸体毒域的伤害节拍、范围和持续时间统一读取 `enemy-config.json.attackSkills.corrosionAura`。死亡动画结束后，尸体在地面留下持续 6 秒的 200×50 腐蚀区域，每 0.5 秒造成 8 点魔法伤害；图鉴实时参数会随配置重载同步更新。
- 逐项静态核对当前 24 个怪物档案：红狼王补齐 512×512、4列、12帧的 idle 图鉴精灵图信息；僵尸犬兼容读取 `textures.frameLayouts.idle` 的 6列12帧布局。当前 24 个怪物的 idle 路径全部存在，缺图时不再伪装成红色圆圈，而显示明确的冷钢“缺图”占位；配置完整性检查新增怪物 `textures.idle` 与嵌套 idle 帧布局契约。
- 机制说明与实现调用链逐项核对后，补上巫婆首次交战召唤伴生煮锅的说明；黑狼阶段配置删除运行时未消费的冲锋、攻速和射程字段，图鉴不再宣称不存在的能力，只保留真实生效的阶段移速变化。
- 从双份怪物配置、BootScene 旧贴图生成与加载、旧育母蜘蛛 AI 配置及遗留默认引用中删除小蜘蛛、普通蜘蛛、狼蛛、育母蜘蛛、骷髅兵、骷髅射手、骷髅犬、亡灵法师。独立的“狼蛛/骷髅射手”武器附魔不是怪物代码，按功能边界保留；未删除历史日志与素材归档。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查胖子尸体毒域、红狼王/僵尸犬图鉴首帧和 24 个怪物的机制页。

### 对话：研究院 LV2/LV3 12 步结构候选出图（2026-08-24）
- 经用户明确授权，将新重建的 LV2/LV3 `Body Depth` 控制图及对应提示词发送到局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、`1024×1024`、12步、CFG 3.5、Depth strength 0.78 各生成默认5张结构候选；LV2 种子为122221～122225，LV3 种子为122231～122235。
- 10张候选均完成 raw、全画布 HSV 去绿、伪地台清理、底边锚定、主体遮罩和审批预览派生链，并生成 LV2/LV3 各自的5张联系表。逐像素检查20张 `body/preview` 派生图，符合项目绿色判定区间的可见绿色像素均为0。
- 视觉筛选建议优先采用 LV2 `v02` 与 LV3 `v02`：两者保持唯一中央塔、四面相连低矮裙楼和四根平顶围柱，尖券窗及塔身分层同属哥特式家族，且 LV3 的塔身高度和装饰密度明显高于 LV2。当前仍停留在结构候选阶段，正式 `assets/terrain/research_institute.png` 未替换，也未启动48步精修；未运行测试、构建、lint、浏览器探针或游戏运行时验证，等待用户确认候选编号后继续。

### 对话：研究院 LV2/LV3 单塔纵向升级建模（2026-08-24）
- 以已确认的 LV1 哥特式母体重建 LV2/LV3：方形 2×2 地基、四面相连低矮裙楼、四根平顶菱形截面围柱及其低位双层飞扶壁全部保持同尺寸同位置；升级只集中在唯一中央塔，塔身高度依次由 LV1 的276提升至 LV2 的336、LV3 的396，冠部、蓝色方锥顶与顶饰同步逐级拔高。三档研究院共享可完整容纳 LV3 的正交画幅尺度和相同底边锚定，避免自动相机让高等级地基在 Depth/control 图中反而缩小。
- LV2 增加双层塔身尖券窗、三道冷灰分层石带、相连学术回廊栏带、加强角扶壁、裙楼学术檐口和蓝顶肋；LV3 增加三层塔身尖券窗、四道石带、放大回廊与冠部、密集冠部角柱、强化蓝顶肋和最高顶饰。两级继续严格使用低饱和蓝、白、冷灰，不引入侧塔、角塔、穹顶、暖色金属或独立附属建筑。
- 恢复 LV2/LV3 Blender 入口、建模 manifest、生成提示词、候选 manifest 和组件登记，并分别输出可编辑 `.blend`、1024 材质预览、完整 Depth、去双层地基 Body Depth 与 48px 可读性预览。Body Depth 改为从已保存模型启动隔离的 Blender 子进程渲染，规避 Blender 5.1 在同一进程连续合成时偶发全黑或残缺；正式 `assets/terrain/research_institute.png` 未替换，也未生成 5080 候选。仅执行本地 Blender 建模渲染、Depth/Body Depth 与 48px 人工视觉检查；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户确认造型后再进入每级默认5张的12步候选出图。

### 对话：图鉴冷钢响应式重构与怪物数据口径修正（2026-08-24）
- 图鉴改为冷钢档案统一外观：角色系统与图鉴内层页签、分类及卡片使用原生按钮和可见键盘焦点；宽栏保持列表/详情双栏，窄栏改为详情滑入层，低高度下压缩非必要标题与间距。列表、分类栏和详情分别承担滚动，统一 8px 冷钢滚动条、稳定滚动槽与 overscroll 边界，系统面板保持 45vw 标准宽度、提供 640px 可读性下限且不超过视口。
- `DataLoader` 转换怪物配置时保留 `enemy-config.json` 完整条目，新增字段不再被白名单静默丢弃；图鉴技能参数折叠区直接遍历 `attackSkills`，配置数值调整并重新载入后会自动反映，不依赖手写说明文本同步改数字。`data/` 与 `public/data/` 当前怪物配置哈希一致。
- 新增 `enemy-base-stats.js` 作为怪物出生基础数值的唯一纯函数口径，`Enemy`、图鉴和采用怪物公式的仓鼠友军共用，不再各自复制公式；只有 `atk/matk/mdef` 允许直接覆盖，`def/crit/critRes` 继续由六维派生。双份 `enemy-config.json` 移除了当前实际不生效的 `def/crit` 旧值，配置完整性契约新增必填数值、ID、`attackSkills` 结构和禁止派生字段检查，防止后续再出现“配置写了但运行时忽略”。`level` 保持成长/经验配置语义，派生战力另由 `combatLevel` 表示，避免图鉴指标改动地牢成长与经验平衡；常规移速继续同步应用 `combat-config.json` 的全局怪物倍率，经验仍按位面、阶级和等级差动态结算。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查 1080p/2K/窄窗口布局、滚动与返回、键盘焦点、怪物基础属性和技能参数同步。

### 对话：玩家普通近战三段范围与打击感优化（2026-08-24）
- 普通剑类三段攻击统一使用扇形判定：一段 `105°/约190px`、二段 `125°/约205px`、三段 `125°/约228px`；第三段按后续明确要求从纯前向矩形恢复为与第二段同张角的扇形，仍保留更远判定半径、36px 前迈和终结段数值。
- `sword.hitBox.minimumBaseRange=165` 作为四把长剑的基础范围下限，运行时对旧存档和兼容装备携带的 77~155px 历史范围同样兜底；正式装备 JSON、初始装备与商店兼容数据统一为 165px，并把四把长剑的说明统一为“三段连击（扇形挥砍）”。
- 三段首次有效命中分别加入 35/45/70ms 顿帧与递增震屏，多目标命中每段只触发一次；原有目标 footprint、高度承载面、墙体遮挡、近战盾反、逐目标一次命中、击退/眩晕、第三段前迈/停留帧/单次音效均保持原链路。
- 属性面板“攻击距离”改为按当前武器、改造距离与三段 `rangeMul` 实时显示 `190/205/228px`，不再显示与实际判定不一致的旧 205px 单值。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查三段扇形边缘命中、二/三段群怪覆盖、第三段贴墙前迈以及顿帧强度。

### 对话：近两日代码审计四项冲突修复（2026-08-24）
- 场景切换失败回滚从“仅放回实体引用”升级为按拆场阶段恢复：拆场前失败只还原状态机；拆场后清理未提交的目标运行态，常驻世界从刚捕获的位面快照重新初始化防守、建筑、资源、迷雾与入侵系统，主城、地牢和瞬态任务实例则恢复实体、友军、地形、墙体与相机快照。裂隙系统新增运行态快照，保留调查进度、活动裂隙、场景尺寸和放置校验器，不再出现任务实例已恢复但裂隙永久消失；已销毁的投射物特效不再作为失效引用塞回效果列表。
- 玩家 F1 RTS 地面寻路取消敌群专用的首次 0~250ms 错峰，继续同步计算；A* 预算延迟或目标不可达时保持原地并重试，确定失败按400ms节流，不再直接朝原目标穿墙硬顶。跨层攻击找不到合法楼梯路线时同样等待重算，不再退化到城墙正下方的地面点。
- 地面方阵在各单位独立投影后增加一次性落点预约：按双方 footprint 半径和16px余量拒绝重叠，并在原槽位360px内确定性搜索替代站位；普通移动和移动攻击共用，楼梯/墙顶继续旁路到高架 FIFO。该搜索只发生在下令瞬间，不增加逐帧队形、共享A*或统一移速成本。
- `public/data/defense-structures.json` 的城墙楼梯单段造价同步为250，与运行时 `data/` 真源和当前经济重平衡记录一致。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查强制制造场景加载异常后的任务/世界恢复、不可达墙体指令、建筑密集区10+单位编队，以及楼梯造价显示与扣费。

### 对话：玩家普通攻击第三段人物线条轻度细化（2026-08-24）
- 仅处理终结突刺人物贴图 `assets/player/attack_sword_3.png`：复用项目既有 `thin-strokes.py` 的已验收 `T=1.2` 口径，对 16 帧深色笔画做一层距离变换收缩与软边混白，缓解肋骨、手臂和腿部轮廓相较前两段过度加粗的问题。
- 暗笔画平均线宽由约 4.22px 收敛到 3.95px，接近一段约 3.63px、二段约 3.76px 的视觉区间；Alpha 通道、`2048×2048` 帧表、4×4 布局、每格 512×512、人物姿势、脚底基线及格内位移全部保持不变，攻击时序、武器跟手、命中和第三段前迈配置未修改。已完成逐帧非空、Alpha 等值与深色背景对比预览检查；未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查第三段从起手到突刺、收势与前两段切换时的线条一致性。

### 对话：研究院推翻旧三塔方案并从 LV1 重建哥特式模型（2026-08-24）
- 删除建模包内旧研究院 LV1、LV2、LV3 以及蓝顶升级短名单四个目录，共清理旧 `.blend`、Depth、12/48 步候选与联系表约 219MB；正式运行中的 `assets/terrain/research_institute.png`、缩略图、窗光层和四张环境光照图全部保留，未让未验收模型进入游戏。
- LV1 改为完整组件化哥特式单体：唯一中央方塔、向北南东西伸出的四面低矮裙楼、四根位于对角间隙且截面旋转45度的菱形围柱，以及连接围柱和中央塔的双层飞扶壁。门窗统一为蓝色尖券玻璃，围柱使用平阶柱帽避免读成额外塔楼；主色严格限定为低饱和蓝、白、冷灰，不再使用棕木、琥珀窗或黄铜装饰。
- 同步重写 Blender 装配、建模 manifest、LV1 提示词、候选 manifest 和组件登记表；旧 LV2/LV3 建模入口及提示暂时移除，待 LV1 视觉方向确认后再以同一母体逐级扩展。新输出包含可编辑 `.blend`、1024 材质预览、完整 Depth、去双层地基 Body Depth 和 48px 可读性预览。
- 修正 Blender 5.1 同一内存场景连续输出两份 Depth 时第二份可能全黑或缺失上半轮廓的问题：Body Depth 输出前重新载入刚保存的预合成 `.blend`，再隐藏 manifest 指定的地基组件并独立渲染。仅执行本地 Blender 建模渲染和人工视觉检查；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户确认 LV1 造型后再进入 12 步出图。

### 对话：四把近战长剑背包装备栏贴图重制并替换（2026-08-24）
- 重新生成生锈的长剑、符文长剑、夜与火之剑、骑士长剑四张背包/装备栏图标；分别以现有手持武器贴图作为造型参考，统一为左下至右上的完整单剑构图，剑尖、护手、握柄与配重球全部保留，不再使用旧版局部裁切、白底或带水印图。
- 四张生成图统一通过项目本地 BiRefNet-general 正式抠图入口重建真实 Alpha，并进行亮灰边缘去污染；成品均为 `1536×1536 RGBA`，主体最长边约占画布 90%、按 Alpha 包围盒居中且四边保留透明安全区。
- 已直接替换原 `iconImage` 指向的四个稳定资源路径，`data/equipment.json` 与 `public/data/equipment.json` 无需改动；战斗中使用的 `equipImage` 手持贴图及武器数值、攻击逻辑均未修改。已完成 PNG 模式/尺寸、Alpha 范围、包围盒边距及 44px 深色装备栏缩略图视觉检查；未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查背包、装备栏与物品详情中的缩略图清晰度和缓存刷新。

### 对话：建筑视野门禁预览缓存（2026-08-24）
- 建筑盲区门禁新增精确 footprint 结果缓存：同一场景、同一战争迷雾网格对象及同一 `revision` 下，重复预览相同菱形占地时直接复用 `VISIBLE` 结论，不再重复执行多边形与雾格相交计算；普通建筑外围16格、方块墙/道路、4格门不同方向、楼梯全部分段和普通圆形兜底均走同一缓存层。
- 缓存只覆盖迷雾可见性，不缓存单位、建筑占用、地形、资源或科技门禁；迷雾可见格变化、场景/网格替换、切换或取消建筑选择时自动失效，左Ctrl连续建造可继续复用。精确顶点不按雾格或小数位量化，并设置2048条上限，达到上限整体清空，避免长时间建造无界增长。补充静态契约断言，但未运行测试、构建、lint、浏览器探针、游戏运行时或性能基准，按项目约定由用户重点检查长距离拖墙/铺路、视野边界移动、F切换4格门方向、楼梯两侧切换及连续建造。

### 对话：建筑 12 步结构候选默认缩减为 5 张（2026-08-24）
- World-122 建模出图管线的 `structureVariants` 默认值由 10 改为 5，脚本在 manifest 缺省时的兜底值同步改为 5；命令行显式传入 `--variants` 时仍可按任务临时增加或减少候选数。
- 同步更新 `SKILL.md → skill/02-ai-asset-pipeline.md` 和 `tools/ai-gen/WORKFLOW.md` 的阶段说明、示例命令及默认参数记录。未运行测试、构建、浏览器探针或游戏运行时验证，本轮仅完成工作流配置与文档静态核对。

### 对话：研究所 LV2/LV3 蓝灰屋顶结构候选重出（2026-08-23）
- 以当前 LV1 研究所的低饱和旧化钢蓝/蓝灰石板屋顶、暖灰石墙、深栗木构和琥珀暖窗为唯一配色基准，收紧 LV2/LV3 提示词，明确排除高饱和钴蓝、电蓝、青色及红/橙/棕/铜色屋顶。
- 候选脚本支持直接复用已建模的主体 Depth 控制图、逐建筑 `paletteConstraint` 和可配置的 ComfyUI 等待时限；LV2/LV3 分别沿用各自组件化白模轮廓，不以文生图重新猜测塔楼结构。
- 经用户明确授权向局域网 `192.168.3.142:8188` 发送两张研究所主体 Depth 与对应提示词，使用 `flux2-dev-depth` 各生成 10 张 12 步结构候选；现阶段推荐 LV2 `v05`、LV3 `v02`，仍等待用户选定编号后再各做 3 张 48 步低重绘精修。
- 修复绿幕抠图只删除画布边缘连通区域、导致塔楼夹缝保留绿色块的问题：LV2/LV3 启用显式全画布 HSV 绿幕清理，从原始 `_raw.png` 重建 20 张候选的 keyed/cleaned/anchored/body/preview 链，并刷新 8 张短名单和 5 张联系表。逐像素检查 113 个交付 PNG，可见与隐藏绿色像素均为 0；原始绿幕图继续留作 48 步精修真源。
- 过饱和蓝顶及早期非蓝顶结果已移入各自 rejected 候选目录留档，未替换正式运行时贴图，也未进入抠图、光照图重建或配置接入；未运行测试、构建、浏览器探针或游戏运行时验证，本轮仅完成候选生成、静态数量核对与视觉筛选。

### 对话：玩家近战武器持续跟手与攻击时序修复（2026-08-23）
- 三段普通近战统一以各自人物攻击动画的当前帧与帧内进度驱动剑柄轨迹，墙钟仅在动画状态不可用时兜底；低帧率、切后台恢复或双 RAF 相位变化不再让一、二段武器提前/落后人物跨帧。逐段同时校验预期动画键和轨迹帧数，避免同为 12 帧的一、二段误读彼此姿态。
- hold 到 recover 改为由 pose session 统一决定：`animationcomplete` 不再按墙钟抢先切 idle，武器参数快照前只预推进已到期的近战定格，使人物与武器在同一渲染帧从攻击末帧进入收势；第三段 `stage3HoldMs=0` 同样连续，枪械 twist/aim 的全局更新顺序未改。
- 近战逐帧与收势入口显式清除枪械朝左瞄准遗留的 `flipY`，修复“枪朝左→切剑→立即攻击”时剑贴图纵向翻转、剑柄脱手；收势起点继承攻击末帧 `stretchX/stretchY`，剑柄反推中心同步计入纵向拉伸，第三段末帧 1.048 倍宽度不再瞬间缩回。
- 挥击音效、命中判定与第三段根位移统一复用 `getSpriteFrameBounds()` 的真实帧起点，支持 `frameDurations/frameWeights` 非等时节奏；一次攻击仍由既有 `soundPlayed` 会话守卫只播放一次挥击音效。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查三段左右朝向全程握把贴手、各段定格转收势、枪切剑立即攻击、第三段音效/命中/前迈节奏及收势尺寸连续性。

### 对话：建筑禁止放置在当前视野盲区（2026-08-23）
- 建筑预览与实际落地接入位面战争迷雾同一 `FogOfWarSystem` 当前可见网格；`UNEXPLORED` 与已经探索但当前重新变暗的 `EXPLORED` 区域都不能建造，点击时提示“视野盲区无法建造”，无迷雾场景不受影响。
- 门禁不只检查建筑锚点：普通建筑完整菱形 footprint、会随建筑生成的外围道路/田地、方块墙与道路单格、4格门全部格及城墙楼梯全部段都必须保持 `VISIBLE`。预览会直接显示为不可放置，且落地前再次复验，不会扣费后才撤销。补充静态回归断言，但未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查未探索黑区、已探索暗区、视野边缘的大建筑/道路环、拖墙、铺路、4格门和楼梯。

### 对话：仓鼠民兵草叉误计体量导致角色放大修复（2026-08-23）
- 按 `SKILL.md → skill/09-monsters-npc.md` 的长武器主体轮廓例外重新量化：民兵 running 整框 Alpha 中位高 181px 包含草叉，不能作为仓鼠体量。对二值轮廓做 5px 形态学开运算去除细长草叉后，主体中位高约 156.5px，与仓鼠牧师 155px 主体基准基本一致。
- 以牧师游戏内 75.684px 可见主体高度反算，民兵由错误放大的 `displaySize:460` 改为 `247.603834`，脚线同步改为 `spriteOffsetY:-45.458516 / footOffsetY:45.458516`；`hudOffsetY:119`、20×100 碰撞、攻击范围和战斗数值保持不变。补充了不允许草叉影响主体缩放的配置契约，但未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点对比民兵与牧师/战士的躯干高度、草叉伸出范围、脚底贴地及攻击/死亡动作切换。

### 对话：上墙掉队单位超时后永久卡住修复（2026-08-23）
- 确认是显式 RTS 高架寻路的恢复链漏洞：单位排队超时后虽已退出 FIFO，但控制器把 `_surfaceExplicitProgressAt` 写为 `0`；下一帧又以 `Number(value) || now` 读取，`0` 被当作缺省值替换为当前时间，导致“已超时”信号每帧被吞掉。结果是该单位既不在队列里，也永远不会重规划、重新选梯和再次入队。
- 显式路线进度时间改为显式的有限数检查，队列超时时直接写入“当前时间减去进度阈值”，保证下一帧立即进入原有恢复流程：释放旧预约、清除旧地面路径、重算高架路线并从新入口重新排队。补充了“超时退队后必须触发一次重规划”回归用例，但未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查掉队者是否会自动回靠、重新入队并最终到达墙顶。

### 对话：多兵上城墙队尾超时掉队修复（2026-08-23）
- 继续定位“每次固定有1～2个士兵留在墙下”：楼梯 FIFO 原本把 `portalQueueWaitTimeoutMs` 当作单位从入队开始的绝对总等待时间，队首持续正常上墙也不会给队尾顺延截止时间；长队最后几人因此在轮到前超时、重规划并重新排到队尾。
- 排队超时改为“无进展看门狗”：只有队首获权、前方单位取消或失效等 FIFO 顺位真实前进时，才为实际前移的后续单位重置等待窗口；重复 `request` 不续期，完全停滞的入口仍会按原 6 秒阈值进入恢复。补充了“队首晋升后队尾不沿用旧截止时间”回归用例，但未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查 6～12 人连续上墙、最后两人、队列中途取消/死亡及真正堵死后的超时恢复。

### 对话：多兵上城墙入口队列互推卡死修复（2026-08-23）
- 定位到部分士兵停在墙下并非缺少楼梯路线，而是楼梯 FIFO 与地面友军分离互相冲突：等待 Portal 许可的单位仍参与单位间推挤，会把已获上行许可的先行者推离入口，等待者自身也被推离路线节点，继而反复回靠、超时和重规划。
- 高架交通友军豁免扩展到 `_surfaceNavWaiting` 队列态：任一方正在楼梯入口排队时只关闭双方 unit-vs-unit 分离，让获权者可以穿过等待队列，后续单位继续按 FIFO 依次获权。墙段、建筑、楼梯侧边、防坠线、Portal 方向许可、反向互斥和表面身份提交均未放宽；未排队的普通地面士兵仍正常互相分离。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查多兵连续上墙、单梯入口拥挤、上下行冲突后恢复、等待单位是否仍受墙体约束，以及最后一名士兵能否离开墙下到达墙顶。

### 对话：RTS复数单位方阵列队移动（2026-08-23）
- 框选多个友军后的右键移动、移动攻击与巡逻不再使用黄金角螺旋散点；改为以点击点为几何中心、朝整组选中单位到目的地的移动方向旋转的紧凑方阵，自动按人数生成前后排与左右列。每个单位按当前位置领取最近空槽，减少行军时相互穿插；玩家本体继续走专用 RTS 控制器，队员和仓鼠部队继续消费原有命令协议。
- 阵型只在下令瞬间以 O(n²) 最近槽分配计算一次，不增加每帧队长、队形约束或全组寻路；每个单位仍独立使用现有落点投影、A*、墙体/建筑避障与分离，因此窄路可自然打散并在终点重组。楼梯和墙顶等高架命令继续交给逐单位表面路线与 FIFO 通行，不把方阵硬塞进狭窄表面。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查2/3/5/10+单位远近距离移动、混合体型、建筑密集区、移动攻击/巡逻，以及通过窄门后到达终点的重新成阵。

### 对话：建筑模式独立指挥镜头、城墙指令箭头与友军自动门寻路修复（2026-08-23）
- 建筑模式不再只是写入 `_buildMode`：开启时显式关闭 RTS/普通指令点选并锁住玩家移动、攻击与武器输入，相机停止跟随玩家，复用指挥模式的 24px 屏幕边缘与 900px/s 自由平移；`Space` 在建筑模式可切换压平视图，但建筑模式本身始终不置为 RTS。退出时恢复玩家输入与镜头跟随。
- 建筑主栏目选择建筑后以独立 `is-placement-hidden` 状态滑出，不销毁栏目也不退出建筑模式；右键或放置中的 Esc 取消选择后滑回，成功放置单体、方块墙行或道路行后同样结束本次选择并自动返回建筑内容。成功落地时若仍按住左 Ctrl，则三类放置统一保留当前建筑选择和隐藏栏目，可直接连续建造；松开后下一次成功放置恢复正常返回，一次性打包重建不受影响。未完成的非法落点仍保留当前预览，便于继续调整。
- RTS 与普通组队栏右键移动统一先解析高架目标；普通队友也按单位补全上下楼路线。绿色箭头同时消费目标地面 `y`、高度 `z` 与承载层 `renderDepth`：屏幕落点仍是 `y-z`，排序深度则至少高于实际城墙/连接器/楼梯层，不再把抬高后的屏幕 Y 当作地面深度而被城墙盖住。
- 关闭城门并非寻路中的硬阻挡，原逻辑统一施加 `GATE_SOFT_COST=6`，两格门洞约等效增加 400px 路程，正是友军舍近绕行开放缺口的根因。现在只有玩家/友军对明确标记、可为友军自动开启且非 `locked` 的门使用正常成本；敌军、常锁门、竞技场门仍保留原软成本。路径缓存与格子 memo 按门通行档隔离，门模式切换会同步局部失效缓存，并使正在行进的玩家/友军按原目标重算。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查建筑选择/取消/完成、Space 压平、四边镜头平移、墙顶/楼梯箭头，以及友军在自动门/常锁门/开放绕路并存时的路线。

### 对话：玩家近战第三段突刺武器跟手与停留帧调整（2026-08-23）
- 第三段武器同步不再只按攻击起始墙钟换算统一进度，而是读取 Phaser 人物攻击动画的实际当前帧与帧内进度；逐帧握点因此与人物拳头使用同一时间源，长帧或非等时帧不会提前跳到下一握点。一、二段已验收映射保持不变。
- `attack_sword_3` 最大伸展区的第10、11帧各由50ms延长到100ms，相当于在终结突刺增加两帧停留；总攻击时长由800ms同步增长为900ms，命中判定仍落在最大伸展末端。
- 第三段按 `sword.attack3.rootMotion` 在第7～10帧执行36px前迈，使用人物动画逐帧真实时长和固定攻击方向做缓出位移；起步前清除残余移动速度，位移统一经过 `WallSystem.resolve` 并即时同步 Collider，不会因前迈穿墙。`_pendingThrust` 仍保留起手固定判定原点，第三段现有攻击范围与平衡数值不随这次动作调整扩大。
- 近战攻击在改写武器状态前增加会话防重，避免重复入口把 `attacking` 覆写成 `swing` 后再次创建 Tween；因此第三段一次攻击只会启动一个挥击音效会话。命中目标时原有的独立命中声保持不变。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查第三段起手到突刺全程握把贴手、左右朝向、36px前迈及贴墙截停、突刺停顿力度、空挥只响一次挥击声、命中反馈及三段后收势。

### 对话：性能页周期排行与一键复制报告（2026-08-23）
- 交互开发工具“性能”页新增最近60/120/240帧统计周期选择；逻辑主循环、Canvas、Phaser同步和DOM UI按周期平均CPU耗时从高到低排列，同时显示占已测分项比例、平均、P95、峰值与周期累计耗时，便于直接判断优化优先级。慢帧数量与比例改为按当前所选周期展示，原实体、HUD、寻路队列和门追击缓存计数保留。
- 新增“复制性能报告”按钮，一键导出可直接粘贴研究的Markdown文本，包含导出时间、实际采样帧数/时长、FPS、帧耗时、完整CPU耗时排行榜和运行计数器；剪贴板API不可用时自动回退到隐藏文本域复制。占比明确只以当前已测CPU分项合计为100%，不冒充整机CPU、GPU或内存占用。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查三个周期切换、排行顺序、自动刷新和复制结果。

### 对话：全体怪物首次贴图尺寸回归修复（2026-08-23）
- 定位到性能提交移除敌人 Phaser Arcade Body 时，把旧 `_configureEnemyBody()` 内的首次视觉尺寸初始化一并删除；敌人创建时已经持有当前纹理，后续 `_syncEnemyAnimation()` 只有纹理变化才重算尺寸，导致普通僵尸等怪物首次出现时按512px等素材原始帧尺寸放大。问题属于共享 `GameScene` 渲染回归，不是 walking 素材本身变大。
- 新增不依赖物理体的敌人纯视觉初始化，首次创建、外部 `enemySpawn` 与动作纹理切换统一按 `options.spriteSize → config.render.spriteSize → size×4` 等比缩放，并恢复非方形帧比例、首帧与圆形占位 tint；红狼王同纹理变身继续保留显式动态尺寸。普通僵尸临时每帧缩放补丁已撤销，僵尸犬尺寸改为读取 `config.render.spriteSize`。没有恢复 Phaser Arcade Body，动态怪物继续使用当前 Collider 圆形 ground footprint。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查普通僵尸 walking、黑狼多纹理、手脑非方形 walking、红狼王变身、僵尸犬及离开视口后返回的尺寸。

### 对话：雪原与森林位面镜头同步世界122广角（2026-08-23）
- 将世界-123·雪原（scene9）和世界-124·林地（scene10）补入大世界广角名单，与世界-122（scene8）、世界-125（scene11）统一使用0.7基础缩放，1080p下可视范围约比1.0增加43%；主神空间、地牢、开火镜头推近、瞄准偏移、观察/指挥模式、边界钳制和小地图逆缩放补偿均保持原逻辑。四个位面名单集中为`ZOOMED_OUT_WORLD_SCENES`，雪原/林地/遗迹三份回归契约同步锁定。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查雪原与林地进入后的视野大小、切回主神空间恢复1.0、开火推近及小地图位置。

### 对话：传送返回主神空间后小地图内容越栏修复（2026-08-23）
- 根因是 `switchScene` 在目标加载器执行完后才提交 `currentScene`：`_loadMainScene()` 虽先写入主神空间 `12288×8192`，随后 `Renderer.generateWorld()` 仍按离场位面判断并把运行时尺寸覆盖成默认 `7680×4320`；主城菱形边界继续按 `12288×8192` 注册，小地图用错误的小世界尺寸换算后便持续放大到栏外。
- `Renderer.generateWorld()` 现可接收显式目标场景，回城的正常快照与兜底重建两条路径统一传入 `main`，不再依赖尚未提交的 `currentScene`。主城地形、WallSystem 和小地图因此共用同一世界尺寸；场景提交后还会在揭开加载层前先应用目标场景 zoom、清除小地图节流并立即重画，避免残留的 0.7 缩放产生短暂越栏。其他场景加载、宽版/地牢小地图规格、迷雾与常规 10Hz 刷新策略均未修改。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查从 scene8~scene11/地牢返回主神空间后的地图边界、玩家/传送门点位及相机视野框是否始终留在地图栏内。

### 对话：所有实时阴影统一增加内收式边缘淡出（2026-08-23）
- 环境光照公式新增统一阴影羽化参数与 `smoothstep` 透明度曲线：单位、敌人、友军、NPC及地牢障碍物共用的 `entity_shadow` 由同一公式生成径向渐隐；中心密度和既有 footprint 尺寸不变，纹理仍只创建上传一次。
- 建筑、城墙、门、楼梯、树木及散布障碍物的共享结构投影改为“先聚簇并集、再向内羽化”：最外轮廓保持原几何，默认8px/5层只向阴影内部收缩；每层 alpha 按 source-over 反解，最深处仍精确达到原 opacity，避免边缘硬切、外扩越过建筑接地边或相邻阴影重复加深。昼夜/地牢强度、太阳方向、阴影长度、渲染深度、雾区过滤及脏帧缓存均未改变。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查单位脚下软边、建筑/城墙投影四周、相邻结构合并边界、夜间及地牢阴影观感。

### 对话：传送返回主神空间后小地图黑图频闪修复（2026-08-23）
- 根因不是第二个 GameScene 或 DOM 小地图重复创建：scene8~scene11 的战争迷雾小地图 `Phaser.Image` 随常驻 GameScene 保留。返回无迷雾的主神空间后，每帧前段会正确隐藏它，但非地图 HUD 恢复分支随后又只按对话框状态无条件点亮旧黑色纹理；真正的小地图同步按 100ms 降频，定期再次隐藏，于是形成约 10Hz 的第二块黑图频闪。
- HUD 恢复战争迷雾小地图时现同时要求当前场景的 `FogOfWarSystem` grid 处于 active；主神空间和无迷雾地牢不会再复活旧位面纹理，scene8~scene11 的正常迷雾、普通小地图静态/动态层、缓存键和 10Hz 更新频率均未修改。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查“主神空间→任一已建位面→返回主神空间”、连续跨位面及对话框/地图模式显隐。

### 对话：仓鼠士兵上下楼梯卡死修复（2026-08-23）
- 根因不是地面 A* 或楼梯几何：玩家控制器每帧会发布 `_surfaceInputIntent`，仓鼠士兵共用的 `MovementSystem` 却没有；而墙顶不安装 `_surfaceMoveAxes`，无法生成备用 `_surfaceMoveChosenAxis`。单位执行 `wall_walk → stairs` 时因此始终被 Portal 意图门拒绝，路线节点无法取得真实楼梯身份，已获得的下行预约还会持续续租并堵住反向上楼队列。
- `MovementSystem` 现为高架路线、楼梯、墙顶与桥接中的 AI/友军发布归一化实际移动意图；排队、无目标及各类提前返回先清空旧意图，避免残留方向误切面。同步复用玩家已有的 34ms 高架位置积分上限，仅限制位移、不改变 AI、动画和路线计时，防止长帧越过窄连接面后反复回夹。路线阶段、FIFO 方向互斥、表面几何与地面 A* 均未放宽。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查单兵/多兵连续上下楼、上下行冲突后的放行、墙顶转入楼梯、楼梯到墙顶及长帧场景。

### 对话：恢复高射速枪械连续开火声（2026-08-23）
- 定位到8月23日音频性能池回归：普通 `playFile` 同路径最多4实例会直接吞掉G18/QJB等55~60ms连射请求；第一轮虽放宽池并抢占旧尾音，但 HTML `<audio>` 的异步 `play()` 与回拨在该间隔下仍会产生启动/seek抖动，且同路径12路不足以覆盖双持G18同周期两发与约600ms尾音。
- 保留普通音效池化与35ms去重，正式枪声在音频系统初始化时异步预解码，逐发播放复用 AudioBuffer 并为每发创建轻量 `AudioBufferSourceNode`；默认全局32、同路径24路，满载时只截断最旧尾音而不丢新枪口瞬态。未预载的新路径首次触发时再解码，解码期间和解码失败路径继续使用高速 HTML Audio 兜底且不重复拉取。玩家主副手、通用远程攻击、敌人位置枪声、Combatant 射击和防御塔枪声继续共用 `playGunshot/playGunshotAt`。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查G18双持、QJB-201、PKM、AKM/M416、防御塔持续扫射及混战中的其他音效。
- 用户复测仍断续后完成全局调用审计，发现玩家 PKM/AKM/M416/QBZ/QJB/能量轻机枪分支一直绕过 `_playFireSound`，直接调用普通 `playFile(lmgSound)`，所以前两轮枪声专用后端从未覆盖这批高射速主武器；现已让手枪、散弹、机枪/步枪三条玩家发射分支全部汇合到逐发枪声入口，并保留改造覆盖和能量轻机枪专用音色。同步修复防御塔基类开火声与枪口层重复播放、散弹塔逐 pellet 重复播放的问题，统一为一次真实击发只提交一次枪声；敌对持枪者、防御塔、仓鼠火枪与赏金猎人也统一进入带位置衰减的逐发枪声入口，避免多单位齐射被普通去重池合并。SAIGA、P4040改造、时空特工与两类仓鼠火枪声加入预解码清单。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户复测上述武器和多塔/多单位混战。

### 对话：研究院 LV2/LV3 建模、经济岗位与科技树科研链（2026-08-23）
- 基于既有 LV1 三塔研究院组件重构通用等级建模器，新增 LV2/LV3 独立可编辑 `.blend`、1024×1024 材质预览、完整 Depth 与去地基 Body Depth。LV2 增加更高塔身、扶壁、双层檐口、门灯与黄铜尖顶；LV3 继续增加观测回廊、鎏金立柱、门廊、附墙天文钟与翼顶饰件，三个等级始终保持同一 2×2 基座、连接主厅和恰好三座塔。
- 研究院新增 `economyType:"research"`，进入经济建筑分类并沿用经济建筑的仓库前置；原“防御结构强化/能源回收矩阵/快速募兵”三项全局升级完整保留在同一详情面板。初始 3 个研究员岗位，每人发挥 20%，未安排岗位时科研点严格为 0；实际科研速度同时受人口效率与范围内最强经济工坊增效影响。
- 研究院 LV1/LV2/LV3 基础科研调整为 1/1.5/2 点/秒；“制度化科研”解锁 LV2 与扩编研究员、精密研究设备、学术分院网络三项新升级，“高塔学府”解锁 LV3。岗位可由 3 扩至 5，精密设备每级增加 0.15 基础科研点，每个位面研究院默认限建 1 座并可提高到 3 座。
- 科技树不再按研究院数量固定加点，而是汇总当前位面与所有后台位面的实际科研速度；研究院等级、岗位、人口超额、工坊增效和全局科研升级在前后台使用同一公式，科技面板显示真实点/秒与对应预计时间。研究院楼层读条继续复用房屋的本栋等级快照字段，跨位面离场后仍可完成。
- LV2/LV3 运行时贴图暂时回退到正式 LV1 贴图；按建筑资产流程，当前仅交付白模/Depth 供确认，未把未验收候选导入正式地形贴图。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查经济分类、仓库前置、0/3/5 人科研效率、两级科技门禁、六项全局研究卡、逐位面数量上限和跨位面科研速度。

### 对话：时间进度栏筛选、事件簇与天气预报详情（2026-08-23）
- 顶部玩家可见名称统一为“时间进度栏”，新增明确标题及“周期天数 · 事件数量”摘要；原距离入侵、目标详情、传送门耐久和前往支援功能全部保留，旧 DOM ID/CSS 类继续兼容现有调用。
- 时间事件改用按位置分配的两条防碰撞泳道并消除原3px纵向重叠，左右端事件向栏内对齐；短于一天的预报改显示游戏小时，“1小时内”改为“即将发生”。事件可用键盘聚焦并显示轮廓，危险动画服从减少动态效果设置。
- 时间进度栏从480px扩展至最高560px并接入冷钢主题变量；事件列表只在事件、位置或可见倒计时变化时重建，不再随10Hz HUD刷新反复创建和解码图标。
- 新增自动事件类型筛选：按钮从当前 provider 的 `type/typeLabel` 动态生成并显示数量，选择后摘要显示“可见/总数”，筛选不影响真实排期；未来事件类型无需修改HUD即可出现。
- 同一近时间窗超过2个事件时合并为 `+N` 事件簇，点击展开可滚动事件列表；天气项可继续点击进入预报详情，展示位面、强度、游戏内开始/结束日期时间和当前状态。详情支持再次点击、关闭按钮和 Esc 关闭。
- 每个事件增加独立定位于事件进度百分比的竖线，当前时间游标抵达事件时会精确重叠，不受左右端图标向栏内避让影响；入侵事件以红色脉冲、天气事件以蓝色脉冲提示，混合事件簇按入侵红色优先，减少动态效果模式保留静态语义色。
- 事件图标新增悬停/键盘聚焦说明层：天气显示位面、强度和起止时间，入侵显示当前状态与触发时间，事件簇显示事件数量和包含类型；移除重复的浏览器原生 `title` 提示，点击天气详情及事件簇展开行为保持不变。
- 时间进度栏默认切换为360px窄幅细版，仅保留同源轨道、事件图标、竖线和当前游标；底部向下箭头展开原560px详细版，展开后箭头反向并可再次收起，详细标题、筛选、袭击说明和支援按钮均未删除。
- 修正时间轴方向：“现在”固定在左侧4%安全锚点，五日窗口随当前游戏时间滑动，天气、袭击及后续事件按剩余时间持续向左移动并在触发时与现在竖线重叠；袭击绝对触发时刻仍由原倒计时决定。底线固定铺满并使用近处危险红到远处安全绿，不再让当前游标向右追赶静止事件。事件移动只更新已有 DOM 的 `left`，不会因10Hz刷新重复重建图标和悬浮窗。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查事件左移方向、触发线重合、天气/袭击倒计时一致性、简版/详细版和悬浮说明稳定性。

### 对话：性能诊断、视口裁切与寻路任务预算（2026-08-23）
- 新增统一 `PerformanceMonitor` 与交互开发工具“性能”页，按最近120帧显示平均FPS、帧耗时平均/P95/峰值、逻辑主循环/Canvas/Phaser/DOM UI分项耗时，以及实体、HUD、寻路队列、门追击缓存计数；性能采样、裁切边距和寻路队列上限集中写入双份 `performance-config.json`。
- `GameScene` 对相机视口外加320px缓冲区之外的敌人、中立实体、防御塔、友军、名字/血条、接触阴影和X光层停止创建或逐帧同步并隐藏视觉对象；实体active、AI、碰撞、寻路、小地图与战争迷雾逻辑保持不变，恢复显示后仍由迷雾执行最终可见性门禁。
- 敌人路径重算与有效性检查统一进入全局优先级队列：每帧最多2次重算、5次检查并受3ms排空预算约束；同一单位任务合并，卡住/过门追击优先，`PATH_DEFERRED`跨帧保留。玩家RTS寻路继续同步执行。开门追击将门/目标全表扫描改为250ms共享缓存，并优先使用1500px空间分区局部候选，原目标优先级和精确LOS不变。
- 未运行测试、构建、lint、浏览器探针、游戏运行或性能采样，按项目约定由用户重点测试沙漠位面30只毁灭怪+约20名友军时性能页数据、镜头边缘单位/血条/阴影恢复、战争迷雾叠加、开关门追击，以及大量单位同时卡墙时的寻路响应。

### 对话：事件时间轴冷钢图标生成与接入（2026-08-23）
- 使用内置图像生成插件参照现有科技图标的冷钢六边形语言，生成位面袭击、小雨、中雨、大雨、暴风雨五枚独立事件图标；成品统一为 `assets/ui/event-icons/` 下的 256×256 RGBA PNG，完整保留边框与透明安全边距。
- 根据统一性反馈，以“大雨”图标作为降雨系列不可变母版重新制作：四档共用完全相同的深灰积雨云、冷钢边框、构图和透明轮廓，只在云下依次使用稀疏雨丝、中等雨丝、密集雨幕，以及密集雨幕加单道无分叉闪电；避免此前各档云层造型和明暗不一致。
- 天气与袭击 provider 新增通用 `iconPath` 字段，顶部时间轴优先显示正式贴图，加载失败时仍回退原 emoji；HUD 不感知具体事件类型，后续 provider 可沿用同一接口接入新图标。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户检查时间轴 18px 实际显示效果。

### 对话：位面谐振塔组件化白模（2026-08-23）
- 新增 `planar_resonator` 组件化 Blender 建筑模型：完整 2×2 双层石质地台、中央金属机座、四座承重轴承柱、三道相互正交的完整黄铜陀螺环，以及唯一悬浮蓝紫晶核；环、晶核和承力结构均保持独立可编辑，并登记到建筑组件索引。
- 已输出可编辑 `.blend`、1024×1024 透明材质预览与同视角 Depth 图；首阶段仅完成建模候选，当时未导入 `assets/terrain`，也未接入科技树、建造配置、能源生产、存档或运行时动画。
- 用户确认白模后，使用同一 Depth、`flux2-dev-depth`、Control 0.88 和 12 步生成 `123170`、`123171`、`123172` 三张结构候选及联系图；三张均保留完整地基、四柱、三环和单晶核，当前推荐四柱连接与环体层次更清楚的 `123171`，等待用户选定后才进入 48 步低重绘精修。
- 用户选定 `123171` 后新增结构锁定的精修提示词，并以同一 Depth、`denoise=0.30` 生成 `1231711`、`1231712`、`1231713` 三张 48 步低重绘候选；三张均未改变三环、四柱、单晶核与完整地基，最终采用蓝紫晶核、旧黄铜和黑铁层次最均衡的 `1231713`。
- `1231713` 经本地 BiRefNet 分割、主体合成、4px 安全边与透明区 RGB 清理，正式导入为 877×702 RGBA `assets/terrain/planar_resonator.png`；运行时显示 278×223、脚线 110px，并锁定标准 2×2 的 256×128 视觉 footprint。同步生成建筑缩略图、ground-fit 记录与环境光照 silhouette/projection/height/normal 派生图。
- 位面谐振塔按后期经济建筑接入：建造消耗 6000 能源，仓库前置，基础 2 个谐振技师岗位、每人发挥 20%、5 人满效。基础晶核每 10 秒产生 100 原始能源并以 80% 回收，满员为 8 能源/秒、初始 2 岗为 3.2 能源/秒；四项本栋升级“谐振频率/晶核增幅/导能回收/谐振技师”满级满岗为 25 能源/秒，仍受全局人口效率、最强经济工坊增效和生产祭品倍率影响。
- 经济科技拆为“位面谐振”解锁建筑、“谐振校准”统一解锁四项升级；当前位面与后台位面共用真实仓库入库、岗位、升级读条和结算公式，满仓产量保留在本栋余量，建筑模块/读条/余量随快照恢复。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查科技门禁、建造扣费、岗位调度、四项升级、满仓恢复入库及跨位面结算。

### 对话：沙漠位面降雨固定为暴风雨（2026-08-23）
- `game-config.json#weatherEffects.rain.schedule.forcedIntensityBySceneId` 将scene8固定为`storm`；新排期、旧存档中已经预排的小雨/中雨/大雨、天气预报以及开发面板手动触发都会统一归一为暴风雨。沙漠位面开发栏只保留“暴风雨”按钮，其他位面继续使用原四档权重。未运行测试、构建或游戏运行时验证，按项目约定由用户重点检查scene8自动降雨、旧档预报图标和手动触发均为暴风雨。

### 对话：自动天气排期、预测塔占位与顶部时间事件轴（2026-08-23）
- 新增 `WorldWeatherSystem`，按统一游戏时钟为scene8~scene11分别预排下一场降雨的开始、结束与强度，间隔0.8~2.2天、持续0.25~0.6天，四档权重为45/30/18/7；排期写入主存档，地牢类型继续由视觉层门禁排除，降雨仍不改变视野、移动、战斗或地形。
- 工程分支新增“天气预测”科技，完成后解锁每个位面限建1座的“天气预测塔（占位）”；塔复用研究院贴图作为明确占位并随现有功能建筑快照持久化。只有对应位面存在存活预测塔时，该位面的天气排期才会向玩家显示。
- 顶部袭击栏改为五日周期时间事件轴：游标与填充随时间前进，袭击和已预测天气以图标、剩余天数标在轴上；原“距离入侵 N 天”、入侵波次、传送门耐久和支援按钮全部保留。新增 `WorldEventTimelineSystem.registerProvider()` 接口，后续时间事件可按统一事件结构接入，不需修改HUD业务结构。
- 交互开发面板的四档降雨按钮改为覆盖自动天气状态，仍只允许调整当前位面。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查科技解锁、预测塔建造/出售/跨位面存档、自动降雨起止、预测图标位置与原袭击支援信息。

### 对话：雪原初级地牢战斗房改为世界单格冰墙竞技场（2026-08-23）
- 雪原初级地牢新增 `combatRoom.wallConstruction:"worldBlock1x1"` 专用构建模式：房间四边严格按世界位面 128×64 的 1×1 footprint 铺设独立墙块，普通房为20个网格中心/边并按房间档位等比例扩展；四个转角各只复用一个墙块并以两条半边闭合碰撞，每块墙携带独立边界线段和 depth，房间改用2:1菱形，避免旧长墙整体拉伸、转角重叠和单深度遮挡。
- 玩家改从随机边中点入场、敌人从对边中点区域刷新；铺墙阶段直接预留中央6格门位，功能门按预留跨度精确映射并维持原关门困场、战斗结束开门和门外出口流程。门资源加载失败时恢复预留墙块，其他地牢仍使用原连续墙与原门闸逻辑。
- 根据实机反馈废弃“二维砖材质投射 Blender 立方体”的粗糙方案，改以现有 `assets/terrain/obstacle_block.png` 作为不可偏移的结构真源：本地 ComfyUI 低重绘仅生成冰材质细节，后处理恢复原墙完整 Alpha、砖块数量/尺寸/排布和等距轮廓。`frozen_wall_block.png` 与现有墙统一按260×259显示、脚线偏移61px并占128×64单格，避免大砖、跨面纹理和等比缩放造成的拉伸。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查四个随机入口、墙块接缝/遮挡/碰撞、门洞通行、普通三波与Boss房离场。
- 冰封竞技场不再复用普通砖拱门和金属栅栏，新增独立 `frozen_gate` 16帧冰锥门：13枚冰锥在关门时从地面升起、开门时沉入薄霜接缝，视觉与碰撞共同覆盖完整6格预留门洞；门洞两侧继续由单格冰墙承担，不再把门框或砖墙横向拉伸。冰封门音效同步改用现有冰墙音效，其他地牢的功能门资源与行为不变。
- 冰锥门二轮精修保持13枚轮廓、640×640单帧、16帧升降和门线几何不变，在确定性生成器中增加晶体多切面、崩边、分叉冰裂、气泡、根部乳霜与接缝碎冰；运行时把同帧门图裁成浅/中/深三段独立 depth，每段按自己的底边 `maxY+3.9` 排序，始终位于同线 `+4` 冰墙之后，单位遮挡缓存同步使用三段门线。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查门墙交界、三段裁剪是否无缝、开关门动画和单位穿门遮挡。
- 雪原初级正式启用五房战斗竞技场，新增独立2:1格心布局：每间房的来路/去路都以格心为端点预留6格门洞，房间间通道固定12格长，两侧首末墙块与房间门端共享、中间逐格铺设；通道地板直接以四个门端点构成并与房间地板并集。每条通道两端使用精确适配6格跨度的冰锥门，复用僵尸竞技场成熟的逐房关门、刷波、清场开门与末房出口生命周期，但不再经过连续墙预制、裁剪或封口补瓦。
- 雪原初级竞技场末房改用开放式宝箱点：保留60秒倒计时、战斗完成判定、开箱奖励与离场守卫，不再额外生成旧式连续墙宝箱房，避免两套冰墙结构在末房中央混搭。

### 对话：建筑造价按科技层级重平衡与矿工营地4岗位（2026-08-23）
- 以科技树累计研究量、建筑功能和4名基础矿工约24.84能源/秒的理论产能为锚点，重排基础、T1、T2、T3、后期及位面专项建筑造价；方块墙/道路保留低成本，四格门固定为600能源、城墙楼梯改为250能源/段、防御塔改为2400能源，配置型建筑同步形成700~4200能源的分层梯度。
- 仓库保留500金币并成为玩家建筑配置中唯一金币币种；房屋、风车、市场、银行、面包屋、经济工坊、军械库及三座位面特色建筑全部改为能源，房屋与仓库现有“首栋免费”开局保护保持不变。历史建筑继续按实例记录的实付币种/金额退款，不迁移旧档成本。
- 矿工营地造价改为800能源，初始岗位由1提高到4，“矿工增援”两级后为5/6；岗位描述、当前位面人口岗位和离线世界模拟统一按 `base + per × level` 计算。新营地仍默认不自动占用人口，需由玩家分配岗位后才生成矿工。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查建筑面板币种/造价、首栋免费、建造扣费与退款，以及矿工营地4/5/6岗位和离场产能。

### 对话：沙漠位面毁灭挑战综合性能收口（2026-08-23）
- 移除敌人 Phaser Arcade Body、玩家/敌人与敌人/敌人的空 overlap 枚举，敌人移动、墙体碰撞和实体分离继续只由现有逻辑系统负责；Phaser 在 WebGL 可用时请求高性能 GPU。毁灭挑战改为队列化生成，每帧最多物化2只，设36只软上限、60只硬上限并为精英/领主预留6个位置；达到上限后低频重试，不丢当前批次，旧存档普通怪计数跨过30/60阈值时也能补触发对应里程碑。开发面板同步显示存活数、软/硬上限和待生成数。
- 防守怪出生即锁定当前位面传送门/基地作为战略目标；无目标重选、结构占用和结构候选改为错峰及共享缓存，9000px战略索敌不再反复查询整张空间网格，只有进入近战准备区才做精确长距离LOS。黑狼等 pacing 怪在防守模式不再额外运行第二套全表目标扫描；墙体长距离射线改为DDA逐格遍历，动态建筑/墙面深度仲裁按X列取局部候选，静态结构排序与日照阴影分别降到4Hz/12.5Hz。
- 战士、盾卫、民兵、射手、斥候、火枪手、牧师、骑士、矿工、丛林祭司及通用侍从的固定周期决策按实体ID错峰；有限距离索敌和友军投射物命中接入同一空间分区，精确距离、阵营、墙体和高度判定保持不变。战争迷雾对同格视野源合并后再扩散，遮罩上传限制为约15Hz，3×3软边卷积改为等价可分离两遍处理。
- 精英攻击预警移除每怪独立Glow滤镜相机，改为无滤镜红色脉冲替身；伤害浮字最多48个并复用最多64个Phaser Text，屏外浮字直接抑制，100ms内同位置伤害合并。文件音效改为每路径最多4声、全局24声的复用池并设35ms同音效保护；DOM HUD由每帧重写降为配置化10Hz。双份`game-config.json`、`world-system.json`、`fog-of-war.json`保持同步，音频预算写入`audio-config.json`。
- 未运行测试、构建、lint、浏览器探针、游戏运行或性能采样，按项目约定由用户重点测试沙漠位面30只毁灭怪+约20名友军、挑战达到36/60上限后的续刷、30/60里程碑、黑狼推进与近墙攻击、友军索敌/弹道、雾边缘、精英红色预警、连续伤害数字和密集音效。

### 对话：降雨按场景类型排除地牢（2026-08-23）
- `RainWeatherSystem` 在原有 `targetSceneIds` 白名单之外增加场景类型门禁，双份 `game-config.json#weatherEffects.rain.excludedSceneTypes` 默认包含 `dungeon`；更新循环、开发按钮与调试模型统一读取 `GAME_CONFIG.scenes[sceneId].type`，因此当前scene7及未来误入白名单的地牢都会拒绝或立即销毁降雨。世界-125“地牢遗迹”仍是 `type:"instance"`，继续作为位面支持降雨。
- 当前降雨仍是仅由交互开发面板手动触发、仅作用于当前位面的纯视听状态，不随机排期、不存档、不产生玩法数值；暴风雨首次闪电等待3.6~7.2秒，后续8.4~17秒随机触发，白屏0.2秒并在0.65秒内恢复，三种合成滚雷随机播放。未运行测试、构建或游戏运行时验证，按项目约定由用户重点检查scene7拒绝降雨、scene8~scene11四档按钮、切场清理和暴风雨时序。

### 对话：新增两足人形怪统一采用矿工僵尸体量基准（2026-08-23）
- 在怪物新增标准工作流的最前置分类步骤中，将当前正式矿工僵尸确立为今后双脚站立人形怪的默认导入基准：512帧格下 `spriteSize:260.7`、`footOffsetY:58`，地面 `collisionRadius:36.3` 且 footprint 圆心不偏移；躯干框 `57.5×158.8` 作为初始参考。新素材按实际可见人体高度和脚线对齐，不按透明画布、伸出武器或宽攻击帧机械缩放；不同帧高的动作须按同一源像素比例换算脚线。
- 该规则仅约束今后新增的两足人形怪，不批量调整任何过往怪物；四足、飞行、爬行、骑乘、巨型/Boss及用户明确指定特殊体量的单位仍走各自类别。只更新技能库与索引，未改动怪物代码、配置或素材；未运行测试或游戏运行时验证，按项目约定由用户在后续新增怪物时执行并排体量/脚线/footprint验收。

### 对话：科技树 33 项冷钢徽章图标生成与导入（2026-08-23）
- 以现有 `assets/ui/icons/technology_tree.png` 为统一母版，使用内置图像生成能力为当前科技树 29 个通用节点和 4 个位面节点逐项生成独立主体；最终图全部完成棋盘格剥离、真实 Alpha 抠图与 1024×1024 RGBA 归一，写入 `assets/ui/technology-icons/`。
- `data/technology-tree.json` 为全部 33 项增加 `iconPath`，保留原 emoji 作为资源失败回退；科技卡片与详情面板统一渲染图片徽章，位面未解锁时继续使用原 TV 雪花遮蔽，不泄露真实图标。
- 根据实机反馈二次清理透明区：所有 Alpha=0 像素的 RGB 归零，并对白色棋盘合成边缘做反推去污染，消除图标外侧白块/白边；卡片图标由40px放大到48px，详情图由58px放大到72px，图片容器强制无背景、无边框和无阴影。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查四个分支的卡片缩略图、详情大图、锁定/可研究/研发中/完成态以及位面遮蔽状态。

### 对话：主神空间仓库宝箱升级并采用单格建筑碰撞（2026-08-23）
- 纠正“建筑碰撞等于建筑外观”的误解：主神空间仓库视觉继续保持独立宝箱，组件化 Blender 白模改为低矮白色大理石箱体、连续拱形箱盖、清晰箱盖接缝、四枚金足、三道金色箱盖箍带、对称凸起卷草纹、中央大型蓝宝石锁芯和多枚小型蓝宝石镶嵌；不再包含库房墙体、地基、门窗或建筑入口。玩家确认的12步结构稿以同一Depth、`denoise=0.30`完成48步材质精修。
- 接受的48步图经本地BiRefNet分割、Blender Depth轮廓收口、半透明边缘最近不透明主体色修复和4px紧裁，生成853×791真实RGBA并正式替换 `assets/npc/warehouse/warehouse.png`；运行时等比显示180×167、脚线83px，点击区同步为紧裁可见范围。像素门槛为单一主连通域、透明区RGB归零、主体不触边且无黄底软边残留。
- 打开状态改以玩家确认的关闭态48步成图作为身份基底，Blender打开态Depth仅约束箱盖结构；精确编辑锁定原箱体比例、三道金箍、金色雕纹、中央/侧面蓝宝石和四足，再用同一打开态结构低重绘完成48步材质精修。成品经BiRefNet真透明处理并导入 `assets/npc/warehouse/warehouse_open.png`，显示为180×181、脚线90px。
- 仓库仍保留 `NPC`、`npcType:"warehouse"`、点击交互、距离自动关闭和 `WarehouseSystem.open()` 原链，仅在仓库面板打开/关闭生命周期中切换打开/关闭贴图；物理/遮挡身份继续使用标准1×1格网建筑，碰撞体积不随箱盖变化。未运行测试、构建、lint、浏览器探针或游戏运行时验证。
- 收口仓库与祭坛的碰撞可视化：保留 NPC/对话业务身份，但格网建筑不再额外绘制 NPC 地面椭圆和垂直圆柱，只显示并使用标准 `iso_rect` 建筑棱柱（仓库1×1、祭坛2×2）。单位接触阴影明确跳过该类实体，结构太阳投影与主体/标签 depth 统一复用同一建筑 footprint 身份；点击区域与仓库/祭坛功能不变。未运行测试或游戏运行时验证。

### 对话：友军严格墙体碰撞与待命警戒（2026-08-23）
- 全面收口队友、战斗仓鼠及矿工的运行中脱困：删除 `findSafeSpawn`/玩家附近落点对 `x/y` 的直接改写，卡死时只清理旧路径并保留当前目标，由 `PathManager + MovementSystem + WallSystem.resolve` 重新规划和小步解围；移除队友按距离定时清路径的旧掉队兜底，避免合法绕墙途中反复丢失 A* 路线。场景初始化、地牢房间切换和跨位面物化仍保留合法出生点同步，不属于运行中移动。
- 楼梯机制不是全局关闭墙碰撞：`_elevatedNavigationBridge` 只负责桥面单位互推。现进一步限制 `WallSystem.ignoreForEntity()`，必须具有已提交的楼梯/墙顶身份且高度有效才能忽略承载墙；地面单位即使残留 `_surfaceWalls` 也执行完整墙段碰撞。防御墙体改为所有阵营统一只认同几何生成的 `WallSystem._coverSeg`，删除玩家/友军不同口径，同时避免重复实体 footprint 把单位从已裁剪放行的门洞中推出；真正站在该墙顶的单位仍保留承载面豁免。
- `RtsTacticalOrderSystem` 新增战斗仓鼠待命警戒：战士、盾卫、民兵、射手、斥候、火枪手、牧师、骑士、轻骑及其骆驼骑兵派生单位，在默认/移动完成后的 `hold` 状态保持原地，但敌军进入统一 `RTS_DEFAULT_ACQUIRE_RANGE`（当前配置900px）后会自动下达攻击；矿工和探险家继续不参战。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查封闭墙体、门洞、楼梯底/顶、卡死重算、队友远距离跟随，以及待命士兵自动索敌。

### 对话：闪光弹式白屏保留UI与纯雷声（2026-08-23）
- 暴风雨白闪层调整到世界画面最高层、GameScene 小地图/HUD之下：触发时地形、角色、特效、战争迷雾、天气和昼夜层全部纯白，独立 HudScene 与 DOM UI 保持可见，形成射击游戏闪光弹式反馈；0.2秒全白和0.65秒渐退参数不变。
- 删除天气配置与运行时对 `lightning-1.mp3/lightning-2.mp3` 闪电技能电击声的引用。项目没有独立雷声素材，因此 `SoundManager.playThunder()` 新增低通棕噪声＋52→27Hz低频正弦的2.8秒滚雷合成，只保留沉闷雷声并服从 SFX/主音量。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查白屏时HUD/小地图/DOM面板是否保留、白屏覆盖完整性，以及雷声是否不再含电击音。

### 对话：暴风雨闪电改为全屏白闪恢复（2026-08-23）
- 暴风雨雷电移除随机分叉电弧和冷蓝短闪，改为触发瞬间在战争迷雾、降雨与昼夜覆盖之上以纯白 alpha 1 覆盖整个游戏视口，精确保持0.2秒，再用0.65秒线性衰减恢复正常亮度；雷声和4.2~8.5秒随机触发间隔保留。
- 双份 `game-config.json#weatherEffects.rain.intensities.storm.thunder` 用 `whiteHoldMs/recoveryMs` 取代旧电弧段数、分支、颜色和短闪参数。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查全白覆盖范围、0.2秒保持、渐退亮度、连续雷击和暂停/切场清理。

### 对话：降雨四档强度与暴风雨雷电（2026-08-23）
- 将首版降雨正式定义为“小雨”，新增中雨、大雨、暴风雨四档配置。随档位逐级缩短雨丝/水花发射间隔、增加单次粒子数与存活上限，并提高纵向落速、横风速度、雨丝尺寸和环境压暗强度；切档时按新上限重建共享粒子池。
- 暴风雨加入 Phaser Graphics 运行时绘制的随机折线主闪电与四处分叉、冷白全屏短闪，并在统一世界时间累计的4.2~8.5秒随机间隔内触发；同步复用现有 `lightning-1/2.mp3` 作为雷鸣 SFX，暂停时调度冻结，离场时闪电、闪光和雷电计时一并销毁。
- 交互开发工具的每个位面降雨行改为“小雨/中雨/大雨/暴风雨”四按钮；点击当前档位结束降雨，点击其他档位直接切换并显示当前强度。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查四档雨量/速度梯度、暴风雨闪电层级与雷声、连续切档、暂停恢复和离场清理。

### 对话：全位面通用 Phaser 降雨特效（2026-08-23）
- 新增 `RainWeatherSystem`，以 Phaser CanvasTexture 和两个受控 ParticleEmitter 构建前景斜雨丝与2:1地表水花，并增加位于战争迷雾/昼夜覆盖之下的冷蓝压暗层；纹理运行时生成，不依赖新增图片素材。
- 粒子只在当前相机扩展范围与位面菱形的交集内发射，雨丝/水花分别设预留池、存活硬上限和单帧补发上限；暂停时冻结粒子，加载、离场或实体视图清理时立即销毁。双份 `game-config.json#weatherEffects.rain` 同步提供品质、频率、数量、寿命、速度、颜色和覆盖强度参数。
- scene8~scene11 的交互开发工具位面行均新增“触发降雨/结束降雨”按钮，仅当前已进入的位面可操作；本轮是纯视觉手动天气，不写存档、不随机排期，也不修改视野、移动、战斗或地形。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查四个位面的雨丝密度、水花接地、镜头移动覆盖、暂停恢复、切场清理及与沙尘暴/雾潮/战争迷雾的层级。

### 对话：沙漠位面六图随机 Loading 背景（2026-08-23）
- 将素材库 `场景/大图/沙漠/` 下 `沙漠.png` 至 `沙漠6.png` 六张图原样导入为 `assets/scenes/loading/desert-1.png` 至 `desert-6.png`；六图均为 1672×941 RGB PNG，覆盖峡谷、沙丘、绿洲、晴昼、夕阳与星夜，项目副本与对应源图 SHA-256 一致，未改动素材库原图。
- 双份 `loading-screen-config.json` 新增 `desertWorld`，把沙漠位面 `scene8` 映射到六图随机池，标题为“世界-122·沙漠加载中...”，沿用统一全屏 cover、进度条、浏览器图片预缓存与至少 1 秒展示；所有既有 `SceneManager.switchScene()` 入口自动生效。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查六图随机轮换、明暗背景下文字/进度条可读性、画面裁切及进出沙漠时的显示时长。

### 对话：碰撞编辑器普通近战锁定矩形可视化（2026-08-23）
- 怪物测试行的“位移轨迹”升级为统一“近战判定”开关：普通近战真正起手时以黄色显示锁定目标和定向矩形快照，命中帧继续用同一快照复查，红色表示通过，橙色表示目标离框、隔墙、承载面不一致或锁定失效；矩形和方向箭头按实际2:1地面投影绘制。原有位移近战蓝色扫掠线、紫色目标 footprint、红色命中点与橙色撞墙截断点继续保留。
- 普通近战调试数据由 `melee-attack-resolver.js` 在解析判定当场记录，只接收碰撞编辑器当前生成的正式测试怪 ID；追击期未进入攻击的范围轮询不记录，最近1.8秒之外自动淘汰，切换对象、重新生成、关闭开关或退出编辑器时与位移轨迹一起清空。伤害、攻击距离、状态机、冷却、目标选择及正式运行渲染均未修改。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户用通用近战怪、黑狼/红狼王撕咬、Mutant-3连击和铠甲骑士连击重点检查黄色起手框固定方向、目标绕后/离框橙色失败、隔墙复查以及红色有效命中。

### 对话：碰撞编辑器位移近战轨迹可视化（2026-08-23）
- 碰撞体积编辑器的怪物测试行新增“位移轨迹”开关：蓝线显示当前正式测试怪每帧的实际扫掠段，紫色轮廓和连线标记发动时锁定目标及其圆形/矩形/等距 footprint，红点标记命中帧轨迹上最靠近锁定目标中心的位置，橙色叉号标记 `WallSystem.resolve` 裁短或滑墙后的实际停止点；面板同时显示技能名、判定结果、锁定目标和命中余量。
- 调试数据由 `motion-melee-sweep.js` 在怪物页明确开启时采样，仅保留最近1.8秒、最多160段；切换对象、再次生成测试怪、切到友军/NPC/墙体等页面、手动关闭开关或退出碰撞编辑器都会清空旧轨迹。伤害、技能冷却、状态机、目标选择和正式运行渲染均未修改。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户打开碰撞编辑器，分别生成红狼王、Mutant-3和铠甲骑士，重点检查高速跨帧蓝线连续性、紫色锁定轮廓、命中红点、撞墙橙叉及关闭编辑器后的清理。

### 对话：雪原位面随机 Loading 背景（2026-08-23）
- 将素材库 `场景/大图/雪原.png`、`雪原2.png` 原样导入为 `assets/scenes/loading/snowfield-1.png`、`snowfield-2.png`；两图均为 1672×941 RGB PNG，源文件与项目副本 SHA-256 一致，未改动素材库原图。
- 双份 `loading-screen-config.json` 新增 `snowfieldWorld`，将 `scene9` 映射到两张背景并随机选取，标题为“世界-123·雪原加载中...”，沿用统一全屏 cover、进度条、浏览器图片预缓存与至少 1 秒展示。世界面板观察、支援/本体切换、实体传送门及任务入口均通过既有 `SceneManager.switchScene()` 生效，无需新增旁路。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查两图随机轮换、标题、画面裁切以及进出雪原时的显示时长。

### 对话：位面毁灭挑战怪潮翻倍与周期成长（2026-08-23）
- 普通怪刷新由每5秒3只翻倍为6只，仍以累计60只普通怪为一个周期；原30只中点精英、60只终点领主的里程碑位置不变。
- 第N周期的中点生成N只精英；领主数量每2个周期增加1只，即第1~2周期各1只、第3~4周期各2只、第5~6周期各3只。双份 `world-system.json` 新增基础数量与增长周期配置，位面调试行同步显示当前周期及本周期精英/领主规模。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户测试刷新节奏和跨周期数量增长。

### 对话：全位面通用毁灭挑战（2026-08-23）
- 新增 `WorldDestructionChallengeSystem`：scene8~scene11 每个位面可独立触发并按 `sceneId + worldEpoch` 持久化。触发5秒后从地图右侧角落开始，每5秒生成3只普通防守怪；累计30只时额外生成1只精英，累计60只时改为额外生成1只领主，随后按每60只一轮继续循环。挑战不会主动结束，传送门被摧毁、位面世代重建或新游戏重置时才清除；离场期间不物化怪物，回场后从下一批继续且不追补积压实体。
- 怪物复用 `DefenseSystem` 正式怪物池、墙体安全出生和“沿途攻击玩家/友军，最终攻击传送门”的防守 AI。毁灭挑战怪物与五日入侵怪物分开计数和清理，不阻塞普通入侵清波，也不发放可无限刷取的防守金币。参数由双份 `world-system.json#destructionChallenge` 控制，主存档写入 `worlds.destructionChallenges`。
- 交互开发工具“位面调整”中为每个位面行增加“触发毁灭挑战”按钮和普通/精英/领主累计数量、下一批倒计时；未接通或传送门已毁的位面显示不可触发，挑战开始后按钮锁定且不提供手动停止。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查四个位面的右侧角落刷怪、5秒节奏、30/60只里程碑、普通入侵并行、离场回场、存读档及毁门重建终止。

### 对话：高速位移近战连续轨迹判定（2026-08-23）
- 新增位移近战共享扫掠合同：按攻击者每帧经 `WallSystem.resolve` 后的实际起终点，对锁定目标的圆形、轴对齐矩形或等距 footprint 做连续线段判定，并在真实接触后才进入伤害、弹反与后置控制；命中线同时复查承载面和墙体，不再依赖单帧终点距离。
- 红狼王飞扑、Mutant-3飞扑和铠甲骑士冲锋均只认发动时锁定的目标，不再在原目标死亡后回退误伤当前新目标。墙体若裁短直冲或把位移改成沿墙滑动，技能会先在本帧实际可达段检查近侧碰撞，再于解析后的落点终止；红烟、残影和扬尘仍沿真实位移位置生成。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查低帧率高速穿过目标、目标死亡/切换、飞扑撞墙、目标隔墙、不同表面目标、盾牌弹反，以及三种轨迹特效是否停在实际落点。

### 对话：军械库维护师移动防卡死（2026-08-23）
- 参考面包师纯视觉岗位的脱困合同，为维护师的巡游和赴维护目标移动增加配置化 `moveGraceMs:5000`。每段移动按“当前直线距离 ÷ 对应速度 + 5秒宽限”建立独立倒计时，正常抵达仍走原贴近建筑逻辑。
- 巡游移动超时后放弃当前目的地、速度归零并切回 idle，随后重新选点；赴维护目标超时且目标仍在军械库覆盖范围内时，从当前位置进入本次 maintenance 动画，完成后释放目标认领，避免墙角、墙门或密集 footprint 让维护师永久行走并长期占住建筑。目标失效或离开覆盖范围仍立即释放，不被超时兜底掩盖。维护师动画、体量、岗位数、减耗及强化石结算均未修改。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查封闭目标、密集建筑、巡游死角与目标中途拆除。

### 对话：建筑面板四分类分页（2026-08-23）
- 建筑面板上方新增“防御建筑 / 经济建筑 / 募兵建筑 / 其他建筑”四分页：防御按墙、门、塔等结构类型归类，经济按矿工营地、`economyType` 与仓库能力归类，募兵按实际启用出兵且配置兵种归类，道路、研究强化、祭坛和位面功能物归入其他；配置型新建筑继续根据现有能力字段自动分类，不另维护建筑 ID 白名单。
- 分页使用冷钢主题变量并补齐悬停、选中、键盘焦点、左右方向键及 Home/End 切换。切换到不同分类时会取消已隐藏的待放置建筑，分类内原有解锁顺序/能源消耗排序、科技折叠、资源不足提示和最终放置校验保持不变。本轮回退范围仅为 `src/world/building-system.js` 的分类/分页逻辑与 `ui/panel-theme-backpack.css` 的分页样式。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查四分页归类、切页后放置状态、科技解锁后刷新及窄屏按钮显示。

### 对话：玩家附着式受击反馈与范围近战第二轮收口（2026-08-23）
- 玩家在有效扣血后新增贴图附着式受击反馈：主体上短暂出现按伤害类型着色的冲击光、交叉裂痕与7枚火花，特效容器每帧跟随玩家贴图位置和动态深度、最多并存6组；闪避、无敌、成功弹反或最终伤害为0时不会误播，原有白闪与伤害数字保留。
- 手脑砸地改为以触手前方实际落点作为唯一锚点：前摇显示300px的2:1地面椭圆预警，第14帧的烟尘、白色放射线、冲击圈和真实伤害全部使用同一落点与椭圆，不再出现特效在前方、伤害却以本体为圆心；放射线外沿同步扩到真实300px范围。
- 时空盾位盾击锁定起手方向，范围由旧360°圆形改为前方200×160px地面矩形；盾缘尾迹修正到真正的盾正面，命中帧显示同形白色四边形。蝇手砸地/灭世重砸、矿石蜘蛛下砸和盾击在目标成功弹反后不再追加眩晕；集合体分圈、蝇手冲击圈、矿石蜘蛛预警圈、手脑嚎叫、红狼王/突变体飞扑与铠甲骑士冲锋经第二轮调用链核对后继续保留既有专用合同。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查玩家连续受击时附着层跟随与颜色、手脑砸地预警/落点/边缘命中、盾击前后方与四方向矩形，以及三类范围眩晕的盾牌弹反。

### 对话：维护师 walking / maintenance 重截为同相完整循环（2026-08-23）
- 用户实机指出首版 walking 与 maintenance 在接缝抽动且没有形成自然循环。根因是首版筛选器只验证“末帧→首帧像普通步幅”，却把同相终点本身也收入精灵表，形成重复姿势/局部动作循环；该版已被本条替代。
- walking 改为完整保留源 f62~f78 共17帧，并删除与 f62 同相的重复终点 f79，按17fps播放；maintenance 改为完整保留 f114~f129 共16帧，并删除同相终点 f130，按16fps播放。两组不再隔帧抽样，保留源视频连续动作。
- BiRefNet v3 报告确认 walking / maintenance 均无空帧、无贴边帧，脚线固定 y=479；循环接缝变化分别为普通相邻变化的0.79倍和0.96倍，满足项目0.5~1.5倍自然接缝口径。正式贴图、配置帧数和播放速度同步覆盖，维护师巡检、岗位与范围逻辑未修改。未运行游戏、构建、lint、浏览器探针或运行时验证，按约定由用户复查两个循环的实际观感。

### 对话：普通模式攻击与地图指令仅响应游戏画面（2026-08-23）
- 新增共享游戏指针边界：普通模式的左右键攻击只在真实游戏渲染表面建立 `Down/Pressed` 状态，点击任意 HUD、右栏、建筑详情或其他 DOM 栏目均不会再触发半自动/全自动攻击、特殊攻击或遗留按住状态；地牢路线模式明确排除。
- 普通模式的中键指令轮盘、移动攻击/巡逻选点与 RTS 世界输入同步改用游戏表面白名单，移除容易漏掉新栏目 class 的黑名单。轮盘只能从游戏画面起手；地图选点按下与松开均需位于游戏画面，从画面拖到栏目后松开会取消本次指针过程但保留待选指令，不会把栏目坐标转换成世界攻击目标。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查普通画面左右键、全自动武器按住、各类栏目点击、中键轮盘，以及移动攻击/巡逻选点跨越栏目边界。

### 对话：沙漠官邸无粮出兵修复（2026-08-23）
- 确认骆驼骑兵配置的 `spawnFoodCost:300` 正常，漏洞来自通用前台招募链把开发工具 `_devInfiniteResources` 同时当成“军事招募免粮”，使沙漠官邸及其他出兵建筑可绕过单次预检与最终扣粮。
- 跨位面资源支付新增按事务关闭开发免单的选项；通用产兵建筑、并行特色通道与仓鼠兵营的军事招募现始终检查并实际扣除粮食。粮食不足时保持读条归零并每秒重试，不生成单位；快照恢复既有单位和双身祭品带来的额外单位仍沿用原有免重复收费合同。
- 开发工具文案同步明确“军事招募仍消耗粮食”。跨位面仓储协议保持不变：当前位面无仓库而其他位面存在仓库网络时，仍可按协议附加费从远端真实库存扣粮。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户测试沙漠官邸单次/持续招募、读条期间耗尽粮食及开发无限资源开启状态。

### 对话：面包师建筑近距交互与移动卡死脱困（2026-08-23）
- 面包师不再必须抵达仓库或面包屋外侧的单一精确脚点：普通目标点到达容差由18px放宽到40px，并为仓库、面包屋分别加入180px、200px的中心服务半径，到达建筑附近即可取粮、开始加工或交货。
- 搬运阶段现在按“预计直线路程 + 5秒宽限”维护可恢复超时；若纯视觉碰撞滑行仍被墙体、footprint或密集建筑挡住，则从当前脚点完成本次交互，避免现有存档中已耗尽移动计时的面包师永久卡死。粮食扣取、仓储容量和交货仍由原有指定仓库事务在阶段完成时重新校验，未修改配方、产量、加工时间、升级或动画状态映射。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查堵塞布局下的连续取粮、返屋加工和成品返仓。

### 对话：军械库维护师三状态视频精灵与巡检逻辑（2026-08-23）
- 核验用户指定 `E:\无尽轮回\游戏\素材库\人物\维护师\1.mp4` 为145帧、24fps，时间轴依次为待机、行走、维护。最终待机采用 f2~f16 每2帧采样后往返的14帧闭环，行走采用 f65~f85 每2帧采样的11帧循环，维护采用 f99~f119 每2帧采样的11帧循环；全部避开 f44/f94 动作转场。
- 三张 BiRefNet 透明精灵图写入 `assets/companions/armory_maintainer/`，统一512×512单格、12fps、脚线y=479，并按仓鼠农民312px可见高度与128显示基准对齐。逐格报告确认三组均无空帧、无贴边帧，循环接缝变化分别约为相邻变化的0.96倍、1.41倍和1.20倍；联系图、GIF和来源帧报告保存在 `tools/ai-gen/_armory_maintainer_20260823/generated/`。
- 新增 `ArmoryMaintainerVisualSystem`：按军械库实际上岗人数物化最多5名纯视觉维护师，平时在本栋当前覆盖范围内随机 idle/walking，自动认领范围内活动建筑并贴近外缘循环 maintenance，结束后释放目标继续巡检。维护动画不回血、不改变军械库数值；移动、建筑占用、左右翻转、遮挡与淡出统一复用平民视觉管线，岗位归零、出售/摧毁、位面离场和系统重置均清理。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查三态切换、覆盖边界、贴近建筑、多人目标分配、体量与遮挡。

### 对话：面包师两组奔跑重截为完整双步循环（2026-08-23）
- 修正上一版空载8帧、抱面包7帧只覆盖局部抬腿、未构成完整跑步循环的问题。重新以腿部相位而非最小全身像素差选窗：空载使用同相端点 f42/f62 之间的 f42~f61，抱面包使用同相端点 f114/f134 之间的 f114~f133，两组均为20帧完整左右脚交替，并避开动作转场与触边帧。
- 两张正式精灵图同步替换为20帧透明表，仍保持512×512单格、脚线y=479及仓鼠农民同体量；播放改为16fps，每轮1.25秒。Alpha报告确认无空帧、无贴边帧，空载/抱面包接缝变化分别约为相邻帧均值的1.25倍/1.32倍。idle、面包师状态映射、加工隐藏、移动速度和经济结算均未修改。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按约定由用户重点检查完整左右脚循环、循环接缝及80px/s下的步频。

### 对话：仓鼠面包师三状态视频精灵图接入（2026-08-23）
- 核验用户指定素材 `E:\无尽轮回\游戏\素材库\人物\面包师\1.mp4` 为 145 帧、24fps，并按真实时间轴拆出待机、未装载奔跑和抱面包奔跑。为避开转场与抱面包触边段，最终 idle 使用 f2~f7 往返的 10 帧闭环，未装载奔跑使用 f55~f62 的 8 帧闭环，抱面包奔跑使用 f104~f110 的 7 帧闭环；BiRefNet 抠图后统一为 512×512 单格、脚线 y=479，可见高度按既有仓鼠农民 312px Alpha 基准缩放。
- 新增 `HamsterBakerVisualSystem`，纯读取面包屋 `_bakeryJob`：待命显示 idle，前往仓库取食物及携带原料返回时播放 `empty_running`，加工阶段将 Sprite 隐藏并退出可见平民集合，加工完成、等待仓位及送货阶段播放 `loaded_running`；岗位归零、出售/摧毁、位面离场与系统重置均清理视觉记录。面包师仍不进入实体、物理、战斗、寻路或独立存档链。
- 三张正式透明精灵图写入 `assets/companions/hamster_baker/`，BootScene 与 `population-economy.json#bakery.workerVisual` 同步登记真实帧数；生成报告确认三组无空帧、无贴边帧、脚线范围固定为479。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查取粮空载奔跑、加工隐藏、完成后抱面包奔跑、左右翻转、建筑遮挡和与其他平民的同屏体量。

### 对话：雪原位面接通时自动生成学院城堡（2026-08-23）
- 将已确认的 `snow_castle` 透明运行候选正式入库并登记为“学院城堡”：沿用288×325显示、161px脚线和严格2×2视觉 footprint；BootScene补齐纹理预加载，建筑配置锁定为scene9唯一的非手建初始特色建筑。
- 雪原 `scene9` 的位面配置补齐 `featureBuilding` 映射，首次接通传送门创建基础快照时会在门侧免费生成学院城堡；新建筑当前仅作为雪原特色体系核心，不擅自增加尚未设计的兵种、升级或生产功能。
- 为已经接通但旧快照中缺少学院城堡的存档加入一次性补齐迁移：`scene9_snow_castle_initial_v1`只会补生成一次，标记随完整快照继续保存，玩家日后主动拆除不会在每次读档时反复复活。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查首次接通、旧档回场、城堡脚线/占格和拆除后再次读档。

### 对话：矿工僵尸第17帧砸击接入方向性单目标判定（2026-08-23）
- 矿工僵尸保留通用攻击的4秒冷却、预警与起手调度，并复用其锁定主目标和方向快照；启动自定义24帧砸击动画时立即关闭通用突刺命中，确保整次攻击只有第17帧这一处伤害源，不会出现通用伤害与动画帧伤害叠加。
- 砸击现在只复查起手锁定目标的方向矩形、承载面与墙体，不再遍历160px内全部敌对单位；目标横移、绕后、隔墙或失效后不会被隔空命中。命中仍按矿工物攻结算并击退75px，弹反成功时禁止追加普通击退；两份敌人配置同步补充明确的 `attack.range: 160`，避免回退到通用突刺距离。
- 至此本轮审阅到的自管普通单体近战已全部接入统一解析器；仍直接枚举敌对单位的代码均属于已确认的范围技、接触伤害或位移技能，继续保留专用合同。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查矿工四方向起手、多人贴身只命中一人、第17帧仅一次伤害、横移/绕后/隔墙以及盾牌弹反。

### 对话：僵尸工头鞭击与提灯矿工砸击接入方向性判定（2026-08-23）
- 僵尸工头鞭击按既有 `thrust width:26` 合同改为锁定起手主目标、方向和鞭梢目标点；命中帧只复查该目标的方向矩形、承载面与墙体，不再对320px内全部敌对单位造成圆形群伤。鞭击视觉同步沿起手点播放，不会出现伤害方向锁住但鞭梢继续追踪转向；伤害×2和1层流血保持不变。
- 提灯矿工120px砸击改为同一方向性单目标合同，预警结束时重新验证，攻击期间锁定朝向，命中帧不再遍历全部近身单位；物攻×1.5、帧时序和冷却保持不变。提灯投掷、落点燃烧区及死亡流程未修改。
- 补齐已迁移普通近战的弹反后置门禁：蝇手锤击被弹反时不再额外击退，时空突击特工斧砍被弹反时不再附加致残与红色命中特效，工头鞭击被弹反时不再叠加流血。手脑砸地、矿石蜘蛛下砸、蝇群接触伤害等明确范围技能保持专用合同。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查多人贴身单目标、横移/绕后、隔墙、鞭击视觉方向、提灯攻击切换及盾牌弹反。

### 对话：全建筑遮挡平民的占用链收敛（2026-08-23）
- 纯视觉平民的轻量占用不再只读取动态 `iso_rect` 建筑：统一加入当前有效 `WallSystem.isoSegments`，关闭门的门洞段、普通墙、边界和楼梯侧边会参与出生/目标投影与分段移动扫掠，开门移除门洞段后自然恢复通行；已有完整 footprint 的掩体排除重复线段，能源矿显式保持不阻挡语义。
- 农夫、银行职员、工程师统一持有权威逻辑 `x/y`；建筑或墙门拓扑把农夫推出时同步平移缓动段起点，避免下一帧按旧起点重新撞回建筑。建筑 Collider 重建和墙段数组增删分别维护轻量 revision，静止平民仅在拓扑版本变化时重新安置。
- 新增开发期建筑接线告警及 `window.CivilianVisualDebug`：`setEnabled(true)` 可显示蓝色建筑 footprint、红色有效墙门段和黄色平民地面圆，`snapshot()` 可读取当前候选与平民脚点。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查新建/出售建筑、关闭/开启门、农夫贴角移动以及三类平民在密集建筑间的遮挡与滑行。

### 对话：蝇手锤击与时空突击特工斧砍接入方向性判定（2026-08-23）
- 蝇手普通单体锤击改为预警结束时重新验证起手条件并锁定主目标、方向和命中快照；第3帧只对锁定目标复查方向矩形、承载面与墙体，目标横移、绕后或隔墙后不再因仍处于100px圆形距离内受伤，击退方向也固定为起手挥击方向。移动刹车同步读取锤击起手profile。
- 时空突击特工的入场斧劈与近战普攻统一接入同一解析器，攻击动作期间锁定朝向；修正旧实现遍历范围内全部敌对单位、把配置定义的近战普攻错误结算成360°群伤的问题。现在每斧只命中起手主目标，命中时仍保留物攻×2、3秒致残、红色下落粒子和近战弹反标记。
- 蝇手砸地/灭世重砸、时空盾卫盾击、闪光弹和双方枪械均保持既有范围或远程合同。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查两类单体攻击的四方向、多人贴身、横移/绕后、隔墙、目标死亡及突击特工形态切换。

### 对话：面包屋经济建筑、科技门禁与仓鼠面包师生产链（2026-08-23）
- 将已完成贴图的 `bakery` 正式登记为 2×2 经济建筑：造价 900 金币、1 个面包师岗位，建筑卡受经济线“面包烘焙”科技门禁；科技树在农业分工后新增“面包烘焙 → 烘焙工艺”，后者统一解锁四项本栋升级，并把经济工程/位面物流向后平移一列保持布局清晰。科技存档升至 v4，旧 v3 存档不会自动获得本次新科技。
- 新增面包师离散生产链：基础 80px/s，前往真实仓库取出 50 粮食、返回面包屋处理 10 秒、生成 5 倍粮食，再搬运至有容量的仓库；缺粮、仓库被毁或满仓时进入等待，已取食材和待返仓成品随建筑快照保存。面包师当前只有逻辑坐标，不进入 `Game.entities`、物理、战斗或寻路，素材后续接入时可直接附着纯视觉 Sprite。
- 新增“快速烹饪 / 美食家 / 食材处理 / 小步快跑”四项 10 级升级：每级分别为处理时间 -0.5 秒、产出倍率 +0.5 倍、植物祭品概率 +0.2 个百分点、移动速度 +5%；植物祭品从既有 20 种植物池等概率抽取并自动存入主神空间仓库，仓库满时保留待入库 ID。前台与后台位面均推进升级、粮食转换和祭品概率，后台不物化平民。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查科技树位置与门禁、50→250 基础批次、仓库缺粮/满仓、四项满级数值、植物祭品自动入库及跨位面离场恢复。

### 对话：右侧技能栏低分辨率完整滚动（2026-08-23）
- 修正技能页活动 Tab 使用100%高度后再叠加系统标题栏与主页签、导致内容底部被右栏裁切的问题：标题栏和主页签保持固定，技能页改为承接剩余高度，筛选栏固定，技能卡列表在自身区域纵向滚动。
- 技能详情继续在技能页边界内独立滚动；列表和详情补充键盘聚焦、滚动区域语义、滚动边界隔离及冷钢滚动条，低窗口高度、技能数量增加或筛选按钮换行时仍可访问全部内容。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查低分辨率下技能列表最末项、鼠标滚轮/触控板/键盘滚动、筛选切换及长技能详情返回。

### 对话：突变体-3与铠甲骑士连击接入方向性判定（2026-08-23）
- 突变体-3五连击与铠甲骑士二连击改为起手锁定主目标和攻击方向；每段命中通过统一近战解析器复查方向矩形、承载面与墙体，目标绕后、横移出挥击区或隔墙后不再因圆形距离仍受伤，也不会切换伤害到旁边的新目标。
- 新增连击快照重锚：只允许突变体既有的命中小突进带着判定原点前移，不重新追踪转向。两类怪物的移动刹车共用连击起手profile；突变体命中时序从±0.5帧窄窗改为跨过命中点即单次结算，低帧率不再漏段，同一大帧内被弹反/冻结会立即截断剩余段。
- 突变体飞扑、铠甲骑士持盾冲锋/格挡、伤害倍率、束缚、击退、眩晕和弹反语义均保持不变。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查两套连击的正面多段命中、横移/绕后躲避、隔墙、低帧率以及首段弹反中断。

### 对话：黑狼与红狼王撕咬接入方向性近战判定（2026-08-23）
- 黑狼和红狼王的自管撕咬状态机接入统一普通近战解析器：起手锁定目标、方向和牙口原点，伤害窗口只检查该锁定目标；目标在前摇中绕后、横移出方向矩形、离开承载面或进入墙后均不再受伤。
- 保留 `biteRange` 起手距离与 `biteHitDistance` 命中容差，狼系移动刹车通过 `getBasicMeleeApproachConfig()` 共用起手profile及2:1逆透视footprint边缘距离。红狼人二阶段50%暴击/2.5倍伤害继续由既有专属命中入口结算。
- 红狼飞扑、红烟轨迹、嚎叫、变身和其他Boss范围/连击技能均未迁移，继续使用各自专用状态机。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查狼形/狼人撕咬的四方向、绕后、退离、隔墙、贴身多人和暴击表现。

### 对话：右上资源栏更名经济面板并与时钟等高（2026-08-23）
- 金币、能源、食物和兵力所在栏统一命名为“经济面板”，保留 `basic-resource-bar` 兼容类与既有刷新入口，并把可访问名称更新为“经济面板”。
- 经济面板与时钟共同读取 `--top-right-instrument-height:74px`，布局收敛为38px三资源行 + 24px兵力行；资源图标由28px调整为24px、兵力图标由22px调整为18px，数字由body档调整为meta数字档，内边距与三列最小宽度同步收紧。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查两块面板上下边线、资源长数字、兵力三位数和窄窗口下的间距。

### 对话：仓鼠民兵与仓鼠斥候体量回正（2026-08-23）
- 确认精灵图文件与帧格本身正常；错误来自上一轮把 running 全 Alpha 高度直接当身体高度。民兵的长草叉拉高包围盒，斥候的前倾横向跑姿压低包围盒，导致两者分别被误缩到 `214.088398` 与 `315.04065`。
- 恢复两者此前确认过的 `displaySize:460`，并同步脚线：民兵 `spriteOffsetY:-84 / footOffsetY:84`，斥候 `spriteOffsetY:-23 / footOffsetY:23`；20×100步兵碰撞、HUD锚定、攻击范围和战斗数值保持不变。
- 友方单位体量工作流补充长武器/低姿态例外：Alpha测量只作为首轮候选与脚线诊断，必须复核主体轮廓，不能再机械覆盖用户已确认的人工体量。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查两者同屏体量、脚底贴地和攻击/奔跑切换。

### 对话：军械库组件化建模（2026-08-23）
- 按 World-122 建筑最高优先级管线新增标准2×2军械库真源：完整矩形石基、加固石砌下层、半木上层和连续深色双坡屋顶；正面保持唯一重型双开门与同墙上层装卸门，并以铁箍扶壁、附墙双盾架、三支长柄武器架、无文字盾剑徽记、墙箱、单灯和侧面窄窗建立军事仓储识别。未生成塔楼、锻炉、烟囱、院落或独立武器棚，避免与军营及铁匠铺混淆。
- 新增 `build_armory()`、建筑级可编辑盾牌/长柄武器组件、manifest 尺寸和低饱和调色、组件登记与专用结构提示词；已输出 `armory_model.blend`、1024²材质预览和同相机 Depth。经用户明确授权，将本次 Depth 与提示词发送到局域网5080，使用 `flux2-dev-depth`、Control 0.88、CFG 3.5、Euler、12步生成 seed `123160/123161/123162` 三张纯黄底结构候选及联系表；三张均保持完整地基、单体连续双坡屋顶、唯一主门且无塔楼/锻炉/烟囱，静态初筛推荐 `123162`，其双盾、三支长柄武器、上层装卸门和侧面窄窗最接近白模，`123161` 在侧墙额外复制了一组武器架。
- 用户同意以推荐的 `123162` 进入48步后，新增低重绘精修提示词并继续使用同一 Depth，以 denoise 0.30、Control 0.88、CFG 3.5、Euler 生成 seed `1231621/1231622/1231623` 三张精修稿及联系表；三张均守住完整地基、唯一主门、双盾、三支长柄武器、装卸门和连续屋顶。静态初筛推荐 `1231622`，其石墙块面、铁门五金、盾牌高光和长柄武器细节最均衡，且没有 `1231623` 上层装卸门偏亮的白框。用户继续采用 `1231622` 后，已基于同相机 Depth 完成真透明、黄底边缘净化与紧裁，固化 `armory_runtime.png`（RGBA 883×751，alpha 单连通且不接触画布边界），建议运行时显示 `288×245`、`footOffsetY:121`；横纵缩放分别为 0.32616/0.32623，并输出标准2×2铺地叠加预览。该出图阶段尚未覆盖正式游戏贴图，也未修改建筑配置、数值、科技、占格、建筑面板或玩法。未运行测试、构建、浏览器探针或游戏运行时验证。
- 将 `1231622` 真透明成品正式导入 `assets/terrain/armory.png`，登记为标准2×2经济建筑并生成128×64建筑面板缩略图与严格256×128视觉 footprint；基础造价1400金币、3000生命，建造继续遵守当前位面仓库前置。科技树版本升至v5，在军事指挥线新增“军械保障 → 军需标准化”：前者解锁军械库，后者统一解锁四项本栋升级，旧档不会自动获得新科技。
- 军械库初始提供2个维护师岗位，600px内所有出兵建筑按最强军械库光环降低实际招募资源消耗；配置基础减耗10%，每名上岗维护师发挥20%，初始2人满岗实际4%，增加人员升满后5人发挥100%。装备护理每级配置减耗+2%（10级）、服务范围每级+100px（10级）、增加人员每级+1岗位（3级）；资源整理修正为未升级即有每分钟1%满员概率，每次升级+1%，按模块Lv.0起算经过9次升级后满级10%，实际概率同样乘维护师发挥率。
- 前台招募报价、真实扣粮与后台位面生产统一读取同一范围减耗并向上取整，多栋军械库不叠加；资源整理产出的强化石自动堆叠进主神空间仓库，仓库满时保存在本栋/后台快照等待入库。军械库模块等级、升级读条、岗位数、分钟进度和待入库强化石均随位面快照保存，离场位面继续升级、减耗与概率结算。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查科技门禁、岗位发挥、范围边界、招募报价/扣粮、强化石入库与跨位面恢复。

### 对话：敌人普通近战方向性判定第一阶段（2026-08-23）
- 新增通用普通近战解析器：起手锁定主目标、攻击方向和判定原点，命中帧只对该目标结算，并复查方向矩形、承载面与墙体；普通攻击不再以攻击者为中心进行360°多目标距离命中。
- 普通僵尸、胖子僵尸、僵尸犬/CircleEnemy及直接使用Enemy的基础怪首先迁移；CombatSystem起手、MovementSystem近战刹车与ThrustAttack命中共用同一profile。历史Boss、技能型范围伤害和自管命中帧保持旧合同，矿工僵尸第17帧砸击显式排除。
- 移除CombatSystem与Enemy回退动画里额外的200ms命中截止，统一由ThrustAttack的hitDurationMs管理，避免188ms前摇后仅剩约12ms、低帧率漏判。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查四方向攻击、目标退离/绕后、隔墙、多人贴身及结构贴边攻击。

### 对话：房屋容量并行驱动军事人口与普通出兵取消建筑上限（2026-08-23）
- 新增当前位面军事人口真源：容量直接复用房屋人口容量，每名已出兵单位占1点，但不占用经济岗位人口；本地、跨位面途中与外派驻军继续占原生产位面的军事人口，旧档恢复已有单位即使暂时超额也不删除，只暂停后续招募。
- 仓鼠草屋、军营、教堂、靶场和骑兵学校移除建筑级 `unitCap`，只受全局军事人口限制；探险家营地、丛林神庙、沙漠官邸继续保留特色建筑/兵种上限，并在前台生产、单次招募、双通道、跨位面统计与后台结算中同时检查两层限制。
- 右上金币/能源/食物资源栏向下扩展第二行，新增“兵力 已出兵/房屋容量”（如12/48）；出兵详情同步显示全局军事人口，位面特色建筑另显自身编制。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查普通建筑持续扩军、特色双重上限、经济岗位不受占用、跨位面外派计数和HUD布局。

### 对话：光魔法中级「圣辉领域」与高级「圣光审判」落地（2026-08-23）
- 光魔法系补齐三段体系（原仅初级圣光）：中级 sanctuaryDomain=跟身圣辉环（友军每 0.5s 回血 + 每 2s 净化 1 个负面，敌方每 0.5s 光伤、僵尸 ×2.5）；高级 holyJudgment=长按蓄力天降巨柱（0.5~2.5s 蓄力 30%~100% 生效、不足 0.5s 失败不进 CD 返还 MP，半径 280→520px 随蓄力，敌方光伤僵尸 ×3 + 非 Boss 不死血量 ≤12%+0.4%/级 净化即死，友方大额回血 + 立即清除全部负面；Alt 自释放落点锁定自身）。
- 数值全部走 skills.json 双份 effectFormula 配置驱动；magic-categories 登记 light 系三技能与 tier 2/3 法杖门槛。新组件 sanctuary-domain-system.js / holy-judgment-system.js + 跟身特效 sanctuary-domain-fx.js（2:1 等距金色圣纹环 + 上升光粒 + 净化脉冲呼吸）；ChargeOrbFx 新增可选 palette 配色板（默认蓝色不变，审判传金色系）。蓄力长按复用贯穿雷枪模板（setHoldKey 安全网/失焦释放/定格收尾），input.js 与 quick-bar.js 按同口径接线。
- 图标 2 张（圣辉领域.png/圣光审判.png）走 klein-4b + skillicon LoRA 管线一次成型，规格对齐系列基准（fill≈70%、cy+29、aspect 0.83/0.88）。教训：normalize-skill-icon.py 的全图纯白键控会把技能高光白芯抠成洞——本次改用无白键归一（scratch normalize_icon_nokey.py），发光系图标入库前必须复核中心是否被抠穿。
- 验证：ESLint 0 错误、vite build 通过、关键测试文件通过（test-regressions 有 1 项「lord 池僵尸 family」失败为并行会话 agent-invasion.json 在途改动，与本次无关）；CDP 实机探针（tools/cdp-holy-magic-verify.mjs）全绿：领域激活/治疗/净化/僵尸每跳伤害致死，审判无法杖拦截/蓄力不足返还/满蓄落柱击杀/净化斩杀鉴别（纯伤害必不死的残血僵尸被净化致死）。按项目约定由用户实机重点检查两技能手感、特效观感与面板数值展示。

### 对话：红狼王两阶段状态机、突变体-3数值与血烟飞扑（2026-08-23）
- 红狼王基础生命、移动速度、等级、六维和基础攻击参数对齐怪物突变体-3；保留红狼王既有体型碰撞、狼形技能射程与12秒飞扑冷却，避免素材几何和专属技能被突变体配置覆盖。
- 狼形飞扑改为冲锋开始时锁定目标所在坐标并向该点位移，不再穿过目标；单次命中造成物理攻击×2并保留3秒致残。删除旧白色/淡蓝光线 Graphics 拖尾，改为沿实际位移轨迹每65ms散布一组红黑烟雾粒子。
- 半血变身期间承伤减少90%；完成后最大生命、六维、物攻/魔攻、物防/魔防、暴击抵抗及移动属性统一提升50%并回满生命。二阶段关闭狼形飞扑，只保留狼人普通攻击与嚎叫；狼人普通攻击独立结算50%暴击、2.5倍伤害，并向统一飘字入口透传同一次暴击结果。
- 怪物图鉴补齐四项技能说明及新变身字段展示，双份敌人/动画配置保持同源。未运行游戏、构建、lint、测试或运行时验证，按项目约定由用户重点检查飞扑落点与撞墙、红烟密度、变身承伤/回满、二阶段属性和暴击伤害。

### 对话：经济建筑面包屋组件化建模（2026-08-23）
- 新增标准 2×2 经济建筑 `bakery` 的组件化 Blender 真源：完整矩形石基、两层半木烘焙铺与连续双坡瓦顶，功能轮廓由侧墙内嵌烤炉、单座粗大烤炉烟囱、附墙店铺浅檐和暖色橱窗共同建立。
- 新增独立可编辑的面包陈列、无文字面包徽记、贴墙面粉袋和柴薪；烤炉、陈列柜与所有生产杂物均与主楼相连，明确禁止生成独立烤炉房、散摊或第二栋建筑。同步登记 manifest 尺寸/调色板、Depth 精修提示词和建筑组件矩阵。
- 用户确认模型后，以同一 Depth、`flux2-dev-depth`、Control 0.88、12 步和种子 `123150/123151/123152` 生成三张纯黄底结构候选及联系表；三张均保持单栋双坡屋、唯一烟囱、侧墙烤炉、正面柜台和完整地基。静态结构初筛推荐 `123151`，其面包徽记、店门/烤炉分区和深色瓦檐层级最清楚。
- 用户接受推荐的 `123151` 后，以该图作为唯一 init image、继续使用同一 Depth，按 48 步、Control 0.88、denoise 0.30 生成 `1231511/1231512/1231513` 三张低重绘精修候选及联系表；三张均保持单栋双坡屋、唯一烟囱、侧墙烤炉、正面柜台和完整地基。
- 静态精修初筛推荐 `1231512`：石材、木构、瓦片层次最均衡，烤炉和橱窗亮度清楚但不过曝，也没有 `1231511` 门楣轻微伪文字纹理。用户交回手工 RGBA 后保留其 Alpha，不再自动分割；新增 `finalize-building-runtime.py --nearest-opaque-edge-rgb`，以最近可靠不透明主体色修复选择工具留下的纯蓝半透明边缘，并把该规则同步沉淀进资产管线分卷。
- 最终紧裁为 `876×746`，等比显示建议 `288×245`、`footOffsetY=121`；2×2 严格标定为 `visualFootprint center=(0.5,0.732653)、size=(0.888889,0.522449)`，精确映射 `256×128` 且 X/Y 缩放差约 0.11%。正式 `assets/terrain/bakery.png` 和单建筑 lighting maps 已入库，Alpha 为单一主连通域、透明脏 RGB 与可见黄边均为 0；人口经济数值、建筑卡、BootScene 注册和生产逻辑尚未接入，等待后续经济设计。未运行游戏、构建、lint、测试或运行时验证。

### 对话：沙漠祭司 RTS 指挥修复与地面友军体量统一（2026-08-23）
- 修复沙漠祭司/丛林祭司复用的 `JunglePriestAI` 漏消费 RTS 移动命令：现与仓鼠牧师同口径处理移动、高架路线、到点转待命、指定攻击和目标失效收口；施法仍保持不可打断，新命令在当前施法结束后立即接管，不再回退跟随玩家。
- 所有非骑乘军事友军与地面施法单位按仓鼠牧师 running Alpha 实体高度统一视觉体量：目标可见高度75.684px，各单位按自身素材内容反算 `displaySize/spriteOffsetY/footOffsetY`；碰撞统一为半径20、40×100、`bodyHeight=100/size=64`，HUD默认偏移119。骑兵继续使用轻骑基准，经济矿工保留岗位特例。
- 友方单位标准工作流新增“步兵与施法友军默认体量”规则，并新增 `measure-friendly-unit-visuals.py` 量化工具；今后无特殊说明的新地面军事/施法友军默认遵循仓鼠牧师视觉与碰撞基准。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查 RTS 移动/攻击、高架目的地、全兵种同屏体量、脚底落地与碰撞编辑器尺寸。

### 对话：建筑放置完成音效与 footprint 烟尘（2026-08-23）
- 建筑面板成功放置实体后统一复用房屋升级完成反馈：音效继续读取 `population-economy.json#house.upgradeCompleteSound` 并通过 UI 声道全局播放，不再使用出售音效；落地烟尘复用非破坏性的 `BuildingFootprintDustEffect`，按每个新实体的真实 footprint 采样，不触发沉陷、失效或占格释放。
- 单体建筑、塔、产兵建筑、墙段、门、4格门和城墙楼梯均接入；批量拖墙逐格产生 footprint 烟尘但整批只播放一次完成音效。道路没有建筑实体，保留原铺路反馈。建造失败、无合法落点或未实际生成实体时不播放。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查不同大小建筑的烟尘覆盖与批量墙音效次数。

### 对话：红狼人白底稳定奔跑动画覆盖入库（2026-08-23）
- 以5080 MiniMax H3生成并在视频端完成纵向稳定的 `red_wolf_king_werewolf_running_white_h3_v6_final.mp4` 为唯一来源；选取同相位双步周期 `f0→f48`，每4帧采样 `f0~f44` 共12帧，覆盖正式 `werewolf_running.png`。循环接缝变化量51.1，处于段内正常步幅39.9~63.8范围。
- 重建继续保持8×6画布、640格、12个有效帧、`footY=606`和115ms/帧运行合同，不修改状态机、显示尺寸、碰撞体积或移动速度。固定全局缩放并锁定躯干锚点，最长落地帧对齐606，其余帧保留自然抬脚；躯干Y范围0.938px，脚底587~606px。
- 白底通过前景连通域抠图、边缘去污染和青幕残色最近毛色替换后入库；静态资产检查为12个连续有效帧、0贴边、0透明脏RGB。旧12帧表保存在本次导入工作目录，GIF预览与JSON来源报告已刷新。未运行游戏、构建、lint、测试或运行时验证，按项目约定由用户重点检查追逐步态、循环接缝、脚步落地和躯干稳定。

### 对话：沙漠祭司双视频精灵与沙漠官邸特色单位（2026-08-23）
- 按用户指定素材边界制作沙漠祭司四组透明精灵表：`1.mp4` 仅取 idle `f0~f35`、running `f46~f57`、dying `f160~f180`，明确舍弃其中 spell 与复位段；`2.mp4` 单独作为 spelling。四组统一为 512×512 单格、固定脚线，并保留联系表、GIF 与 JSON 抽帧报告。
- 新增正式友军 `desert_priest`，接入配置驱动预加载、祭司法术 AI、沙漠官邸生产选择、兵种登记、科技解锁、单位详情/图鉴数据源、存档 roster 恢复与世界-122 后台战斗结算；沙漠官邸仍默认选择骆驼骑兵，沙漠祭司生产周期85秒、食物360。
- 沙漠祭司与仓鼠牧师统一使用 `enemy` 六维派生公式；其基础生命与六维属性精确设为仓鼠牧师的85%，移动速度独立设为160px/s，法术伤害与施法参数因仓鼠牧师没有可比攻击字段而保持原设计。
- 沙漠官邸攻击/生命/机动/防御四项通用升级同步覆盖骆驼骑兵与沙漠祭司；“骆驼惊吓”继续常态显示但仅作用骆驼骑兵。未运行游戏、构建、lint、测试或浏览器探针，按项目约定由用户重点检查四组动画切换、施法出手帧、双兵种选择生产、共享升级、存读档与跨位面后台结算。

### 对话：雪原日式城堡组件化建模（2026-08-23）
- 新增雪原场景独特建筑 `snow_castle` 的组件化 Blender 真源：完整矩形石垣、三层退台与同轴多段宽阶、唯一前门楼、中央四层收分天守，以及通过覆顶回廊接入主体的左右镜像双层橹塔。
- 新增可复用的日式宽檐覆雪屋顶、城堡墙面木构/暖窗和多段阶梯装配函数；深色瓦檐、浅色灰泥、独立积雪面和克制金色屋脊顶饰均保持独立英文对象，方便后续单独调整。当前阶段只建立模型、材质预览与 Depth 真源，不接入游戏建筑配置或正式贴图。
- 经用户确认模型后，使用同一 Depth、`flux2-dev-depth`、控制强度0.88、12步和种子 `123140/123141/123142` 输出三张纯绿底结构候选及联系表；静态结构检查暂推荐 `123141`，其完整地基、四层天守、唯一门楼、连续阶梯和左右相连橹塔最清楚。尚未进入48步精修、透明抠图或正式资产入库，等待用户选择结构候选。
- 用户接受推荐的 `123141` 后，以该图作为唯一 init image、继续使用同一 Depth，按48步、Control 0.88、denoise 0.30生成 `1231411/1231412/1231413` 三张低重绘精修候选及联系表；三张均保留完整地基、三层退台、四层天守、双橹塔、门楼与阶梯。静态初筛推荐 `1231412`，其瓦片和石垣最清楚、积雪边缘克制、暖窗亮度较平衡；尚未抠图或覆盖正式资产，等待用户确认最终精修稿。
- 用户继续接受推荐的 `1231412` 后，从原始RGB绿幕执行确定性背景提取、内孔绿幕清理、20px边缘去污染和4px紧裁，得到 `755×853` RGBA运行时候选；Alpha主体为单一连通域，按等比显示推导为 `288×325`、`footOffsetY=161`。沿用建筑标准自动派生算法生成2×2入库建议：`visualFootprint` 中心 `(0.5,0.798462)`、宽深比 `(0.888889,0.393846)`，严格映射 `256×128`、水平偏移0，X/Y比例差约0.12%。正式 `assets/terrain/snow_castle.png`、出兵/功能配置、scene9生成、BootScene登记和光照图均未修改，等待后续明确接入任务。

### 对话：红狼人running躯干重心二次稳定（2026-08-23）
- 确认残余上下位移来自素材内部而非运行时：低Alpha边缘虽统一触及y=590，但实心脚掌和中央躯干仍有明显帧间起伏。对用户12帧透明表执行第二轮无重采样整像素校准，Y位移为`+16/0/0/-2/-3/0/+3/0/0/-3/-2/+6`，不改水平位置、内容细节、帧数和1.38秒循环速度。
- 中央躯干Alpha加权质心范围由25.785源像素压缩到6.785源像素，按狼人235.9375px显示尺寸换算，游戏内上下波动由约9.51px降至2.50px；第0→1与第11→0帧接缝同步收口。
- running专属`footY`由590调整为606并同步双份动画配置与代码兜底：最大下移帧继续贴逻辑地面，其余帧只保留自然抬脚，不会沉入地面。当前素材已备份，GIF预览和量化报告已刷新。未运行游戏、构建、lint或运行时验证，按项目约定由用户重点检查追逐时躯干稳定、脚底落地和循环接缝。

### 对话：位面特色出兵建筑共享升级规则复核（2026-08-23）
- 探险家营地与丛林神庙确认已由 `_isTroopProducer` 统一进入建筑级共享升级组：通用卡分别同步本建筑双兵种，赏金、侦察视野、丛林之王、丛林之力、灵动加速等独特卡常态显示并只结算到各自 `unitKinds`；共享读条、持续升级、存档恢复和后台位面完成均携带完整目标兵种列表。
- 沙漠官邸当前仅生产骆驼骑兵，运行结果原本没有串用，但“骆驼惊吓”缺少专属声明；现补充 `unitKinds:["camel_cavalry"]`，确保未来扩充第二兵种时仍按独特升级处理。位面祭坛和守夜烛台不是出兵建筑，不进入单位共享升级规则。
- 按项目约定未运行测试、构建、lint、浏览器探针或游戏运行时验证，本轮为静态配置与调用链复核；由用户重点检查两座双通道特色建筑的通用/专属卡作用范围，以及沙漠官邸“骆驼惊吓”卡牌与效果保持不变。

### 对话：建筑阴影同步 visualFootprint 语义（2026-08-23）
- 复核确认结构阴影已复用实体最终 `visualFootprint` 拟合、最终显示高度和同帧缓存，无需改变阴影几何；将接触面变量与调试来源由笼统 alpha 语义改为 `visual_footprint`，自动分层日志同步区分 visualFootprint 与真正 alpha 兜底。
- 阴影知识分卷改为显式标定优先、显示参数确定性派生、异形/缺配置素材才扫描 alpha；独立道路/地面铺装、碰撞和建造占格继续与阴影采样解耦。未运行测试或游戏运行时验证，按项目约定由用户检查正午及晨昏影根。

### 对话：所有出兵建筑统一建筑级升级组（2026-08-23）
- 骑兵学校、仓鼠军营、靶场、草屋、教堂及位面特色出兵建筑统一为“建筑持有一组升级卡”：卡牌不再跟随当前生成兵种筛选或隐藏，切换兵种只影响下一次出兵。没有 `unitKinds` 的通用项目在读条完成后同步提升本建筑全部兵种；有 `unitKinds` 的独特项目保持单卡常显，但只结算到配置指定兵种。
- 骑兵学校的攻击加速、攻击强化和生命强化同步骑士/轻骑，“冲锋强化”仅骑士、“轻骑机动”仅轻骑；兵营的战士/盾卫、靶场的火枪/射手及其余多兵种建筑按同一标准处理。旧存档中通用项目等级已分叉时取最高等级对齐，旧读条与后台位面结算也会按当前建筑适用兵种重建共享目标。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查切换兵种前后升级卡始终完整、通用升级同步两类现存/后续单位、骑兵两张专属卡只影响对应兵种，以及读条跨位面恢复。

### 对话：丛林祭司“丛林之力 / 灵动加速”专属升级（2026-08-23）
- 丛林神庙共享升级组新增两张仅适用于丛林祭司的独特卡：“丛林之力”Lv.1~6 使闪电、冰锥、火球在基础 Lv.1 上额外 +1~+6 级；“灵动加速”Lv.1 冷却 -10%，之后每级再 -5%，Lv.6 为 -35%。
- 魔法等级直接同步三种技能对象，闪电保留原基础伤害并按 `lightningStrike` 等级公式的成长比例放大；冷却同时覆盖祭司轮换施法间隔与冰锥/火球组件自身冷却，升级时按新倍率压缩当前剩余冷却。现存单位、后续补员、跨位面增援、全局升级存档和后台战斗估算均接入同一升级补丁。
- 新增 209×209 透明冷钢图标 `jungle-power.png` 与 `nimble-haste.png`。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查 Lv.0/1/6 三法术等级、三种冷却、当前冷却即时缩短、持续升级互斥和图标显示。

### 对话：仓鼠骑士与骆驼骑兵统一到轻骑视觉基准（2026-08-23）
- 不再用相同 `displaySize` 代表相同体量，改按移动动画 Alpha 主体中位高度归一化到仓鼠轻骑的179.765625px屏幕主体高度：仓鼠骑士由390微调为393.333333，骆驼骑兵由390缩为296.425121。
- 按各自素材脚底中位线同步重算精灵/实体脚线：骑士 `spriteOffsetY/footOffsetY=-74.5/74.5`，骆驼骑兵为`-129.7/129.7`，HUD基准统一190；碰撞、属性、移速和动画节拍均不变。
- 骑兵默认视觉标准已写入怪物/NPC分卷：以后没有特殊说明时，新骑兵必须先量化移动表的 Alpha 主体高度与脚底线，再按轻骑公式换算，禁止机械照抄390。未运行游戏、构建、lint或运行时验证，按项目约定由用户重点检查三种骑兵同屏体量和脚底落地。

### 对话：三级房屋共用母体组件化建模（2026-08-23）
- 参考当前 `house_lv1/2/3` 半木住宅贴图，在 World-122 建筑真源中新增 `build_house_level()` 与三级薄封装：三者严格复用同一紧凑2×2两层石基半木承重体、连续双坡瓦顶、烟囱、门窗、角柱和双灯，不通过增加占地、第三层或独立附属房表达升级。
- Lv.1 保留朴素门窗与基本照明；Lv.2 以附墙门廊、小型木阳台、花箱、木桶和货箱表现生活改善；Lv.3 在同一母体上增加雕花铜饰阳台、镀金木构、家徽、更多花饰、华丽灯具与两枚小型屋脊顶饰。新增组件全部保持独立英文对象名，并同步登记组件表、manifest 三级调色/共用尺寸和三份结构提示词。
- 已输出三级可编辑 `.blend`、同相机材质预览、Depth 控制图及本地联系表；经用户授权，仅向局域网5080提交三张 Depth、三份对应提示词和生成参数，完成每级2张、共6张12步结构参考图：Lv.1 `123110/123111`、Lv.2 `123120/123121`、Lv.3 `123130/123131`。用户接受推荐的 `123111`、`123121`、`123130` 后，已分别以对应12步图作为 init image、继续使用同一 Depth，并以 `denoise=0.30` 完成三张48步低重绘精修；静态目视确认三级均保留同一两层母体、完整坡顶和对应升级组件，未长出第三层、独立侧翼或额外主体。正式 `assets/terrain/house_lv1.png`—`house_lv3.png`、人口容量、科技门禁、升级费用和 visualFootprint 均未修改。未运行测试、构建、浏览器探针或游戏运行时验证，等待用户验收48步结果后再做真透明与 footprint 入库。
- 用户确认48步结果后，已用边缘连通绿幕抠图 `threshold=120` 生成真透明 RGBA，并以已知 matte 色修复边缘；全图自动分割会误吃石基、Depth 蒙版会裁掉 Lv.3 底座、全图封闭绿清理会误伤花叶，均未入库。最终仅对 Lv.3 左灯架使用 `remove-matte-rect` 局部清理封闭绿幕，`finalize-building-runtime.py` 同步增加已有 RGBA 的限定矩形 matte 清理能力，不改变矩形外 Alpha。
- 正式 `house_lv1/2/3.png` 已替换为紧裁 `855×823`、`837×829`、`846×846` RGBA；保持显示宽度 `270/270/298`，按等比画布更新为 `displayH=260/267/298`、`footOffsetY=129/132/148`，并重标 v8 strict visualFootprint。离线派生清单确认三级 mapped footprint 均严格为 `256×128`、`centerAdjustX/Y=0`，同步更新房屋缩略图；人口容量、科技门禁和升级费用未改。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查三级升级换图时的同地基贴合、尺寸递进和花箱/门廊边缘。
- 修复建筑面板房屋预览未同步：面板与建造幽灵读取 `producer-buildings.json#house`，此前仍使用旧 `270×310 / footOffsetY=155`，且未填写 `thumbnailPath` 时会优先回退并缓存正式 `assetPath`，新生成的专用缩略图没有被消费。现已同步为 Lv.1 的 `270×260 / 129` 与对应 v8 visualFootprint，并从当前正式 `house_lv1.png` 实际重建 `assets/ui/building-thumbnails/house_lv1.png`；配置切换到新文件名以避开旧 `house.png` 缓存，离线生成器也会遵循建筑显式指定的缩略图输出路径。重新派生后去除旧/新尺寸造成的重复 house ground-fit，面板卡片、建造预览和正式实体恢复同源。

### 对话：矿工营地冷钢详情与经济建筑升级卡统一（2026-08-23）
- 矿工营地详情收敛到冷钢右栏：45vw 独立栏目、采矿物流双列档案、岗位安排/矿工就绪进度、统一资源语义色与出售/关闭交互；移除旧棕色内联面板样式。
- 矿工营地六项升级改为共用升级卡，开始时扣除真实金币/能源，按项目时长读条，完成后才提升本栋模块并同步存活矿工；当前读条写入世界快照，离场位面继续推进，回场恢复剩余时间。
- 房屋升级由旧信息块改为同款升级卡和实时进度条；复核银行、经济工坊、仓库原有卡牌、读条、快照及后台结算链保持一致。风车和市场没有配置升级项目，保持原功能不额外造卡。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查矿工营地布局、六卡读条、切换位面恢复，以及房屋/银行/工坊/仓库升级进度。

### 对话：教堂贴图匹配 visualFootprint 版本8（2026-08-23）
- 按最新显式 `visualFootprint` 规则重新标定单塔教堂：不再沿用迁移期按旧480×464显示值反推的456.5×228局部占地，而以组件模型和实际落地底座确定源图 footprint 中心 `(424.5,635.5)`、宽深 `747×374`，归一化写入 `centerXRatio/centerYRatio/widthRatio/depthRatio`，继续使用 `strict` 模式精确映射标准2×2的256×128逻辑占格。
- 教堂配置兜底同步调整为 `displayW=293`、`displayH=283`、`footOffsetY=140`；版本8派生结果为约293.5×283、水平视觉偏移1px，mapped footprint 严格为256×128。正式PNG、牧师生产/升级、碰撞、占格、道路、亮窗蒙版与四张光照图均未修改；实体、建造幽灵、亮窗和阴影继续消费同一标定。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查教堂底座覆盖范围、建造幽灵落地一致性及四周单位遮挡。

### 对话：建筑生成与调整自动继承 visualFootprint（2026-08-23）
- 新增配置级统一派生入口：普通建筑没有手写 `visualFootprint` 时，运行时、建造幽灵、结构阴影、离线清单生成器与校准工具会共同从 `displayW/displayH/footOffsetY` 生成严格中心/宽深标定；只有两种标定来源都缺失时才回退alpha猜测。
- 建筑索引和世界-122建筑标准工作流同步收口：新建、换图、调整尺寸至少维护显示宽高与脚线；语义中心需要精调时用显式归一化标定覆盖；禁止重新引入 `anchorAdjustX/Y`。配置或贴图变化后必须刷新版本化manifest。

### 对话：怪物 family 支持多分类标签（2026-08-23）
- 怪物分类由单值互斥升级为向后兼容的多标签协议：保留 `family` 作为主分类，同时允许通过 `families` 声明多个分类；新增统一的 `getEnemyFamilies/hasEnemyFamily` 查询入口，旧怪物仅配置 `family` 时行为不变。
- 僵尸犬恢复主分类 `family:"僵尸"`，并登记 `families:["僵尸","动物"]`。僵尸地牢怪池、死寂雾潮和圣光仍将其识别为僵尸，“丛林之王”也会将其识别为动物；怪物图鉴同步支持出现在多个分类页并展示多个标签。
- 双份敌人配置已同步。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查僵尸犬的僵尸地牢刷新、圣光/雾潮判定、动物双倍伤害与图鉴双标签。

### 对话：红狼人running脚线锁定与变身线性放大25%（2026-08-23）
- 对用户微调后的12帧透明running做无重采样整像素脚线校准：alpha底边由 `590/595/590/587/590/590/590/591/590/585/590/588` 统一到格内y=590，对应逐帧位移为 `0/-5/0/+3/0/0/0/-1/0/+5/0/+2px`；只改垂直位置、不改水平位置和播放速度，原图保存在专用工作目录。
- 狼人形态视觉目标倍率设为1.25：640格按512参考换算的显示边长由188.75px增至235.9375px。2秒transform期间按线性进度从1.00逐帧放大到1.25，Phaser同纹理内动态同步displaySize；`footOffsetY`随尺寸重算，transform的footY=590持续锚定实体逻辑脚线，Canvas回退绘制同口径。
- 狼人碰撞体同步按1.25放大并随变身线性增长：当前半径45、90×90矩形/90高胶囊在完成后变为半径56.25、112.5×112.5矩形/112.5高胶囊；Phaser物理体、逻辑矩形、地面footprint和3D Collider共用同一倍率，变身中死亡则恢复狼形基准。双份动画配置已同步。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查running脚底稳定、变身脚线不漂、最终视觉体量和碰撞编辑器中的25%增幅。

### 对话：红狼人12帧running恢复减速前播放速度（2026-08-23）
- 撤销此前对狼人running整体播放速度降低33%的调整：以减速前23帧@60ms、约1.38秒/圈为基准，折算当前用户微调12帧表为115ms/帧，完整循环恢复至约1.38秒。12帧透明素材、0~11有效帧、8×6画布、walk/pacing节奏、状态机和实体移动速度均保持不变。
- 双份 `animation-config.json` 与 RedWolfKing 代码兜底同步更新，GIF预览同步恢复为约1.38秒/圈。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查恢复速度后的步频、循环接缝与实际追逐位移是否匹配。

### 对话：标记改名与美洲豹“丛林之王”升级（2026-08-23）
- 铁匠铺能力“标记箭”显示名缩短为“标记”，能力 ID、概率、持续时间、适用兵种和存档等级均保持不变。
- 丛林神庙共享升级组新增美洲豹战士专属“丛林之王”：Lv.1 伤害 +10%，之后每级 +2%，Lv.6 为 +20%；激活后对 `family:"动物"` 的怪物造成 2 倍最终物理伤害，并实时同步现存/后续美洲豹战士。
- 黑狼、红狼王登记为“动物”；僵尸犬保留“僵尸”主分类并通过多标签同时登记为“动物”，双份敌人配置同步。新增 209×209 冷钢专属图标 `jungle-king.png`。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查 Lv.0/1/6 普通伤害、动物双倍伤害、三类目标识别、存档恢复及升级卡图标。

### 对话：红狼人running切换为用户微调12帧透明循环（2026-08-23）
- 接受用户在同目录同名 `werewolf_running.png` 上完成的微调与背景移除结果；实图仍为8×6、640格画布，Alpha扫描确认第0~11格连续有效，因此运行时只注册这12帧，后续空格不进入循环，也不再次抠图或覆盖用户素材。
- BootScene有效帧改为0~11，双份动画配置和RedWolfKing代码兜底同步为12帧。为保留此前确认的整体慢速，run按172.5ms/帧播放，12帧完整循环仍约2.07秒；共用同表的walk/pacing按原循环总时长折算为124.583333/153.333333ms。状态机、实体移动速度、显示尺寸与footY均不变。
- 未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查新12帧循环接缝、透明边缘、实际追逐步频和脚底起伏。

### 对话：建筑显式 visualFootprint 中心标定（2026-08-23）
- 放弃将 alpha 底边、稳定横截面或屋檐轮廓作为普通建筑正式中心真源；拟合算法升级到版本8，每张建筑配置改用归一化 `centerXRatio/centerYRatio/widthRatio/depthRatio` 显式描述素材内部的 footprint，运行时把标定中心直接映射到逻辑占格中心。
- `strict` 模式分别求 X/Y 显示比例，标定宽深严格映射到2×2的256×128或基地4×4的512×256；现有 `displayW/displayH/footOffsetY` 已迁移为首批绝对标定，4处旧 `anchorAdjustX/Y` 相对修正直接移除，不再把历史右上/左上补偿固化进新中心。实体、建造幽灵、亮窗、运动层和结构阴影继续共享同一结果，运动层同步支持独立X/Y比例。
- alpha 自动识别仅保留给缺少显式标定的新素材作临时兜底以及 `autoFootprint:true` 异形建筑；未修改PNG原图，也未反写碰撞、占格、道路或寻路。
- 已刷新版本8派生清单；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查各建筑中心、宽深、建造幽灵落地一致性及风车/亮窗/阴影同步。

### 对话：红狼人低伏追捕跑RIFE循环插帧并接入（2026-08-23）
- 以已确认可用的23帧低重心追捕跑为原表，复用既有红狼动画优化协议：透明区最近前景色填充、RGB/Alpha分通道RIFE v4.6、插值帧脚底均值校准；循环动作额外处理第23帧→第1帧回绕对，正式运行时表扩展为46帧、8×6、640格。原23帧表保存在插帧工作目录，支持重新生成与回退。
- BootScene有效帧更新为0~45，双份动画配置和RedWolfKing代码兜底同步为46帧；run帧时长由90ms减半为45ms，walk/pacing因共用同一表同步减半为32.5/40ms，完整running循环继续保持约2.07秒，实体移动速度、动作姿态和状态机逻辑不变。静态资产检查为46个有效帧、0贴边、0透明脏RGB、脚底589~590px。
- 未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查追逐中的帧间连续性、循环接缝、脚底稳定和动画步频与实际位移是否匹配。

### 对话：红狼人追捕跑整体速度再降低33%（2026-08-23）
- 按当前60ms/帧基准将变身后二阶段狼人running的播放速度整体降低约33%，帧时长调整为90ms；23帧完整循环由约1.38秒延长到2.07秒。保持原始视频、抠图、精灵表、帧序列、低重心追捕姿态及实体移动速度不变。
- 双份 `animation-config.json`、RedWolfKing代码兜底、离线构建参数和GIF预览节拍同步为90ms。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查动画步频是否与实际追逐位移匹配。

### 对话：探险家营地专属升级图标（2026-08-23）
- 为“赏金”生成独立冷钢图标：钱袋、悬赏徽章与金币构成明确的击杀悬赏语义，不再复用银行“印钞机”贴图。
- 为“侦察视野”生成独立冷钢图标：黄铜望远镜配合蓝色扫描视域，不再复用攻击射程/准星贴图；两张图均统一为现有升级图标的 209×209 规格、冷钢铆钉边框及透明圆角，并保留悬停放大需要的主体边距。
- 升级配置已切换到专属资源路径；未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查升级卡图标加载、缩放清晰度与悬停放大是否完整显示。

### 对话：测试/静态检查基线修复（2026-08-23）
- 基线体检：`npx vite build` 通过；ESLint 从 3 错误/33 警告降为 0 错误/33 警告（修复 quest-system.js 兼容门面三个 setter 箭头函数隐式返回触发 no-setter-return，运行语义不变）。
- `npm test` 链在 test-world122-building-roads.mjs 中断：32 通过/1 失败——「快照恢复重建道路环」断言仍要求 `BuildingRoadSystem.attach(barracks, ...)` 字面量，而并行会话的重构已删除该行（小屋/军营路径合并）；属测试与在途重构不同步，未改动，待并行工作收尾后对齐断言。因 && 链式中断，其后测试文件本轮未执行。

### 对话：红狼人低伏追捕跑播放频率放缓（2026-08-23）
- 保留23帧低重心、躯干前压、双臂外张、双爪张开的追捕跑素材不变，仅将变身后二阶段 `transformedFrameDurations.run` 从50ms/帧调整为60ms/帧；完整循环由约1.15秒延长到1.38秒，播放频率由20fps降至约16.7fps，仍快于walk的65ms/帧。
- 双份 `animation-config.json`、RedWolfKing代码兜底和GIF预览节拍同步为60ms。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查追逐速度与实际位移是否匹配、循环是否仍流畅。

### 对话：建筑贴图按 footprint 宽深同步缩放（2026-08-23）
- 接地拟合算法升级到版本7：不再把“稳定底座横截面所在行”误当作 footprint 纵向中心；横截面仅负责底座宽度和横向中心，主体中央底边作为前顶点，再按逻辑 footprint 的2:1宽深比反推源图中的真正 footprint 中心。
- 普通建筑贴图允许在运行时同步调整显示宽高：测得底座宽度精确映射到2×2或4×4 footprint 宽度，并保持原图纵横比等比缩放；旧的屋檐/台阶安全限幅不再把底座缩回约189~210px，附件越界改为仅输出诊断。尺寸与中心由同一次拟合求解，不修改PNG原图，也不反写碰撞、占格、道路或寻路。逐建筑 `anchorAdjustX/Y` 仍作为测量校准量进入统一约束。
- 已同步升级离线清单与校准字段；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查建筑底座宽深是否覆盖占格、中心是否重合，以及实体/建造幽灵/亮窗/运动层/结构阴影是否继续同位缩放。

### 对话：红狼人 running 重做为低重心追捕跑（2026-08-23）
- 保留其余四个狼人动作和状态机接线，仅重新生成 running：同一红狼人母版、1024×576青幕、MiniMax H3 124帧/16步、seed `2026082316`，动作改为臀胯下沉、膝盖深屈、躯干前压、双臂持续外张、双手张爪的追捕姿态，排除直立人类式摆臂跑。
- 新原始视频保存为 `assets/videos/red_wolf_king_werewolf_running_chase_h3.mp4`；从同相第34~100帧每3帧采样，正式 `werewolf_running.png` 更新为23帧、6×4、640格、footY 590。躯干锁定消除水平漂移，手爪与尾巴均完整入格；0空帧、0贴边格、青幕残留0、透明区非零RGB为0。BootScene endFrame和双份动画配置帧数同步更新。
- 未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查变身后追逐时的低重心姿态、双臂/双爪展开度、23帧循环接缝和左右翻转观感。

### 对话：赏金猎人标记与探险家营地双专属升级（2026-08-23）
- 铁匠铺“标记箭”适用兵种扩展到仓鼠赏金猎人；斥候与赏金猎人的投射物命中统一调用同一标记入口，共享当前升级等级、触发概率、3秒持续时间、目标受所有伤害+15%和状态免疫规则，普通仓鼠火枪不获得该效果。
- 探险家营地新增赏金猎人专属“赏金”：Lv.0不产生额外悬赏金币，Lv.1按怪物默认地牢金币额外获得1.25倍，之后每级+0.15，Lv.6为2倍；继续与怪物本次正常掉落共用同一个随机底数，不改变祭品倍率和原掉落顺序。
- 探险家营地新增探险家专属“侦察视野”：基础视野由2000调整为1600px，每级+200px，Lv.6为2800px；探险家视野跳过昼夜、沙尘暴和死寂雾潮等负倍率，祭品与墙顶/楼梯等正向倍率继续生效。两张专属卡沿用同类建筑唯一持续升级、全局等级、存档恢复及现存/后续单位实时同步。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查标记触发、Lv.0/1/6悬赏金币、各级视野半径及昼夜/特殊天气切换。

### 对话：建筑贴图与 footprint 中心锁定融合（2026-08-23）
- 修正普通格网建筑接地拟合在完成底座中心对齐后，又因底边越界直接增加 `footOffsetY` 将整栋贴图继续向上推的问题；安全限幅不再覆盖中心对齐结果。
- 拟合算法升级到版本6：主体稳定底座中心与逻辑 footprint 中心一旦重合即锁定，左右轮廓和逐列下包络越界时只围绕该中心等比缩小，不再通过向右/向上二次平移收口；此前已通过实图预览确认的 `anchorAdjustX/Y` 改为同一次拟合内的中心测量校准量，不再被棱柱模式忽略。实体、建造幽灵、亮窗、运动层和结构阴影继续消费同一最终变换，碰撞、占格、道路与寻路保持不变。
- 已更新离线派生清单生成与校准诊断字段；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查各建筑 footprint 中心重合、预览落地不跳变、主体不再单向上漂，以及亮窗/风车运动层/阴影保持同位缩放。

### 对话：赏金猎人视觉尺寸与探险家原地踏步修复（2026-08-23）
- 仓鼠赏金猎人正式精灵的有效主体高度远大于普通火枪素材，显示尺寸由226按 alpha 内容框归一化为112，贴地偏移同步调整为-43；仅修正视觉大小与脚线，不改变碰撞、属性、射程、枪口或悬赏结算。
- 仓鼠探险家的卡住看门狗改为检查5秒窗口内的真实坐标位移，不再把绕开建筑时“到终点的直线距离暂未缩短”误判为卡死；确认原地未推进时会同时清除当前路径与旧中继目标，再从全图合法区域重选目的地。探险状态锁、12分钟计时、观察/挖掘与奖励协议保持不变。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查赏金猎人与其他步兵的主体高度/脚底位置，以及探险家在建筑密集区能否持续绕行。

### 对话：红狼王变身后二阶段狼人五动作生成与接入（2026-08-23）
- 通过局域网5080 MiniMax H3，以同一红狼人母版和1024×576青幕首帧生成 idle、running、attacking、howling、dying 五段124帧原始视频；循环/回位动作锁首尾同帧，死亡只锁首帧并在首次稳定侧卧后取帧。原始MP4保存在 `assets/videos/red_wolf_king_werewolf_*_h3.mp4`。
- 新增统一构建器，固定为 transform 末帧同尺度的内容高413px、640格、footY 590：导出狼人 idle 20帧、running 22帧、attacking 21帧、howling 20帧、dying 20帧；逐帧颜色抠图、去青幕污染、躯干/来源坐标锚定并生成GIF预览和采样报告。五表均0空帧、0贴边格、青色残留0、透明区非零RGB为0。
- BootScene、双份动画配置和 RedWolfKing 二阶段状态机已接入狼人五动作；变身结束后不再固定 transform 末帧，正常按 idle/run/attack/howl/death 切换。现有飞扑因本轮未生成独立狼人飞扑表，暂复用狼人 attacking 动作并保留既有白色拖尾。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查变身末帧到 idle 的尺寸衔接、跑步循环、攻击命中节奏、嚎叫锁定、飞扑复用观感和狼人死亡保尸。

### 对话：骆驼骑兵专属升级“骆驼惊吓”（2026-08-23）
- 沙漠官邸 `desert_cavalry_standard` 新增专属第五项升级“骆驼惊吓”：600px持续光环，Lv.1使范围内敌方最终伤害输出降低10%，之后每级+2%，Lv.6为20%；同类光环不叠加、取当前全局升级效果，费用与读条沿用1000金币+500能源+60秒。
- 新增并接入 `assets/ui/building-upgrades/camel-fright.png` 冷钢升级图标：以单峰骆驼的驼峰、长颈、短耳与厚唇建立主体识别，蓝色环形冲击波和受压制敌兵表达范围降伤；配置保留骆驼 emoji 作为图片缺失时的降级显示。
- 光环以200ms节拍刷新350ms的敌方状态，不直接乘除敌人基础攻击力；统一伤害来源修正接入玩家、侍从/仓鼠友军和建筑/防御结构三条受击链，覆盖近战、远程、魔法与特殊攻击。升级实时同步现有/后续骆驼骑兵并随全局兵种升级存档；世界-122后台因无逐实体坐标，在至少一名存活骆驼骑兵参战时按当前等级折算怪群输出。未运行测试、构建、lint或游戏运行时验证，按项目约定由用户重点检查Lv.0/1/6、进出600px范围、多个骆驼不叠加、状态免疫敌人及离场后台结算。

### 对话：教堂单塔组件化重构与12步结构候选（2026-08-23）
- 旧教堂贴图不直接重绘，新增 `build_church()` 组件化建模真源：标准2×2紧凑石砌礼拜堂、连续深蓝双坡屋顶、中央入口门厅、入口玫瑰窗、附墙扶壁与蓝色/琥珀色组合琉璃窗；按用户调整删除右侧钟塔和屋顶灯亭，仅保留画面左侧一座与主体相连的钟塔，钟室内加入独立铜钟几何。
- 已生成可编辑 `church_refactor_model.blend`、同相机材质预览、Depth 控制图和专用结构提示词；正式 `assets/terrain/church.png`、既有牧师生产玩法、占地、数值及亮窗资源均未修改。
- 经用户明确授权，将新版单塔教堂 Depth 与提示词发送至局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、12步、CFG 3.5、Control 0.88 生成 seed `122960`—`122964` 五张结构候选及联系表。五张均未新增右塔；静态初筛推荐 `122964`，其单塔单钟、尖顶、连续右侧屋顶和蓝橙琉璃窗最符合白模。`122960/122962` 为平顶钟塔，`122961` 增生成双层钟室，`122963` 增生成双钟。未运行测试、构建、浏览器探针或游戏运行时验证，本阶段仅完成建模与12步候选生成，等待用户选定后再进入48步精修。
- 用户选定 `122960` 后，将组件模型与 Depth 同步收敛为画面左侧平顶垛口钟塔、双可见钟面、无右塔/尖塔/塔顶十字架，并以该12步图作为 init，在再次明确授权后使用48步、denoise 0.30、Control 0.88 生成 seed `122965`—`122967` 三张低降噪精修候选及联系表。三张均保留单塔和连续右侧礼拜堂屋顶；静态初筛推荐 `122966`，其塔顶垛口最规整、钟体和彩窗清晰且未出现 `122967` 正立面的小色斑。正式 `assets/terrain/church.png` 尚未覆盖，等待用户确认48步最终候选。
- 经用户确认采用推荐项后，将48步 `122966` 提升为正式 `assets/terrain/church.png`：纯绿底确定性转为真实 RGBA、紧裁为856×827，配置按画布等比更新为480×464并同步 SHA-256；标准2×2 footprint、碰撞、牧师生产与升级数值均未改变。新增教堂专属 `anchorAdjustY:24` 将门阶/扶壁前缘收回接地容差，派生拟合结果为约293.5×283.5、前缘下伸7px；同步重建128×64面板缩略图、仅蓝/琥珀琉璃窗的同画布亮窗蒙版，以及教堂 silhouette/projection/height/normal 四张光照图。亮窗生成器新增按资产名增量更新与彩窗双色提取，避免重写其他建筑蒙版。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点检查建造幽灵/实体贴地一致、左钟塔遮挡、右侧屋顶、彩窗脉冲和昼夜阴影。

### 对话：传送门世界-122名称补充沙漠标识（2026-08-23）
- 传送门与位面进度系统的 `scene8` 显示名称由“世界-122”改为“世界-122-沙漠”，双份 `world-system.json` 同步；场景 ID、存档键、构造状态及传送逻辑均未改变。未运行测试或游戏运行时验证，按项目约定由用户检查传送门世界列表、传送门实体名称及相关入侵提示。

### 对话：骆驼骑兵基础移速调整（2026-08-23）
- 按最新口径将 `hamster_camel_cavalry` 的基础行走与奔跑速度从242统一调整为210，与仓鼠骑士相同；生命690、普攻115、基础物防45、四项升级倍率及沙漠官邸生产参数均保持不变。未运行测试或游戏运行时验证，按项目约定由用户检查移动速度与移速升级后的实际节奏。

### 对话：骆驼骑兵实体化与沙漠官邸出兵升级（2026-08-23）
- 将上一阶段 `camel_cavalry` 视觉骨架升级为正式友军实体 `HamsterCamelCavalry`，以仓鼠骑士为战斗基准统一提高15%：生命600→690、普攻100→115、移速210→242（241.5按整数系统取整）、物理防御39→45；保留2秒普攻节奏，但素材无冲锋动作，因此不复制仓鼠骑士的冲锋技能。攻击动画第9帧结算，四动作继续使用已完成的骆驼骑兵精灵表。
- 沙漠官邸改为通用出兵建筑，只生产骆驼骑兵：90秒/名、300粮食、单位上限5；接入生产实体工厂、碰撞编辑目录、指挥编队、全局升级识别、存档恢复和世界-122后台抽象结算。
- 新增沙漠官邸独立升级项目 `desert_cavalry_standard`，先开放伤害+2.5%/级、生命+5%/级、移速+1.5%/级、防御+5%/级，均为10级；升级不与骑兵学校共享等级，并同步作用于现有单位、后续生产单位与后台兵力。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查官邸面板、招募消耗/上限、四项升级实时生效、跨位面/读档恢复及四动作衔接。

### 对话：探险家营地升级标题与卡片布局归一（2026-08-23）
- 探险家营地升级说明不再作为 `#pbModules` 的独立两列网格子项占用一张卡片位置；“读条完成后全局生效”、共享适用范围和同类建筑持续升级限制均改为紧跟“单位升级”标题的小字说明，并在标题区域按可用宽度自动换行。
- 标题区继续横跨升级面板两列，三张共享升级卡从下一行开始，恢复为其他出兵建筑使用的两列升级卡排列；金币/能源持有量保留在同一标题区并在空间不足时自然换行。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查探险家营地的三张升级卡顺序、窄面板换行、资源摘要以及持续升级按钮边界。

### 对话：出兵建筑升级卡按钮溢出修复（2026-08-23）
- 出兵建筑升级区继续保留两列卡片，但单卡由横向挤压改为纵向布局：名称、资源需求与进度条在上，“升级 / 持续升级”双按钮在下排列，并为持续升级按钮分配更多横向空间；卡片、内容区、操作区和按钮网格统一限制为卡片宽度并允许内部收缩，持续升级按钮不再越过卡片边界。
- 样式只限定仓鼠兵营与 `.producer-building-panel.is-troop-producer`，不改变铁匠铺、研究院、仓库及经济建筑共用卡片。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查不同出兵建筑的两列升级卡、长项目名、持续升级/持续中状态、读条进度和较窄窗口下的按钮边界。

### 对话：仓鼠兵种三分类与骆驼骑兵动画骨架（2026-08-23）
- 新增仓鼠军事单位统一三分类：民兵、战士、射手、盾卫、斥候、火枪、探险家、赏金猎人为步兵；骑士、轻骑与新骆驼骑兵为骑兵；牧师为法术。矿工及非仓鼠特色友军不强行归类，指挥模式单选详情会在原身份后显示分类。
- 从用户指定的 720×720、24 FPS、241 帧 `骆驼骑士/1.mp4` 中提取待机24帧、行走16帧、攻击16帧、死亡16帧；BiRefNet 透明抠图统一为 512×512 帧格、8列精灵表，脚底固定在479px，循环段排除重复终点，一次性动作按视觉变化重采样。
- 新增 `camel-cavalry-visual.json` 并由 BootScene 预载、注册 `companion_camel_cavalry_idle/walk/attack/dying` 四个动画键。本轮按要求只建立视觉与分类骨架，不配置生命、伤害、速度、AI、碰撞、招募费用或骑兵学校入口。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户验收四动作透明边缘、循环衔接、尺寸与后续实体化方向。

### 对话：传送门详情构造按钮可用提示（2026-08-23）
- 传送门建筑详情右侧“构造传送门”按钮在 `WorldProgressionSystem.getConstructableWorlds()` 返回至少一个首次构造或重建候选时，显示冷钢底色上的金色闪烁与向外脉冲光环；候选为空或构造完成后随既有 `refresh()` 自动恢复普通按钮，不复制地牢解锁和生命周期判断。
- 按钮改用原生 `type="button"`、冷钢 `.bp-button` 及动态 `aria-expanded/aria-label`；系统启用减少动态效果时停止循环动画，保留静态金色外发光提示。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查有/无构造候选、展开/收起列表、完成构造后熄灭，以及“减少动态效果”系统设置下的静态提示。

### 对话：建筑贴图改为接地中心匹配 footprint（2026-08-23）
- 普通格网建筑不再用整栋主体最外层 alpha 包围盒中心强行贴满 footprint 左右竖线；改为在主体下部识别稳定底座横截面，将其水平中心对齐 footprint 中心、纵向位置对齐 footprint 左右顶点所在的中心线。屋檐、塔楼和不对称附件只参与整体安全限幅，不再把建筑主体拉偏或过度缩小。
- 底座横截面优先匹配 footprint 宽度，同时允许整体轮廓每侧小范围外伸，并允许逐列底边在前侧两条斜边外保留少量台阶/门槛；超过容差才自动缩小或上移。逻辑 footprint、碰撞中心、占格、道路和寻路完全不变，实体、建造幽灵、亮窗、运动层和阴影继续共用同一视觉变换。
- 接地拟合清单升级到算法版本5，旧版本派生结果不再注册；缩略图/拟合生成器和静态校准输出同步增加接地中心及外伸容差字段。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查骑兵学校、仓鼠军营、探险家营地及高塔类建筑的主体中心、底座与道路衔接，以及预览落地前后是否保持同位。

### 对话：主神空间祭坛接入建筑图层与建筑工作流补强（2026-08-23）
- 主神空间祭坛继续保留 NPC 点击、对话和既有弹窗身份，不伪造可被敌方索敌的 `_isDefenseStructure`；在现有标准2×2 footprint 后补接 `setupStructureDepth()`，并让 `_isGridBuilding` 结构进入墙、门、普通建筑共用的 footprint 拓扑排序，修复祭坛按普通 NPC 创建顺序显示而错误遮挡玩家的问题。
- 格网建筑的主体 alpha 接地拟合和结构太阳投影不再依赖战斗阵营字段；结构拓扑最终 depth 同帧同步到主体、亮窗、运动 overlay 和标签。`skill/07-world122-defense.md` 的建筑开发标准工作流新增强制图层检查：首次导入、替换或调整建筑贴图/尺寸/锚点/附属层时，必须核对 footprint 与结构深度接线，并做后、前、左前角、右前角四站位遮挡矩阵。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查主神空间祭坛前后与左右前角的玩家遮挡、祭坛与附近传送门/NPC的稳定层序、太阳投影位于主体下方，以及点击后原祭坛对话框和功能保持不变。

### 对话：地牢小地图恢复正常尺寸并与状态栏避让（2026-08-23）
- 地牢战斗（`scene7`）不再继承全局 `252×150` 宽版小地图，改为读取双份 `game-config.json#minimap.dungeon`，恢复左上 `(10,60)`、`150×150` 的正常尺寸与位置；主世界现有宽版规格保持不变。
- 小地图静态层缓存键补入最终宽高和锚点，场景切换造成布局规格变化时会可靠重绘；点击、拖动和战争迷雾层继续共用 `_minimapLayout()`，同步使用同一实际区域。Buff/Debuff 栏横移至小地图右侧 `left:172px`，保持菜单下方、组队栏上方的纵向安全区，不再遮住地牢小地图。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查进入地牢战斗后小地图为150×150且位于左上正常位置、静态墙层与动态点位完全重合、小地图点击/拖动映射准确，以及状态栏与小地图、组队栏均无重叠。

### 对话：仓鼠军营迁入通用出兵建筑（2026-08-23）
- 仓鼠军营不再由 `HamsterBarracksSystem` 单独维护；建造清单、实体创建、主循环更新、场景 setup/teardown、建筑详情、兵种切换、模块升级、集结点与兵线枚举全部统一到 `ProducerBuilding(cfgKey:'hamster_barracks')` / `ProducerBuildingSystem`。原造价1500能源、2000生命、45秒生产周期、5人上限、战士/盾卫及125/150食物消耗保持不变。
- 存档与后台模拟改以 `data/producer-buildings.json#hamster_barracks` 为唯一运行时配置真源；新快照保存为 `kind:'producer'`。旧快照的 `kind:'barracks'` 仍可读取，但恢复时立即适配为通用建筑，升级读条、持续升级、混编兵种、集结点和生产剩余时间继续保留，后续保存自然完成格式迁移。
- 矿工营地（`HamsterHut`）明确保留为独立经济建筑，不并入军事出兵类。建筑贴图拟合与缩略图工具也改从通用配置枚举仓鼠军营，避免同一建筑同时维护两份运行时参数。遗留军营模块/JSON暂不删除，以免破坏历史诊断脚本的直接导入，但游戏主流程已不再引用。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查旧档军营恢复、新建军营生产与升级、集结点保存，以及矿工营地补员采矿不受影响。

### 对话：红狼王变身脚下与腰部红黑弥漫粒子（2026-08-23）
- 红狼王 `_isTransforming` 的2秒窗口新增四路 `smoke_particle` 软雾发射器：脚下红雾/黑雾和腰部红雾/黑雾；红色通道使用 ADD 发光混合，黑色通道按项目黑粒子契约使用 NORMAL，粒子向周围缓慢扩散、放大并淡出。
- 腰部发射点随变身进度从脚底上方44px平滑升至74px，脚下通道固定跟随 collider 脚线；四路深度紧贴动态仲裁后的怪物贴图且偏移均小于0.1，避免红狼王在墙后时粒子穿墙。变身结束、死亡、实体失效或进入地牢路线地图模式时立即停止并销毁发射器，雾可见性同步纳入 FogVisualAdapter。
- 未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点检查脚下/腰部两组红黑雾的浓度、腰线随直立过程的位置、暗色地面上黑雾可见性、墙体遮挡以及变身结束后的即时回收。

### 对话：Buff/Debuff 左上安全区布局优化（2026-08-23）
- 状态栏从横跨屏幕顶部的自由换行长条改为 `252px` 独立安全区，固定在左上正常小地图右侧、菜单下方、组队栏上方；使用四列紧凑图标，三行以内完整展示，更多状态在该区域内部滚动，不再覆盖小地图、顶部角色信息、HP/MP或右上资源栏。
- 状态卡只显示图标、层数和剩余时间，名称与完整效果继续由既有悬停浮窗提供；路线选择和出征准备期间自动隐藏状态栏，避免与左侧地牢信息栈及全屏出征面板重叠。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查1~12个状态时的四列排列、层数/时间可读性、超过12个状态后的局部滚动，以及地牢路线/出征准备切换时显隐是否正确。

### 对话：沙漠官邸建筑本体接入（2026-08-23）
- 将用户选定的 layout v3、48 步 seed `122952` 精修稿完成纯黄底抠图、BiRefNet 辅助边缘核对与保守去色边处理，正式落为透明运行时贴图 `assets/terrain/desert_mansion.png`；BootScene 注册稳定纹理键 `desert_mansion`，并生成独立 128×64 建造缩略图与算法版本4的标准2×2棱柱贴合数据。
- 新增世界-122特色建筑“沙漠官邸”：仅允许在 scene8 建造，上限3座，无外围特殊铺装，使用详情面板且当前无生产、兵种、升级或额外功能；手动追加建造由位面科技“沙漠官邸营造术”解锁。
- scene8 位面配置接入特色建筑落点，第一世代缺失基础快照在传送门左下偏移 `(-330,+180)` 免费生成一座；已有完整 scene8 快照不做旧档迁移，避免超出“首次进入免费生成”的建筑本体范围。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点测试新档首次进入落点、贴地与遮挡、科技前后建造门禁、scene8限定、3座上限及详情面板无额外功能状态。

### 对话：红狼王变身精灵图导出与游戏接入（2026-08-23）
- 将已验收的 H3 狼→狼人视频从124帧中截取20帧：有效形变段0~91帧均匀采样，最后一格强制使用真实第123帧；导出为 `assets/enemies/red_wolf_king/transform.png`（5×4、640格），固定同比例、水平居中、footY 590，并按现有狼形262px内容高校准首帧屏显尺寸。
- 导出器 `tools/ai-gen/build-red-wolf-transform-sheet.py` 自动检测视频进入尾帧的时间点、青幕抠图、最近真实毛色去除H.264蓝青轮廓污染、固定脚线摆放并保存采样报告；本次图集20格全部非空、主体贴边格为0、青色残留为0、透明区非零RGB为0。
- BootScene 新增 `enemy_red_wolf_king_transform`；双份 `animation-config.json` 同步 transform 路径与5×4/20帧/640格布局。红狼王半血变身2秒内完整播放20帧，完成后固定末帧保持狼人形态；在后续狼人动作素材到位前，二阶段移动、攻击、冻结和主动嚎叫均保持末帧，死亡仍沿用现有狼形 dying。未运行游戏、构建、lint、浏览器探针或运行时验证，按项目约定由用户重点测试半血触发、首帧尺寸衔接、脚底稳定、20帧播放速度、变身后不闪回狼形，以及死亡切回狼形 dying 的当前过渡。

### 对话：状态栏、地牢路线提示、右栏顺序与障碍物图层修复（2026-08-23）
- 恢复实际动态 HUD 构建链中的 `#statusBarContainer`，Buff/Debuff 继续复用既有左上角样式、状态数据与悬停说明；根因是容器曾被移除，并非单纯被其他图层遮挡。
- 将“当前地牢：僵尸地牢”从屏幕中央独立 fixed 标签迁入左侧 `#dungeonRouteInfoStack`，固定排在预期奖励、特工入侵概率之后；路线信息栈统一控制显隐和左侧黑幕定位。右侧属性点提示入口改到科技树下方最末位。
- 僵尸地牢与世界-125 地牢遗迹的预制障碍物不再整组强抬到附近墙体之上，也不再套用编辑器绝对深度差；每件改按自身碰撞 footprint 前缘进入玩家/建筑共用的世界 Y 排序，仅保留极小同层顺序偏置，旋转件也不再按整张贴图包围盒底部长期遮挡玩家。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查状态效果出现/消失与悬停说明、路线左侧三栏顺序、属性点入口顺序，以及玩家从障碍物前后经过时的遮挡关系。

### 对话：红狼人母版与首尾帧变身视频（2026-08-23）
- 基于当前四足红狼母版重新生成独立红狼人母版，保持红黑毛色、金色眼睛、犬科头部、长鬃毛和完整尾巴；补齐双手爪、双脚趾、耳朵与尾端安全边距，产出透明运行时候选 `assets/enemies/red_wolf_king/red_werewolf_master.png`，未复用已经退出运行时的旧狼人资产。
- 经用户明确授权，将 1344×768 青底红狼首帧和新红狼人尾帧发送至局域网 `192.168.3.142:8188`，使用 MiniMax H3 首尾帧模式、20 步、5.17 秒、124 帧、seed `2026082301` 生成 `assets/videos/red_wolf_king_transform_h3.mp4`；五节点抽帧未见多头、多肢、断尾或出框，变身约在前半段完成，后半段保持完成姿态。
- 已静态核对视频为 H.264、1344×768、24fps、124 帧，逐帧主体贴边数为 0；未启动游戏、构建、浏览器探针或运行时验证，按项目约定由用户检查实际播放观感与后半段定姿时长。

### 对话：探险家营地共享升级卡与出兵建筑持续升级（2026-08-23）
- 探险家营地升级区不再按仓鼠探险家、赏金猎人拆成两组，改为固定显示攻击、机动、生命三张共享升级卡；机动与生命同步提升两种兵种，攻击强化仍由 `building-upgrades.json#unitKinds` 限定为只作用于赏金猎人。并行生产的位面特色建筑默认沿用这一共享卡与共享等级契约，旧档中同项目等级不一致时取最高等级同步，避免损失已购买进度。
- 仓鼠兵营及全部配置型出兵建筑的模块卡统一为“升级 / 持续升级”双按钮，铁匠铺和研究院能力卡同步使用同一冷钢按钮布局。同一配置建筑类别在当前场景与后台位面合计只能保留一个持续升级项目；读条期间可停止续升，但不能切换到另一项目。
- 修正持续升级资源不足后被取消的问题：前台每秒复查科技、全局读条占用与资源，条件满足后自动扣费并继续下一档；满级、目标失效、出售或被毁才清理。离场后台只结算已经开始的当前读条，持续目标单独保存，回到位面后立即恢复轮询。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点测试3座探险家营地的唯一持续项目、资源不足后自动开工、三张共享卡等级同步，以及攻击强化不影响仓鼠探险家。

### 对话：出兵建筑详情整合集结部署（2026-08-23）
- 指挥模式单选仓鼠兵营或配置型出兵建筑后，不再在 `45vw` 详情栏左侧额外挂出重复生命值、当前兵种和集结状态的小面板；普通单位与多选单位的 RTS 详情位置和内容保持不变。
- 两类出兵建筑详情新增共用冷钢“集结部署”分区，按“起点 → 目的地 → 全局后备/当前采用 → 操作提示”重排，实时区分独立集结、继承全局兵线和科技未解锁状态；独立点仍只接管该建筑之后生成的单位，右键可达位置设置、占地高亮、金色引导线和存档数据链均未改变。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查两类出兵建筑详情的集结分区布局、右键更新后的实时文案、独立/全局优先级，以及普通单位详情没有位置变化。

### 对话：删除建筑独立地基并用同源道路补齐（2026-08-23）
- 移除通用 `building_foundation_2x2` 贴图、BootScene 加载、`BUILDING_FOUNDATION_CONFIG`、生产建筑/军营/矿工营地/传送门配置字段，以及 GameScene、战争迷雾、压平视图中的 `foundationSprite` 生命周期；碰撞、占格和寻路保持原样。
- `BuildingRoadSystem` 在原外围12格之外，为建筑中央4格创建同源纯视觉补片：道路环使用 `building_road_tiles`，风车田地环使用 `building_field_tiles`。补片只填主体透明处与外围铺装之间的断口，不提供道路移速、不写手铺道路快照、不可退款，建筑拆除时随主体删除；建造预览同步显示完整4×4铺装。
- 未运行测试、构建或游戏运行时验证，按项目约定由用户重点检查建筑脚下接缝、道路/田地纹理连续性、建造预览和拆除后的道路保留行为。

### 对话：开发面板一键解锁包含未开放位面科技（2026-08-23）
- 交互开发面板“技能 → 解锁全部科技”明确包含全部 `planeResearch`：即使对应位面尚未开放，也会直接完成位面专项科技并解除关联建筑、兵种和升级门禁。
- 修正科技树显示优先级：未完成的位面科技仍按正常规则保持马赛克与禁止研究；已经完成的位面科技不再被“位面未开放”重新遮蔽，卡片和详情会显示真实完成状态。开发按钮提示与结果反馈同步说明包含未开放位面专项科技。未运行测试、构建或游戏运行时验证，按项目约定由用户测试。

### 对话：世界-125 遗迹大石板地砖替换（2026-08-23）
- 位面僵尸地牢（scene11）地板不再沿用地牢探险模式的 `blackbrick_7/8`：旧砖内部砖缝实测约 ±37°，与地板线 30° 基准和建筑底边 2:1（0.5）口径都不符，随机翻面进一步打乱走向。新砖 `ruinslab-1/2.png`（512×256 菱形）由生图管线产出：远程 5080 flux2-dev 出深色石板无缝材质（4×4 轴对齐大板、接缝差 1.7、压暗对齐旧砖亮度 53），`tools/ai-gen/build-dungeon-slab-tiles.py` 按逆等距映射切成 2:1 菱形砖——砖缝严格平行菱形边（与建筑地基/道路/底边同 0.5 口径），两块变体按整砖周期错相，混铺时砖缝全场连通。
- BootScene 注册 `ruinslab_1/2`；`_loadScene11` 地板 profile 换为新键（glow:false、底色 #050505 不变）。旧 blackbrick-7/8 保留不动，僵尸地牢中级/高级继续使用。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试世界-125 地板与建筑底边的角度契合、砖缝跨砖连通、分块边界无细缝。

### 对话：沙漠官邸三层组件化白模与 12 步结构候选（2026-08-23）
- 为世界-122 沙漠特色建筑建立稳定键 `desert_mansion` 的组件化建模真源：完整矩形地基、三层居中逐级收分的砂岩官邸主体、一座中央大洋葱穹顶、左右相连翼楼小穹顶、唯一中央拱门与三级入口台阶；保留左右镜像的双火炬状塔楼，每座由八角塔身、外扩平台、暗色灯室、暖光窗和火焰状青铜塔冠组成。
- 新增 `desert_mansion_dome()` 可编辑旋转体与 `build_desert_mansion()`，同步登记 manifest 低饱和沙岩/青铜绿调色、专用结构提示词和建筑级组件；已生成 `desert_mansion_model.blend`、材质预览与同相机 30° Depth。
- 经用户明确授权，将新版 Depth 与提示词发送至局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、12 步、CFG 3.5、Euler、Control 0.88 生成 seed `122940`—`122944` 共 5 张结构候选及联系表；`122941` 因凭空增加第三座塔淘汰，初筛推荐三层退台、双塔、入口和完整地基最稳定的 `122944`，`122942` 为次选。
- 五张候选背景均偏灰，尚不符合后续抠图所需的纯黄底约束；已补强提示词中的纯色背景要求，等待用户选定结构后先解决背景合规，再进入 48 步低重绘。特色科技门禁、首次进入免费生成、传送门旁落点和正式 `producer-buildings.json` 接入尚未实施。本轮未运行测试、构建或游戏运行时验证，仅完成 Blender 资产生成、5080 候选生成与静态视觉检查。
- 按用户对 `122944` 的布局反馈生成 layout v2：固定最底层地基不动，通过 manifest `bodyOffset:[0,44]` 将全部建筑组件沿固定相机下的画面左上方向整体平移，覆盖原后侧裸露地基并保持底脚高度不变；双火炬塔移到左右翼楼前肩，扩大附着连接段并避开中央入口。已生成独立 `desert_mansion_layout_v2_model.blend`、预览和 Depth；经再次明确授权，以新版 Depth 生成 `122944`—`122946` 三张 12 步候选，纯黄底要求已生效，三张均保持三层主体、三穹顶、双塔和完整地基，静态初筛推荐塔脚、入口净空与整体比例最干净的 `122946`。
- 根据 layout v2 右侧地基缺失、建筑越界的反馈继续从建模端修正为 layout v3：左右翼楼中心受固定地基宽度约束，外墙各保留 8 单位边距，双塔随翼楼向内重排；`DesertMansion_UpperPlinth` 与最底层地基固定居中，不再跟随三层主体的 `bodyOffset` 平移。已生成独立 `desert_mansion_layout_v3_model.blend`、预览和 Depth，本地预览中四个地基角完整可见；经用户再次明确授权，以新版 Depth 和提示词生成 seed `122947`—`122949` 三张12步候选及联系表，三张均保留完整右侧地基、双塔、三座建筑穹顶与三层退台，静态初筛推荐构图最均衡、塔楼关系最清楚的 `122948`。
- 经用户同意，以 `122948` 为初始图并继续使用 layout v3 Depth，按48步、CFG 3.5、Euler、Depth 0.88、denoise 0.30 生成 seed `122950`—`122952` 三张低重绘精修候选和联系表；三张均保住完整地基、三层退台、双塔与入口位置。静态初筛推荐砂岩层次、入口雕花、塔楼暖光和轮廓最稳的 `122952`；`122951` 的主穹顶尖饰近似新月形，与无宗教符号约束冲突，予以淘汰。

### 对话：地牢路线收益与特工概率信息栈优化（2026-08-23）
- 路线页左上两块信息改为共享冷钢信息栈：预期收益在上、特工入侵概率在下，整体按上方主背景图 contain 后的左黑幕真实宽度居中；紧凑高度止于左侧组队栏上缘之前，不再由两个系统分别使用 `top:18px` 与 `20vh` 定位而互相/与组队栏重叠。
- 预期收益不再显示仅供出征列表说明的 `dungeonList.reward`，也不再引用未实际启用的 Boss 奖励卡；当前地牢名称与等级仍由 `dungeonType` 解析，首领金币直接读取实际 Boss 发奖区间，精英宝箱装备读取实际档位和50%掉率，祭品范围按当前等级真实有效权重计算。
- 特工概率在判定开始前显示0%与剩余节点数，开始后显示当前回合真实 RNG 概率；入侵触发后状态显示“已入侵”，同时内部概率立即归零，捕获消费、战胜和系统重置路径再次兜底归零。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查D级以上概率推进/触发/归零、F/E级不显示特工栏、各地牢收益档位，以及1080p/2K左侧信息栈与组队栏间距。

### 对话：红狼王普通攻击末段快速咬合（2026-08-23）
- 普通攻击从完整插帧的23帧压缩为21帧：保留0~18帧的前摇、张嘴与扑咬爆发，删除原第19/20帧的慢速闭嘴过程，只留下第19帧快速咬合过渡与第20帧完全闭嘴，使张嘴→闭嘴在2帧内完成。
- 帧时长仍为50ms，攻击逻辑总时长仍为1.2秒；第20帧闭嘴后定格到动作结束，42%~75%命中窗口、伤害、定身、冷却与水平防抖不变。飞扑23帧、阶段时长和白色拖尾完全不动。
- 攻击母版、BootScene、双份动画配置、逐帧水平偏移及 RIFE 复现脚本已同步到21帧。未运行游戏、构建、浏览器探针或运行时测试，按项目约定由用户重点体验最后两帧的咬合速度和打击感。

### 对话：红狼王攻击与飞扑 RIFE 插帧丝滑化（2026-08-23）
- 确认普通攻击和飞扑原母版均只有12帧；沿用已定稿的 RIFE v4.6 RGB/Alpha 分通道方案，在每对相邻原帧间补一帧，重建为5×5网格、23个有效帧。一次性动作不做尾→首回绕，透明区先做最近前景色填充防黑边，中间帧再按相邻原帧脚底均值校准，保留全部原帧且不复制帧。
- 普通攻击改为23帧×50ms，1.2秒攻击总时长、42%~75%命中窗口与逻辑定身均不变；飞扑改为0~7帧蓄力、8~22帧冲锋，prepare/charge仍各900ms，位移、命中、致残和白色拖尾不变。
- BootScene、双份动画配置、水平防抖偏移生成器与怪物状态机注释同步到23帧；处理脚本和攻击/飞扑GIF预览保留在 `tools/ai-gen/` 供后续复现与验收。未运行游戏、构建、浏览器探针或运行时测试，按项目约定由用户重点观察扑咬爆发段、飞扑腾空/落地衔接与左右朝向稳定性。

### 对话：红狼王普通攻击防横抖与飞扑白色拖尾（2026-08-23）
- 普通咬击在出手瞬间锁定水平朝向，目标穿过中心线时不再逐帧左右翻面；同时为新版 12 帧攻击母版加入仅 X 轴的逐帧内容中心校正，左右朝向会镜像偏移，保留原素材纵向动作、伤害窗口和逻辑坐标。
- 飞扑沿用原有 ADD 加色世界特效链，将 80ms 的白蓝点链改为 55ms 叠加的三段渐细白色光带，增加淡蓝外辉光与 220ms 淡出，形成更连续的白色拖尾；飞扑方向、速度、距离、命中和致残逻辑不变。
- `data/sprite-offsets.json` 与 `public/data/sprite-offsets.json` 已同步，偏移生成脚本加入红狼王攻击母版及“只输出水平校正”能力。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试普通攻击左右朝向稳定性，以及飞扑拖尾连续度、层级和淡出效果。

### 对话：全建筑贴图按碰撞棱柱统一缩放与接地（2026-08-23）
- 普通格网建筑不再直接信任各配置的显示宽高做非等比拉伸：新增基于真实 alpha 的棱柱约束拟合，以现有2×2/4×4 footprint 为唯一几何真源，让完整可见贴图左右边严格贴住碰撞棱柱两条竖线且不越界，并用两条前侧下边约束逐列下包络，实现统一接地。
- 修正拟合输入边界：只读取建筑主体 `sprite.texture` 的 alpha；独立 `foundationSprite` 继续作为固定地面贴花，完全不参与主体宽度、接地点和缩放，避免把地基误判成上方建筑贴图。建造幽灵同样只使用主体纹理。
- 修复骑兵学校等抠图素材被边缘零散 alpha 噪点误判为建筑主体的问题：拟合前用8邻接连通域保留有效建筑及合理附件，过滤远离主体的微小碎屑；骑兵学校不再被左侧孤立像素按整张3774px画布压缩和错误居中。下包络扫描同时改为逐列精确检查，避免大图降采样漏掉局部凸出像素。
- 实体与建造幽灵共用同一拟合结果，旧 `anchorAdjustX/Y` 在棱柱约束模式下不再二次破坏边界；房屋换级、镜像、风车运动层、亮窗蒙版和结构阴影同步消费最终等比缩放。显式 `autoFootprint` 的异形建筑及墙、门、楼梯、防御塔等专用组合渲染保留原有专属几何，不改碰撞、占格或寻路。
- 派生拟合清单升级到算法版本4并加入连通域诊断。校准工具现覆盖生产建筑、矿工营地、仓鼠兵营、全部房屋等级和4×4基地，直接报告最终棱柱尺寸/锚点、原始/有效 alpha 边界、组件保留数和丢弃像素。本轮运行了只读静态校准审计与派生清单生成器；未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查骑兵学校、草屋，以及范围线开启时各普通建筑左右贴边、两条前侧底线接地、建造预览落地不跳变、风车/亮窗/阴影是否继续同位缩放。

### 对话：右侧栏目 ESC 与同键关闭统一（2026-08-23）
- 排查并统一人物状态、技能、背包、图鉴、任务、世界传送、队员管理与科技树八类常驻栏目：ESC 均优先关闭当前右栏，不继续打开暂停菜单；浏览器键盘与 Electron 转发 ESC 共用实际关闭生命周期。
- 组队栏新增可见态与 `toggleManage()`，P 再按一次可关闭；科技树新增 `toggle()`，Y 再按一次会完整恢复科技树自行暂停的游戏与计时器。任务/世界栏纳入 `rightSidebar` BasePanel 分组，CapsLock/K/Tab/U/L/O 原有同键切换保持不变；侧栏图标入口同步改用 toggle 语义。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户逐项检查八个快捷键的打开→同键关闭、打开→ESC 关闭，以及关闭后右侧入口、游戏暂停状态和输入是否正常恢复。

### 对话：右侧栏目图标比例与悬停裁剪修复（2026-08-23）
- 右侧常驻入口保留原 74×74 点击槽位、顺序、快捷键与点击行为，图标改用统一 68×68 内框和 `object-fit:contain`；技能、世界传送等非正方形源图不再被强制拉伸，视觉尺寸与其余冷钢六边形入口统一。
- 侧栏及按钮开放视觉溢出，悬停时仅提升当前按钮层级，保留原有放大特效并完整显示越出槽位的图标，不再被按钮边界裁剪。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查技能图标比例、各入口静态大小、悬停四边完整性及点击热区是否仍为原尺寸。

### 对话：碰撞体积编辑器纸面预览、独立测试生成与友军接入（2026-08-23）
- 怪物测试按钮不再解冻并带走当前调参预览体；怪物与友军均保留冻结纸面预览，每次点击测试按钮都会在纸面预览的逻辑位置另生成一只对应的正式单位，可连续生成验证碰撞效果。
- 碰撞体积编辑器新增“友军单位”分组，覆盖仓鼠矿工及13种可生产军事单位；圆柱 footprint、躯干矩形、偏移和贴图尺寸沿用怪物同款拖拽规则，分别保存回各兵种自己的配置文件。友军实体统一从配置读取碰撞半径、高度、躯干判定和偏移，使编辑结果同时作用于预览、测试单位及后续正式生产单位。
- 未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点测试怪物/友军连续生成位置、14类友军预览与贴图缩放、保存刷新后碰撞配置，以及正式生产单位是否读取新参数。

### 对话：建筑亮窗脉冲可读性增强（2026-08-23）
- 保留现有精确窗形蒙版、加法混合、战争迷雾和建筑遮挡链，只重做亮度包络：白天强度底线由20%提高到36%，默认基础透明度由0.68提高到0.84，主呼吸振幅由约±10%提高到±26%，并改用平滑曲线拉开亮/暗停留；双频微闪由4.5%降到1.8%，避免背景闪动掩盖主脉冲。风车与银行原有偏低的独立透明度同步提高到0.90/0.86。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点检查白天、黄昏、深夜三档亮度，近景/远景脉冲辨识度，以及压平视图和战争迷雾下仍正确隐藏。

### 对话：地牢路线黑屏、冷钢路线 HUD 与主神空间祭坛占地（2026-08-23）
- 修复地牢路线界面仍被黑色 Phaser 画布覆盖：地图模式不再错误假设 Phaser Canvas 是 `#gameContainer` 直接子元素，而是按稳定 `.phaser-game-canvas` 类隐藏上层画布，让下层 `#gameCanvas` 正常显示地牢背景、路线、节点与提示。
- 路线选择 HUD 收敛到冷钢主题：预期奖励改为左上档案卡并避开左侧组队栏；安全撤离、放弃返回移除旧整板图片，改为带键盘焦点语义的冷钢原生按钮；地牢标题、生命/魔法状态栏同步使用主题字体、边框和层级。
- 主神空间祭坛保留原 `npcType:'altar'`、点击范围和对话框入口，只把地面占地改为配置化标准 2×2 建筑 footprint，并在创建后重建 Collider；双份 `game-config.json` 保持同步。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试路线背景/节点恢复、不同分辨率左右 HUD 不重叠、两个按钮交互，以及祭坛四边碰撞和原对话框。

### 对话：指挥模式建筑远程详情、集结面板与小地图拖动（2026-08-23）
- 指挥模式点击建筑统一取消玩家距离限制，打开塔、小屋、兵营、配置型建筑及掩体/门/楼梯详情时不再退出指挥模式；掩体类新增详情专用入口，不启动建设、放置或回收监听器。点击详情内按钮被纳入 RTS 指针阻断，避免指令穿透。
- 出兵建筑左键单选现在会同时打开既有建筑详情并保留 `producer` 选择、独立集结右键语义、真实占地高亮和集结引导线；紧凑的出兵/集结信息面板在桌面分辨率下停靠到右侧 `45vw` 建筑详情栏左侧，普通单位详情位置不变。
- 指挥模式小地图支持左键按住连续拖动摄像头：初次按下只接受真实地图内容，拖出内容区后坐标钳在地图边缘，拖动期间暂停屏幕边缘平移并保留当前单位选择。压平建筑视图在战争迷雾同步后统一关闭建筑装饰特效，覆盖风车旋转层、窗户脉冲层及后续共用建筑 overlay/windowGlow 的效果；退出压平后由常规渲染恢复。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点测试远距离逐类建筑详情、出兵建筑集结小面板位置、压平特效隐藏/恢复、小地图单击和长按拖动边界。

### 对话：出征钥匙提示与地牢路线界面层级修复（2026-08-23）
- 出征准备页加宽左侧钥匙/奖励说明栏，并让它和中部面板统一按边框盒计算共享宽度；长钥匙状态不再横向溢出，说明栏内边距也不会额外越界遮住中部左下提示。
- 地牢路线选择模式为 Phaser 画布登记专用类并显式隐藏该上层画布，让下层 `#gameCanvas` 独占显示地牢配置背景、路线、节点和提示；同时在 `body.map-mode` 下强制收起可能残留的出征遮罩、面板及说明栏，避免异步切场期间再次盖黑路线页。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试持有/不持有匹配钥匙时的出征提示布局，以及进入地牢后的背景、路线节点、路线提示和退出按钮。

### 对话：红狼王母版六动作与动画状态机重构（2026-08-23）
- 红狼王运行时美术收敛到 `assets/enemies/red_wolf_king/`：导入同一母版生成的 idle、running、attack、pounce、howl、dying 六套透明序列；移除 `assets/enemies/` 根目录下11张旧红狼狼形/变身/人形贴图，BootScene 与双份动画配置不再加载任何旧资源。
- 红狼王继续沿用黑狼的手动 `setFrame` 状态机，但视觉状态重构为唯一狼形：巡游/移动共用16帧 running，近咬12帧完整播放并在第5~9帧命中，飞扑前4帧蓄力、后8帧冲锋；半血二阶段仍保留原伤害翻倍、回血和减伤窗口，但阶段表现改为狼形嚎叫，不再变成人形，二阶段主动嚎叫与激励机制保持。
- 640格攻击/飞扑按512基准动态补偿显示尺寸，并按各表脚底线同步 `footOffsetY`，避免切动作缩小或漂浮；新视频已含完整身体运动，关闭旧程序化弹跳/拉伸叠加。新增2秒 dying + 1秒末帧保尸生命周期，死亡不再立即销毁贴图。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试六态切换、两种攻击命中时点、半血阶段、贴地尺寸和死亡清理。

### 对话：F1指挥模式与玩家RTS控制（2026-08-23）
- 指挥模式快捷键统一为 F1，再按 F1 返回普通模式；切换在捕获阶段先于通用输入处理，进入后清除残留移动、鼠标、蓄力技能、无人机操控与盾防状态。指挥期间玩家的 WASD、冲刺、闪避、主副攻击、换弹、切武器、拾取、技能和快捷物品输入均锁定，菜单与非角色栏目保持可用。
- 玩家本体接入现有 RTS 单选、框选、双击同类与数字编组，可与其他友军混合选中；右键空地通过玩家专用控制器消费现有地面 A*、高架路线、墙体解析和玩家移速修正，抵达后进入独立 hold，不写友军 `_command`，也不把玩家 `maxSpeed` 清零。指挥移动默认使用奔跑表现，并按长枪、单手枪、双持枪和近战姿态复用各自 running 配置，但不获得冲刺速度、不消耗耐力，也不触发冲刺攻击与冲刺烟尘。
- 右键敌人会按当前主手武器射程追击并持续执行现有普通攻击链；枪械打空后沿用原弹药系统自动换弹，换完只要目标仍有效便继续攻击。SUPER90、.357左轮等逐发装填武器在指挥攻击中禁止输入打断，必须等现有逐发流程装满并结束 reloading 后才恢复开火；弓类保留原蓄力与释放，退出指挥、目标死亡或新命令会清理旧路线和攻击意图。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点测试 F1 双向切换、残留按键、玩家框选/编组、不同武器奔跑姿态、右键追击、标准弹匣续射、SUPER90与.357满弹后续射，以及退出后普通控制恢复。

### 对话：矿工营地人口岗位、返营提交与背包扩容（2026-08-23）
- 矿工营地改为人口驱动的经济建筑：新建后不再免费直接出矿工，详情面板可用空闲人口增减矿工岗位；“矿工增援”保留原等级与费用，但改为增加岗位容量。仓鼠矿工继续作为可寻路、可受击的经济实体存在，统一从 RTS 点选、框选、编队和指令入口排除，旧命令也会被清理。
- 矿工默认背包改为300，矿点产出先装入个人背包，满包后返回所属矿工营地，只有营地实际提交成功的数量才进入仓库；仓库满时剩余资源继续留在矿工身上等待，撤销岗位也会先返营提交再离岗。新增“背包扩容”5级，每级+100，并实时同步现有矿工。
- 挖矿动画触发从可能因 `animationcomplete` 丢失而卡住的布尔锁改为单调挥锄序号，每次有效采矿命中都会触发一次动画；人口岗位、矿工携带量、营地升级与后台位面采矿均接入快照/恢复和人口效率结算。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点测试人口增减、满包返营、满仓等待、跨位面恢复、5级扩容和连续挖矿动画。

### 对话：指挥模式单位详情、升级与状态影响栏（2026-08-22）
- 指挥模式单选单位详情改为冷钢战术 HUD：身份、生命/魔法、六维与作战数据重新分区，单选态扩宽至396px并按动态顶部限制最大高度；多选与出兵建筑仍保持紧凑宽度，720px以下贴右自适应、430px以下属性网格降为两列。
- 新增“兵种升级”栏，实时读取全局兵种模块与特殊能力等级；特殊能力配置补充机器可读 `unitKinds`，探险家/祭司等无普通攻击兵种继续遵守模块筛选。仓鼠矿工单独读取所属矿工营地的建筑级模块，不误套全局兵种等级。
- 新增“BUFF / DEBUFF”栏：标准状态读取选中实体自己的 `statusEffects`，并兼容中毒、流血、魔力易伤、无人机易伤等旧式层数/计时字段；层数、条件化效果值与剩余时间按当前运行时字段持续刷新，结构不变时不重建列表 DOM。详情查询改为纯读取，不再每帧调用 `calculateCombatStats()` 覆盖已应用的防御升级。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户重点测试升级后即时显示、激励/中毒/流血等状态倒计时、状态到期移除、矿工所属营地升级及1080p/2K布局。

### 对话：探险家营地与丛林神庙通用兵种强化（2026-08-22）
- 探险家营地与丛林神庙接入共享的特色兵种强化项目：攻击强化每级 +2.5%、机动强化每级 +1.5%、生命强化每级 +5%，均为10级；每级沿用通用军事升级费用1000金币、500能源和60秒读条。攻击强化仅向仓鼠赏金猎人与美洲豹战士开放，仓鼠探险家和丛林祭司不显示、也不能启动该项目。
- 双通道募兵面板按兵种分组展示独立全局等级，并显式携带升级目标兵种；完成后继续通过既有全局兵种升级链同步场上单位、后续新兵、存档恢复及后台位面结算。同一栋建筑仍只允许一条升级读条。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试四个兵种的面板项目筛选、读条、即时属性变化和跨位面恢复。

### 对话：友方单位楼梯与城墙通行防卡死（2026-08-22）
- 新增统一高架友军碰撞豁免：任一友方移动单位真正取得 `stairs / wall_walk` 表面身份或处于墙梯桥接帧后，该友军对不再执行实体 footprint 分离；`MovementSystem` 同时关闭高架友军自身的同阵营排斥，避免墙顶单位、楼梯单位或入口外单位把通行者反向推离窄表面。单位回到 `ground` 后判定自动失效，恢复正常碰撞体积。
- 楼梯交通保留入口 FIFO、方向锁、预约 TTL 与超时恢复；首个友军实际进入楼梯后，同方向友军允许按队列流水获权，反方向及敌军继续互斥，避免“已关闭物理碰撞但仍被整段楼梯单持有者永久挡住”。墙体、建筑、防坠线、地面 A*、表面原子提交和高架独立恢复均保持原契约。同步更新寻路分卷说明。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试多人连续上楼、墙顶交错穿行、上下行冲突等待、下楼恢复碰撞及楼梯口敌友混行。

### 对话：友方单位寻路与多选编队落点优化（2026-08-22）
- 普通静态建筑的真实 `iso_rect` footprint 接入寻路硬障碍，并以 250ms 节流的建筑签名同步拓扑版本；防御塔、掩体、门闸和墙体楼梯继续遵守原有友军通行、门等待及高架导航契约。建筑中心类战斗目标会先投影到 footprint 外最近开放点，避免路线穿楼或对不可站立中心重复重算；树木寻路半径同步改用真实碰撞半径。
- 连通性预检查增加“拓扑版本 + 半径桶 + 目标格”的短时分量预读，同一编队可复用已访问格结论，保留既有 3ms 帧预算，不重新引入全图 Region 重建；PathManager 在拓扑变化后立即检查前方最多8段完整走廊并局部修复，不再只按 1.5~2.5 秒周期检查离散节点。
- RTS 右键移动、移动攻击和巡逻为多选单位分配等距地面槽位，并把被建筑占用的点击点投影到附近合法落点；高架路线仍逐单位交给统一高架导航。脱困逻辑统一识别 RTS、探险、出兵离场和高架落点意图，候选点同时遵守墙体忽略集合与建筑寻路障碍。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试建筑密集区绕行、建造/拆除后的改道、多选队形、探险长距离移动、门闸等待和出兵离场。

### 对话：建筑错误遮挡军事单位修复（2026-08-22）
- 玩家、敌人（含保留尸体）和军事友军的自然渲染深度统一严格读取逻辑脚底 `entity.y - entity.z + 10`，不再让 `spriteOffsetY`、动作帧脚底校正、呼吸微动或贴图中心偏移污染建筑遮挡仲裁；当前帧 alpha 包围盒仅继续负责可见宽度与遮挡高度，敌人主贴图在同帧仲裁后不会被后续阴影同步覆写。
- 修正仓鼠火枪手长期遗留的 `spriteOffsetY:-36 / footOffsetY:17` 错配，脚底偏移改为从贴图中心偏移反算 36px；仓鼠赏金猎人继承同一修复。建筑 footprint 拓扑、墙后遮挡、高架深度和固定地表层规则保持不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试各军事单位在建筑四个前缘、角点与动画切换时的遮挡关系。

### 对话：F级“冰封世界-初级”地牢与世界-123解锁（2026-08-22）
- 新增 `frozenBeginner / frozenDungeonBeginner`：路线、节点数、三波遭遇和独立Boss战参考僵尸地牢初级，评级F、消耗F级钥匙；普通、精英、Boss三套池均锁定 `blackWolf` 作占位，`eventScope:"frozen"` 在未设计冰封限定事件前只抽通用事件。
- 冰墙严格复用现有 Blender 城墙段几何，只由 ImageGen 生成平视冰块砌体材质，再经 `render-cover-real.py` 渲染和 `prep-frozen-wall.py` 紧裁标定；成品 `frozen_wall_straight.png` 为684×659透明图，底边斜率0.6400、拟合残差最大0.9px。`frozen` 墙样式接入既有菱形墙生成器，四角继续由同一直墙程序化拼合，功能门暂复用现有门闸。
- 地牢地板连续纹理能力从世界分块地板扩展到单房菱形、Boss房和竞技场，冰封初级因此直接复用世界-123无缝雪面并保持30°纵向压缩；新增独立“冰封世界加载中”配置，避免误显示僵尸地牢标题与背景。
- 世界-123从未开放构造改为完成 `frozenBeginner` 后进入免费首建传送门候选，通关结果仍沿用统一地牢进度记录与位面状态刷新链；同步双份地牢、载入和世界配置，并重生成8地牢要素表。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试冰墙拼接、雪地显示、全黑狼波次和通关解锁。

### 对话：出征界面背景替换（2026-08-22）
- 使用素材库 `场景/背景图/出征界面.png` 覆盖项目唯一出征背景 `assets/ui/expedition-bg.png`，旧背景图内容已移除；两处出征面板样式继续共用同一路径与居中 `cover` 规则。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试实际界面遮罩、文字可读性与裁切效果。

### 对话：废案场景清退与世界-123任务架构一期（2026-08-22）
- 删除无玩家入口的 `scene3` 列车、`scene4` 古堡、`scene5` AI测试场，以及仅由旧裂隙任务进入的 `scene2` 雪地：移除注册、分发、四个内嵌 loader、专属更新/小地图分支、程序化雪树纹理和双份旧配置；共享 Portal、BlackWolf 与世界-123正式雪原素材保留。
- 小鼠侍从裂隙主线改去 `scene9 / 世界-123·雪原`。任务通行必须同时匹配活动任务、目标场景与 quest mode；普通世界-123仍遵守正式传送门构造门禁。任务使用瞬态雪原实例，跳过永久坐标、兵线、建筑、资源、快照、迷雾与入侵运行时，裂隙/返回门/黑狼出生点按菱形可行走区采样并清除重叠雪松与地表装饰。
- 新增双份 `quests.json`、不可变 `QuestRegistry` 与版本化 `QuestStore`，把定义、任务进度和活动会话拆开；旧 `QuestSystem.QUESTS` / `QuestState` 保留为兼容门面。主存档接入任务序列化/恢复，新游戏统一重置，旧存档缺任务字段自动使用初始态，活动裂隙的位置与进度可在再次由侍从进入时续接。
- 补齐任务生命周期边界：返回门必须确认切回主城后才结算；任务实例内原地读档会重建裂隙/返回门，无活动会话的存档则退回主城，主城续接任务时不会套用瞬态雪原坐标；任务存读档不会改写队友驻留或消费世界-123永久兵线队列；蟠桃原地续命不再误判任务失败；黑狼出生点增加雪原菱形内缩校验，侍从结算对话只认整条任务完成状态。未运行测试、构建、lint、浏览器探针或游戏运行时验证，按项目约定由用户测试。

### 对话：祭品同稀有度覆盖与地牢钥匙准入（2026-08-22）
- 位面祭坛改为每个稀有度仅允许1件祭品生效，新献祭会覆盖旧的同级祭品并把持续时间刷新为30分钟；最多同时生效6件。祭品效果改为主神空间、世界与地牢共用同一份限时状态，进入地牢不再冻结或转为本局永续，旧存档的同稀有度多条记录会按保存顺序保留最后一条。
- 废止“携带对应稀有度祭品进入地牢”：F~A 地牢严格对应商店 `anchorTokenF~A` 钥匙，确认出征自动检测背包与仓库并优先从背包消耗1枚；钥匙保留既有代币合成兼容，但不能在位面祭坛献祭。
- 出征面板删除祭品填充栏、祭品效果栏、重置按钮和强制打开的右侧背包，只保留地牢/队伍选择及左侧钥匙持有量提示；主神空间祭坛改用位面祭坛同款 `defense_base` 贴图与尺寸。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试同级覆盖、30分钟跨场景到期、背包/仓库扣钥匙、无钥匙拦截和祭坛显示。

### 对话：新单位动画平滑与沼泽林地载入背景（2026-08-22）
- 丛林祭司 AI `_stop()` 硬清零改 `_stop(dt)` 指数衰减（0.85/帧）、跟随 120px 缓出、滑行保持 walk；仓鼠探险家补目的地缓出与待命滑行保持（衰减站定此前已有）；赏金猎人/美洲豹战士继承火枪手/战士 AI 自动获得全套平滑。
- 新增沼泽/丛林载入背景 3 张（gpt-image-2），`loading-screen-config.json` 注册 `swampForest`（scene10，三张随机），与僵尸地牢同一读图机制；同步产出沼泽丛林怪物情绪图与 13 只怪物设计案（docs/沼泽丛林怪物设计-2026-08-21.xlsx，未实装）。
- 骨雕战面贴图透明底退化（模型画了背景板）经重新生成修复；教训沉淀 skill/02。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。


### 对话：任务档案面板全面冷钢化（2026-08-22）
- 任务栏从出征/奖励组合工厂中的预创建 DOM 迁移为 `QuestSystem` 自主管理的 `BasePanel` 懒实例，统一挂载到 `rightSidebarPanelLayer`；保留 L 键、右栏入口、NPC 打开、Esc 关闭及接受任务后的既有流程，不改裂隙、撤离和奖励规则。
- 面板重构为“档案索引＋行动详情”双栏：新增待领取/进行中/已完成统计、任务状态标签、总进度、发布者/目标区域、逐项目标进度、奖励档案和状态化底部操作；裂隙、撤离和完成事件会在面板开启时原位刷新。
- 列表与接受操作改为原生按钮和事件委托，补齐 `aria`、键盘焦点、关闭后焦点恢复、空状态和窄屏布局；删除旧棕色硬编码任务样式，全部视觉收敛到冷钢主题变量与通用面板组件。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试任务栏打开/关闭、接受任务、NPC 对话衔接、任务进度和 1080p/2K 布局。

### 对话：死寂雾潮玩法化与守夜烛台（2026-08-22）
- 世界-125死寂雾潮从开发用纯视觉态升级为统一天气玩法态：友军单位基础视野在雾潮中乘 `0.6`；进入存活守夜烛台的照明范围后获得“烛火庇护”并恢复为 `1.0`，玩家进出范围会收到明确提示。夜晚继续先乘全局 `0.5`，因此范围外最终为 `0.3`、范围内以雾潮 `0.9` 修正得到 `0.45`；祭品、高地等原有单位视野倍率继续乘算，建筑视野和攻击距离不受该单位惩罚。
- 雾潮中的僵尸按配置 `family:"僵尸"` 统一获得移动速度 `×1.2`、攻击/技能触发间隔 `×0.85`；通用普攻、自管技能冷却、接触/活体常驻伤害与召唤攻击计时均接入同一运行时时钟，不改写基础属性，也不改变攻击距离、动作动画、前摇、投射物飞行或技能位移，结束雾潮或离开scene11后即时恢复。
- 复用现有三烛台素材新增可建造特殊建筑“守夜烛台”：仅scene11允许建造，完成“地牢探险团”科技后解锁，消耗500能源，拥有1200生命、碰撞实体且可被摧毁/出售。世界-125环境散布已移除36座不可摧毁的装饰烛台，并排除含烛台的预制组合，保证场景中每座烛台都是同一套可建造实体、费用、科技、生命和庇护语义；新建筑接入既有烛光、火星、战争迷雾、位面快照与销毁生命周期。
- 位面科技树在“地牢探险团”正下方新增“烛火工事学”，并补充同列纵向科技连线；研究后开放每座烛台独立的“增幅灯芯”升级，基础照明/庇护半径260px、每级+50px、最高5级，费用与读条由配置驱动，离场快照和后台位面结算可续跑。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试科技解锁、scene11建造门禁、范围边界、昼夜视野乘算、烛台受击销毁、升级恢复及僵尸节奏。

### 对话：地牢探险接入全局时间与世界观察（2026-08-22）
- 修订 8 月 21 日的地牢冻结规则：统一昼夜时钟、五日入侵与特殊天气排期在 `scene7` 中继续流动；后台位面 1Hz 结算恢复资源、生产、探险奖励、兵种升级和科技进度，地牢地图/事件界面的主循环早退前也会推进全局生产与兵线。
- 世界按钮在地牢中恢复观察切换：首次切出会保留当前地牢的路线/战斗实体、特效、墙体、地形、相机和交互界面；世界列表增加当前地牢行，可从其他位面“返回本体”恢复同一探险现场。观察期间可选择单位并下达指令，但玩家本体仍留在地牢，入侵“本体支援”继续禁用。
- 入侵到达五日阈值后不再等待离开地牢才开始，活动入侵也会继续后台推进；地牢成功、失败、安全撤离或主动放弃仍复用原 `_recordRunResult`，F→A 难度追加的入侵进度比例和每局一次登记契约未改。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试。

### 对话：丛林祭司四动作精灵图导入（2026-08-22）
- 从用户提供的单段 720×720、24 FPS 视频中分离丛林祭司四个动作：待机 32 帧、奔跑 30 帧、施法 17 帧、死亡 16 帧；循环段剔除重复端点，一次性动作按视觉变化重采样，四套透明 RGBA 精灵表已导入 `assets/companions/jungle_priest/`。
- BiRefNet 抠图统一角色缩放、水平锚点和 480/512 脚底基线；施法表使用 640×512 帧格保留法杖外扩光芒，其余使用 512×512。配置、BootScene 与实体动画 ID 已切到 `jungle_priest`，不再继承仓鼠牧师占位贴图。
- 丛林祭司法术改为完整 17 帧单次施法锁：第 8 帧（约 583ms）才结算闪电/冰锥/火球；起手与释放帧统一复验目标存活、`hittable`、敌对阵营、能源节点、有效魔法射程与视线，隔墙目标不再空放并消耗冷却，墙顶射程加成与弹体一致，冰锥/火球也能正确命中全场敌对的入侵特工。原生命、六维、伤害、移速、2800ms 攻击间隔、三法术轮换及丛林神庙生产/升级/后台模拟协议保持不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试四动作、奔跑循环、施法命中时点、死亡清理、贴地尺寸和 HUD 高度。

### 对话：建筑选择预览卡顿全链优化（2026-08-22）
- 建筑接地拟合新增离线派生 manifest：20 组普通建筑按 texture/frame、源尺寸、显示尺寸和 2×2 nominal footprint 精确索引；幽灵、实体和结构阴影继续共用 `resolveStructureGroundFit()`，已知图不再在首次点选时扫描 alpha。缺项兼容路径由 Phaser 逐像素 `getPixelAlpha` 改成一次画布批量读回并限制为 768px 长边采样。
- 建筑面板生成并改用 22 张 128×64 透明专用缩略图，DOM 图片延迟加载、异步解码；资源、跨位面倍率或建造条件变化改为原位更新卡片黑化、标题和价格，不再重建整片网格。科技折叠和排序变化仍保留权威重建。
- 建造预览移动合并为每渲染帧最多计算一次；按下鼠标会以点击坐标同步复验，松开拖墙/道路会以最终坐标重算，正式 `_place()` 的占用、敌军警戒、科技、场景条件和支付校验均未减少。普通幽灵、对齐线和12格外围道路在同一场景内隐藏复用，关面板/切场时销毁并取消旧帧回调；4格门和楼梯保持原专用组件预览。
- 新增 `tools/generate-building-preview-assets.mjs` 作为建筑原图或显示尺寸变更后的统一再生入口。未下采样或替换世界内建筑原图，世界显示尺寸、逻辑占格、碰撞和建造效果不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试首次点选大型建筑、快速移动后立即放置、拖墙松手终点、资源黑化恢复和连续切换建筑。

### 对话：生产建筑亮窗昼夜闪烁（2026-08-22）
- 为草屋、铁匠铺、教堂、研究院、骑兵学校、三级房屋、麦田风车、银行、市场、探险家营地和经济工坊生成 13 张与正式主体同画布的透明亮窗蒙版；只保留窗口、灯具和发光开口，银行维持蓝色冷光，其余建筑维持原有暖色，不修改主体贴图、占格或碰撞。
- `producer-buildings.json#windowGlow` 成为资源和参数入口，支持按当前主体纹理选择 `variants`，因此房屋升级后自动切换对应蒙版；BootScene 按配置统一预载，不在场景代码硬编码建筑清单。
- 中立建筑渲染链新增 ADD 亮窗层：精确跟随主体位置、尺寸、旋转、镜像和结构深度，并纳入地图模式、战争迷雾与销毁生命周期；白天弱光、夜间增强，各建筑按稳定 ID 错开相位，以低幅平滑脉冲和双频微闪避免同步跳闪。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试昼夜强度、房屋升级换图、风车层级及银行蓝窗观感。

### 对话：传送门取消科技门禁并重排研发线路（2026-08-22）
- 移除工程分支“位面工程”科技及其 `building:portal` 解锁项；位面满足既有地牢打通条件后即可在传送门面板首次免费构造，不再检查研发状态，未打通位面仍由可构造列表和最终构造入口双重阻断。
- 工程线收紧为“工程制图 → 城防工事 → 防御塔工程”，防御塔工程左移一列；远征后勤移除位面工程前置并继续承接集结网络，位面祭祀改接市场流通，位面物流协议继续承接经济工程，清除全部悬空与跨分支连线。

### 对话：交互开发面板位面页恢复纵向滚动（2026-08-22）
- 修复位面页下方世界行与天气按钮被裁切：后置通用样式曾把页签内容从 `overflow:auto` 覆盖为 `overflow:hidden`，现以位面页专属选择器恢复纵向滚动，并让长列表从顶部开始自然扩展。
- 开发面板增加视口高度上限，窗口高度不足720px时也会缩入可视区；头部和页签栏保持固定，只滚动位面内容。静态复核确认世界-125“触发死寂雾潮”按钮仍会生成、插入世界行并进入既有事件分流。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试滚动和按钮点击。

### 对话：侦查营地关闭外围道路（2026-08-22）
- `explorer_camp`（项目内显示名“探险家营地”）新增 `perimeterTile:"none"`，建造预览、正式放置、世界特色建筑生成及快照恢复均不再为其产生外围道路环。
- 营地仍保留中央标准 2×2 占格，建筑碰撞、出兵、数量限制和其他建筑的外围道路规则不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试新建及位面恢复后的营地周边。

### 对话：主神空间 BGM（2026-08-22）
- 导入用户提供的 `罗马庭院.wav`，配置为主神空间（`main`）循环背景音乐；沿用现有音乐声道音量与 1.5 秒切场淡入淡出，其他场景音乐映射不变。
- 新游戏启动直接进入主神空间、不经过统一切场函数，因此在确定初始 `main` 场景后显式启动对应 BGM；从其他场景返回主神空间时仍由原切场链自动恢复。未运行测试、构建、音频播放或游戏运行时验证，按项目约定由用户测试首次进入与返回主神空间时的播放。

### 对话：位面开发面板触发世界-125死寂雾潮（2026-08-22）
- T键交互开发面板“位面”页在scene11世界行新增“触发死寂雾潮”按钮和待命/进行中状态；只允许当前已进入世界-125时触发，激活后同一按钮切换为“结束死寂雾潮”，点击结果使用既有开发Toast反馈并立即重绘状态行。
- `GameScene` 新增世界-125氛围调试门面，UI不直接访问私有环境实例；`World125AtmosphereSystem` 以基础配置合并 `fogTide.visual` 后重建共享粒子池，雾潮中尸雾更密、更大、更持久，腐化尘加速增量，冷绿覆盖加深、烛光与火星衰弱，并锁定全场唯一气流方向。
- 死寂雾潮当前是scene11内的手动纯视觉调试态：离开场景或清理场景即结束，不随机触发、不计游戏日、不写存档、不修改视野、AI或战斗。双份scene11配置中的雾潮视觉参数保持一致。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试按钮切换、效果强度和离场清理。

### 对话：建筑面板移除传送门并黑化资源不足建筑（2026-08-22）
- `producer-buildings.json#portal` 新增 `playerBuildable:false`，世界核心传送门继续由位面生命周期自动生成、恢复与重建，但不再进入玩家建筑面板或建造选择链。
- 建筑卡新增金币/能源可支付检查：资源不足时复用既有建造条件未满足的低透明灰黑样式和原因提示，并禁止选中；跨位面附加造价、首栋免费建筑及无限资源调试状态继续读取各自权威真源。
- 面板每 500ms 只在造价或“可支付/不可支付”状态发生变化时重绘网格，采集资源但尚未跨过价格门槛时不反复重建卡片。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试传送门缺席、金币/能源不足黑化及资源补足后的恢复。

### 对话：麦田风车采用新主体并补帧优化旋转（2026-08-22）
- 将用户提供的 1124×1399 风车图作为新的静态主体，仅移除图片自带的边缘连通棋盘背景并等比缩放到 512×640；未重绘、修补或重新合成建筑主体细节。
- 根据用户反馈恢复原扇叶实际显示大小：扇叶从 1024 源帧按 0.75 重排进 768×768 帧，再以 428×428 显示，最终视觉比例严格保持原来的 `428 / 1024`；主体继续使用 276×340，不再为对轴同步缩放两层。
- 根据连续实机反馈分两次向左上校准轴承点，最终主体运行画布轴心由初版 `(366, 275)` 调整为 `(349, 268)`；将扇叶源轴心重排到自身帧中心 `(384, 384)` 后，overlay 精确偏移为 `(+50.1328125, -27.625)`。本轮相对上一版再左移约 `4.31px`、上移约 `3.72px`，轴帽前景遮挡随同重新生成，未单独漂移。
- 轴心确认后另行校准整栋贴图与标准 2×2 footprint：新增 `anchorAdjustX:-5 / anchorAdjustY:5`，让主体、扇叶与轴帽整体向左上各移动 5px；只改变视觉锚点，逻辑占格、碰撞、田地和实体世界坐标不变。
- 在原 16 个 3D 关键帧之间以双向半角变换和预乘 alpha 混合补到 32 帧，帧率由 8.7 提升至 17.4 FPS，完整循环时长保持不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试轴心、遮挡、扇叶原尺寸、贴地位置和循环流畅度。

### 对话：世界-125“尸雾遗迹”常驻环境特效（2026-08-22）
- scene11 新增纯视觉 `World125AtmosphereSystem`：镜头周围持续生成贴地冷色尸雾与前景灰绿腐化浮尘；两层在同一时段共享唯一等距气流方向，保持18~32秒后整体换向并清空旧方向粒子，使用独立硬上限且不进入实体、物理、AI或存档。
- 增加低于战争迷雾的冷色呼吸覆盖层；地面尸雾位于 `GROUND_WEATHER`，前景尘与火星位于99974，全部由99980战争迷雾最终遮挡，离开scene11、进入加载或清理场景时统一销毁。
- 视检确认现有317×640三烛台素材的俯视烛盘、底脚和约89×180运行时尺寸适合黑砖等距地牢，因此未重新建模生图。36座单体及预制组合烛台按三处真实火焰源图坐标注册FOW感知呼吸光，并共用一个镜头内火星发射器，避免逐烛台创建粒子系统。
- 双份 `game-config.json` 新增 `scene11.environmentEffects.dungeonAtmosphere` 配置真源；常驻氛围不修改单位视野、战斗、探索记录或后台模拟。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试可见度、层级、气流观感与性能。

### 对话：右上基础资源图标替换（2026-08-22）
- 将右上基础资源栏原金币、能源、食物 emoji 替换为用户确认的透明 PNG：磨损金币堆、蓝色能源晶簇和无麦穗长面包；三张正式资产统一归档到 `assets/ui/resource-icons/`。
- 资源栏图标改用不可拖拽的 DOM 图片并保持原标签、数值来源和增量刷新逻辑不变；冷钢主题统一以 28×28 `object-fit:contain` 显示，避免裁切透明边缘。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试实际清晰度与布局。

### 对话：麦田风车改为主体与扇叶分层动画（2026-08-22）
- 移除“整栋风车精灵表替换主体”的渲染方式，改为静态无叶片主体 `wheat_windmill_body.png` 与16帧透明扇叶 `wheat_windmill_rotor.png` 两个独立 Sprite；原完整风车图仅保留为详情缩略图。
- 主体与扇叶均使用1024×1024同源画布、428×428显示规格、同一原点和同一世界坐标，扇叶转轴直接按源画布重合，不再使用此前动画主体的 X+19/Y−9 或候选合成阶段的人工偏移。
- 中立建筑渲染链新增配置化 overlay 动画生命周期，同步主体位置、镜像、深度、地图模式、战争迷雾和销毁；标准2×2占格、碰撞、田地、经济生产与面板缩略图保持不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试轴心、层级、贴地尺寸和循环效果。

### 对话：美洲豹战士待机左右微动（2026-08-22）
- 美洲豹战士待机状态新增水平正弦微动：左右最大各 2px、完整周期 2 秒；多实例按单位 ID 错开相位，避免同步摆动。
- 微动只加到 Phaser 精灵渲染坐标，不修改实体世界坐标、碰撞、脚底锚点、寻路或动画帧。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试实际幅度与观感。

### 对话：世界-122 沙尘暴锁向与速度翻倍（2026-08-22）
- 沙尘暴暴风档位新增 `lockDirection`：从爆发到消散期间，贴地风线与前景沙雾始终保持同一个固定方向，不再执行普通扬沙的周期随机换向；普通天气换向逻辑保持不变。
- 暴风贴地层速度由 `320~520` 提升至 `640~1040 px/s`，前景层由 `190~320` 提升至 `380~640 px/s`，两层均精确增加 100%；同时小幅提高发射密度、粒子上限、尺寸、透明度和相机外扩范围。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试实际风压感与性能。

### 对话：美洲豹战士四动作精灵图导入（2026-08-22）
- 从用户提供的两段 720×720、24 FPS 视频中截取并抠出美洲豹战士：`1.mp4` 的静止段生成 24 帧待机，3~5 秒筛出 20 帧无重复端点奔跑循环；`2.mp4` 的 0~2 秒重采样为 18 帧攻击循环，3~5 秒生成 24 帧单次死亡动画。
- 四套素材使用 BiRefNet 保留白色口鼻与毛发边缘，统一透明背景、角色缩放和脚底基线；以独立 RGBA 精灵表导入 `assets/companions/jaguar_warrior/`，攻击为 640×512 单帧，其余为 512×512 单帧。
- `jaguar-warrior-config.json` 新增独立显示尺寸、脚底/HUD 偏移及 idle/walk/attack/dying 动画配置；BootScene 纳入资源预载与动画注册，美洲豹实体改用 `jaguar_warrior` 动画键，并按死亡动画真实帧数与帧率延后清理。
- 原生命、攻击、移速、攻击间隔、致残及丛林神庙生产链数值保持不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试四动作、奔跑循环接缝、攻击节奏、死亡清理、贴地比例和 HUD 高度。

### 对话：全局按钮点击音效（2026-08-22）
- 导入用户提供的 `点击音效.mp3`，以稳定路径 `assets/sounds/ui/button_click.mp3` 入库，并通过 `audio-config.json#uiCues.buttonClick` 接入 UI 声道。
- `SoundManager` 初始化时统一监听原生按钮、按钮型输入框和 `role=button`；动态面板无需逐个绑定，禁用按钮、惰性区域、地图点击和拖拽格子不播放。右侧栏目原有开关音效继续独立保留。未运行测试、构建、音频播放或游戏运行时验证，按项目约定由用户测试。

### 对话：位面科技改为全卡黑白电视噪点（2026-08-22）
- 移除未解锁位面科技卡片原有的双尺度马赛克、错位跳动和低频扫描线，仅保留高对比黑白电视雪花噪点。
- 噪点层改为不透明覆盖整张研究卡牌并缩小纹理平铺尺寸，底层科技内容只保留极弱轮廓；“位面未知”状态文字、面板关闭暂停和减少动态效果回退继续保留。

### 对话：世界-122 沙尘暴特殊天气（2026-08-22）
- 新增仅属于荒漠位面 scene8 的沙尘暴随机事件：默认每次平静 3~6 游戏日后排期，提前 6 游戏小时在顶部提示栏预警，爆发后随机持续 1~2 游戏日；预警、开始、结束和下一次排期均跟随统一游戏时钟，并随主存档恢复。
- 沙尘暴激活时切换到暴风粒子档位：提高风速、发射密度和阵风强度，扩大相机外扩发射面积、粒子尺寸与硬上限，并延长粒子寿命；贴地风线与前景沙雾仍保持同一全局风向，模式开始/结束时才重建发射器。
- 玩家与友军单位的战争迷雾最终视野半径新增 scene8 沙尘暴 `×0.5`，与夜晚、祭品和高地倍率继续乘算，因此夜晚沙尘暴为基础视野的 `0.25`；传送门、塔和出兵建筑视野不受影响，也不修改 AI 感知、攻击距离或永久探索记录。
- T 键交互开发面板的“位面”页在世界-122行新增“触发沙尘暴”按钮，并显示平静、预警或进行中状态及剩余游戏日。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试随机预警、1~2日持续、昼夜叠加视野与暴风表现。

### 对话：位面科技电视雪花噪点（2026-08-22）
- 新增 96×96 可平铺 SVG `fractalNoise` 冷钢噪声纹理，叠加到现有双尺度马赛克之上；纹理以 0.24 秒错帧位移模拟电视雪花，相邻卡片继续采用不同相位。
- 噪点只改变纹理采样位置，不做整卡高频亮度闪烁；原低频扫描、面板关闭暂停和“减少动态效果”静态回退保持有效。未运行游戏或运行时视觉验证，按项目约定由用户测试实际观感。

### 对话：仓鼠赏金猎人专属开火音效（2026-08-22）
- 导入用户提供的赏金猎人 `1.mp3`，以稳定路径 `assets/sounds/friendly/hamster_bounty_hunter_attack.mp3` 入库，并替换赏金猎人原先复用的仓鼠火枪声音。
- 沿用既有火枪投射物发射链：攻击动画第 9 帧真正创建投射物后播放一次专属音效，并按赏金猎人世界坐标进行距离衰减；攻击起手、命中和空弹均不额外播放。未运行测试、构建、音频播放或游戏运行时验证，按项目约定由用户测试。

### 对话：矿工营地升级项目名称调整（2026-08-22）
- 矿工营地专属升级项目“攻击加速”“攻击强化”“仓鼠增援”分别改名为“工作加速”“采矿强化”“矿工增援”；其他建筑的同类攻击升级名称不变。
- 本次仅调整显示名称，升级效果键、数值、等级上限、费用和存档项目 ID 均保持不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试矿工营地升级面板显示。

### 对话：探险家与赏金猎人图标导入（2026-08-22）
- 导入用户确认的仓鼠探险家“伸缩望远镜＋T柄铲子”图标，以及复用现有冷钢底图、以结构完整六发左轮为主体的仓鼠赏金猎人图标。
- 统一图标映射新增 `explorer` 与 `bounty_hunter`；探险家营地的兵种名称、当前出兵信息，以及 RTS 指挥模式的单选详情和多选数量列表自动复用既有图标渲染链路，无需改动出兵配置或面板布局。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试探险家营地与 RTS 单选/混合框选显示。

### 对话：世界-122 扬沙全局风向统一与周期换向（2026-08-22）
- 贴地风线与前景沙雾改为共享同一个全局风向；单粒子只随机沿风速度，不再分别叠加横风偏移或角度抖动，同一风段内所有扬沙严格同向。
- 初始方向继续读取等距地面局部轴 `windUv`；每段默认保持 12~20 秒，再随机选择一个在屏幕投影后与上一方向至少相差 60° 的新方向。换向时先清空旧方向存活粒子，再同步更新两个发射器，避免画面短暂出现两股相反风向。
- 风向计时继续消费统一世界 `delta`，暂停、加载和离场期间不推进；双份 `game-config.json` 已同步新增可调的保持时长与最小换向角。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户刷新并重新进入世界-122测试。

### 对话：位面僵尸地牢世界 BGM（2026-08-22）
- 导入用户提供的 `幽洞回声.wav`，并将其配置为位面僵尸地牢世界（`scene11`）的循环背景音乐；沿用现有音乐声道音量与 1.5 秒切场淡入淡出。
- 普通僵尸地牢（`scene7`）继续使用原 `dungeon_echo.mp3`，其他场景音乐映射不变。未运行测试、构建、音频播放或游戏运行时验证，按项目约定由用户测试进入/离开位面僵尸地牢世界时的播放与切换。

### 对话：世界-122 扬沙与地面区分度校准（2026-08-22）
- 对照实际 `floor_sand_seamless` 与 `floor_mud_seamless` 后确认，上一版扬沙 tint 与米黄/土黄地面处于同一色相和明度区间，叠加生命周期淡出、环境 alpha 和 scene8 的 0.7 缩放后缺少可辨轮廓。
- 贴地风线改为浅奶金、饱和橙沙、深赭阴影三档随机色，alpha 由 `0.36→0` 提高为 `0.72→0.08`，宽度约翻倍；前景沙雾提高到 `0.34→0.05`、尺寸扩大并将中档上限由 32 调整为 40。夜间环境倍率下限由 0.55 提高到 0.7，保持沙尘色同时建立明暗反差。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户完整刷新并重新进入世界-122复测。

### 对话：位面独特科技马赛克动画优化（2026-08-22）
- 未解锁位面科技的遮蔽层改为 14px/30px 双尺度冷钢马赛克，使用 1.2 秒六段错位跳动；相邻卡片采用不同负延迟，不再同步移动或形成单一重复棋盘。
- 新增 2.4 秒低频青灰扫描干扰，底层科技内容保持模糊和低透明；马赛克主体以 `transform/opacity` 动画为主，避免高频全卡闪烁。
- 科技面板关闭时暂停遮蔽动画；系统启用“减少动态效果”时自动退化为静态马赛克。未运行游戏或运行时视觉验证，按项目约定由用户测试。

### 对话：各位面保底水晶距离调整（2026-08-22）
- 各持久位面新生成资源布局时，额外三矿保底簇改为从传送门 `1200px` 半径的随机方向选点；保底数量、簇内相邻扩张和位面世代确定性随机保持不变。
- 5 个远距主矿簇继续要求中心及实际矿格距离传送门至少 `3000px`，本次不改变主矿数量或安全距离。已有完整位面快照保持原位置，位面首次物化或世代重建后采用新距离。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试各位面新世代的保底矿位置。

### 对话：仓鼠正规兵种图标接入出兵与指挥面板（2026-08-22）
- 导入用户确认的仓鼠民兵、战士、射手、盾卫、斥候、火枪、牧师、骑士、轻骑 9 张统一风格图标，并以独立兵种映射作为 UI 单一入口；未配置图标的特色单位和敌军继续使用中性符号兜底。
- 仓鼠兵营及配置型出兵建筑把图标放到当前兵种和兵种选择名称之前；RTS 指挥模式右上角单单位详情新增身份图标，出兵建筑详情新增当前兵种图标，多选详情改为图标、名称、数量分行展示，并将墙顶/楼梯层级信息独立为辅助区。
- 沿用冷钢主题变量重排标题、数字徽标、状态条和多选卡片，面板宽度由 252px 调整为 284px，并补充窄屏回退。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试各出兵建筑、单选/框选、混合兵种和高层单位布局。

### 对话：世界-122 扬沙完全不可见修复（2026-08-22）
- 修复环境配置被启动期 `SceneManager.scenes` 缓存遮住的问题：`GameScene` 改为优先读取 `GAME_CONFIG` 真源，开发期新增 `environmentEffects` 后不会继续按旧对象判定为未启用。
- 移除不适合环境沙雾的 `smoke_particle`（实际内容仅为 18×4 棕色细条）和通用 `impact_dot` 复用；扬沙系统改为运行时生成专用白色软风线与椭圆沙雾纹理，再统一 tint 为沙色。前景深度调整为无冲突的 99975，仍低于战争迷雾和昼夜覆盖。
- 默认中档提高到贴地/前景最多 64/32 个存活粒子，缩短发射间隔、提高尺寸和透明度；删除发射前随机命中迷雾 `VISIBLE` 单元的重复过滤，避免基地遮挡、夜间或观察镜头下两个 emitter 长期保持 0 粒子，最终不可见区域继续由上层迷雾遮罩负责。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户复测。

### 对话：阴影系统算法审计整改（2026-08-22）
- 共享结构阴影层新增逐实体雾区状态与可见性 revision：绘制前读取战争迷雾真源，隐藏单一建筑只过滤对应 job 并强制重画，不再误把注册占位 Graphics 当成真实阴影，也不会隐藏整层其他建筑。
- 树木、桶状仙人掌等接触型静态影由“旋转拉长椭圆”改为水平 2:1 footprint 沿归一化太阳方向的凸扫掠；静态中心 offset 与 length 统一为同一屏幕长度口径，不再随等距方位额外缩短或把底座转成竖椭圆。
- 新增 `_getUnitRenderFootprint` 统一玩家 z、友军视觉插值脚点与其他单位 Collider 中心；单位阴影和“范围”红圈共用该入口，范围模式补画队伍成员与世界友军。
- 共享层支持互不相交阴影各自的 `shadow.opacity`，阴影聚簇补齐共线相贴判定；静态几何回归契约同步更新夜间 40%、地牢 55%、动态 0.30078125、静态 0.1925 与水平胶囊扫掠规则。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试雾区边界、树木/仙人掌三时相、墙顶玩家、友军移动及相邻建筑阴影。

### 对话：麦田风车循环精灵图导入（2026-08-22）
- 导入用户确认通过的 4×4、16 帧透明风车旋转精灵图；单帧 512×640，以 8.7 FPS 无限循环，在场景中替换麦田风车主体的静态显示。
- 动画资源、切帧和播放参数收口到 `producer-buildings.json`；原 `wheat_windmill.png` 继续用于建筑详情缩略图与既有光照配置，避免面板显示整张精灵图。
- 根据用户实机反馈增加动画专属显示规格：按原图与 16 帧 alpha 有效面积将显示框校准为 330×407，并以动画平均视觉重心补偿 X+19/Y−9；静态建造预览、标准 2×2 占格和碰撞保持不变。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试循环衔接、贴地比例、阴影与多风车同时播放效果。

### 对话：右上角基础资源栏（2026-08-22）
- 在右上角时间控件左侧新增金币、能源、食物三项基础资源栏；金币汇总背包内全部金币堆，能源与食物汇总当前位面全部活动仓库。
- 资源栏与时间控件改为同一弹性 HUD 容器，与顶部状态栏共享 12px 顶部基线；资源栏固定为与顶部状态栏一致的 50px 高度，不跟随时间栏拉伸，日期文本宽度变化时仍保持稳定相邻。
- 复用现有实时 HUD 刷新入口，资源数值发生变化时才更新对应 DOM，并为发生变化的数字单次播放缩放闪动与金色辉光；首次初始化不闪动，减少动态效果偏好下自动停用动画。沿用冷钢 HUD 外壳、数字字体和资源语义色。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试。

### 对话：世界-122 风吹扬沙环境效果首版（2026-08-22）
- 新增 scene8 专属 `WindblownSandSystem`：使用 Phaser 4 两个常驻粒子发射器分别绘制贴地风线与前景薄沙雾，复用运行时白色粒子纹理并以沙色 tint 着色，不新增图片资产、不重烘焙分块地板。
- 扬沙参数由双份 `game-config.json` 的 `environmentEffects.windblownSand` 统一控制，包含低/中/高质量倍率、等距地面风向、阵风周期/强度、发射频率、存活上限、寿命、速度、尺寸、透明度和颜色；默认启用中档。
- 贴地层登记为固定 `GROUND_WEATHER=-994.3`，前景层固定在 99970（战争迷雾 99980 与昼夜覆盖 99990 之下）；只在当前相机视口外扩 160px 且位于世界菱形/已探索视野内发射，两个池分别预分配并限制最多 48/24 个存活粒子。
- 阵风只消费统一 `worldDelta`；暂停、加载和地牢隔离不会推进或继续生成，离开场景时显式销毁。该效果不创建实体、不参与物理/AI/伤害/存档/后台结算。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试视觉密度、昼夜、暂停与切场行为。

### 对话：位面祭坛、仓鼠军营门禁与位面独特科技栏目（2026-08-22）
- 军事指挥线新增“军营建制”（研究量 90），接在军事组织之后并解锁仓鼠军营；盾阵学顺延为军营建制的后续科技。
- 经济与位面线新增“位面祭祀”（研究量 180），以位面工程为前置并解锁世界-122的位面祭坛；两类建筑均复用建筑栏折叠隐藏、选中校验和落地二次校验。
- 科技面板顶部新增“位面独特科技”栏目；地牢探险团和丛林神庙仪式从经济页移入该栏目。未解锁对应位面时项目仍保留原位置并使用动态马赛克遮蔽卡片与详情，解锁后原卡片直接解除遮蔽并允许研究。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试两个建筑门禁、科技线路与新栏目显示。

### 对话：基础建筑费用调整（2026-08-22）
- 矿工营地由 1000 调整为 500 能源，仓鼠草屋由 250 调整为 500 能源。
- 方块墙改为每块 50 能源，继续使用 C 级防御数值；4 格门造价保持不变。
- 当前位面没有活动房屋时，首栋房屋消耗 0 金币，后续恢复原价 300 金币；建筑记录实际支付成本，免费房屋回收不返金。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试。

### 对话：全单位阴影匹配 Collider footprint（2026-08-22）
- 新增单位地面 footprint 统一入口 `resolveUnitGroundFootprint`；玩家、敌人/入侵特工、队伍成员、世界友军、NPC 与训练靶的接触阴影全部严格读取 `Collider.radius`，以水平 2:1 椭圆绘制，尺寸不再各自推导。
- 修复 `collisionShape:'rect'` 单位误用躯干受击矩形绘制阴影的问题：玩家和矩形配置 NPC 不再按 `collisionWidth/Height` 拉成长影；NPC 的 `colliderOffsetX/Y` 会通过 Collider 圆心同步作用于阴影。友军仍允许以视觉插值脚点覆盖中心，但半径只能来自 Collider。
- NPC/训练靶阴影深度由旧 `e.y+9` 改为仲裁后 Sprite depth−0.1；建筑继续使用结构太阳投影并跳过单位接触椭圆，避免双重阴影。“范围”按钮的红色 footprint 同步复用新入口，形成可直接对照的唯一真源。新增纯几何回归脚本但未运行；按项目约定未运行测试、构建、浏览器探针或游戏运行时验证，由用户测试。

### 对话：仓库/商店面板图层被遮罩覆盖修复（2026-08-22）
- 实机探针（elementFromPoint 多点采样）定位：`#panelOverlay` 被挂载进 right-sidebar 层（z=20000），active 时盖住仓库/商店等 body 级 4000 面板并吞掉全部点击；回退挂载到 uiLayer（z=10），恢复"遮罩低于所有面板"的原设计。
- 附带：NPC 立绘 z-index 5001→3999（低于业务面板），默认 pointer-events:none，调整立绘工具激活时才恢复可拖拽。
- 浮窗 ttIcon 此前从未被 renderTooltip 赋值，已修复为显示物品贴图（44px，emoji 兜底）。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。


### 对话：仓鼠民兵尺寸与右栏/火球命中音效（2026-08-22）
- 仓鼠民兵 `displaySize` 由 300 调整为与当前仓鼠斥候一致的 460；按民兵帧内真实脚底比例同步调整贴地、深度脚线和头顶 HUD 偏移，碰撞半径、攻击范围、移速与战斗数值不变。
- 导入用户提供的“栏目音效”为稳定 UI 音效；统一右侧栏目挂载层中的普通面板和从属模态在开启、关闭时播放同一声音，遮罩层不监听，避免一次操作双响。
- 导入用户提供的“火球击中”音效并替换双份技能配置；直接命中、撞墙爆炸和达到最大射程的空爆均播放，同一颗火球只响一次。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试民兵四状态贴地/HUD、各类右栏开关单次音效，以及火球直接命中、撞墙与最大射程空爆音效。

### 对话：防御塔纳入高级工程科技（2026-08-22）
- 工程科技线在“位面工程”之后新增“防御塔工程”（研究量 240），完成后才在建筑栏显示并允许建造防御塔。
- 防御塔建造能源由 300 调整为 1000；同步更新防御塔配置中的原价/出售返还兜底基数，新建防御塔继续按自身实际建造成本结算返还。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试科技门禁、建筑排序、1000 能源扣除及出售返还。

### 对话：科技树三线重排与经济体系门禁（2026-08-22）
- 科技面板重排为“工程 / 军事指挥 / 经济与位面”三分页：军事与指挥合并成一条发展线，经济建筑新增聚落规划、农业分工、市场流通、信用金融、经济工程和位面物流协议等节点；跨分支前置继续在详情中明确展示。
- 仓库和一级房屋保持基础可用，不再要求科技；“聚落规划”解锁二级房屋，“住房优化”解锁三级房屋，房屋升级按钮隐藏时保留原槽位，升级业务层同步执行科技校验。
- 探险家营地与丛林神庙改为“位面专项研究”，不再依赖主科技线；对应位面解锁后才在经济与位面页出现并进入可研究池，完成后同时解锁特色建筑和其全部特色兵种。
- 仓库“跨位面协议”升级和特色建筑双通道募兵补齐底层科技校验；科技存档协议升级为 v3，旧科技存档自动完成本次新增经济节点，避免已有经济功能被版本升级重新锁定。
- 科技界面沿用冷钢主题，新增三分页、动态画布和位面专项分区；未调整兵线、右侧栏目或其他既有界面的位置。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试。

### 对话：单位指令入口、城门感应与背包消耗品拖拽优化（2026-08-22）
- 中键指令轮盘改为按选中目标能力生成：仅所选单位中包含仓鼠探险家时显示“探险”，其他队友和仓鼠单位不再出现无效探险按钮。
- 左下单位指令框除 RTS 指挥选择外，也响应普通模式组队栏中的正式玩家队友选择；移动攻击与巡逻继续复用统一战术命令，并在普通模式地图选点时消费同次鼠标输入，避免被旧左键纯移动覆盖。
- 基地门和玩家建造城门的自动感应统一排除 `wall_walk` 单位，墙顶单位的二维投影即使靠近门线也不会开门；地面及合法楼梯接近规则不变。
- 拖动背包消耗品时始终保持背包可见，允许在格子间调整位置；指针第一次越过拖拽开始时的背包范围后，只临时把底部快捷栏提升到背包图层之上，拖拽结束即恢复原层级。投放到背包和快捷栏之外仍按原规则丢弃，仓库双面板拖放保持原行为。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试普通/RTS选中切换、轮盘按钮过滤、墙顶与地面单位开门、背包内换位、越界投放快捷栏及越界投放其他区域丢弃。

### 对话：位面能源矿 5+1 世代随机簇（2026-08-22）
- 覆盖固定四簇矿物生成：每个位面新世代改为按独立资源随机流生成 5 个密集主矿簇，每簇独立随机 `10~12` 个能源矿；所有主簇中心和实际矿格均禁止落在距本位面核心传送门小于 `3000px` 的范围内。
- 额外在传送门半径 `3000px` 的随机方向生成 1 个三矿保底簇；所有簇继续使用 1×1 等距格四邻扩张，必须一次放满配置数量才接纳，并同时满足菱形边界、跨簇不重格和真实墙体/环境障碍检查。
- 资源布局版本升级为 `energy_clusters_v2 / generationVersion 2`，同一位面世代使用确定性种子，重复进入不会重随机；传送门摧毁后重建、世代号递增时才生成新布局。`data/` 与 `public/data/` 的世界配置保持同步。
- 完整旧快照首次载入时将剩余矿量与枯竭状态迁移到新版随机布局，并按旧快照剩余矿数裁掉多余新节点，避免借升级刷新已采矿物；已采空的旧位面仍保持空矿。仅传送门基础快照的 `nodes:[]` 改为“尚未首次物化”，不再误清首次生成矿簇。
- 仙人掌不再依赖已删除的固定矿簇坐标预留空圈；能源矿在环境障碍之后生成，通过真实 `WallSystem.canMoveTo` 选择合法格。同步调整资源快照静态契约与 scene8 能源簇探针，但未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试。

### 对话：移动速度状态浮窗格式修复（2026-08-22）
- “移动速度”和“详细移动速度”浮窗不再把 `toFixed()` 生成的字符串二次传入仅接受数字的公共格式器；两个分支先局部归一化速度，再以数字渲染帧速度和取整后的秒速度。
- 公共 `fmt()`、角色实际移动速度、战斗数值与存档协议均未修改。未运行测试、构建或游戏运行时验证，按项目约定由用户测试两个移动速度浮窗。

### 对话：建筑阴影统一平行投射方向（2026-08-22）
- 排查确认太阳投影向量本身全局一致；左右侧边方向不一的根因是普通建筑默认启用了下/中/上三层 alpha 低模，左右包络可能分别被不同宽度和高度的屋檐、塔楼或侧翼接管，连接后自然形成两条不同斜率的边。
- 普通建筑默认算法改为“真实 alpha 接地轮廓的单体正交扫掠”：所有离地延长点只沿同一个太阳方向向量平移，左右终端边由同一轮廓的两端生成，严格保持平行；真实底边和非矩形接地区仍保留，不回退到 placement footprint。
- 自动 alpha 分层改为显式 `shadowCaster.autoParts:true` 才启用，供少数经过逐建筑验收的复杂轮廓使用；显式 `shadowCaster.parts[]` 协议保持不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试日出/正午/日落三个方向及左右镜像建筑。

### 对话：仓鼠赏金猎人正式精灵、枪口弹道与双倍悬赏金币（2026-08-22）
- 将赏金猎人从复用火枪素材的占位单位替换为独立 8列×4行精灵：idle 8 帧、running 13 帧、attacking 13 帧、dying 11 帧；BootScene 独立预载和注册动画，死亡移除时长跟随 11 帧动画。
- 基础生命调整为 150，六维为力量5/敏捷25/智力3/体质12/精神3/幸运30并沿用怪物属性公式；移动速度160px/s、基础射程600px、攻击间隔与完整攻击动画均为1.25秒。
- 复用仓鼠火枪 1248px/s 黄色曳光弹；攻击第9帧（约769ms）发射，弹道起点、预判角度和墙阻挡检测统一改从朝向自适应的枪口坐标发出，而未配置枪口偏移的原仓鼠火枪保持原行为。
- 赏金猎人继续继承铁匠铺穿甲弹升级；击杀敌人时与该怪物正常金币掉落共用同一次随机底数，额外给予玩家“不含祭品加成的默认地牢击杀金币×2”，原正常金币掉落及祭品倍率不变。
- 将赏金猎人登记为 troop-line 正式军事单位，支持世界编队与跨位面序列化/重建。导入四张正式 RGBA 精灵至 `assets/companions/hamster_bounty_hunter/`。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试左右朝向枪口位置、第9帧出膛、1.25秒攻击节奏、穿甲弹生效、普通/精英/领主双倍悬赏金币及跨位面编队恢复。

### 对话：能源矿枯竭沉陷与永久清除（2026-08-22）
- 单个能源矿储量（HP）由 `2000~4000` 调整为 `5000~8000` 均匀随机；`gatherRatio` 同步由 `0.5` 调整为 `1.0`，每点有效采集伤害按 100% 转化为能源，因此完整单矿实际入库总量为 `5000~8000`。
- 能源矿覆写默认生物死亡流程：采空瞬间保持实体活跃但禁止继续命中，先切换对应的暗灰裂纹枯竭贴图并展示 `650ms`，避免默认 `DamageableEntity.onDeath()` 提前将其设为 inactive、跳过后续状态推进。
- 枯竭展示结束后复用正式 `BuildingSinkEffect`：临时提供标准 1×1 footprint 给地下遮罩与烟尘采样，播放建筑摧毁同款垂直沉陷、接缝遮盖和 DustEffect，并立即从 `EnergyNodeSystem.nodes` 与 `Game.entities` 清除；精灵由沉陷特效接管并在结束时销毁。
- 移除矿点 `90s` 原地刷新配置与复活分支。快照改存枯竭转场余时，并允许空矿点数组覆盖场景 setup 的初始矿簇，保证全部采完后离场再返回不会重新出现；只有位面正式重置或世代变化、没有沿用当前快照时才按种子重新生成矿簇。旧快照中的枯竭矿会迁移为新版短暂枯竭后沉陷清除。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试普通态→枯竭态→沉陷→完全消失、连续采空相邻矿物、全矿采完后切换位面再返回以及旧存档迁移。

### 对话：移除位面建筑陷阱体系（2026-08-22）
- 从位面建筑清单、放置判定、实体创建、点击交互、RTS 指挥、场景清理与贴地渲染分层中完整移除地刺、地雷、减速带、燃烧区四类 F→A 防守陷阱；同步删除独立实体/面板系统和数值配置。
- 删除科技树“机关学”及仅为陷阱存在的 `buildingKind` 解锁协议。旧存档恢复会按当前科技节点过滤已删除项目，不会保留失效研究目标。
- 删除 24 张 `trap_<type>_<grade>.png` 位面陷阱贴图、对应生成脚本/提示词、专用 CDP 探针，并清理关联样式、静态回归引用和知识库条目。
- 僵尸地牢战斗房的 `trap-system.js`、`trap_idle.png`、`trap_anim.png`，以及地牢事件陷阱贴图/音效/配置均保留不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试。

### 对话：建筑阴影真实底边全面修复（2026-08-22）
- 普通建筑阴影根部不再只返回吸附到 26.565°/30° 的标准四边形；在稳定底座横截面范围内逐列提取主体贴图真实 alpha 下包络，并以轻量折线简化保留台阶、门槛及左右不对称接地转折。建造 footprint、碰撞及独立 foundation 仍不参与视觉阴影。
- 自动阴影轮廓与上层 alpha 截面统一复用 Sprite 的 `anchorAdjustX/Y` 和镜像变换，修复茅草屋、骑兵学校等手调贴图位置后阴影仍停留在逻辑格心的问题；场景重建时清空 alpha 拟合缓存，避免同路径换图继续读旧轮廓。
- 共享结构阴影层移除会越过接地边的 `1.02×` 整体放大及 6px 居中描边，改为原尺寸单次填充；相交阴影并集除规则采样外强制保留所有输入顶点扫描行，避免最终合并阶段再次吞掉底角。同步补充静态回归契约，并修复阴影截图工具因普通建筑无 manifest silhouette 而跳过目标的问题。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试新旧建筑底边、左右角、镜像建筑、夜间与地牢强度以及相邻阴影并集。

### 对话：丛林神庙正式贴图导入（2026-08-22）
- 用户选定 48 步精修稿 `jungle_temple_refine48_from122926_seed1229263_raw.png`；按后续反馈取消白模 Depth 硬轮廓裁切，仅从原图边缘识别并剥离黄色背景，完整保留原图右侧塔柱、太阳徽记、暖色门内灯光与全部建筑外伸轮廓，紧裁得到真实 RGBA `755×846`。
- 正式导入 `assets/terrain/jungle_temple.png`，资源键从复用的 `church` 切换为稳定键 `jungle_temple`；关闭额外 `building_foundation_2x2`，避免与贴图自带完整神庙地基重复叠加。
- 按宽 `288` 等比标定为 `288×323`、`footOffsetY=159`，素材哈希写回配置；静态像素检查为单一主连通域、不触碰画布边界，完整保留原图地基四角。
- 仅登记并增量生成 `jungle_temple` 的 silhouette/projection/height/normal 四张光照图；神庙训练、科技门禁、林地首次生成、占格、碰撞和存档协议均未修改。未运行测试、构建或游戏运行时验证，按项目约定由用户测试实际显示与接地位置。

### 对话：仓鼠探险家 2000px 视野与夜间视野减半（2026-08-22）
- 仓鼠探险家增加实体级基础视野 `2000px`，独立覆盖斥候 profile，不改变普通仓鼠斥候的 1450px 视野。
- 环境光照系统新增统一昼夜视野倍率：当太阳 `daylight <= 0.12` 进入夜晚时，所有有效视野源最终半径乘 0.5，白天恢复 1；迷雾主画面、小地图、RTS 可见性门禁继续共享同一视野网格。
- 夜晚倍率与阈值写入并同步双份 `fog-of-war.json`；祭品视野加成及墙顶/楼梯倍率仍沿用原链路。未运行测试、构建或游戏运行时验证，按项目约定由用户测试。

### 对话：能量矿石堆组件化白模（2026-08-22）
- 按当前运行时真实的四形态协议重做能量矿石堆建模真源，不扩成旧文档曾规划但未落地的 12 形态：双尖主晶、三冠晶簇、斜向主晶和密集晶群均保持独立轮廓，正常/枯竭状态复用同一套结构 Depth。
- 公共组件库新增可参数化 `rough_boulder()` 与 `faceted_crystal_prism()`；每个矿点由 12 块相互重叠的自然岩块组成单一矿堆，晶柱按 manifest 数组嵌入矿堆，所有岩块与晶柱均保留稳定英文对象名和独立可编辑网格。
- 统一建筑包新增 `energy_node_1`—`energy_node_4` 的尺寸、低饱和蓝晶/炭黑岩石调色、四份正常态提示词和共用枯竭态提示词；已生成四套可编辑 `.blend`、透明材质预览与同相机 30° Depth。
- 经用户明确授权，将四张 Depth 与对应提示词发送至局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、12 步、CFG 3.5、Euler、Control 0.88，对每种形态分别生成 seed `122931`、`122932` 两张结构候选，共 8 张并汇总为联系表；四组均完整保持单一岩石矿堆和目标晶柱轮廓，静态比选统一推荐 `122932`。
- 用户确认统一采用 seed `122932` 后，以对应 12 步稿为 init、同形态 Depth 为结构约束完成四张 48 步普通态精修：seed `1229321`、CFG 3.5、Euler、Control 0.78、denoise 0.30；枯竭态最终采用强化无青光提示词、seed `1229323`、Control 0.88、denoise 0.70，舍弃仍带青色内发光的首轮枯竭稿。
- 普通态与枯竭态均使用纯黄底提取叠加各自 Blender Depth 轮廓清理，删除生成投影、散石和黄色缝隙，输出真实 RGBA；枯竭态统一全去饱和。正式覆盖 `assets/terrain/energy_node_v3_1..4.png` 与 `energy_node_depleted_v3_1..4.png`，原 8 张矿物贴图不保留额外副本。
- 每个矿点显示宽度基准由 `84` 调整为 `112`，明确绑定唯一 128×64 等距格（1×1）；生成从簇心最近合法格开始，只沿已成功放置格的四邻格连续扩张，替代圆盘随机散点与 115px 排斥，形成扎堆密集相邻矿簇。矿物仍保持零碰撞、可穿行，避免密集簇阻塞矿工和寻路。
- 快照新增矿点格坐标；旧快照不再恢复旧散点位置，而是在新密集布局上按顺序迁移储量、枯竭状态与复活计时，新快照按精确格心恢复并同格去重。采集倍率、储量、90 秒复活、四形态洗牌池与簇数量不变。仅完成静态透明度/轮廓与真实 diff 检查；未运行测试、构建或游戏运行时验证，按项目约定由用户测试。

### 对话：仓鼠探险家全图随机勘探（2026-08-22）
- 探险移动目标改为从当前位面完整可玩范围随机抽取，并同时过滤地图边界、墙体/建筑占用与起终点不可连通区域；连通搜索队列耗尽时明确判定不可达，仅搜索预算耗尽时交给后续 A* 判断，实际行走继续复用统一 `MovementSystem` 寻路。
- 默认排除距当前位置不足 180px 的近点，最多抽样 96 次；极小或拥挤地图会回退到本轮最远合法点，避免永久停在原地。
- 抵达每个目标后明确切换 18 帧 `viewing` 动画，按 1500ms 播完再重新全图选点；12 分钟探险、结束挖掘、奖励和停止探险协议不变。未运行测试、构建或游戏运行时验证，按项目约定由用户测试。

### 对话：传送门关闭外围自动道路（2026-08-22）
- 传送门配置增加 `perimeterTile: "none"`，复用现有数据驱动外围地块开关；玩家建造预览、正式放置、世界核心传送门创建及位面快照恢复均不再为传送门预留或生成 4×4 外围道路环。
- 传送门自身固定 2×2 footprint、贴图地台、碰撞、旅行、重建、入侵目标和存档协议保持不变；其他建筑的道路/田地规则不受影响。未运行测试、构建或游戏运行时验证，按项目约定由用户测试。

### 对话：位面祭坛与骑兵学校地台对齐（2026-08-22）
- 静态测量两张正式 RGBA 贴图的真实地台 alpha 轮廓：位面祭坛当前可见地台约 `204×102`，骑兵学校约 `213×107`，均小于固定 2×2 footprint 的 `256×128`，因此左边缘无法贴合占格。
- 位面祭坛等比显示尺寸由 `220×183` 调整为 `276×230`，脚点回退值同步为 `114`；骑兵学校由 `266×224` 调整为 `322×271`，脚点回退值同步为 `135`，并移除会把地台额外左推 4px 的横向手调，原有 `anchorAdjustY: -10` 竖向视觉微调保持不变。
- 仅修改单体建筑显示配置；2×2 footprint、放置碰撞、建筑功能、贴图文件及环境光照派生图均未改动。未运行测试、构建或游戏运行时验证，按项目约定由用户测试实际显示与接地位置。

### 对话：丛林神庙组件化白模（2026-08-22）
- 沿用既有 `jungle_temple` 玩法配置、林地首次生成、科技门禁和单位训练协议，只重做当前复用 `church` 的占位视觉，不新增第二套建筑系统。
- 将神庙限定为单体功能建筑：完整方形地基、三层居中退台、唯一中央宽阶、后部封闭圣殿、唯一暗门、低矮阶梯冠部与单一太阳徽记；少量苔藓藤蔓必须附着于主体，禁止扩展成遗迹群、独立祭坛、雕像或多栋神庙。
- 按用户反馈强化为恢宏对称版本：圣殿上方新增三层逐级收分的中央楼层，左右新增完全镜像且与主体相连的双塔台，每座塔台包含塔身、方形观景平台、四角立柱和石顶盖；前立面窗板、层级退台与藤蔓同步采用左右对称布局。
- 新增 `build_jungle_temple()`、低饱和橄榄灰石材 manifest、专用结构提示词和建筑级组件登记；已生成可编辑 `jungle_temple_model.blend`、材质预览和同相机 `jungle_temple_depth.png`，正式贴图与 `producer-buildings.json` 尚未替换。
- 未运行测试、构建或游戏运行时验证；本轮只执行建模所需的本地 Blender 资产生成与静态预览检查，由用户确认后再进入 12 步候选阶段。

### 对话：初始金币归零与开发面板资源按钮（2026-08-22）
- 新建角色的初始背包不再附带 10000 金币；治疗药水和魔力药水保持不变，旧存档已有金币不受影响。
- T 交互开发面板的“技能”栏新增“+10000 金币 / 能源 / 食物”三个按钮；金币按背包容量入包，能源与食物按正式仓库协议入库，容量不足的余量保留并在后续有仓库空间时自动入库。
- 按项目约定未运行测试、构建或游戏运行时验证，由用户测试。

### 对话：市场组件化白模与 12 步候选（2026-08-22）
- 将市场加入统一聚落建筑包：单体石木交易厅、宽四坡屋顶、四个正面固定摊位、两个侧面固定摊位和相连 L 形条纹棚组成完整 2×2 建筑；增加大型钱币广告牌、四块无文字商品图标吊牌、固定秤与双灯，所有摊位和招牌均保留为独立英文对象并固定在主体结构上。
- 新增 `market` manifest 尺寸、低饱和调色覆盖、专用结构/材质提示词和组件登记；已生成 `market_model.blend`、材质预览、完整 Depth 及不含 `Market_Foundation` 的同相机主体 Depth，正式 `placeholder_market.svg` 未修改。
- 经用户明确授权，将新版市场 Depth 与提示词发送至局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、12 步、CFG 3.5、Euler、Control 0.88 生成 seed `122924`—`122928` 共 5 张结构候选及联系表；初筛推荐 `122925`，等待用户选图后再进入 48 步精修。
- 用户选定 12 步 `122926` 后再次明确授权，以该图作为 init image、同一 Depth、Control 0.78、Denoise 0.30、48 步生成 `1229261`—`1229263` 三张低重绘精修稿及联系表；静态检查六个固定摊位、L 形棚、主广告牌、商品吊牌、完整地台与相机视角均未发生结构漂移，推荐 `1229263` 等待最终选择。
- 用户确认采用推荐的 `1229263`：以完整 Depth 轮廓约束剥离黄底、地台外投影及棚架内部残留黄块，保留暖灯和整块石板台，紧裁得到真实 RGBA `876×653`；正式导入 `assets/terrain/market.png`，资源键由 `placeholder_market` 切换为稳定键 `market`。
- 市场配置关闭额外 `building_foundation_2x2`，按宽 276 等比标定为 `276×206`、`footOffsetY=101`，素材哈希写回配置；仅登记并增量生成 `market` 的 silhouette/projection/height/normal 四张光照图，市场兑换、岗位、价格压力、占格、碰撞和存档逻辑均未修改。
- 未运行测试、构建或游戏运行时验证；本轮完成建模、生图、真透明处理、正式资源入库和静态配置核对，按项目约定由用户测试实际显示与接地位置。

### 对话：简洁大理石传送门组件化白模（2026-08-22）
- 不参考旧 `portal.png` 造型，为传送门建立独立组件化白模：一块完整浅阶大理石地台、两根宽方柱、单一道半圆拱、单片青蓝门芯、窄黄铜内嵌线和一块克制拱顶石；明确禁止塔楼、屋顶、雕像、废墟、碎石、多重法环和祭坛群。
- 在统一建筑包增加可编辑 `portal_arch_ring()`、`portal_core()` 与 `build_portal()`，尺寸和低饱和象牙白大理石/青蓝光色写入 manifest，专用提示词要求 30° 正交视角及地基四角完整可见，并同步登记建筑级组件。
- 已生成 `portal_model.blend`、材质预览和同相机 `portal_depth.png`；当前正式 `assets/terrain/portal.png`、传送逻辑、科技门禁、世界生成、占格、碰撞和存档均未修改，等待确认后再把 Depth/提示词送入 12 步结构候选阶段。
- 经用户明确授权，将 `portal_depth.png` 与 `portal.txt` 发送至局域网 `192.168.3.142:8188`，使用 FLUX.2 Depth（控制强度 0.88、12 步、CFG 3.5、Euler）生成种子 `122921`—`122925` 共 5 张结构候选；初筛推荐 `122925`，正式贴图仍未替换，等待用户选图后再进入 48 步细化。
- 用户选定 `122925` 后，以相同 Depth、控制强度、CFG 与采样器生成 48 步精修稿 `portal_s48_seed122925_raw.png`；静态目视确认两柱一拱、青色门芯、窄黄铜内圈和完整双层地基均保持稳定，正式贴图仍等待去黄底、裁切与导入确认。
- 按用户确认将 48 步稿经建筑专用边框连通色键、白模 Depth 轮廓约束、黄底边缘去污染和紧身裁切后覆盖 `assets/terrain/portal.png`；运行显示参数由旧 `288×293 / footOffsetY 147` 等比更新为 `288×253 / footOffsetY 125`，传送逻辑、科技门禁、世界生成、占格、碰撞和存档保持不变。
- 仅增量重建 `portal` 的 silhouette/projection/height/normal 四张环境光照派生图；静态像素核对为 RGBA `879×771`、单一主连通域且不触碰画布边缘。未运行测试、构建或游戏运行时验证，按约定由用户测试实际显示与接地位置。

### 对话：武器改造冷钢图标全量升级（2026-08-22）
- 盘点 17 把武器的 301 个改造选项，将旧资源中不符合部件语义的复用拆分为 143 种规范图标；仅保留消音器、轻量延长件与轻型剑体三组同义合并。
- 生成 4 张严格 6×6 的冷钢图标母版并切分为 143 张独立 `209×209` PNG，按近战、法杖、枪械/能量武器部件统一暗石墨金属底板与克制功能色；旧 `assets/icons/craft` 目录保留，便于回退。
- 双份 `craft-config.json` 已同步改用 `assets/icons/craft-cold-steel/` 路径；装备悬浮提示复用改造面板的图片渲染函数，避免新资源路径被直接显示成文本。
- 已静态核对双份配置哈希一致、301 个引用全部落盘、143 张图标无缺失/孤儿且尺寸一致；按项目约定未运行测试、构建或游戏运行时验证，由用户测试。

### 对话：全单位移动攻击与两点巡逻指令（2026-08-22）
- 左下指令栏扩展为所有选中友军通用，新增“移动攻击”和“巡逻”按钮；点击按钮后再点地图目标位置，右键或 Esc 可取消取点。
- 移动攻击采用传统 RTS A-Move 语义：单位向目标点移动，900px 统一感知范围内发现敌人会转入自身原有攻击逻辑，目标消失后继续前往原终点，到达后待命。
- 巡逻记录单位下令时的位置与目标位置，在两点之间持续往返；沿途遇敌自动攻击，战斗结束后恢复当前巡逻路段。墙顶/楼梯仍使用既有逐单位高架路线解析。
- 中键轮盘“主动攻击”同步替换为“移动攻击”，轮盘与左侧按钮统一走同一高层命令系统；队友、近战/远程仓鼠共用语义，矿工和仓鼠探险家仅执行移动段，不获得攻击能力。探险中的探险家继续拒绝这两项命令。
- 按项目约定未运行测试、构建、lint 或游戏运行时验证，由用户测试。

### 对话：建筑升级冷钢图标统一替换（2026-08-22）
- 按冷钢档案色板生成 6×6 图标母版并切分 34 张独立 `209×209` PNG，覆盖 47 个建筑升级使用点；相同语义升级共用资源，银行服务辐射复用远程范围图标。
- `building-upgrades.json` 为模块与能力新增数据驱动 `iconImage`，原 emoji 仅保留为资源缺失时的降级；升级数值、费用、等级和效果未改动。
- 共用升级卡、矿工营地旧式升级行以及研究院、铁匠铺、出兵建筑、仓库、经济工坊、银行的悬停说明均优先渲染新图标，并统一 34px 卡片尺寸与 30px 浮窗尺寸。
- 已静态核对 47 个配置项均有有效图片路径；按项目约定未运行测试、构建或游戏运行时验证，由用户测试。

### 对话：仓鼠探险状态锁与停止指令（2026-08-22）
- 仓鼠探险家进入探险后锁定状态：跟随、待命、攻击、巡逻、采集、右键移动/攻击等玩家指令均不再覆盖探险行为，12 分钟计时和当前阶段保持不变。
- 左下探险指令栏在任务执行期间改为显示“停止探险”及剩余时间；只有该按钮能主动中止任务，中止后不结算奖励并恢复待命。
- 位面快照以探险活动标记为准保存剩余时间，避免外部瞬时指令写入影响后台持久化。
- 按项目约定未运行测试、构建、lint 或游戏运行时验证，由用户测试。

### 对话：仓鼠探险家正式素材与 12 分钟位面探险（2026-08-22）
- 仓鼠探险家替换斥候占位视觉，接入 4×8 精灵图：idle 10 帧、running 16 帧、viewing 18 帧、digging 13 帧、dying 13 帧；按实际 alpha 内容校准显示尺寸与脚底锚点。
- 单位改为无攻击的视野/勘探定位，使用怪物同款六维公式：力量 15、敏捷 10、智力 3、体质 10、精神 3、幸运 30，初始生命 125，移动速度 120px/s。
- RTS 左下指令栏新增“开始探险”：选中探险家后可发起一次 12 分钟任务；单位在当前位面菱形范围内随机移动，到点停留播放 viewing，结束位置播放 digging，头顶实时显示进度条。
- 奖励改为互斥三档：50% 从地牢工艺品/林地食物/沙漠矿物池随机获得 1 件祭品；25% 获得 `500×玩家等级` 金币；25% 同时获得强化石×1、改造券×1与 `10×玩家等级` 魔法粉尘。后台位面快照同步保存每名探险家的剩余时间，完成后只结算一次，不再沿旧逻辑每 45 秒重复产出。
- 按项目约定未运行测试、构建、lint 或游戏运行时验证，由用户测试。

### 对话：世界-122 传送门与位面祭坛职责拆分（2026-08-22）
- 修复 `DefenseSystem.base` 在多位面初始化后指向世界传送门、且防御交互优先把它送进献祭面板的问题：防御系统不再把入侵目标绑定为献祭建筑，传送门继续只负责位面旅行、重建和入侵目标。
- 新增基础配置建筑 `plane_altar`，显示名为“位面祭坛”：仅世界-122可建、无需科技研究、免费放置、单座上限，复用原基地核心贴图与 5000 耐久/90 双防；按普通建筑走建筑面板、2×2 占格、出售/摧毁下沉和位面快照恢复。
- 通用配置建筑交互按 `panelMode: tribute` 将祭坛分发到献祭系统；祭坛出售或被摧毁时主动解除面板引用，失效祭坛不能继续消耗背包祭品。既有10种祭品上限、30分钟时长、同名刷新、背包扣除、状态栏和存档序列化逻辑保持不变。
- 位面祭坛详情改为右侧 45vw 冷钢栏目，复用统一建筑缩略图/生命条头部，并按标题20px、分区16px、正文14px、辅助12px重排献祭协议、建筑属性、生效列表和背包祭品；保留旧面板 id/class 仅用于存量探针与通用样式兼容。
- 完成静态调用链检查：祭坛配置会自动进入建筑面板，通用 ProducerBuilding 快照捕获/恢复 `cfgKey` 可覆盖祭坛，传送门点击不再被 DefenseSystem 提前消费；同步更新献祭回归脚本的静态契约。未运行测试、构建或运行时验证，按项目约定由用户测试。
- 后续代码审计修正：献祭运行时按模式区分，scene8~scene11 世界模式共享祭品效果，进入主神空间或地牢模式后停用；祭品过期/离开世界模式/清空会立即回算友军最大生命，并以单向刷新回调消除 store 与效果聚合器之间的循环依赖。传送门配置限制为单座，基础快照写入稳定世界核心 id，旧档只按出生点零成本核心迁移，并封住详情出售与建筑回收对世界核心的绕过路径。
- 献祭计时规则进一步统一：世界祭坛的30分钟只在 scene8~scene11 世界模式累计，进入主神空间、地牢或退出游戏时冻结剩余时长，返回任意世界后续计；新存档保存 `remainingMs`，旧 `expiresAt` 存档读取时兼容折算，暂停期间状态栏保留“已暂停/剩余时间”提示。地牢出征携带祭品改为本次地牢全程永续，特殊祭品状态栏不再使用约16分40秒的 `999999ms` 假长计时，而由 `DungeonMapSystem.shutdown()` 在通关、安全撤离、放弃或异常退出时统一移除。
- 模式互斥复查修正：效果聚合器以 `DungeonMapSystem.active` 硬选单一来源，地牢只读本次 `_carriedItems`、其他场景只读激活世界 store；`ExpeditionSystem.depart()` 旁路显式停用世界祝福，并在地牢 init 后重算属性/同步状态。蟠桃使用次数拆为世界与地牢两份，本次地牢 init/shutdown 只重置地牢次数，世界同类祭品再次献祭才刷新世界次数。

### 对话：夜晚与地牢保留低强度阴影（2026-08-22）
- 单位与建筑阴影不再在夜晚随 daylight 衰减到完全消失：室外白天保持现有 100% 强度，深夜保留 40%，晨昏在两档之间平滑过渡。
- 地牢战斗场景使用固定 55% 环境阴影，不受被冻结的世界昼夜时间二次衰减；地牢路线地图模式仍隐藏阴影。
- 仅调整环境透明度系数，单位水平 footprint、建筑 alpha 接地/分层造型、太阳方向、投影长度和刚修正的左右影角均保持不变；两个环境档位可由 `nightShadowStrength` / `dungeonShadowStrength` 配置调整。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：矿工营地组件化白模与 12 步候选（2026-08-22）
- 将矿工营地加入统一聚落建筑包：一座低矮石木矿棚、唯一石砌矿洞入口和唯一前右附着式卷扬机组成单体建筑，卷扬架、卷筒、绳索、导轨矿笼、矿石、拱券石、灯笼及连续棚顶均保留为独立英文对象。
- 新增 `miner_camp` manifest 尺寸与低饱和调色覆盖、专用结构/材质提示词和组件登记；已生成 `miner_camp_model.blend`、材质预览、完整 Depth 及不含地基的同相机主体 Depth。
- 经用户明确授权，将矿工营地 Depth 与提示词发送至局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、12 步、CFG 3.5、Euler、Control 0.88 生成 seed 122919~122923 共 5 张结构候选及联系表。
- 五张候选均保持单体矿棚、唯一矿洞口、前右附着卷扬机、导轨矿笼和完整石板台，未出现人物或独立矿车；静态比选推荐 `122922`，其石拱、暖光、卷扬连接和矿笼可读性最均衡。现有正式 `assets/terrain/mine.png` 尚未覆盖，等待用户选择后再进入 48 步低重绘精修。
- 用户采用推荐的 12 步 `122922` 后，经再次明确授权，以该图为唯一 init image、继续使用同一 Depth，以 48 步、CFG 3.5、Euler、Control 0.78、denoise 0.30 生成 seed 1229221~1229223 三张低重绘精修候选及联系表；三张均保持原结构，用户最终采用推荐的 `1229222`。
- 使用完整 Depth 保留 `1229222` 原图自带的整块石板台，仅剥离黄底、台外投影和卷扬架内部残留底色；紧裁得到真实 RGBA `882×665` 并覆盖正式 `assets/terrain/mine.png`。
- 矿工营地配置关闭额外 `building_foundation_2x2`，避免双层地台；正式图按宽度 277 等比标定为 `277×209`、`footOffsetY=103`，并仅重建 mine 的 silhouette/projection/height/normal 四张光照图及对应资产记录。
- 未运行测试、构建或游戏运行时验证；本轮只完成建模、候选生成、静态视觉检查和正式资源入库，由用户测试。

### 对话：仓鼠军营双塔组件化白模与 12 步候选（2026-08-22）
- 参考现有军营双塔轮廓，将仓鼠军营加入统一聚落建筑包：一座中央石砌操练厅、左右两座完整瞭望塔、连续双坡屋顶、强化双开门、箭窗、附墙长矛架、盾徽与门灯均保留为独立可编辑对象。
- 新增军营级 manifest 尺寸与低饱和调色覆盖、专用结构/材质提示词和组件登记；已生成 `hamster_barracks_model.blend`、材质预览、完整 Depth 及不含独立地基的同相机主体 Depth。
- 经用户明确授权，将军营 Depth 与提示词发送至局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、12 步、CFG 3.5、Euler、Control 0.88 生成 seed 122861~122865 共 5 张结构候选；五张均保持双塔、中央大厅、单一大门与完整地基，用户选定 `122863`。
- 以 `122863` 为唯一 init image，继续使用同一 Depth，以 48 步、CFG 3.5、Euler、Control 0.78、denoise 0.30 生成 seed 1228631~1228633 三张低重绘精修候选；三张均保留原结构，用户最终选定 `1228632`。
- 按后续确认改为保留 `1228632` 原图自带的完整石板台：使用完整 Depth 仅剥离黄底和台外投影，保留双塔窗洞、门内暖光与附墙军用组件；紧裁为真实 RGBA `887×789` 并覆盖正式 `assets/terrain/barracks.png`。
- 军营配置关闭额外 `building_foundation_2x2`，避免双层地台；正式图按宽度 275 等比标定为 `275×245`、`footOffsetY=120`，并仅重建军营的 silhouette/projection/height/normal 四张光照图及对应资产记录。
- 未运行测试、构建或游戏运行时验证；本轮仅执行建模、图像候选生成、静态像素检查和正式资源入库，由用户测试。

### 对话：探险家营地组件化白模（2026-08-22）
- 为 `explorer_camp` 新建独立组件化 Blender 白模，不再把草屋贴图作为最终视觉方向：单体结构由低矮帆布指挥厅、与主体相连的唯一木制瞭望塔、固定梯、附着式补给雨棚和结构内储物柜组成，全部收在固定 2×2 地基内。
- 增加地图板、罗盘徽记、开启入口和暖灯作为探险识别件；明确禁止独立帐篷、篝火、人物、旗帜、散落箱车和第二栋建筑，保持地牢前进基地的克制轮廓。
- 已登记建筑级组合件并生成 `explorer_camp_model.blend`、材质预览与同相机 Depth 控制图；经用户明确授权，将营地 Depth 与专用提示词发送至局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、12 步、CFG 3.5、Euler、Control 0.88 生成 seed 122914~122918 共 5 张结构候选及联系表。
- 五张候选均保持单体帆布指挥厅、唯一相连瞭望塔和附着补给雨棚；用户采用推荐的 `122916` 作为唯一初始图，以同一 Depth、48 步、CFG 3.5、Euler、Control 0.78、denoise 0.30 生成 seed 1229161~1229163 三张低重绘细化候选及联系表。
- 三张细化图均保留原结构；用户确认采用推荐的 `1229162`。导入时保留原图完整石板地基，仅用边缘连通黄底键控移除背景、地基外投影及开放木构中的残留黄块，并定点恢复被黄底判定误伤的塔楼灯窗与主厅暖光细节；最终得到真实 RGBA `881×824`。
- 新增正式 `assets/terrain/explorer_camp.png` 和独立预加载键；营地配置由复用 `thatch_hut` 改为 `explorer_camp`，关闭额外 `building_foundation_2x2`，按宽 268 等比标定为 `268×251`、`footOffsetY=124`，并只生成该贴图的 silhouette/projection/height/normal 四张光照图及资产记录。
- `finalize-building-runtime.py` 增加可选的封闭哑光色清理与局部 alpha 恢复参数，供有开放塔架、封闭亮窗的黄底建筑在不重绘主体像素的前提下完成确定性抠图。
- 未运行测试、构建或游戏运行时验证；本轮只完成候选确认、透明裁切、正式资源入库与静态配置核对，由用户测试。

### 对话：20 件工艺品祭品与友方向祭品引擎（2026-08-22）
- 新增工艺品祭品家族 20 件（普通/优质/稀有/史诗/神话/传说 = 4/4/4/4/2/2），题材为丛林部落手工艺；数值全部落在既有数值带，普通~稀有 1 增益 + 1 减益、史诗以上纯增益，定价沿用 100~3200。
- 祭品引擎新增友方向四键：friendlyAtkPercent / friendlyMaxHpPercent / friendlyMoveSpeedPercent / visionRangePercent，对全体友方单位（世界-122 生产单位 + 队伍侍从）生效；首次启用 staminaRegenPercent 与 killHpHealPercent 之外的冷门键位轮换。
- 四个新特效：血藤缠杖「血藤寄生」全体友方吸血 10%（伤害扣血点结算）；狼烟图腾旗「狼烟结界」玩家 1200px 内友方移速 +10%（光环取最高档）；双身翡翠神像「双身」出兵建筑每次招募数 ×2（额外单位不收食物）；黄金星象仪「风调雨顺」银行/风车资源产出 ×1.5（前台与后台位面模拟同步乘算），另带基础视野 +100%。
- 贴图经 gpt-image-2 管线生成（透明底 1024×1024），统一按 alpha bbox 裁剪 + 88% 主体占比归一化，落位 assets/items/tributes/crafts/。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。


### 对话：研究院三塔组件化白模（2026-08-22）
- 参考现有研究院贴图重建组件化 Blender 白模：一座贯穿主厅的中央高塔、两座完整石砌侧塔、连接式半木研究大厅和两翼坡屋顶；三塔均保留独立可编辑的塔身、图书室暖窗与蓝灰陡坡顶。
- 研究院加入统一建筑包 manifest、专用结构/材质提示词和组件登记；新增建筑级 `paletteOverrides`，只覆盖研究院的石材、灰泥、蓝灰屋顶与暖窗配色，不改变其他建筑。
- 已生成 `research_institute_model.blend`、材质预览和同相机 Depth 控制图；候选阶段未提前改动正式 `assets/terrain/research_institute.png`。
- 经用户明确授权，将研究所 Depth 与提示词发送至局域网 `192.168.3.142:8188`，使用 `flux2-dev-depth`、12 步、CFG 3.5、Euler、Control 0.88 生成 seed 122901~122910 共 10 张结构候选；联系表已生成，全部保持三塔与连接大厅，尚未进入 48 步精修。
- 按反馈移除白模塔楼窗后的书架横条，并把提示词收紧为“所有塔楼与主厅开口均为封闭明亮暖光窗”，明确禁止书柜、书架、书本、开放陈列和可见室内家具；使用更新 Depth 重新生成 seed 122911~122913 三版，三版均未再出现书柜效果。
- 用户选定 12 步 `122913` 后，以该图为唯一 init image，继续使用同一 Depth 和亮灯窗口提示词，以 48 步、CFG 3.5、Euler、Control 0.78、denoise 0.30 生成 seed 1229131~1229133 三张低重绘精修候选；三版均保持三塔、蓝灰屋顶和全亮灯窗，未出现书柜回流，尚未抠图或替换正式资源。
- 用户最终选定 48 步 `1229132`：用不含 `Research_Foundation` 的同相机主体 Depth 遮罩剥离生成图黄底、投影和石板地台，保留封闭暖光窗内部细节；紧裁得到真实 RGBA `712×722` 并覆盖正式 `assets/terrain/research_institute.png`。
- 按后续反馈改为保留 `1229132` 原图自带的完整石板地基：仅去除黄底，不再剥离生成地台；正式资源紧裁为 `873×798`，按宽 256 等比标定为 `256×234`、`footOffsetY=116`。研究院配置关闭额外 `building_foundation_2x2`，避免双地基叠加，并同步更新资源 SHA-256 与四张光照图。
- 未运行测试、构建或游戏运行时验证；本轮仅执行完成建模所需的 Blender 生成与预览检查，由用户测试和确认外观。

### 对话：建筑阴影左侧影角修正（2026-08-22）
- 修复自动高度截面按宽度虚构对称 iso 菱形纵深的问题；这段人造前/后顶点会额外生成斜边，右侧水平锚修正后仍可能把左侧远端影角拉歪。
- 下层、中层、上层自动部件改为直接保留当前主体 alpha 的实测左右边界，并仅加入凸包运算所需的最小 Y 厚度；真实接地纵深继续只由 0~24% contact polygon 提供。
- 建筑接地、独立 foundation 排除、太阳方向、显式 `shadowCaster.parts`、墙门楼梯与共享阴影层契约不变。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：单位阴影恢复水平 footprint 接触影（2026-08-21）
- 修复单位 2:1 ground footprint 椭圆被整体旋转到太阳影向、在影向接近屏幕 Y 轴时变成竖直椭圆的问题。
- 玩家、敌人、友军和 NPC 默认阴影改为严格绑定脚下 footprint：中心不偏移、宽高倍率默认 1、旋转恒为 0，不再随晨昏方向膨胀；脚底锚点、透明度、夜间淡出和本体 depth 继承保持不变。
- 保留 `shadow.directional=true` 作为特殊单位显式方向尾影开关；当前配置没有启用项，因此普通单位全部走水平接触影。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：仓库四项升级与跨位面资源损耗（2026-08-21）
- 仓库新增四项本栋独立升级：立体货架每级增加 5000 共享物理容量（基础 15000，10 级）；晶格压缩舱与密封谷仓分别令能源/粮食容量占用每级降低 3%（10 级时占用 70%）；跨位面协议初始额外消耗 50%，每级降低 5%，10 级归零。
- 跨位面协议统一覆盖建造、军事出兵和建筑升级：当前位面没有活动仓库而其他位面存在仓库网络时，报价、资源检查和实际扣除读取远端仓库；多座远端仓库取最高协议等级。当前位面有仓库时只使用本地库存并按原价结算，市场兑换、生产入库、出售退款与维修不套用跨位面加价。
- `EnergyManager` 改为按能源/粮食各自压缩系数计算共享物理占用、剩余容量和可入库数量；仓库面板分别显示本仓“仓储容量”与全部仓库“位面总容量”进度条，并展示本仓/位面物理占用、两种压缩率、跨位面损耗与四张升级卡。升级费用从 300 金币、500 能源、30 秒起，每级分别增加 100 金币、150 能源和 5 秒。
- 仓库模块、升级读条、升级后容量与逐仓库存写入位面快照；后台位面继续推进仓库读条，并使用同一压缩口径结算被动能源、采矿、风车粮食与出兵耗粮。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：建筑阴影自动造型分层（2026-08-21）
- 补齐独立 shadow caster 的造型层：普通建筑首次出现时，从当前主体贴图 alpha 按下层、中层、上层提取低模横截面，让屋顶收窄、偏置塔楼与双侧翼参与太阳投影，不再让所有建筑退化为同一种矩形影。
- 修复自动截面只按 `pixelX-contactX` 换算、遗漏主体 Sprite `visualOffsetX` 中稳定底座中心校正的问题；截面现在从 Sprite 中心完整映回逻辑脚点，右侧远端影角不再因水平锚误差产生错误斜率。
- 自动分析最高按 384×384 降采样并按贴图/帧/显示尺寸/高度/接地签名缓存；每个高度带最多保留两个主要实体，合并窗洞等小缝并过滤低占用噪点，避免逐帧扫描贴图。
- 准确的主体接地多边形继续作为 0~24% 根部，独立地基、建造 footprint 与碰撞不回流；显式 `shadowCaster.parts` 仍优先，`autoParts:false` 可按建筑关闭自动分层，墙、门、楼梯、掩体与散布障碍物路径不变。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：市场应急兑换与反套利定价（2026-08-21）
- 数值审计确认旧配置中标准交易只产生约 `0.018~0.020` 压力，但每秒恢复 `0.0375~0.050`，一次价格影响仅维持约 `0.36~0.53 秒`；现将每金币压力影响从 `0.002` 提高到 `0.0125`，基础恢复从每秒 `0.025` 降至 `0.0005`，商人恢复加成从每人 `+50%` 降至 `+25%`。
- 新增 15% 固定交易损耗：买入金币成本永远不低于基准价 115%，卖出金币收入永远不高于基准价 85%；压力与动态价差只能继续恶化当前方向价格，商人无法消除亏损底线，防止压力操纵和双向循环刷资源。
- 中性价格下标准买入改为实际扣除 `92 能源 → 8 金币`，标准卖出为 `10 金币 → 85 能源`；立即买卖同量资源最多只能收回约 74%，整数舍入只会进一步增加损耗。单笔压力约需 2.2~3.3 分钟恢复，连续同向交易会持续恶化报价。
- 市场详情新增最低交易损耗展示，交易按钮改为显示真实扣款而非 100 能源预算上限；同步更新市场与人口经济 UI 通用规则。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：建筑阴影与地基/建造占格解耦（2026-08-21）
- 普通建筑阴影新增独立 shadow caster：默认从主体 Sprite 当前 alpha 的实际接地区拟合根部，独立 foundation Sprite、建造占格与碰撞 footprint 不再决定视觉阴影；无有效拟合时才兼容回退旧 footprint。
- 建筑阴影默认高度改由主体视觉高度决定；配置支持 `shadowCaster.contactPolygon` 精确覆盖，以及带 `baseZ/topZ` 的低模分层部件，适配塔楼、挑檐等异形结构。
- 普通建筑继续禁用易随换图失配的 manifest 剪影；掩体、门、楼梯及散布障碍物保持原专用几何链，共享阴影层、太阳连续运动和重叠单次填充契约不变。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：仓鼠单位动画平滑与平民尺寸统一（2026-08-21）
- 仓鼠 10 兵种 AI 统一平滑启停：站定动作（挥击/射击/施法/采矿）改 0.85/帧指数衰减，决策分支（跟随到位/间隔待机/hold/RTS 到位）删除瞬时清零改由 MovementSystem 摩擦渐近，跟随 120px 内缓出减速，缓停滑行期（>25px/s）保持 walk 动画防滑冰；cancelForCommand 与骑士冲锋保留硬停。
- 纯视觉平民（农民/银行家/工程师）moveWorker 改速度渐近 + 60px 缓出到达，到位判定带速度门槛；农民田间移动改 easeInOutQuad 分段插值。
- 平民动画尺寸统一：population-economy.json 每个动画新增 scale/footRatio（逐帧 alpha bbox 实测），civilian-visual-utils.applyCivilianAnimSize() 在 play 时统一显示尺寸并修正脚底锚点；修复银行家 running 比 idle 大 10.6% 的跳变，工程师举锤姿势性增高不缩放。
- 单位动态阴影透明度再加深 25%（0.240625 → 0.30078125），静态投影不变。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。


### 对话：建筑遮挡平民修复与动态图层管线收敛（2026-08-21）
- 修复农民、工程师、银行家等纯视觉平民只用固定 18px 横向范围参与建筑前缘仲裁的问题；改为读取当前动作帧真实 alpha 可见宽度，人物贴图已与建筑前角重叠时能够正确抬到建筑之上。
- 二次修复建筑左右前角误判：横向可见范围扩张后不再把菱形两条前边一起取最大 Y，而是只采样离单位中心最近的真实前边，避免另一侧斜边把实际位于建筑前方的平民压到整张建筑贴图下面。
- 玩家、敌人、友军与纯视觉平民统一使用 `sprite-depth-profile`：一次扫描并缓存当前帧非透明上下左右边界，透明留白不制造假遮挡，宽身体/武器也不会因仅使用碰撞半径而漏判。
- 农民、工程师、银行家移除各业务系统内的逐帧深度写入，改由 `GameScene` 在建筑 footprint 拓扑深度落定后统一仲裁，消除旧建筑 depth 与本帧 Sprite depth 的更新时序分叉。
- 确认持续复现的主因是纯视觉平民没有空间占用：银行家/工程师直线移动会穿进建筑、随后被正确排序到整图之下。现新增配置化 18px 轻量地面半径，出生点/游荡点/交谈点/维修点先推出建筑 footprint，移动过程分段扫掠并在正面受阻时沿建筑边缘滑行；平民仍不进入战斗实体、物理和存档链。
- 补齐独立地基分层：地基不再使用“建筑主体 depth - 0.2”，正常/压平视图统一从 `world-render-layers.js` 读取固定地表层。道路、范围圈、地基、结构阴影、压平投影、贴地装置依次使用 -995/-994.8/-994.6/-994.4/-994.2/-994，消除三个对象共用 -994 后依赖创建顺序的问题。
- 建筑拓扑变化时统一把静止平民推出新 footprint，并同步重投影游荡、交谈和田间目的地，解决“建筑后来盖到待机平民身上”仍会复发的缺口；农民、银行家、工程师删除各自创建阶段的临时 depth，创建与逐帧统一走 `civilian-visual-utils`。
- 银行和工坊服务范围圈取消 `building.y - 998` 混合深度，纳入固定地表层；满级工坊的大范围椭圆上缘不再反盖动态单位。
- `WallSystem` 每个渲染帧只提取一次动态结构候选，避免每个单位重复遍历包含全部战斗单位的 `Game.entities`；墙、门、楼梯、高架 Z 与建筑拓扑深度规则保持不变。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：银行覆盖重叠收益递减（2026-08-21）
- 银行改为逐栋房屋计算服务覆盖层数：房屋只被 1 家银行覆盖时收益 100%，被 2 家覆盖时每家按 67% 结算，被 3 家及以上覆盖时该房屋对所有覆盖银行均不再产生金币；非重叠房屋不受影响。
- 前台实时结算与后台位面结算共用 `33% / 3 家归零` 配置；银行详情新增原始服务人口、折算有效人口、重叠房屋与最高覆盖数，并用红色说明重叠规则。
- 建造配置新增可复用的 `buildWarning`，选择银行建造时在底部提示栏以红色显示收益递减与三家归零警告，取消选择后清空。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：仓鼠斥候贴图尺寸对齐民兵（2026-08-21）
- 按待机、移动与攻击帧的非透明主体中位高度，将仓鼠斥候显示基准从 340 调整为 460，使游戏内主体高度接近仓鼠民兵；同步重算贴地与深度脚线偏移为 23px，不改变碰撞、AI 或战斗属性。
- 复用斥候素材的仓鼠探险家同步获得相同视觉尺寸修正。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：银行范围人口结算与四项升级（2026-08-21）
- 银行改为统计服务半径内存活房屋的当前等级人口容量，并按“服务人口 × 印钞机比例 × 上岗职员 × 人口效率 × 工坊增效”每轮结算金币；基础周期 10 秒，金币继续沿背包、主人空间仓库、银行坐标掉落的既有路由入库。
- 新增金融工具（结算速度每级 +5%，10 级）、扩容人员（初始 1 人，每级 +1，2 级）、印钞机（初始每人每轮 10%，每级 +5%，10 级）和服务辐射（初始 600px，每级 +50px，10 级）四项本栋独立升级；默认升级费用/读条沿用经济工坊的 500 金币、750 能源、30 秒起且每级 +5 秒。
- 银行详情面板同步工坊结构，显示覆盖房屋、服务人口、每人金币、结算周期、本轮金币与平均产出，提供四张升级卡和 tooltip；选中银行时显示透视压缩的服务覆盖圈，升级后实时刷新。
- 银行模块等级、升级读条、结算进度和小数余量均写入位面快照；后台位面按同一范围人口与离散周期公式推进，旧档岗位数会按新的本栋岗位上限安全夹取。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：压平视图改为基础功能（2026-08-21）
- 取消科技树“要塞投影”项目及 `flat_view` 科技门禁；压平视图在 scene8~scene11 默认可用，不再需要研究。
- 保留既有输入契约：正常直接操控与建筑模式使用鼠标滚轮，RTS 指挥与观察者模式使用空格。
- 按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：地牢位面特色建筑与四类特色单位（2026-08-21）
- 僵尸地牢首次构造传送门时在门旁生成 1 座探险家营地，林地位面首次构造时生成 1 座丛林神庙；免费生成只发生在首次世代，传送门或建筑被毁后的重建不再补发。两类建筑只能在所属位面建造、单个位面上限 3 座。
- 两类特色建筑接入双通道独立募兵：各自拥有单位上限、粮食消耗、单次/持续/暂停模式和独立进度条，位面快照与后台生产分别保存、推进两条队列。
- 新增仓鼠探险家、仓鼠赏金猎人、美洲豹战士、丛林祭司：探险指令按位面产出工艺品/食物/矿物祭品及仓库金币；赏金金币按背包→仓库→地面路由且继承穿甲弹；美洲豹普攻附加 3 秒致残；祭司轮换闪电、冰锥、火球。
- 科技树新增地牢探险团、丛林神庙仪式；对应位面未解锁时卡片用动态马赛克遮蔽并禁止研究，位面解锁后恢复正常显示与研究，完成项目后才允许继续修建对应特色建筑。
- 特色建筑暂复用草屋/教堂贴图，四类单位暂复用斥候/火枪/战士/牧师动画。按项目约定未运行测试、构建或运行时验证，由用户测试。

### 对话：经济建筑冷钢 UI 与字体统一（2026-08-21）
- 房屋、麦田风车、银行、市场和经济工坊统一使用冷钢档案式标题、双列数据卡、说明文字、岗位与进度结构；标题、正文、元信息和数字分别落到既定 20/16/14/12px 字体层级。
- 普通信息收口为冷白/灰白，仅金币、能源、粮食与警告状态保留语义色；资源数字统一使用数字字体和等宽数字，升级卡、交易、岗位、关闭及出售按钮统一交互状态。
- 移除经济面板内零散的业务行内字体和配色，保留动态宽度、既有 DOM 标识、岗位/交易/升级事件及全部经济计算不变；窄屏时统一退化为单列布局。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：风车与银行产量条改用稳定业务数据（2026-08-21）
- 修复麦田风车“粮食产量”和银行“金币结算”第二进度条仍绑定 `_economyTickMs`、每秒从0涨满后跳回0的问题；该字段只是内部低频结算时钟，不代表实际产量。
- 两根条统一改为“当前实际每秒产出 ÷ 满岗位基础每秒产出”，同时显示真实粮食/金币每秒数值；岗位、人口降效及经济工坊增效变化都会实时反映，超过基础满产时进度条封顶100%但数值继续显示真实增益。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：银行家平民交谈与纯视觉平民淡出规则（2026-08-21）
- 将素材库仓鼠银行家 `idle.png`（11帧）、`running.png`（18帧）、`speeching.png`（23帧）接入银行岗位；只有实际安排工作人口时才生成，且继续保持纯 Phaser Sprite，不进入实体、友军、碰撞、物理、寻路或独立存档链。
- 银行家会在全图随机寻找未被占用的农民或工程师，使用跑步动画直线接近到 50~80px，面向对方完整播放一次交谈动画后进入 8~14 秒随机 CD；无目标或冷却时在银行附近随机移动/待机。
- 新增纯视觉平民通用注册表与统一淡出销毁入口；农民、工程师、银行家在岗位减少、建筑出售/摧毁、位面离场或系统重置时均先退出目标池，再淡出 320ms 后销毁，后续平民禁止直接销毁 Sprite。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：传送门换 AI 新贴图 + 建筑阴影退回 footprint 凸包（2026-08-21）
- 生图管线新途径 ithinkai gpt-image-2 出传送门建筑图（`_denoise30_dimetric.png`：
  双边滤波降噪 32.7% + 垂直压缩 0.9056 把 isometric ~29° 校正为 dimetric 26.565°，
  接地边斜率 0.501/0.499 达标），裁剪 909×908 替换 `assets/terrain/portal.png`
  （旧图备份 NAS scratch `portal_backup_20260821.png`）。
- 世界-122 传送门核心视觉改走新贴图：`world-system.json`（data+public 双份字节一致）
  `scene8.coreVisual` texture defense_base→portal、440×439、footOffsetY 220。
- 建筑太阳阴影简化：`_syncStaticSunShadows` 对建筑（有 entity）`_silCache` 置 null，
  只用 footprint 凸包，不再合并 manifest 剪影（剪影随贴图失配反复出大范围错影）；
  散布障碍物保留凸包∪剪影。GameScene/scene-manager 注释与 06 分卷投影算法节已同步。
- 新增探针 `tools/cdp-portal-shot.mjs`（scene8 传送门正午/黄昏截图；含无头环境
  Phaser 需手动 `PhaserGame.init()` 的坑与黄昏时钟推进用法）。
- 已用 CDP 无头探针实机验证正午/黄昏阴影表现正常、无控制台报错；按约定未跑
  npm test 与构建。

### 对话：经济工坊正式贴图与建筑候选验收规则沉淀（2026-08-21）
- 经济工坊使用玩家确认的正式透明 PNG，保持标准 2×2 固定占地、现有工坊升级、岗位、增效与工程师维修逻辑不变；候选版本不作为运行时资源。
- 补充 World-122 建筑生图验收：实际 alpha 通道优先于视觉棋盘格，假透明必须重新抠图并按真实背景色去污染；底线只测承重底座的 alpha 下包络，门口与装饰越线不改变 `±26.565°`、2:1 地基标准。
- 新增窄高“塔楼化”预筛与亮度/饱和度/噪点指标边界；只有玩家明确接受的候选才能提升为稳定英文资产键。已终止的风车候选、遮罩、预览和生成缓存不纳入本次提交。

### 对话：生图管线新增 ithinkai 中转站 gpt-image-2 途径（2026-08-21）
- 新增 `tools/ai-gen/gptimage2-gen.py`：ithinkai 中转站（`https://token.ithinkai.cn/v1`，
  OpenAI 格式 `/images/generations`）生图客户端，定位为生图管线云端第四途径
  （双机 ComfyUI → 智谱 API → ithinkai）；无水印、按 token 计费（实测单张约 400 tokens）。
- key 不落仓库：环境变量 `ITHINKAI_API_KEY` → `%USERPROFILE%\.ithinkai\config.json`。
- 实测要点：返回图片 URL 有时效需立即下载；CDN 拒绝 urllib 默认 UA（脚本已伪装浏览器）；
  curl 直传中文 prompt 乱码错图，脚本 UTF-8 提交中文正常，仍建议英文提示词。
- 文档同步：`tools/ai-gen/WORKFLOW.md` 入口优先级/入口矩阵/注意事项三处，
  `skill/02-ai-asset-pipeline.md` 生图入口新增小节。
### 对话：经济岗位第二进度条数据源修复（2026-08-21）
- 修复经济工坊“本轮生产”错误绑定共享一秒计时器而持续往返跳动：改为稳定的“增效发挥”，宽度取 `actualEfficiency / configuredEfficiency`，文字同时显示发挥率与实际增效。
- 市场第二根条改为“交易效率”，按有效商人人效/岗位容量显示；风车与银行分别改名“粮食结算/金币结算”，无人上岗时固定为 0%，有岗位时继续显示真实一秒结算周期。
- 岗位标题中的模糊“效率”改为“人口效率”，明确它只表示位面人口超额降效；进度条继续复用冷钢主题渐变，没有复制业务色板或修改经济公式。
- 按约定未启动游戏、构建或浏览器验证，仅执行 JavaScript 语法与diff静态检查。

### 对话：仓鼠工程师岗位视觉与人口闭环（2026-08-21）
- 将素材库仓鼠工程师 `idle.png`（8帧）、`running.png`（12帧）、`fixing.png`（14帧）入库，均按 8列×4行、512×512 单帧注册为独立 `worker_` 动画，不进入友军实体链。
- 工坊正式接入人口岗位：工程师数量升级决定 2~5 个岗位容量，玩家实际安排的人口决定精灵生成数、经济增效与可并行维修数；零岗位时不生成、不增效、不维修，前后台位面结算保持同一口径。
- 上岗工程师空闲时在工坊 300px 内随机循环待机/跑动；接单途中持续跑动，抵达受损建筑后持续播放维修动画。没有碰撞体、物理体或寻路请求，出售、摧毁及位面离场沿用工坊清理链销毁。
- 已核对素材尺寸与连续有效帧，并执行 JavaScript 语法、JSON解析及diff静态检查；按约定未启动游戏、构建或浏览器实机验证。

### 对话：友军动画全量审计与状态机微调（2026-08-21）
- 审计工具 `tools/_audit_all_companion_sheets.py`：按配置帧区间量化全部 12 个友军单位
  约 50 个动作的网格/空帧/脚基线/中心漂移/贴边——露娜全绿；伊莉丝 attack/windmill 的
  bbox 中心漂移为武器挥弧固有（躯干/脚底锚已锁定，实测误报）；仓鼠 attack 同理。
- 真实问题已修：① 伊莉丝 run 起步 16fps→循环 26fps 帧率跳变 → 两段统一 24fps（对齐露娜，
  循环周期 1.0s×180px/s=180px 更贴近自然步幅）；② 仓鼠射手 walk 帧 0 是异源大帧
  （底边 493 vs 其余 ~334，每循环必跳一次）→ 动画区间改 [1,10] 剔除；③ 仓鼠牧师祈祷动画
  被打断后 priestPrayer 标记残留致永久禁播 → 加自愈分支（照斥候 shooterSwing 同款模式）；
  ④ 脚底线纵向对齐（tools/ai-gen/hamster-feet-align.py，逐帧底边对齐中位数、锚定不变）：
  射手/牧师/轻骑兵/矿工/战士 walk 底边 std 3.5~5.5→≈0，火枪手 dying 帧 7 离群低 121px 归位
  （std 30.7→0）。备份 assets/companions/_backup_feet_align_20260821/。
- 状态机审计结论：_animState 生产-消费闭环（howl/transform/pacing 属敌人管线不误报）；
  attack/windmill/defend 因 _frozenForCast 守卫不可能被防御抢切，atkPlayed/wmPlayed 无泄漏路径；
  hamsterDying 仅在 idle 分支复位，依赖 AI 复活先回 idle（现成立，记录备查）。
- 顺手订正 GameScene run 分支过时注释（"循环 11~23 帧" → startFrames/loopFrames 口径）。
- 按项目约定未运行测试/实机验证，由用户测试（重点：伊莉丝起步→奔跑衔接、射手走路不再
  每循环跳一下、各仓鼠走路颠簸感消失、火枪手死亡动画无闪低帧）。

### 对话：地牢隔绝状态统一冻结外部世界（2026-08-21）
- 新增统一的地牢隔绝状态判断：进入 `scene7` 后冻结太阳时间、五日入侵、后台位面结算、科技研究、资源生产、补员出兵、建筑升级与兵线推进；地牢自身地图、战斗和特工入侵仍正常运行。
- 世界面板及观察模式切场在地牢中统一阻断，点击其他世界或入侵支援时通过顶部提示“地牢中阻断了与外部世界的联系”；正常结算或撤离返回主神空间不受影响。
- 静态审计发现原实现已冻结太阳和五日入侵，但后台科技研究及遗留激活生产系统仍可能推进，本轮已补齐。按约定未运行游戏、构建或浏览器实机验证，由用户测试。

### 对话：仓鼠农民收割动画音效（2026-08-21）
- 将素材库 `仓鼠农民/1.mp3` 入库为 `assets/sounds/friendly/hamster_farmer_harvesting.mp3`，路径与音量由人口经济配置维护。
- 每只纯视觉仓鼠农民只在进入 `harvesting` 状态时播放一次，不在动画更新帧重复触发；使用农民当前世界坐标和 1400px 最大传播距离进行衰减。
- 按约定未启动游戏、构建或浏览器实机验证；仅执行静态语法、JSON与diff检查。

### 对话：经济工坊、自动维修与经济岗位冷钢面板（2026-08-21）
- 新增仓库前置的 2×2 经济建筑“经济工坊”及建筑/工程师 SVG 占位符；默认 1200px 范围、10% 配置效率、每秒 5% 最大生命维修、2 名工程师。
- 新增四项逐建筑升级：范围 +100px（10级）、配置效率 +2%（10级）、维修效率 +1%（10级）、工程师 +1（3级，最终5名）；统一复用建筑升级卡、费用提示与读条，并随位面快照保存/后台完成。
- 工程师只维护纯视觉 Sprite 和点到点直线调度，不进入实体、碰撞或寻路系统；每人独占一个受损建筑，抵达后才免费维修。范围内出现敌方或特工时全部维修暂停并返岗。
- 工坊对范围内风车、银行和市场提供经济增效；每名工程师发挥配置效果的20%，默认2名实际为4%，5名满员发挥100%。多栋工坊对同一建筑只取最强光环，不叠加。
- 离场位面同步结算工坊升级、经济增效与无敌情时间窗内的抽象工程师行程/维修；选择工坊时显示随升级变化的等距覆盖圈，关闭详情即清除。
- 经济岗位详情按冷钢档案重排，新增岗位安排百分比条与1秒生产进度条，填充颜色按低→中→高渐变；本轮只做静态语法、JSON与diff检查，未启动游戏、构建或浏览器实机验证。

### 对话：出征队友选择恢复与仓鼠兵种留守（2026-08-21）
- 在出征面板原位置恢复“玩家 + 3 名正式队友”选择区：点击已有队友进入管理并可移出，点击空位打开招募界面完成选择或替换；下方出征栏保持五列并压缩队友卡高度，避免挤占祭品与操作区。
- 明确地牢随行边界：只有 `PartySystem.members` 正式队友随玩家进入；`Game.friendlyUnits` 中的仓鼠兵种及其他场景友军在确认出征时暂存，从地牢运行态移除，回到主神空间后以原对象、原坐标恢复。
- 静态排查确认旧流程只清 `Game.entities`、没有隔离 `Game.friendlyUnits`，确有仓鼠兵种被地牢渲染读取的路径；本轮已补齐暂存/恢复生命周期。按约定未运行游戏、构建或浏览器实机验证，由用户测试。

### 对话：沼泽初级通关解锁森林世界（2026-08-21）
- `世界-124·林地（scene10）` 的首次传送门构造条件改为完成 `swampBeginner`；失败、撤离或未完成不会解锁。
- 林地开放传送门构造，通关后进入现有 `AVAILABLE` 流程，首次构造保持免费；僵尸初级仍只负责解锁世界-125，不会提前解锁林地。
- 同步 `data/public` 两份世界配置及世界进度结构检查；按约定未运行测试、构建或游戏。

### 对话：地牢出征三栏界面与点击层修复（2026-08-21）
- 出征准备收敛为最左侧规则说明、中部地牢选择与出征栏、右侧玩家背包；右栏锁定装备背包页并隐藏其他系统页签，移除面板内重复的队员编组区，常驻组队栏在出征期间只隐藏、不改变原定位置。
- 修复右侧背包统一遮罩覆盖中部出征面板的问题：出征状态禁用该遮罩的视觉与点击捕获，地牢下拉框、祭品格和操作按钮均可直接操作；关闭或确认出征时同步恢复普通层级。
- 按冷钢档案规范统一标题、分区、控件、格子、按钮、字体和响应式布局；未运行游戏、构建或浏览器实机验证，由用户测试。

### 对话：沼泽地初级/中级地牢与黑狼占位生态（2026-08-21）
- 新增沼泽地-初级（E，30~35 房、10~25 级、1200 金币）和沼泽地-中级（D，45~50 房、25~40 级、1500 金币）；房间数、路线、战斗/事件比例、精英概率、宝箱岔路和等级奖励沿用僵尸地牢对应档位标准。
- 两档战斗事件显式启用现有五房蛇形竞技场，并复用沼泽地板、地图背景、柴墙、藤门、转角与通道预制；新增地牢级 `combatArena.enabled` 和 `wallStyle` 覆盖，避免 E 档回退单房间或新类型回退僵尸砖墙。
- 沼泽初/中/高级的普通、精英、领主槽和 Boss 占位遭遇统一使用 `poolKeys: ["blackWolf"]`，当前只生成黑狼；高级 Boss 同步退出集合体 Boss，改走黑狼独立遭遇，等待森林精英/领主/Boss 后续替换。
- 沼泽事件题材与僵尸事件池分离：30% 通用事件保持不变，其余 70% 仅进入 `swampEventPlaceholder` 占位事件；占位事件无奖励、无战斗、无专属背景，便于后续整批替换。
- 同步 `data/public` 两份地牢配置、经验配置映射、结构回归清单与 `dungeons-table.md` 生成器；地牢一览表现已登记 7 个地牢并按题材拆分限定事件池。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：矿工补员改为免费（2026-08-21）
- `data/hamster-miner-camp-building.json#respawnEnergyCost` 由100调整为0；前台矿工营地与后台位面模拟共用该配置，矿工死亡后的全部补员不再消耗能源。
- 军事单位继续消耗粮食，本次不改出兵成本、采矿产率、补员时间或矿工数量上限。
- 按项目约定未运行游戏、构建或浏览器实机验证，由用户测试。

### 对话：军事出兵改为消耗粮食（2026-08-21）
- 所有军事生产建筑的单位成本保持原数字不变，但配置字段由 `spawnEnergyCost` / `unitSpawnEnergyCost` 改为 `spawnFoodCost` / `unitSpawnFoodCost`；民兵50、斥候75、战士125、盾卫150、射手120、火枪180、牧师240、轻骑220、骑士300。
- 仓鼠兵营、草屋、教堂、靶场和骑兵学校的单次/持续招募统一检查并扣除当前位面仓库粮食；粮食不足时暂停读条并显示真实所需数量。快照恢复已有单位仍不收费。
- 后台位面结算同步按各兵种配置扣除逐仓 `storedFood`，并记录 `foodSpentOnUnits`；矿工补员不属于军事出兵，费用为0。
- 风车产率由每名农夫0.25提高到1.5粮食/秒，满4岗位为6粮食/秒：可支撑大多数单栋军事建筑持续运作，高频斥候仍需要粮食储备或更多风车。
- 招募面板增加粮食库存与粮食成本展示；无限资源开发开关同步豁免粮食消耗。
- 按项目约定未运行游戏、构建或浏览器实机验证，由用户测试。

### 对话：人口经济仓储与银行溢出闭环（2026-08-21）
- 风车纯视觉农民上限由 3 调整为 4，与 4 个农夫岗位一致；仍只创建 Phaser Sprite，不进入实体、友军、碰撞或寻路系统。
- 当前位面没有活动仓库时，房屋、麦田风车、银行和市场统一显示仓库前置锁定；选择建筑与最终放置入口均再次校验，仓库建成/毁坏时建筑栏在 500ms 刷新周期内更新。
- 风车粮食改为逐仓 `storedFood`，与能源共享仓库总容量；仓库详情展示能源/粮食分项，仓库有任一库存时禁止出售，被毁时两类资源一并损失。旧顶层粮食快照在仓库物化后一次性迁移。
- 银行金币固定按“玩家背包 → 主人空间仓库 → 银行位置地面掉落”路由；背包和主人空间仓库新增返回实际存入数量的部分入库接口，避免部分成功后重复掉落。
- 主人空间仓库新增主存档序列化/恢复，银行转存进去的金币及既有仓库物品现在随存档保留；旧存档缺少该字段时维持兼容。
- 跨位面后台银行在两级存储都满时，将溢出量写入对应银行快照，回场后才在该银行坐标物化金币掉落；粮食、共享仓储容量与生产余量同样纳入后台结算。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：建筑栏连续排列与双模式排序（2026-08-21）
- 建筑栏不再为未解锁项目保留空槽：锁定建筑改用折叠门禁，所有已解锁建筑无空行连续排列；兵种、升级项目和兵线栏目仍维持原位占位规则。
- 默认“解锁顺序”读取科技实际完成次序，基础建筑置前、同一科技解锁的建筑保持原清单次序；科技完成后只重绘建筑网格，不中断当前放置或回收操作。
- 建筑页面新增排序按钮，可在“解锁顺序”和“能源消耗”之间切换；能源模式按能源造价从低到高排列，金币建筑统一置后并保持原清单顺序。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：友军单位阴影贴脚修复——锚点兜底语义错误（2026-08-21）
- 用户问题：队友阴影不贴合脚部椭圆 footprint。结论：**是 footprint 锚点设置的错，
  不是阴影公式**——椭圆几何（2:1 压扁、groundRadius 定宽、影向拉伸）不变。
- 根因：`_getFootOffsetY` 无显式配置时兜底 = displayHeight/2（格底边），而 v2 出图管线
  脚底基线固定在 **0.9375×格高**（格底留透明）——影子掉到脚底下：walk/run/defend
  （640 格）≈13.5px、idle（512 格）≈10.8px、attack（1024 格）≈21.7px。
- 修复：友军阴影锚点改"footOffsetY 显式配置优先（仓鼠系），否则实测当前帧内容底边
  （alpha>8 底行比例，按帧缓存，新增 `_getVisibleFrameBottomRatio`，与 HUD 顶部扫描同源）"——
  任意素材基线都贴脚，含无配置的仓鼠矿工/牧师（基线 ~0.68）。
- 备注：队友 groundRadius 是硬编码默认 26（companion.js），觉得影圈大小不合适再调它。
- 追加修复：队友深度仲裁锚点（_companionSprites 深度循环）同根因——footOffsetY 无配置时
  同样改实测帧底边兜底，友军遮挡排序与阴影现在共用同一真实脚底线；队友 groundRadius
  改为可配置（companion-config.json `groundRadius`/`collisionRadius`，缺省 26 不变）。
- 按项目约定未运行测试/实机验证，由用户测试（重点：伊莉丝/露娜/仓鼠影子贴脚程度、
  队友在墙前/墙后的遮挡排序是否仍正常）。

### 对话：队友渲染零素材微动三件套（2026-08-21）
- 待机呼吸：单帧 idle 的队友（伊莉丝/露娜/纯跟随队员）正弦缩放 ±1.8%/∓0.6%、脚底锚定、
  相位按 id 打散；多帧 idle 动画（斥候）与移动/攻击/施法状态不叠加。
- 动作切换残影：贴图键切换瞬间旧帧 alpha 0.32 残影 110ms 淡出，柔化 walk↔run↔idle↔attack
  硬切顿挫；每队员至多一个残影，离队即销毁。
- 伊莉丝 run 循环 32→26fps 消跑步滑步（每周期位移 135→166px，贴近自然步幅）。
- 均落在 GameScene._syncCompanionSprites + companion-config.json；未运行测试/实机验证，
  由用户测试（重点：idle 呼吸幅度是否自然、切换残影是否穿帮、跑步滑步是否减轻）。

### 对话：人口经济建筑代码骨架（2026-08-21）
- 新增配置驱动的人口经济：房屋提供 6/10/16 人口容量；风车、银行、市场直接分配数值岗位，不创建平民实体，不接入寻路、碰撞、战斗或友军实体表。
- 新增麦田风车（标准 2×2）：复用建筑 4×4 预约真源，但外围 12 格生成田地占位符而非道路；田地不提供道路移速，建筑拆除/摧毁时随建筑清理。
- 新增岗位调度闭环：建筑详情支持 −1、+1、最大分配；风车每名农夫每秒产 0.25 粮食，银行每名职员每秒产 0.1 金币，市场至少 1 名商人才能交易，更多商人降低价差并加快价格回归。人口容量不足时所有岗位按比例降效。
- 新增风车仓鼠农民纯视觉层：安排农夫岗位后，最多按 `visualWorkerCap` 生成 3 个 Phaser Sprite，在外围 12 格田地内循环待机、奔跑和收割；不进入实体/友军表，不带碰撞、寻路、血条或战斗逻辑，岗位归零及建筑/位面清理时同步销毁。
- 岗位数、粮食/金币生产余量和市场压力写入建筑快照，位面粮食库存写入顶层人口经济快照；离场位面由 1Hz 抽象模拟继续结算粮食、金币与市场回归，不物化平民或逐单位运行 AI。
- 新增 7 张 SVG 占位资源，建筑卡支持配置 `assetPath`；正式美术可替换资源与显示尺寸而无需改经济逻辑。
- 麦田风车已换用 5080 深度锁定管线生成的透明 PNG 主体；保持标准 2×2 固定基座、现有显示尺寸和外围田地逻辑不变。
- 风车外围田地已换用 5080 生成的无缝麦田材质，并由确定性脚本裁成 4 帧 128×64 透明等距田格；12 格按坐标稳定取帧，不再重复同一张 SVG。
- 房屋 Lv.1/Lv.2/Lv.3 已换用 5080 深度锁定管线生成的透明 PNG：同一 2×2 固定基座、统一 45°/30° 等距视角和地面底线，按单层民居、两层住宅、高容量大宅递进；人口容量、升级费用、显示尺寸和经济逻辑不变。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：伊莉丝动画僵硬审计与三路优化（2026-08-21）
- 审计（tools/_audit_elise_sheets.py）：精图/网格/脚底基线全部达标（基线 std≈0.3px），
  僵硬根因 = 水平中心漂移（v2 对齐用全内容质心，挥剑/伸盾时质心被四肢带偏：walk ±36、
  run 循环段 ±52、attack ±127、windmill 相邻帧跳 173px）+ 六动作平均中心互不一致
  （切换横跳 30~70px）+ 帧率只有基准一半（walk 12帧@10fps vs 露娜 26帧@24）。
- A 重切（tools/ai-gen/elise-recenter.py，无损整像素平移）：锚点改躯干带/脚底带质心——
  idle/walk/run/defend 躯干锚硬锁定格心（std 0.2~0.3）；attack 脚底锚 3 帧平滑+步变 ≤40px
  保留前冲趋势，首尾站姿对齐 idle 脚底锚（−30.5）。windmill 不重切（内容 860/896 无余量，
  v2 质心锁定对旋转剑弧本就合理）。原图备份 _backup_before_recenter_20260821/。
- B 配置快修（companion-config.json）：walk 10→14fps；run 起步段 [0,22]→[0,9]
  （旧配置把整序列当起步播两遍循环段），起步/循环 12→16fps。
- C 插帧（tools/ai-gen/elise-interp.py + elise-interp-feet-fix.py，rife-ncnn-vulkan v4.6）：
  RGB 透明区最近色填充防黑晕，alpha 作灰度单独过 RIFE 再回贴；walk 12→24 帧@28fps
  （5×5）、run 循环 12→24 帧@32fps（起步 10 帧不动，表 5×7），循环含回绕缝；
  中间帧脚底线二次校准（RIFE 软化底行 2~14px → 整像素竖移对齐邻帧均值）。
  备份 _backup_before_interp_20260821/。
- 验证：离线审计指标 + 逐帧拼条目检通过；未运行游戏内实机验证，由用户测试。

### 对话：位面战争迷雾系统（2026-08-21）
- scene8~scene11 接入 RTS 三态战争迷雾：未探索区域纯黑、已探索区域保留暗化地形与静态建筑、当前视野恢复正常；逻辑网格默认 128 像素、100ms 更新，主画面使用单张低分辨率 CanvasTexture，避免逐格创建显示对象。
- 视野由玩家、队友、军事单位、斥候、骑兵、传送门、出兵建筑和防御塔并集合成；墙顶单位获得 2 倍、楼梯单位获得 1.2 倍视野。观察相机本身不提供视野，墙、门、道路、仓库和普通建筑不提供视野。
- 敌人、入侵特工和掉落物只有处于当前可见区时才显示；实体贴图、阴影、名字/血条、Boss 血条、X 光层、浮字、投射物及小地图标记使用同一判定。迷雾不修改实体 active、AI、战斗、碰撞、寻路或后台位面模拟。
- RTS 点选、框选、已选目标清理和自动寻敌统一拒绝迷雾内敌人；空地移动、集结与探索指令仍允许落在迷雾中。
- 每个位面只持久化 explored 位图并绑定 worldEpoch，visible 每次进入实时重算；位面毁灭/重建清空探索。旧完整存档缺少迷雾字段时兼容为全图已探索，只有传送门基础快照的新位面仍从黑图开局。
- 自查修复：正式队员由 `PartySystem.members` 管理、并不在 `Game.entities`，现仅在玩家实际物化于当前位面时纳入 900 视野；观察模式不会借用未入场队伍开图。大型敌人/建筑改按 footprint 与可见格相交判定，不再等中心点进视野才突然出现。
- 自查修复：眩晕/冻结/激励、通用战斗特效、建筑受损火烟、持续地面区域、暴风雪、雷云和局部环境光统一接受迷雾裁切；只停视觉生成与显示，不停伤害 tick。可见网格改为双缓冲复用，去掉每 100ms 的整表复制。
- 视觉接口收口：新增 `FogVisualAdapter` 注册/同步协议，EffectManager 自动注册普通特效，独立地面区域主动注册；新特效通过 `getFogPosition/getFogVisuals/setFogVisible` 声明迷雾边界。暴风雪粒子、雷云和共享爆发粒子迁入统一接口，不再由 GameScene 逐字段判断。
- 小地图不再循环绘制 96×64 个迷雾格，改为与主画面共享同一张低分辨率 CanvasTexture；动态实体、玩家箭头和相机框保留独立 Graphics 层。
- 迷雾遮罩增加配置化 3×3 柔边、已探索边界渐变和显隐过渡；默认展开 180ms、收缩 260ms。过渡只作用于遮罩 alpha，敌人、RTS 门禁和特效裁切仍读取二值逻辑网格。
- 新增高度感知 LOS：墙段与建筑 footprint 写入阻挡网格，按观察者眼高和阻挡顶部高度判定；墙顶单位仅忽略脚下墙段，同格起点补精确射线，避免粗网格穿墙。
- 阻挡网格在墙段端点、高度或建筑结构变化时失效重建，并保留 1 秒兜底检查；门扇无论开关均不阻挡视野，门柱及两侧城墙仍正常遮挡。
- 配置真源为 `data/fog-of-war.json`，并同步 `public/data/fog-of-war.json`；按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：科技树审计问题修复（2026-08-21）
- 研究院每秒产出的研究点改为跨队列连续结算：项目完成后的溢出点数立即投入下一项，不再浪费；整条队列 ETA 与实际结算统一为“剩余研究量 ÷ 当前速度”。
- 科技完成/解锁不再重建建筑栏或取消正在进行的放置/回收；原有建筑卡继续由门禁原位显隐，陷阱只在栏目末尾增量增删。仅在读入低科技存档导致当前操作重新上锁时终止对应操作。
- 全屏科技树开启期间拦截其他全局栏目快捷键，ESC 仍由科技面板独占关闭；Electron ESC 转发走同一规则。
- 科技配置校验新增真实目标注册表核对，建筑、建筑类别、兵种、升级项目和游戏机制 ID 拼写错误会在载入时明确报错。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：单位太阳影对齐 footprint 修复（2026-08-21）
- 根因：unit_projection 退役后胶囊接触影只剩"整体平移 offset、不旋转、几乎不拉伸"——
  影块随太阳方位斜向滑离脚底（小半径单位最明显），与"长轴沿太阳方向拉伸"的定稿口径不符。
- 修复（ensureShadow，GameScene）：椭圆长轴 = footprint 宽 + 影向位移量，旋转到影向
  （atan2(offsetY, offsetX)），中心只走位移一半——影根始终盖住脚底；位移 ≤3px（近正午）
  不旋转，保持 2:1 地面椭圆。玩家/怪/NPC 锚点仍为 collider.x/y（footprint 圆心）。
- 友军（队友/仓鼠单位）阴影锚点改为视觉脚底 sprite.y + footOffsetY（与深度仲裁同锚点）：
  精灵位置已含 z 抬升与帧格归一化修正，修掉高架友军阴影留在地面、以及纯跟随队员
  （无 AI 逻辑坐标）阴影错落的隐患；原 fallbackCenter 因 collider 优先生效是死代码。
- 观察记录（未改，属在途工作）：敌人精灵当前不做 z 抬升渲染（仅玩家/友军有 -z），
  敌人阴影与之一致；若后续敌人精灵抬升，敌人阴影需同样 footprint.y -= z。
- skill/06 胶囊影条目订正：透明度 0.385→0.1925（与代码一致），补几何定稿段落。
- 追加调整：单位（动态）阴影强度加深 25%——DYNAMIC_SHADOW_OPACITY 0.1925→0.240625；
  建筑静态影保持 0.1925 不变。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：恢复研究项目详细浮窗（2026-08-21）
- 修复研究院、铁匠铺及出兵模块的悬停详细浮窗已生成但被右侧栏目遮挡的问题：共用升级浮窗改为挂载到 `rightSidebarPanelLayer` 内部，处于建筑详情面板之上、全局菜单之下。
- 已存在于 `body` 的旧浮窗会在下次显示时自动迁移到正确层级，无需重开页面；研究项目说明、当前/下级效果、目标、费用和读条时间保持原样。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：出兵建筑面板冷钢字体统一（2026-08-21）
- 仓鼠兵营与配置型出兵建筑详情面板按“冷钢档案”规范统一为四档文字层级：面板标题 20px、栏目与项目标题 16px、正文及操作按钮 14px、资源/耐久/进度与辅助说明 12px；原 10px 升级费用和说明不再过小。
- 普通土黄色、土绿色文字统一改为冷白/灰白；金币、能源、进度、警告等有明确含义的状态色继续保留。生成兵种按钮、模块升级按钮及三块内容容器同步收口为冷钢控件外观。
- 配置型建筑只在实际出兵模式下启用本轮样式，研究院、铁匠铺、仓库、传送门和普通详情模式不受影响；未改动招募、升级及科技门禁逻辑。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：科技树目标队列、研究反馈与统一门禁（2026-08-21）
- 科技状态升级为 v2：任意未完成科技都可设为远端目标，系统自动生成缺失前置队列并逐项推进；目标取消或完成后继续按原规则随机选择当前可研究项目，切换目标仍保留各节点已有进度。
- 科技树新增目标队列/自动随机模式、当前项目和整条队列 ETA、队列序号、远端目标完整前置路径高亮及研究计划预览；完成提示同步列出本次解锁内容。
- 新增统一 `TechnologyGate`：建筑卡、回收按钮、兵种选项、研究项目和兵线功能统一处理隐藏占位、鼠标/键盘禁用、可访问状态及解锁刷新；底层功能校验保持不变。
- 科技配置载入时检查重复节点、无效研究量、缺失或循环前置、不可达节点、非法解锁类型及重复解锁归属；主存档兼容 v1 当前项目和全部已有研究进度。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：主神空间菱形化对齐世界-122（2026-08-21）
- 主神空间从 4096×4096 方形改为 12288×8192 菱形世界：复用 `_scene8Diamond`（边斜率 0.5）
  与 `_registerScene8Boundary`，分块惰性地板按菱形裁剪、区外全黑；四条硬边界墙改 noVisual
  隐形兜底，菱形四边注册 `_boundary` 阻挡段。hub_brick 大理石砖池与相机 1.0 缩放保持不变。
- 坐标统一平移 (+4096,+2048)：传送门建筑→(5458,3759)（祭坛左 480px 关系不变）、训练靶/
  DPS靶/测试目标/testArea、obstacle-layout 3 件手摆障碍；NPC/祭坛/仓库/武器排/掉落/墓碑
  均为 origin 相对坐标自动跟随。SceneManager init 兜底同步新尺寸与 diamondFloor。
- `_loadMainScene` 新增菱形落点守卫：旧档/旧会话缓存坐标落在新菱形外时回退主城原点。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：出兵建筑升级读条与研究项目费用展示统一（2026-08-21）
- 研究院、铁匠铺与出兵建筑升级项目统一使用同一项目卡：移除“悬停查看说明”，改为直接显示下一等级所需金币与能源，并统一等级、按钮、进度条和剩余时间格式。
- 仓鼠兵营及全部配置型出兵建筑的下方模块升级由即时生效改为读条完成后生效；默认读条60秒，与现有铁匠铺/研究院项目一致，开始读条时扣资源，完成后提升对应兵种的全局模块等级并同步场上单位。
- 模块读条记录固定携带兵种与模块ID，同一兵种同一项目禁止多建筑并行；场景快照、读档恢复和离线位面结算均支持模块进度，避免切换兵种或离开世界后结算错位、丢失进度。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：全局科技树初版与功能门禁（2026-08-21）
- 修正科技门禁对既有 UI 布局的干扰：隐藏的指挥入口不再导致左侧兵线栏目按零尺寸错算到顶部；建筑卡、兵种按钮、研究项目与回收按钮均保留原始占位，仅隐藏内容，解锁后在原定位置恢复。科技树入口追加到既有右栏按钮之后，不再挤动任务、世界和队员栏目的预设顺序。
- 科技树视觉按“冷钢档案”主题统一：所有科技卡改为不透明实体底，研究线路固定在卡片下层；可研究项目使用冷白卡、已完成项目使用青色卡、不可研究项目保留深灰钢色，并同步统一字体层级、选中/焦点态、详情栏、滚动条和操作按钮。
- 右侧栏目新增“科技树（Y）”，采用全屏右滑栏目和工程/军事/指挥三条分支；只能点击右上角关闭按钮或按 ESC 收回，打开期间暂停游戏时间。
- 研发不新增显式科技资源：科技只保存项目进度；每座存活研究院按配置提供 1 点/秒，前台与后台位面研究院叠加。没有手动项目时，从当前满足前置条件的未完成科技中随机选择，切换项目保留已有进度。
- 初版科技开始控制建筑卡片、出兵建筑兵种选项、研究院能力项目、建筑回收、RTS 指挥、驻守/集结、压平视图和跨位面增援；未在科技表登记的现有基础功能默认保持可用。
- 主存档新增独立 `technologyTree` 状态；没有该字段的旧存档自动解锁全部科技，避免已有玩法被回锁。交互开发工具“技能”页新增“一键解锁全部科技”按钮，供功能测试。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：指挥模式单建筑独立集结点（2026-08-21）
- 指挥模式可左键单选仓鼠兵营及全部配置型出兵建筑，右键当前位面可达位置设置该建筑的独立集结点；
  建筑选中提示改为直接描绘真实 footprint 底面，使用呼吸式金色填充和双层轮廓，不再用椭圆估算；
  独立状态同步显示，地面连通与墙顶/楼梯正式路线在保存前校验。
- 独立集结按位面、世代和建筑稳定ID保存，优先于左侧全局跟随/待命/自订状态，但不会改写该状态；前台新兵
  出生及出口朝向均优先读取所属建筑设置，后台新增编制也不会被全局跨位面兵线抽走。存档升级v3并兼容
  v1/v2，建筑销毁或出售时同步清理记录。
- 左侧兵线“自订”只有成功保存全局点位后才清空所有建筑独立集结；取消取点、不可达或设置失败不清空。
- 集结标识升级为持续的金色流动虚线：从当前画布上沿外的天空沿视觉Z轴下落，以箭头和落点环指向
  `physicalY-z` 的真实地面、楼梯或墙顶位置；镜头平移、缩放时实时校准，独立集结点优先显示。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：墙顶法系特效图层统一与仓鼠建筑错层排查（2026-08-20）
- 火球、冰锥的飞行主体、尾迹、爆炸/碎裂、冲击波与火球环境光统一读取物理地面 Y 和发射时
  高架深度上下文；显示位置仍按 Z 抬升，墙顶特效不再被承托墙贴图覆盖，楼梯保持普通遮挡。
- 排查仓鼠士兵建筑错层：建筑 footprint 仲裁入口与玩家一致，剩余风险集中在友军自然深度仍由
  贴图位置和 `footOffsetY` 拼接；火枪手已发现 `spriteOffsetY=-36` 与 `footOffsetY=17` 不配对，
  矿工缺少显式脚底配置。正确约束与全兵种/建筑角点回归范围已沉淀到技能分卷。
- 按项目约定未运行测试、构建或浏览器实机验证，由用户测试。

### 对话：阴影系统唯一性审计与收口（2026-08-19）
- 全量枚举阴影渲染路径：静态太阳影唯一出口 = 共享结构阴影层（结构 hull 多边形 +
  静态胶囊椭圆，重叠先几何并集再单填充）；单位胶囊影（玩家/怪/友军/NPC）有意
  独立保留（移动体逐帧更新）；地牢房间装饰接触影（ObstacleSpawnSystem，scene7
  专用、非太阳驱动）为另一场景的独立语义。
- 审计抓出的死链：wall-system 散布障碍的 `useProjection` 门用
  `textures.exists(key+'_projection')` 判定，而投影贴图预加载早已清理——门永假，
  仙人掌/雪松静默退化为胶囊椭圆。修复：门改为 manifest 是否有 shadowSilhouette 列，
  四类仙人掌 hull+剪影路径实机复验复活（贴合、轮廓正确）。
- skill/06 工作流同步：删除"BootScene 预加载 _projection"过期步骤与提交约定。
- 契约 40/40、regressions 179/179、wall-depth 10/10、render-order 5/5 绿。


### 对话：墙顶、楼梯与墙下射墙顶的统一弹道规则（2026-08-20）
- 正式墙顶`wall_walk`发射的弹体在任意方向上忽略发射瞬间的整块承托墙链，取消平台体积、出缘角度与动态改瞄高度；楼梯`stairs`发射不获得穿墙豁免，非承托墙仍照常阻挡，墙顶20%最大射程奖励保持不变。
- 墙体碰撞由布尔值升级为返回最近墙体、交点和轨迹参数。墙下向墙顶单位射击时，只有弹道精确穿过目标躯干模型且首堵墙属于目标承托墙，才让模型命中优先；否则友方弹体被友方墙截停且墙体零伤害，敌方弹体对友方墙结算弹体直接伤害。
- 标准枪弹、敌方开火预判、法系飞行物、符文剑及射手/斥候/火枪手/露娜自定义弹道接入统一墙体上下文；删除旧`wallClearance/platformSeamTolerance/visibleTargetFractions`配置并更新专项契约。复查时修正“楼梯来源误用墙顶模型例外”，专项契约27/27、嵌墙契约10/10、友军战斗契约8/8、目标运行时代码语法14/14通过；未运行构建或浏览器实机验证。
...
### 对话：跨位面兵线v2事务、后台驻军与队友驻留升级（2026-08-20）
- 修复跨位面单位工厂写入只读`hp`导致的物化异常；增援队列改为成功后提交，创建失败、无安全落点和切换
  回滚时保留记录，不再静默丢兵。兵线存档升级v2并兼容v1，保存稳定单位ID、原生产建筑/位面/世代、血量、
  有效状态、目标承载面和队友驻留位置。
- 产兵建筑人口统一统计本地、途中和异世界存活单位；快照保存建筑ID与`troopLineDeployed`，前后台补员共享
  原建筑总编制口径，来源按建筑ID、位面和世代联合匹配。离场时仍在走向传送门/集结点的单位会转为权威
  增援记录，不因快照只保存兵种数量而丢失命令。独立驻军加入后台入侵DPS并按配置化吸收比例承担伤亡，回场只恢复幸存单位。
- 跨位面落点接入墙体/建筑/单位/边界安全检查，高架集结保存墙体与楼梯ID并从传送门重新规划路线；非跟随
  队友保留原位面驻留并暂停AI/渲染，观察与入侵支援仍不隐式带兵，位面毁灭时剧情队友撤回主城。RTS编队改按稳定ID恢复，
  兵线面板补充驻军、途中与编制统计。目标位面暂时没有安全落点时每750ms节流重试，空间释放后无需重新进位面即可继续物化。
  按项目约定未运行测试、构建或浏览器实机验证，由用户测试。
### 对话：阴影重叠加深根治（2026-08-19）
- 残留源一：共享层淡出环按质心放大三档 scaled fill，相邻阴影的环互相叠乘
  （0.30+0.30→0.51 预层透明度）——楼梯/墙排接缝处肉眼可见变深。
- 残留源二：树木/散布障碍物的胶囊影是独立 Sprite（alpha 0.1925），与结构层
  跨系统叠加（合成≈0.35，近两倍深）。
- 根治（用户口径"重叠调成一整个、同一强度"）：**重叠/相贴的阴影 job 先几何并集
  再绘制**——`_mergeShadowJobsIntoClusters` 顶点互含/边相交聚簇（bbox 外扩 8px 粗筛），
  簇内用 `getUnionOfPolygons`（getUnionShadowPolygon 的 N 元推广）在太阳帧包络合并成
  单一多边形，一次填充统一强度、只沿外轮廓描边一次（簇内无接缝）；互不相交的
  保持独立（禁止桥接远处建筑）。淡出环同步废弃为轮廓单次描边（lineStyle 6/0.32）。
- 静态胶囊（树/散布障碍）改为旋转椭圆 16 边形直接作为多边形 job 汇入共享层
  （精灵仅留作注册键）；移动单位（玩家/怪/友军/NPC）仍是独立胶囊 Sprite——
  单位站在建筑影子里会略深（物理合理，唯一保留的跨系统叠加）。
- 复验：双仓库近距+墙排+双楼梯集群 09:00/16:30 全场阴影色深均匀无加深带；
  契约 36/36、regressions 179/179、wall-depth 10/10、render-order 5/5 绿。

### 对话：掩体墙阴影回到 footprint 凸包（2026-08-19）
- 用户口径"墙壁阴影基于 footprint 生成"：方块墙/各档护墙（`_isDefenseCover`）
  在 ensure 里 `_silCache` 置 null——多边形本体回退 footprint 凸包（方块墙=1×1
  菱形、护墙=面线薄条），沿全局影向挤出；剪影只保留实测高度（measuredHeight）。
  墙贴图内容在大画布内偏移大，剪影实体四边形比 footprint 宽近一倍且沿墙斜向歪轴。
- 复验：实机 4 块方块墙成排——09:00 一条整带贴墙根、16:30 统一朝左；
  契约 36/36、regressions 179/179 等全绿（footprint/elevated-ranged 各 1 项红
  为并行会话在途，与本次无关）。

### 对话：城墙楼梯整体太阳阴影 + 影向统一（2026-08-19）
- 根因（实机探针 + 主轴分析坐实）：楼梯由多块主体拼接（segmentSprites 逐段 z 抬升），
  实体级单影只盖第 0 段（"碎裂"）；贴图是对角斜墙，剪影展开主轴偏离全局影向
  40°~52°、e1/e2 镜像互歪（"七扭八斜"）；同位置建筑影向偏差仅 4°~9°。
- 定稿（用户口径"视作一块整体"）：全段 footprint 顶点合并成梯轴长带，单个凸包
  沿全局影向统一挤出——一条连续影带，与全场建筑严格同向；高度取各段剪影实测
  最大 measuredHeight（影长不回退）。`_ensureStairSunShadows` 专用注册，
  `_structureSunShadows` 对楼梯回到单值（分段数组方案仅存在了一版）。
- 复验：实机双楼梯（e1/e2）+ 研究院 + 仓库 + 基地 09:00/16:30 影向一致、
  影带无分段接缝；契约 36/36、regressions 179/179、wall-depth 10/10、
  render-order 5/5 绿。

### 对话：建筑阴影贴合审计——manifest 失配根因与保鲜护栏（2026-08-19）
- 换思路审计：放弃简化复刻，直接导出运行时真实多边形（`_polyState`）+ 实机用
  构造器生成建筑（ProducerBuilding/HamsterBarracks/HamsterHut）核对——运行参数与
  离线计算完全一致（锚点 ±1px、measuredHeight 精确吻合），算法贴合成立。
- 根因：church/research_institute 贴图被替换后 manifest 未同步重建（条目仍是旧
  坐标系：1239/1126 宽 vs 新图 1039/1051，frontY 差 10~14px，15~23 列越界）→
  两楼阴影整套错位。其余 34 资产与贴图一致。
- 修复：手术式重建这两条目的 alphaBBox/shadowSilhouette 与 4 张派生光照图；
  契约测试 `test-sun-shadow-geometry.mjs` 新增保鲜护栏（逐资产对照当前贴图
  bbox/剪影列/frontY，±2px 容差）——换贴图不重建 manifest 直接红测。
- 契约 36/36、regressions 179/179、wall-depth 10/10、visual-anchor 11/11 绿；
  footprint 套件 32/33（挂的 1 项为墙门遮挡源模式契约，系并行会话在途重构，
  与本次改动无关）。审计工具：tools/_audit-realpoly.mjs、tools/cdp-shadow-fit-dump.mjs。
- 按项目约定未做浏览器实机验证，交由用户测试。
...
### 对话：共享阴影层深度修正——道路之上（2026-08-19）
- 层深 −998（道路 −995 之下）导致阴影被道路完全遮挡；当时放道路之下是应
  "道路残片"投诉——复查确认残片实为交叠加深在路面的表现（共享层已根治），
  物理上阴影应压暗路面如压裸地。
- 修正：`STRUCTURE_SHADOW_LAYER_DEPTH` −998 → −994（地板块 −1000 与道路 −995 之上、
  实体之下）——阴影在道路上正常可见且不遮挡实体。
- eslint 0 error + build 通过。建议实机检查点：①阴影跨道路连续可见、路面
  同深压暗；②道路与裸地之间无遮挡截线；③交叠不加深保持。

### 对话：世界切换栏目状态错乱修复（2026-08-20）
- 核对世界按钮、场景编号和 `SceneManager` 分发后确认映射本身正确；故障来自观察模式与切换失败时的
  非原子状态写入。切换入口现在显式返回成功/失败，拒绝重复加载、无效场景和未建传送门，面板只在
  确认抵达目标世界后同步指挥模式，避免“界面像已切换但实际仍在原世界”。
- 普通切换新增完整事务回滚，覆盖原场景实体/特效/相机、玩家坐标、观察模式、本体世界及各位面坐标；
  观察态回滚不再错误插入玩家。观察主城不生成或移动本体、不消费重生点，也不会污染主城离场坐标。
- scene9~scene11 返回时恢复各自离场坐标，不再一律重置到传送门；地牢中禁止从世界栏目直接跳走，
  防止绕过探险成功、失败或放弃的统一结算。已补充静态回归契约；按项目约定未运行测试、构建或浏览器验证，交由用户测试。
...
### 对话：铁栅栏门太阳阴影——世界面线锚定专用路径（2026-08-19）
- 审计结论：门无阴影的两层根因——①`BuildableGate` 无 spriteCfg（中立精灵是
  白色占位圆，剪影无从挂载）；②门的地面接触是**对角面线**（faceA→faceB），
  不是 V 形底座，剪影实体四边形模型不适用。
- 实现：`_ensureGateSunShadow` 专用注册——body = 门自身 iso footprint 薄矩形
  （沿面线，正确）；剪影取 `cover_gate_<grade>` 帧 0（已在清单，FRAME_CROPS 裁剪），
  列按贴图接触跨度映射到实体 `_faceLine`（世界面线真源）；`_facingLeft` 时
  列与前顶点一次性镜像。env `getSilhouetteShadowPolygon` 新增 `groundLine` 选项
  （近边=面线、离线偏差沿屏幕 +y 展开、内容高=自身列高位移）。
- test-sun-shadow-geometry 35 项（groundLine 四点：端点对齐/位移/无面线回退）+
  footprint 33 项 + regressions 179 项 + wall-depth 10 项 + eslint 0 error + build 全绿。
- 建议实机检查点：①铁栅栏门出现太阳阴影、贴门面线；②开门动画期间影保持
  （剪影取关闭帧 0，可接受）；③门与邻墙阴影交叠同深（共享层）。

### 对话：压平视图范围技能与连锁法术 Z 轴全面修复（2026-08-20）
- 弹道终点统一为“夹取本帧真实终点 → 三维墙体线段检测 → 命中/到射程结算”；修复火球、冰锥最后一帧
  可越墙命中，以及符文飞剑越过最大射程前未检查墙面的边界。
- 新增物理撞击点承载面解析：投射物保持真实 `x/y`，按 `impactZ` 在地面、楼梯、墙顶与连接面中选择
  最近层；不再用弹体中心高度反推屏幕坐标，避免窄墙顶和楼梯边缘把爆炸结算到相邻层。
- 符文飞剑快照真实 `flyZ` 与墙体上下文，飞行、躯干碰撞和墙体阻挡统一走三维门禁；火球、冰锥、
  陨石、熔岩、暴风雪、地面持续区、闪电、圣光及其粒子/浮字统一按 `displayY = physicalY - z` 显示。
- 闪电与圣光的鼠标候选改按投影位置排序，并在施法释放帧重新验证目标存活、可命中、射程、阵营/建筑
  限制和真实高度 LOS，禁止读条期间目标换层、绕墙或失效后仍结算。
- 高度上下文改为缺失即关闭命中；所有 `Ground*`/`Vertical*` 调用必须显式传入地表或体积快照，禁止
  静默回退二维跨层命中。压平开关仍只影响显示，不参与任何战斗判定。
- 按项目约定未运行测试、构建或浏览器实机验证，交由用户测试。

### 对话：墙体与楼梯统一拓扑、移动支撑与旧协议清理（2026-08-20）
- 新增`elevated-topology.js`，统一缓存同格多墙、邻墙连接面、连通分量、楼梯入口和空间桶；重叠墙不再
  覆盖索引，楼梯只允许吸附外露且整组占地合法的墙面，拓扑变化统一递增路线revision。
- 高架移动改为从`lastValidated`到本帧落点连续扫掠；无真实支撑时回到最后有效面或当前墙链安全中心，
  禁止伪造`wall_walk`，且只有楼梯底部Portal允许把z切回地面。
- 远程投射物的二维脚底/躯干兼容命中增加真实弹道垂直区间门槛；楼梯与墙顶射击仍按目标高度生成弹道、
  按交点高度判断墙体阻挡，但不会再隔层提前命中未与弹道相交的单位。
- 图只保留RTS/BFS规划职责，即时移动固定使用真实墙顶/连接面/踏步；删除运行时graph/surface双模式、
  随机角度脱困和重复墙顶路线拼装，墙/连接/楼梯查询改用局部空间桶。
- 完成射击台遗留协议迁移：统一`WallStaircase/staircases/_surfaceStaircase/z`命名，删除旧贴图加载、旧资产、
  旧平台探针与标定工具；当前存档仍使用`kind=wall_staircase, stairVersion=2`。
- 按项目约定未运行测试、构建或浏览器实机验证，交由用户测试。

### 对话：位面生命周期短期路线图阶段5/6收口（2026-08-20）
- 新建或重建传送门后获得配置化 1 游戏日保护，保护截止时间随位面状态保存；入侵真实候选池排除
  保护位面，所有候选受保护时保留当前进度，待保护结束后再选点。
- 审计修正普通帧入侵增量：倒计时改为读取环境时钟本帧真实增量，太阳时钟暂停、游戏暂停和地牢冻结
  时保护期、入侵与后台时间保持同一口径，不再出现倒计时走动但保护期不结束的分叉。
- 传送门耐久新增 50%/25%/10% 配置化分段预警；顶部入侵 HUD 显示实时耐久和严重级别，异世界
  入侵可直接将玩家本体转移过去支援，10% 阶段和毁门时提供撤离提示。
- 审计补齐地牢边界：地牢探险期间支援按钮隐藏且方法入口二次拦截，必须先走原有成功/失败/放弃
  结算结束探险，不能通过跨世界支援绕开地牢结果记录。
- 交互开发工具新增“位面”页签，显示生命周期状态、世代、保护时间、生成种子、快照摘要与真实候选池；
  支持通过统一游戏时钟推进 1/5 天，以及复用正式毁灭事务模拟摧毁选定传送门。
- 入侵波次重平衡、新怪物和更多地牢—世界关系未混入本轮，保留为独立内容阶段；补充跨系统回归契约，
  按项目约定未运行测试、构建或浏览器实机验证，交由用户测试。

### 对话：建筑面板楼梯与道路缩略图调整（2026-08-20）
- 城墙楼梯面板图标显式锁定当前默认方向`e2_pos`的正式下段贴图，与场景默认楼梯方向一致，不再依赖
  面板运行时纹理字段的隐式回退。
- 从4帧`building_road_tiles.png`无损裁取第0帧，新增独立`building_road.png`作为道路面板缩略图；
  场景道路仍使用原4帧精灵表和坐标哈希变体，不改变铺路表现。
- 按项目约定未运行自动化测试、构建或浏览器实机验证，交由用户测试。

### 对话：阴影强度再降 50%（2026-08-19）
- 0.385 → 0.1925（继上轮 −30% 后再 −50%，用户口径"还是太深"）；
  静态/动态同一常量，夜影衰减公式不变（白昼 0.1925、黄昏 0.029、深夜 0）。
- test-sun-shadow-geometry 32 项（新基准锁定）+ Vite build 通过。

### 对话：叠墙环境楼梯吸附合法性修复（2026-08-20）
- 修复楼梯占地检查忽略整条连通墙链的问题：现在实体重叠只忽略实际连接的目标墙，楼梯段路径上的
  内层墙、叠墙和其它阻挡墙会正常判定为不可建造；连通墙链仍仅用于墙顶导航和边线采样兼容。
- 楼梯吸附只返回鼠标范围内最近的合法外露墙面；若附近候选全部不可建造则取消并隐藏预览，不再让
  不可到达墙显示红色预览或抢占更远的合法吸附位置。
- 预览视觉段改为在候选合法性筛选完成后仅为最终候选生成，避免叠墙较多时每次鼠标移动为所有失败
  方向重复分配楼梯预览数据。
- 按项目约定未运行自动化测试、构建或浏览器实机验证，交由用户测试。

### 对话：楼梯—墙顶接缝卡死收口（2026-08-20）
- 修复高架路线仅按平面距离和Z判定到达的问题：墙顶/楼梯节点现在必须取得匹配的surface身份后才推进、
  完成并释放楼梯预约，避免单位仍在楼梯桥接区却提前清空墙顶目的地。
- 楼梯地面入口恢复为ground阶段，单位继续使用原有地面A*抵达入口；进入真实踏步后才暂停地面寻路。
- 增加`handoff_to_wall/handoff_to_stairs`阶段和目标surface选优；顶部接缝连续3帧零进展时，沿真实
  连接中心线最多3px定向恢复，不启用随机脱困、不扩大墙面或楼梯碰撞容差。
- 补充RTS墙顶身份交接回归断言；按项目约定未运行测试、构建或浏览器实机验证，交由用户测试。

### 对话：墙/楼梯阴影太短 + 抬升结构影浮空修复（2026-08-19）
- 定位（离线模拟复核）：①墙/楼梯的 data.height 由 footprint 半径估算（方块墙 108），
  远低于贴图实测立面高（墙 ~199）——正午延长段被低估一半以上，视觉"太短"；
  ②楼梯/射击台这类抬升结构的精灵屏幕位置含 z 抬升，阴影锚在精灵位置会浮空、
  与底座脱节（看着像方向不一致）。
- 修复一：`_resolveShadowSilhouette` 新增 iso 地面线以上内容高的 75 分位实测
  `measuredHeight`；`_syncStructureSunShadows` 提前解析剪影缓存并用实测值覆盖
  height/maxOffset（方块墙正午延长 20→37、黄昏上限 54→99；楼梯按其真实内容高），
  剪影迷航归一 maxHeight 与 profile.length 统一到同一实测值。
- 修复二：抬升结构（`_isWallStaircase`/`_isFiringPlatform`）阴影锚点改为
  footprint 四边形前顶点（地面真源），不再用抬升后的精灵位置——影子落回地面。
- 离线模拟：方块墙阴影成型为立方体短影、楼梯影落在地面四边形前缘；
  test-sun-shadow-geometry 32 项 + footprint 31 项 + test-regressions 179 项 +
  eslint 0 error + build 全绿。建议实机检查点：①墙影沿墙展开且方向与建筑一致；
  ②楼梯影在墙脚地面而非浮空；③门影贴合；④相邻楼梯交叠同深。

### 对话：要塞式建筑压平视图第一阶段（2026-08-20）
- 世界-122/123/124/125 在建筑面板、RTS 指挥或观察者上下文中支持按空格切换压平视图；捕获阶段拦截该次
  Space，避免同时触发玩家闪避，退出这些上下文或切离持久世界时自动恢复普通视图。
- 修复指挥按钮保留焦点时 Space 仍进入玩家输入并触发翻滚：按钮点击后主动失焦，模式切换清理已有 Space，
  合法上下文中的键盘事件在捕获阶段始终与角色输入隔离；文本框仍可正常录入空格但不会切换压平或翻滚。
- 压平只接管 Phaser 显示层：建筑、塔、门、楼梯分段和非障碍墙件改为真实 footprint 平面投影；保留现有
  建筑地基并下放到地表层，门的开闭、楼梯上升方向和建筑类别使用不同轮廓表达。
- 建筑实体、碰撞、寻路、生命、选取、弹道和高度状态均不修改；压平时只回收建筑附着火焰/烟雾及建筑太阳影，
  环境障碍阴影继续保留，退出后按当前状态自动重建。
- 高低层专项收口后完成压平适配静态复核：现有近战高度门禁、远程高度 LOS、弹体逐段墙碰撞和 RTS
  楼梯/墙顶路线保持唯一真源；本轮没有修改这些战斗与导航判定。
- 墙顶单位新增青色高度环/上箭头/真实 z 投影线，楼梯单位新增黄色阶梯标记；RTS 属性面板显示当前层和高度。
  同一屏幕点的墙上/墙下单位可连续点击轮换，并优先当前已选层，避免压平后选错目标。
- 通用弹体被隐藏墙体真实截停时显示短时橙色阻挡标记；该反馈只消费既有碰撞结果，不改变弹道或伤害。
- 压平视图补充指令层级反馈：地面/墙顶/楼梯移动点分别用绿/青/黄菱形标识，攻击目标显示红色准星；
  高层目标保留真实 z 投影线，混层编队显示外圈并提示各单位按自身承载层寻路。复数单位面板同步统计各层数量。
- 完成范围技能与连锁法术统一 Z 轴结算：新增双份 `combat-elevation.json` 作为同层容差/飞行体积补偿真源；
  `elevation.js` 提供施法者、落点及真实飞行高度快照，`Ground*`/`Vertical*` 形状统一先判高度再判 footprint。
- 火球/冰锥、暴风雪、陨石/熔岩、毒雾/燃烧区、火焰护甲、旋风、无人机、符文剑、冰墙及敌方范围技
  均保存并消费明确承载层；持续区不会因施法者随后上下墙而换层，墙上/墙下投影重合不再互相结算。
- 闪电、雷暴领域和感电过载改为每一跳从上一目标重新检查真实高度 LOS，墙体或高度阻断会截断后续传导；
  全部规则独立于压平开关，普通与压平视图继续使用同一命中结果。
- 按项目工作约定，本轮未运行自动化测试、构建或浏览器实机验证，交由用户直接测试。

### 对话：能源经济审计基线整改落地（2026-08-20）
- 按确认口径只平衡自动矿工，不纳入玩家、露娜、伊莉丝等普通队友：自动矿工从单位配置读取
  `energyGatherRatio=0.125`，其他采矿来源继续使用全局50%采集倍率。
- 矿工营地结构参数拆到 `hamster-miner-camp-building.json`，矿工基础属性保留在单位配置，升级成长与
  逐级费用保留在 `building-upgrades.json`；前后台统一消费 `miner-economy.js` 的逐击取整与暴击期望公式。
- 采矿/伤害/攻速每级调整为+6%/+1.5%/-1%，矿工数量模块上限2级（满编3只）；采矿能源费逐级
  250~800，增员两级5000/8000，模块费用不再由面板固定写死。
- 首座仓库在当前世界无仓库时免费，后续仍为500金币；单仓容量由5000提高到15000，实际建造成本
  写入建筑实例，免费仓出售不会产生返利。
- 民兵/斥候/战士/盾卫/射手/火枪/牧师/轻骑/骑士生产能源费分别为
  50/75/125/150/120/180/240/220/300；矿工正常补员100能源且至少保底1只免费。前后台均扣费，
  快照恢复物化不重复收费，能源不足时保留生产进度并等待资源。
- 全局被动能源只由前台世界结算；无前台世界时仅落到最近离开的一个后台快照。什一税仍按各世界
  实际牧师数量结算。墙/塔修理提高到8/7耐久每能源，所有建筑与塔的回收按剩余耐久比例衰减。
- 按项目约定未运行自动化测试、构建或浏览器实机验证，交由用户测试。

### 对话：单位阴影简化——胶囊拉伸椭圆为唯一做法（2026-08-19）
- 用户口径：玩家/怪物/友军单位不需要建筑那样的精细轮廓。现状盘点：
  `_syncEntityShadows` 本就全覆盖（玩家+敌人 enemy/agent+友军+中立 NPC/靶），
  胶囊接触影 + 高档位帧剪影（unit_projection）双套并行——帧剪影那条正是
  "精细轮廓"路线，单位上用不上且每帧吃 canvas 采集/256 缓存/回收链。
- 落地：`ensureShadow` 只留胶囊分支（椭圆长轴沿太阳方向拉伸、宽深随
  footprint=groundRadius 唯一真源、随仰角收放、透明度 0.385×夜影衰减与建筑
  同口径、深度跟随本体仲裁后 −0.1 保留墙体遮挡继承）；
  删除 `_getDynamicProjectionTexture`/`_clearDynamicProjectionTextureCache`/
  `_refreshDynamicProjectionCachePolicy` 方法与 `_dynamicProjection*` 全部字段
  及 create/update/shutdown 钩子。
- test-sun-shadow-geometry 32 项 + footprint 31 项 + test-regressions 179 项 +
  eslint 0 error + Vite build 全绿。建议检查点：①玩家/怪物/友军脚下椭圆影
  随太阳偏移、正午短晨昏长；②深夜无影；③单位过墙时影随本体隐藏。
...
### 对话：阴影系统审计整改落地（2026-08-19）
- 按审计建议整改（不改设计、只清性能与死链）：
- **性能（最大项）**：多边形逐帧重建改 epsilon 脏检查（太阳角 <0.11° 或延长段 <0.5px
  或顶点签名不变时复用缓存多边形，子像素步进无跳变），共享阴影层干净帧整体跳过
  clear+重画（Graphics 指令缓冲自留存）；场景重启时复位层脏检查状态（防首帧误判
  干净帧导致阴影不可见）。稳态开销≈0，太阳移动时按需重算。
- **死代码/死数据清除**：BootScene 删除派生图预加载（雪松/仙人掌/建筑
  projection+silhouette 运行时已不消费，剪影数据走 manifest shadowSilhouette 列）；
  manifest 删除 base/contactQuad 死字段（anchorMode 保留占位——建筑贴图回归测试
  仍断言它）；GameScene 删除 ShadowDebug.setInset（唯一 anchorInset 写入方）、
  _clearStructureShadowBakeCache 空占位方法、_silCache.maxHeight 死字段。
- 契约：test-sun-shadow-geometry 改 manifest 口径为 shadowSilhouette 覆盖（32 项全绿）；
  footprint 31、layer-assets 14/16（挂的 2 项是 barracks/blacksmith 配置显示参数——
  并行会话正重构 producer-buildings.json，待其收口）；eslint 0 error + build 通过。
- 未做的审计建议（留档备择）：ASSETS 手列清单即"哪些资产需要阴影"的过滤器，
  全目录扫描会把地砖/装饰都纳入（不该要）；若要更自动，建议贴图入库钩子里
  强制跑 build-lighting-maps.py，而不是扩大清单。

### 对话：世界位面解锁、统一时间与五日入侵模型（2026-08-20）
- 新增 `world-system.json` 配置真源和 `WorldProgressionSystem`：世界位面按地牢通关记录解锁；
  当前测试链为“僵尸地牢初级（zombieBeginner）通关 → 世界-125 可构造”。世界-123/124
  暂不开放构造，但保留完整世界运行时，待后续地牢关系配置。
- 世界传送门建筑新增“构造传送门”入口，只列出满足条件且未建成/待重建的世界；无候选时显示
  “暂无可以构造传送门的世界”。首次构造免费；摧毁后按配置消耗 1000 金币和 2500 能源重建。
  未建成或已摧毁的目标会从传送列表和主神空间实体入口拦截。
- 新增 `WorldInvasionSystem`：以 `EnvironmentLightingSystem` 为唯一时间源，每 5 个游戏日从
  已建传送门的世界中随机选择目标；波数、数量、怪物池、解锁日和按周期/波次成长倍率全部配置化。
  前台世界复用真实 `DefenseSystem` 刷怪，后台世界按快照防御力抽象结算；传送门为最终目标。
- 地牢探险期间世界时间、后台生产和入侵倒计时统一冻结；成功、失败、安全撤离或主动放弃均推进
  入侵进度，F/E/D/C/B/A 分别增加完整周期的 10%/20%/30%/40%/50%/60%，成功同时登记世界解锁。
- 传送门被摧毁后不再保留位面遗迹：立即删除目标世界的完整快照与旧坐标，位面内玩家/观察者
  强制回主城，主城对应入口同步撤除；未重建前任何入口都不能载入该世界。
- 重建传送门只恢复网络资格，下一次进入按该世界基础规则从零生成，不恢复旧建筑、掉落物、单位、
  矿点或道路。为避免最后一个传送门被毁后软锁，主城仅在全网断线时开放旧传送门应急重建，
  不开放从未搭建过的新世界。
- 新增位面生命周期短期路线图并完成第一阶段：传送门首次构造或重建后立即生成基础位面快照；
  新游戏初始世界、旧档恢复和入侵选目标前都会补齐“已建门但无快照”的异常状态。基础快照只含
  时间锚点、满耐久传送门和空建设状态，首次进入按场景规则物化资源，离场后升级为完整快照。
- 完成生命周期第二阶段：位面状态统一为 `LOCKED/AVAILABLE/ACTIVE/DESTROYED/REBUILDING`，
  旧布尔字段保留为兼容派生值；v1 存档会迁移到新状态机。首次构造世代号为1，每次重建递增，
  快照、后台模拟、入侵活动与传送门实体统一校验 `worldEpoch`，旧波次回调或旧实体死亡事件不能
  再摧毁已经重建的新位面。
- 完成生命周期第三阶段：位面毁灭改为按 `sceneId + worldEpoch` 幂等的事务；先标记毁灭并清除
  快照、旧坐标和入口，再撤离玩家/观察者。实况捕获与存档序列化会拒绝未建门位面，毁门与保存
  同帧发生也不能复活旧建筑；强制回城绕开通用 rollback，失败后彻底清场并最多重试三次。
- 完成生命周期第四阶段：`world-system.json` 新增配置化 `resetPolicy`，统一基础模板、生成版本、
  按世代种子、资源规则、毁灭清除项与全局保留项。传送门/快照持久化生成版本与种子；同一世代
  的地表点缀、障碍和能源点稳定复现，重建进入新世代后换新布局。v1/v2 旧档自动补生成元数据，
  不覆盖已有完整建设快照；重建保护天数只预留配置，实际筛选留给第五阶段。
- 世界-122/123/124/125 统一接入建筑、能源点、矿工、小屋、兵营、配置产兵建筑、道路、RTS 指挥、
  多世界快照及后台生产；各世界切换时按 sceneId 独立捕获/恢复，存档新增 progression/invasion/scenes。
- 顶部新增全局入侵进度条：任意世界实时显示五日倒计时或目标世界与当前波次；地牢内保持显示但暂停。
- 按用户工作约定，本次未运行自动化测试、构建或浏览器实机验证，交由用户直接测试。

### 对话：高架远程战斗全面审计整改（2026-08-20）
- 修改前备份：`backup/elevated-ranged-audit-fixes-20260820-104626`。
- 仓鼠牧师自动治疗/攻击、RTS指定攻击和目标筛选统一读取墙顶魔法射程；
  `HolyLightSystem.triggerOn`恢复最终射程与带高度LOS门禁。墙上600→720，近墙可施法，
  真正被远墙遮挡时移动调整位置，不再隔墙结算。
- 火球/冰锥在发射时为每个spike快照`maxRange`；飞行期间上下墙不再改变已发射弹体射程。
  同帧同时跨过目标点和最大射程时，射程外目标先按maxRange终止，禁止越界命中。
- 露娜普通光球、射手、斥候和火枪手在决策与出膛两处检查统一高架LOS：
  优先选择当前射程内可直射目标，远墙遮挡时不再原地持续浪费弹药。
- 魔法倍率拆为“最大射程”和“技能作用范围”两条：墙顶20%只增加施法者到目标的maxRange；
  武器改造仍可扩大闪电/圣光锁定容差和闪电传导距离，墙顶不再额外扩大这些范围。
- 2px墙顶净空快照当前承托墙、墙段和发射点，只在220px邻域生效；同墙链远端及其它墙恢复真实墙高。
  配置允许显式设为0，不再被`|| 2`强制覆盖。
- `test-elevated-ranged.mjs`扩展至22项并加入`npm test`；浏览器探针改为复用真实已加载模块单例，
  实际推进近墙/远墙弹道并验证枪手LOS、牧师720射程与远墙遮挡，最终`errors: []`。

### 对话：出兵建筑与友方单位全面审计整改（2026-08-20）
- 友军六维由展示值改为真实战斗入口：固定攻击配置作为初始基准，当前物攻按比例影响伤害，
  幸运/暴抗参与1.5倍暴击判定；友军承伤统一加入物防/魔防减伤。十种仓鼠实体不再直接扣
  原始HP。
- 激励从直接乘除运行时字段改为 `_inspireMul` 临时修饰器，移动系统与物理伤害动态消费；
  激励期间升级、装备/六维重算后，到期不再把新属性错误除低。
- 单位死亡同步从所属 `units/miners` 列表删除，解决长期补员后历史死亡引用无限增长。
- 世界122快照新增 `unitRoster`，兵营/靶场/草屋/骑兵学校等混编部队逐兵种保存恢复；
  出口预约失败的恢复缺额按原兵种800ms队列补齐，后台DPS逐兵种计算。
- 后台模拟按研究真实完成时点分段：被动能源、快速募兵、结构生命、横扫、标记和激励不再
  回溯整段离线时间；结构HP研究保持“已损失生命量”；牧师什一税加入后台余数累计与战报。
- 全局铁匠/研究能力增加跨建筑占用检查，等级提升和读档都按 `maxLevel` 钳制，杜绝多建筑
  并行把Lv10推到Lv11。
- 新建出兵建筑首只单位改为等待完整生产周期；出售按实际 `_buildCost/_buildCurrency`
  退款。军营成本、尺寸、周期、上限迁入 `data/hamster-barracks-building.json` 唯一真源。
- 金币读取/扣除支持多个金币堆；升级百分比改为累计后保留小数，2.5%/级与1.5%/级不再
  显示成3%/2%。切换兵种提示改为“按新周期重新计时”。
- 补齐牧师仓鼠仇恨分类、骑士移动朝向/烟尘、火枪手卡死看门狗、战士指令打断、图鉴军营
  来源；射手死亡表由错误11帧修为真实10帧，矿工采矿PNG无损重存清除尾部异常数据。

### 对话：仙人掌镜像对齐 + 墙壁/门/楼梯接入阴影系统（2026-08-19）
- **部分仙人掌没对齐**：镜像实例（flipX）的接地线/前顶点未随列镜像——非对称贴图
  （单臂仙人掌）镜像后接地计算全错。修复：`_resolveShadowSilhouette` 在 flipX 时
  一次性镜像列与前顶点（mirrorX = texW−x），下游坡度截断/地面线/展开全部用镜像后
  几何，flipSign 归一。
- **楼梯交叠加深排查**：楼梯实体走 `_neutralSprites`→结构阴影→共享层，
  核心 alpha 1 恒等不存在加深；其贴图为 Blender 烘焙自带 AO 暗面
  （并行会话资产侧），若仍加深请确认其贴图烘焙光照而非本系统。
- **底部墙壁和门接入阴影**：`build-lighting-maps.py` 清单新增 obstacle_block、
  wall_stair_1x1_h/v、楼梯 8 方向变体、cover_gate_A~D（新增 FRAME_CROPS 帧裁剪，
  16 帧 spritesheet 取关闭帧 0 量测）——36 组派生数据；`_syncStructureSunShadows`
  移除 `_isDefenseCover` 排除（方块墙/门获得太阳阴影，能源矿仍排除）。
- test-sun-shadow-geometry 33 项 + eslint 0 error + Vite build 通过。
  （旁注：并行会话正改 blacksmith 贴图/配置，layer-assets 测试一项红待其收口，
  与本链无关。）建议检查点：①镜像仙人掌影根对齐；②方块墙/门出现太阳阴影且
  相邻不加深；③楼梯交叠区同深；④道路无残片。

### 对话：城墙高架远程战斗统一算法（2026-08-20）
- 新增 `combat/elevated-ranged.js` 与 `combat/ranged-line-of-sight.js`。仅玩家/友军处于
  `wall_walk` 时获得 1.2 倍射程；楼梯途中、地面单位和敌人不加成。配置真源为
  `defense-structures.json.wallWalk.rangedCombat`，data/public 双份同步。
- 玩家枪械/弓等标准投射物由 `ProjectileFactory` 统一在发射瞬间应用射程并快照墙体上下文；
  魔法距离统一经 `getMagicRangeMultiplier` 乘算，覆盖火球、冰锥、闪电、圣光、暴风雪、
  陨星、冰墙及贯穿雷枪。
- 露娜技能选择/普通光球、仓鼠射手、斥候和火枪手的 AI 开火距离与实际飞行距离同步读取
  高架射程；露娜光球与火枪手补齐独立 `z/vz`，三类自定义友军弹道均走带高度墙碰撞。
- `WallSystem.projectileBlocked` 的矩形墙判定改为 Liang-Barsky 交叠区间，在真实进入/离开
  墙体位置插值弹道 Z，不再用整段平均高度误挡靠近射手的相邻墙。锁定魔法和雷枪视线同样
  复用该高度交点算法；远处弹道降到墙顶以下时仍正常被墙阻挡。
- 投射物视觉统一支持 `y-z` 与 `visualAngle`，深度与标准弹丸同为 `groundY+500`。
  修改前备份：`backup/elevated-ranged-20260820-101034`。
- 验证：高架远程专项 16/16、嵌墙弹道 10/10、墙梯完整探针 `errors: []`；
  浏览器运行态确认玩家投射物 1000→1200、魔法倍率1.2、墙上射手720射程可在700px开火，
  地面射手600射程不会开火，`cdp-elevated-ranged.mjs` 最终 `errors: []`。

### 对话：墙面与楼梯统一高架导航面（2026-08-19）
- 新增 `src/world/unified-elevated-navigation.js`，统一收集墙顶、楼梯踏步、并排楼梯共享缝和
  墙—楼梯桥接候选；候选一次排序、一次提交，`owner/wall/platform/z` 不再来自不同表面。
- 墙顶下楼新增 24px 提前捕获带；捕获只在脚底仍受墙顶承托、输入朝向楼梯且本帧实际位移
  正在离开墙中心时启动。整次查询保存同一份位移方向，扫掠回退和碰撞后复核不再被旧输入反吸。
- “楼梯共享缝”和“墙—楼梯桥接”改为两种独立身份；低层单位不能命中投影重叠的顶层接口，
  完整进入墙顶承托区后立即交回 `wall_walk`，解决并排楼梯横移被抬升及上墙后再次吸回楼梯。
- 桥接期间关闭同层单位分离并忽略当前连接组边线；普通墙顶/楼梯仍保留高度分层碰撞，
  `resolveCollisions` 后统一复核最终高架面。
- 修改前备份：`backup/unified-elevated-surface-20260820-015841`。验证：
  RTS 32/32、世界快照 42/42、墙深度 10/10、ESLint 0 error；
  `cdp-wall-stair-inspect.mjs` 最终 `errors: []`，五档宽度上下墙、三座并排楼梯多层横移、
  友军路线、拥挤推挤、平台倒序及碰撞后身份一致性全部通过。

### 对话：共享结构阴影层——交叠不再加深 + 道路无残片（2026-08-19 十三轮）
- 两问题同根：相邻楼梯/建筑的阴影各自绘制，交叠区 alpha 叠加自然加深
  （0.385→0.62）；且阴影 Sprite 深度=本体−0.1（≈3600）压在道路（−995）之上，
  路面留下暗影碎片。
- 修复：新增场景级**共享结构阴影层**（一个 Graphics，深度 −998 = 地板块
  −1000 之上、道路 −995 之下）。每帧将所有结构阴影多边形（凸包∪剪影片络，
  逐帧纯几何不变）汇入该层：核心两环 alpha 1 实心 + 外圈三环小 alpha 淡出，
  整层再乘统一透明度（0.385×夜影衰减）——**任何交叠都与单影同深**，
  道路瓦片在层之上自动遮盖阴影，路面不再留残片。
- 附带收益：羽化从"每层复合反解 alpha"简化为"核心实心 + 外圈三环"，
  内部重叠在层内 alpha 1 即恒等，不需要任何反解。
- test-sun-shadow-geometry 33 项 + eslint 0 error + Vite build 通过；
  未做其它自测（按用户要求）。建议检查点：①相邻楼梯/建筑阴影交叠不再加深；
  ②道路上无阴影残片；③阴影边缘淡出观感；④全天连续无跳变。

### 对话：墙顶下楼梯外侧位置卡死的显式handoff修复（2026-08-19）
- 反向全宽探针复现：横向10%~50%可以下楼，75%/90%位置一直被wall_walk保留，无法切成stairs；
  原因是墙顶状态只允许命中顶部连接凸包，同时禁止检查上段踏步。
- 新增顶部handoff接管区：输入明确朝楼梯时，上段最后35%区域返回`handoffDown`候选；
  候选排序赋予显式交接优先级并原子锁定对应楼梯，后续不再与墙顶面反复竞争。
- 横向10%/25%/50%/75%/90%五个位置全部成功从wall_walk切换stairs，0掉地、0地下帧。
- 同层碰撞先推开17px后继续向楼梯移动，10帧内重新进入stairs，全程不掉地；
  碰撞后surface、平台顺序、友军路线、多楼梯和密集Shift回归继续通过，探针`errors: []`。

### 对话：圆形仙人掌/仓库阴影三项修正（2026-08-19 十二轮，非贴图问题）
- 定位（离线模拟逐条复核，贴图本身无问题）：
  ①桶状仙人掌是球体、底部单点接触——iso 坡度截断在球底 ±1 列即触发，
  实体被截成 2px 细条、阴影成薄饼；②仓库归一参考用列高 75 分位（=241，
  多数列是石板/板条箱矮件），主屋（983）位移仍被压扁；
  ③悬空件/球面的地面线取值错误（球面下半被当地面线，影子贴球皮而非落地面）。
- 修复：①实体真源分流——建筑保留剪影接地实体，散布障碍物（无 entity）回退
  geo foot 手调碰撞四边形（球体/异形不适用平底截断法，手调 foot 才正确）；
  ②归一参考改 `data.height`（实体自身阴影高度，与 profile.length 同源同单位，
  不再被矮件稀释）；③地面线规则改 `max(贴图底缘, 前顶点 iso 0.5 线)`——
  平底建筑即 V 形底座本身，球面/手臂/挑檐高于 iso 线时影子落回地面。
- test-sun-shadow-geometry 33 项（iso 地面线净空落位锁定）；未做其它自测
  （按用户要求）。建议检查点：①桶状仙人掌为球面椭圆影而非薄饼；
  ②仓库主屋全幅延伸、右侧无碎片；③细长仙人掌影根贴主干。

### 对话：墙面/楼梯全面审计整改与模块拆分（2026-08-19）
- 拆分前完整备份到`backup/wall-stair-audit-20260820-011732`，保留当前脏工作区真实版本。
- 修复生产时序：`resolveCollisions`后新增高架surface最终提交，解决surface确认后又被同层单位推入
  接缝的问题；重算不重复推进卡死看门狗。
- 楼梯/共享面/墙顶改为收集全部候选后稳定排序，surface owner与platform原子提交；
  反转`platforms`构建顺序后结果保持一致。
- 高架路线期间暂停MovementSystem通用卡死重算/脱困和CompanionAI瞬移；仓鼠骑士冲锋补齐
  `ignoreForEntity`。AI单位排斥复用统一垂直区间，墙下单位不再改变墙上友军方向。
- 楼梯组横向容差与沿坡相位拆分（8px/1.5px），禁止错级踏步强行合组；teardown改副本遍历。
- 快照新增原墙坐标，恢复优先8px精确绑定，旧锚点回退半径从520收紧到96；方块墙链统一复用
  墙顶邻接图。
- 新增`physics/elevation.js`、`world/elevated-surface-state.js`、
  `world/wall-stair-group.js`三个模块；GameScene低层图层查询改用空间分区。
- 探针新增碰撞后surface、平台顺序、友军路线、密集Shift、旧实例自愈和语义错误门；
  拆分后完整浏览器回归通过，RTS 32/32、快照42/42、墙深度10/10、ESLint 0 error。

### 对话：建筑升级扣款、独立生产选择与详情面板关闭修复（2026-08-19）
- 升级不扣金币根因：矿场/兵营/产兵建筑/铁匠铺与研究能力升级均读取
  `_devInfiniteResources` 并直接跳过支付。现统一走 `payBuildingUpgradeCost`：
  升级始终扣金币与能源，测试无限资源只保留“建筑放置免费”；支付失败不升级，
  能源扣除异常时退还已扣金币。
- 同类建筑切换兵种看似同步根因：相邻2×2建筑的点击矩形重叠，交互循环总返回数组中
  第一栋建筑，实际反复操作的是同一实例。产兵建筑与兵营均改为收集命中候选并选择离
  点击位置最近的建筑；通用产兵建筑额外复制实例级运行时配置，生产类型与读条继续逐建筑
  由 `unitType/_spawnTimer` 保存恢复。
- BasePanel新增 `buildingDetail` 分组关闭策略。产兵/铁匠铺/研究/仓库/教堂/传送门、
  矿场、兵营、防御塔和陷阱详情现在均支持Esc关闭；在面板外单击鼠标左键或右键关闭全部
  建筑详情，并阻止该次点击穿透到攻击或场景操作。Electron转发Esc走相同入口。

### 对话：实建并排楼梯仍掉缝的旧实例自愈与容差修复（2026-08-19）
- 合成精确格楼梯可通，但实建/旧存档可能有数像素取整偏差；原中心容差3px、共享边总差24px会漏认，
  看起来相邻的楼梯仍保留内部双侧边且没有共享surface。
- 楼梯组中心容差提高到8px、共享边总差提高到48px、接缝命中保护提高到4px。
- 新增250ms拓扑签名自检：热更新后旧实例缺少共享数组、组版本不一致或位置集合变化时，
  `DefenseSystem.update`自动注销旧边并重建整组，无需重新放置楼梯。
- 偏差探针给每相邻楼梯加入5px错位并删除全部共享数据；运行时自动自愈成功，
  共享面仍为`[3,6,3]`、边界`[3,1,3]`，全段/多高度横移通过。
- A+S横向移动153.68px抵达第3座，0掉地、0图层违规、0卡帧；顶部五横向位置、友军上墙及
  删除3→2→1座边界恢复继续通过，完整探针`errors: []`。

### 对话：新增玩家友方单位·仓鼠轻骑（2026-08-19）
- 素材接入：`assets/companions/hamster_light_cavalry/`，四套8×4透明精灵表；
  idle 8帧、running 11帧、attacking 12帧、dying 11帧。源目录缺少已合成的
  `dying.png`，已从其中11张死亡单帧按项目纯色底抠图管线生成并打包。
- 数据：怪物六维公式，力量20/敏捷15/智力3/体质20/精神3/幸运5，
  HP 250、移速230；显示尺寸390，脚底校准 `spriteOffsetY:-91 / footOffsetY:91`。
- AI：寻找最近敌人近战攻击，跳过能源矿点；每2秒一次，攻击动画第9帧造成
  60点物理伤害；无目标时跟随玩家，支持RTS移动/攻击/待命指令。
- 骑兵学校新增“仓鼠轻骑”选项：轻骑生成周期60秒，仓鼠骑士改为90秒。
  生产、动画、仇恨、全局升级、图鉴数据源及世界122后台DPS结算均已接入。
- 骑兵升级项目新增按兵种过滤：骑士保留冲锋强化，轻骑改显示机动强化，
  通用攻击加速/伤害/生命强化两者共享。

### 对话：仙人掌阴影重新设计——接地曲线坡度截断 + 净空位移模型（2026-08-19 十一轮）
- 根因两条：①实体四边形左右顶点直接取全剪影最外列——仙人掌手臂尖（悬空）
  被误当实体宽度，身体影成了臂展大的悬空四边形；②悬空部分位移按自身厚度计算，
  手臂影悬空贴身而非落在地面远处。
- 修复一（footprint 重新设计）：`_contactGroundInfo` 从最低接地点沿接地曲线
  按 iso 地面坡（≈0.5，容差 0.75）截断——贴地列（iso V 形底座）全部保留，
  悬空列（手臂/旗帜/挑檐）一律排除出 footprint；实体宽=主干真实接地宽。
- 修复二（净空位移模型）：位移按该列**地面线**计算——贴地列 zBot=0
  （V 形底座不误判悬空）；悬空列的地面线按截断端点水平延伸，影子按
  "地面线→自身顶缘"的净空落到地面远处（手臂影贴地且离干）。
- test-sun-shadow-geometry 33 项（截断排除 + 净空落位各锁定）；未做其它自测
  （按用户要求）。建议检查点：①仙人掌影根贴主干底部、臂影落地；②基地/仓库
  V 形底座侧顶点无误位移；③桶状仙人掌球体影形态。

### 对话：楼梯间隙与顶部墙接口偏心掉落修复（2026-08-19）
- 几何复核：上下段接口误差约0.000015px，最高踏步→墙顶误差约0.000004px；
  楼梯顶Z与墙顶Z均为125，高差0，排除“模型顶部不够墙高”猜测。
- 并排楼梯踏步间原生约8px空档已由共享接缝覆盖；下段/上段各取15%/50%/85%高度横跨三座楼梯，
  全部保持stairs且高度量化正确。
- 真正根因是顶部连接面重复扣单位半径：80px通道对玩家半径30只剩19px总宽，偏离中心即失去surface。
  现在只扣0.5px边线厚度，有效半宽恢复39.5px；单位半径继续由WallSystem侧边碰撞负责。
- 横向10%/25%/50%/75%/90%五个位置上墙全部成功，最终均为wall_walk，0掉地、0地下帧。
- 友军4节点上楼、多楼梯A+S横移、Shift密集墙顶与压力测试继续通过，完整探针`errors: []`。

### 对话：友方单位楼梯上墙掉地卡死修复（2026-08-19）
- 根因：友军高架路线沿用40~42px水平到达半径和34px高度容差，而楼梯出口→墙中心仅约32px；
  单位尚未真正到达上段出口，就会连续跳过出口与墙中心节点，直追后续墙点并跨空掉地。
- 高架route节点统一收紧为12px水平/12px高度；普通无route地面命令继续使用40px到达半径。
- 侍从、仓鼠射手和仓鼠斥候删除各自复制的路线推进代码，统一复用
  `resolveRtsMoveDestination`；其余仓鼠兵种原本已走该入口。
- 友军实景验证：4节点路线逐点消费，57帧从地面入口走完两段楼梯，最终
  `wall_walk/Z=125`、目标墙正确、0 ground帧、0地下帧、0连续停步。
- 多楼梯横移、玩家上下楼、Shift密集墙顶与压力测试继续通过；RTS测试32/32，完整探针`errors: []`。

### 对话：阴影重叠不再增强——淡出 alpha 按目标合成反解（2026-08-19）
- 问题：五段外扩淡出是放大版整片覆盖，内部把五层 alpha 全部叠加
  （0.385 被叠到约 0.58），违背"重叠部分不要增强"的口径。
- 修复：绘制顺序改外层先画、内层后画，每段 alpha 按目标合成反解
  `w=(target−prev)/(1−prev)`（ bands 1.10/1.07/1.045/1.02/1.0 对应目标合成
  0.16/0.31/0.52/0.78/1.0）——核心区域五层合成后**精确等于 profile.opacity**，
  不再增强；边缘带按 78%/52%/31%/16% 渐出，淡出观感保留。

### 对话：两座及多座同方向相邻楼梯横向自由通行（2026-08-19）
- 根因：每座楼梯独立注册左右`_stairEdge`，并排后内部接缝形成双重隐形墙；两张Blender踏步可走面
  之间另有约8px真实空档，仅删除边线仍会短暂失去surface。
- 新增楼梯组识别：方向、坡向、段数和高度一致，且首段沿横向相邻一格的楼梯自动合并。
- 每对组员为下段、上段和墙顶接口分别生成共享接缝面，并注销内部共享侧边；踏步高度继续按九级
  量化，图层取两侧楼梯最大深度。
- 新建、读档、回收、摧毁均重建楼梯组外轮廓。3座时共享面`[3,6,3]`、边界`[3,1,3]`；
  删除为2座后边界`[3,3]`，再删为1座后共享面0、完整5条外边恢复。
- A+S横向移动157.35px，从第1座穿过第2座到第3座，全程stairs、0掉地、0图层违规、0卡帧；
  纵向上下楼、Shift密集墙顶与压力测试继续通过，完整探针`errors: []`。

### 对话：阴影实体改由剪影接地曲线生成 + 归一高度 75 分位（2026-08-19 十轮）
- 三问题同一定位：①基地阴影向光照侧偏出——逻辑 4×4 实体中心（4200,3968）在贴图
  真实底座中心（≈4197,3986）的后上方 18px，实体锚点错了；②仓库右侧破碎尖刺——
  归一高度用全图最高列（塔 983），主体位移被压扁成细带；③仙人掌底部对不上——
  实体用碰撞 foot 矩形，与贴图底部不接。
- 修复一（实体/轮廓同一锚点真源）：`getSilhouetteFootprintVertices` 由剪影接地曲线
  直接生成实体四边形（front=最低接地点、left/right=接地曲线两端、back 按 2:1 镜像），
  与轮廓展开共用同一锚点/比例/镜像——对齐由构造保证，逻辑格网、碰撞 foot、
  autoFootprint 拟合全部退出阴影链（无剪影数据时仍回退 footprint）。
- 修复二（归一高度 75 分位）：归一参考从全图最高列改为 75 分位列高——
  高矮混合建筑（仓库主屋+板条箱）主体获得全幅位移，塔尖拉长按 maxOffset 钳制。
- test-sun-shadow-geometry 30 项（接地四边形五点 + 钳制一点）；未做其它自测
  （按用户要求）。建议检查点：①基地阴影中心贴底座、不向光照侧偏；②仓库为
  完整建筑影、右侧无碎片；③仙人掌影根贴主干底部；④边缘淡出与减淡 30% 观感。

### 对话：密集单位导致墙顶Shift卡死与图层污染修复（2026-08-19）
- 用户观察“怪物/单位越多越容易卡死和错层”后，确认实体分离仍是纯二维：墙下怪物与Z=125墙上玩家
  footprint重合时仍会推挤；分离推力再次过墙时又未携带墙顶ignore上下文，形成墙/单位双重拉扯。
- `Game.resolveCollisions`新增Collider垂直区间过滤（2px容差）；高度不重叠直接跳过实体分离。
  所有分离后的`WallSystem.resolve`统一携带`ignoreForEntity`，同层墙上拥挤仍可正常散开。
- `GameScene`对墙上玩家/侍从增加脚下低层单位深度纠正：仅处理垂直完全分离且footprint近邻的单位，
  高层深度至少为低层Sprite+0.1，避免密集怪物间歇覆盖墙上单位。
- 密集探针：墙下12单位重合时墙上玩家位移0，玩家深度3061>低层最大3047.92；同高度单位仍产生
  17px正常分离。墙下单位存在时Shift+D+W移动102.8px、0卡帧、0掉地、图层违规0并抵达上墙。
- 墙顶八方向压力、楼梯上下行、翻滚与两段220×220渲染继续通过，完整探针`errors: []`。

### 对话：阴影四项调整——凸包∪剪影片络合并 + 桶状仙人掌剪影化 + 强度−30% + 边缘淡出（2026-08-19 九轮）
- **仓库破碎断裂**：离线多边形模拟定位——纯剪影剪切在高度方差大的建筑上
  （仓库塔 983px vs 墙 300px）位移悬殊退化成细带。修复为**包络合并**
  （`getUnionShadowPolygon`）：同一 (u,v) 采样线上取凸包实体与剪影轮廓的
  u 跨度并集，凸包保主体、剪影贡献轮廓，边界单调无自交、单层无双倍加深；
  无剪影数据回退凸包。
- **仙人掌**：桶状（圆球）由胶囊接触影改 `kind:'projection'`——圆球剪影
  进阴影（与细长系同管线）；细长系（单/双臂/多节）同走合并管线。
- **强度 −30%**：`STATIC/DYNAMIC_SHADOW_OPACITY` 0.55 → 0.385
  （夜影衰减公式不变：白昼 0.385、黄昏 0.059、深夜 0）。
- **边缘淡出**：三段硬边羽化改五段质心外扩（×1.0/1.02/1.045/1.07/1.10，
  alpha 1.0/0.42/0.26/0.15/0.08 几何衰减），形成 ~10% 尺寸柔和淡出带。
- test-sun-shadow-geometry 25 项（合并五点：覆盖双源/回退/包络无自交）；
  未做其它自测（按用户要求）。建议检查点：①仓库阴影为实体 blob 无破碎；
  ②桶状仙人掌呈圆球影、细长系影随形态；③全场影深统一减淡 30%；
  ④阴影边缘柔和无硬边。

### 对话：墙顶稀有卡死看门狗与压力测试（2026-08-19）
- 在已知边界/转角修复之上增加最后保险：输入非零且连续2帧位移<0.05px，并且当前墙存在正向出口
  时，才启动3px范围内的可行点搜索；真正外墙边缘不会强制移动。
- 搜索候选包括真实出口目标、出口轴及输入方向±15/30/45/60/90°，只接受仍受墙顶surface承托、
  不明显逆输入的最高分点；恢复后立即清零看门狗。
- 192帧八方向压力测试：0掉地、0图层违规、最大连续零位移2帧，正常路径看门狗0触发；
  人工连续两帧零位移用例成功在墙面内恢复3px且不掉地。
- 直墙、偏差墙、L转角、四墙、楼梯上下行、翻滚与两段220×220渲染继续通过，探针`errors: []`。

### 对话：贴合的轮廓阴影——逐帧剪影多边形（2026-08-19 八轮）
- 形变烘焙失败根源复查：跳动/错配来自**缓存分桶与 x 尺错配**，不是几何本身。
  正解 = 剪影也做成逐帧纯几何（与凸包同一条命）：`build-lighting-maps.py` 新增
  `shadowSilhouette` 逐列量测（每列 [texX, topY, bottomY]，≤128 列 + 全图最低接地点），
  `getSilhouetteShadowPolygon` 运行时按当前太阳方向直接展开多边形——
  无烘焙、无分桶、无缓存，连续性与凸包同为构造保证。
- 多边形构造：近边 = 贴图实测**接地曲线**（各列 bottomY 的 V 形，基地阶梯底座、
  非对称建筑都精确贴）；远边 = 各列顶线按列高沿影向位移（最高列恰好位移 length，
  塔尖/旗杆轮廓进入阴影）；影宽 = 建筑真实全宽（不再被 footprint 钳小）。
  (u,v) 帧排序取包络防自交，质心外扩三遍羽化同前；flipX 列镜像；
  无剪影数据时回退 footprint 凸包（21 个光照资产已全覆盖）。
- test-sun-shadow-geometry 20 项（剪影七点：近边=接地曲线/最高列位移=length/
  矮列按高比/黄昏镜像/V 形曲线/退化安全）；未做其它自测（按用户要求）。
  建议检查点：①基地阴影贴阶梯底座且远端带大理石轮廓；②兵营塔楼轮廓入影；
  ③全天连续无跳变、18:35 后无影；④与凸包版对比轮廓观感。

### 对话：墙顶偶发原地踏步与转角死点修复（2026-08-19）
- 根因一：目标越界后的边界滑动只在反向采样成功时执行；采样完全失败后走备用中心回夹，
  该路径漏掉滑动恢复，会每帧回到同一个合法边缘点。
- 根因二：分叉处只尝试匹配度最高的一条出口；该出口在当前脚底位置没有承托时，不会尝试
  第二相邻墙方向。
- 滑动统一移到全部回夹路径之后，按匹配度逐个尝试所有墙口/楼梯口，并同时尝试“朝真实目标收敛”
  与原轴方向。
- 增加转角死点小内缩：从当前墙中心方向依次内缩1.5/3/5px重试，禁止大幅瞬移。
- 新增W→D→S→A→W多方向循环：五阶段均能从边界恢复并跨越上/右/下/左/上墙，
  全程0 ground帧、图层违规0、最大单帧约4.44px，完整探针`errors: []`。

### 对话：墙顶改为二维自由表面移动（2026-08-19）
- 放弃玩家每帧投影中心线的轨道移动：保留`navigationMode=graph`用于RTS/BFS拓扑，新增
  `movementMode=surface`，即时移动以真实墙顶多边形与墙间连接面联合区域为权威。
- W/A/S/D现在分别产生纯上/左/下/右位移，斜向保持原向量；墙内不生成移动轴、不旋转输入。
- 仅在撞到联合面边界时处理滑动：先夹到最远安全点，再用剩余位移朝真实邻墙中心或楼梯入口收敛；
  不持续吸附中心线，也不替玩家自动连续拐弯。
- 地面碰撞半径仍为30，墙顶surface承托半径设为24，解决墙缝与转角净空不足，同时要求单位中心
  始终位于真实墙顶/连接面，未扩大墙贴图外侧空气区域。
- 实景验证：孤立墙W/A/S/D均严格按输入方向移动；A+S与单S可跨连续墙，A/A+W可走入楼梯；
  偏差墙、L转角、翻滚、图层和上下两段220×220楼梯通过，0图层违规，探针`errors: []`。

### 对话：阴影回到 footprint 凸包思路重建 + 夜影彻底归零（2026-08-19 七轮）
- 用户裁决：轮廓形变路线连续引入错位/跳动/分割/夜影残留，回到 footprint 思路重建。
- **唯一做法（建筑+散布障碍物）**：阴影 = footprint 四边形 ∪ 四边形沿影向平移
  length 的凸包（`getStaticShadowHull`，Andrew monotone chain），每建筑一个
  Graphics 逐帧直画 + 质心外扩三遍羽化（×1.06/×1.02/×1.0，alpha 0.15/0.35/1.0）。
  **逐帧纯几何、无烘焙无分桶无缓存**——太阳连续移动时阴影必然连续，
  "跨桶瞬间移动一段"从构造上消除；顶点真源 = `isoFootprintVertices`
  （`_pixelFootprintLocal` 像素拟合优先，与放置/遮挡/范围显示同一组顶点）。
- 退役：轮廓形变烘焙（_bakeStructureShadowWarp）、边链/扫描线纯函数、
  烘焙缓存与 LRU、contactQuad 消费链、固定画布方案——弯路记录留档。
- **夜影彻底归零**：透明度公式改 `0.55 × clamp((daylight−0.1)/0.2)`——
  daylight≥0.3（约 06:35~17:25）白昼全强度，18:00 仅余 0.08 基本不可见，
  ≤0.1（约 18:35~05:25）完全为 0（Sprite 同步隐藏），黄昏残留也一并消除。
- test-sun-shadow-geometry 13 项（凸包七点 + 透明度三档 0.55/0.084/0）；
  未做其它自测（按用户要求）。建议检查点：①基地/兵营阴影贴 footprint 且全天
  连续无跳变；②18:35 后全场无影；③正午/晨昏影长随仰角自然变化。

### 对话：阴影三问题修复——四边形贴图真源统一 + 锚点跳动消除（2026-08-19）
- 实测数据定位三根因：①碰撞拟合四边形（基地408×204）系统性小于贴图真实
  （bbox 684px≈437显示）——阴影小一圈；且宽剪影被挤进窄四边形 v 范围，
  与四边形实心段脱节成分割；②基地与 autoFootprint 建筑的阴影四边形
  均不吃贴图真源，与地面衔接不贴合；③烘焙 extent/cross 24px 分桶让画布尺寸
  与锚点随桶跳变——阴影位置"一瞬间移动一段+小范围线性"。
- 修复一（四边形真源统一）：`build-lighting-maps.py` 新增 `contactQuad` 量测
  （贴图 bbox 菱形：前顶点=最低行质心、宽=bbox 全宽、深=宽/2 的 26.565° 2:1），
  与轮廓形变的投影图行（源贴图 x）同一把 x 尺；运行时前顶点对齐视觉脚底，
  四边形与轮廓天然贴合。优先级：contactQuad → 像素拟合 → 视觉锚点 fit →
  逻辑 iso → AABB。
- 修复二（跳动）：烘焙画布改按四边形外接圆**固定尺寸**、锚点恒定
  （origin 不再随桶变），分桶只保留方向 2°/length 2px 内容细分，
  形态变化 ≤2px/2° 近线性；烘焙键 v3 前缀。
- 未自测（按用户要求）。建议检查点：①基地阴影贴阶梯底座外缘无分割；
  ②兵营/铁匠铺/靶场阴影贴地衔接；③时间流逝时阴影连续平移无跳变；
  ④放大后表盘与 HUD 无遮挡回退。

### 对话：墙顶单方向键回弹与楼梯Portal抢键修复（2026-08-19）
- 复现确认单按A时楼梯Portal匹配度1.0、相邻墙边约0.894，旧逻辑把A解释成下楼并在第7帧切为
  `stairs`；A+S因墙边匹配度更高才表现正常。
- 墙顶移动轴新增`wall/stair_portal`类型；当Portal仅比最佳墙边高出不足
  `graphPortalOverrideMargin=0.2`时优先墙边，避免楼梯抢占普通单键移动。
- 单按A、S成功连续走过相邻墙，反向单按D、W成功返回楼梯墙；A+S双键保持正常，
  A+W明确外斜输入仍可进入楼梯下行。
- 全部单键用例0 ground帧、图层违规0、最大普通单帧约4.2px；四墙与完整楼梯探针`errors: []`。

### 对话：墙上单位图层闪烁与楼梯邻墙卡死修复（2026-08-19）
- 图层根因：墙顶图边跨两块墙时，表面深度只取当前归属墙，且`GameScene`没有消费已计算的
  `_surfaceRenderDepth`；切墙途中较深相邻墙会间歇盖住单位。
- 图边表面深度改取两端墙`_faceDepth`最大值；玩家、侍从和普通实体的最终深度至少为
  `_surfaceRenderDepth+1`。
- 卡死根因：表面查询先楼梯后墙顶，`wall_walk`单位只要落入连接面就可能被抢成`stairs`，
  没有确认当前输入是否真的朝楼梯下行。
- 增加Portal意图门：所选移动轴与墙中心→楼梯入口方向点积≥0.98才允许下楼；沿相邻墙移动时
  连接面不再抢占状态。
- 逐帧验证：墙顶/楼梯图层违规0；楼梯墙→相邻墙全程无`wall_walk/stairs`抖动、0卡帧、0掉地；
  直墙、1px偏差墙、L形转角、四墙、翻滚与上下两段220×220楼梯继续通过，完整探针`errors: []`。

### 对话：基地阴影与贴图不符修复——阴影四边形改吃视觉锚点实测（2026-08-19）
- 现象：其他建筑阴影正常，基地阴影仍与贴图不符。贴图实测：基地是大理石方盒+阶梯底座，
  底座可见宽约 437px、深约 219px（26.565°），而阴影用的逻辑 4×4 菱形为 512×256——
  大出约 17%，四边均溢出贴图底座；且基地未开 autoFootprint，像素拟合从未应用到它。
- 修复：`_syncStructureSunShadows` 的阴影四边形真源改优先级——实体已应用的像素拟合
  → 视觉锚点 `resolveStructureGroundFit().localVertices`（与放置幽灵/遮挡同一测量源）
  → 逻辑 iso 菱形 → AABB 兜底。所有建筑阴影统一贴贴图可见底座，不再吃逻辑格网尺寸。
- 未自测（按用户要求）：建议实测检查点——①基地阴影四边是否贴住阶梯底座外缘、
  ②正午/斜向延长段是否仍连续、③兵营/铁匠铺等 autoFootprint 建筑阴影无回退。

### 对话：基地夜间残影与阴影断裂双修复（2026-08-19）
- **夜影**：深夜（daylight=0，约 18:35~05:25）基地/障碍物/单位阴影仍按 0.55 渲染成黑块。
  修复：静态与动态阴影透明度统一随 daylight 衰减——`opacity = 0.55 × clamp(daylight/0.2)`，
  daylight≥0.2 全强度、黄昏转淡（18:00 约 0.36）、深夜归零；个体 `shadow.opacity` 覆盖不受影响。
- **断裂**：实机转储烘焙画布发现基地阴影=四边形实心菱形+脱节的第二颗深色菱形。
  根因一：`PROJECTION_BOTTOM_BANDS["defense_base"]=0.20`（08-18 为旧旋转矩形设计的
  带状采样），带状图内容贴向远端，单层烘焙的延长段与实心段在两侧脱节——
  单层管线下禁用带状采样，defense_base 恢复 full-alpha 投影。
  根因二：烘焙用 `ctx.filter=blur(1px)` 会把 1px 行 alpha 摊薄到 0.3~0.6，
  实心段/轮廓段重叠区更深（明暗两截）。改行高 1.5px（相邻行 50% 重叠）代替 blur，
  覆盖区 alpha 饱和到 1；烘焙键加 v2 前缀防旧缓存。
- 实机：正午/斜向基地阴影单一连续、全影同深；深夜三相截图残影消失（alpha=0）；
  test-sun-shadow-geometry 17 项（黄昏 0.359/深夜 0 锁定）+ footprint31/图层16/
  放置12/wall-depth10/渲染序5/视觉锚11 + eslint 0 error + Vite build 全绿。

### 对话：实建连续墙只能在楼梯首墙移动修复（2026-08-19）
- 合成探针一直使用精确`±64,±32`格心，未覆盖实建/旧存档墙心1~2px取整偏差；墙顶连接虽然配置
  `neighborCenterTolerance=2`，但邻墙索引先用精确世界坐标键，未命中时容差根本没有执行。
- `_blockWallNeighbors`改为精确键优先、随后在配置容差内搜索最近墙，最终仍由墙顶连接面几何
  二次复核，不会误连远墙。
- 新增1px偏差墙回归：路线由空数组恢复为`drift_wall_a → drift_wall_b`，真实Input D+S成功切墙，
  最大单帧约4.44px、0 ground帧。
- 精确直墙、L形转角、四墙最上方、翻滚与楼梯两段220×220渲染继续通过，完整探针`errors: []`。

### 对话：恢复城墙楼梯两段渲染与四向对称（2026-08-19）
- 修复中立建筑渲染重构覆盖楼梯专用分支：父实体此前只按`idleKey`创建下段Sprite，上段完全缺失，
  导致楼梯看起来缩小一半。
- 恢复`_syncWallStaircaseLayers`与`data.segmentSprites`契约：上下段分别读取`visualSegments`
  的纹理、Blender锚点、220×220尺寸和独立深度。
- 楼梯绕过普通中立实体`flipX(_facingLeft)`；e1/e2四方向独立贴图不再二次镜像。
- 地图模式隐藏/恢复、动态深度、沉陷接管和销毁清理均覆盖全部分段Sprite。
- 实景探针确认上下两段均存在且尺寸220×220，e1/e2正负四方向图层正确，完整探针`errors: []`。

### 对话：恢复墙顶移动瞬移与节点僵硬优化（2026-08-19）
- 在确认楼梯缺段来自独立渲染分支被覆盖后，恢复此前撤回的墙图移动优化；该组改动不调整楼梯模型、
  纹理、显示尺寸或吸附配置。
- 墙归属在`graphNodeSwitchT=0.995`后交接；越过节点时把剩余距离继续投到输入选择的下一边，
  横向贴轨每帧最多3px。
- 墙顶/楼梯玩家位置积分最多34ms，翻滚计时使用同一积分量，消除浏览器长帧产生的大跨度瞬移。
- 联合实景验证：上下两段仍为220×220，四方向图层正确；普通墙顶最大单帧约4.44px，
  四墙最上方、左右转入、直墙、L形转角和翻滚均可达且0 ground帧，完整探针`errors: []`。

### 对话：HUD 24h 太阳针表盘（2026-08-19）
- 右上角时间胶囊新增 SVG 表盘（`#gameTimeDial`，60px）：针=太阳方位——
  上=正午、右=日落、下=午夜、左=日出，玩家看表即知太阳在天空的位置；
  上半圆金色白昼弧、下半圆蓝黑夜弧 + 地平线与 24 刻度（四主二十副）。
- 弃用精灵图帧方案（24h×60min 帧数爆炸且跳变）；指针是单根 SVG `<g>`，
  旋转轴心在表盘中心，`refreshGameTime` 内由 `getSun().phase` 驱动
  （`rotate(phase×360−90, 24, 24)`），与太阳/阴影/时间文本同源同帧刷新，
  暂停/读档自然冻结恢复，无独立秒表。
- 实机：cdp-sun-dial-verify 四时相角度断言 4/4（日出−90/正午0/日落+90/午夜180）
  + HUD 截图 verify-shots/sun-dial-*.png；eslint 0 error + Vite build 通过。

### 对话：阴影系统全面审计与三项隐患修复（2026-08-19）
- 静态审计发现：①雪松 5 种姿态在 BootScene 预加载了投影图，但 ISO_WALL_GEO
  从未配 `shadow.kind='projection'`——08-18 起一直静默退回椭圆接触影（配置遗漏）；
  ②`getStaticShadowHull`/`_convexHull` 自六轮单层化后已成死代码（契约测试还在测死代码）；
  ③烘焙缓存 LRU 逐出无活键守卫，被逐键的阴影精灵会握死帧一帧（渲染可能抢先抛异常）。
- 修复：雪松补 `kind:'projection'`（heightMul 0.8/maxOffset 72/visual 1.1/1.1）；
  死代码与 11 项失效用例清除（test 头注同步）；LRU 逐出前收集
  `_staticSunShadows` 的 `_bakedKey` 活键集合跳过，缓存清理时同步置空。
- 实机审计（tools/cdp-sun-shadow-audit.mjs）：122 正午 41 影 = 1 建筑 + 32 障碍 hull
  + 8 胶囊，**可见透明度全集 = {0.55}**（均匀达成）、深度违规 0；质量档
  medium 33+8 → low 0+0 → high 33+8 切换零异常；123 雪松 38 株全部 hull 注册、
  透明度同 0.55、雪地定向影可见；全程 error/unhandledrejection 监听为空。
- 回归：test-sun-shadow-geometry 16 项 + footprint31/图层14/放置12/wall-depth10/
  渲染序5/视觉锚9 + eslint 0 error + Vite build 全绿。
...
- 道路专项测试扩至31项，快照扩至42项；怪物速度9项、矿工56项、斥候47项、
  ESLint 0 error与Vite build通过。

### 对话：阴影深浅统一——恒定透明度 + 单层烘焙（2026-08-19 六轮）
- 深浅不一三来源：①透明度随仰角漂移（正午 0.57/晨昏 0.76）；②凸包接地层与轮廓层
  叠加，交界处出现更深色核；③单位与建筑透明度曲线不同。
- 统一：`STATIC/DYNAMIC_SHADOW_OPACITY = 0.55` 全局恒定（颜色纯黑不变），个体仍可
  `shadow.opacity` 覆盖；删除 SHADOW_OPACITY_MULTIPLIER 与仰角透明度项。
- 单层化：`_bakeStructureShadowWarp` 同画布两段成影——①四边形实心段
  （`getStaticShadowRowSpan` 多边形扫描线逐行填充近缘→边链）；②轮廓延长段
  （投影图沿边链形变，24px 重叠埋缝）。一张纹理一个透明度，注册键即显示精灵，
  Graphics 接地层退役，从构造上消灭叠加深斑。
- 事故记录：编辑批处理期间 GameScene.js 被意外清空；已从 git HEAD 恢复并逐项
  重放全部阴影改动（恢复脚本 tools/_tmp_restore_gamescene.py 用后已删）。
  并行会话在 GameScene.js 的未提交改动（`_getVisualOffsetX` 重写 + autoFootprint
  门控 `entity.spriteCfg?.autoFootprint === true`）依其测试断言与修改前读档重建，
  test-world122-building-footprint 31 项复核通过；其余改动均在其它文件，未受影响。
- 实机三时相：正午/黄昏/黎明阴影同色同深，四边形体内无深核、远端轮廓柔和；
  test-sun-shadow-geometry 25 项（扫描线跨度 + 透明度恒定）+ footprint31/图层14/
  视觉锚9/wall-depth10/渲染序5 + eslint 0 error + Vite build 全绿。

### 对话：非墙2×2建筑外围自动道路环与4×4建造预约（2026-08-19）
- 用户澄清最终目标为“精致细致的白色石砖”。正式资产再次由5080重出，最终采用
  `flux2-klein-4b-walltex`浅色砖石LoRA候选：小型矩形白色石灰岩/象牙白方砖、
  细冷灰凹陷砖缝、轻微倒角与矿物颗粒，规则错缝铺设；回收6%曝光以保留纹理层次。
  最终平均RGB约`[202,200,197]`、亮度标准差18.2、P99亮度231.3、过曝像素约0.1%，
  白色石材占比约90%，同时保持清晰砖缝和边缘细节。
- 用户否定首版道路中的浅色石砖后，5080以新提示词重新生成：主体改为暗土褐压实路基，
  仅保留少量炭灰/深褐碎石，显式排除白石、浅灰石、米白石灰岩、白砖和亮色砂浆。
  最终4帧不透明像素平均RGB约`[91,82,76]`、平均亮度83.6、P99亮度98，
  亮度>145像素为0%，不再出现白色石砖。
- 通过远程5080 `flux2-dev-fp8` 生成低饱和无缝石质道路材质；`floor-asset.py`
  新增 `road-stone` 类型，再由 `build-building-road-tiles.py` 确定性投影为4帧
  `128×64`、2:1菱形精灵表 `assets/terrain/building_road_tiles.png`。AI只负责材质，
  格线角度、透明轮廓与尺寸由脚本锁死。
- 防御塔、矿工营地、军营及配置建筑仍保留中央2×2实体footprint，但放置预约扩为4×4：
  中央4格建筑 + 外围12格道路。16格逐格检查世界边界、墙体/建筑/障碍和既有道路预约，
  任一格不合法即禁止建造。
- 建造幽灵新增外围12格道路预览：合法格绿色、非法格红色，建筑主体在任一格失败时同步变红；
  点击前即可看到具体受阻位置。建造成功后批量清理4×4范围的散布障碍与地面点缀。
- `BuildingRoadSystem`统一管理道路精灵、16格预约和生命周期；出售、回收、沉陷摧毁及场景离开
  自动释放，世界-122快照恢复时由建筑坐标重新生成，无需在存档中重复保存道路Sprite。
- 新增 `test-world122-building-roads.mjs` 20项；建筑footprint 31项、建造回归27项、
  快照41项、eslint 0 error及Vite build通过。

### 对话：静态太阳投影统一为单管线——矩形路径剔除（2026-08-19 五轮）
- 用户审计发现建筑（凸包+边链形变）与散布障碍物（旋转矩形贴图）仍是两套系统；
  矩形根边穿四边形必错位（太阳侧露角、正午铺宽带），且与边链形变轮廓平行维护成本高。
- 散布障碍物（双臂/单臂/多节仙人掌、雪松）迁入同一管线：视觉底座矩形四角
  （foot × visualWidthMul/DepthMul、纵深 ×0.5）作 footprintVertices 注册 hull；
  轮廓形变支持 **flipX 行镜像**（采样 frac 反转且进缓存键 `_f` 后缀，防镜像串缓存）。
- 剔除：`getStaticShadowFrame` 及矩形/旧启发式分支、registerStaticSunShadow 投影字段
  （projection/projectionOrigin/projectionFlipY/projectionModelWidth/crossSpread）、
  ISO_WALL_GEO 三处 crossSpread 死配置；胶囊接触影（树木/桶状仙人掌/墙件/单位）保留。
- 顺带修复形变烘焙 x 映射符号错误（`at.u - OV - minU` → `at.u - minU + OV`，
  内容整体沿 −D 偏 48px，远段被裁短 24px）。
- 实机取证（cdp-obstacle-shadow-debug）：散布 40 株 = 28 投影型全部 hull 注册可见
  （7 双臂 + 8 单臂 + 13 多节）+ 12 桶状胶囊，基地 1 栋，无漏注册；
  test-sun-shadow-geometry 19 项 + 建筑/放置/wall-depth/渲染序回归 29/12/10/5 +
  eslint 0 error + Vite build 全绿。

### 对话：四墙菱形最上方墙不可达修复（2026-08-19）
- 复现持续按W从菱形下墙前往上墙时，下墙→左墙后又被墙面落点校正吸回旧边，形成下墙/左墙振荡；
  根因是`WallSystem`按输入选择新边，而`DefenseSystem`仍按全图最近几何独立选择边。
- 玩家WASD与翻滚方向统一写入`_surfaceInputIntent`；移动层选中的道路轴同步为
  `_surfaceMoveChosenAxis`，墙面投影只在当前墙的直接相邻边中优先采用该轴，并加入92%节点归属迟滞。
- 定向实机验证：持续W按键按“下墙→左墙→上墙”切换，0 ground帧；左右墙分别斜向进入上墙成功，
  从下墙翻滚也按“下墙→左墙→上墙”抵达。直墙往返、L形转角、楼梯按键/翻滚与高速位移回归无错误。

### 对话：统一建筑底部 footprint 公式收口（2026-08-19）
- 格网物理改为单一公式：单格宽128、Y投影0.5；1×1=128×64格心锚定，
  2×2=256×128前顶点锚定（中心offY=-64），4×4=512×256（offY=-128）。
- 普通生产建筑、矿工营地、兵营、基地默认固定标准格网footprint；贴图alpha只负责
  `footOffsetY`与视觉X校正，不再改变碰撞宽深、中心、放置边界或遮挡前缘。
- `autoFootprint:true` 仅保留给未来显式异形建筑；墙、门、楼梯继续使用各自线段/多边形几何。
  热更新或旧实例若残留 `_pixelFootprintLocal`，会自动恢复标准格网并重建Collider。
- 建造幽灵、边界判定、实体重叠、出兵、攻击距离、范围显示和图层遮挡统一读取同一标准公式。

### 对话：墙顶改为Stronghold式中心线图导航（2026-08-19）
- 放弃“墙顶自由二维多边形+持续回夹”作为移动权威。新增`navigationMode=graph`：
  每块方块墙顶中心为节点，相邻墙为边，楼梯连接为Portal边；wall_walk单位每帧投影到当前
  连续墙图最近边，只有Portal允许离开墙图。
- 墙顶WASD意图按相邻墙/楼梯边投影，转角按点积选择最匹配正向分支；墙顶点击同样落到图边，
  远端仍使用墙块BFS路线。原贴图顶面多边形保留用于视觉标定和Portal承托。
- 修复完整主循环的两层反向推挤：Z>1且墙属于`_surfaceWalls`时跳过承托墙iso_rect实体分离；
  wall_walk单位忽略挂在当前墙链上的楼梯侧边，stairs单位仍受侧边约束。
- DashSystem撞墙判断改为比较墙图投影后的目标，正常沿墙转向不触发反弹。
- 真实Input主循环验证：墙顶A+S 48帧移动约163px且0卡帧；反向精确移动一格71.55px；
  墙顶翻滚移动约181px、0 ground帧；L形转角D+S移动一格并进入分支墙；
  楼梯按键/翻滚进入墙顶均保持wall_walk/Z=125。高速15组合及原有边界、图层回归通过。

### 对话：传送门视觉放大 + 固定标准2×2 footprint（2026-08-19）
- 现有 `portal.png` 保持不变；显示由288×305放大为 `300×317`，`footOffsetY=159`，
  视觉接地点继续通过 alpha 自动校正。
- 传送门配置新增 `autoFootprint:false`：构造时保留 `applyBuildingFootprint(this,2)` 的
  标准256×128碰撞，不再被能量涡/门框底部像素拟合成约246×123。
- 建造幽灵同样跳过像素footprint替换，因此预览、边界、放置判重与落地实体统一使用
  标准2×2；其他生产建筑仍保持自动像素拟合。
- 传送目的地、2000能源造价、3000HP、防御、出售和快照逻辑全部保持不变。

### 对话：轮廓阴影沿 footprint 轮廓边链逐行形变——双层错位根治（2026-08-19 四轮）
- 三轮远段条带被实机指出错位：条带根边是垂直于影向的直边，而凸包远缘是四边形
  真实边链（正午∧形、黄昏<形），两套阴影出发点不同必然错开；垂直剪切剪影
  已在三轮验证放弃（billboard 立面贴地平线必塌成一条线）。
- 最终公式（用户给定方向，本方落地）：轮廓阴影按四边形朝向 +D 的**轮廓边链
  （terminator）逐行形变**——边链上每点的立面各自沿 +D 投射 length，形变后
  根边=四边形边链本身，与凸包接地层共用同一边界；远缘=边链+length，两层恒贴合。
- 新增纯函数 `EnvironmentLightingSystem.getStaticShadowTerminator`（外法线·D≥0
  的边按 v 升序成链，含质心锚点）；`_bakeStructureShadowWarp` 按 v 逐行
  （1px/行）插值边链 u 与世界 x，从预投影图取对应行（行=源贴图左→右，
  底座质心偏移修正）画 length+24px 重叠段；方向 7.5°/length 8px/跨度 24px
  分桶，LRU 96，切换回收同前（先销毁剪影精灵再 remove，防死帧黑屏）。
- 实机三时相：正午影沿前顶点边链收窄、黄昏/黎明影沿侧边链延展，远端均带
  屋顶/底座轮廓起伏，两层无可见接缝；test-sun-shadow-geometry 30 项
  （边链三点链/远缘/背向剔除/2:1 菱形 45° 远缘在右顶点）+ eslint 0 error + build。
- 注意：并行会话「锁底座版草屋」入库后 test-world122-building-layer-assets
  挂 1 项（thatch_hut 贴图/config 已落地 271×191，测试期望仍写 192），
  与阴影链无关，留待该会话收口。

### 对话：锁底座版草屋正式入库（2026-08-19）
- 使用兵营底座作为几何参考生成的 `草屋.png` 替换 `assets/terrain/thatch_hut.png`；
  源图1536×1024自带透明背景，alpha>16紧身裁剪为1401×990。
- 原图综合角度26.892°、底座比例1.972:1、中心偏移0.1px；底座右侧约多7px，
  运行时统一footprint负责对称归一与格心对齐。
- 显示标定为 `271×191`、`footOffsetY=96`；自动拟合约257.3×128.7、
  26.565°、offsetX 0.29，匹配2×2占地。
- 同步重建 thatch_hut 的 projection/silhouette/height/normal；仓鼠民兵/斥候产出、
  10秒周期、模块升级、造价、HP、出售和存档逻辑全部保持不变。

### 对话：真实输入下墙顶/楼梯连接卡死修复（2026-08-19）
- 新增真实`Input.keys`主循环探针后复现：楼梯进入墙顶虽不掉落，但斜墙链按A+S 48帧仅移动
  约12.8px、43帧零位移；反向同样大量卡帧。局部`WallSystem.resolve`测试此前遗漏了完整主循环。
- 根因一：WASD对角线45°与墙轴26.565°不一致，窄footprint通道反复回夹。
  wall_walk单位新增相邻墙/楼梯方向轴，`WallSystem.resolve`将位移投影到最匹配正向轴，
  保留轴向速度分量；DashSystem撞墙判断改为比较投影目标。
- 根因二：`Game.resolveCollisions`仍把脚下方块墙的iso_rect当普通地面建筑，将墙顶玩家推出。
  现Z>1且墙属于`_surfaceWalls`时跳过该承托墙实体分离。
- 根因三：墙顶道路经过楼梯接口时仍被楼梯`_stairEdge`截停。wall_walk单位现忽略挂在当前
  连续墙链上的楼梯侧边；stairs单位侧边碰撞保持。
- 真实主循环验证：楼梯→墙顶按键和翻滚均0 ground帧、最终wall_walk/Z=125；
  墙顶A+S 48帧移动约157px且0卡帧，连续跨越两块墙；墙顶翻滚移动约181px、0 ground帧；
  反向成功抵达墙链端点。高速15组合、四向层级、边界回夹和跨墙回归继续通过。

### 对话：建筑阴影凸包接地 + 投影图远段轮廓条带结合（2026-08-19 三轮）
- 二轮凸包版实机"贴合但千篇一律"：所有建筑阴影都是同一几何形，丢了旧版屋顶/旗杆轮廓。
- 垂直剪切剪影方案验证后放弃： billboard 立面在贴地平线时物理上就是一条线，
  黄昏轮廓必然塌扁；改用**预投影图远段条带**——投影图（源 alpha 放倒）远端本就是
  屋顶轮廓，按 extentAlong+length 比例截取远段（含 24px 回退重叠埋缝），
  条带刚性不随方向变形，只随四边形远缘旋转平移。
- 双层结构：凸包接地层（轻 alpha 0.10/0.20/0.50 三遍羽化）保证 footprint 精确接地；
  轮廓条带层（alpha ×0.85、depth +0.001）提供建筑轮廓；横向仍走 manifest
  `base.centerX` 质心偏移（projectionCrossOffset，flipX 变号）。
- 烘焙缓存 `_bakeStructureShadowTail`：extentAlong(24px)/length(8px) 分桶、LRU 96、
  场景/质量切换与 shutdown 统一回收（`_clearStructureShadowBakeCache`）。
- 渲染事故教训：缓存回收只 `textures.remove(key)`，精灵残留死帧
  （Frame.sourceSize=null），渲染管线每帧抛异常→整屏大黑块；修复=回收前先销毁
  剪影精灵 + 同步时检测死帧强制 setTexture（与 unit_projection 先回退
  entity_shadow 同一教训）。
- 实机三时相：正午影随前顶点收窄且带大理石底座阶梯轮廓、黄昏/黎明长影贴底座
  延展且远端带轮廓起伏、太阳侧无露角；test-sun-shadow-geometry 23 项 +
  建筑回归 29/14/9 + eslint 0 error + Vite build 全绿。

### 对话：楼梯连接处翻滚/冲刺掉落卡死修复（2026-08-19）
- 根因：玩家翻滚与冲刺先移动，`DefenseSystem._updatePlatformStates`在本帧稍后才检查surface；
  高速单帧位移可跳过约9.5px连接中心通道，旧逻辑只看落点并执行
  `上一帧stairs + 本帧无surface → Z=0`，随后被墙碰撞锁死。
- 新增高架surface扫掠：有效楼梯/墙顶帧保存最后安全位置；上段stairs或wall_walk高速落点无
  surface时，沿本帧轨迹反向每3px采样、最多96段，夹到最远有效surface。楼梯底部仍可正常下地。
- `stairs→wall_walk`切换改为立即对齐Z=125，不再从最后一级踏步高度平滑抬升造成短暂下沉。
- 墙系统楼梯边界统一使用Y÷0.5后的等距点线距离；连接转向区使用上段矩形与墙顶目标边的
  凸包外边界，墙上单位只能经专用连接通道下楼。
- 安全CDP验证：普通左上移动11帧抵达墙顶；10/20/40/80/120px × -20°/0°/+20°
  共15种高速位移全部保持非ground，80/120px的6种越界位移触发扫掠回夹，落墙后Z均为125；
  四方向层级、18级人物层、四角footprint、四向跨墙和同墙对面楼梯保持通过。

### 对话：楼梯左上图层、承重凸出与后侧通行修复（2026-08-19）
- 审计确认四方向共用`wallDepth+0.2`导致后侧楼梯反盖墙：e1_pos/e2_pos上段与墙各重叠
  约8092像素（约54%）。现`ascendingSign=1`后侧层改为`wallDepth-0.2`，
  `ascendingSign=-1`前侧保持`wallDepth+0.2`；驻梯深度移除根结构层干扰。
- Blender确认旧上段支撑90.51×90.51、踏步宽80，每侧凸出5.25，加bevel约6.25。
  上下段底座重建为80宽×90.51长定向承重体，bevel统一0.65，保持完整接地；
  八张资产重渲染并同步全部Blender锚点。
- 后侧真实移动暴露屏幕圆碰撞误差：80宽踏步投影侧边间距约63.25px，旧墙系统按屏幕圆半径30
  判定会误挡。`_stairEdge`改为Y÷0.5后的等距地面点线距离，边线半厚配置为0.5。
- 转弯连接区改为“上段四点+墙顶目标边两点”凸包，只注册外边界，入口/出口开放；
  墙顶单位只可通过连接通道下楼，避免后侧踏步与墙顶重叠区把墙外点误吸成stairs。
- 安全CDP验证：e1±/e2±四方向前后层关系全部正确；左上真实移动11帧抵达墙顶，
  0掉落、0卡死；18级人物层、四角footprint回夹、四向跨墙和墙顶目标全部保持通过。

### 对话：建筑太阳投影按 footprint 四边形挤出重做（2026-08-19 二轮）
- 一轮旋转矩形方案实机仍不对齐：矩形根边穿过四边形会在太阳侧露角（黄昏右下角黑楔）、
  正午在前顶点下方铺一条与贴图无关的恒定宽带——矩形近似是结构性错位，不可调参修复。
- 按 skill 建筑 footprint 思路重做：阴影 = **真实 footprint 四边形 ∪ 四边形沿影向平移
  length 的凸包**。顶点真源 `isoFootprintVertices(entity)`（`_pixelFootprintLocal`
  像素拟合优先，与放置/建造幽灵/遮挡/范围显示同一组顶点）；影根由凸包构造精确贴住
  地面四边形，只沿 +D 延展，锚点/inset/贴图实测宽度等补偿全部退场。
- 新增 `EnvironmentLightingSystem.getStaticShadowHull`（Andrew monotone chain，纯函数）；
  渲染改为每建筑一个 Graphics，每帧重画凸包 + 质心外扩三遍模拟羽化
  （per-object FX 太贵，见 skill/12 Phaser 4 filters 教训）；深度 = 建筑 − 0.1 不变。
- 注册链：`registerStaticSunShadow` 新增 `hull` 分支（创建 Graphics 作 Map 键）；
  `_syncStructureSunShadows` 建筑一律 hull，不再消费投影贴图/manifest 锚点；
  `ShadowDebug.inspect` 输出 hull 顶点。障碍物（仙人掌/雪松）与树木保持
  预投影贴图 + 一轮方向自适应几何（小物件矩形近似误差可忽略）。
- 实机三时相：正午影随前顶点收窄（无宽带）、黄昏/黎明长影贴底座延展、太阳侧无露角；
  test-sun-shadow-geometry 23 项（凸包六边形/不拓宽/不露角/包含全部顶点）+
  eslint 0 error + Vite build 全绿。

### 对话：太阳投影影根错位与细长失真修复——方向自适应几何 + 贴图实测底座（2026-08-19）
- 实机两问题：①影根钉在 footprint 中心，阴影与模型底座错位；②显示宽被
  max(footprintH, 0.35×footprintW, length×crossSpread) 压到模型一半不到、长轴硬加
  footprint 全宽，视觉比实际细长。
- 新算法收敛为唯一真源 `EnvironmentLightingSystem.getStaticShadowFrame`（纯函数，渲染/契约测试共用）：
  菱形方向跨度 extentAlong/extentCross = 2·max(a|u|, b|v|)；影长 = extentAlong + 延长段；
  影宽 = max(垂直跨度, 贴图实测底座宽×|dirY|)；影根 backTuck = extentAlong/2 埋入模型底座之下，
  沿 ⊥ 施加质心 crossOffset。
- 贴图实测：`build-lighting-maps.py` 新增 `base.centerX/width`（alpha bbox 底部 12% 接触带
  列质心）；建筑（GameScene）与散布障碍物（wall-system）注册时换算 `projectionModelWidth`
  （alphaBBox 宽×texScale）与 `projectionCrossOffset`（质心偏移，flipX 变号）；
  defense_base 旧 120/-120 inset 补偿归零。
- 关键配套修复：投影图旋转后按内容 bbox 紧身裁剪——带状采样资产（defense_base 底部 20%）
  旧图内容只占长轴 20%，运行时整图拉伸把内容压回建筑底下（晨昏长影被 footprint 吞没、
  正午贴影完全不可见的根因）。
- 建筑延长段上限 maxOffset 随体量放开（max(43, height×0.5)），晨昏长影不再被恒定 43 吞掉；
  障碍物/树木沿用各自 shadow.maxOffset，接触影与胶囊路径不变。
- 探针：`tools/cdp-sun-shadow-verify.mjs` 三时相冻结（configure animateSun:false + startPhase）
  + ShadowDebug.inspect + 截图；太阳配置必须走 `window.EnvironmentLightingSystem`（main.js 挂载）——
  HMR 后裸路径 import 拿到的是第二实例，配置不到游戏真正读的太阳。
- 坑：重建光照派生图前先关无头 Edge（页面内存映射贴图会导致 PIL save 随机 Errno 22）。
- 回归：test-sun-shadow-geometry 14 项契约 + footprint29/视觉锚9/图层14/wall-depth10/渲染序5
  全绿 + eslint 0 error + Vite build；实机三时相截图 tools/verify-shots/sun-shadow-{noon,dusk,dawn}.png
  （正午影宽=模型实测宽、晨昏长影贴底座延展、影根埋入无错位）。

### 对话：墙顶完整footprint约束，消除走在空气上（2026-08-19）
- 否定“把footprint延展到整张墙贴图高度”的方案：墙贴图下半部是竖直侧墙，纳入地面判定会
  扩大空气可走区。改为检查单位真实脚底圆是否完全被“墙顶四边形∪墙间连接面”联合区域承托。
- 墙顶在iso地面坐标中实测约79.38×79.38；玩家`groundRadius=30`，内缩后仍有安全中心区域。
  `wallWalk.maxUnitRadius`调整为30，使用24个外圆点+24个0.7内圈点、容差1.5做完整footprint判定。
- 人物越出安全区时，不再把中心夹到贴图原始边缘；改为沿越界点到当前墙顶中心做18次二分，
  找最近的完整footprint安全点。墙顶鼠标目标也按半径30自动向内修正。
- footprint采样限制在当前承托墙、连接面两端墙及其四邻格，避免墙链较长时逐样本全量扫描。
- 安全CDP验证：墙顶中心可完整承托玩家；四个原始顶点均判不安全，目标向内修正约20–24px；
  人物主动越出四角10px时回夹约25–40px，保持wall_walk/Z=125且完整footprint受支撑；
  四向跨墙位置误差仍为0，远端BFS和18级楼梯回归保持通过。
- 用户继续复现“上墙瞬间掉地卡死”后新增真实逐帧移动探针，确认旧传送采样是假通过：
  修改前第8帧在(8284.78,3597.78)从stairs直接变ground/Z=0，下一帧被墙碰撞锁死，
  距墙顶中心仍24.9px。
- 根因是楼梯连接面只到墙顶原始边缘，而半径30的安全中心区还在内侧20~24px。
  连接面改为保持80宽并延长到墙顶安全中心；人物完整footprint进入安全区后再从stairs切到
  wall_walk。修复后真实移动15帧抵达中心，0掉落、0卡死，最终surfaceKind=wall_walk。
- 延长连接面后曾覆盖墙顶前外角；连接面中心通道最终按
  `40半宽-30人物半径-0.5边线厚度=9.5px`收窄。最终真实上墙仍全绿，四个墙外角也全部回夹到
  footprint安全区，不再由连接面制造空气可走点。

### 对话：相邻石墙墙顶道路连通（2026-08-19）
- `obstacle_block`真实顶面小于逻辑1×1格，四个相邻方向的顶面间隙实测均约7.04px；
  原每墙独立四边形会让单位在接缝处失去`wall_walk`并被回夹，无法走到隔壁墙。
- 新增`blockWallTopConnectorGeometry`：自动找两块邻墙彼此相对的顶面边，按最短端点配对
  组成窄四边形连接面。只填墙间接缝，不扩大外墙可走边界；配置容差2px、邻墙格距容差2px。
- `_wallWalkSurfaceAt`和墙顶点击解析共同识别连接面；驻墙单位跨缝保持Z=125、位置不回夹，
  `_surfaceWall`在进入下一块顶面后切换到邻墙。
- 楼梯可服务完整连续墙链；墙顶RTS目标新增四邻格BFS，路线逐块经过真实顶面中心，不再从
  楼梯出口直线斜穿到远端墙。
- 安全CDP验证：e1±/e2±四方向各25个跨墙采样全部通过、位置误差0；隔壁墙目标复用现有楼梯；
  远端路线正确经过`wall_v_-1 → wall_v_0 → wall_v_1 → wall_v_2`。

### 对话：26.565°合格版教堂正式入库（2026-08-19）
- 最新素材 `教堂.png` 为1536×1024完全不透明白底；使用项目
  `make-transparent-icon.py` 从边缘泛洪抠底、保留最大主体、羽化去白边，再紧身裁为938×993，
  覆盖正式 `assets/terrain/church.png`。
- 抠图后综合角度25.91°、底座比例2.058:1、中心偏移0px，进入允许误差；运行时统一
  footprint吸附为26.565°并保持2×2居中。
- 显示标定为 `270×286`、`footOffsetY=143`；自动拟合约254.0×127.0、
  26.565°、offsetX -0.58。
- 同步重建 church 的 projection/silhouette/height/normal；仓鼠牧师产出、90秒周期、
  模块升级、造价、HP、出售和存档逻辑全部保持不变。

### 对话：城墙楼梯白灰砖 + 驻梯图层 + 墙顶真实可走面（2026-08-19）
- 使用项目本机ComfyUI `flux2-klein-4b-nolora`生成白灰砖：seed 122820、1024×1024、
  12步；0.95亮度校正后经无缝化和128px周期边缘融合，正式纹理
  `stair_tread_whitegray.png`的H/V边差均为0、平均RGB约192/193/192。
- 白灰纹理追加周期锐化和高对比处理（contrast 2.4 / lift 15 / sharpen 0.8）；
  Blender材质Value=1.10并重新导出八张楼梯资产。成品亮度约116、标准差12.0~15.1，
  浅砖面与深灰砖缝清晰分离；灰黑版保留为可回滚版本。
- 修复玩家偶发陷入楼梯：`_updateDynamicDepths`末尾的普通中立实体循环此前会把楼梯主Sprite
  （下段）从3628.2重新抬到根层3708，发生在玩家排序之后。现多段楼梯跳过该覆写，并以
  整座楼梯真实最高层约束玩家/友军。
- 方块墙顶面改为从`obstacle_block.png`量取真实像素四边形：
  rear(512,85)、right(733,196)、front(512,307)、left(291,196)；点击、移动和越界约束统一
  使用该四边形，楼梯到墙顶保留24px连续过渡带。
- 新增实体墙顶连接面：楼梯顶边两端分别连接到墙顶多边形最近点并注册侧边阻挡；
  连接面持续返回stairs/topZ，进入墙顶后再切wall_walk，消除实际移动中掉地卡缝。
- 对面第二座楼梯不是有意禁止；吸附改为优先最近合法候选，避免最近红色候选遮蔽同墙对面
  稍远的绿色候选。实景中自动对面候选与强制同墙对面候选均为`ok=true`。
- 安全CDP验证：18/18踏步玩家深度高于楼梯；墙顶中心+四顶点5/5正确命中；
  楼梯顶端→墙顶中心13/13采样保持Z=125，无落地帧；四方向、自动对面和同墙对面吸附
  全部通过，段间/墙接口误差仍低于0.00002px。

### 对话：26.565°合格版矿工营地正式入库（2026-08-19）
- 使用素材库 `场景/建筑/新建文件夹/矿工营地.png` 替换 `assets/terrain/mine.png`；
  源图1536×1024自带透明背景，alpha>16紧身裁剪为1200×1013。
- 像素实测综合角度26.966°、底座宽高比1.966:1、左右边界完全对称，
  水平中心偏移0px，进入墙体26.565°/2:1允许误差。
- 显示标定为 `268×226`、`footOffsetY=113`；自动拟合约255.0×127.5、
  26.565°、offsetX≈0，匹配2×2占地。
- 同步重建 mine 的 projection/silhouette/height/normal；矿工生成、1000能源造价、
  1500HP、升级、补员、仓储等待、出售和存档逻辑全部保持不变。

### 对话：城墙楼梯左右端碰撞修正 + 灰黑砖材质重做（2026-08-19）
- 四方向吸附坐标经安全CDP探针确认全部合法；实际场景左右端误拒来自建造底座二次地形采样：
  完整1×1 footprint判定后，四角和边中点又被统一扩张18px，越出斜格误撞邻墙。
  楼梯专用采样改为中心12px、边缘2px；完整1×1占地与实体重叠规则不变。
- 使用项目ComfyUI生图管线生成灰黑砖。远程墙材LoRA因其他会话长视频占队列，未中断对方任务，
  改用本机3080 Ti的`flux2-klein-4b-nolora`，seed 122819、1024×1024、12步；成功后删除
  本任务远程pending prompt，并关闭本轮8190本地服务。
- 新纹理经`make-seamless.py`和128px周期边缘融合，最终H/V边差均为0；
  入库`tools/ai-gen/_depth_templates/stair_tread_grayblack.png`，平均RGB约77/77/78。
- Blender改用Object坐标Box重复投影，踏步、立面、下段底座与上段承重体统一贴灰黑砖；
  移除楼梯支撑对`obstacle_block.png`成品贴图的错误复用，重新导出八张四方向资产。
- 最终独立浏览器验证：四方向均`ok=true`，段间误差0.000015px、墙接口误差0.000007px，
  新纹理资产全部实际加载，运行时无异常。

### 对话：城墙楼梯可移动区域精确匹配贴图（2026-08-19）
- Blender生成器为上下段四方向新增踏步通道四角锚点：
  `walkEntryA/B`与`walkExitA/B`；`data/defense-structures.json`及运行时副本同步入库。
- 每段运行时把四个屏幕锚点按`baseZ/topZ`还原成地面世界四边形；
  `surfaceAt`、两侧阻挡线和RTS/AI中心路线统一读取这个四边形，不再把完整1×1菱形当作
  可走楼梯。
- 楼梯高度从连续斜坡插值改为与模型一致的九级踏步；单位位于楼梯时脚底Z直接贴当前一级
  顶面，不再因高度缓动落入踏步内部或漂在踏步上方。
- 修复点击楼梯贴图的目标反算：逐级尝试九个踏步高度，只接受同一段、同一级的四边形命中；
  RTS路线停在实际点击踏步，不再按`baseZ`误算目标或无条件走到墙顶。
- 同一屏幕像素可能沿相机射线穿过多个踏步；候选选择先服从上下段Phaser实际绘制层，
  再按Blender 30°/45°相机方向选择最近表面，避免点到被遮挡的后方踏步。

### 对话：城墙楼梯按当前26.565°建筑提示词重建（2026-08-19）
- 修正旧版“名义俯角30°、实际方位角0°并反解不对称进深”的建模方式；新版
  `build-block-wall-staircase.py` 使用真正的正交dimetric相机：
  elevation 30°、azimuth 45°、无透视，地面两轴实测斜率
  `-0.50000010 / +0.50000000`。
- 每段改为真实正方形1×1地块（边长`64√2`），投影严格对应游戏
  `(-64,32)/(64,32)`格轴；模型垂直高度按`62.5/cos30°`换算，游戏内每段抬升仍为62.5。
- 上下段继续独立建模、独立碰撞：下段保留完整薄接地板，上段增加从地面直达第二段起点的
  完整实心承重体，九级踏步全部限制在本段1×1占地内，不再用伸出地块的toe/landing。
- Blender 5.1重新导出四方向上下段八张PNG，显示画布改为220×220并同步
  `data/public data`锚点；四方向入口到出口误差均小于0.00003px，alpha边界顶部至少72px、
  底部约75px，无裁切。校准场景保存为
  `tools/ai-gen/_block_wall_stair_rebuild/block_wall_stair_reference.blend`。

### 对话：26.565°合格版研究院正式入库（2026-08-19）
- 使用素材库 `场景/建筑/新建文件夹/研究院.png` 替换
  `assets/terrain/research_institute.png`；源图1536×1024自带透明背景，alpha>16紧身裁剪为
  1233×998。
- 像素实测综合角度26.659°、底座宽高比1.992:1、左右边界完全对称，
  水平中心偏移约-0.12px，符合墙体26.565°/2:1标准。
- 显示标定为 `267×216`、`footOffsetY=108`；自动拟合约254.8×127.4、
  26.565°、offsetX -0.22，匹配2×2占地。
- 同步重建 research_institute 的 projection/silhouette/height/normal；研究项目、被动能源、
  快速募兵、墙门强化、造价、HP、出售和存档逻辑全部保持不变。

### 对话：铁匠铺白底修正版重新入库（2026-08-19）
- 最新素材 `铁匠铺.png` 为1536×1024完全不透明白底；使用项目
  `make-transparent-icon.py` 从画布边缘泛洪抠底、保留最大主体、羽化并去白边，再紧身裁为
  1235×996，覆盖正式 `assets/terrain/blacksmith.png`。
- 抠图后综合角度27.01°、底座比例1.962:1、中心偏移0.12px，进入可接受范围；运行时统一
  footprint吸附为26.565°并保持2×2居中。
- 显示标定更新为 `267×215`、`footOffsetY=108`；自动拟合约256.4×128.2、
  26.565°、offsetX -6.4。
- 同步重建 blacksmith 的 projection/silhouette/height/normal；铁匠铺全部玩法逻辑保持不变。

### 对话：图鉴栏整改——收回修复 + 硬编码清除 + 友军栏目（2026-08-19）
- 图鉴栏点击面板外区域可收回（与状态/背包同一 systemPanel 抽屉动画）；
  修复子页面 UIState 键滞留导致永久拦截收回——改为面板 DOM 实际激活才拦截，滞留键自愈。
- 数据排查：枪械详情半自动分支 `+5°`/`500ms` 两行无源硬编码移除；散布统一走
  spreadParams 逐武器真源，独头弹逐层散布走 weapon-fx-config。
- 新增「友军」栏目：仓鼠 7 兵种 + 矿工 8 卡片，数据走 UNIT_KIND_CFG + 矿工配置唯一真源，
  产出建筑由产兵配置反查；详情 = 基本信息 + 六维 + 怪物公式派生 + 攻击参数。
- 风格：友军卡片青蓝点缀、主 tab 金色激活，契合背包/属性栏金棕主色调；面板标题改「图鉴」。
- 回归：test-codex-ally.mjs 10 项契约 + cdp-codex-ally.mjs 实机 3 项 + 截图验收。

### 对话：26.565°合格版靶场正式入库（2026-08-19）
- 使用素材库 `场景/建筑/新建文件夹/靶场.png` 替换
  `assets/terrain/shooting_range.png`；源图1536×1024自带透明背景，alpha>16紧身裁剪为
  1325×1005。
- 像素实测综合角度26.062°、主边线26.70°/26.525°、底座宽高比2.045:1，
  水平中心偏移约0.11px，符合墙体26.565°/2:1标准。
- 显示标定为 `271×206`、`footOffsetY=103`；自动拟合结果约254.3×127.2、
  26.565°、offsetX -0.1，匹配2×2占地。
- 同步重建 shooting_range 的 projection/silhouette/height/normal；仓鼠火枪/射手产出、
  60/45秒周期、模块升级、造价、HP、出售和存档逻辑全部保持不变。

### 对话：26.565°合格版铁匠铺正式入库（2026-08-19）
- 素材库实际文件名为 `铁匠铺.png.png`；源图1536×1024自带透明背景，alpha>16紧身裁剪为
  1409×1016并替换 `assets/terrain/blacksmith.png`。
- 像素实测综合角度26.306°、左右主边线均约26.575°、底座宽高比2.023:1，
  水平中心偏移约0.1px，符合墙体26.565°/2:1标准。
- 显示标定为 `270×195`、`footOffsetY=98`；自动拟合结果约254.8×127.4、
  26.565°、offsetX≈0，匹配2×2占地。
- 同步重建 blacksmith 的 projection/silhouette/height/normal；铁匠铺造价、HP、能力升级、
  研究等级、出售和存档逻辑全部保持不变。

### 对话：26.565°合格版兵营正式入库（2026-08-19）
- 使用素材库 `场景/建筑/新建文件夹/兵营.png` 替换 `assets/terrain/barracks.png`；
  alpha>16 紧身裁剪为1157×886，中心偏移约0.12px，左右底座对称。
- 像素实测底座角度26.403°、左右边线26.375°/26.425°、宽高比2.014:1，
  符合墙体26.565°/2:1标准。
- 为让游戏内底座精确对应约256×128，显示标定为 `271×208`、`footOffsetY=104`；
  自动拟合结果约255.3×127.7、26.565°、offsetX≈0。
- 同步重建 barracks 的 projection/silhouette/height/normal；造价、HP、产兵、升级、
  单位类型、补员和存档逻辑全部保持不变。

### 对话：矿工营地替换仓鼠小屋美术试点（2026-08-19）
- 使用 `素材库/场景/建筑/新建文件夹/矿工营地.png` 替换 `assets/terrain/mine.png`；
  alpha>16 紧身裁剪由1536×1024变为1205×965，按 `displayW=288` 等比标定为
  `displayH=231`、`footOffsetY=116`，同步重建 mine 的 projection/silhouette/height/normal。
- 用户可见名称从“仓鼠小屋/仓鼠矿场”统一为“矿工营地”；内部 `hamster_hut` id、矿工生成、
  1000能源造价、1500HP、升级、仓储等待、补员、存档等玩法链全部保持不变。
- 美术验收结论：透明裁边、水平中心基本达标（offsetX -1.08px），但底座测得
  **26.565°、约159×79**；未达到新资产目标 **30°、约256×148的完整2×2底座**。
  因此本图可作为替换试点，但不能作为后续30°建筑的合格基准图。

### 对话：射击台改为自动延伸城墙楼梯 + 伪3D攻击高度（2026-08-19）
- 原“射击台”改为配置驱动的“城墙楼梯”：当前墙模型投影实测墙顶高度125，
  两段各抬升62.5；顶段上沿直接锚定墙面真实接触点，不再借用墙体所在格。
- Blender 5.1以当前230×52×150墙模型作参考，搭建墙+下段+上段完整装配体：
  每段占1×1、进深106、宽110、九级踏步，两段物理坡度35.28°；地面、段间和墙顶
  接口误差均为0。删除参考墙后分别导出上下段及e1/e2正负方向资产。
- 贴图按Blender投影的entry/exit锚点从墙顶向地面逐段反推，四方向地面入口误差
  精确为0px，解决旧版墙内多一格、楼梯间大缝和无法登墙。
- 二次实机反馈修正：上段加入从地面到第二段入口的实心承重基座；上下段统一固定
  `ortho_scale=300` 导出，杜绝按各自包围盒缩放导致比例不同。承重基座底部埋入地面
  18个Blender单位，避免30°视觉与26.565°碰撞格差异造成悬空。
- 吸附和通行从“只忽略当前一块墙”改为端点BFS收集完整连续墙链，四种
  `e1/e2 × 正/负`方向均可贴墙，登顶穿越墙缝时不会被相邻墙段再次挡住。
- 新楼梯存档标记 `stairVersion=2`；旧 `platform` 和旧版无版本楼梯不再恢复，
  清除墙上孤立旧楼梯。楼梯Sprite深度统一为目标墙深度+0.2，墙体不再覆盖楼梯。
- 第三轮改为上下段完全独立：每段创建自己的1×1 `Collider`，Sprite不再从墙顶串联定位；
  Blender导出记录每段 `surfacePx`，把斜面中点对齐到
  `segment collider center - segment middleZ`。四方向地面、段间、墙顶三项误差均为0。
- 浏览器实景追查确认当前正式墙为 `obstacle_block` 方块墙，而非旧长掩体。新增
  `build-block-wall-staircase.py`，以原方块墙64×32×80菱形柱为参考重做两段：
  四方向分别直接建模，模型40高度映射游戏62.5，显示统一200×360.844。
- 每段增加完整128×约71模型单位菱形底座，下段薄接地板、上段实心承重柱；实际alpha
  最低点位于footprint前缘下方4～13px（略埋地、不悬空）。
- CDP实景验证：四方向全部可放；两个独立Collider斜面中心误差0；段间误差0.00010、
  墙接口0.00013；墙深度3628、上段3628.21、下段3708，墙不再覆盖楼梯。
- 防御墙注册 `topZ/wallWalkWidth`，玩家、侍从与仓鼠友军统一通过楼梯连续改变实体 `z`，
  墙顶目标解析为“楼梯入口→分段中心→墙顶”的表面路线。
- 投射物增加 `z/vz` 直线弹道，墙体阻挡同时比较交点高度；删除站上高台后无条件忽略
  全部己方掩体的旧特例。近战增加垂直攻击差限制。
- 补齐传统 `RangedAttack`、敌人自带射击、感知LOS、战术小队LOS、仓鼠射手/斥候弹道与
  法系飞行物的高度链；玩家冲刺、击退和友军移动在高架表面只忽略当前连接墙。
- 移除后续添加的墙顶深浅双线附加层；该层在实机中被识别为墙上残留的细长楼梯，
  城墙恢复只显示原始 `obstacle_cover_*` 贴图。
- 新增楼梯组合预览、按段计价、详情、回收、整组沉陷、存档及旧 `platform` 快照兼容。

### 对话：侧栏改版与快捷键整合（2026-08-19）
- 世界切换按钮换正式图标：`素材库/UI/世界切换.png` 白底抠图入库 `assets/ui/icons/world_switch.png`，
  按钮收编进 hud-panels-misc 侧栏静态构建（不再由面板模块注入）。
- 侧栏顺序：技能↔背包对调、世界传送↔组队对调（世界传送在任务后、队尾为队员管理）；
  快捷键徽标同款同位置：世界传送 O、队员管理 P、图鉴让位为 U。
- 暂停整合进菜单：Esc 打开菜单即暂停（游戏循环+定时器+Phaser 三重），P 键独立暂停拆除。
- 回归：test-side-menu-keys.mjs 12 项契约 + cdp-side-menu.mjs 实机 5 项 + 侧栏截图验收。
...
### 对话：建筑 footprint 按贴图底部像素自动拟合（2026-08-19）
...
- 二轮修正角度与居中：仓库实测更接近引擎旧投影26.565°，军营/靶场更接近美术30°；
  拟合角度自动吸附到最近档。底座左右宽度改为对称，Collider中心固定回到2×2格心；
  最低像素只用于纵向接地，不再把台阶/门槛等单侧凸出误当整栋建筑前顶点。
- 逻辑建造锚点仍严格落在2×2格网；贴图、建造幽灵和碰撞四边形共同围绕该锚点校准。
  `iso-footprint.js` 补齐像素四边形的点判定、距离、圆形分离与SAT重叠，攻击距离、出兵、
  放置、阴影、范围显示和图层遮挡自动复用。
- 最终拟合：仓库264.9×132.5 / 26.565° / offsetX +1.4；靶场286.9×165.6 /
  30° / offsetX -1.9；军营260.2×150.2 / 30° / offsetX -8.1。三者碰撞中心X均为0，
  不再偏离2×2占地中央。
