...
- 拖动背包消耗品时先保持背包可见，允许在格子间调整位置；只有指针越过拖拽开始时的背包面板范围才临时隐藏面板，以便投放到底部快捷栏，拖拽结束后恢复。仓库双面板拖放保持原行为。
- 未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户重点测试普通/RTS选中切换、轮盘按钮过滤、墙顶与地面单位开门、背包内换位和越界投放快捷栏。
