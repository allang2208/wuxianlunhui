# 变更日志
### 对话：墙壁—楼梯—Z 轴统一移动项目收口（2026-08-20）
- 城墙顶面、墙间连接面、楼梯踏步、并排楼梯共享面和墙梯 Portal 已统一进入高架拓扑查询与原子表面提交；友方、敌方和 RTS 路线共用上下楼、排队、交接、回地面及失效恢复规则，不再分别维护互相冲突的高度状态。
- 补齐地面入口、楼梯边缘、墙梯交接、相邻墙缝与四墙交汇处的连续承托；并排楼梯按组共享宽入口，建造吸附只接受可达外墙，同时允许在两座楼梯之间补建合法空位。
- 近战按承载面身份限制跨层命中，远程按 Z 轴弹道和真实墙体交点判定阻挡；楼梯导航侧边不再误挡远程攻击，原有索敌和 AI 决策职责保持不变。
- 最终防坠方案改为墙顶真实外轮廓 guard：墙墙共享边和楼梯 Portal 边开放，其余外露边仅对高架单位阻挡；范围显示模式的红线直接显示同一份权威几何。用户已实测确认整个平台、楼梯和墙壁移动达到预期；按项目约定未额外运行测试、构建或浏览器验证。

### 对话：顶部入侵进度条四色渐变（2026-08-20）
- 入侵进度条按危险度连续经过绿色、蓝色、黄色、红色；倒计时进度越高越接近红色。
- 入侵发生后进度条宽度仍表示传送门剩余耐久，但颜色按反向危险度计算：满耐久为绿色，低耐久逐步转红。
- 按项目约定未运行测试、构建或浏览器实机验证，交由用户测试。

### 对话：要塞式压平视图与统一 Z 轴结算（2026-08-20）
- 新增 `FlatViewSystem`：仅接管建筑、墙、门、楼梯与高架单位的 Phaser 显示；实体坐标、碰撞、寻路、弹道和伤害继续读取原有高度真源。
- 建造、RTS 与观察者上下文在捕获阶段隔离 Space，清除通用输入中的遗留按键；按钮失焦、文本输入和重复按键均按模式级快捷键契约处理。
- 新增双份 `combat-elevation.json` 与统一高度上下文；范围形状、持续区域、飞行法术、近战、连锁闪电和圣光释放帧统一执行承载层、体积、LOS 与终止帧墙检。
- 飞行体、范围区域、光束、粒子和浮字统一使用 `displayY = physicalY - z`，逻辑伤害中心始终保持真实物理 `x/y`。
- 审计验证：RTS 35/35、高架远程 22/22、回归 179/179；定向 ESLint 0 error，Vite 生产构建通过。


### 对话：右侧栏目面板统一顶层挂载（2026-08-20）
- 新增 `rightSidebarPanelLayer` 专用挂载层，解决状态/背包仍受 `#uiLayer(z=10)` 父级堆叠上下文限制、
  队员/位面/任务又各自挂载导致单独调整 z-index 仍互相遮挡的问题。
- 状态、装备背包、技能、图鉴共用系统面板，任务、位面、队员管理统一挂到该层；队员招募作为
  从属模态再高一级。普通 HUD、建筑面板、开发面板等均位于其下，暂停菜单等全局模态仍保留最高层级。
- 按项目约定未运行测试、构建或浏览器实机验证，交由用户测试。

### 对话：能源获取、成长与消耗数值审计（2026-08-20）
- 建立当前能源模型：玩家/友军采矿按实际伤害50%入库；基础矿工约25能源/秒，满经济模块的
  6名矿工约624能源/秒；研究被动最高10/秒，什一税满级每名牧师1/秒；单仓容量5000。
- 盘点当前消耗：基础功能建筑各一座约7100能源；单位/矿工模块满级202500能源，铁匠铺能力
  65508能源，研究院研究62826能源，全部升级合计330834能源（另需476118金币）。
- 审计结论：建筑单价局部可用，但整体闭环不合理。矿工数量与效率模块固定500能源导致数十秒
  回本，自动化后能源迅速退化为形式成本；免费出兵/补员令长期消耗有限，最终只剩仓库满仓。
- 发现待整改项：后台采矿漏算攻击伤害/攻速/暴击，满级营地后台375/秒而前台约624/秒；
  全局被动研究可能按每个驻留世界重复结算；墙/塔修理成本显著高于半价回收后重建；新入侵
  胜利不再发放旧十波配置中的500能源；无仓库时无法采矿，存在金币冷启动软锁。
- 下次调整基线：首仓免软锁并提高容量；基础矿工目标5~8/秒、单营地满成长30~45/秒；经济
  模块改递增成本并降低采矿倍率；单位生成/补员加入能源费用；修理必须优于同耐久回收重建；
  被动能源全局只结算一次，前后台统一消费同一采矿公式。

### 对话：手动铺路 + 道路范围移速提升20%（2026-08-19）
- 建筑面板新增`道路`，单格造价10能源；复用方块墙拖动手势，可单击铺1格或沿e1/e2主轴
  长按拖动铺一排。逐格检查边界、实体、建筑预约和已有道路，只对成功新增格扣费；
  能源不足时保留已铺部分并停止。
- `BuildingRoadSystem`新增手动道路格、自动/手动贴图共享、移除与快照接口；
  世界122快照新增`roads:[{i,j}]`，离场捕获、回场恢复，仍不保存Phaser Sprite。
- 道路脚底范围移动速度×1.2：玩家在最终`targetSpeed`链动态乘算，常规怪物、仓鼠兵种和
  队友在`MovementSystem._getEnemyMoveSpeed`乘算；不修改基础速度，离开道路立即恢复。
  击退、冲锋、Dash与卡死恢复不吃道路倍率。
- 道路专项测试扩至31项，快照扩至42项；怪物速度9项、ESLint 0 error与Vite build通过。

### 对话：非墙2×2建筑外围自动道路环与4×4建造预约（2026-08-19）
- 防御塔、矿工营地、军营及配置建筑保留中央2×2实体footprint，建造预约扩为4×4：
  中央4格建筑 + 外围12格道路。16格逐格检查世界边界、墙体/建筑/障碍和既有道路预约，
  任一格不合法即禁止建造。
- 建造幽灵新增外围12格道路预览：合法格绿色、非法格红色，建筑主体在任一格失败时同步变红；
  建造成功后批量清理4×4范围的散布障碍与地面点缀。
- `BuildingRoadSystem`统一管理道路精灵、16格预约和生命周期；出售、回收、沉陷摧毁及场景离开
  自动释放，世界-122快照恢复时由建筑坐标重新生成，无需在存档中重复保存道路Sprite。
- 道路资产通过远程5080生成：最终采用`flux2-klein-4b-walltex`白色石砖候选，再由
  `build-building-road-tiles.py`确定性投影为4帧128×64、2:1菱形精灵表。
  成品平均RGB约`[202,200,197]`、亮度标准差18.2、P99亮度231.3、过曝像素约0.1%。
- 新增道路专项测试与安全CDP探针，覆盖16格预约、12格道路、重叠拒绝、生命周期、
  资源注册、红绿预览和快照恢复。

### 对话：世界-125地牢遗迹 + 生成世界标准工作流（2026-08-19）
- 新增 `scene11 / 世界-125·地牢遗迹`：12288×8192菱形地图，复用僵尸地牢高级
  `blackbrick_7/8` 地砖池和2048分块烘焙，镜头与世界-122统一为0.7。
- 新增 `world125-environment.js`：每次入场随机生成28根石柱、36座烛台和22组摆墙纯障碍
  预制组合，统一经过footprint、菱形边界、排除带、间距、碰撞与图层校验。
- 世界切换面板、主神空间传送门和建筑传送门均增加世界125入口；BGM复用地牢环境音乐。
- 修复世界-122旧快照/HMR矿点在门打开时露出：恢复时按矿簇、基地禁区、建筑碰撞和最小
  间距过滤，恢复后立即清理并同步寻路障碍。
- `SKILL.md`新增“生成世界标准工作流”，固化定义世界、地板、场景、环境、入口、镜头、
  持久化和验收的完整管线。
- 回归：世界125 14项、传送门/世界123/124相邻测试、随机散布重复测试、ESLint与Vite build通过。

### 对话：观察模式与指挥模式 RTS 化（2026-08-19）
- **世界切换改为仅相机跳转**：世界切换面板切世界不再瞬移玩家——观察模式下目标世界
  不生成玩家实体、相机自由平移、自动进入指挥模式；玩家坐标按世界记忆
  （`Game._worldPlayerPos`），面板标记「本体所在」并给「返回本体」入口，返回即原位恢复。
- **中键指令轮盘统一到指挥模式**：轮盘指令下达全部选中单位（仓鼠部队/队友一视同仁），
  不再只限队友；非成员单位按模式映射直写 `_command`（aggressive→自由索敌、
  patrol→移动驻守、gather 仅矿工生效）。
- **指挥模式 RTS 化**：屏幕四缘边缘平移相机；双击同类单位全选屏幕上同类型友军；
  Ctrl+0~9 编队 / Shift+数字加选 / 数字键选中（capture 阶段优先，快捷栏数字键让位）；
  退出指挥模式镜头回归玩家。
- 观察模式守卫：仓鼠不跟随不在场玩家（8 实体统一）、出兵集结点回建筑自身、
  玩家攻击输入抑制扩展到观察世界。
- 回归：test-world-observer.mjs 20 项契约 + cdp-world-observer.mjs 实机 6 项 +
  既有 RTS/快照/结算套件不回退。

### 对话：建筑 footprint 按贴图底部像素自动拟合（2026-08-18）
- 核对靶场与军营素材 alpha 轮廓，确认问题属实：靶场真实接地前顶点位于贴图中心右侧
  约42显示像素，军营约31像素；旧算法只扫描最低像素校正Y，导致贴图地基与2×2
  footprint 横向错位。
- `structure-visual-anchor.js` 新增底部窄楔接地点扫描：中央20%~80%区域寻找最低高 Alpha
  像素，再沿最多3%高度按底部优先加权求接地X；上层轮廓宽度超过12%即停止。
- 参考墙/门“真实接地面线生成碰撞”的方式，从前顶点向上扫描 nominalHeight 20%~75%：
  找到底座宽度达到局部最大值94%的第一行，以该行左右 alpha 边界作为侧顶点，再镜像推导
  后顶点，自动生成四边形 footprint；宽度、深度、Collider中心和前后遮挡线同步更新。
- 逻辑建造锚点仍严格落在2×2格网；贴图、建造幽灵和碰撞四边形共同围绕该锚点校准。
  `iso-footprint.js` 补齐像素四边形的点判定、距离、圆形分离与SAT重叠，攻击距离、出兵、
  放置、阴影、范围显示和图层遮挡自动复用。
- 实机拟合：仓库267.5×131（接近原256×128成功基准）；靶场274.5×173；
  军营262×149。CDP探针确认贴图接地点、碰撞前顶点和实体锚点重合。
- 验证：接地点/拟合9/9、footprint29/29、安全出兵12/12、建筑回归27/27、
  Vite production build通过。

### 对话：多世界并行 M2 阶段一 + 性能前置优化（2026-08-19）
- **后台活 tick 驱动**（`src/world/world-sim-driver.js`）：玩家不在世界-122 时按 1Hz
  增量结算驻留快照——世界后台持续运转，失守/胜利/清波/建筑损失即时浮字通知；
  结算锚点用快照 capturedAt（读档离线时长完整结算）；前台全真时停 tick。
- **波次结算改造**：`wave.progressSec` 跨 tick 累计交战时长，怪物输出按实际交战时长
  分段结算；防守 DPS 每波重算（塔毁则波次停摆待玩家）；移除一次性模式的无防守 ×10 放大，
  两种口径行为一致。
- **世界切换面板升级**：122 行直读驻留快照实况（波次/建筑/损毁/仓库能源），
  打开期间 1.2s 自动刷新。
- **性能前置优化**：分离碰撞 O(n²) → SpatialPartitionSystem 网格宽相（近邻查询+索引去重，
  行为口径不变）；静态实体（墙/门/台/矿点）`_dormantBand` 休眠带 1/4 帧率聚合 dt；
  小地图动态层 100ms 降频。
- **CDP 探针加固**：模块 URL 表启动前捕获 + 裸路径优先（资源缓冲逐出/HMR ?t= 双坑）。
- 回归：test-world122-sim.mjs 17 项 + test-world122-snapshot.mjs 41 项 +
  cdp-world122-snapshot/sim-switch 实机 15+11 项全绿。

### 对话：世界-122后台抽象结算与世界切换面板（多世界并行 M1）（2026-08-18）
- 新增 `src/world/world122-sim.js`：纯数据后台结算引擎——产兵按兵种周期×募兵研究倍率、
  矿工采矿按速率并受仓库/矿点双封顶、读条完成升全局等级、波次按 TP 预算与塔/兵 DPS
  简化结算（墙/门→建筑→基地顺序承伤）、胜利奖励能源写入快照仓库；`commit:false` 为
  无副作用预览。
- `applyWorld122Snapshot` 物化前先按离场时长结算：回场波次进行中则重开、被毁建筑不复活、
  后台失守快照作废重开局；回场浮字战报（击退波次/采矿/补员/研究/损失）。
- 恢复补员修复：出口槽位 750ms 预约窗口导致爆发生成互撞——首只立即生成，缺额挂
  `_restoreTopUp` 按 800ms/个补齐（兵营/产兵/矿场三系统同口径）。
- 新增世界切换面板：侧边菜单「🌐 世界」按钮随时打开，列出主神空间/122/123/124，
  122 行显示快照概况+离线预估战报，一键 SceneManager.switchScene 传送。
- `window.World122Snapshot/World122Sim` 挂载（探针不再依赖会被逐出的 performance 资源表）。
- 回归：test-world122-sim.mjs 13 项 + test-world122-snapshot.mjs 30 项 +
  cdp-world122-sim-switch.mjs 实机 11 项全绿。
...
- 普通建筑2×2、基地4×4、方块墙1×1；修正贴图前缘锚点语义，2×2碰撞中心上移64px、
  基地上移128px，解决贴图与碰撞体错位。
- 长墙和门按真实face线生成窄长地面矩形。4格门中间栅栏中心从错误的`anchor.y+32`
  恢复为门anchor，并精确占2×1格；门内格阻挡、相邻墙可贴边、转角镜像门可共享柱。
- 生产建筑加入固定出口槽位、建筑/单位占用校验、750ms预约、堵塞100%排队与500ms重试，
  新生单位先走离场点，消除密集建筑群出生卡死。
- 建筑面板外点击改为捕获阶段处理：空白处关闭，放置中和建筑本体点击不误关。
- 门专项23项、footprint17项、出兵12项、建筑回归27项及完整`npm test`、Vite build通过。

### 对话：仓鼠火枪与靶场生产建筑（2026-08-18）
- 新增仓鼠火枪：生命150；六维8/30/3/12/3/16，怪物公式派生；移速120，
  每2.5s造成80物理伤害，最近敌人AI且不攻击矿点。
- 接入8×4精灵表：idle9、running11、attacking21、dying15；第10帧出膛并播放fire.mp3。
- AimHelper提前量瞄准目标贴图中心；Phaser绘制54×4黄色ADD曳光弹，弹速1248（P4040口径）。
- 新增靶场贴图 `shooting_range.png` 1024×1092；通用生产建筑配置1200能源、
  30s出兵、上限5，生产仓鼠火枪和仓鼠射手，复用升级模块与出兵进度条。
- 仓鼠兵营移除射手生产，仅允许仓鼠战士与仓鼠盾卫；旧实例配置保留兼容。
- 火枪接入BootScene、GameScene、全局兵种升级、世界122目标优先级。
- 新增 `test-hamster-musketeer.mjs` 19项并接入完整测试。

### 对话：世界-122仓库建筑与能源仓储真值迁移（2026-08-18）
- 使用素材库 `场景/建筑/仓库.png`，裁透明边并缩为
  `assets/terrain/warehouse.png` 1024×1094；新增单格仓库建筑，500金币建造、
  2500耐久、每座默认容量5000。
- `EnergyManager` 从玩家背包能源堆迁移为多仓库聚合：每仓独立存量/容量，
  `getEnergy/getCapacity` 自动汇总；旧玩家/队友背包能源转为待入库，建仓后自动装填。
- 玩家、仓鼠矿工、玩家队友采矿产出在矿点受击时直接入仓；满仓时不继续扣矿点，顶部提示
  “仓库已满，请修建更多的仓库”。
- 满仓后仓鼠矿工回仓鼠小屋待命，仓库出现空间后自动复工；采集指令中的玩家队友停止采矿并
  切换待命。小屋移除失效的“背包扩容”模块和矿工背包/暂存展示。
- 仓库面板实时显示本仓存量、全部仓库总量/总容量和仓库数量；全部既有能源面板继续调用
  `EnergyManager.getEnergy()`，因此自动显示多仓合计。
- 主存档新增 `world122.energyStorage`；旧地面能源拾取和手动拾取统一入仓。
- 审计修复：能源出售/回收先校验仓库剩余空间，防止拆除后退款丢失；胜利奖励显示实际入库量；
  非空仓库禁止出售，仓库被摧毁会明确提示损失能源。
- 新增 `test-warehouse-energy.mjs` 13项，重写 `test-energy.mjs` 15项仓库聚合测试。

### 对话：世界-122防守怪统一目标优先级与远目标兜底（2026-08-18）
- 新增 `defense-target-priority.js`：128px距离档位优先；同档按
  仓鼠→正式玩家队友→玩家→普通建筑→基地排序，再比较footprint距离/威胁/残血。
- 两阶段搜索：320px内选择本地目标；本地为空时选择远处可达结构；结构全部消失后才启用
  远单位最终兜底。当前仍锁定有效结构时禁止误触发远单位兜底。
- 移除世界-122旧“player faction统一+2”评分路径，修复20px仓鼠仍输给7000px基地的问题。
- 正式玩家队友补 `_isPartyCompanion/_enemyTargetable/hittable` 和 `hp/maxHp` 入口，
  使其既能被锁定也能实际受到近战/远程伤害。
- 黑狼 pacing AI 与开门追击改用统一选择器；墙门阻挡攻击和结构攻击人数分摊继续保留。
- 新增 `test-defense-target-priority.mjs` 16项并接入 `npm test`；结构分摊19项、
  PartySystem 268项和寻路性能基准保持通过。

### 对话：世界-122建筑面板与RTS指令反馈细化（2026-08-18）
- 仓鼠军营生成类型按钮移除“仓鼠民兵”，仅保留战士/射手/盾卫；底层民兵构造兼容保留，
  草屋继续可以生成民兵。
- 建筑模式下，在主建筑面板和任一建筑详情面板之外单击左键或右键，会关闭墙门内嵌详情及
  防御塔、陷阱、矿场、军营、铁匠铺、研究院等独立详情面板。
- 铁匠铺能力浮窗把 `warrior/shooter/guard/scout` 映射为仓鼠战士/射手/盾卫/斥候；
  研究院浮窗标签由“目标兵种”改为“目标效果”。
- 指挥模式选中可攻击单位后右键敌人，只有实际下发攻击命令时才触发目标贴图720ms红白交替
  闪现；仅选择不可攻击矿工时不触发。闪现渲染接入 GameScene tint 真源，结束后恢复受击白闪。
- 回归覆盖军营按钮、面板外左右键关闭、能力目标文案和RTS红白指令反馈。

### 对话：4格门吸附四向修复与关闭帧面板图标（2026-08-18）
- 根修 F 切换漂移：方向切换从最后一次原始鼠标世界坐标重算，不再把半格门锚点传回
  `_blockCellOf` 二次舍入；连续切换20次保持同一格位，不再每次向下漂移32px。
- 新增 `gate4-grid.js` 纯几何真源：新建门同时评估正/负半格候选，优先合法且离鼠标最近者；
  长墙枚举全部四格窗口并选最近项，不再固定偏向鼠标负方向。
- 四连墙交叉/L角同时存在 e1/e2 候选时由 F 决定方向；只有单轴四连墙时仍自动跟随墙向。
- 门点击落地强制使用已校验半格锚点，不再在吸附失败时用鼠标原坐标尝试生成离格门。
- 建筑面板 `gate_4cell.png` 改由 `tools/compose-gate4-icon.py` 从
  `cover_gate_D_bars.png` 第0帧关闭状态截取，并按游戏实际顺序合成
  “后柱→关闭栅栏→前柱”；透明 RGBA 输出344×324。
- 新增 `test-gate4-snap.mjs` 14项并接入 `npm test`；建筑回归增加方向切换与图标来源校验。

### 对话：世界-122研究院贴图接入与建筑单格化（2026-08-18）
- 使用素材库 `场景/建筑/研究院.png`：裁除 4096×4096 画布的大面积透明边，缩放为
  `assets/terrain/research_institute.png` 1024×1093；显示标定为 144×154、
  `footOffsetY=77`，预览与实际实体一致。
- 新增 `building-footprint.js` 单一真源。除墙、门、陷阱外的可建造建筑（防御塔、矿场、
  军营、草屋、铁匠铺、研究院、射击台）统一占 1 个方块墙格，实体碰撞统一为
  128×64 矩形、`collisionRadius=26`。
- 上述建筑强制吸附 1×1 格网；同格拒绝、相邻格允许；完整边界和清障范围复用单格参数。
  射击台保留可登台的特殊无推挤逻辑，但建造格与受击矩形已统一。
- 防御塔、射击台和方块墙的幽灵预览改为读取实体视觉常量，消除预览/落地尺寸双源。
- 新增 `test-world122-building-footprint.mjs` 9 项并接入 `npm test`；完整测试和 Vite
  production build 通过。

### 对话：世界-122研究院代码接入（2026-08-18）
- 新增研究院配置，自动进入建筑列表，造价500能源、HP2200、def70/mdef90；复用铁匠铺
  能力工坊读条、持续升级、费用增长和半价出售面板。
- 两项全局研究：①“防御结构强化”共享等级，同时让方块墙与4格门最大生命每级+10%；
  ②被动能源每级每秒+1。旧墙/门独立研究等级按较高值自动迁移到共享等级。
- 新增 `research-system.js`：研究完成即时刷新场上墙门，新建结构构造时自动应用；生命上限
  提升同步增加当前生命差值；被动能源按完整秒结算，仅世界-122 active 时运行。
- 研究等级复用 ability-store 主存档；读档后 `refreshWorld()` 更新现有结构。
- 能力面板描述扩展 percent/flat 两种显示，研究院与铁匠铺分别显示正确作用域文案。
- 正式资源键为 `research_institute`，路径为 `assets/terrain/research_institute.png`。
- 新增 `test-research-institute.mjs` 并接入 `npm test`；完整测试与 Vite build 通过。

### 对话：建筑模式正式计价、半价回收与footprint修复（2026-08-18）
- 新方块墙与4格门统一使用 C 级墙数值：1600 HP / 400 能源；4格门保留已验收 D 视觉。
- 旧 F→A 长墙/旧门从建筑列表移除，底层兼容代码保留；陷阱暂不调整。
- 建筑详情新增三列排版的「回收」按钮，玩家放置建筑按真实成本半价返还；4格门整组回收，
  点击石柱也进入门详情，拆除按门→墙顺序防幽灵碰撞。
- 方块墙、4格门、射击台详情改读真实贴图/名称/价格，不再显示旧D墙275能源。
- 建筑放置按真实碰撞半径与完整 footprint 判断：修复兵营/产兵建筑使用28px、
  固定70px间距导致碰撞重叠，以及射击台/大型建筑伸出世界边界。
- 拖墙在失焦、画布外或面板上松开时取消；批量建造逐块扣400能源，余额不足停止，
  构造失败自动退款。
- 回归：世界-122建筑测试22/22、指挥测试12/12、完整 `npm test`、Vite build、
  `git diff --check` 全过。

### 对话：世界-122指挥模式代码审计修复（2026-08-18）
- **矿工指令**：矿工正式消费 RTS move/hold，目标不可达走附近可达点；保持“只采矿不攻击”，
  右键敌人对矿工降级 hold，不再静默忽略命令或继续旧移动。
- **模式互斥**：指挥模式与建筑模式双向关闭；RTS 鼠标过滤补 `.wall-editor-panel`；
  指挥点建筑后退出 RTS，掩体详情走完整 `BuildingSystem.open()` 生命周期。
- **跨场景清理**：离开 scene8 时全队命令回 follow，清目标、战术点、路径和残余速度。
- **即时响应**：右键移动/待命立即取消盾卫/民兵挥击及射手/斥候尚未出膛动作。
- **选中体验**：单击与框选由脚底小圆改为身体屏幕矩形，高大精灵点击身体可选中。
- **面板真值**：攻击读取仓鼠运行时 AI/aiConfig，移速读取配置速度，不再因站定显示“—”。
- **回归**：新增 `test-rts-command.mjs` 12 项；`test-party-system.mjs` 268 项一并接入
  `npm test`。完整测试、Vite production build、`git diff --check` 全过。
...
- 实现（`defense-system.js` 的顶部 HUD）：
  - `_createHud` 新增进度条结构（`#dhWaveRow`/填充 `#dhWaveBar`/百分比 `#dhWavePct`）；
  - `_updateHud` 刷新间隔 250ms→100ms，实时刷新；倒计时阶段
    （prep 准备 30s / break 波间 10s）显示进度 = 1 - _phaseTimer/周期，
    wave 战斗阶段隐藏；CSS transition 0.2s linear + 颜色渐变
    （<50% 金、<80% 橙、≥80% 青绿）——与仓鼠兵营进度条完全一致。
  - `.defense-hud` CSS 改为纵向布局（阶段+钱币一行，进度条下方）。
- 验证：CDP 实机 4/4——prep 显示且随时间增长（2%→36%）、
  wave 隐藏、break 重新显示；eslint 0 error + vite build。

### 对话：铁栅栏门两项——门-墙相接"左在右之前"图层 + 左墙柱旁补栅栏柱（2026-08-16）
- **需求**：① 门与墙相连时也套用门对门的"左在右之前"拼接缝图层规则；
  ② 门左墙柱旁少一根栅栏柱（贴图空隙），补一根同款柱，开关门时同步滑动。
- **实现**：
  - `syncGateSeamDepths` 追加门对掩体扫描（`_isDefenseCover`）：门 B 端 ≈ 墙 A 端 →
    门在墙左 → 门右柱抬到墙 `_faceDepth + 0.5` 之上；门 A 端 ≈ 墙 B 端 → 墙在门左 →
    门左柱压到 `_faceDepth - 0.5` 之下。只调门柱，不动墙的单一深度（墙两端可能同时接门）；
  - 栅栏补柱：`tools/bake-gate-missing-pillar.py` 把六档 `cover_gate_*_bars.png`
    每帧左叶首柱复制一份、左移一个柱距（38px）粘贴（帧 0~8，9+ 叶已滑出），
    补柱随开关门 16 帧动画同步滑动；帧 0 左叶由 5 根变 6 根（x≈120/158/196/234/273/319）。
- **验证**：`tools/cdp-gate-seam.mjs`——基地门两侧接墙"左在右之前"（右柱 2084 > 墙
  2083.5、左柱 2190.6 < 墙 2277.5）+ 合成门对门回归 3/3；npm test 全绿 + vite build ✓。
### 对话：射击台九版——台阶单通道 + 图层统一口径 + 贴墙吸附（2026-08-16）
- **① 台阶单通道**：新增两条**台阶侧墙阻挡段**（走廊两侧半宽+26 外扩、入口下方 30 到
  台面前缘后 30 延伸）——爬台阶途中不能左右下台，只能从台阶底部上/下，不能左右绕路
  （CDP STAIR_CHANNEL：blockedSide=true、passDown/passUp=true）。
- **② 图层**：按沉淀的「建筑图层统一口径」（SKILL §4 / setupStructureDepth）审计——
  平台此前 `_faceLine=null` 未注册接地线，属于偏差。九版改为构造时
  `setupStructureDepth(this, spriteCfg.size/2)`（_faceLine 水平线 ±149、_faceDepth=
  接地线 y+12），同时保留台上单位显式 `max(仲裁, _faceDepth+1)` 覆盖（台面高于接地线，
  仲裁窗口不生效）。CDP LAYER：unified=true、halfW=149=精灵半宽。
- **③ 贴墙吸附 + 尺寸审计**：
  - 尺寸：平台**台面主体显示宽 260px 与掩体墙（260px）一致**（世界尺寸不同：平台体
    340×84×102 vs 掩体 230×52×150，但显示宽度对齐，贴墙拼接视觉一致）。
  - 吸附：恢复 `_snapPlatformToWall`——平台台面边与墙 face 线对齐：v 墙("/")用后边
    B→L（实体=墙中点+法线×130.7）、h 墙("\")用右边 R→B（法线×161.1）；法线朝鼠标
    侧、F 镜像翻另一侧；无墙时回退自由放置。CDP WALL_SNAP：snapWorks=true。
- **验证**：CDP 探针新增 STAIR_CHANNEL/LAYER/WALL_SNAP 全绿；eslint 0 error +
  vite build ✓ + npm test 全绿。

### 对话：基地门右柱叠 3 个矿点——控制台实锤 + 运行时防叠图自愈（2026-08-16）
- **用户控制台输出实锤**：`(1324,2110)` 有 **3 个矿点实体**（血量 2645/2916/3527，
  精确同点）——就是“极地门右边柱子贴图叠在一起”的真相；另有 (2183,107)、
  (2137,199)、(3031,354) 三个不在任何当前簇的残留节点。
- **定位**：全仓只有 `EnergyNodeSystem.setup` 创建节点且只在当前 4 簇（3000,1500 /
...
- 需求：① 仅按 B 打开建设页面时点击建筑才弹详情，其他时候不弹；② 建设页面时无视距离点击对应建筑；③ 详情面板放建设页面左侧并排。
- 实现：
  - `BuildingSystem.tryInteract`：开头判断 `!this.active` 直接返回 false（非建设模式不响应），并删除 260px 距离检查；
  - 塔/险阱/小屋/兵营四个系统 tryInteract：`Game._buildMode`（建设模式）时跳过 260px 距离限制；
  - 详情面板 CSS：`.build-panel #bpDetail` 改为 position:fixed、right:436px（面板左侧并排），
    并排时不再隐藏建筑网格/操作行/提示。
- 验证：CDP 实机 7/7——非建设模式点掩体不弹；建设模式 1131px 远点掩体弹详情且网格并排；
  详情 right=436px fixed；远距离点兵营/塔打开各自面板；eslint 0 error + vite build。

### 对话：射击台全面审计——空气墙/预览错位/吸附 + 阶梯贴图（2026-08-16）
- **审计① 碰撞 vs 贴图（空气墙）**：视觉台面边缘与游戏菱形经像素核验是吻合的，但
  单向登台阻挡段（半厚 6）把角色挡在视觉边缘内 ~28.5px（玩家半径 22.5 + 半厚）=
  "空气墙"。修复：阻挡段改为**台面菱形外扩 26px 的多边形三条边**（miter 角闭合，
  无角部缺口，半厚 2）——角色能走到视觉边缘（CDP：stopDist=0），也走不出去
  （stop2=66.7，单向登台保留）。
- **审计② B 面板预览 vs 实际位置**：幽灵尺寸 260×242、footOffset 121 是旧资产标定，
  实际精灵是 297×225 / offsetX=-25.6 / footOffsetY=49——预览比实际偏下 72px、偏右
  26px。修复：`_showPreview` 尺寸 297×225、`_ghostAnchor(x,y)=(x-25.6,y-49)`、
  `_ghostFootOffset()=49`；自由放置建筑（射击台）可放置时幽灵变绿（放置反馈）。
- **审计③ 吸附**：掩体/门端点吸附仍正常（CDP coverSnapWorks=true，吸附到
  (693,1896)）；射击台按设计为自由放置（无吸附目标），位置合法即绿。
- **审计④ 台阶贴图**：原踏步是 flat light 素色，无辨识度。新增程序化阶梯贴图
  `stair_tread.png`（512² 暖色石阶踏步面 + 深色前缘立面，辨识度强），渲染管线新增
  `material:'stair'`（spec.stair_tex 指定）；重渲染后踏面呈现清晰台阶纹理。
- **验证**：CDP 探针新增 EDGE_STOP/GHOST/SNAP 用例全绿；eslint 0 error + vite build ✓
  + npm test 全绿；新贴图 684×519（几何不变，标定不变）。

### 对话：兵营出发进度条实时刷新 + 平滑增长（2026-08-16）
- 需求：进度条要实时刷新，有增长效果（原只在点击/升级等交互时重绘，面板打开期间不动）。
- 实现：
  - 进度条元素加 id（`hbSpawnBar`/百分比 `hbSpawnPct`/剩余秒数 `hbSpawnNext`）；
  - 新增 `_tickProgress()`：每 100ms 只更新进度条宽度/颜色/文本，不重建 DOM（避免按钮闪烁）；
  - 面板打开 `_startTicking()` 启动 setInterval，关闭 `_stopTicking()` 清理；
  - CSS transition 0.2s linear 配合 100ms 刷新 = 平滑增长。
- 验证：CDP 实机 5/5——起点 20%、推进后 51%、百分比文本与宽度一致、打开时定时器运行/关闭时清理；eslint 0 error + vite build。

### 对话：能源节点叠图“还是有”——运行的是旧构建（2026-08-16）
- **用户复测**：门口右边叠图仍在。
- **源码侧已三重重证**：新加载 scene8 无叠图（间距 115 修复，27 对重叠→0）、
  防御清理生效（残留节点 setup 强制清除）、簇位 (3000,1500) 远离基地。
- **根因**：用户运行的游戏是**旧构建**——打包版/预览用 `dist/`，此前 dist 由旧代码
  构建（簇位 (2000,1300) + 间距 85 叠图），源码修复从未进到实际运行的游戏。
- **处理**：重新 `vite build` 重建 dist，并**校验打包产物**：
  `clusters:[{x:3e3,y:1500,...spread:320},...]`（新簇位）+ 间距 115 均进包。
  用户重启游戏（打包版重启应用 / dev 模式硬刷新或重启 dev server）后生效。
### 对话：兵营初始上限即 5 个——移除“仓鼠增援”模块（2026-08-16 用户澄清）
- 用户澄清：“上限为 5 个”指“初始上限就有 5 个”，不是从 1 个升增援。
- 实现：`unitCount()` 直接返回 `unitCap`（5），不再参与增援等级；
  移除“仓鼠增援（数量上限 +1/级）”模块（模块只剩 4 个：
  攻击加速/攻击强化/机动强化/生命强化）——兵营初始就能养 5 个兵。
- 验证：CDP 实机 3/3——unitCap=5、初始 unitCount=5、模块不含 count；
  eslint 0 error + vite build。

### 对话：射击台重建模——台阶沿主体前脸法线摆正（2026-08-16）
- **问题**：实机反馈"台阶歪斜、没对齐射击台主体"。
- **Blender 无头检查**（tools/ai-gen/check-platform-align.py，本机 Blender 5.1）：
  台阶 pos 沿世界 -Y 排列（[0,-84/-54/-24]），主体 rot 44.8 前脸法线 (0.704,-0.710)——
  每级台阶位移方向与前脸夹角 45~55°（应为 0°），顶阶背面距前脸 33 单位、底阶 93 单位
  （台阶整串悬空）。台阶自身 rot 44.8 与主体平行（踏步方向对），错在**摆放方向**。
- **修正 spec**：台阶改沿前脸法线（局部 -y）排列：pos (61.3,-33.8)/(40.1,-12.5)/
  (19.0,8.8)，z 保持 13/30/43/60/73/90；顶阶背面贴脸（间隙 0）、中/底阶沿法线
  30/60。复核：disp_xy_angle_to_normal≈0°，顶阶投影正好落在台面前缘。
- **重渲染 + 重标定**：render-cover-real.py 重渲（材质 factory_wall_tex.png 提亮暖灰
  匹配旧观感）→ 紧身裁剪 684×519 → 覆盖 assets/terrain/firing_platform.png(+h)；
  calibrate-platform.py 按相机矩阵投影标定新几何：显示 297×225、offsetX=-25.6/
  footOffsetY=49；台面菱形 L(-173.6,-60.4) F(-122.5,-33.8) R(86.0,-137.2)
  B(34.9,-162.9)；台阶走廊 E(0,0)→D(-27.4,-81.1) 长 85.6 半宽 110 dir(0.320,0.947)。
  单向登台（台面三边阻挡段）沿用新菱形。
- **验证**：CDP 探针全绿（表面全覆盖、精灵锚点、脚线=台面、单向登台、死亡清理）；
  eslint 0 error + vite build ✓ + npm test 全绿。旧贴图备份：
  tools/ai-gen/_platform_align/firing_platform_old.png。

### 对话：每个兵营仓鼠兵上限 5 个（unitCount 硬 cap）（2026-08-16）
- 需求：每个兵营的仓鼠兵上限为 5 个。
- 实现：`BARRACKS_CONFIG.barracks.unitCap: 5`，`unitCount()` 返回
  `Math.min(unitCap, 1 + 增援等级)`——默认 1 个、仓鼠增援每级 +1，
  4 级达 5 个上限，5 级封顶不再超过（原最多 6 个）；
  增援模块描述注明“兵营上限 5”；面板状态区 x/5 自动生效。
- 验证：CDP 实机 5/5——lv0=1、lv3=4、lv4=5、lv5=5（封顶）；
  eslint 0 error + vite build。

### 对话：仓鼠兵营放大——高度对齐小屋 147、占地匹配（2026-08-16）
- **需求**：角度修正成功后，兵营视觉偏小——放大模型、占地面积匹配仓鼠小屋。
- **实现**：
  - 显示尺寸按贴图内容等比（682×589，比例 1.159）放大：displayW 150→**170**、
    displayH 130→**147**（高度对齐小屋）、footOffsetY 65→**73**（294×170/682）；
    宽度 170 = 147×682/589，44.8° 菱形不变形，角度保持与防御塔/掩体一致。
  - 命中盒随显示放大：`BARRACKS_HIT.hw` 75→**85**（= displayW/2，覆盖整屋）；
    遮挡线 setupStructureDepth 自动用 displayW/2=85，覆盖放大后的显示宽度。
  - 物理占地与小屋一致：radius 保持 **40**（小屋同为 40）。
  - 单位生成点外沿起算：`_findUnitSpawn` 最小距离 = halfW+20（半宽 85 → 105px），
    兜底点同步外移，防止生成点落在放大后的建筑内。
- **验证**：CDP 实机 8/8——displayW/H/footOffsetY=170/147/73、spriteCfg 尺寸、
  radius 与小屋一致（均 40）、faceLine 覆盖显示宽、生成点 ≥105px 在建筑外；
  eslint 0 error + vite build。

### 对话：仓鼠兵营默认生成调整——删除基地旁自动战士/射手 + 兵营显示比例修正（2026-08-16）
- **需求**：① 删除默认在基地旁生成的仓鼠战士和射手（单位只由仓鼠兵营 30s 生成）；
  ② 兵营模型没问题但角度要匹配仓鼠小屋/墙壁等其他建筑。
- **实现**：
  - scene-manager `_loadScene8` 删除 `HamsterWarriorSystem.setup(player)` /
    `HamsterShooterSystem.setup(player)`（含 import 与离场 teardown 引用）；
    两个系统文件保留（测试脚本引用），主流程不再自动生成。
  - 兵营显示比例按贴图内容等比修正：贴图内容 682×589（比例 1.159），
    显示从 150×147 改为 **150×130**、footOffsetY 74→**65**（294×150/682）。
    原 150×147 把 44.8° 菱形垂直拉长 13%（接地线斜率 0.50→0.57），
    与防御塔/掩体角度错位；修正后宽同小屋、角度与 44.8° 建筑一致。
- **验证**：CDP 实机——scene8 就绪后 entities/friendlyUnits 均无默认
  hamster_warrior / hamster_shooter；`BARRACKS_CONFIG` displayH=130、
  footOffsetY=65；eslint 0 error + vite build。

### 对话：能源节点“贴图叠在一起”根因——簇内间距 < 贴图尺寸（2026-08-16）
- **用户复测**：门口叠图仍在（极地门右柱、可采集蓝色大矿点、贴图叠在一起）。
- **根因实锤**（tools/cdp-node-overlap.mjs 实测）：节点贴图最大 91px
  （nodeSize 84 × 显示缩放最高 1.08），但簇内最小间距只有 85px——**全图 27 对
  节点贴图重叠（其中 4 对 <90px 严重叠图）**，四个簇都有。这就是“门口/门柱叠了
  一堆矿”的观感来源：玩家在任一簇附近建门/屋，就会看到互相叠压的矿点堆。
- **修复**：
  - energy-node-system：节点最小间距 85 → **115**（保证任意两节点贴图间有明确空隙）；
  - energy-config：各簇 spread +50（320/320/320/300），14 节点仍能全部落位。
- **验证**：<110px 重叠对 27 → **0**（<90px 4 → 0），最近间距 115px，总数仍 54；
  防御清理（残留节点 setup 强制清空）+ 簇位 (3000,1500) + 右键移动/箭头回归全部
  通过；test-party-system 268/268、eslint 0 error、vite build ✓。
### 对话：能源节点“门口叠图”根因 + 并行 reset 事故恢复（2026-08-16）
- **用户反馈**：基地门右柱位置出现可采集蓝色大矿点、贴图叠在一起。
- **排查**：全新加载/反复进出世界-122 均为 54 节点、无重叠、无堆积；代码无任何
  “门口/出门生成矿”路径。叠图只可能来自**运行会话残留节点**（多次 setup / HMR
  模块重载 / 旧布局残留叠加）。
- **并行事故**：并行会话 `git reset` 清掉了本人未提交改动（能源簇调位 (3000,1500)
  、移动指令、伊莉丝音效、左右键移动+箭头等）——已全部重新恢复并回归通过。
- **防御修复**（energy-node-system.setup）：生成前先移除场上所有残留 `_isEnergyNode`
  实体，**无论会话如何残留都不会堆积叠图**。探针验证：注入 5 个门口残留节点 →
  重新 setup → 归位 54、门口 0。
- **验证**：簇位 (3000,1500) 生效（旧位 0 / 新位 14 / 出生点最近 2073px / 旧建屋点
  门口 0 矿）；右键移动+箭头、选中/指令、采集入包、档案恢复、音效全部回归通过；
  test-party-system 268/268、eslint 0 error、vite build ✓。
...
- 用户澄清：要删的是**脱离在房屋主体外面**的那根（前左塔，世界 (-97.9,-193.1) 悬在
  房前），不是屋后那根（后左塔）。
- 上一轮误删后左塔已恢复（local -18.5,68）；本轮删除前左塔（local -205.5,-68）。
- 最终塔布局：保留 前右(70.5,-68) + 后左(-18.5,68) 两座贴主体塔；删除 前左 + 后右
  两座脱离主体外的塔。重渲入库 `assets/terrain/hamster_barracks.png`（692×598，
  瓦行角 -26° 平行斜边不变）。

### 对话：仓鼠兵营删左塔——后左塔与后右塔同模式细高柱（2026-08-16）
- 用户要求：左边的柱子也删掉（此前已删后右塔）。
- 定位：左侧两塔——前左（前景大柱 x12-144）、后左（x156-172，从 y=35 顶部升起的
  细高柱，与已删后右塔同模式）。按"也删掉"口径删除**后左塔**（local -18.5,68）。
- 重渲入库 `assets/terrain/hamster_barracks.png`（636×623），左上露出红瓦屋顶；
  剩余前左/前右两座前景塔。

### 对话：仓鼠兵营屋顶 v4——去掉檐口圈，斜面加厚回归棱柱实心楔（2026-08-16）
- 用户纠错：加厚逻辑错了——要"斜面本身加厚"，不是斜面下加一圈屋檐（红檐口板）。
- 回退：删除红檐口厚板；屋顶回归棱柱实心楔（H 68→76，更厚实），瓦片保持正确
  （瓦行角 -26° 平行斜边，条纹测试实证）。
- 尝试过的厚斜板方案（roof_slab 图元：顶/边红瓦、底黑砖，两块在屋脊相接）：
...
- **未改码**；如要让“小屋周边无矿”，建议做**小屋禁矿带**（放置时移走周边节点）
  或调簇位，待用户拍板。
### 对话：射击台七版——表面可走一对一标定（自由放置高台；修复"空气墙/悬浮"）（2026-08-16）
- **背景**：六版自由高台上线后实机仍打回："走上去再往前走，形同走上空气墙、悬浮在空中"。
  逐像素审计贴图 + 建模投影（正交 30°、rot 44.8）后确认五/六版几何与贴图**完全错位**：
  ① 贴图不对称——台面菱形在右上、台阶在左下，入口=贴图 (211,581) 而非底部中央；
  ② 台面是水平面（世界 z=102 → 屏幕恒定抬升 97.6 display px），六版却按 178px 恒定
  抬升，脚浮在台面上方；③ 台阶轴沿左下↔右上对角线（dir (-0.401,+0.916)），六版走廊
  仍竖直 (0,1)；④ 满抬区只有 40px，台面实际深 ~105px——再往前走抬升截断 = 空气墙。
- **修复：表面可走模型**：单位逻辑坐标 = 台面/台阶的表面屏幕位置（无抬升；
  `_platformLift` 只作"在台上"标记 0/1，深度覆盖 + 弹道忽略用）。
  - 精灵渲染：`spriteCfg.offsetX=51.1` / `footOffsetY=96.3` 把贴图入口锚定到实体；
    GameScene 中性精灵支持 `spriteCfg.offsetX` 平移。
  - 台面菱形（相对实体）：L(-78.9,-84.9) F(-27.6,-52.0) R(181.1,-184.4)
    B(129.8,-217.3)；`isOnPlatform` = 点-in-菱形 || 点-in-台阶走廊
    （E(0,0)→D(42.3,-96.5)，长 105.4、半宽 104）。
  - 越墙三件套 + 深度覆盖（`_faceDepth+1`）保留；自由放置/无贴墙/沉陷死亡/详情面板
    沿用六版。
- **验证**：eslint 0 error、vite build ✓、npm test 全绿；CDP 探针改造——台阶走廊 +
  台面菱形全覆盖（前角→后角 21 点无断崖，无空气墙）、精灵锚点 = (e.x+51.1, e.y-96.3)、
  玩家脚线 = 台面表面（误差 ≤2px）、无墙线/无密封段、沉陷死亡清理、贴图渲染。
  建模沿用五版 v7（台阶固定左下↔右上对角；如需"四面可登/哨塔护栏"再重建模）。

### 对话：射击台单向登台（2026-08-16）
- **需求**：走上射击台后不能再从另一边下来——单向，只能从同一边上、同一边下。
- **实现**：台面菱形左/右/后三边注册 `_platformEdge` 阻挡段
  （`WallSystem.isoSegments`，halfThick 6，`_cover/_owner=平台`——怪物被挡也转火
  平台），只留台阶所在前边（F→R 整条经投影验证都在台阶走廊内）进出；台上不能从
  其它边走下去、地面单位也不能从其它边走上台；`onDeath`/`destroy` 清理阻挡段。
- **验证**：CDP 探针 EDGE——edgeSegCount=3、向后角/左角走被挡（blockedBack/
  blockedLeft=true）、向前边 D 走直达（passFront=true）、入口→台阶顶通路畅通
  （passClimb=true）、死亡后 lingering=0；eslint 0 error + vite build ✓ +
  npm test 全绿。
