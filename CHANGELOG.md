...
- 已输出可编辑 `.blend`、1024×1024 透明材质预览与同视角 Depth 图；当前仅完成建模候选，未导入 `assets/terrain`，也未接入科技树、建造配置、能源生产、存档或运行时动画。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户先确认模型方向。
...
- 用户选定 `123171` 后新增结构锁定的精修提示词，并以同一 Depth、`denoise=0.30` 生成 `1231711`、`1231712`、`1231713` 三张 48 步低重绘候选；三张均未改变三环、四柱、单晶核与完整地基，当前推荐蓝紫晶核、旧黄铜和黑铁层次最均衡的 `1231713`，等待用户确认后再做真透明、footprint 验收与正式入库。
...
- 雪原初级地牢新增 `combatRoom.wallConstruction:"worldBlock1x1"` 专用构建模式：房间四边严格按世界位面 128×64 的 1×1 footprint 铺设独立墙块，普通房为21个网格中心/边并按房间档位等比例扩展；四个转角各只复用一个墙块并以两条半边闭合碰撞，每块墙携带独立边界线段和 depth，房间改用2:1菱形，避免旧长墙整体拉伸、转角重叠和单深度遮挡。
...
- 在同一组件真源上新增打开状态：箱盖绑定后侧铰链并向后开启62°，箱体、四足、正侧面宝石和卷草纹继续复用关闭版，仅增加深色内衬与空的蓝色仓储辉光。已渲染独立 `.blend`、preview、Depth并完成12步结构候选；结构确认前不进入48步，也尚未加载为运行时打开贴图。
- 仓库仍保留 `NPC`、`npcType:"warehouse"`、点击交互、距离自动关闭和 `WarehouseSystem.open()` 原链，仅物理/遮挡身份使用标准1×1格网建筑：`applyBuildingFootprint(..., 1)` 给出128×64等距菱形碰撞，`setupStructureDepth()` 接入建筑拓扑深度。未运行测试、构建、lint、浏览器探针或游戏运行时验证。
