...
- 世界-125死寂雾潮从开发用纯视觉态升级为统一天气玩法态：友军单位基础视野在雾潮中乘 `0.6`；进入存活守夜烛台的照明范围后获得“烛火庇护”并恢复为 `1.0`。夜晚继续先乘全局 `0.5`，因此范围外最终为 `0.3`、范围内以雾潮 `0.9` 修正得到 `0.45`；祭品、高地等原有视野倍率继续乘算，建筑视野和攻击距离不受该单位惩罚。
- 雾潮中的僵尸按配置 `family:"僵尸"` 统一获得移动速度 `×1.2`、普通攻击间隔 `×0.85`；倍率只进入最终移动和攻击计时链，不改写基础属性、攻击距离、动作动画或技能冷却，结束雾潮或离开scene11后即时恢复。
- 复用现有三烛台素材新增可建造特殊建筑“守夜烛台”：仅scene11允许建造，完成“地牢探险团”科技后解锁，消耗500能源，拥有1200生命、碰撞实体且可被摧毁/出售。原随机生成的36座环境烛台仍是纯装饰，不提供不可摧毁的安全区；新建筑接入既有烛光、火星、战争迷雾、位面快照与销毁生命周期。
...
- 丛林祭司法术改为完整 17 帧单次施法锁：第 8 帧（约 583ms）才结算闪电/冰锥/火球，释放前复验目标、距离与视线；原生命、六维、伤害、移速、2800ms 攻击间隔、三法术轮换及丛林神庙生产/升级/后台模拟协议保持不变。未运行测试、构建、浏览器探针或游戏运行时验证，按项目约定由用户测试四动作、奔跑循环、施法命中时点、死亡清理、贴地尺寸和 HUD 高度。
