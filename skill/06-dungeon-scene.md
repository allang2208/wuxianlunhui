...
- 准备页与确认打包共用`TroopLineSystem.getStrategicCandidates()`：合并当前`Game.entities`、友军列表及本位面产兵建筑名册，以共享军事兵种表和稳定单位ID识别、去重。来源兵营/原始位面不能代替当前驻留判断，不扫描其他位面快照来凑名单。
...
- 世界-126：复用废弃矿洞连续地面与现行配置中的纯视觉小物；五款正式矿洞障碍由
  `world126-environment.js` 按逻辑位面实例的世代 seed 散布，保持获批视角且不翻转/旋转。
  固定 `scene12` 只作模板预览，正式剧情必须通过独立实例进入；障碍依次通过菱形内缩、
  玩家/传送门排除、中心距、footprint 间距与墙碰撞探针，深度固定取 footprint 前缘。
...
- **沼泽发光植物显式例外（2026-08-30）**：用户要求墙上生成五款发光植物后，`swampStone.wallDecorations:'swampGlowPlants'`
  接入同一挂饰系统但跳过矿洞火把/粒子；`data/swamp-wall-plants.json`与public副本登记五款双墙向原生渲染、挂点、
  发光器官投影点和独立柔光参数。仍只读最终内侧直墙，复用门口/转角/出生点避让、坐标散列与数量上限；
  不修改已确认细枝墙或左右生长藤蔓门，不加入碰撞或地板小物。光源使用现有ADD环境柔光，随挂饰生命周期注销，
  非物理遮挡点光源；新挂饰不自动启用其他地牢。来源、文件及实机待验收项见`docs/swamp-wall-plants.md`。
...
- **真实阶级与模板晋升分开**：`matchPoolRanks:true` 已选中的真实 `elite/lord/boss` 必须保留自己的名称和配置数值；模板的 `eliteModifier`、统一改名与晋升倍率只允许作用于从 normal 临时晋升的占位怪，不能再次套到真实精英上。自动注册表若要供既有 `new MonsterClass(x,y)` 调用，暴露值必须是可构造的普通函数或类；箭头工厂即使可直接调用，也不能冒充构造器。
...
9. **僵尸六格门口墙柱/栅栏/图层合同（2026-08-27）**：和冰封门一样，门洞端点保留普通
   ...
   闭合有效高 `225.7447px`，不得恢复比墙顶明显更高的旧门框比例。`depthSlices:6 + tuckEndSlices:true` 必须由竞技场门、
   全局 `WallGate`、`ChestRoomSystem` 独立门同时消费；首尾门片分别取门洞外端点 `Y+3.9`，中间门片按自身底边
   `maxY+3.9` 排序，墙柱为 `maxY+4`。门叶模型平面必须落在 `GATE_WORLD_A/B` 定义的墙基准线，Alpha 收进
   `gateX`；历史 mask 若尚未由新模型重渲染，只能在 geometry 中登记一次性 `materialFinalizeOffset`，新模型必须导出
   `[0,0]`，不得把该补偿写进运行时锚点。离线总览也必须按同一六段规则合成，禁止用整门单一中心 depth 伪验收。
...
- `swampStone` 与 `swamp_living_block_a/b/c/d` 保留为兼容键；最新要求只保留细碎枯枝并做随机版本，现行四款为碎枝随机A/B/C/D，删除分叉粗根/横卧断木/大扭结以及后层粗根填充，不再用粗壮结构拉开区分度。仍共用 `data/swamp-stone-wall-kit.json` 几何/预载数据，public副本同步，`wallDesign: deadwood-thicket`。脚点 `[512,761.9959]`、显示260×259、footprint128×64、wallH132、halfThick13不得逐款漂移。
- 四款只用短小分叉细枝，基础半径不超过0.042；种子831011/831077/831149/831233改变面层走向、分叉、簇位置与疏密，填充/接缝/接地也不能藏粗枝。用户已确认采用当前四款与拼接外观，后续无新视觉要求时保留现有图/模型/种子，不主动重渲；外观确认不等于实机验收。制作种子固定、运行时世界坐标选款稳定；独立原尺寸对照与连续混排分别出图，不能用固定ABCD序列冒充运行时混排。木纹UV沿单根弯枝延伸，细枝纹理频率不能高到被平均成光滑泥色；同灯组/同材质族下调整枝条覆盖平衡阴影，不靠逐款曝光制造差异。枝间孔隙不表示可通行，不新增砍伐，禁镜像。世界坐标64/32步长会令散列低两位固定，四款须取 `(hash >>> 8) % 4`；通过样式 `blockVariantHashShift` 限定，不改其他地牢选款。
- 枯枝墙制作入口沿用 `docs/swamp-living-wall-kit.md` 中脚本名，当前源归档为 `_swamp_deadwood_wall_kit_20260830`，只输出/安装四张墙图，已确认藤蔓门保持只读；旧石柱安装器在植物墙或枯枝墙激活后不得整套回退。门专用 `--gate-only` 必须保留当前墙登记，只更新门字段。
- 清理后旧石柱脚本始终只出/装藤蔓门，不再依赖旧石柱PNG；共用函数、门blend和16帧仍是有效来源。绿植/石柱废案移出版本库，清单见 `docs/swamp-delivery-cleanup-20260830.json`，最终枯枝拼装GIF保留；纯展示重跑保留当前外观确认和安装状态。
- 三档战斗使用 `worldBlock1x1`、6格门、8格通道；末房继续实体宝箱房。旧 `swamp` 连续墙样式保留，不能删除其预制件依赖。
- 门叶独立建模，端点单格墙兼任门柱；最新动画为左右根部固定、藤梢向中央生长/向两端回缩、叶片随后展开，16帧0关闭/15清空。逐帧曲线几何及叶片关键帧是真源，禁止整图压缩或恢复沉降；六片排序与端片内 tuck 沿用通用逻辑，不设置整片升降用的 `leafMotion`。重建入口、源文件和离线样张见 `docs/swamp-stone-wall-kit.md`；离线素材测量不能代替实机通行/遮挡验收。
...
以下为本地实现合同，功能仍待战略框架合入后发布；范围、未实现功能及人工验收见`docs/world-map-settlers-2026-08-31.md`。

- 组建从真实空闲居民迁出人口，默认10人、300粮食、500能量，参数读`world-strategy.json/settlers`；资源不足须退回人口预约，回调延后到记录提交。不能复制士兵、直接清除岗位或重复收建城费。
- 移民以独立ID与路线存档，最多4队；共享世界时钟、地形图与到达队列。停令完成已走出的当前路段；恢复存档不再次扣费或赠人，建城再次校验并一次性消费该队。不为地图外的居民创建实体场景。
- 城市间距按六边格`max(abs(dq),abs(dr),abs(dq+dr)) >= 5`判定，不按寻路长度。存活敌城、己城和未发现位面预留城址均受保护；营地不扩5格禁建圈，但据点与废墟本格不重叠。敌城/营地仍只可摧毁，不改为占领。
- `worldMap.reservations`独立持久化，不随据点列表变化重新随机。旧档缺项只补齐一次，保留合法种子候选并避开已有存活移民新城；已知入口优先，不能迁移已建城市。预留不提前发现、解锁或刷守军；定位/首次接通才转为入口。位面重建只更新世代，战略城址不移动；新游戏才重生成整图。
- 战略新城修复的显示与扣款共用`StrategicCampaign.settlementRepairQuote`及`CrossPlaneResourceSystem.quote`。点击时复核显示金额与免费状态，变化则不扣费并刷新；向原支付服务传基础费用，避免重复跨位面倍率。此约定只针对战略新城修复，不改普通建筑修复规则。
- 新城人口是独立定居记录，不伪装成同地貌老基地的人口/仓库。当前未实现独立城市内景、住房岗位、增长或补给源，不能仅添加“进入城市”按钮宣称完成。
- 移民使用旗帜兵棋，缩放/轻摇/接地点只影响显示，不移动真实逻辑格。篷车废案不是新旗源链依赖；保留原生模型、纹章、脚本及投影元数据，来源与候选验收状态见对应制作记录。

### 战略层状态与战场隔离

以下为世界战略开发的实现合同；当前本地接入与远端发布范围分别见`docs/world-strategy-expedition.md`及`docs/world-strategy-publication.md`，不表示已完成运行时验收。

- 世界地图只持有六边格、信标、军团和据点记录，低频推进；只在接敌时加载当前一处战场，不能为每格或每场后台战争创建完整实体场景。后台聚合结算是近似，不承诺与现场物理战斗等价或未经测量的性能数字。
- 位面信标在所属区域选格后按`sceneId/cellId/worldEpoch`保存；重开面板不重新抽签，旧入口不因视觉换图迁移。击败守军写独立资格，普通地牢通关和真实传送门构建仍由各自系统决定。
- 多城入侵以`state.battles[sceneId]`为权威，`active`仅兼容摘要。现场/后台切换、解围暂停与剩余波次均指定城市；不能把旧全局单场状态乘上敌军数量。位面重建还需校验世代，避免旧战事污染新城。
- 出征确认前选择不改变单位；确认后`strategicTroops`作为携军唯一账本，原兵营保留外派人口占用。只物化本次编组，未选单位留守；死亡、撤退、返营和转场失败不复制实体或返还一份额外兵力。
- 行军态可存档；加载中、战略系统忙碌或战斗态禁止存读档。读档先恢复世代/队友/兵线记录，再恢复战略地图；携军档延后原位面实体生成，不能把旧现场快照写入新存档。普通地牢钥匙、失败与入侵额外推进不由战略战斗冒充结算。
- 战利品领取沿用原物品入包入口，仅扣除实际收取量，满包和部分堆叠保留余额。地图选择、同格视觉错开与兵旗脚点不改真实格坐标；全屏世界/编组面板独占输入，避免快捷键穿入玩家或RTS。
- 战略迷雾与scene8～scene11的`FogOfWarSystem`相互独立，不能复用位面内部128px像素网格、遮挡器或小地图数据。战略层按六边地格维护“当前可见/已经探索/从未探索”三态；探索记录必须携带世界地图key，旧档缺字段时从已定位信标和当前侦察源初始化。
- 战略视野只由真实来源提供：基地、出征军团/分遣队、斥候、移民/新城、天气预测塔、位面观测阵列及已完成政策。移动敌军、敌军列表和持续追击只使用当前可见情报；曾发现的固定敌方据点可在已探索区保留，未知区悬停不得暴露地貌和敌情。
- 玩家普通下令、路线预估和补给线路只能规避已知敌人；实际接触、拦截和战斗继续读取完整战略状态，不能为了防意外让寻路泄露隐藏敌军。远景迷雾用单张Canvas缓存，近景只遍历视口地格，地形耗时镜片也不能绕过情报过滤。
- 侦察单位或战略建筑完成行军/部署后必须立即刷新视野并合并探索记录，不能等面板轮询才揭图。战略迷雾参数属于世界地图生成配置，不应修改战斗场景可见半径、武器视线或位面内部小地图。

### 等距投影素材规范（按场景构建体系选择，禁止跨体系混用）

**战略六边格世界地图另用55°正交俯视（从水平地面量起）**，相机位于-Y、+Z向上、滚转0°；地貌与兵旗共用`tools/ai-gen/world-map-camera.py`，完整说明见`docs/world-map-camera.md`。下文的战斗场景投影/角色billboard规则不用于制作战略地图模型。
军团旗布保留用户选定的V2约16.7°前倾与完整纹章，不能为了投影统一再次压成V3竖直旗布；地貌、旗座、接地点和地面圈仍按55°统一。
