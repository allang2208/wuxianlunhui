...
- 审计事件须遍历成功、失败、无检定结果及实际投递入口，不能只读材料表。限定事件按自身等级匹配奖励预算，通用事件按当前地牢等级；保留金币、恢复、Buff、失败代价与战斗配置，不给所有分支机械追加物品。
- 事件预览与结算共用分级函数；旧 `rewards` 与 JSON 的 `items` 先归一，再缩放数量。强化石/改造券/粉尘沿用正式模板和第 04 卷堆叠规则；占位普通矿石奖励与合法矿石祭品必须区分。
- 卷轴品质读取正式 `common/uncommon/rare` 字段，不拿地牢 F–A 等级直接过滤；模板按 `scrollId` 匹配，不能假定对象键总是 `enchant_scroll_${id}`。
- 奖励计划完整解析后才锁定；解析失败允许重新选择，投递失败保留同一计划重试，不重新抽取或重复发奖。与战利品暂存/邮件协议相关的实现必须同源迁入。
- 声称物攻/魔攻双收益的 Buff 必须同时写入两种字段；持续整局的图标用 `persistent` 生命周期，不用有限毫秒常量冒充永久；离场与实际增益统一清理。

### 地牢进入提交点与异常返回

- `ExpeditionSystem.depart()` 绕过普通切场入口，不能假定 `switchScene()` 替它回滚。先完成资源准备与初始化，在同步提交段扣钥匙并绑定世界目标；扣费回执只恢复本次已扣数量。
- 入场前保存主城现场，初始化前标明地牢现场。未提交失败不登记通关/失败或推进入侵；仅清理本次新建奖励 run，不覆盖旧邮件、回执或已结算记录。
- 死亡、资源故障、放弃、安全撤离、通关保留各自奖惩边界；回城失败可以重试，但惩罚/结算只执行一次，回滚保留暂存友军。旧 loading 淡出不得隐藏新一轮加载层。
...
- **传送门等复合建筑的接影层**：先读取 PNG 真实 Alpha，不能根据忽略透明通道的预览断言平台没有拆分。宽平台通过显式 `groundContact.depthMode: ground` 绘于 `STRUCTURE_GROUND_CONTACT`，顺序为道路/铺装 < 地台 < 共享结构阴影 < 建筑立面；原 `rearFx` 墙脚覆盖层不全局降层。
- **同画布拆层对齐**：`alignToBody` 让地台读取主体最终拟合中心，避免再次缩放独立脚点造成重影；固定地台不参与竖直门体遮挡候选。影根使用门柱底座/门槛的闭合接触面，不使用全平台，也不把下边缘折线直接闭成过窄薄带；不改碰撞与占格。压平视图注销建筑阴影仍属预期。
