...
- 盾卫、民兵、战士的普攻接近停步、攻击起手和命中复查统一调用`canMeleeReachTarget`：先检查`canMeleeShareSurface`，再检查真实XY线段的墙体遮挡，避免距离足够就隔墙原地挥击。
- 忽略集合只包含原有承载面豁免与被攻击结构自身的墙段/矩形；不得因目标是建筑而忽略其他墙。距离、伤害、出手时钟和范围技能筛选继续由原兵种规则负责。
