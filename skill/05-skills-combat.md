...
- 普通受击、确认命中附带状态和持续伤害共用正式无敌门禁；月影与闪避期间既不追加毒，也不扣毒/流血跳伤，但计时继续流逝。调试无敌与盾牌/弹反的原有处理顺序不得顺手改变。
- 弹反和暴击结果在基础受击后立即快照，避免附伤嵌套受击覆盖；击杀状态、击杀计数和枪械反馈在本次全部附伤完成后判断，不使用基础伤害后的中间 HP。
- 以上是共享战斗链的审查约束；迁入分支前先确认该分支的同层判定、命中上下文与确认命中钩子已经齐备，不能只移植调用点而漏掉协议。
