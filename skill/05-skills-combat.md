...
- 接近停步、攻击起手和命中复查必须共用承载面与墙体门禁：先检查`canMeleeShareSurface`，再检查真实XY线段遮挡；不能仅因距离足够就隔墙原地挥击。`canMeleeReachTarget`用于既有接近/范围分支，新校准的普通近战由`friendly-melee.js`复用`createBasicMeleeSnapshot/canImpactBasicMelee`。
- `attackRange`决定起手停点，`attackImpactRange`与`meleeWidth`决定接触时的伤害区域；两者都包含目标footprint，不重复额外叠加目标半径。起手锁定原目标与方向，接触时复查其存活、阵营、可命中性、同层、距离及墙体；不得挥击中途换目标或转动已提交方向。
- 忽略集合只包含原有承载面豁免与被攻击结构自身的墙段/矩形；不得因目标是建筑而忽略其他墙。范围技能逐目标复查，普通攻击校准不隐式改动冲锋位移、范围技能半径或控制时长。

#### 普攻接触姿态与距离标定

- 先按当前帧布局、`displaySize`、脚点与攻击态逐帧缩放还原显示姿态，再选实际伸出/接触帧。长武器的Alpha外沿只作辅助，不按透明画布宽度或主体身高直接反推攻击距离；视觉缩放、身体碰撞与武器攻击距离分别维护。
- 同一单位的开场/连续攻击若接触姿态不同，分别配置帧与距离；不要用收刀、背身或残影最远帧作为统一命中时点。侧向源图只能支持静态校准，不代表任意朝向、敌人体型的实机像素级接触已经通过。
- 高速弹检查本帧完整三维线段，并按目标/墙接触先后结算；穿透逐目标扣次数，遇首道有效墙终止。新射击不能覆盖旧在途弹，弹丸序号与显示回收独立于角色动作序号。

本节新增入口对应仓鼠工作区实现，合入前须确认共享接口及兵种依赖；Git发布状态见`docs/audits/hamster-cleanup-and-publication-2026-08-31.md`，不得假定远端main已经具备全部入口。
...
- **形变动作使用逐帧锚点表（毒蛆喷毒 2026-08-28）**：头部、口器或枪口在动画中发生明显位移时，不能继续用单个 `forward/up` 估值。以正式未翻转帧格像素记录释放窗口内每帧锚点，运行时用当前纹理帧号、Sprite 实际显示宽高、origin 与 `flipX` 转成世界坐标；一次性喷雾和后续每颗投射物必须共享这个函数。提交前用左右镜像静态叠加图检查锚点，生成视频自带的毒液束/弹体则只在动作 ROI 内移除，喷雾与弹道继续由 Phaser 负责。
...
##### 友军无弹道扇形枪击合同（2026-08-27，仓鼠防暴队）

- **独立释放链，不伪造弹道**：可继承成熟友军 AI 的接敌、前后摇、RTS 接口和动画节拍，但释放帧必须走专用 AI；清掉基础投射物状态，不生成 tracer、飞行物或命中爆炸。攻击起手只快照一次地面方向，后续动画与命中不得因目标移动而旋转扇形。
- **配置与地面几何同源**：`attackRange`、`attackArcDegrees` 是唯一口径，需求给出的最大距离必须做硬上限。方向角按 `atan2(dy / PERSPECTIVE_SCALE_Y, dx)` 计算，使用 `surfaceEffectFromEntity` + `GroundSector` 查询；接敌距离也消费同一投影度量，避免 AI 已进入射程但主目标落在狭窄扇形外。
- **逐目标即时结算**：候选目标先过滤阵营、存活/激活状态与不可攻击节点，再逐个执行 `hasRangedLineOfSight` 和 DamagePipeline；同一实体每次释放最多命中一次。无弹道不等于穿墙，也不能静默继承投射物单位的穿甲、弹速或命中升级。
- **伤害与散弹火花彻底分离**：伤害在释放帧即时完成；枪口火与散弹喷射仅由 `EffectManager.spawnCosmetic` 驱动，进入 `worldEffectsGroup`、迷雾和深度体系，短生命周期后成对销毁。VFX 不选敌、不补伤害，默认只画稀疏火花束，不用整块扇形填充冒充实际判定范围。
- **功能定位必须可读**：该模板用于“定向、低伤、多目标压制”，与单体投射物、爆炸溅射或高伤范围单位做数值和表现区分；后台简化模拟若尚无明确 AOE 计数合同，可保守维持单目标结算，不能凭空放大离屏战力。
