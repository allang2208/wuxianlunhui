...
- **既有经济纵列插入双节点的迁移合同**：在已完成的能源/农业/金币纵列中插入“建筑解锁→本栋标准化”时，旧档若已拥有原后继科技，迁移必须同时补齐这两个新增必经节点；只沿该后继的强制前置闭包补全，不赠送旁支、OR候选或未完成进度。修复完成态、当前研究/队列与节点进度后再写入新版本，配置版本和 `TechnologySystem.VERSION` 同步推进，未完成旧档继续保留原研究比例。
