...
- **覆盖圈与后台边界**：打开工坊建筑详情时绘制按 `PERSPECTIVE_SCALE_Y` 压缩的覆盖椭圆，升级范围后实时重绘，关闭/出售/摧毁/离场必须销毁。后台位面不物化工程师，只在已确认无敌的 prep/break 时间窗保存抽象目标与剩余行程，抵达后结算维修；经济增效继续按坐标范围与最强光环结算。
