...
- 后台模拟须消费前台已有的加工、待入库、送仓、返程阶段与阶段余时；不能把离场时长直接折算成生产轮数，绕过真实运输与返程成本。
- 到仓才入库，满仓保留货物；回场后才能开启下一轮。断路或无人按原规则冻结，唤醒时间取当前阶段剩余时长，等待入库采用既有重试节奏。
- 保留道路折线、当前脚点、人口效率及已付费升级完成边界；移植前确认三类农场任务字段与后台调度协议均已合入。不以静态计算声称多农场竞争仓容或长短步进已经实测一致。
