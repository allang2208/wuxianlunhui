...
- **持续消耗**：单位生产能源费配置为民兵50、斥候75、战士125、盾卫150、射手120、火枪180、
  牧师240、轻骑220、骑士300。矿工死亡补员100能源，至少保底1只免费。前后台都在真正生成时扣费；
...
- **门禁布局不变量**：隐藏锁定按钮时必须保留其原始网格/列表占位，禁止通过 `filter()`、条件模板或
  `display:none` 让其他既有控件补位；解锁后控件直接在原槽位恢复。浮动栏目不得用隐藏锚点的零尺寸
  `DOMRect` 计算坐标；新增常驻入口必须追加在既有栏目之后，不得改变原按钮序号。
- **统一门禁与配置校验（v2）**：玩家 UI 的科技可见性统一通过 `TechnologyGate` 绑定，负责占位、鼠标、
  键盘、`aria-hidden`、`inert` 与解锁事件刷新；业务底层继续调用 `TechnologySystem.isUnlocked`。
  科技配置载入时必须检查重复节点、无效研究量、缺失/自指前置、循环依赖、不可达节点、非法解锁类型与
  重复解锁归属，禁止把这些校验散落在各消费面板。
...
- **界面契约**：科技树挂载右侧栏目统一层但覆盖全屏，沿用右滑 0.25 秒动效；正常交互只允许右上角关闭按钮
  和 ESC 关闭，不提供遮罩点击、再次点击入口或其他隐式收回路径。开发工具“技能”页的一键解锁用于测试。
