...
- **插入中间时代的兼容合同**：时代显示等级与持久化ID分离。插入近代III级时使用独立 `*_industrial` ID，原现代 `*_level_3` ID可继续保留而显示IV级；不得批量改名导致旧档拥有权丢失。布局列号只控制展示，真实前置仍读数据。军备共享时代门槛只付一次，各建筑路线不互相串成必修；民用过渡不得无意绑上整条军事/矿业支线。
- **视觉与完整编制分离**：近代建筑先完成并不等于新兵种已开发。行级占位不登记可生产单位拥有权，实际募兵选择最近已解锁且整档实现的编制；视觉等级可继续提升，已有现代终点不得被中间占位永久堵住。现存部队、生产来源和人口账本不因建筑换形重写。
- **时代调价与旧档迁移**：只调整新增层与必要的后段节点，保留全局科研曲线。对未完成且调价的节点按旧成本换算研发百分比；已完成后段只补必要前置，不赠送兄弟路线、不重放完成奖励、不补收费用，再从前置真源重建目标队列。迁移实现须与现有版本链一起发布，不可把依赖未合入基础协议的配置单独推到主线。

### 农场送仓精灵的体量与步频

- 先区分岗位人数、实际物流链数量与显示精灵数，不能把10个岗位画成10条送仓链。按已有业务阶段显示：加工/回场隐藏，结算后抱货，满仓或部分入库继续抱货，全部入库后才空手返回；隐藏的待机不因画风差异额外生成整段动画。
- 统一有效身体高度和脚点，各动作分别声明固定scale/footRatio；显示画布大小不是身体高度。送仓与空返沿用各自源动作的自然腾空，不逐帧贴地。
- 每个Sprite按实际运输速度除以基准速度设置独立播放倍率，包含运输升级和人口效率；不要修改全局共享动画FPS。停路/待仓按原阶段定格，恢复后接着播放。统计须分开记录基础速度、满级速度、人口效率及田间工人与送仓工的职责。

### 市政厅玩家基地与科技演进

- 市政厅是独立玩家建筑，不接管传送门的旅行、入侵目标或毁灭判定。只在建设位面共用入口中、快照恢复和传送门建立后尝试生成；占地、道路、矿点、障碍与落脚区域共同参与安全选址。
- 首次建立标记按位面与epoch保存：空地不足不记成功，旧档补建一次，已建立后被毁不自动赠送；位面重建才按新epoch初始化。出售、打包、回收和手工重复建造需同时保护。
- 默认LV1，后续形态消费既有科技解锁；新建、在场、读档和后台快照使用一致的最高解锁等级。视觉升级不能重置位置、ID、占格或已损失耐久，不重复收取手动升级费用。
- 政策规划随所属基地保存独立纯数据，不放进全局Buff或共享模板。模拟预算与真实货币分离；没有正式收益协议时，不在UI中直接修改产出。

### 首城授予与首轮建设教程（2026-09-05）

- 首次F级教学地牢成功后只进入`awaiting_king`；必须与小鼠大王交谈后进入`selecting`，再从配置显式标记`firstFoundingCandidate:true`且具备合法预留城址的正式位面中手动确认。当前正式首城只允许`scene8`；测试直连、模板预览和仅满足普通地牢前置的位面不能混入候选、正式目录、传送目的地或存档。
- 首城教程只在玩家首次实际进入获授位面后激活，固定业务链为“免费仓库→实际入库300能源→免费房屋与首名居民→麦田风车及农夫→研究所→研究员与手动研究目标→首批科研进度”。任务只监听建筑、岗位、`EnergyManager.depositEnergy()`和科研系统的真实事件；余额变化、地面掉落、自动分配岗位或伪造研究进度都不能推进。
- 仓库冷启动物资与采矿达标后的建设补给属于一次性教学补助，不计入周期生产；没有空闲居民时，教程应明确引导玩家从风车撤下农夫并调任研究员。科研进度首次真实增长即结束教程，由小鼠大王通讯收束并交还自由扩建，不强迫玩家等完整科技研究结束。

### 经济与出兵建筑持续升级的共享占用

- 同一建筑类别的本栋经济目标与全局能力目标共用一个持续名额，覆盖在场建筑和有效后台快照；矿工营地共用同类名额。UI禁用与业务入口都复用共享占用判定。
- 自动续升每档调用原升级入口，由原入口校验科技/费用并支付；资源不足保留意图和等待原因，不预扣未来费用。关闭面板不停止续升。
- 停止只取消后续意图，已付款读条继续；满级、项目失效、出售/毁损清理意图。卡片保留升级/持续升级位置，活动目标始终可停止。
- 读档去重在等级、世界快照和世代恢复之后进行；排除前台自己的快照，忽略过期/被毁/满级目标。重复意图可清除，不取消当前工程、不退费或重复扣费。
- 本地升级效果仍逐栋独立；既有手动队列不因持续互斥被合并。后台仅完成已开始的一档，回场恢复续升，不能将保留目标描述为后台自动连续支付。

### 后台建筑升级的完成同步与时间边界

- 共享模块提交全局等级后，必须按适用`unitTypes`逐兵种调用现场同步入口；组合名称只用于报告，不能当作单个兵种键。定时后台与入场补结算共用完成入口，驱动器不得重复消费同一升级副作用。
- 研究等级实际增加后触发现有`ResearchSystem.onResearchLeveled()`，立即更新当前场景墙门耐久与募兵计时；满级未增加、只读预览和重复回场不触发。现场刷新与快照自身的耐久/计时更新分别负责各自对象。
- 本栋等级、仓储与经济模块、矿工模块在完成时点切分后台时间：先按旧人口、旧岗位效果和旧仓容结算到该时点，再提交升级，后段使用新状态。只把账本唤醒时间设为升级终点，仍会让“先升级后结算”的实现回溯收益，不能代替分段。
- 分段共用原生产周期余数与快照字段；预览只推进副本和临时研究等级，不修改全局等级或现场实体。新引入分段时保留被动能源的小数余量，研究恰好在终点完成也要把募兵剩余时间与结构耐久转换到新等级，供下一段继续。
- 已归零工程在时间起点完成，未到期保留剩余时间，被毁或已替换队列不再提交。持续目标在全部分段结束后收口，后台仍只完成已付款的一档，不新增后续支付或改变升级效果配置。
...
- **经济建筑科技归属（v46，2026-09-02）**：需要科技才能建造的经济建筑，其建筑解锁科技必须同时直接登记该栋 `upgradeProject` 的全部模块/能力解锁；禁止再插入仅用于开放本栋升级的“标准化、校准、工艺”后置科技。研究建筑科技后，建造权限与本栋升级项目同时开放。研究所、仓库、风车、银行、矿工营地等基础可建建筑没有 `building:<id>` 科技所有者，可继续使用独立的行业、等级或后期功能科技。`TechnologySystem` 配置校验必须反查建筑所有者与升级所有者一致；升级卡按钮和各 `startUpgrade()` 业务入口仍须双重校验，不能因配置归属正确而省略扣费入口门禁。
...
- **天气塔升级闭环（2026-08-24，2026-09-02科技合并）**：`weather_forecasting`（天气预测）同时解锁天气塔建造与本栋四项升级，不再设置仅用于升级的后置科技。天气塔通过 `economyType:"weather_forecast"` 接入标准人口经济面板，恒定只有1个
...
- **科技门禁**：医疗线固定接在军事指挥的 `shield_formation` 之后；`field_medicine`（战地医疗）同时解锁战地医院建造与 `hospital_rounds / hospital_medicine / hospital_triage / hospital_staff` 四项本栋升级，不再插入独立医疗标准化节点。升级按钮和 `FieldHospitalSystem.startUpgrade()` 必须同时校验科技，禁止只做 UI 灰态。
...
- **专属图标**：四项医院升级使用 `assets/ui/building-upgrades/hospital-*.png` 的256方形冷钢四铆钉框；医疗科技使用 `assets/ui/technology-icons/field_medicine.png` 的1024六边形冷钢徽章。图片路径是主显示源，配置中的 emoji 只保留为加载失败回退。
...
5. **遮挡/占地接入**：构造中走 `applyBuildingFootprint(this, 2)` +
   ...
   - **导入/换图/调图必须同步检查图层（2026-08-23，强制）**：任何建筑首次导入，或修改
    ...
    - **复合生产建筑的内部视觉对象（2026-08-25，奶酪农场）**：逻辑占格、内部活动区与渲染层必须
     ...
     带镜像正门的生产建筑必须在构造器初始化岗位/物流任务之前接收最终 `mirror`，门点横坐标与贴图
     `flipX` 使用同一真源；读档也必须把快照朝向传入构造器，不能在任务生成后补写朝向。道路可达性
     缓存至少包含建筑道路签名、有效接入口、建筑镜像，以及仓库坐标、镜像和有效接入口；仅缓存道路
     拓扑版本与仓库 ID 会在搬迁、镜像或仓库入口改变后复用过期的断路/旧路线结果。
...
- **自动入池发布边界与领队职责**：只有当前分支已经合入数据驱动目录、统一构造器、编队、资源预算及前后台/存档消费链时，`enemy-config.invasion` 才能称为运行时接入；否则它只是前向元数据，禁止连同其他会话未入库的战略模块一起强推。协议合入后，显式 `invasion.role:"leader"` 只允许已完成攻城适配的 `elite/lord/boss`，只改变入侵编队职责、不改地牢/图鉴 `rank`；若精英被用作唯一领队且系列没有另一只精英，编队规则必须明确允许普通成员生成强化精锐护卫，不能跨系列借兵或让普通怪越级领队。
...
- **资源转换闭环与视觉员工合同**：金币→能源、能源→食物、食物→金币等转换建筑必须由前台、后台和面板共用同一实际周期/单轮投入产出函数，并核算基础及全部产量/减耗满级后的闭环回收率始终小于1。工坊、酒馆等速率增益只能同比缩短周期并提高投入频率，不能只放大产出而改变单轮回收比。建筑外的员工状态机只表现 idle/walking/职业动作，独立限制可见人数；不得推进业务周期、搬运资源或重复记账，存档只保存恢复视觉所需的稳定身份与状态。
- **配置与状态分离**：科技节点、前置、研究量、布局和解锁目标统一维护在 `data/technology-tree.json`；
  ...
  小麦风车同样作为基础经济建筑直接开放，“农业分工”只登记其四项本栋升级的拥有权；
    ...
    `蒸汽动力 -> 经济工程 -> 位面物流协议`形成独立蒸汽支线；经济工坊与本栋升级均由`经济工程`直接解锁，升级按钮禁用时保留卡片信息且业务层必须二次校验。
  工程线按 `工程制图 -> 城防工事 -> 防御塔工程` 延伸；位面打通后，传送门首次构造直接由位面生命周期开放，
  不登记科技拥有权，也不得在传送门面板额外检查研发状态。可构造列表和最终构造入口仍必须校验对应地牢解锁条件。
  - **经济与位面横向分支（v46，2026-09-02）**：经济页按住房、农业、金币、能源与仓储分行，住房位于顶部并作为跨分支时代门槛。需要科研建造的经济建筑在本建筑科技上直接携带其全部升级项目，不再通过“标准化/校准/工艺”节点二次解锁；因此六级住房不再指向已删除的升级专用节点，七级住房只保留真实后续建筑/机制门槛。当前没有七级能源节点，不得为填满版面伪造空科技。
  能源分支保留新能源线“深钻工程 → 风力发电 → 光伏发电 → 位面谐振”和蒸汽线“深钻工程 → 蒸汽动力 → 经济工程 → 位面物流协议”。布局坐标只决定显示位置，真实可达性始终由`prerequisites`决定；布局调整必须同步科技 JSON 版本与`TechnologySystem.VERSION`，新增强制祖先要为已完成后继的旧档补齐，删除的节点只过滤旧状态而不转赠无关科技。
  - **军事建筑纵向骨架与黑火药时代门槛（v46，2026-09-02）**：`thatch_hut_level_1`仍是初始完成的纵向骨架起点；“仓鼠草屋·I级 → 军营建制 → 阵地射击 → 骑兵学 → 战地神学”只负责依次开放一级军事建筑，军营/靶场/骑兵学院/教堂不再额外显示无效果的I级占位科技。各建筑横向顺序统一表达为“一级建筑科技 → 本线二级编制 → 黑火药时代门槛 → 近代三级 → 现代四级（已有时）”。靶场二级“长弓整编”必须位于黑火药之前，保留长弓与弩手的前近代编制；`gunpowder`随后单独解锁仓鼠火枪，并作为全部已发布近代军事科技的共同起点。多前置近代节点继续显式列出本线二级、黑火药及必要的蒸汽动力；现代四级只依赖对应近代三级，禁止重复罗列已经传递满足的祖先。v46旧档若已完成黑火药而未记录长弓二级，恢复时补齐`shooting_range_level_2`以避免完成节点倒挂。
  ...
  - **科研升级与整级换兵边界**：`producer-buildings.json#recruitmentTiers`是等级、建筑名称、槽位、角色画像和替换目标唯一真源。可研究的等级科技通过`recruitmentTier:<tierId>`全局门禁；科研完成后，当前建筑、读档恢复与后续新建实例立即使用该级名称和建筑贴图。兵种换代独立遵守完整开发门槛：同一级所有兵种必须登记运行时`unitTypes`并移除行级`placeholder`后，才按同一槽位替换后续可招募兵种并重置该次生产周期；已生成单位、混编快照和外派部队保持原兵种，不做实体变形。尚未完整开发的等级允许先完成科研与建筑外观升级，但继续沿用最近一档完整可玩兵种，避免引用缺失实体。`supplementalUnitUnlocks`是明确的独立单位科技例外：字符串条目沿用“到来源等级自动退出”，对象条目可用`{key, untilTierLevel}`显式指定退出等级。黑火药在长弓二级之后单独加入仓鼠火枪，并以`untilTierLevel:3`保证靶场三级整级启用时退出。不能用该字段泄漏长弓、钢盾、动力甲或其他占位兵种；同级两条路线全部完成后必须移除对应补充入口和独立过渡科技，把两条单位解锁合并回完整等级节点。
  - **同级单条路线完整时的独立科技合同（战壕突击兵历史过渡，2026-09-02）**：等级节点若带`recruitmentTierId`且同级仍有占位，校验器禁止在该等级节点提前登记任何`unit`解锁。完整单条路线可暂时使用不带`recruitmentTierId`的独立科技，只解锁自己的`unit:<key>`，并将该键加入生产建筑`supplementalUnitUnlocks`；等级科技仍只负责建筑外观和时代前置，未完成的兄弟行继续保留`placeholder:true`。独立科技应依赖所属等级的建筑/时代节点，但不得成为更高完整等级的额外强制前置；旧档已完成更高稳定等级时只补齐新插入的时代祖先，不免费补独立单位科技。前台招募、生产恢复和后台模拟均使用同一单位键。兄弟路线完成时，必须在同一个版本迁移中删除独立科技和补充入口、移除全部行级占位，并由完整等级节点显式拥有两条单位解锁；v47军营III即按此规则把战壕突击与钢盾突击合并回整级编制。
...
#### 实际居民、幸福度与口粮合同（2026-08-31）

本节记录本地已实施的业务合同；代码接入与Git发布必须分别确认，发布边界见 [人口经济交付记录](../docs/population-economy-publication-2026-08-31.md)。

- **人口与住房分离**：`total`是实际居民，`capacity`是住房供给，`free=max(0,total-used)`；同一位面只有一条人口时钟。每栋新房成功建成只赠1人，失败回滚、恢复快照和升级都不赠人；升级只扩容。满房/超房继续增长，住房变化保留进度，拆房不直接减员；无居民且无住房时才不增长。
- **周期百分比先加减**：唯一配置为`population-economy.json#populationGrowth`。周期=`max(minIntervalMs,baseIntervalMs*(1-foodModifier-housingModifier-happinessModifier-tributeModifier))`，不能改为多个速率倍率相乘。默认20秒、下限2秒；供粮+20%、满房−80%，幸福度50时合计−60%，即32秒/人；满房且初期缺粮合计−100%仍为40秒/人，不是饥荒。
- **独立口粮边界**：每人每20秒3食物（0.15/秒），只扣本位面存活仓库实存，库存不足吃掉余粮并记不足，不走跨位面或开发工具免费支付。前三次不足为−20%，第4—7次依次−40/−60/−80/−100%；只有食物自身到−100%才每20秒流失1人，进度反向归零。补粮后等下次足额结算恢复；住房/幸福不覆盖饥荒。基础4农夫产6粮/秒可供40人口，但还要预留工业和招募开销，不能当作实机调平结论。
- **六项幸福度**：每个位面/世代独立0—100，初始50，20秒与口粮同边界更新，目标为50加食物保障、住房供给、住房品质、娱乐服务、商业便利、安全感；每期向目标最多+3/−6。增长修正为`(幸福度−50)*0.008`并限±0.4，进入同一百分比池；无人时冻结。参数只读`populationHappiness`，不在UI重算。
- **按真实服务计幸福**：酒馆只累计宴饮服务窗口，按岗位/人口效率折算娱乐名额；商场只累计已付能源的营业批次，以范围内实际居民加权，重叠房屋取最高服务量。空房、运输中、待粮、未营业不虚增幸福，不能把结算末尾状态当整期服务。安全只读当前位面/世代战事与核心生命，不以屏幕附近怪物数量判断。详细计分与玩家说明见 [幸福度说明](../docs/population-happiness-2026-08-31.md)。
- **共享时间与旧档**：前后台共用`population-growth.js/population-happiness.js`，按出生/流失、口粮和房屋升级边界切段，先结算旧人口的该段生产，再提交人口变化与口粮；不得每房重复推进或被战斗停帧缩放。旧档按`max(原住房容量,原岗位占用)`迁移，恢复不重发赠人；保存人口/方向/进度/缺粮/幸福余数，后台账本唤醒包含口粮。住户按房屋容量用整数最大余数分配，稳定ID破同分，空房不贡献收益或品质。

#### 每秒资源净收支扩展合同（2026-08-31）

- `EconomyFlowSystem`只统计、不持有余额或支付：周期来源复用业务快照，其他成功入库/扣款按`resourceAccounting.observationWindowMs`（默认60游戏秒）均摊，统一`net=income-expense`。不是库存导数或全位面总收入；暂停不按现实时间稀释，切场/读档重建观察窗口。
- 新模块通常走`EnergyManager/GoldManager`资源入口自动统计实际成功量；有固定周期时注册`registerRateProvider(id,()=>[{resource,income,expense,intervalMs,label}])`，实际周期入账传`{accounting:{providerId:id}}`避免双算。建造/升级/招募/交易不加周期标识；恢复、后台入账和内部搬运用`ignore:true`排除。直接改余额的模块必须在成功提交后显式`record`，预检和失败不能记账。
- 居民口粮始终列足额需求，缺粮不等于不再消耗；其他经营按真实开工条件计入，满仓未入库产能不算收入。金币统一路由背包→主神空间仓库并只统计一次，溢出掉落未拾取不当入库。不能在HUD硬写农夫、铸币或能源数值，后续费用变化必须兼顾该入口。完整接口示例与混合统计边界见 [净收支说明](../docs/economy-net-flows-2026-08-31.md)。
...
- **人口语义**：住房容量由房屋等级求和，实际居民由位面人口时钟维护，经济岗位只可占用实际居民；研究院、天气预测塔、风车、银行、皇家铸币局、市场、工坊、位面谐振塔和矿工营地在建筑实例上保存 `assignedWorkers` 数值，通过 `setAssignedWorkers/adjustAssignedWorkers` 占用全位面人口。普通岗位不创建实体；矿工营地是明确例外：每个新增岗位在岗位变更回调内立即物化一名不可选择的经济矿工实体，用于寻路、受击、背包携带与返营提交，但不接受 RTS 指令、不参与主动战斗；只有出口受阻才允许短重试，不能套用死亡补员读条。“矿工增援”只增加岗位容量，不能绕过人口直接生成矿工。
...
- **生产与仓储语义**：风车按农夫数生产粮食，当前每名农夫 1.5 粮食/秒、满岗位风车 6 粮食/秒。粮食逐仓写入 `storedFood`，与 `storedEnergy` 共享 `storageCapacity`；满仓时未入库产量留在本栋整数余量中等待空间，不得写入脱离仓库的全局库存。所有军事出兵从同一位面仓库扣粮，恢复快照中已有单位不收费；银行逐栋统计服务半径内存活房屋的实际入住人口，并按该房屋同时被多少座活动银行覆盖折算：1 家 100%、2 家 67%、3 家及以上 0%。单轮金币 = 折算有效服务人口 × 每人金币比例 × 上岗职员数 × 全局人口效率 × 最强工坊增效，并按本栋金融工具决定的周期离散结算；重叠只惩罚实际重叠区域内的房屋，非重叠房屋保持原收益，前台与后台必须读取 `population-economy.json#bank` 的同一参数。所有产金建筑的金币固定路由为玩家背包 → 主人空间 `WarehouseSystem` → 产出建筑坐标的金币掉落。金币在背包与主人空间仓库中分别执行单格无限堆叠：同一容器始终只保留一个金币格，拾取、产金、手动存取和旧档恢复都必须合并到该格；禁止重新引入999/99999分堆上限或允许拆分金币。背包和主人空间仓库必须提供“实际存入数量”接口，禁止用布尔返回猜测溢出量；主人空间仓库物品必须随主存档序列化，否则转存金币会在读档后丢失。人口流失导致岗位超额时先撤非粮食岗位再撤粮食岗位，沿原岗位回调交接；矿工携带物不删除。人口效率兜底只用 `total / used`，不得继续拿住房容量当劳动力。
...
- **五级科研建筑（v30）**：稳定序列固定为“研究所 → 大学 → 高能实验室 → 位面观测阵列 → 跨位面中枢”，配置中的 `researchTier` 依次为1—5。大学为兼容旧档继续使用稳定配置键 `high_energy_laboratory`；新高能实验室使用独立键 `high_energy_research_laboratory`，禁止把大学旧实例迁移成实验室。二至五级统一使用 `economyType:"advanced_research"` 和每栋 `researchFacility` 配置，不各写结算器；满员基础科研/岗位/造价依次为大学`2.0点/6人/15000能源`、高能实验室`2.5点/8人/19000能源`、位面观测阵列`3.0点/6人/22500能源`、跨位面中枢`4.0点/10人/36000能源`。前三座上位设施每个位面最多1座；高能实验室要求控制2个位面，观测阵列要求3个位面，跨位面中枢要求5个位面且全存档全局唯一。前台和后台必须按同一岗位、人口、工坊、酒馆、集群和祭品乘区计算，再进入全局科研软上限；逻辑footprint与视觉`visualFootprint`继续分离。高能实验室正式贴图使用独立`high_energy_laboratory.png`运行时路径，逐栋校准视觉脚印，不改4×4逻辑占格。
- **高能实验室四项本栋升级闭环（2026-09-01，2026-09-04 适配）**：`high_energy_laboratory_economy`固定提供磁约束增幅、自动化实验台、探测器网络、数据标定链，分别驱动本栋基础科研、单人岗位效率、集群识别半径和单类集群增效；Lv.0为`2.50点/秒、12.50%/人、640px、3%/类`，满级为`3.50点/秒、17.50%/人、1120px、5%/类`，8个岗位不因自动化减少，岗位发挥率与集群总增效仍分别封顶100%和12%。配置只声明项目和效果；`PopulationEconomySystem`负责前台等级、费用、读条与结算，`world122-sim.js`读取同一项目值，`world122-snapshot.js`成对保存/恢复`advancedResearchModules / advancedResearchUpgrade`，后台账本必须登记升级字段。四项升级由`high_energy_laboratory_science`与建筑一并开放，面板按钮和扣费入口都须权威校验；图标使用正式256px与运行时128px镜像，Emoji仅作回退。
...
- **算力重心数值闭环（2026-08-26，2026-09-02科技合并）**：`producer-buildings.json#computing_center`只声明4×4建造、贴图、`economyType`和`upgradeProject`；岗位效率与资本反馈率读取`population-economy.json#computing_center`，基础算力、人口模型、液冷能耗和岗位容量读取`building-upgrades.json#computing_center_economy`，禁止在面板或结算分支复制数值。前台与后台统一按`（基础算力 + 位面人口 × 人口模型 + 玩家总金币 × 资本反馈率）× 岗位效率 × 全局人口效率`结算金币，并按同一运行倍率扣当前位面仓库能源；基础6岗、每岗10%，扩编满级10岗，液冷满级把满负荷能耗从140降至100能源/秒。`distributed_computing`同时门禁建造与四项本栋升级，升级按钮灰态和扣费入口都必须权威校验。快照必须成对捕获并在`_restoreProducer()`物化时回灌`computingCenterGoldRemainder / computingCenterEnergyRemainder / computingCenterModules / computingCenterUpgrade`，后台只可推进同一模块等级和读条。四项升级正式图标使用`assets/ui/building-upgrades/computing-*.png`，科技图标使用`assets/ui/technology-icons/distributed_computing.png`；图片是主显示源，Emoji只作加载失败回退。
...
- v46共84个科技节点；删除14个只负责经济建筑升级的后置节点与4个无效果军事I级占位节点后，折算总成本为138680点；工程48790、军事指挥34890、经济与位面46360、位面独特8640。这个快照同时采用“长弓整编→黑火药→已发布近代III级→现代IV级”的军事顺序；后续增删节点必须重算，不得把本组数字当成永久常量。
- v47把草屋、军营、靶场和骑兵学院的近代III级各收口为两条完整非炮兵兵线，等级节点同时显式拥有`recruitmentTier`与两个`unit`解锁；战壕突击临时独立科技及军营补充入口随钢盾路线完成而删除。旧档若已完成对应现代IV级，只补齐其新插入的近代III级祖先，不能反向补未完成的独立兵种或改写已经生成的旧单位。
...
- **位面谐振塔发电口径（2026-08-23，2026-09-02科技合并）**：`producer-buildings.json#planar_resonator` 只登记结构、6000 能源造价与升级项目，岗位比例读取 `population-economy.json#planar_resonator`，四项本栋等级读取 `building-upgrades.json#planar_resonator_economy`。单轮可入库能源 = 晶核原始产能 × 导能回收率 × `min(1, 上岗技师 × 20%)` × 全局人口效率 × 最强工坊增效 × 生产祭品倍率；基础 10 秒/轮、100 原始能源、80% 回收、2 岗，岗位升级至 5 后满效。未入库整数继续保存在本栋 `_workProductionRemainder`，禁止满仓时丢失或写入脱离仓库的全局能源。“位面谐振”同时解锁建筑与四项本栋升级；UI 门禁与 `startResonatorUpgrade()` 业务校验必须并存。`resonatorModules`、`resonatorUpgrade`、岗位、周期和余量必须进入快照，`world122-sim` 用同一离散周期公式推进后台发电与升级。
- **风力电站发电口径（2026-08-27调整，2026-09-02科技合并）**：`producer-buildings.json#wind_power_plant` 登记标准2×2逻辑占格、静态主体/面板/独立叶轮和科技门禁；逻辑 footprint、碰撞与寻路按2×2处理，静态主体换图不得改动已经确认的24帧叶轮图集，只能根据新主体轮毂重新标定 overlay 偏移。岗位参数读取 `population-economy.json#wind_power_plant`，四项本栋升级读取 `building-upgrades.json#wind_power_plant_economy`。发电按固定周期把“基础产能 × 岗位效率 × 全局人口效率 × 最强工坊倍率 × 酒馆倍率 × 全局生产倍率”写入本位面仓库；满仓余量保留在本栋，禁止写入顶层全局能源或跨位面借仓。“风力发电”同时解锁建筑与四项本栋升级，UI门禁和业务入口双重校验；岗位、模块、升级读条、生产周期与余量进入建筑快照，当前位面和离场位面复用同一公式，后台不创建叶轮 Sprite。
- **光伏电站发电口径（2026-08-26，2026-09-02科技合并）**：`producer-buildings.json#solar_power_plant` 登记标准4×4结构、每位面2座上限、`upgradeProject`与`perimeterTile:"road"`；放置预览、黑雾校验、占地预约、正式落地、打包重建和拆除必须共用四周外围道路口径。岗位容量与每人20%效率读取`population-economy.json#solar_power_plant`，追日周期、阵列原始产能、储能转换率和岗位扩编统一读取`building-upgrades.json#solar_power_plant_economy`。基础3岗时为`600 × 90% ÷ 4秒 × 60% = 81能源/秒`，四项满级并扩至5岗时为`800 × 100% ÷ 3秒 = 266.67能源/秒`，再统一乘人口效率、最强工坊、酒馆与全局生产倍率。`solar_power`同时解锁建筑与四项本栋升级；前台/后台必须复用同一离散周期、满仓余量、岗位、模块和升级读条快照。四张升级图标固定使用256方形冷钢四铆钉徽章，科技使用1024六边形冷钢徽章，Emoji仅作加载失败回退。
...
- **军械库减耗与整理口径（2026-08-23，2026-09-02科技合并）**：军械库由 `population-economy.json#armory`、`building-upgrades.json#armory_economy` 与 `ArmoryEconomySystem` 驱动，属于仓库前置的标准2×2经济建筑，不产兵。基础服务半径600px、满员减耗10%、维护师岗位2个；实际效果 = 配置值 × `上岗维护师数量 × 20%`，5名满效，多栋军械库覆盖同一出兵建筑时只取最强减耗。“军械保障”同时解锁军械库与装备护理/服务范围/资源整理/增加人员四项本栋等级；资源整理未升级时即有1%满员分钟概率，每次升级+1%，按模块Lv.0起算经过9次升级后满级10%，强化石自动堆叠进主神空间仓库，满仓时数量保存在本栋快照。前台报价、实际扣粮、后台位面招募必须读取同一减耗倍率并按整数向上取整，禁止只改面板价格或只改当前位面。
...
- **面包屋同级位面食物加工建筑（2026-09-03，沙地炊坊/雪原熏制坊）**：同类建筑复用`BakeryEconomySystem`的取粮→返店→加工→送仓状态机，但必须按建筑独立保存`modules/upgrade/job/outputRemainder`，前后台结算使用同一批次公式。多岗位加工建筑只把`assignedWorkers × workerOutputEfficiencyShare`乘到最终成品，不能重复乘加工速度、原料或运输；面包屋/连锁餐馆未声明该字段时保持原倍率。天气与位面特性同样只乘最终成品：沙地炊坊在干旱时可设置天气倍率下限，雪原熏制坊读取`planeOutputMultipliers`，雪原初级风车单独读取自身20%位面倍率，奶酪农场不受该限制。详情面板同时展示岗位、批次、天气、位面、待返仓量和升级读条，低岗位导致成品少于投入时明确警告。正式资产需成套登记主体/缩略图/科技徽章/四项升级图标、环境光照、strict地面拟合与Body Depth语义阴影；定稿后删除未选候选及可再生中间图，只保留可编辑模型、当前Depth、获准12/48步raw与记录、正式Alpha/复核图、阴影代理和运行时元数据。
- **面包屋升级与祭品边界**：基础处理 10 秒、产出 5 倍、植物祭品 1%、移速 80px/s；快速烹饪/美食家/食材处理/小步快跑每级分别 `-0.5s / +0.5倍 / +0.2pp / +5%`，均为 10 级本栋升级。“面包烘焙”同时解锁建筑与四项升级，UI 禁用与 `BakeryEconomySystem.startUpgrade()` 业务校验必须同时存在。祭品只从既有植物祭品 ID 池抽取，直接进入主神空间 `WarehouseSystem`；满仓时在建筑/后台快照保留待入库 ID，不能掉落到错误位面或静默丢弃。后台只做同参数的距离+处理周期聚合、粮食仓储和概率结算，不创建面包师 Sprite。
...
- **皇家铸币局上位产金（2026-08-24，2026-09-02科技合并）**：`royal_mint` 是“信用金融 → 主权铸币”之后解锁的标准2×2经济建筑，每个位面由 `buildLimit:1` 权威限制一座；当前正式美术未定稿时只允许复用银行贴图作功能占位，候选不得提前进入运行资源。它不读取房屋覆盖、不参与银行重叠衰减，而是按“上岗铸币工 × 单人单批金币 × 人口效率 × 最强工坊增效”离散产金，并从本位面仓库同时扣除“上岗铸币工 × 单人单批能源 × 人口效率”的能源与“上岗铸币工 × 60”的固定食物；任一资源不足时批次停在就绪态，禁止跨位面借用资源或继续累计无限批次。基础2岗、4金币/人/批、30能源/人/批、60食物/人/批、10秒/批，满基础岗位为0.8金币/秒、6能源/秒与12食物/秒；精密币模、自动冲压、能源熔铸、财政编制满级后达到4岗、8金币/人/批、20能源/人/批、60食物/人/批、约6.67秒/批，即4.8金币/秒、12能源/秒与约36食物/秒（外部增效前）。人口超额只降低金币与能源口径，固定岗位食物成本不变；“主权铸币”同时门禁建筑与四项本栋升级并保存为 `mintModules / mintUpgrade`，小数金币保存在 `mintGoldRemainder`；前台、后台和详情面板必须使用同一批次公式。
