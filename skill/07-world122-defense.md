...
- **主目标与次目标分区**：普攻锁定的主目标只结算一次完整伤害；扇区循环必须排除或识别主目标，只有其他命中目标按 `baseAoeDamageMultiplier` 结算 AOE，禁止把主伤害与 AOE 再叠加。仓鼠特战的扇区不设目标数量上限；没有配置固有 AOE 的旧单位继续保留原攻击口径。
...
- **前后台同源**：`world122-sim._unitDps()` 与 `producer-building-system.getMilitaryUnitProfile()` 都必须读取同一兵种基础 AOE、当前全局能力等级和能力配置的 `expectedExtraTargets`。`expectedExtraTargets` 是后台抱团期望值，不是前台目标上限；不能只改实体 AI，导致休眠位面或兵线驻军漏算战力。
- **侦察视野继承**：斥候、游侠、狙击手可共享显式最远视野，但攻击距离仍按近、近、远分级。复用 `HamsterScout` 的弩手、长弓等子类不得因基类合并配置而意外继承斥候视野；基类只在直接斥候或子类显式提供 `fogVisionProfile / fogSightRadius` 时赋值。
