...
- **墙顶混合导航**：`wallWalk.navigationMode="graph"`只负责RTS点击、远端BFS路线和楼梯拓扑；
  `wallWalk.movementMode="surface"`负责玩家与单位的即时移动。墙顶真实多边形与墙间连接面的联合区域
  是移动权威，禁止每帧把WASD投影到中心线。
...
- **稀有卡死看门狗**：surface模式下，输入非零且连续`stuckFrameThreshold=2`帧相对上一安全点
  位移<0.05px时才检查；当前墙必须存在输入正向出口，否则视为正常外墙阻挡。恢复搜索限制在
  `emergencyRecoveryRadius=3`px，依次尝试出口目标、出口轴与输入方向±15/30/45/60/90°，
  只采用仍有surface承托且不明显逆输入的最高分点。恢复后清零计数，禁止连续推行或传送。
...
- **衔接验证必须走真实移动链**：逐点传送只能验证surface覆盖，不能发现`WallSystem.resolve`
  ...
  `ignoreForEntity → WallSystem.resolve → _updatePlatformStates`直到墙顶中心，并断言无ground帧、
...
- **高速位移必须做surface扫掠**：玩家翻滚/冲刺先于`DefenseSystem.update`移动，16ms翻滚约
  ...
  `_surfaceSafeX/Y`；若上一帧是wall_walk或上段stairs而本帧落点无surface，沿安全点→落点
  反向每3px采样（最多96段），夹到最远有效楼梯/连接面/墙顶点并保持高度。
  楼梯底部不启用该保护，允许正常下地；stairs→wall_walk切换必须直接对齐目标Z，禁止平滑下沉。
- **同墙对面楼梯不是禁用设计**：第一座楼梯放置后，同一墙块正对面仍应合法。
  `_snapFiringPlatformGrid`收集附近候选后优先返回最近的绿色合法候选；只有全部冲突时才返回
  最近红色候选。避免最近墙块/方向被占用时遮蔽稍远但合法的同墙对面位置。
