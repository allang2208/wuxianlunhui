...
5. **遮挡/占地接入**：构造中走 `applyBuildingFootprint(this, 2)` +
   ...
   - **固定地表层契约**：道路（含建筑中央纯视觉补片）、服务范围圈、结构阴影、压平投影和贴地装置
     不随建筑主体做世界 Y 排序，统一读取 `world-render-layers.js`，按
     `-995 / -994.8 / -994.4 / -994.2 / -994` 严格递增且互不相等；建筑主体继续使用结构拓扑 depth，禁止再写
...
- `producer-buildings.json.blacksmith.spawnEnabled=false`：铁匠铺不产兵，改为毒箭/自动防御/
  横扫/标记/穿甲弹五项能力读条升级；等级真源 `ability-store.js`，对对应兵种全局生效。
...
- **位面初始特色建筑迁移（2026-08-23）**：`world-system.json#worlds.*.featureBuilding` 是首次接通时免费生成特色建筑的唯一映射；雪原 `scene9` 必须生成 `snow_castle`（显示名“学院城堡”）。新增映射若需补齐已接通旧档，必须配置稳定 `migrationId`，由 `ensureWorldBaseSnapshot` 在同一 `worldEpoch=1` 快照中至多补一次并写入 `featureBuildingMigrations`；完整快照捕获必须继承该标记，防止玩家后续拆除后每次读档又被重新生成。
...
- **入口**：scene8~scene11 正常直接操控与建筑模式统一使用鼠标滚轮：下滚进入压平、上滚恢复；
  只在游戏画布上响应，面板内容区、RTS、观察者、墙体/碰撞编辑模式不抢滚轮。建筑模式、RTS 指挥
  与观察者上下文均可用 `Space` 切换；普通直接操控时 Space 继续只属于玩家闪避。
  离开支持场景自动关闭。
...
- **建造门禁**：玩家建筑只能落在当前 `VISIBLE` 网格，已经探索但当前重新变暗的 `EXPLORED` 区域同样拒绝。
  预览与实际落地必须共用 `FogOfWarSystem.isPolygonFullyVisible` 严格枚举多边形实际覆盖的雾格；检查完整
  菱形占地、4格门全部格、楼梯全部段，以及随建筑生成的外围道路/田地，禁止只检查锚点或使用“任一格可见”
  的 `isAreaVisible` 后让结构伸进盲区。无迷雾场景保持原放置规则。
...
- **人口语义**：人口上限只由房屋等级求和；研究院、天气预测塔、风车、银行、市场、工坊、位面谐振塔和矿工营地在建筑实例上保存 `assignedWorkers` 数值，通过 `setAssignedWorkers/adjustAssignedWorkers` 占用全位面人口。普通岗位不创建实体；矿工营地是明确例外：每个已分配岗位物化一名不可选择的经济矿工实体，用于寻路、受击、背包携带与返营提交，但不接受 RTS 指令、不参与主动战斗。“矿工增援”只增加岗位容量，不能绕过人口直接生成矿工。
...
- **风车农民视觉**：`HamsterFarmerVisualSystem` 只为当前位面、已有岗位的风车维护 Phaser Sprite，显示数不超过 `visualWorkerCap`（基础 4 个岗位，“田垄扩建”满级后 6 个岗位，因此视觉上限为 6）。农民在 12 格田地环的相邻格间直线移动，循环 `idle → running → harvesting`；不创建物理体、碰撞体、寻路请求或存档对象。岗位归零、建筑出售/摧毁、位面离场时必须销毁 Sprite，回场按 `assignedWorkers` 重建。
...
- **风车本栋升级与产量公式（2026-08-24）**：`wheat_windmill_economy` 提供“精选麦种、风帆传动、田垄扩建、轮作农法”四项本栋升级。满级分别把单人基础产量从 1.5 提升到 2.0、传动倍率从 1.0 提升到 1.2、岗位从 4 提升到 6、田地产量倍率从 1.0 提升到 1.25；原生满级满员产量固定为 `6 × 2.0 × 1.2 × 1.25 = 18 粮食/秒`，再统一乘人口效率、范围内最强工坊增效与祭品生产倍率。详情面板的“满员配置产量”不包含这三个外部倍率。`windmillModules / windmillUpgrade` 进入建筑快照，后台位面必须用同一公式推进升级与产粮；旧档缺字段时按四项 Lv.0 恢复为原4岗位、6粮食/秒。
- **研究院全局/本栋边界（2026-08-23，2026-08-24 拆分）**：研究院通过 `economyType:"research"` 进入经济建筑分类。`research_structure_hp / research_passive_energy / research_recruit_speed` 三项仍由全局能力表保存，一次升级对所有位面共同生效；塔楼 LV1→LV3、`research_staff` 扩编研究员和 `research_base_points` 精密设备改为每座建筑各自保存，详情面板必须分成“本栋研究院升级”和“全局通用研究”两块，禁止再让本栋岗位/设备等级扩散到后续新建研究院。旧档中的两项全局本栋等级只在恢复时一次性复制给旧档已有的每座研究院，并转存为 `researchModules / researchUpgrade`；新建研究院从 Lv.0 模块开始。
- **研究院科研点与数量上限**：LV1/LV2/LV3 本栋基础科研分别为 1/1.5/2 点/秒，后两级必须由“制度化科研/高塔学府”解锁；初始 6 个研究员岗位，每人发挥 10%，未上岗时严格为 0，初始满编发挥 60%，两级本栋扩编后容量依次为 8/10 人并发挥 80%/100%。实际科研速度 =（本栋等级基础科研 + 本栋精密设备加成）× `min(1, 上岗研究员 × 10%)` × 全局人口效率 × 最强经济工坊增效；所有前台/后台位面的速度由 `WorldSimDriver` 汇总后只推进一次全局科技树。每个位面研究院上限不再是可购买项目：基础 1 座，“制度化科研”完成后 2 座，“高塔学府”完成后 3 座；旧版 `research_institute_capacity` 只隐藏保留为旧档已投入等级的兼容上限。
...
- **气象科研汇总口径（2026-08-24）**：气象科研与研究院速率共同进入 `WorldSimDriver` 的单次全局科技推进，但不计入科技树面板的“研究院数量”。前台读取 `PopulationEconomySystem.getWeatherForecastResearchSnapshot()`，后台读取同一模块等级、1岗位、全局人口效率和最强经济工坊增效；岗位与 `weatherModules` 随快照恢复，禁止只在当前位面加科研或把天气塔数量误显示成研究院数量。
...
- **经济岗位第二进度条真源**：研究院、天气预测塔、工坊、市场、风车和位面谐振塔必须显示稳定的业务发挥率，禁止绑定共享 `_economyTickMs`。研究院、气象科研、风车与谐振塔百分比都取“当前实际每秒产出 ÷ 满岗位配置每秒产出”；天气塔尚未升级气象科研时改显示岗位监测状态。市场显示有效商人人效，工坊显示实际/配置增效；银行因已改为明确的 10 秒离散结算，第二根条专门显示本栋结算周期进度、剩余时间和本轮金币，未上岗或覆盖人口为零时归零待命。
...
- **多位面边界**：离场位面只在快照中结算房屋读条、风车粮食、位面谐振塔能源、银行金币和市场压力衰减，不创建精灵、不寻路、不逐平民更新。房屋等级、升级读条、岗位数、生产余量与市场压力属于建筑实例字段；粮食和谐振能源随仓库结构逐仓保存，顶层 `populationEconomy` 只保留版本/迁移元数据。后台银行若背包与主人空间仓库均满，把溢出量记在对应银行的 `pendingGoldDrop`，回场物化银行后在其坐标生成掉落，禁止在错误位面提前创建实体。
...
- **面包屋离散搬运生产（2026-08-23）**：`producer-buildings.json#bakery` 只登记结构/贴图/造价，岗位与基础批次参数读取 `population-economy.json#bakery`，四项本栋等级与费用读取 `building-upgrades.json#bakery_economy`。每栋固定 1 名面包师，按“到指定仓库取 50 粮食 → 回面包屋加工 → 把整批成品送回有容量仓库”推进；取粮与交货必须调用 `EnergyManager` 的指定仓库事务，仓库不足、被毁或满仓时等待，禁止用全局粮食加减伪造搬运行程。面包师业务记录挂在建筑上并随建筑快照保存，但不进入实体/物理/战斗/寻路；Sprite 只读取这条记录的位置和阶段，并注册到通用平民视觉入口。建筑交互不得要求抵达单一精确坐标：仓库和面包屋分别按 `warehouseServiceRadius` / `bakeryServiceRadius` 接受附近脚点；直线移动超过预计路程加 `moveGraceMs` 后允许从当前位置完成纯视觉交互，避免 footprint、墙体或密集建筑让生产阶段永久卡死。
