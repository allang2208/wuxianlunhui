...
5. **遮挡/占地接入**：构造中走 `applyBuildingFootprint(this, 2)` +
   ...
   - **导入/换图/调图必须同步检查图层（2026-08-23，强制）**：任何建筑首次导入，或修改
     `texture/displayW/displayH/footOffsetY/anchorAdjust/overlay/windowGlow` 后，都不能只检查贴地和尺寸；
     必须同时确认实体具备 `_isGridBuilding + _structureDepthMode:'iso_footprint'`，主体进入
     `_syncStructureRenderOrder()`，运动 overlay、亮窗与标签跟随最终结构 depth 通道。保留 NPC、商店、
...
7. **验收**：打开左下「范围」，确认标准2×2碰撞始终为256×128且格心不偏移，贴图底座
   ...
   - **图层验收矩阵（所有建筑贴图变更必做）**：分别让玩家/友军位于建筑后方、前方、左前角、
     ...
     交叠时主体拓扑稳定，阴影在主体下，亮窗/运动部件在主体上且不穿墙。静态检查只能确认接线，
     不能代替这组运行时视觉验收。
...
- **视觉恢复**：首次接管时保存每个 Phaser 对象原本的 `visible/depth`，退出时精确恢复；建筑底部铺装
  ...
  `windowGlowSprite` 窗光脉冲；由于战争迷雾是后置可见性仲裁，必须在雾同步后再次压制这些装饰层，退出
  压平后交回常规建筑同步自然恢复。
...
- **人口语义**：人口上限只由房屋等级求和；研究院、风车、银行、市场、工坊、位面谐振塔和矿工营地在建筑实例上保存 `assignedWorkers` 数值，通过 `setAssignedWorkers/adjustAssignedWorkers` 占用全位面人口。普通岗位不创建实体；矿工营地是明确例外：每个已分配岗位物化一名不可选择的经济矿工实体，用于寻路、受击、背包携带与返营提交，但不接受 RTS 指令、不参与主动战斗。“矿工增援”只增加岗位容量，不能绕过人口直接生成矿工。
...
- **建筑亮窗蒙版（2026-08-22，2026-08-23 强度修订）**：亮窗必须从当前正式主体提取为与原图尺寸、透明边和原点完全一致的 RGBA 蒙版，通过 `producer-buildings.json#<building>.windowGlow` 配置并由 BootScene 统一预载；禁止复制整栋贴图做高亮、重绘主体或用圆形泛光覆盖墙体。中立建筑渲染链以 `BlendModes.ADD` 将蒙版放在主体之上、运动 overlay 之下，并同步主体位置、显示尺寸、旋转、镜像、结构深度、地图模式、战争迷雾和销毁生命周期。亮度读取 `EnvironmentLightingSystem.getSun().daylight`：默认白天强度底线为 `0.36`、夜间为 `1.0`、基础 alpha 为 `0.84`，再用平滑呼吸曲线按默认 `±0.26` 拉开明暗差；双频微闪默认只保留 `0.018` 的轻扰动，不能压过主脉冲。每栋实例按稳定 ID 生成不同相位，禁止逐帧随机 alpha 造成跳闪；单栋仍可用 `dayStrength/nightStrength/baseAlpha/pulseAmount/pulsePeriodMs/flickerAmount` 覆盖。多级房屋按当前 `spriteCfg.idleKey` 解析 `windowGlow.variants`，升级换图时必须同步换蒙版；该层永远不进入实体、占格、碰撞、阴影或存档。
...
- **研究院等级与科研点口径（2026-08-23）**：研究院通过 `economyType:"research"` 进入经济建筑分类并保留 `research_standard` 全局能力升级。LV1/LV2/LV3 本栋基础科研分别为 1/1.5/2 点/秒，后两级必须由“制度化科研/高塔学府”科技解锁；初始 3 个研究员岗位，每人发挥 20%，未上岗时严格为 0，岗位扩编后最多 5 人满效。实际科研速度 =（本栋等级基础科研 + 全局精密设备加成）× `min(1, 上岗研究员 × 20%)` × 全局人口效率 × 最强经济工坊增效；所有前台/后台位面的速度由 `WorldSimDriver` 汇总后只推进一次全局科技树。研究员岗位、基础科研加成和每位面研究院上限分别由 `research_staff`、`research_base_points`、`research_institute_capacity` 三项全局能力控制，默认每个位面限建 1 座，建造权威校验必须按当前位面计数。
...
- **经济岗位第二进度条真源**：研究院、工坊、市场、风车和位面谐振塔必须显示稳定的业务发挥率，禁止绑定共享 `_economyTickMs`。研究院、风车与谐振塔百分比都取“当前实际每秒产出 ÷ 满岗位配置每秒产出”，市场显示有效商人人效，工坊显示实际/配置增效；银行因已改为明确的 10 秒离散结算，第二根条专门显示本栋结算周期进度、剩余时间和本轮金币，未上岗或覆盖人口为零时归零待命。
