...
- **高能实验室四项本栋升级闭环（v45，2026-09-01）**：`high_energy_laboratory_economy`固定提供磁约束增幅、自动化实验台、探测器网络、数据标定链，分别驱动本栋基础科研、单人岗位效率、集群识别半径和单类集群增效；Lv.0为`2.50点/秒、12.50%/人、640px、3%/类`，满级为`3.50点/秒、17.50%/人、1120px、5%/类`，8个岗位不因自动化减少，岗位发挥率与集群总增效仍分别封顶100%和12%。配置只声明项目和效果；`PopulationEconomySystem`负责前台等级、费用、读条与结算，`world122-sim.js`读取同一项目值，`world122-snapshot.js`成对保存/恢复`advancedResearchModules / advancedResearchUpgrade`，后台账本必须登记升级字段。四项升级只由`high_energy_laboratory_standardization`门禁，面板按钮和扣费入口都须权威校验；图标使用正式256px与运行时128px镜像，Emoji仅作回退。
