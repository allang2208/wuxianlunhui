...
- **配置与状态分离**：科技节点、前置、研究量、布局和解锁目标统一维护在 `data/technology-tree.json`；
  ...
  - **科研升级与整级换兵边界**：`producer-buildings.json#recruitmentTiers`是等级、建筑名称、槽位、角色画像和替换目标唯一真源。可研究的等级科技通过`recruitmentTier:<tierId>`全局门禁；科研完成后，当前建筑、读档恢复与后续新建实例立即使用该级名称和建筑贴图。兵种换代独立遵守完整开发门槛：同一级所有兵种必须登记运行时`unitTypes`并移除行级`placeholder`后，才按同一槽位替换后续可招募兵种并重置该次生产周期；已生成单位、混编快照和外派部队保持原兵种，不做实体变形。尚未完整开发的等级允许先完成科研与建筑外观升级，但继续沿用最近一档完整可玩兵种，避免引用缺失实体。`supplementalUnitUnlocks`是明确的独立单位科技例外：字符串条目沿用“到来源等级自动退出”，对象条目可用`{key, untilTierLevel}`显式指定退出等级。黑火药在长弓二级之后单独加入仓鼠火枪，并以`untilTierLevel:3`保证靶场三级整级启用时退出；近代军营钢盾路线仍占位时，`trench_assault_training`继续独立解锁战壕突击兵。不能用该字段泄漏长弓、钢盾、动力甲或其他占位兵种。
  - **同级单条路线完整时的独立科技合同（战壕突击兵，2026-09-02）**：等级节点若带`recruitmentTierId`且同级仍有占位，校验器禁止在该等级节点提前登记任何`unit`解锁。完整单条路线必须使用不带`recruitmentTierId`的独立科技，只解锁自己的`unit:<key>`，将该键加入生产建筑`supplementalUnitUnlocks`；等级科技仍只负责建筑外观和时代前置，未完成的兄弟行继续保留`placeholder:true`。独立科技应依赖所属等级的建筑/时代节点，但不得成为更高完整等级的额外强制前置；旧档已完成更高稳定等级时只补齐新插入的时代祖先，不免费补独立单位科技。前台招募、生产恢复和后台模拟均使用同一单位键，整级完成后补充入口自动被正常槽位替代。
