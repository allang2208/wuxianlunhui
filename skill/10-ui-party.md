...
- 序章后的剧情引导使用独立`OpeningDialogue`全屏层，背景可复用已确认的主神空间分镜，立绘复用小鼠大王正式`portrait`；它不调用世界内`NPCDialogue`，不提前生成玩家/NPC，也不复制任务或存档状态。逐字阶段单击只补全文，再次单击才前进；按钮、画面单击、Enter/Space/右方向键、Esc跳过与Tab焦点循环必须走同一一次性生命周期。引导结束后才释放底层`inert`并进入游戏启动链。
