...
- **消耗品拖拽与背包显隐（2026-08-22）**：从玩家背包拖动消耗品时，先快照
  `systemPanel.getBoundingClientRect()`；指针仍在该范围内必须保持面板可见，允许交换背包格位置。
  只有第一次越过启动时边界后才临时隐藏 `systemPanel/panelOverlay`，为下方快捷栏让位；
  `dragend` 无论是否完成投放都恢复原 display。仓库打开时继续保持双面板可见，不进入隐藏逻辑。
