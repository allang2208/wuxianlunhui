...
- 主界面“开始游戏”只打开“新游戏/读取游戏”选择层；读取游戏在存档系统完成前保持原生禁用态并说明原因。“新游戏”是玩家显式重开入口，必须用 `force: true` 播放序章，不能被历史观看标记短路；当前固定顺序为“静态序章完成或SKIP → 小鼠大王GAL式剧情引导完成或跳过 → 唯一的`Game.start()`”，不得提前启动世界再覆盖画面。
- 静态分镜使用两个全屏 `img` 双缓冲交叉切换，镜头轻推、闪电、雨线、能量与最终白场只写在主题CSS；不把对白框、字幕或噪点颗粒叠回已确认的纯画面叙事。正式帧放在 `assets/scenes/opening/`，未选稿、重复预览和中间缓存不进入Git。
...
- 序章后的剧情引导使用独立`OpeningDialogue`全屏层，背景可复用已确认的主神空间分镜，立绘复用小鼠大王正式`portrait`；小鼠大王的身份解释统一为“原本是普通仓鼠，经历轮回与修炼后成为小鼠大王”，不改现有称号与形象。它不调用世界内`NPCDialogue`，不提前生成玩家/NPC，也不复制任务或存档状态；GAL只讲世界观和下一项剧情目标，WASD、鼠标与战斗操作留到玩家取得控制后再引导。逐字阶段单击只补全文，再次单击才前进；按钮、画面单击、Enter/Space/右方向键、Esc跳过与Tab焦点循环必须走同一一次性生命周期。引导结束后才释放底层`inert`并进入游戏启动链。
- **跳过全部教程分流（2026-09-05）**：GAL右上角的“跳过剧情”只结束当前对话并正常开始教程；“跳过所有教程”经二次选择进入自由探索或直接首城。两条路线均发放200金币并归档首次探索、首城建设任务；直接首城只授予首城资格，仍需与小鼠大王交谈并手动确认合法候选，不登记地牢胜利、经验、掉落或其他永久解锁。
...
- 首次F级新手试炼的入口说明和四房提示复用冷钢决策模态；取消不消耗首次提示，确认才进入房间。事件房固定标注“事件选择训练”，不能用“随机事件”误导；首领房首次进入可按教程合同补满状态，奖励房明确以领取并确认奖励作为成功提交点。
...
- **DOM图标加载边界**：科技树 `iconPath` 与建筑升级 `iconImage` 由DOM图片直接读取相对路径，不需要登记到 Phaser `BootScene` 纹理预载。新增一枚建筑科技图和四枚本栋升级图时，仍须在64px科技卡、48px详情/升级卡下逐枚复核语义差异、完整外框与失败emoji回退；不能用额外预载掩盖错误路径。
...
- **科技树线路可读性合同（2026-09-02）**：工程、军事与经济页默认从左向右表达研发顺序；多条远距离前置不得沿同一像素轨迹重叠，三项以上右侧目标共用“来源短横线 + 独立竖向总线 + 分支线”，同列总线按来源错开轨道。多前置目标在卡片左侧绘制独立端口、`AND` 汇流框与单一箭头，跨页前置不伪造长线，改在卡片上方显示真实科技名的 `↗` 标签。选择科技后，先压低非相关线，再在全部基础线之上绘制一层连续的“青色宽光晕 + 白色流动虚线 + 发光分叉点”；共享总线只高亮通往本次目标的实际区段，不能整根点亮。线路图例固定在画布外，不随横向滚动消失，并解释已完成、待研发、选中路线、共享总线、AND 与跨分支标签；减少动态效果偏好下停止虚线流动。
...
- **首次探索与首城任务UI（2026-09-05）**：`first_city_founding`与`first_plane_settlement`均以`QuestStore`为唯一任务真源。常驻目标条锚在小地图右侧12px，避开左下RTS栏目，外壳和16/14/12/11px文字层级直接消费冷钢主题；高频数值刷新与无障碍阶段播报分离，只在阶段变化时播报。
- 世界目标使用带深琥珀描边的半透明金色圆圈，按目标本帧真实贴图/点击区并集计算且逐帧跟随；采矿阶段另在玩家头顶绘制指向当前位面最近存活矿脉的金色箭头。建筑卡焦点必须落在整张`.we-thumb`点击卡片内部，用内描边避免滚动容器裁掉外圈；指引不得自动寻路、攻击、点击或分配岗位。
- 正式大地图常驻目录只展示已登记航图发现或曾真实建城的位面；首城选择是唯一可临时展示待建候选的入口，并且只消费`getFirstFoundingCandidates()`。测试直连、模板预览以及静态`entries/destinations`名单不得产生玩家可见按钮或“尚未构建”占位文字，开发测试入口只能留在交互开发工具。
- 新手文案首次使用“F级钥匙（时空锚点）”，后续简称“F级钥匙”；采矿写“实际入库300能源”，科研毕业写“研究进度开始增长”。首城建设的建造步骤明确提示按B，房屋和研究所尚未建成时不把金圈错误套在上一座建筑上。
