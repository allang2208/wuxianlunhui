...
- 生硬根因是 AI 决策层瞬时 `vx=0; vy=0`；MovementSystem 自带速度渐近（无移动目标
  `friction≈0.85/帧`，有目标按 `accel` 向目标速度收敛），决策层只设 `maxSpeed=0` 即缓停。
- 士兵 AI 四件套（仓鼠 10 兵种统一）：①站定动作（挥击/射击/施法/采矿）每帧
  `damp=0.85^(dt/16.67)` 指数衰减、<1px/s 收零；②决策分支（跟随到位/间隔待机/hold/
  RTS 到位）删硬清零；③跟随 120px 内 `maxSpeed=walkSpeed×max(0.3, dist/120)` 缓出；
  ④ `MovementSystem.update` 后速度 >25px/s 时强制 `_animState='walk'`，防 idle 姿势滑冰。
- 保留硬停：`cancelForCommand`、防御性 attack 残留复位、骑士冲锋（直接改坐标不走速度积分）。
- 纯视觉平民（农民/银行家/工程师）走各自 `moveWorker`：期望速度指数渐近 + 60px 内随距离
  线性衰减；到位判定需距离近且速度 <10px/s 才吸附，防高速冲过判定点。
- 调参：衰减率 0.85（越小停越快）、滑行阈值 25、缓出半径士兵 120 / 平民 60。
...
- 每座楼梯使用单持有者 FIFO 队列，预约 TTL、排队超时、入口半径、重规划周期和失败恢复上限全部由
  `defense-structures.json.wallWalk.surfaceNavigation` 驱动；规划时把队列长度折算为入口代价，分流到
  较空闲楼梯。
