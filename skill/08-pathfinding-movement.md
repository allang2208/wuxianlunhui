...
- `StrategicGarrisonAI`只发布防守意图；移动、硬控、攻击、弹道、承托与碰撞仍由共享系统执行。远程配置决定兵种角色；墙顶定点射手仅选择射程和LOS有效目标，不追下墙，承托破坏后按原坍塌/落地流程转地面防守。
- 近战按受击墙段与附近进攻者实际位移调防，不读取玩家尚未执行的指令。当前共享战况500ms、调防3秒、预测1.5秒且限制预测距离；站位避开设施、楼梯和已分配位置，使用守点追击半径与回防状态防止无限追出城。
- 单位决策250ms并错峰，每帧最多4次决策、每次最多4次LOS；候选游标跨次推进，不能总从被墙挡住的前几名开始。站位只做有限局部候选，不逐个预跑A*；寻路继续服从公共预算，预算延后不等于不可达。
- 相同意图重发时保留`MovementSystem`正在执行的`_isReposition`绕障点及计时器，只有目标/状态/站位实质变化才清旧路径；否则低频指挥也会反复打断脱困。驻守实例的感知与移动门禁须共同接线，不能每帧被通用AI覆盖。
- 决策节流不节流表面复核、碰撞、硬控或战斗。退出释放该场驻守记录与高架跟踪；这些预算不代表实测FPS或全帧耗时上限。
...
- **预览与正式搜索隔离**：悬停只读结构资格，不创建、覆盖或取消单位的正式成本查询；命令才持有搜索状态。
- **活跃请求与结果缓存分开**：TTL/LRU只能回收闲置结果，在途成本场须保留直到完成或取消；容量满让新请求等待，不能反复驱逐正在计算的长路线。
- **高架借道必须分层计价和分段执行**：地面与墙顶即使XY相同也使用不同节点；只连接真实楼梯/墙塔边，成本采用实际折线路程。返回结果拆为地面路径、入口、高架航点、独立出口；不得把高架边当作地面直达。
- **重选梯与等待续算分开**：局部超时允许开启新规划代次、重新冻结入口成本；pending轮询不能重启搜索。堵塞入口采用有期限、随拓扑失效的非负加价，不永久封禁唯一楼梯；入梯者仍须先安全离梯。
- **追击复用同时检查承载面**：XY小幅移动不能证明目标仍在同层；还须比较目标身份、墙/楼梯组及Z快照。目标换层后重新规划，不能继续沿旧借道路线追向原层。
...
- **终点精度不能沿用普通move的40px容差（2026-08-30）**：`formationSlot:true`经命令克隆和
  战术命令恢复保留，地面最终槽位统一用4px容差；`MovementSystem`在控制/击退处理之后逐帧检查到位，
  路径跟随和普通移动末段限速，避免120ms决策间隔跨过终点。该规则不接管高架路线、恐惧/击退/技能位移，
  也不把途中散开改为刚性行军队形。布局与预约必须同按投影后的安全间距核算，详见第7卷终点方阵合同。
- **编队玩家末段不能只跟到网格中心**：4px槽位精度必须配套实际目标点的接近逻辑；64px内先验证
  PathFinder落点/线段与WallSystem遮挡，再发布朝真实槽位的减速意图。不能无条件直冲、瞬移或改变基础移速。
...
- **入口等待必须发布零位移（2026-08-29）**：共享 RTS 航点解析器在 FIFO 拒绝/阻塞时必须把本帧
  `destination` 收回单位脚下，同时保留真实 `routeDestination/routeIndex` 供后续获权继续；玩家控制器与
  各兵种 AI 因而统一原地等待。仅把 `vx/vy` 清零但仍把被占用 Portal 返回给 AI，会在下一层重新加速顶入口。
...
- **高架碰撞提交匹配玩家（2026-08-29）**：士兵处于路线、排队、楼梯、墙顶或桥接态时，
  `WallSystem.resolve` 的统一表面结果就是本帧终点；位置积分沿用玩家的 34ms 长帧限制，禁止随后再执行地面世界 X/Y 轴
  滑动。斜向楼梯经表面投影后再次拆轴会把单位推回踏步边缘，是玩家丝滑而士兵卡梯的双口径根因。
- 敌军不是全量准入：`enemyArchetypes` 白名单当前只开放僵尸和黑狼近战。远程若本层已有射程与 LOS
  则不抢楼梯；近战的停步、起手、命中与骑士冲锋都必须同时满足二维范围和垂直可达条件。

#### 友军运行时脱困与墙体豁免边界（2026-08-23）

- **统一诊断入口**：T → 交互开发工具 → 导航，具体开关、快照限额、真实单位选择、观察模式隔离及导出合同见第03卷“导航诊断页”。取证仅订阅既有恢复/失败分支，不能新增全场轮询、触发寻路或续租FIFO；诊断记录不是自动脱困指令。运行时代码发布状态见 `docs/world122-navigation-diagnostics-progress-2026-08-30.md`。
