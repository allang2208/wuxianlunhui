...
- 强化石、改造券、魔法粉尘分别使用 `enhancement_stone`、`reforge_ticket`、`magic_dust`；普通材料不能通过临时拼装同名矿石替代祭品或强化材料。复制正式模板，保留图标、用途、售价及物品身份。
- `stack` 是现有数量，`maxStack` 是每格上限。强化石与改造券的有效上限由 `getItemMaxStack()` 固定为 9999，兼容缺少 ID 的同名旧实例；背包容量预判、拾取、仓库和奖励规划均须读同一规则，不能只修改新奖励模板。粉尘正式模板也为 9999；不擅自删除或重整历史库存。
- 出售堆叠按“原单件半价 × 实际数量”同时生成预览与收款；确认整笔金币可接收且收款成功后，才清空出售栏。关闭商店逐件转移，放不下的物品保留在原出售栏，不借用地牢战利品寄存规则。
- 满背包与满仓库双向换位，容量计算先扣除本次移出的物品，再计算腾出的格子；转移失败保留原物和数量。交易内存保留不等于关闭游戏后可恢复，持久化另按存档合同实现。
