...
- 台阶、露台、连廊先共用锁定正交相机和实体支撑；踏步两侧必须闭合，柱距在模型坐标中均分。台基只留一张承载面，不能将旧露台与新主台重复叠加。模型/Depth解决结构问题，颜色贴图不补几何空洞。
- 白石统一以矿物纹、逐砖方向和粗糙度差异控制；扩大周期、使用均匀天空反光，避免重复的局部矩形亮斑。预投影的连续铺装使用 `textureScaleY:1`，不能再次做等距压缩。高反射和全场均匀哑光都不能替代材质参照。
- 显示尺寸、脚点深度、实体碰撞与地形可移动多边形分别维护。主城通行区域由实际砖地与后方露台并集形成，山景不属于地面；柱脚/挡墙作为实心障碍。切场景清理范围并使寻路缓存失效；旧档位置只在入场恢复时修正，普通移动不能瞬移回出生点。
- 昼暮夜背景锁定原山形、相机和裁切；读取 `EnvironmentLightingSystem` 已保存的世界相位。叠图渐变需按 source-over 反解中间层 Alpha，不能直接把三张权重作为逐层透明度。云雾/远鸟共用世界基线裁切并位于建筑后方；光池和嵌线位于地台之上、阴影/人物之下，灯芯沿灯具脚点深度排序。白石上的 ADD 光易饱和为白色，小范围 NORMAL 暖色灯芯与外层光晕分开。
- 当前实现真源：`src/world/main-hub-architecture.js`、`walkable-area.js`、`main-hub-atmosphere.js`，双份 `game-config.json` 的主城配置；可重建入口见 [主神空间归档](../tools/ai-gen/_main_space_architecture_20260903/README.md)。保留最终 packed blend、对象归属/相机清单、选中纹理和生成参数，离线预览不等于实机验收；草案清理不能删除后续编辑的直接输入。
