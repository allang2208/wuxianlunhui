...
**场景化建筑组件的相机门禁（2026-09-05 主神空间复盘）**：可放置建筑的30°/44.8°标准不能机械替代大型场景自己的游戏镜头；场景一旦已有统一母相机，该相机的投影类型、旋转、`orthoScale`、画布原点、对象世界变换和左右基线就是唯一视角真源。单件组件必须在完整母场景中原位渲染，再按屏幕包围框只做裁切；禁止为方便生图把立柱、翼廊、台基或灯台重新居中、重新取景、改成正立面相机，或用二维拉伸/翻转补角度。进入12步或其他材质精修前，先把原生 Blender 组件按计划坐标合回目标地板与背景，核对同高度顶面的可见量、左右侧面的压缩关系、接地线和成对组件的世界镜像；任一项冲突就回模型/相机修正，Depth与提示词不能挽救错误透视。若方案明确保留现有运行时地砖，Blender不得再输出覆盖整场的替代地面；只输出实际需要的台阶/坐凳基座、立柱/拱廊和灯台等组件，并把背景、固定台基层、遮挡层、碰撞边界与阴影代理分开登记，不借美术换图改动既有光照或阴影参数。
...
- `tools/ai-gen/prompts/world122-building-style.md`：12步与48步共享的唯一 `world122-building-v4` 画风真源。最高优先级是次世代PBR逻辑的游戏向材质表现：石材保留自然风化肌理，木质构件呈现长期使用磨损，黄铜装饰做旧并自然氧化；光影统一使用柔和左上顶侧光，在建筑识别度与写实质感之间保持平衡。公共画风不再强制任何建筑语法，但会按建筑年代强制路由毛石地台或清水混凝土地台；半木石、哥特、现代、工业或未来主体结构仍只由白模和资产级 manifest 决定。两阶段每一张正式候选都必须完整注入该公共画风块，缺少 `styleVersion`、`styleTemplate`、地台路由或正文时必须停止。
