...
- 无左右解剖朝向的抽象首领用各动作`frameLayouts`还原独立裁框和共同中心根点，渲染固定`flipX:false`；移动只改变Collider，悬浮升降留在素材时钟。技能起手仍按目标地面方向生成椭圆/定向判定，但不能因此镜像或旋转壳体。多段牵引、脉冲等事件使用同一手动帧时钟与0-based阈值，每个目标逐次复查同层、地面形状和LOS，自管技能不得泄漏到通用普攻。

### 远程精英的停步迟滞与固定落点预警

- 追击刹停、重新追击与释放硬射程分开配置：例如340停步、390重追、420释放上限。迟滞绑定当前目标身份；目标失效、换层或遮挡时解除停步。等待冷却不是施法锁，必须让位于击退、恐惧、出入口导航、登台和显式移动指令。
- 动作开始即计冷却时，1500ms收势只消费同一4200ms周期的一部分，不在结束后再补一次完整冷却。释放用逐帧时长累计阈值，取消动作保留已消耗冷却。
- 落点在释放帧锁定，预警不追踪目标。特效首个update只消费本帧越过释放阈值后的余量，避免900ms预警因重复消费dt缩短；地面显示椭圆与伤害形状共用长短轴及坐标系。
- 明确释放前后生命周期：未释放受控制/死亡取消；已释放独立效果按设计完成，切场/战斗结束清理。伤害后再施加实际命中状态，同名减速只刷新；移动入口只乘一次，移除、净化与状态栏同时收口。

### 特殊攻击审计：单次结果、控制、召唤与尸体边界

- 弹反结果只属于本次受击：在玩家受击入口的所有提前返回之前重置旧标记，伤害调用返回后立即快照；后续击退、控制、附伤读取该次结果，不能隔帧读取盾牌上的残留状态。同步反伤回调可能改变实体状态，继续处理前重新确认动作仍有效。
- `bind`限制位移，不等同眩晕。飞扑/冲锋同时检查起手与运动阶段，途中取消后不能解控补冲；保留原地攻击和收势。钻地传送在隐藏前及写入出土坐标前分别确认落点通行；失败取消当前攻击并恢复可见/碰撞/HUD标记，不另选未预警的攻击位置。
- 自管死亡、停尸、淡出共用游戏dt，跨阶段余量连续消费；公共回收不能用墙钟抢先删除未完成尸体。死亡解除石化/冻结，定格入口仅限制活体；死亡奖励仍只经过一次原入口。更新、渲染和资源驻留都应覆盖淡出阶段。
- 召唤前登记完整视觉依赖，释放前再次确认就绪；异步回调只加载资源，不补发实体。释放事件先标记消费，资源失败或无合法落点就少生成并正常收势，不能把时钟退回释放帧无限等待。落点避让墙、召唤者、目标、玩家、友军及同批幼虫，采用同一地面透视和占据半径。
- 固定落点的弹道在起手快照合法遮挡豁免；只忽略来源承托与原目标自身结构，不豁免中间其他墙。实体脚点在动作切换当帧更新，不能等场景完成定位后才由贴图选项补写。
- 清单必须区分“代码已改”“源素材已交付”“用户实机已验收”。专项修复完成不代表所有动作、性能和组合场景已通过；静态预算也不是实测卡顿原因。
### 恐怖普通怪：单时钟状态机、脚点同步和尸体淡出

- 配置/注册/地牢工厂/Boot资源登记必须一起交付，`data` 与 `public/data` 同步。普通怪使用同级六维伤害公式；地牢专属怪设置 `poolWhitelistOnly`，只追加到指定恐怖地牢 `poolKeys`，保留 `matchPoolRanks`，不污染通用池、矿洞池或首领名单。
- `frameDurations` 是动作帧和伤害事件的共同时间源。近战接现有 melee timeline，远程事件在跨过释放时刻时只消费一次；先置已消费标记，再调用伤害/格挡回调，随后确认动作仍有效，防止同步反伤死亡后继续出手。
- 切换动作立即同步该动作的 `footOffsetY`。场景先同步位置再切纹理，等到 `_getPhaserOptions()` 才更新脚点会跳一帧；外部控制入口已先写入 `_animState` 时，也必须刷新脚点，但同状态不重置循环时钟。碰撞脚点不跟随高举武器移动。
- 死亡动画、末帧留尸、淡出是三个阶段。实体死亡更新、视口可见性、位置同步、动态排序与资源驻留都必须覆盖 `_fadeTimer`；已有入口优先复用 `Game.isPreservedCorpse()`，资源收集器在没有活Sprite时也需保留仍在淡出的尸体。
- 投射物 `size` 是碰撞直径，不是半径；骨镖半径3传入工厂6。定向弹体显式 `rotateWithVelocity:true`，独立 `displaySize` 只影响显示；新建、池复用和释放都重置可选字段，默认球形弹体维持原朝向与尺寸。
- 静态代码接入、素材预算记录不等于实机通过；未获测试授权不跑游戏/构建，明确由用户验证左右出手、控制中断、死亡到淡出和同级战斗手感。


### 自管多技能首领：射程口径、控制入口与事件时钟

- 同时有远程最小距离与近战技能时，只有当前距离可用的技能才能扩大追击刹停距离；目标进了远程盲区，不能仍以远程最大射程停车。地面椭圆的起手/刹停距离按目标方向做同一透视投影；真实世界坐标弹道的射程与落点裁限使用同一坐标口径，避免纵向只飞到半程。
- 手动选帧的首领只用一份逻辑动作时钟，动画帧与0-based释放事件共同消费它；多个事件逐一记录已消费状态，累计dt跨过整个窗口也只能各触发一次。按“已释放的攻击动作”计充能时，一次三连发只计一次，首发前中断不计数。
- 硬控合同必须覆盖主循环的特殊入口：石化会绕过怪物常规update，因此在 `updateWhilePetrified` 中取消未释放事件/预警，继续调用基类状态与DOT更新。明确已发射弹体是继续还是取消，死亡/离场统一清理；`super.update` 内DOT致死后立即退出自管攻击分支。
- 易伤窗口走正常伤害入口而不是直接扣血，说明倍率是在输入攻击量还是最终伤害阶段应用；死亡动画、停尸与淡出使用已有 `_deathAnimTimer/_corpseTimer/_fadeTimer` 合同，死亡奖励只调用一次。代码接入与素材报告不等于运行时验证，沿用用户的测试授权边界。
...
#### 双形态变身怪：源内容尺度、运行时视觉倍率与碰撞倍率分离（2026-08-27 红狼王）

- 四足形态切两足形态时，不能只比较 Alpha 高度：两足可能更高但轮廓远窄，实际可见面积和躯干体量反而明显变小。先以变身首帧/末帧、稳定待机帧和统一脚线为基准，同时比较有效体高、有效宽度与可见面积；极端攻击姿态、腾空帧和倒地尸体不参与常态体量定标。
- 素材侧先统一同一形态的内容尺度和脚线。红狼人正式表以约290px站立源体高、640格、`footY:590`统一待机/奔跑/攻击/飞扑/嚎叫/死亡；所有动作共用一个运行时视觉倍率，禁止每张表各自补偿或逐帧拉伸。
- 运行时必须把 `werewolfVisualScale` 与 `werewolfCollisionScale` 分开。视觉倍率只决定贴图显示和由脚点推导的偏移；碰撞倍率只缩放变身前捕获的 `collisionRadius/collisionWidth/collisionHeight/collider.height`，不得因美术调大顺带扩大攻击或受击判定。红狼王当前用户实测定稿为视觉1.8、碰撞1.25，这是该首领的结果，不是其他变身怪的默认值。
- 同一张变身表内要按变身进度连续插值视觉与碰撞倍率；切换到新纹理时必须再次应用当前 `spriteSize`，否则纹理帧格变化会把角色缩回创建时尺寸。脚线继续由 `footY/frameHeight` 计算，HUD 使用可见模型顶部，不另写状态专属 Y 偏移。
- 死在变身中途时恢复基础碰撞体并播放狼形死亡；完整变身后死亡继续使用狼人倍率和狼人死亡表，但尸体生命周期、末帧定格和移除时序保持原合同。调尺度不应改生命、伤害、速度、技能范围或尸体时长。
...
新制/重做先进入 [动画工作流](00-workflows.md#动画) 定动作范围与当前阶段，再按需读取下面的插帧合同。

统一执行 [16b §4](16b-animation-alignment-and-timing.md#4-插帧与循环)，不重复强制加工已流畅源帧。采用RIFE时保留关键帧、循环/一次性分流、Alpha和事件时钟映射；无需插帧时交付原时钟正式表与GIF。
...
#### 召唤型首领更换尚未存在的召唤物：分阶段接入合同（2026-08-28）

- 用户指定新的召唤物、但该实体尚不存在时，只把目标数量、分类与验收口径登记到 `TODO.md`；在替代物拥有正式资产、配置和实体工厂前，不提前修改首领的 `summonType/count/skills`，现有可运行召唤保持不变。
- 固定顺序为：母图与身份锁 → 动作视频、透明精灵表与 RIFE → `data/public` 双份怪物配置 → 工厂导出与 BootScene 驻留 → 墙体安全出生、目标继承和生命周期 → 同一提交切换首领配置、技能说明与图鉴文本。
- 禁止用程序占位图、其他怪物贴图或临时实体冒充新召唤物。正式切换必须同时维护唯一注册键、单次生成数、存活上限、受阻位置回退、死亡注销和首领死亡后的召唤物存续策略。
- 验收至少覆盖：完整释放数量、达到存活上限后的拦截、墙边/狭窄处安全出生、召唤物死亡后的补充、首领死亡后的生命周期，以及配置和说明中不再残留旧召唤物名称。

#### 多阶段怪物动作的单一时钟合同（2026-08-29，岩芯钻虫）

- 入土→隐藏→破土、持械→断裂→空手等多阶段动作，逐阶段以正式精灵表为真源登记 `duration/frameCount/contactFrame|impactFrame`；帧号统一 0-based。双份 `enemy-config.attackSkills`、`textures.frameLayouts` 和怪类代码兜底值必须完全一致。
- 最终裁掉土痕、烟尘、投射物等边界帧后，必须重新计算阶段帧数、墙钟和命中帧；禁止只改 `frameLayouts`。否则逻辑会提前隐藏、延后恢复，或在错误画面结算伤害。
- 特效或生成瑕疵只按动作特定 ROI/连通域清理：保留角色、持有物和获准自然轨迹，剔除独立飞入的投射物、离体木块或纯地面残留。清理后仍以实际成品表回写状态机时钟，不以原视频帧号继续驱动。
...
6. **确定冲锋位移真源**：若玩法状态机在世界坐标中推进 Collider，攻击视频必须表现原地蓄力/旋转/冲刺姿态并锁定躯干根节点；重建精灵表时固定中心和脚线。只有明确选择“表内位移”时才保留视频根运动，禁止状态机位移与表内位移叠加造成二次冲锋或切回待机瞬移。
...
2:1 地面投影的定向范围技能必须同时保存逻辑地面角和屏幕角。地面判定角按 `atan2(dy / PERSPECTIVE_SCALE_Y, dx)` 计算，`GroundSector` / `GroundDirectedRect` 与伤害复查使用该角；贴图朝向、舌头或屏幕空间连线使用 `atan2(dy, dx)`。从逻辑角推导落点/VFX 时，Y 分量必须再乘 `PERSPECTIVE_SCALE_Y`，否则斜向毒池、预警和伤害会发生可见错位。

召唤型 Boss 的候选落点除 `WallSystem.isPositionSafe` 外，还要按逆投影地面距离检查 Boss、本次目标、玩家/友军、现存召唤物和同一次施法已选中的落点；间距至少覆盖双方碰撞半径与设计缓冲。有限次数内找不到安全点就跳过该只召唤，禁止强塞墙内或叠在同伴脚下；召唤实例仍必须在创建时写入零经验、零金币、零掉落和不计训练击杀的统一召唤标记。
...
- 自管待命/移动/攻击/死亡的普通怪，接触判定与渲染帧共同使用 `stepBasicMeleeTimeline` 的累计dt；手动返回 `frame` 时不再返回启动第二时钟的 `animState`。配置帧数、总时长和接触帧同源，禁用通用攻击调度，避免隐形第二击。
- 攻击锁定 `primaryTarget/basicMeleeSnapshot`，接触窗跨帧也只消费一次，再通过公共同地表/方向形状/视线和DamagePipeline结算。没有逻辑推进的停步攻击，起手范围不得大于真正命中范围，否则刚进入起手范围的静止目标也会空挥；有短推进时才单独配置更远的接敌距离并重锚，不能重新瞄准。
- 眩晕/冻结/石化/冲刺眩晕/恐惧均清未结算攻击并保留冷却。主循环跳过子类update的石化路径要在 `updateWhilePetrified` 同步取消；恐惧逃跑只在进入时取消攻击，不要每帧把walk重置到idle后再切walk，否则动画会卡首帧。
- 死亡入口只结算一次，仍调用基类奖励/掉落；动画、尸体停留、淡出用累计时间跨阶段处理，最后钳住末帧并释放Sprite。保持基类尸体保留与延迟移除字段，不能用循环death替代生命周期。
...
#### 5. 验证
仅用户授权测试后按改动选择 lint / vite build / test-collider / test-config-integrity；实机验收关注 idle/walk 切换、朝向翻转、游走范围与停留节奏、名字标签位置。
...
#### 2026-08-27 骑兵升级线母图身份锁定

- 骑兵母图要同时锁定“骑手血统”和“坐骑血统”：轻骑升级沿用轻骑仓鼠的脸型、体型、主色与猫；重骑升级沿用骑士仓鼠的
  脸型、板甲语言与猫。只参考同批高阶候选会把骑手变成陌生角色，必须把已入线的低阶单位作为独立参考输入。
- 时代进入热兵器不要求突然机械化。若路线仍强调骑兵机动/冲锋，可把侦察车占位收敛为骑猫侦察步枪手，把坦克占位收敛为
  防爆重装、凯夫拉猫甲与动力助推爆矛的重骑兵；这是兵种路线决策，必须在索引登记 `replacesPlaceholderUnitKey`，不能由
  生图阶段自行替换运行时单位。
- 修改范围必须逐项硬锁。用户只要求“脸部收敛”时，提示词明确保持身体宽度、四肢、姿势、武器和服装不变；只换武器、头饰
  或护甲时同理列出保留项。骑乘重装还要逐项锁仓鼠、猫的四足比例、鞍具、缰绳、武器朝向、猫甲和成对助推器，禁止把猫甲
  画成人形马甲或让装备左右数量漂移。

#### 友军动画生产

直接进入 [动画工作流](00-workflows.md#动画) 定动作范围、阶段与返工入口；素材完成后返回本卷完成友军接线。

与敌人共用 [16](16-character-sprite-production.md) 的预算/来源及 [16b §4](16b-animation-alignment-and-timing.md#4-插帧与循环) 的按需插帧合同；不维护第二套门禁。友军专属接线见下方“友军战斗与生命周期统一”“友军资源驻留登记”。
...
- **普通仓鼠步兵不随装备升级扩大占地**：弩手、冠军、突击、特战、方阵、防暴队、重机枪同样
  使用上述半径20、身体高100、矩形40×100规格；重盾、重甲与长武器本身不构成特殊体型声明。
  后续驻守间距应读取规范地面半径并另设部署间隔，不从贴图、武器长度或受击高度反推占地。
...
- **先核对真实继承链**：Companion并非DamageableEntity子类，不能复制调用后者私有方法的承伤/控制代码而不补齐接口。共享圣佑、净化、控制中断与防御返回值使用同一语义；弹反只消费本次承伤结果，不读取上一次缓存。
- **一次攻击只有一个动作时钟**：AI通过`friendly-attack-timing.js`推进，渲染读取同一时钟；命中、出膛、连射、弃筒和音效从同一源帧映射。普通帧延迟为`(1-based frame - 1) / fps`，炮组已有0-based释放帧与变时长数组则按其原协议累计，禁止统一减一。
- **炮组的动作、冷却和在途弹体分层**：`attack.durationMs`只锁住开火/后坐/装填动作，`attackInterval`仍表示起手到下次起手的完整周期；动作播完应回待机，不能把长冷却填成慢放。0-based`attackReleaseFrame`按进入该帧前的`frameDurations.slice(0, frame).sum()`触发唯一出膛事件；已出膛弹体脱离动作时钟独立飞行，后续硬控、死亡或冷却不得把它追回或重复生成。
- **首帧从零开始、旧回调不得越界**：本帧才起手的动作不能扣掉起手前整帧dt；每次攻击/冲锋/祈祷递增动作序号，切换时注销旧回调。渲染完成不代替AI结束判定，控制暂停时逻辑与画面同时暂停。
- **提速同时消除动画下限**：缩短过慢前摇时保留完整帧序，适度缩短多余空等；若升级后的间隔小于完整动作，在起手快照播放倍率，让整段动作和释放时点一起匹配有效间隔。施法/火箭保留独立技能节奏；弹丸飞行仍用真实dt，机枪后续子弹只推进实际出膛后的剩余时间。
- **原自然段落必须保留**：战士等单位仍使用配置的起手段与连续段，不把素材运动重新拉直；伤害在所选接触帧结算一次，不能靠独立周期立即扣血。旧首航条目中的固定帧数/FPS属于历史配置，当前以兵种JSON与共享时钟为准。
- **中断与在途分离**：硬控取消尚未释放的普通攻击，但已发出弹体继续独立推进；原本锁定的祈祷/探索动作按原规则暂停，不扩大允许取消的范围。派生AI尤其要核对自己的起手覆盖是否也初始化时钟。
- **死亡按实际动画、身份先判子类**：基础死亡等待从单位当前死亡配置计算，炮组保留专属逐帧时钟/尸体规则；清掉动作、路径、冲锋临时碰撞与所属建筑引用。类似长戟继承民兵的单位，`getUnitKind`先判派生标记；不猜测迁移已经丢失身份的旧档。
- **前后台口径一起核对**：多发动作的后台输出计入真实发数；当完整动作已随攻速压缩，不再沿用旧动画时长作为后台攻击下限。技能冷却、弹道飞行和后台近似模型仍各自保留，不把配置推导值写成实机测量。

上述新接口对应本地仓鼠审计实现，远端接线状态与逐单位参数见`docs/audits/hamster-cleanup-and-publication-2026-08-31.md`及其关联报告；源帧校准和代码核对不代替用户授权后的运行时验收。
...
- 工程器械还必须登记专属投射物纹理依赖、最小/最大射程、弹道高度与速度、落点AOE/衰减和后台`expectedExtraTargets`。前台AOE实体筛选、兵线估算与世界后台估算必须包含同一兵种键；炮组占2人口时公共人口表、前台生成预检/扣账和后台生成事务共同消费，禁止在某一入口退回默认1人口。
...
- 精灵图入 `assets/companions/<id>/`（idle/walking/mining/dying 各一张），
  按有效内容选择固定帧格与紧凑列行数（512×512、8列仅是旧实例）；先核对真实布局/有效帧，再配frameCount、脚点和显示比例。
...
- **动物/自然词条基线（2026-08-27）**：所有 `family:"动物"` 怪物默认同步 `families` 增补自然语义，即
  `families:["动物","自然"]`；僵尸犬是明确例外，继续保留 `family:"僵尸"`、`families:["僵尸","动物"]`；熊德鲁伊继续在既有
  `families:["动物","大型"]` 基础上叠加 `"自然"`。该调整不直接改变属性、AI、伤害、掉落或固定刷怪配置；现有及后续通过
  `getEnemyFamilies/hasEnemyFamily` 查询 `"自然"` 的系统会统一识别这些动物怪物。
