...
class SlashEffect {
        ...
        const SkillManager = {
            ...
            renderSkillDetail(skill) {
                ...
                if (skill.id === 'swordMastery') {
                    ...
                    const knockbackDist = baseKnockback + 150 + skill.level * 5;
                    ...
                    if (nextEffect) {
                        ...
                        const nextKnockback = baseKnockback + 150 + (skill.level + 1) * 5;
        ...
        class Player extends Entity {
            constructor(x, y) {
                ...
                this._dashTrailScreenPoints = []; // 剑气轨迹屏幕坐标点
                this.attacks = {
            ...
            triggerDashAttack(entities) {
                ...
                this._dashSlashShown = false; // 白色扇形特效标记
                this._dashBounceApplied = false; // 撞墙弹回标记
                this._dashTrailScreenPoints = []; // 重置剑气轨迹
                this._sprintDuration = 0;
            ...
            updateDashAttack(dt, entities) {
                ...
                if (progress < 0.2917) {
                    ...
                    this._dashState = 'recover';
                } else {
                    this._isDashing = false;
                    this._dashState = 'idle';
                    this._dashTimer = 0;
                    this._dashBounceApplied = false;
                    this._dashTrailScreenPoints = []; // 清空剑气轨迹
                ...
                if (this._dashState === 'slash') {
                    ...
                    if (Game.showAttackRange && !this._dashRangeShown) {
                        ...
                        EffectManager.add(new AttackRangeEffect(this._dashStartPos.x, this._dashStartPos.y, attackAngle, range, 2 * Math.PI / 3, 'sector', 600));
                    }
                    // 白色扇形特效（已删除，改用剑气轨迹）
                    if (!this._dashSlashShown) {
                        this._dashSlashShown = true;
                    }
                    // 攻击判定（150ms窗口）
            ...
            renderWeapon(ctx) {
                ...
                // === 近战（剑等）渲染 ===
                    ...
                    if (this._isDashing) {
                        ...
                        // 记录剑尖屏幕坐标（用于剑气轨迹）
                        if (0.2917 <= dashProgress && dashProgress < 0.6667) {
                            if (ctx.getTransform) {
                                const m = ctx.getTransform();
                                const tipScreenX = m.a * 0 + m.c * (-s / 2) + m.e;
                                const tipScreenY = m.b * 0 + m.d * (-s / 2) + m.f;
                                this._dashTrailScreenPoints.push({ x: tipScreenX, y: tipScreenY });
                            }
                        }
            ...
            render(ctx) {
                ...
                ctx.restore();
                // 绘制剑气轨迹（冲刺攻击挥砍阶段）
                if (this._dashTrailScreenPoints.length > 1) {
                    ctx.save();
                    ...
                    ctx.shadowBlur = 8;
                    ctx.beginPath();
                    ctx.moveTo(this._dashTrailScreenPoints[0].x, this._dashTrailScreenPoints[0].y);
                    for (let i = 1; i < this._dashTrailScreenPoints.length; i++) {
                        ctx.lineTo(this._dashTrailScreenPoints[i].x, this._dashTrailScreenPoints[i].y);
                    }
                    ctx.stroke();
                    // 绘制半透明填充
