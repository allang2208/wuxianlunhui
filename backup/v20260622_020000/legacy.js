...
class SlashEffect {
        ...
        const SkillManager = {
            ...
            addMeleeExp(player, hitCount, killCount) {
                ...
                let gained = 0;
                // 每次攻击积累1点经验，5级以后攻击不再积累经验
                if (sm.level <= 5) gained += 1;
                // 每次击杀目标增加10点经验
                gained += killCount * 10;
                // 每次攻击到大于1个敌人时增加3点经验
                if (hitCount > 1) gained += 3;
                if (gained <= 0) return;
                sm.exp += gained;
                // 检查升级
            ...
            onLevelUp(player, sm) {
                ...
                text.textContent = `剑精通升级！Lv.${sm.level}`;
                document.body.appendChild(text);
                setTimeout(() => { if (text && text.parentNode) text.remove(); }, 1500);
                // 更新攻击冷却（应用新的冷却缩减）
            ...
            renderSkillDetail(skill) {
                ...
                body.innerHTML = `
                    ...
                    <div class="sd-section">
                        <h4>升级方式</h4>
                        <p>• 每次攻击积累 1 点经验（5级后不再获得）</p>
                        <p>• 每次击杀目标增加 10 点经验</p>
                        <p>• 每次攻击到多个敌人时增加 3 点经验</p>
                    </div>
                `;
