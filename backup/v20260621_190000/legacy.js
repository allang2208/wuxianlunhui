...
class SlashEffect {
        ...
        const WEAPON_ANIM = {
            ...
            windupMs: 200, swingMs: 500, recoverMs: 600,
        };
                ...
                // === 近战（剑等）渲染 ===
                    ...
                    if (isAttacking) {
                        ...
                        if (anim.state === 'windup') {
                            ...
                            thrustOffset = -s * 0.35 * easeInQuad(t);
                        } else if (anim.state === 'swing') {
                            const t = anim.timer / wa.swingMs;
                            if (t < 0.6) {
                                const pt = t / 0.6;
                                thrustOffset = -s * 0.35 + s * 1.25 * easeOutQuad(pt);
                            } else {
                                thrustOffset = s * 0.9;
                            }
                        } else {
                            const t = anim.timer / wa.recoverMs;
                            thrustOffset = s * 0.9 * (1 - easeOutQuad(t));
                        }
        ...
        const WeaponAnimConfig = {
            ...
            pistol: { holdOffsetX: 0, holdOffsetY: 4, timingMul: 0.06, animType: 'recoil', recoilAmount: 0.15 }, // G18 1100RPM: 总动画约48ms < 55ms cooldown
