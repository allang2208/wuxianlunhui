...
class SlashEffect {
        ...
        const SkillManager = {
            ...
            renderSkillDetail(skill) {
                ...
                if (skill.id === 'swordMastery') {
                    ...
                    html += `<p style="margin-top:6px;color:#a0907a;font-size:12px;">触发条件：长按Shift冲刺超过1.5秒后，使用近战武器左键攻击</p>`;
                }
                html += `</div>`;
                body.innerHTML = html;
        ...
        class Player extends Entity {
            ...
            update(dt, entities) {
                ...
                if (!this.isDodging && !this._isDashing) {
                    ...
                    if (isPistol) {
                        ...
                        if (isMelee && this._sprintDuration >= 1500 && !this._isDashing) {
                            // 冲刺攻击触发
            ...
            triggerDashAttack(entities) {
                ...
                EffectManager.add(new FloatingTextEffect(this.x, this.y - 30, '冲刺攻击！', '#4a9eff'));
            }
            updateDashAttack(dt, entities) {
                ...
                if (progress < 0.40) {
                    ...
                    const targetX = this._dashStartPos.x + this._dashDirection.x * dashDist * easedProgress;
                    const targetY = this._dashStartPos.y + this._dashDirection.y * dashDist * easedProgress;
                    const resolved = WallSystem.resolve(targetX, targetY, targetX, targetY, this.collisionRadius);
                    this.x = resolved.x; this.y = resolved.y;
                }
                if (this._dashState === 'slash') {
                    if (Game.showAttackRange && !this._dashRangeShown) {
                        ...
                        EffectManager.add(new AttackRangeEffect(this.x, this.y, attackAngle, 100, Math.PI, 'sector', 600));
                    }
                    const slashStart = 0.25 * totalMs;
            ...
            _checkDashHit(entities) {
                ...
                entities.forEach(entity => {
                    ...
                    if (MathUtils.pointInSector(entity.x, entity.y, this.x, this.y, attackAngle, range, arc)) {
                        ...
                        entity.applyKnockback(attackAngle, 15);
                        EffectManager.add(new HitEffect(entity.x, entity.y));
            ...
            renderWeapon(ctx) {
                ...
                // === 近战（剑等）渲染 ===
                    ...
                    if (this._isDashing) {
                        ...
                        if (dashProgress < 0.15) {
                            // charge: 武器后缩 0-15%
                            const t = dashProgress / 0.15;
                            dashOffset = s * 0.3 * easeInCubic(t);
                            dashAngle = 0;
                        } else if (dashProgress < 0.25) {
                            // rotate: 顺时针旋转90° 15-25%
                            const t = (dashProgress - 0.15) / 0.10;
                            dashOffset = s * 0.3 * (1 - easeOutQuad(t));
                            dashAngle = -Math.PI / 2 * easeInOutCubic(t);
                        } else if (dashProgress < 0.40) {
                            // slash: 快速挥砍180° 25-40%
                            const t = (dashProgress - 0.25) / 0.15;
                            dashAngle = -Math.PI / 2 + Math.PI * easeOutQuad(t);
                            dashOffset = -s * 0.1 * Math.sin(t * Math.PI);
                        } else {
                            // recover: 缓慢复位 40-100%
                            const t = (dashProgress - 0.40) / 0.60;
