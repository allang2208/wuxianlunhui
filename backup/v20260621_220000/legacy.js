...
class SlashEffect {
        ...
        const WEAPON_ANIM = {
            ...
            windupMs: 150, swingMs: 350, recoverMs: 200,
        };
                ...
                // === 近战（剑等）渲染 ===
                    ...
                    if (isAttacking) {
                        ...
                        if (anim.state === 'windup') {
                            ...
                            // 收回阶段：对调后使用快速、easeInCubic
                            thrustOffset = s * stab.stabDist * (1 - easeInCubic(t));
                        }
        ...
        const WeaponAnimConfig = {
            ...
            stab: {
                ...
                stabMs: 350,        // 刺击时间（ms）— 对调后：慢速
                recoverMs: 200,     // 收回时间（ms）— 对调后：快速
                windupDist: 0.35,   // 蓄力回退距离（倍率）
                stabDist: 0.9,      // 前刺距离（倍率）— 对调后：短距离
                recoverDist: 1.8,   // 收回幅度（倍率）— 对调后：长距离
                easeIn: easeInCubic,    // 蓄力缓动：前急后缓
                easeOut: easeOutQuad,   // 刺击缓动
                easeRecover: easeInCubic, // 收回缓动：对调后使用蓄力缓动
