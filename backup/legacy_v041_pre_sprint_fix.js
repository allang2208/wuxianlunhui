...
class SlashEffect {
        ...
        class Player extends Entity {
                ...
                if (isSprinting && !this._isDashing) {
                    ...
                    if (this._sprintDuration >= 750 && this.skills && this.skills.dashAttack) {
                        ...
                        if (this._dashConvergeTimer >= 400) {
                            ...
                            EffectManager.add(new DashConvergeEffect(this.x, this.y));
                        }
                    }
            ...
            triggerDashAttack(entities) {
                ...
                // 圆形金色浮动特效（触发时）
                EffectManager.add(new DashAuraEffect(this.x, this.y));
            ...
            updateDashAttack(dt, entities) {
                ...
                if (this._dashState === 'slash') {
                    if (Game.showAttackRange && !this._dashRangeShown) {
                        ...
                        EffectManager.add(new AttackRangeEffect(this._dashStartPos.x, this._dashStartPos.y, attackAngle, 175, Math.PI, 'sector', 600));
                    }
            ...
            _checkDashHit(entities) {
                ...
                const baseKnockback = this.attacks.melee.config.knockback || 8;
                const skillLevel = this.skills.dashAttack.level;
                const knockback = baseKnockback + 100 + skillLevel * 5;
                // 攻击范围 = 武器攻击范围 + 175 + 技能等级*5
                const baseRange = this.attacks.melee.config.range || 165;
                const range = baseRange + 175 + skillLevel * 5;
                // 角度 = 120度
                ...
                entities.forEach(entity => {
                    ...
                    if (MathUtils.pointInSector(entity.x, entity.y, this._dashStartPos.x, this._dashStartPos.y, attackAngle, range, arc)) {
                        ...
                        entity.applyKnockback(attackAngle, knockback);
                        EffectManager.add(new HitEffect(entity.x, entity.y));
                ...
                // === 近战（剑等）渲染 ===
                    ...
                    if (this._isDashing) {
                        ...
                        if (dashProgress < 0.2917) {
                            // 旋转阶段 0-350ms：从待机(0°) 顺时针旋转90° → 剑尖朝上
