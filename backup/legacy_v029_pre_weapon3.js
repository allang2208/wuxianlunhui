...
class SlashEffect {
        ...
        class HitEffect {
            constructor(x, y) {
                ...
                this.imageIndex = Math.random() < 0.5 ? 1 : 2;
                if (!HitEffect._images) {
                    HitEffect._images = [
                        null,
                        Object.assign(new Image(), { src: 'assets/effects/1.png' }),
                        Object.assign(new Image(), { src: 'assets/effects/2.png' })
                    ];
                }
            }
            update() {
            ...
            render(ctx) {
                const img = HitEffect._images[this.imageIndex];
                if (!img || !img.complete || img.naturalWidth === 0) return;
                const alpha = Math.min(1, this.life / 200);
                const screenPos = Renderer.worldToScreen(this.x, this.y);
                const size = 40 * (0.8 + (1 - this.life / this.maxLife) * 0.2);
                ctx.save();
                ctx.globalAlpha = alpha;
                ctx.translate(screenPos.x, screenPos.y);
                ...
                pt.entities.forEach(entity => {
                    ...
                    if (forward >= -entityRadius && forward <= range + entityRadius && 
                        ...
                        if (source && source.data && typeof source.data.str === 'number' && typeof source.data.dex === 'number') {
                            ...
                            const baseAtk = (currentWeapon && currentWeapon.weaponId === 'weapon2') ? 15 : 10;
        ...
        const SkillManager = {
            ...
            addMeleeExp(player, hitCount, killCount) {
                ...
                // 如果技能面板正在打开，同步刷新UI
                if (SystemUI.isOpen && SystemUI.currentTab === 'skill') {
        ...
        class Player extends Entity {
            ...
            switchWeaponMode() {
                ...
                if (currentItem && currentItem.equipImage) {
        ...
        const EquipManager = {
            TEST_EQUIPMENTS: {
                ...
                weapon2: { weaponId: 'weapon2', name: '骑士长剑', type: '单手剑', icon: '⚔', iconImage: 'assets/icons/knights_sword_v3_macro.png', equipImage: 'assets/weapons/knights_sword_v3_equip.png', category: 'weapon_melee', equipSlot: 'weapon2', stats: [{ name: '物理攻击', value: '18-23' }, { name: '攻速', value: '1.1' }], desc: '骑士团的标准制式长剑，剑身修长，锋利且坚韧', level: 5, rarity: 'rare', weaponType: 'sword' },
                ...
                function buildTooltip(item) {
                    ...
                    if (item.stats && item.stats.length > 0) {
                        statsHtml = item.stats.map(s => {
                            let value = s.value;
                            // weapon1/weapon2 动态计算攻击力
                            if ((item.weaponId === 'weapon1' || item.weaponId === 'weapon2') && s.name === '物理攻击' && self.player && self.player.data) {
                                const d = self.player.data;
                                const baseAtk = item.weaponId === 'weapon2' ? 15 : 10;
            ...
            renderDetail(item) {
                ...
                if (item.stats && item.stats.length) {
                    ...
                    item.stats.forEach(s => {
                        let value = s.value;
                        // weapon1/weapon2 动态计算攻击力
                        if ((item.weaponId === 'weapon1' || item.weaponId === 'weapon2') && s.label === '物理攻击' && Game.player && Game.player.data) {
