...
const Renderer = {
            ...
            generateWorld() { const cw = this.canvas.width || window.innerWidth || 1920, ch = this.canvas.height || window.innerHeight || 1080; this.canvas.width = cw; this.canvas.height = ch; CONFIG.VIEW_WIDTH = cw; CONFIG.VIEW_HEIGHT = ch; const sm = (typeof SceneManager !== 'undefined') ? SceneManager : ((typeof window !== 'undefined' && window.SceneManager) ? window.SceneManager : null); if (sm && sm.currentScene === 'main') { CONFIG.WORLD_WIDTH = 7650; CONFIG.WORLD_HEIGHT = 3800; } else { CONFIG.WORLD_WIDTH = cw * 4; CONFIG.WORLD_HEIGHT = ch * 4; } console.log('[WorldGen] canvasSize=' + cw + 'x' + ch + ', WORLD=' + CONFIG.WORLD_WIDTH + 'x' + CONFIG.WORLD_HEIGHT + ', scene=' + (sm ? sm.currentScene : 'none') + ', canvas=' + (this.canvas ? 'OK' : 'NULL')); this.terrainTexture = MapGenerator.generateTerrainTexture(CONFIG.WORLD_WIDTH, CONFIG.WORLD_HEIGHT); WallSystem.init(CONFIG.WORLD_WIDTH, CONFIG.WORLD_HEIGHT); },
