...
class SlashEffect {
        ...
        class Player extends Entity {
            ...
            _spawnWeaponGlowParticle(s) {
                ...
                // 粒子沿剑身分布：y 从剑柄(-s/2) 到 剑尖(s/2)，x 贴近剑刃两侧
                const bladeX = (Math.random() - 0.5) * w * 0.75;
                const bladeY = (Math.random() - 0.5) * s * 0.95;
                this.weaponGlowParticles.push({
                    x: bladeX,
                    y: bladeY,
                    vx: (Math.random() - 0.5) * 0.15,
                    vy: -0.25 - Math.random() * 0.35, // 缓慢上浮
            ...
            _updateWeaponGlow(dt, s) {
                if (Math.random() < 0.5) this._spawnWeaponGlowParticle(s);
                this.weaponGlowParticles.forEach(p => {
                    p.life -= dt;
                    p.y += p.vy;
                    p.x += p.vx + Math.sin((p.maxLife - p.life) * 0.006 + p.pulseOffset) * 0.2;
                    p.size *= 0.998;
                });
            ...
            _renderWeaponGlow(ctx) {
                ...
                this.weaponGlowParticles.forEach(p => {
                    ...
                    const fadeIn = Math.min(1, (1 - lifeRatio) * 3); // 淡入
                    const fadeOut = Math.min(1, lifeRatio * 2); // 淡出
                    const alpha = Math.min(fadeIn, fadeOut) * 0.5; // 最大透明度 50%
