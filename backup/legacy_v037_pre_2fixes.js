...
/** 处理拖放放下逻辑 */
            ...
            bindEquipTooltip() {
                ...
                function buildTooltip(item) {
                    ...
                    // 合并策略：优先使用 item 的数据，codexItem 补充缺失字段（解决背包与装备栏显示不一致）
                    const fullItem = { ...item };
                    if (codexItem) {
                        for (const key of Object.keys(codexItem)) {
                            const itemVal = fullItem[key];
                            const codexVal = codexItem[key];
                            if (itemVal === undefined || itemVal === null || itemVal === '') {
                                if (codexVal !== undefined && codexVal !== null && codexVal !== '') {
                                    fullItem[key] = codexVal;
                                }
                            }
                        }
                    }
                    ...
                    // 回退到图鉴硬编码数据
                    if (!attackParams && fullItem.attack) {
