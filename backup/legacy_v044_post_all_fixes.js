...
class SlashEffect {
        ...
        class DashSlashEffect {
            constructor(x, y, angle, arc, range) {
                this.x = x; this.y = y;
                this.angle = angle; this.arc = arc; this.range = range;
                this.life = 500; this.maxLife = 500; this.active = true;
            }
            update() {
                this.life -= 16;
                if (this.life <= 0) this.active = false;
            ...
            render(ctx) {
                ...
                for (let i = 0; i <= 20; i++) {
                    const a = this.angle - halfArc + (this.arc * i / 20);
                    const r = this.range * (0.5 + progress * 0.5);
        ...
        const SkillManager = {
            ...
            renderSkillDetail(skill) {
                ...
                if (skill.id === 'swordMastery') {
                    ...
                    const knockbackDist = baseKnockback + 100 + skill.level * 5;
                    const triggerTime = 333 * (1 - (skill.level - 1) * 0.03);
                    const baseRange = 165; // 默认武器攻击范围
                    const attackRange = baseRange + 25 + skill.level * 5;
                    ...
                    if (nextEffect) {
                        ...
                        const nextKnockback = baseKnockback + 100 + (skill.level + 1) * 5;
                        const nextTrigger = 333 * (1 - skill.level * 0.03);
                        const nextRange = baseRange + 25 + (skill.level + 1) * 5;
        ...
        class Player extends Entity {
            ...
            update(dt, entities) {
                ...
                if (isSprinting && !this._isDashing) {
                    ...
                    const dashLevel = (this.skills && this.skills.dashAttack && this.skills.dashAttack.level) || 1;
                    const triggerTime = 333 * (1 - (dashLevel - 1) * 0.03);
                    // 持续触发金光汇聚特效（当满足条件时，每400ms触发一次）
                    if (this._sprintDuration >= triggerTime && this.skills && this.skills.dashAttack) {
            ...
            updateDashAttack(dt, entities) {
                ...
                if (progress < 0.40) {
                    ...
                    // 检测是否撞墙（被阻挡）
                    const hitWallX = Math.abs(resolved.x - targetX) > 1;
                    const hitWallY = Math.abs(resolved.y - targetY) > 1;
                    if ((hitWallX || hitWallY) && !this._dashBounceApplied) {
                        // 撞墙弹回：向反方向弹回30%距离，形成缓和效果
                        ...
                        const bounceResolved = WallSystem.resolve(this.x, this.y, bounceX, bounceY, this.collisionRadius);
                        this.x = bounceResolved.x; this.y = bounceResolved.y;
                        // 撞墙烟雾效果
                        EffectManager.add(new SmokeEffect(resolved.x, resolved.y));
                    } else {
                        this.x = resolved.x; this.y = resolved.y;
                    }
                    // 如果完全无法移动（被墙完全阻挡且已弹回），提前结束冲刺
                    if (Math.abs(resolved.x - this._dashStartPos.x) < 1 && Math.abs(resolved.y - this._dashStartPos.y) < 1 && progress > 0.1 && this._dashBounceApplied) {
                        this._isDashing = false;
                        this._dashState = 'idle';
                        this._dashTimer = 0;
                ...
                if (this._dashState === 'slash') {
                    ...
                    // 首次进入挥砍阶段时，添加白色扇形特效
                    if (this._dashTimer >= slashStart && this._dashTimer < slashStart + dt + 5) {
                        const attackAngle = Math.atan2(this._dashDirection.y, this._dashDirection.x);
                        const skillLevel = (this.skills && this.skills.dashAttack && this.skills.dashAttack.level) || 1;
                        const baseRange = (this.attacks.melee && this.attacks.melee.config && this.attacks.melee.config.range) || 165;
                        const range = baseRange + 25 + skillLevel * 5;
                        const arc = 2 * Math.PI * 4 / 3; // 240°（增加33%幅度）
                        EffectManager.add(new DashSlashEffect(this.x, this.y, attackAngle, arc, range));
                    }
            ...
            _checkDashHit(entities) {
                ...
                const baseRange = (currentItem && currentItem.attack && currentItem.attack.range) 
                    ...
                    || 165;
                const range = baseRange + 25 + skillLevel * 5;
                // 角度 = 240°（动画幅度增加33%，命中判定保持120°）
                const arc = 2 * Math.PI * 4 / 3; // 240° 用于动画特效
                const hitArc = 2 * Math.PI / 3; // 120° 用于实际命中判定
                entities.forEach(entity => {
                    if (entity === this || !entity.active || !entity.hittable) return;
                    if (this._dashHitSet.has(entity)) return;
                    if (MathUtils.pointInSector(entity.x, entity.y, this._dashStartPos.x, this._dashStartPos.y, attackAngle, range, hitArc)) {
                ...
                // === 近战（剑等）渲染 ===
                    ...
                    if (this._isDashing) {
                        ...
                        if (dashProgress < 0.2917) {
                            // 旋转阶段 0-350ms：从待机(0°) 逆时针旋转90° → 剑尖朝下，同时武器前移15px
                            const t = dashProgress / 0.2917;
                            dashAngle = Math.PI / 2 * easeInOutCubic(t);
                            dashOffset = 15; // 前移15px（反转方向）
                        } else if (dashProgress < 0.6667) {
                            // 挥砍阶段 350-800ms
                            const t = (dashProgress - 0.2917) / 0.375;
                            if (t < 0.133) {
                                // 前50ms：在旋转结束后的后移45px基础上，继续后移30px
                                const pt = t / 0.133;
                                dashOffset = -45 - 30 * easeOutQuad(pt); // 后移45px + 30px
                                dashAngle = Math.PI / 2;
                            } else {
                                // 后350ms：以主角为中心，从朝下扇形旋转240°（180° * 1.33）到朝上
                                const pt = (t - 0.133) / 0.867;
                                dashAngle = Math.PI / 2 - Math.PI * 4/3 * easeOutQuad(pt); // 240°挥砍
                                dashOffset = -45 - 30 * (1 - easeInOutCubic(pt)); // 在-45px基础上收回30px
                            }
                        } else {
