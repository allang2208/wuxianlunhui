...
class SlashEffect {
        ...
        const SkillManager = {
            ...
            addMeleeExp(player, hitCount, killCount) {
                ...
                // 升级提示文字：显示技能图标 + 技能名称 + 升级效果
                const effect = sm.getEffect(sm.level);
                const text = document.createElement('div');
                text.className = 'level-up-text';
                text.innerHTML = `
                    <span class="lu-icon">${sm.icon}</span>
                    <span class="lu-title">${sm.name} 升级！Lv.${sm.level}</span>
                    <span class="lu-effect">剑攻击+${effect.atkBonus}  冷却-${(effect.cooldownReduction * 100).toFixed(0)}%  敏捷+${effect.dexBonus}</span>
                `;
                document.body.appendChild(text);
                setTimeout(() => { if (text && text.parentNode) text.remove(); }, 2500);
                // 更新攻击冷却（应用新的冷却缩减）和动画同步
            ...
            renderSkillGrid() {
                ...
                const expPercent = sm.level >= sm.maxLevel ? 100 : Math.min(100, (sm.exp / sm.maxExp) * 100);
                card.innerHTML = `
                    <div class="skill-icon">${sm.icon}</div>
                    <div class="skill-name">${sm.name}</div>
                    <div class="skill-level">Lv.${sm.level} / ${sm.maxLevel}</div>
                    <div class="skill-exp-bar"><div class="skill-exp-fill" style="width:${expPercent}%"></div></div>
                `;
                card.onclick = () => this.renderSkillDetail(sm);
                ...
                body.innerHTML = `
                    <div class="sd-section">
                        <h4>技能效果</h4>
                        <div class="sd-stat-row"><span class="sd-stat-name">当前等级</span><span class="sd-stat-val">Lv.${skill.level}</span></div>
                        <div class="sd-stat-row"><span class="sd-stat-name">剑攻击加成</span><span class="sd-stat-val pos">+${effect.atkBonus}</span></div>
                        <div class="sd-stat-row"><span class="sd-stat-name">敏捷加成</span><span class="sd-stat-val pos">+${effect.dexBonus}</span></div>
                        <div class="sd-stat-row"><span class="sd-stat-name">剑类攻击间隔缩短</span><span class="sd-stat-val pos">${(effect.cooldownReduction * 100).toFixed(0)}%</span></div>
                        ${nextEffect ? `
                        <div class="sd-stat-row" style="margin-top:8px;border-top:1px solid rgba(100,160,255,0.2);padding-top:8px;">
                            <span class="sd-stat-name">下一级攻击加成</span><span class="sd-stat-val pos">+${nextEffect.atkBonus}</span>
                        </div>
                        <div class="sd-stat-row"><span class="sd-stat-name">下一级敏捷加成</span><span class="sd-stat-val pos">+${nextEffect.dexBonus}</span></div>
                        <div class="sd-stat-row"><span class="sd-stat-name">下一级剑类攻击间隔缩短</span><span class="sd-stat-val pos">${(nextEffect.cooldownReduction * 100).toFixed(0)}%</span></div>
                        ` : '<div class="sd-stat-row" style="margin-top:8px;color:#7a9a6a;">已达到最高等级</div>'}
                    </div>
                    <div class="sd-section">
                        <h4>升级进度</h4>
                        <div class="sd-exp-track">
                            <div class="sd-exp-bar"><div class="sd-exp-fill" style="width:${expPercent}%"></div></div>
                            <span class="sd-exp-text">${skill.exp}/${skill.maxExp}</span>
                        </div>
                        <p style="margin-top:8px;color:#a0a0a0;font-size:12px;">${skill.level >= skill.maxLevel ? '已满级' : `还需 ${skill.maxExp - skill.exp} 点经验升级`}</p>
                    </div>
                    <div class="sd-section">
                        <h4>升级方式</h4>
                        <p>• 每次击中敌人积累 1 点经验（多敌人=多倍）</p>
                        <p>• 同时攻击到两个以上敌人时，额外获得 3 点经验</p>
                        <p>• 每次击杀目标增加 10 点经验</p>
                    </div>
                `;
                ...
                if (backBtn) {
                    backBtn.onclick = () => { 
                        ...
                        this.renderSkillGrid(); // 返回时刷新网格，确保数据最新
                    };
                }
            }
        };
        ...
        const ItemDatabase = {
            items: {
                ...
                rune_sword: {
                    ...
                    stats: [{ name: '物理攻击', value: '45-55' }, { name: '攻速', value: '1.1' }, { name: '暴击率', value: '+5%', pos: true }],
                ...
                knights_sword: {
                    ...
                    stats: [{ name: '物理攻击', value: '18-23' }, { name: '攻速', value: '1.1' }],
        ...
        const EquipManager = {
            TEST_EQUIPMENTS: {
                ...
                weapon: { weaponId: 'weapon1', name: '生锈的长剑', type: '单手剑', icon: '⚔', iconImage: 'assets/icons/1-rusty_sword_macro.png', equipImage: 'assets/weapons/1-rusty_sword_euip.png', category: 'weapon_melee', equipSlot: 'weapon', stats: [{ name: '物理攻击', value: '12-18' }, { name: '攻速', value: '1.2' }, { name: '暴击率', value: '+3%', pos: true }], desc: '一把锈迹斑斑的旧剑', level: 1, rarity: 'common', weaponType: 'sword' },
                ...
                weapon2: { weaponId: 'weapon3', name: '训练用弓', type: '弓', icon: '🏹', iconImage: 'assets/icons/bow_icon.png', category: 'weapon_ranged', rarity: 'common', level: 1, weaponCategory: 'mainhand', weaponType: 'bow', weaponAsset: { framePrefix: 'assets/weapons/bow_frame_', frameCount: 8, framePad: 2 }, stats: [{ name: '物理攻击', value: '8-14' }, { name: '攻速', value: '1.0' }, { name: '射程', value: '600' }], desc: '一把简陋的弓，勉强能射出箭，适合初学者练习', equipSlot: 'weapon2' },
...
/** 处理拖放放下逻辑 */
            ...
            bindEquipTooltip() {
                ...
                function buildTooltip(item) {
                    ...
                    if (fullItem.stats && fullItem.stats.length > 0) {
                        statsHtml = fullItem.stats.map(s => {
                            let value = s.value;
                            // weapon1/weapon2/weapon4 动态计算攻击力
                            if ((fullItem.weaponId === 'weapon1' || fullItem.weaponId === 'weapon2' || fullItem.weaponId === 'weapon4') && s.name === '物理攻击' && self.player && self.player.data) {
                                const d = self.player.data;
                                let baseAtk = 10;
                                if (fullItem.weaponId === 'weapon2') baseAtk = 23;
                                else if (fullItem.weaponId === 'weapon4') baseAtk = 40;
                                value = Math.round(baseAtk + d.str * 0.05 + d.dex * 0.1);
                            }
                            return `<div class="tt-stat"><span class="tt-stat-name">${s.name}</span><span class="tt-stat-val ${s.pos ? 'pos' : ''}">${value}</span></div>`;
                        }).join('');
                    }
                    ...
                    if (fullItem.attack) {
                        ...
                        if (fullItem.attack.range) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">攻击距离</span><span class="tt-stat-val">${fullItem.attack.range}px</span></div>`;
                        if (fullItem.attack.attackInterval) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">攻击间隔</span><span class="tt-stat-val">${fullItem.attack.attackInterval}ms</span></div>`;
                        if (fullItem.attack.hitType) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">命中类型</span><span class="tt-stat-val">${fullItem.attack.hitType}</span></div>`;
                        if (fullItem.attack.damageType) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">伤害类型</span><span class="tt-stat-val">${fullItem.attack.damageType}</span></div>`;
                    }
                    // 武器动画参数
                    if (fullItem.animation) {
                ...
                function positionTooltip(e) {
                    ...
                    if (top + 280 > window.innerHeight) top = e.clientY - 280;
                    if (top < 10) top = 10;
            ...
            bindInventoryTooltip() {
                ...
                function buildTooltip(item) {
                    ...
                    ttType.textContent = item.type + (item.rarity ? ` | ${item.rarity}` : '') + (item.level ? ` | Lv.${item.level}` : '');
                    let statsHtml = '';
                    if (item.stats && item.stats.length > 0) {
                        statsHtml = item.stats.map(s => {
                            let value = s.value;
                            // weapon1/weapon2/weapon3/weapon4 动态计算攻击力
                            if ((item.weaponId === 'weapon1' || item.weaponId === 'weapon2' || item.weaponId === 'weapon3' || item.weaponId === 'weapon4') && s.name === '物理攻击' && self.player && self.player.data) {
                                const d = self.player.data;
                    ...
                    ttStats.innerHTML = statsHtml;
                    let extraHtml = '';
                    if (item.category) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">分类</span><span class="tt-stat-val">${item.category}</span></div>`;
                    if (item.weaponType) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">武器类型</span><span class="tt-stat-val">${item.weaponType}</span></div>`;
                    if (item.equipSlot) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">装备槽位</span><span class="tt-stat-val">${item.equipSlot}</span></div>`;
                    if (item.weaponId) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">武器ID</span><span class="tt-stat-val">${item.weaponId}</span></div>`;
                    if (item.weaponCategory) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">武器分类</span><span class="tt-stat-val">${item.weaponCategory}</span></div>`;
                    if (item.stack > 1) extraHtml += `<div class="tt-extra-row"><span class="tt-stat-name">堆叠数量</span><span class="tt-stat-val">${item.stack}</span></div>`;
                    ttExtra.innerHTML = extraHtml;
        ...
        const CodexManager = {
            ...
            renderDetail(item) {
                ...
                if (item.category && item.category.includes('weapon')) {
                    ...
                    if (item.weaponType) html += this.detailRow('武器类型', item.weaponType);
        ...
        const Game = {
            ...
            update(dt) {
                ...
                // 鼠标悬停自动拾取：鼠标移动到装备上触发金色特效时，且在150px范围内，自动拾取
                let hoverPickup = false;
                this.entities.forEach((entity, key) => {
                    if (hoverPickup) return;
                    if (entity instanceof DropItem && entity.active) {
                        // 检查玩家与装备距离是否 <= 150px
