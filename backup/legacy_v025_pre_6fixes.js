...
class SlashEffect {
            ...
            checkTriangleHit(source) {
                ...
                pt.entities.forEach(entity => {
                    ...
                    if (forward >= -entityRadius && forward <= range + entityRadius && 
                        ...
                        // weapon1 攻击力 = 10 + 力量*0.05 + 敏捷*0.1，四舍五入
                        if (source && source.data && typeof source.data.str === 'number' && typeof source.data.dex === 'number') {
                            baseDamage = Math.round(10 + source.data.str * 0.05 + source.data.dex * 0.1);
                        }
        ...
        class Player extends Entity {
            constructor(x, y) {
                ...
                this.attacks = {
                    melee: new ThrustAttack({ cooldown: 500, range: 150, width: 35, damage: { min: 12, max: 20 }, knockback: 8 }),
...
/** 处理拖放放下逻辑 */
                ...
                function buildTooltip(item) {
                    ...
                    if (item.stats && item.stats.length > 0) {
                        statsHtml = item.stats.map(s => {
                            let value = s.value;
                            // weapon1 动态计算攻击力：10 + 力量*0.05 + 敏捷*0.1
                            if (item.weaponId === 'weapon1' && s.name === '物理攻击' && self.player && self.player.data) {
                                const d = self.player.data;
                                value = Math.round(10 + d.str * 0.05 + d.dex * 0.1);
                            }
            ...
            renderDetail(item) {
                ...
                if (item.stats && item.stats.length) {
                    ...
                    item.stats.forEach(s => {
                        let value = s.value;
                        // weapon1 动态计算攻击力：10 + 力量*0.05 + 敏捷*0.1
                        if (item.weaponId === 'weapon1' && s.label === '物理攻击' && Game.player && Game.player.data) {
        ...
        const Game = {
            ...
            pickupNearbyItems() {
                ...
                const range = 25; // 半径25px，直径50px的圆
                let pickedCount = 0;
                this.entities.forEach((entity, key) => {
                    if (entity instanceof DropItem && entity.active) {
                        const dx = entity.x - px, dy = entity.y - py;
