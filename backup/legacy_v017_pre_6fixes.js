...
class SlashEffect {
        ...
        class AttackRangeEffect {
            ...
            render(ctx) {
                const alpha = Math.min(1, this.life / (this.maxLife * 0.75)) * 0.5;
                const screenPos = Renderer.worldToScreen(this.x, this.y);
                ctx.save();
                ctx.translate(screenPos.x, screenPos.y);
                ctx.globalAlpha = alpha;
                ...
                if (this.type === 'sector') {
                    ...
                    // 三角形攻击范围：顶点在玩家位置，底边在range处
                    const halfW = this.width / 2;
                    const a = this.angle;
                    const v1x = 0, v1y = 0;
                    const v2x = Math.cos(a - Math.atan2(halfW, this.range)) * this.range;
                    const v2y = Math.sin(a - Math.atan2(halfW, this.range)) * this.range;
                    const v3x = Math.cos(a + Math.atan2(halfW, this.range)) * this.range;
                    const v3y = Math.sin(a + Math.atan2(halfW, this.range)) * this.range;
                    ctx.beginPath();
                    ctx.moveTo(v1x, v1y);
                    ctx.lineTo(v2x, v2y);
                    ctx.lineTo(v3x, v3y);
        ...
        class ThrustAttack extends Attack {
            ...
            execute(source, targetX, targetY, entities) {
                ...
                // 存储攻击数据，供swing阶段进行三角形攻击判定
                source._pendingThrust = {
                    range: this.config.range,      // 125px
                    width: this.config.width,      // 25px
                    angle: attackAngle,
            ...
            checkTriangleHit(source) {
                ...
                const halfW = width / 2;
                // 三角形三个顶点：玩家位置（顶点）、底边左右
                const v1x = source.x, v1y = source.y;
                const v2x = source.x + Math.cos(angle - Math.atan2(halfW, range)) * range;
                const v2y = source.y + Math.sin(angle - Math.atan2(halfW, range)) * range;
                const v3x = source.x + Math.cos(angle + Math.atan2(halfW, range)) * range;
                const v3y = source.y + Math.sin(angle + Math.atan2(halfW, range)) * range;
                ...
                pt.entities.forEach(entity => {
                    ...
                    // 三角形命中判定
                    if (MathUtils.pointInTriangle(entity.x, entity.y, v1x, v1y, v2x, v2y, v3x, v3y)) {
                        pt.hitSet.add(entity);
                        hitCount++;
                        const baseDamage = Math.floor(MathUtils.randomRange(pt.damage.min, pt.damage.max));
        ...
        const SkillManager = {
            ...
            onLevelUp(player, sm) {
                ...
                text.innerHTML = `
                    ...
                    <span class="lu-effect">剑攻击+${effect.atkBonus}  冷却-${(effect.cooldownReduction * 100).toFixed(0)}%</span>
                `;
            ...
            updateMeleeCooldown(player) {
                ...
                const sm = player.skills.swordMastery;
                const effect = sm.getEffect(sm.level);
                const baseCooldown = 1100;
                const reducedCooldown = baseCooldown * (1 - effect.cooldownReduction);
                player.attacks.melee.maxCooldown = Math.max(300, reducedCooldown);
            },
            renderSkillGrid() {
        ...
        class Player extends Entity {
            constructor(x, y) {
                ...
                this.attacks = {
                    melee: new ThrustAttack({ cooldown: 1100, range: 125, width: 25, damage: { min: 12, max: 20 }, knockback: 8 }),
            ...
            updateEquipSlots() {
                ...
                document.querySelectorAll('.diablo-slot').forEach(slot => {
                    ...
                    if (item) {
                        ...
                        iconEl.textContent = slot.querySelector('.slot-icon').dataset.default || '⚔';
            ...
            bindEquipTooltip() {
                ...
                function buildTooltip(item) {
                    ...
                    if (item.stats && item.stats.length > 0) {
                        statsHtml = item.stats.map(s => `<div class="tt-stat"><span class="tt-stat-name">${s.name}</span><span class="tt-stat-val ${s.pos ? 'pos' : ''}">${s.value}</span></div>`).join('');
                    }
