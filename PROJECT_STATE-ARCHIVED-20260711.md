...
> ⚠️ 本文件已停止维护（最后更新 2026-07-11，V0.198）。
> 项目现状——版本号、待办、已完成功能——一律以 [CHANGELOG.md](CHANGELOG.md) 与 `git log` 为准，请勿再据此判断当前状态。
...
## 原工作准则（已随文档归档，仅供参考）
1. 每次修改前运行 `node scripts/backup.js`
2. 修改后构建验证
3. 不要重复做"已完成"列表中的功能
4. 不要删除任何现有功能
5. 最小修改：只动目标功能
6. 版本号在 build 时自动递增（运行 `npm run build`）
7. Git 已跟踪所有 src 文件，可回滚到任意提交
